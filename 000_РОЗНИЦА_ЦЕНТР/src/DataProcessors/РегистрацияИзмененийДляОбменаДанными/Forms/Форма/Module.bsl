...
Процедура СписокНаборовЗаписейВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ПараметрыЗаписи <> Неопределено Тогда
		ОткрытьФорму(ПараметрыЗаписи.ИмяФормы, Новый Структура(ПараметрыЗаписи.Параметр, ПараметрыЗаписи.Значение), ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура СообщитьОРезультатахРегистрации(Результаты)
	...
	Иначе
		...
		Попытка
			...
			СсылкаНавигации = ПолучитьНавигационнуюСсылку(УзелОбменаСсылка);
		
		Исключение

			СсылкаНавигации = Неопределено;
...
Процедура ПрочитатьДеревоМетаданных()
	...
	СтруктураДанных = ЭтотОбъект().СформироватьСтруктуруМетаданных(УзелОбменаСсылка);

//	Удаляем строки, которые нельзя редактировать.

	МетаДерево = СтруктураДанных.Дерево;
	...
	СтруктураАвторегистрацииМетаданных = СтруктураДанных.СтруктураАвторегистрации;
	СтруктураПредставленийМетаданных   = СтруктураДанных.СтруктураПредставлений;
	СтруктураИменМетаданных            = СтруктураДанных.СтруктураИмен;
...
Функция СтруктураКлючаНабораЗаписей(Знач ТекущиеДанные)
	...
	ИменаКлючей = МассивИменКлючейНабораЗаписей(СписокНаборовЗаписейИмяТаблицы);
	Для Каждого Имя Из ИменаКлючей Цикл
		Попытка	//	LNK 12.07.2019 11:28:26
	...
	Если Измерения.Свойство("Регистратор") Тогда
		...
		Иначе
			...
			Результат.ИмяФормы = СписокНаборовЗаписейИмяТаблицы + ".ФормаСписка";	//	LNK 12.07.2019 11:59:56

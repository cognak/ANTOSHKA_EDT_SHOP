...
//
Функция СформироватьСтруктуруМетаданных(ИмяПланаОбмена = Неопределено, РежимПредставления = 0) Экспорт
	
//	LNK 23.03.2024 11:14:11
//	РежимПредставления = 0	- представление, то есть Синоним
//	РежимПредставления = 1	- Имя (Имя метаданных)
//	РежимПредставления = 2	- Синоним (Имя)
//	РежимПредставления > 2	- Имя (Синоним)
...
//	Параметры:
	...
	ТекПараметры.Вставить("РежимПредставления"		, РежимПредставления);
	...
	Иначе	ПланОбмена = ИмяПланаОбмена.Метаданные();
...
Процедура СформироватьУровеньМетаданных(ТекущийНомерСтроки, Параметры, ИндексКартинки, ИндексКартинкиУзлов, ДобавлятьПодчиненные, ИмяМета, ПредставлениеМета)
	...
	Для Каждого Мета Из Метаданные[ИмяМета] Цикл
		...
		//	Без учета плана обмена
			...
			Наименование    = МетаПредставление(Мета, Параметры.РежимПредставления);
		...
		Иначе
			...
			Если НЕ Элемент = Неопределено И ПравоДоступа("Чтение", Мета) Тогда
...
Функция МетаПредставление(Мета, РежимПредставления)
...
//	РежимПредставления > 2	- Имя (Синоним)
	...
	Если РежимПредставления = 0 Тогда

		Представление = Мета.Представление();

	ИначеЕсли РежимПредставления = 1 Тогда

		Представление = Мета.Имя;

	ИначеЕсли РежимПредставления = 2 Тогда

		Представление = Мета.Представление() + " (" + Мета.Имя + ")";

	Иначе

		Представление = Мета.Имя + " (" + Мета.Представление() + ")";

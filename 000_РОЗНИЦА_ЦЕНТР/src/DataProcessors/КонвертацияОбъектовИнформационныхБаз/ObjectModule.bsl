...
//	LNK 11.09.2017 08:24:43
	ОбменДаннымиСервер.УдалитьРегистрациюВТестовомУзле(УзелДляОбмена, Истина);
...
//	LNK 04.09.2017 13:36:08
	КонфигурацииИдентичны = ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(УзелОбменаЗагрузкаДанных) = "ПоМагазинуКонвертация";
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Попытка
		...
		Если  Объект.ДополнительныеСвойства.Свойство("СоответствияОбъектовИнформационныхБаз", СоответствияОбъектовИнформационныхБаз)
			...
			Если ОбменДаннымиПовтИсп.ПланОбменаСодержитОбъект(ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(СоответствияОбъектовИнформационныхБаз.УзелИнформационнойБазы), "РегистрСведений.СоответствияОбъектовИнформационныхБаз") Тогда
...
Процедура ОтменитьПроведениеОбъектаВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если КонфигурацииИдентичны Тогда

	//	LNK 04.09.2017 13:38:54
		Возврат;
...
//	в режиме сопоставления для основных объектов замену не выполняем
	Если КонфигурацииИдентичны ИЛИ РежимПоискаОсновногоОбъекта И РежимЗагрузкиДанныхВТаблицуЗначений() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьДокументВБезопасномРежиме(Документ, ТипОбъекта)
	...
	Если Документ.Проведен И НЕ КонфигурацииИдентичны Тогда
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	ИмяРегистра = СтрЗаменить(ИмяРегистра, "РегистрНакопленияЗапись.", "");	//	LNK 04.09.2017 14:10:55
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Отбор" Тогда
			...
			//	Объект = РегистрыСведений[ИмяРегистра].СоздатьНаборЗаписей();
				Объект = СтруктураСвойств.Менеджер.СоздатьНаборЗаписей();
	...
	//	ЕстьКоллизия = ПроверитьНаличиеКоллизийДляОбъекта(Объект, "РегистрСведений", ПриоритетОбъектаОбмена, ИмяРегистра);
		ЕстьКоллизия = ПроверитьНаличиеКоллизийДляОбъекта(Объект, СтруктураСвойств.ИмяТипа, ПриоритетОбъектаОбмена, ИмяРегистра);
...
Функция ПроверитьНаличиеКоллизийДляОбъекта(Объект, ИмяТипаОбъекта, ПриоритетОбъектаОбмена,
	...
	//	LNK 05.09.2017 20:13:14 - мы на периферии! Какие коллизии? Приоритет ЦБ.
		Возврат Ложь;
...
Функция ПолучитьДанныеОбъектаСтрокойДоИзменения(Объект, ДополнительнаяИнформация, ИмяТипаОбъекта)
	...
	ИначеЕсли ИмяТипаОбъекта = "РегистрСведений" ИЛИ ИмяТипаОбъекта = "РегистрНакопления" Тогда
		
		Если ИмяТипаОбъекта = "РегистрНакопления" Тогда

				МенеджерТипа = РегистрыНакопления;

		Иначе	МенеджерТипа = РегистрыСведений;

		КонецЕсли;

		СтарыйНаборЗаписей = МенеджерТипа[ДополнительнаяИнформация].СоздатьНаборЗаписей();
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Попытка
						...
						Если ЗаписатьОбъект Тогда
							...
							ИЛИ (РежимЗаписи = РежимЗаписиДокумента.Проведение И КонфигурацииИдентичны)
							ИЛИ (РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения И КонфигурацииИдентичны)	Тогда
							...
							ИначеЕсли РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
								...
								Если ЗаписатьОбъект
									...
									И Объект.Ссылка <> Неопределено
									И НЕ КонфигурацииИдентичны	Тогда
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Если ЗаписатьОбъект Тогда
						...
						Если ЗаписатьОбъект
							И НЕ КонфигурацииИдентичны
							И НЕ ИмяТипаОбъекта = "Константы"	//	LNK 05.09.2017 20:23:36 - нет у константы ссылки... вот жеж косяков в БСП!!!
							И Объект <> Неопределено

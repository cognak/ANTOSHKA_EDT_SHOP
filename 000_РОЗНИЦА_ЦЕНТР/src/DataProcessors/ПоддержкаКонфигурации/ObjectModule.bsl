...
Функция ПолучитьТекстСкриптаНовый(ПараметрыОбновления)
	...
	Возврат ВставитьПараметрыСкрипта(ПолучитьМакет("ОбновлениеНовое").ПолучитьТекст(), ПараметрыОбновления);
...
//	Определение пути к информационной базе.
	...
	Результат = СтрЗаменить(Результат, "[КаталогОбновления]"					, ПараметрыОбновления.ФайлОбновленияОбъект.Путь);

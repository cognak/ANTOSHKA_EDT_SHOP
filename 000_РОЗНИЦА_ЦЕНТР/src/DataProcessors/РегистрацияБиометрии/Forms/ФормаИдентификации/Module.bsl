...
&НаСервере
Функция НайтиТелефонКонтрагентаСотрудника(Сотрудник)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	               ...
	               |ГДЕ
	               |	КонтрагентыКонтактнаяИнформация.Ссылка.ФизЛицо = &ФизЛицо
...
Процедура УстановитьНомерТелефона()
	...
	Если СтрокаТелефон = Неопределено Тогда 
		//2025-09-25 sa поищем телефон в контрагенте привязанного к этому физлицу
		НомерТелефона = НайтиТелефонКонтрагентаСотрудника(Сотрудник);

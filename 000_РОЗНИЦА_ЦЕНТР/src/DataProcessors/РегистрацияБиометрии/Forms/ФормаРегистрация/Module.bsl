...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда
		Сообщить("Не вдалося визначити співробітника для якого реєструється біометрія!");
...
Процедура РегистрацияОтпечаткаХ64() Экспорт     
	...
	Если ((ПоследнийОтпечаток <> ПредпоследнийОтпечаток) И (ЗначениеЗаполнено(ПоследнийОтпечаток))) Тогда  
		...
		Иначе          
			...
			ЭтаФорма.Информация = "Пересадка пальця? Дані вже зареєстровані за іншим співробітником!";			
		КонецЕсли;
	КонецЕсли;
...
КонецПроцедуры
 ...
 Процедура СохранитьОтпечатокВБазе(Отпечаток) 
	//2025-09-15 sa дополнительная проверка что сотрудник указан
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда
		Сообщить("Не вдалося зберегти відбиток у базі даних. Не вказано співробітника для якого реєструється біометрія!");
		ЭтаФорма.Закрыть(Новый Структура("Успешно",Ложь));
	КонецЕсли;
	
	Если СохранитьОтпечатокВБазеСервер(Отпечаток) Тогда
		Сообщить("Відбиток збережено у базі даних!");
		ЭтаФорма.Закрыть(Новый Структура("Успешно",Истина));
	Иначе
		Сообщить("Не вдалося зберегти відбиток у базі даних");
		ЭтаФорма.Закрыть(Новый Структура("Успешно",Ложь));
	КонецЕсли;
КонецПроцедуры

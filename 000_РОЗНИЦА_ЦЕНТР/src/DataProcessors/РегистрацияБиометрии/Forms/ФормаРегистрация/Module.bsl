...
Процедура ПодключитьУстройство()
	...
	Иначе  
		...
		Успешно = МенеджерОборудованияКлиент.ВыполнитьДополнительнуюКоманду("ПодключитьУстройство",ВходныеПараметры,ВыходныеПараметры,ИдентификаторУстройства,СтруктураПараметров.Параметры);
		Если Успешно Тогда
...
&НаКлиенте
Процедура ПодключитьУстройство2()
	ОписаниеОшибки = "";
...
Процедура РегистрацияОтпечаткаХ64() Экспорт     
	...
	ЭтаФорма.Информация = "Приложите палец к сенсору. Попытка №"+(НомерРегистрации+1)+" из 3"; 
	...
	Если ((ПоследнийОтпечаток <> ПредпоследнийОтпечаток) И (ПоследнийОтпечаток <> "")) Тогда  
		...
		Иначе          
			...
			ЭтаФорма.Информация = "Пересадка пальца?"+Символы.ПС+"Данные уже зарегистрированы за другим сотрудником!";

...
Процедура ЗавершающиеДействияПриИзмененииСервер()
	Если ТипЗнч(РезультатФоновогоЗадания) = Тип("Структура") И РезультатФоновогоЗадания.Свойство("ИдентификаторЗадания") Тогда
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	Если Не Объект.ИзменятьВТранзакции Или Не ПодсистемаСуществует("СтандартныеПодсистемы.БазоваяФункциональность") Тогда
		...
		Возврат Новый Структура("Статус, АдресРезультата", "Выполнено", АдресХранилища);
	КонецЕсли;
	...
	РезультатФоновогоЗадания = МодульДлительныеОперации.ВыполнитьВФоне(ИмяПроцедуры, ПараметрыЗадания, ПараметрыВыполнения);

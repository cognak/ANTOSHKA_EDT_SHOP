...
Функция ПодготовитьСтруктуруДанных(СтруктураНастроек) Экспорт
	...
	Если СтруктураНастроек.ИсходныеДанные <> Неопределено Тогда
		...
		|ВЫРАЗИТЬ(NULL КАК СТРОКА(1)) КАК Маржинальность,		
		|ЗНАЧЕНИЕ(Перечисление.ТипСкидкиНаценки.ПустаяСсылка) КАК ТипСкидкиНаценки,
		|ЗНАЧЕНИЕ(Справочник.СкидкиНаценки.ПустаяСсылка) КАК СкидкаНаценка,
		|ЗНАЧЕНИЕ(Перечисление.ТипыПереоценок.ПустаяСсылка) КАК ТипПереоценки,
		|ВЫРАЗИТЬ(NULL КАК СТРОКА(100)) КАК ПричинаУценки,
		...
		КонецЕсли;
		Если СтруктураНастроек.ИсходныеДанные.Колонки.Найти("Маржинальность") = Неопределено Тогда

			СтруктураНастроек.ИсходныеДанные.Колонки.Добавить("Маржинальность", Новый ОписаниеТипов("Строка"));
							...
							Если НаименованиеКолонки <> Неопределено Тогда
								...
								ИначеЕсли ВРег(НаименованиеКолонки) = ВРег("ОЧ")  тогда
									Если ЗначениеЗаполнено(СтрокаТовары[НаименованиеКолонки]) тогда
										если  ПечататьОЧ тогда
											Если СтрокаТовары[НаименованиеКолонки] > 0 тогда
												Область.Параметры[ПараметрШаблона.Значение] =  "ОЧ / ПЧ від " + Формат(СтрокаТовары[НаименованиеКолонки],"ЧДЦ=; ЧГ=3,0");
											КонецЕсли; 
										Иначе
												Область.Параметры[ПараметрШаблона.Значение] =  "";
										КонецЕсли;
								...
								ИначеЕсли НаименованиеКолонки = 	"БонусАкцияСписан" ИЛИ НаименованиеКолонки = 	"БонусБазаНачислен"  тогда 
									Если ЗначениеЗаполнено(СтрокаТовары.ШаблонЦенникаДляПечати) тогда 
										...
										Если ТипСкидкиНаценки = Перечисления.ТипСкидкиНаценки.НаПервуюПозицию тогда
											ЦенаДляБонусов = СтрокаТовары.ЦенаСоСкидкой;
										Иначе
											ЦенаДляБонусов = СтрокаТовары.ЦенаЦелаяЧасть;
										КонецЕсли;		
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли 	УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КомплексныйА6") Тогда
		
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КомплексныйА6",
				"КомплексныйА6",
				ПечатьЦенникМебельА6(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,Истина)
	ИначеЕсли 	УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КомплексныйА6") Тогда
		
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КомплексныйА4Простой",
				"КомплексныйА4Простой",
				ПечатьЦенникМебельА4Простой(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,Истина)
	ИначеЕсли 	УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КомплексныйА6БезПиктограмм") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "КомплексныйА6БезПиктограмм",
				"КомплексныйА6БезПиктограмм",
				ПечатьЦенникМебельА6БезПиктограмм(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,Истина)
	Иначе
	...
	СтруктураНастроек.ОбязательныеПоля.Добавить("Маржинальность");	
...
Запрос.Текст = 
		...
		|	НоменклатураДополнительныеРеквизиты.Значение КАК Страна,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(СпрНоменклатура.Марка.ВысокомаржинальнаяТМ, Ложь) = ИСТИНА
		|			ТОГДА ""8""
		|		ИНАЧЕ ""0""
		|	КОНЕЦ КАК Маржинальность
		|ИЗ
...
//				Область.Параметры["стараціна"+А] = "стара ціна";
		...
		Иначе
			Область.Параметры["ОЧ"] = "ОЧ / ПЧ від " + ОЧ + " платежів від " + Формат(Цена/ОЧ,"ЧДЦ=") + "грн./міс*" ;
...
Запрос.Текст = 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОЧ = 0 тогда 
			...
			Область.Параметры["ПояснениеОЧ"] = "";
		Иначе
			Область.Параметры["ОЧ"] = "ОЧ / ПЧ від " + ОЧ + " платежів від " + Формат(Цена/ОЧ,"ЧЦ=") + "грн./міс*" ;
			Область.Параметры["ПояснениеОЧ"] = "*сума платежу від повноі вартості набору";
		КонецЕсли;
...
Функция ПечатьЦенникМебельА6(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
ТабличныйДокумент  = Новый ТабличныйДокумент;
КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
...
Функция ПечатьЦенникМебельА4Простой(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
ТабличныйДокумент  = Новый ТабличныйДокумент;
КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
	
Макет = УправлениеПечатью.ПолучитьМакет("Обработка.ПечатьЭтикетокИЦенников.КомплексныйА4Простой", КодЯзыкаПечать);
Область = Макет.ПолучитьОбласть("ОбластьЦенник");
...
Функция ПечатьЦенникМебельА6БезПиктограмм(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
ТабличныйДокумент  = Новый ТабличныйДокумент;
КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;
	
Макет = УправлениеПечатью.ПолучитьМакет("Обработка.ПечатьЭтикетокИЦенников.КомплексныйА6БезПиктограмм", КодЯзыкаПечать);
Область = Макет.ПолучитьОбласть("ОбластьЦенник");

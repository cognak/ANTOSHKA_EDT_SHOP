...
Процедура  ПередатьРазмерыЦенниковДляЗаписи()
	// + Отключил запись размера ценников(управляется из Нав) Бакан Т.С. 2024.08.07
...
//	СтруктураРезультата = Обработки.ПечатьЭтикетокИЦенников.ПодготовитьСтруктуруДанных(СтруктураНастроек);
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.ТаблицаТоваров Цикл
		...
		Попытка
			...
			НоваяСтрока.Маржинальность        = СтрокаТЧ.Маржинальность;
		...
		Попытка
			...
			Иначе
				ТипСкидкиНаценки = СтрокаТЧ.ТипСкидкиНаценки;
			КонецЕсли;  	
			БезПиктограм = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаТЧ.Номенклатура,"ПечатьЦенниковБезПиктограмм");
			Шаблон =  ПолучитьШаблон(СтрокаТЧ.Номенклатура,ТипСкидкиНаценки,БезПиктограм);
			Если НЕ ЗначениеЗаполнено(Шаблон) тогда 
				Шаблон =  ПолучитьШаблон(СтрокаТЧ.Номенклатура,ТипСкидкиНаценки, НЕ БезПиктограм);
			КонецЕсли;
			НоваяСтрока.ШаблонЦенника        = Шаблон;
...
&НаСервере
Функция ПолучитьШаблон(Номенклатура,ТипСкидкиНаценки,БезПиктограмм)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(Номенклатура,"ТолькоОбычныйЦенник") тогда 
		...
		Запрос.Текст = 
			...
			|	И ХранилищеШаблонов.БезПиктограмм = Истина			
	...
	Иначе
		...
		Запрос.Текст = 
			...
			|	И ХранилищеШаблонов.БезПиктограмм = &БезПиктограмм		
		...
		Иначе
			 ...
			 Эксклюзив = Ложь; 
		 КонецЕсли;
		Запрос.УстановитьПараметр("БезПиктограмм", БезПиктограмм);	 
...
&НаСервере
Функция ПолучитьШаблонПоРазмеру(Номенклатура,ТипСкидкиНаценки,БезПиктограмм)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ХранилищеШаблонов.Ссылка КАК ШаблонЦенника
...
Процедура ДобавитьТовары(Таблица)
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.ТаблицаТоваров Цикл
		...
		Маржинальность = ПолучитьМаржинальность(СтрокаТЧ.Номенклатура);   
		НоваяСтрока.Маржинальность =Маржинальность; 	
	...
	Если ЗначениеЗаполнено(Объект.РазмерЦенника) тогда
		Для Каждого Строка из Объект.Товары цикл 
			Попытка		
				...
				БезПиктограм = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Строка.Номенклатура,"ПечатьЦенниковБезПиктограмм");
				Шаблон =   ПолучитьШаблонПоРазмеру(Строка.Номенклатура,ТипСкидкиНаценки,БезПиктограм);
				Если НЕ ЗначениеЗаполнено(Шаблон) тогда 
					Шаблон =   ПолучитьШаблонПоРазмеру(Строка.Номенклатура,ТипСкидкиНаценки,НЕ БезПиктограм);
				КонецЕсли;
...
Процедура ЗаполнитьСкидкиНаценкиНаСервере()
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл  	
		...
		Попытка
			БезПиктограм = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаТЧ.Номенклатура,"ПечатьЦенниковБезПиктограмм");				
			Шаблон =  ПолучитьШаблон(СтрокаТЧ.Номенклатура,СтрокаТЧ.ТипСкидкиНаценки,БезПиктограм);
			Если НЕ ЗначениеЗаполнено(Шаблон) тогда 
				Шаблон =  ПолучитьШаблон(СтрокаТЧ.Номенклатура,СтрокаТЧ.ТипСкидкиНаценки, НЕ БезПиктограм);
			КонецЕсли;
			СтрокаТЧ.ШаблонЦенника        = Шаблон;
...
&НаСервере
Функция ПолучитьМаржинальность(Номенклатура)
	ВысокомаржинальнаяТМ = "0";
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВЫБОР
		...
		|			ТОГДА ""8""
		|		ИНАЧЕ ""0""
		|	КОНЕЦ КАК ВысокомаржинальнаяТМ
		|ИЗ
		|	Справочник.Номенклатура КАК Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Марки КАК Марки
		|		ПО Номенклатура.Марка = Марки.Ссылка
		|ГДЕ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ВысокомаржинальнаяТМ = ВыборкаДетальныеЗаписи.ВысокомаржинальнаяТМ;
	КонецЦикла;
	
Возврат ВысокомаржинальнаяТМ;
...
Функция ПолучитьОЧ(Номенклатура)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	МИНИМУМ(УслугиПредоставления.Ссылка.ПериодПредоставления) КАК ПериодПредоставления,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ТабРезультат.ПериодПредоставления) КАК ПериодПредоставления
		|ИЗ
...
&НаКлиенте
Процедура ПечатьКомплексногоЦенникаА6(Команда)
		ТекстОшибки = НеУчтенКомплектА4Гарнитура();
	Если ТекстОшибки <> "" тогда
		Сообщить(ТекстОшибки); 		
		Возврат;     		
...
&НаКлиенте
Процедура ПечатьКомплексногоЦенникаА6БезПиктограмм(Команда)
		ТекстОшибки = НеУчтенКомплектА4Гарнитура();
	Если ТекстОшибки <> "" тогда
		Сообщить(ТекстОшибки); 		
		Возврат;     		
...
&НаКлиенте
Процедура ПечатьКомплексногоЦенникаА4Простой(Команда)
		ТекстОшибки = НеУчтенКомплектА4Гарнитура();
	Если ТекстОшибки <> "" тогда
		Сообщить(ТекстОшибки); 		
		Возврат;     		

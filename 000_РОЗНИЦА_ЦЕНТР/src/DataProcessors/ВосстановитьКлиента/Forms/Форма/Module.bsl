...
#Область ОбработчикиСобытийЭлементовТаблицыФормыНайденныеКлиенты1
...
Процедура ВосстановитьНаСервере()
	...
	Для Каждого СтрТ Из Объект.НайденныеКлиенты Цикл
		...
		Поиск = Новый Структура("Тип,Вид", Перечисления.ТипыКонтактнойИнформации.Телефон,
		...
		Если Телефон = Неопределено Или Телефон.Количество() = 0 Тогда
			...
			УправлениеКонтактнойИнформацией.ЗаписатьКонтактнуюИнформацию(ОБКонтрагент,
																		 СтрТ.Данные,
																		 Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента,
		...
		Исключение
			Сообщить("НЕ Записан элемент " + ОБКонтрагент.Наименование + " " + ОБКонтрагент.Код + " "
				+ ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры

...
Функция ВыполнитьЗапрос(ТекстЗапроса, ПараметрыЗапроса, СпособВыгрузки, СоставРезультатов, ДляСсылокВыводитьГУИД, РазмерВыборки) Экспорт
	...
	Для каждого ОписаниеПараметра из ПараметрыЗапроса Цикл
		...
		ИначеЕсли ТипЗнч(Значение) = мТипОписаниеТипов Тогда
			...
			Иначе	ИмяПараметраТип = ОписаниеПараметра.ИмяПараметра;
					Прервать;
	...
	Если СоставРезультатов = 1 Тогда // только запросы
		...
		Моксель = ПолучитьРезультатыЗапросов(МассивРезультатов, СпособВыгрузки, РазмерВыборки);
...
Функция ПолучитьРезультатыЗапросов(МассивРезультатов, СпособВыгрузки, РазмерВыборки)
	Если МассивРезультатов.Количество() > 1 Тогда
		Моксель = ЗаполнитьМоксельПакета(МассивРезультатов, СпособВыгрузки, РазмерВыборки);
	Иначе
		Моксель = ЗаполнитьМоксель(МассивРезультатов[0], СпособВыгрузки, РазмерВыборки);
	КонецЕсли;
	Возврат Моксель;
КонецФункции
...
Функция ЗаполнитьМоксельПакета(МассивРезультатов, СпособВыгрузки, РазмерВыборки = 0)
	СловоЗапрос = "Запрос";
	Моксель = Новый ТабличныйДокумент();
...
Функция ЗаполнитьМоксель(Результат, СпособВыгрузки, РазмерВыборки = 0)
	...
	Если СпособВыгрузки = 1 Тогда // список
		...
		ВыборкаДанных = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
		РазмерВыборки = ВыборкаДанных.Количество();
		Моксель.Вывести(ВывестиПлоско(ВыборкаДанных, Ячейка, КоличествоКолонок - 1));
	...
	ИначеЕсли СпособВыгрузки = 2 Тогда // дерево
		...
		РазмерВыборки	= ВыгрузкаВДерево.Строки.Количество();
...
Процедура ЗаполнитьУровеньПроксиДерева(КоллекцияПриемник, КоллекцияИсточник, ИмеетсяШиринаКолонок)	Экспорт
...
Функция ПолучитьДеревоЗначенийИзПрокси(ДеревоЗапросов)	Экспорт

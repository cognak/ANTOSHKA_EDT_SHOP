&НаСервере
Процедура ЗанестиДанныеВРегистр(КодМагазина,ДАТА,Сумма)
	
		Организация = ПолучитьОрганизацию(ОбменДаннымиПовтИсп.ПолучитьДанныеУзла().Магазин);	
	...
	Если не Магазин=Справочники.Магазины.ПустаяСсылка() Тогда
		НовыйПлан = РегистрыСведений.ПланПродажПоЧасам.СоздатьНаборЗаписей();
		...
		НовыйПлан.Отбор.Период.Установить(НачалоЧаса(ДАТА));
			попытка
				ЗаписьГигиена =  НовыйПлан.Добавить();
				ЗаписьГигиена.Магазин = Магазин;
				ЗаписьГигиена.Период = НачалоЧаса(ДАТА);
				ЗаписьГигиена.Организация = Организация;
				ЗаписьГигиена.ПланПродаж=Сумма;
...
пока  КодМагазина<>"" И  ДатаСтрока<>"" цикл 
	//НомерСтроки=НомерСтроки+1;
		...
		Сумма = СтрЗаменить(СокрЛП(ТабличныйДокумент.Область("R"+Формат(Формат(НомерСтроки,"ЧГ="),"ЧГ=")+"C"+Формат(3,"ЧГ=")).Текст)," ","");
		Если ЗначениеЗаполнено(КодМагазина) Тогда
			ЗанестиДанныеВРегистр(КодМагазина,ДАТА,Число(Сумма));

...
Процедура ОбновитьСотрудниковКоллЦетраНаСервере()
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботаСотрудниковКоллЦентра.СрезПоследних(, ) КАК РаботаСотрудниковКоллЦентраСрезПоследних
	...
	ДатаС  = НачалоДня(ТекущаяДата())-3*60*60;
	ДатаПо = КонецДня(ТекущаяДата())-3*60*60;

...
Процедура ОбновитьДанныеВыполнить(Команда)
	...
	Перем Идентификатор;	//	LNK 09.04.2024 07:34:29
	...
	Если НЕ Идентификатор = Неопределено Тогда
		...
		СтрокиТекущие = ТаблицаРегламентныеЗадания.НайтиСтроки(Новый Структура("Идентификатор", Идентификатор));

		Если НЕ СтрокиТекущие.Количество() = 0 Тогда

			Элементы.ТаблицаРегламентныеЗадания.ТекущаяСтрока = СтрокиТекущие[0].ПолучитьИдентификатор();
...
Процедура ОбновитьТаблицуРегламентныхЗаданий(ИдентификаторРегламентногоЗадания = Неопределено)
	...
	ТаблицаРегламентныеЗадания.Сортировать("Наименование");	//	LNK 24.02.2017 13:20:58
...
Процедура ОбновитьСтрокуТаблицыРегламентныхЗаданий(Строка, Задание);
	...
	Строка.Метаданные = Задание.Метаданные.Имя;

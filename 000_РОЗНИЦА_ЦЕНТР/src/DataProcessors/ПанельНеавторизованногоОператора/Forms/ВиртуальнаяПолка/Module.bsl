...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьТекущийЭлемент(Элементы.НоменклатураСписок.Имя);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" Тогда
		...
		Если ИмяСобытия = "ScanData" Тогда
			...
			СтруктураПараметровКлиента.Вставить("ИмяТекущегоЭлемента", "Номенклатура");
...
Процедура НоменклатураСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ Элемент.ТекущиеДанные = Неопределено И НЕ Элемент.ТекущиеДанные.ЭтоГруппа Тогда
		...
		Объект.Номенклатура = ВыбраннаяСтрока;
...
Процедура НоменклатураПриИзменении(Элемент)
	...
	СтруктураПараметровКлиента = Новый Структура(
		"НеизвестныеДанныеПО, ИмяТекущегоЭлемента"
		, Ложь
		, Элемент.Имя);
...
Процедура ИнициализацияРеквизитов()
	...
	НоменклатураСписок.Параметры.УстановитьЗначениеПараметра("Склады" , Результаты[1].Выгрузить().ВыгрузитьКолонку("Ссылка"));
//	НоменклатураСписок.Параметры.УстановитьЗначениеПараметра("Магазин", Объект.Магазин);
...
&НаКлиенте
Процедура УстановитьТекущийЭлемент(ИмяТекущегоЭлемента)

	ИмяЭлемента = ИмяТекущегоЭлемента;
...
Процедура УстановитьТекущийЭлементЗавершение()
	...
	Если ИмяЭлемента = "Номенклатура" И НЕ Объект.Номенклатура.Пустая() Тогда

		Элементы.НоменклатураСписок.ТекущаяСтрока = Объект.Номенклатура;
...
Процедура ОбработатьДанныеПоКодуКлиент(СтруктураПараметровКлиента, Штрихкод = "")	Экспорт
	...
	УстановитьТекущийЭлемент(СтруктураПараметровКлиента.ИмяТекущегоЭлемента);

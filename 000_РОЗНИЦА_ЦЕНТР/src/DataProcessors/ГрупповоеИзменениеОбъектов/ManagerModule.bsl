...
//	LNK 07.03.2017 11:49:51
	Перем БылИсключительныйРежим;
...
//	LNK 07.03.2017 11:47:14
	Если Параметры.Свойство("ИсключительныйРежим") И ТипЗнч(Параметры.ИсключительныйРежим) = Тип("Булево") Тогда

		БылИсключительныйРежим = ТехническаяПоддержкаВызовСервера.ИсключительныйРежим();
		ПараметрыСеанса.ИсключительныйРежим = Параметры.ИсключительныйРежим;
	...
	Для Каждого Ссылка Из ОбрабатываемыеОбъекты Цикл
		...
		Исключение
			...
			Если ИзменятьВТранзакции Тогда
				...
				ВосстановитьИсключительныйРежим(БылИсключительныйРежим);
				Возврат;
			КонецЕсли;
...
//	LNK 07.03.2017 11:51:39
Процедура ВосстановитьИсключительныйРежим(БылИсключительныйРежим)

	Если ТипЗнч(БылИсключительныйРежим) = Тип("Булево") Тогда

...
Процедура ИсторияОпераций(Команда)
	...
	Если ИсторияОперацийСписок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Изменение элементов не выполнялось ранее.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиВЖурналРегистрации(Команда)
	...
	ОткрытьФормуМодально("Обработка.ЖурналРегистрации.Форма", ПараметрыЖР, ЭтотОбъект);
...
//	LNK 07.03.2017 11:32:15
	СтруктураПараметров.Вставить("ИсключительныйРежим", ТехническаяПоддержкаВызовСервера.ИсключительныйРежим());

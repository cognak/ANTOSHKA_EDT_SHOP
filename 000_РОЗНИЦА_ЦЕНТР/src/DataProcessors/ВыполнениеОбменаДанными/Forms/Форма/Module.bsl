...
//	Пропускаем инициализацию, чтобы гарантировать получение формы при передаче параметра "АвтоТест".
	...
	ОбновитьПовторноИспользуемыеЗначения();	//	LNK 16.09.2022 06:40:22
	
	Параметры.Свойство("ЗакрытьФорму", ЗакрытьФорму);
...
//	LNK 02.01.2018 14:11:37
	ЕстьОшибки = Ложь;//КонфигурацияИзменена() И ОбменДаннымиПовтИсп.ГлавныйУзел() = УзелИнформационнойБазы;
	...
	Если ВидТранспортаСообщений = Перечисления.ВидыТранспортаСообщенийОбмена.WS Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, НастройкиТранспортаWS);
...
Процедура ПерейтиВЖурналРегистрации(Команда)
	...
	ОткрытьФормуМодально("Обработка.ЖурналРегистрации.Форма", ПолучитьСтруктуруДанныхОтбораЖурналаРегистрации(УзелИнформационнойБазы), ЭтотОбъект);

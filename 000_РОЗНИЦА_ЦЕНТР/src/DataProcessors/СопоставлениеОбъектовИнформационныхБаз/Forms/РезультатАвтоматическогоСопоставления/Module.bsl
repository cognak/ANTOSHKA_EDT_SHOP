...
Процедура ПриОткрытии(Отказ)
	...
	Если ТаблицаАвтоматическиСопоставленныхОбъектовПустая Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Одинаковые данные не найдены.
...
Процедура УстановитьВидимостьПолейТаблицы(Знач ИмяТаблицыФормы, Знач МаксимальноеКоличествоПользовательскихПолей)
	...
	Для НомерПоля = 1 По МаксимальноеКоличествоПользовательскихПолей Цикл
		...
		ЭтотОбъект.Элементы[ПолеИсточника].Видимость = Ложь;
		ЭтотОбъект.Элементы[ПолеПриемника].Видимость = Ложь;
	...
	Для Каждого Элемент ИЗ Объект.СписокИспользуемыхПолей Цикл
		...
		ЭтотОбъект.Элементы[ПолеИсточника].Видимость = Элемент.Пометка;
		ЭтотОбъект.Элементы[ПолеПриемника].Видимость = Элемент.Пометка;
		
		// устанавливаем заголовки полей
		ЭтотОбъект.Элементы[ПолеИсточника].Заголовок = Элемент.Представление;
		ЭтотОбъект.Элементы[ПолеПриемника].Заголовок = Элемент.Представление;

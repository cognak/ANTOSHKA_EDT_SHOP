...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НадписьОбновлениеКонфигурацииВыполняетсяПриОбменеДаннымиСГлавнымУзлом.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Элементы.НадписьОбновлениеКонфигурацииВыполняетсяПриОбменеДаннымиСГлавнымУзлом.Заголовок, ОбменДаннымиПовтИсп.ГлавныйУзел());
...
Процедура ГиперссылкаПроверятьНаличиеОбновленияПриЗапускеНажатие(Элемент)
	...
	Результат = ОткрытьФормуМодально("Обработка.ОбновлениеКонфигурации.Форма.НастройкаРасписания", ПараметрыФормы, ЭтотОбъект);
...
Процедура НадписьПереченьРелизовНаДискахИТСНажатие(Элемент)
	Если ПустаяСтрока(ПереченьРелизовНаДискахИТС) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Информация о перечне релизов не обнаружена.'"));
	КонецЕсли;
...
Процедура НадписьРезервнаяКопияНажатие(Элемент)
	...
	Результат = ОткрытьФормуМодально("Обработка.ОбновлениеКонфигурации.Форма.НастройкаРезервнойКопии", ПараметрыФормы, ЭтотОбъект);
...
Функция ОбработатьСтраницуФайлОбновления(ВыдаватьСообщения = Истина)
	...
	Если Объект.НуженФайлОбновления = 1 Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ИмяФайлаОбновления) Тогда
			Если ВыдаватьСообщения Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Укажите файл поставки обновления конфигурации.'"));
			КонецЕсли;
		...
		Если НЕ Файл.Существует() ИЛИ НЕ Файл.ЭтоФайл() Тогда
			Если ВыдаватьСообщения Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Файл поставки обновления конфигурации не найден.'"));
			КонецЕсли;
			ТекущийЭлемент = Элементы.ПолеФайлОбновления;
			Возврат Страницы.ФайлОбновления.Имя;
		КонецЕсли;
...
Функция КаталогШаблонов()
	...
	КаталогПоУмолчанию = КаталогAppData() + "1C\1Cv8\tmplts\";

...
&НаКлиенте
Процедура Макри(Команда)
	ВыгрузитьТМНаСервере();
КонецПроцедуры

&НаСервере
Процедура ВыгрузитьТМНаСервере()
	Марка = Неопределено;
	УстановитьПривилегированныйРежим(Истина); 
	...
	Пока выборка.Следующий() Цикл
		...
		СпрМарка = ВнешниеИсточникиДанных.ТерминалыСбораДанных.Таблицы.dbo_ENT.НайтиПоПолю("IDN", СокрЛП(Выборка.IDN));
		Если СпрМарка.Пустая() Тогда
			Марка = ВнешниеИсточникиДанных.ТерминалыСбораДанных.Таблицы.dbo_ENT.СоздатьОбъект();   
		Иначе	Марка = СпрМарка.ПолучитьОбъект();

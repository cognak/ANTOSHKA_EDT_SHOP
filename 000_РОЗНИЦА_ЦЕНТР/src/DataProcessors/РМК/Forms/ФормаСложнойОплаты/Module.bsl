	...
	Если НЕ СанкционированноеЗакрытие Тогда
		...
		Если СтрокиВыбранныхОплат.Количество() > 0 Тогда
			...
			Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Продажа") Тогда
				...
				ТекстВопроса =  НСтр("ru = 'Вы хотите отменить операцию (-ии)?'"); 
...
Процедура УстановитьЗаголовокФормы()
	...
	Если НЕ ИспользоватьТерминал(УслугаБанкаДанные, УслугаБанка, ИспользоватьПодключаемоеОборудование)  Тогда
		...
		Заголовок = "ЭТ отключен «" + УслугаБанка + "»";

	Иначе

		Заголовок = "Оплата чека ККМ";
...
Процедура УстановитьОформлениеЭлементов()
	...
	Если Элементы.КомандаИспользованиеБонусов.Видимость И НЕ Элементы.КомандаИспользованиеБонусов.Доступность = ДоступностьКоманды Тогда
		...
		ПоказатьОповещениеПользователя("Оплата бонусными баллами",, "Оплата бонусными баллами запрещена!", БиблиотекаКартинок.Предупреждение32);
...
Процедура КомандаEnter(Команда)
	...
	Если Оплата.Итог("Сумма") = 0 Тогда
		...
		ТекстОшибки = НСтр("ru = 'Чек ККМ должен быть оплачен.'")
...
Процедура КомандаСторно(Команда)
	...
	Если ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыОплатЧекаККМ.ПлатежнаяКарта") Тогда
		...
		Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Продажа") Тогда
			...
			ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(НСтр("ru = 'Данные об оплате отправлены в банк.'"), 
		...
		Иначе
			...
			ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(НСтр("ru = 'Данные об отмене отправлены в банк.'"), 
			...
			Если ЗначениеЗаполнено(ИдентификаторУстройстваФР) ИЛИ ИспользоватьБезПодключенияОборудования Тогда
				...
				//
					Если РезультатФР ИЛИ ИспользоватьБезПодключенияОборудования Тогда
						...
						ПараметрыОткрытия = Новый Структура;
					//	ПараметрыОткрытия.Вставить("Заголовок"           , НСтр("ru = 'Авторизация операции'"));
						ПараметрыОткрытия.Вставить("Заголовок"           , НСтр("ru = 'Сумма оплаты'"));
				...
				Иначе
					...
					ЗаголовокИнформации = НСтр("ru = 'При подключении эквайрингового терминала произошла ошибка:'");
					ТекстСообщения = НСтр("ru = '""%ОписаниеОшибки%"".
										  |Операция по карте не была выполнена.'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ОписаниеОшибки);
...
Процедура ОплатаПлатежнойКартойЧерезЭквайринговыйТерминалАвторизация(Результат, ДополнительныеПараметры)	Экспорт
	...
	Иначе
		...
		ЗаголовокИнформации = НСтр("ru = 'Операция оплаты на эквайринговом терминале прошла успешно?'");
		...
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("Заголовок"           , НСтр("ru = 'Введите код транзакции (RRN)'"));
		...
		Тогда
			...
			ПараметрыОткрытия = Новый Структура;
			ПараметрыОткрытия.Вставить("Заголовок"           , НСтр("ru = 'ГЛУБИНА КРЕДИТА «" + УслугаБанка + "» в платежах'"));
				...
				Если РезультатЭТ Тогда
					...
					Если РезультатФР ИЛИ ИспользоватьБезПодключенияОборудования Тогда
						...
						Иначе
							...
							ЗаголовокИнформации = НСтр("ru = 'Операция отмены оплаты на эквайринговом терминале прошла успешно?'");
...
//	Если ИспользоватьПодключаемоеОборудование Тогда
	Если Истина Тогда
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru = 'Будет осуществлён возврат по терминалу: " + Строка(ВыбраннаяСтрока.ЭквайринговыйТерминал) + "'");
			...
			ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(
				"Внимание!", ТекстСообщения, "Нет",, ЭтотОбъект);

...
//	Пропускаем инициализацию, чтобы гарантировать получение формы при передаче параметра "АвтоТест".
	...
	//2026-09-04 sa Заполняем ТЧ Товары и СкидкиНаценки
	Если Параметры.Свойство("АдресТовары") Тогда 
		времТовары = ПолучитьИзВременногоХранилища(Параметры.АдресТовары);
		Если времТовары <> Неопределено Тогда
			Объект.Товары.Загрузить(времТовары);
		КонецЕсли;
	КонецЕсли;
	Если Параметры.Свойство("АдресСкидкиНаценки") Тогда
		времСкидкиНаценки = ПолучитьИзВременногоХранилища(Параметры.АдресСкидкиНаценки);
...
//	ИспользоватьБанковскиеУслуги = (ИспользоватьБанковскиеУслуги = Истина) И Элементы.КомандаКарта.Доступность;
	...
	ДоступнаКомандаКарта = ПродажиСервер.ЕстьВозможностьОплатыПлатежнойКартой(МассивКассККМ);
	...
	Если ТипЗнч(ДанныеЗаказа) = Тип("Структура") И ДанныеЗаказа.ЭтоУзелИнтернетМагазина = Истина Тогда
		...
		Если ДанныеЗаказа.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.НеОплачен") Тогда
			...
			ДоступнаКомандаКарта = (НЕ ДанныеЗаказа.СуммаДоставки = 0 И ДанныеЗаказа.СуммаДоставкиОплачена = Истина);

		КонецЕсли;
	
	КонецЕсли;
	//2025-03-10 sa Новое правило, если в чеке продажи было несколько видов оплаты, то возврат такого чека только за Наличные.
	Если ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда
		Если Параметры.Свойство("ЧекККМПродажа") И Параметры.ЧекККМПродажа.Оплата.Количество()>1 Тогда
			ДоступнаКомандаКарта = Ложь;	
		КонецЕсли;
	КонецЕсли;
	ДоступнаКомандаКарта = ДоступнаКомандаКарта И ПроверитьМожноЛиПроводитьПовторнуюОплатуКартой();
	
	Элементы.КомандаКарта.Доступность = ДоступнаКомандаКарта;
...
&НаСервере
Функция ПроверитьМожноЛиПроводитьПовторнуюОплатуКартой()
	
	РезультатПроверки = Истина;
	//2025-03-18 sa Нужно определить была ли оплата картой в чеке. Если была и ФР Мария, то больше платить картой не разрешаем.
	ПараметрыКассыККМ = ЗначениеНастроекПовтИсп.ПолучитьПараметрыКассыККМ(КассаККМ);
	Если КассаККМ.ПодключаемоеОборудование.ОбработчикДрайвера = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикResonanceФискальныйРегистратор
		И ВидОперации = Перечисления.ВидыОперацийЧекККМ.Продажа
			Тогда
			Для Каждого СтрокаОплаты_ Из Оплата Цикл
				Если СтрокаОплаты_.ВидОплаты.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ПлатежнаяКарта Тогда
...
Процедура КомандаEnter(Команда)
	...
	СуммаНаличнойОплаты = СуммаОплатПоТипу(ПредопределенноеЗначение("Перечисление.ТипыОплатЧекаККМ.Наличные"));

	Если Оплата.Итог("Сумма") = 0 ИЛИ СуммаНаличнойОплаты > СуммаНаличныхОграничение Тогда

		ЗаголовокИнформации = "Помилка оплати за чеком"; 

		Если Оплата.Итог("Сумма") = 0 Тогда
			
			ТекстОшибки = "Чек ККМ має бути сплачений.
						|Необхідно зазначити суму оплати за чеком.";
	...
	//2026-09-07 sa передаем в РМК Адрес таблицы Товары и адрес таблицы СкидокНаценок
	СтуктураОтвета.Вставить("АдресТовары", АдресТаблицыТовары());
	СтуктураОтвета.Вставить("АдресСкидкиНаценки", АдресТаблицыСкидкиНаценки());
...
Процедура КомандаНаличные(Команда)
	...
	СтрокиВидаОплаты = ТекущиеВидыОплат.НайтиСтроки(Новый Структура("ВидОплаты", ПредопределенноеЗначение("Справочник.ВидыОплатЧекаККМ.Наличные")));

	Если НЕ СтрокиВидаОплаты.Количество() = 0 Тогда

			СтрокаВидаОплаты = СтрокиВидаОплаты[0];

	Иначе	СтрокаВидаОплаты = ТекущиеВидыОплат.Добавить();
			
			СтрокаВидаОплаты.ВидОплаты = ПредопределенноеЗначение("Справочник.ВидыОплатЧекаККМ.Наличные");
			СтрокаВидаОплаты.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипыОплатЧекаККМ.Наличные");

	КонецЕсли;
	
	СтрокаВидаОплаты.Сумма = СуммаОкругленная(СуммаКОплате);
	
	СтрокаВидаОплаты.ПредставлениеСуммы      = СтрокаВидаОплаты.Сумма;
	СтрокаВидаОплаты.ПредставлениеВидаОплаты = "Наличные";
	СтрокаВидаОплаты.Организация             = Организация;
	
	ТекущаяГруппыСтраницы("ГруппаСтраница");
	ТекущийЭлемент = Элементы.ТекущиеВидыОплат;
	Элементы.ТекущиеВидыОплат.ТекущаяСтрока = СтрокаВидаОплаты.ПолучитьИдентификатор();
...
#Область ПогашениеПодарочногоСертификата
...
Процедура КомандаСертификат(Команда)
	...
	ПараметрыОткрытия = Новый Структура(
		"Организация"
		, Организация	//	LNK 29.12.2023 10:14:59
	);
...
//	LNK 01.09.2020 05:03:33 - обновим флаг доступа принудительно..
	...
	ПараметрыОткрытия.Вставить("Магазин", Магазин);	//	LNK 28.02.2024 15:35:08
...
&НаСервере
Функция АдресТаблицыТовары()
	
	АдресТаблицы = ПоместитьВоВременноеХранилище(Объект.Товары.Выгрузить(), Новый УникальныйИдентификатор);
...
&НаСервере
Функция АдресТаблицыСкидкиНаценки()
	
	АдресТаблицы = ПоместитьВоВременноеХранилище(Объект.СкидкиНаценки.Выгрузить(), Новый УникальныйИдентификатор);
...
Процедура ЗаполнитьТаблицуВидыОплаты(ТипОплаты, ВозвратПоУслугеБанка = Ложь)
	...
	Для Каждого ТекСтрокаТаблицаВидовОплат Из ТаблицаВидовОплат Цикл
		...
		СтрокаТаблицы = ВидыОплат.Добавить();

		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ТекСтрокаТаблицаВидовОплат);
		СтрокаТаблицы.ТипОплаты = ТипОплаты;
...
Процедура ОбновитьФорму(ВыполнитьЗакрытие = Ложь)
	...
	Если ОбщаяСуммаОплаты >= СуммаОкругленная(ИтогПоОрганизации) Тогда
		...
		Элементы.КомандаКарта.Доступность		= Ложь;
		Элементы.КомандаКредит.Доступность		= Ложь;
		Элементы.КомандаСертификат.Доступность	= Ложь;
	...
	Иначе
		...
		Если НЕ УслугаБанка.Пустая() И УслугаБанкаДанные.РазрешитьСмешаннуюОплату Тогда
			
			УстановитьДоступностьОплат();
			
		Иначе
			
			УстановитьДоступностьОплат(
				  УслугаБанка.Пустая() ИЛИ НЕ УслугаБанкаДанные.ТипИнтернетБанкинга = ПредопределенноеЗначение("Перечисление.ТипыИнтернетБанкинга.ПустаяСсылка")
				, УслугаБанка.Пустая() И УслугаБанкаДанные.ТипИнтернетБанкинга = ПредопределенноеЗначение("Перечисление.ТипыИнтернетБанкинга.ПустаяСсылка")
			);
...
&НаКлиенте	//	LNK 10.01.2023 06:50:37
Функция СуммаОплатПоТипу(ТипОплаты)
	...
	Для каждого СтрокаВидовОплат Из СтрокиВидовОплат Цикл
		...
		СуммаТипаОплаты = СуммаТипаОплаты + СтрокаВидовОплат.Сумма;
	
	КонецЦикла;
	
	Возврат СуммаТипаОплаты
			...
			Если ЗначениеЗаполнено(ИдентификаторУстройстваФР) ИЛИ ИспользоватьБезПодключенияОборудования Тогда
				...
				//
					Если РезультатФР ИЛИ ИспользоватьБезПодключенияОборудования Тогда
						...
						ПределСуммы           = ИтогПоОрганизации - СуммаОплатПоТипу(ПредопределенноеЗначение("Перечисление.ТипыОплатЧекаККМ.ПлатежнаяКарта"));
...
Процедура ОплатаПлатежнойКартойЧерезЭквайринговыйТерминалАвторизация(Результат, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ЗначениеВыбораЧисло) Тогда
			...
			Если СуммаОперации < (ИтогПоОрганизации-СуммаОплаты) тогда 
				ОстатокКопеек = (ИтогПоОрганизации -СуммаОплаты - СуммаОперации) % 1;
				Если ОстатокКопеек  > 0.001 Тогда
					Сумма1 =  СуммаОперации + ОстатокКопеек;
					Сумма2 =  СуммаОперации + ОстатокКопеек - 1;
					ТекстИнформации = "Увага! У разі змішаної оплати (коли друга частина оплачується готівкою) суму для оплати карткою треба вводити з копійками! " + Сумма1 + "грн. " + "чи "  +Сумма2 + "грн." ;
				   	ЗаголовокИнформации = "Бажаєте змінити сумму оплати?";
	...
	ОтветБанка = Новый Структура("Сумма,Скидка",СуммаОперации,0);
	...
	Если НЕ СтруктрураЭквайринговыйТерминал.ИспользоватьБезПодключенияОборудования
		...
		//	Выполнение операции на ЭТ
			РезультатЭТ = МенеджерОборудованияКлиент.ВыполнитьКоманду(ИдентификаторУстройстваЭТ,
																	  ...
																	  ВходныеПараметры,
																	  ВыходныеПараметры);
			//2026-09-07 sa !!!! обязательно убрать
			//РезультатЭТ = Истина;
			//ВыходныеПараметры = Новый Массив;
			//вМассивОплат = Новый Массив;
			//вМассивОплат.Добавить("СлипЧек");
			//вСлипЧек = "ПриватБанк
			//	|
			//	|VISA
			//	|473121965******8
			//	|КОД АВТОРИЗАЦИИ: 956345
			//	|RRN: 116694068692
			//	|СУММА: "+(ОтветБанка.Сумма - Окр(ОтветБанка.Сумма/10,2))+" ГРН
			//	|КЛИЕНТ: 
			//	|МЕРЧАНТ ID: S1OS0KNY
			//	|ТЕРМИНАЛ ID: S1OS0KNY";
			//вМассивОплат.Добавить(вСлипЧек);
			//вСтруктураОтвета = Новый Структура;
			//вСтруктураОтвета.Вставить("Банк","ПриватБанк");
			//вСтруктураОтвета.Вставить("ДействительнаДо","2706");
			//вСтруктураОтвета.Вставить("Клиент","");
			//вСтруктураОтвета.Вставить("КодRRN","116694068692");
			//вСтруктураОтвета.Вставить("КодАвторизации","956345");
			//вСтруктураОтвета.Вставить("МерчантИД","S1OS0KNY");
			//вСтруктураОтвета.Вставить("НомерКарты","473121965******8");
			//вСтруктураОтвета.Вставить("НомерЧека","1401334322");
			//вСтруктураОтвета.Вставить("ОписаниеОшибки ", Неопределено);
			//вСтруктураОтвета.Вставить("Скидка ",Окр(ОтветБанка.Сумма/10,2));
			//вСтруктураОтвета.Вставить("СлипЧек ",вСлипЧек);
			//вСтруктураОтвета.Вставить("Сумма", ОтветБанка.Сумма);
			//вСтруктураОтвета.Вставить("ТерминалИД","S1OS0KNY");
			//вСтруктураОтвета.Вставить("Эмитент","VISA");

			//ВыходныеПараметры.Добавить("473121965******8");
			//ВыходныеПараметры.Добавить("116694068692");
			//ВыходныеПараметры.Добавить("1401334322");
			//ВыходныеПараметры.Добавить(вМассивОплат);
			//ВыходныеПараметры.Добавить("956345");
			//ВыходныеПараметры.Добавить("2706");
			//ВыходныеПараметры.Добавить("");
			//ВыходныеПараметры.Добавить("VISA");
			//ВыходныеПараметры.Добавить("S1OS0KNY");
			//ВыходныеПараметры.Добавить("S1OS0KNY");
			//ВыходныеПараметры.Добавить(вСтруктураОтвета);
			////эмуляция оплаты
		...
		Если РезультатЭТ Тогда
			...
			Если ВыходныеПараметры.Количество() > 10 Тогда
				ОтветБанка = ВыходныеПараметры[10];
				Если ТипЗнч(ОтветБанка) = Тип("Структура") И ОтветБанка.Свойство("Скидка") И ОтветБанка.Скидка <> 0 Тогда
					Сообщить("RRN: "+ОтветБанка.КодRRN);
					//есть скидка платежной системой, нужно распределить ее по чеку перед фискализацией.
					Успешно = РаспределитьСкидкуПлатежнойСистемы(ОтветБанка);
					Если Успешно Тогда
						ИтогПоОрганизации = ИтогПоОрганизации - ОтветБанка.Скидка;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		...
		ПараметрыОткрытия.Вставить("ПрочитатьКарту"		 , Ложь);	//	LNK 30.11.2022 15:47:23
		ПараметрыОткрытия.Вставить("ЗапретитьОтмену"     , Истина);	//	LNK 16.04.2024 17:30:38
		...
		Тогда
			...
			ПараметрыОткрытия = Новый Структура;
			ПараметрыОткрытия.Вставить("Заголовок"           , "ГЛИБИНА КРЕДИТУ «" + УслугаБанка + "» у платежах");
	...
	//2026-09-07 sa !!!! Здесь нужно сделать сумму за вычетом скидки. Или же тут указываем полную сумму иначе форма будет требовать доплату на сумму предоставленной скидки.
	//СтрокаОплаты.Сумма     = СуммаОперации;
	СтрокаОплаты.Сумма     = ОтветБанка.Сумма - ОтветБанка.Скидка;
	...
	//2025-03-18 sa По окончанию оплаты картой проверяем если ФР = Мария, то блокируем последующие оплаты картой
	Элементы.КомандаКарта.Доступность = Элементы.КомандаКарта.Доступность И ПроверитьМожноЛиПроводитьПовторнуюОплатуКартой();

КонецПроцедуры

&НаСервере
Функция РаспределитьСкидкуПлатежнойСистемы(ОтветБанка)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |ГДЕ
	               |	СкидкиНаценки.ТипСкидкиНаценки = ЗНАЧЕНИЕ(Перечисление.ТипСкидкиНаценки.ПлатежнаяСистема)
	               |	И СкидкиНаценки.ПометкаУдаления = ЛОЖЬ";
	
	Рез = Запрос.Выполнить().Выбрать();
	Если Рез.Следующий() Тогда
		СкидкаПлатежнойСистемы = Рез.Ссылка;
	Иначе
		Сообщить("Увага: Не призначено знижку платіжної системи. Можуть бути розбіжності між банківським чеком та фіскальним реєстратором!");
		Возврат Ложь;
	КонецЕсли;
	
	СуммаСкидкиКРаспределению = ОтветБанка.Скидка;
	СуммаТоваров_ = Объект.Товары.Итог("Сумма");
	СтрокаСМаксСуммой = 1; МаксСумма = 0;
	Для Каждого Строка_ Из Объект.Товары Цикл
		ДоляСуммыВСкидке = Строка_.Сумма / СуммаТоваров_;
		СуммаСкидкиПоСтрокеПС = СуммаСкидкиКРаспределению * ДоляСуммыВСкидке;
		Строка_.СуммаАвтоматическойСкидки = Строка_.СуммаАвтоматическойСкидки + СуммаСкидкиПоСтрокеПС;
		СтрокаСкидка = Объект.СкидкиНаценки.Добавить();
		СтрокаСкидка.КлючСвязи = Строка_.КлючСвязи;
		СтрокаСкидка.СкидкаНаценка = СкидкаПлатежнойСистемы;
		СтрокаСкидка.Сумма = СуммаСкидкиПоСтрокеПС; 
		СуммаСкидкиКРаспределению = СуммаСкидкиКРаспределению - СуммаСкидкиПоСтрокеПС; 
		
		Строка_.Сумма = Строка_.Цена * Строка_.Количество - Строка_.СуммаАвтоматическойСкидки - Строка_.СуммаРучнойСкидки;
		Строка_.СуммаВсего = Строка_.Сумма;
		Строка_.СуммаНДС        = ОбработкаТабличнойЧастиТоварыКлиентСервер.РассчитатьСуммуНДСПоСуммеСНДС(Строка_.Сумма
														, НДСОбщегоНазначенияКлиентСервер.ПолучитьСтавкуНДСЧислом(Строка_.СтавкаНДС)
														, Объект.ЦенаВключаетНДС);
		Строка_.ПроцентАвтоматическойСкидки = 100-100* Строка_.Сумма/(Строка_.Цена * Строка_.Количество); 
		Если Строка_.Сумма > МаксСумма Тогда 
			МаксСумма = Строка_.Сумма;
			СтрокаСМаксСуммой = Строка_.НомерСтроки;
		КонецЕсли;
			
	КонецЦикла;
	
	//2026-09-03 sa если что-то осталось дораспределяем
	Если СуммаСкидкиКРаспределению <> 0 Тогда
		Строка_ = Объект.Товары.НайтиПоИдентификатору(СтрокаСМаксСуммой);
		Строка_.СуммаАвтоматическойСкидки = Строка_.СуммаАвтоматическойСкидки + СуммаСкидкиКРаспределению;
		СтруктураОтбораСкидки = Новый Структура("СкидкаНаценка,КлючСвязи",СкидкаПлатежнойСистемы, Строка_.КлючСвязи);
		МассивСтрок_ = Объект.СкидкиНаценки.НайтиСтроки(СтруктураОтбораСкидки);
		Если МассивСтрок_.Количество() = 0 Тогда
			СтрокаСкидка_ = Объект.СкидкиНаценки.Добавить();
			СтрокаСкидка.КлючСвязи = Строка_.КлючСвязи;
			СтрокаСкидка.СкидкаНаценка = СкидкаПлатежнойСистемы;
		КонецЕсли;
		СтрокаСкидка.Сумма = СтрокаСкидка.Сумма + СуммаСкидкиКРаспределению; 
		...
		Строка_.СуммаНДС        = ОбработкаТабличнойЧастиТоварыКлиентСервер.РассчитатьСуммуНДСПоСуммеСНДС(Строка_.Сумма
														, НДСОбщегоНазначенияКлиентСервер.ПолучитьСтавкуНДСЧислом(Строка_.СтавкаНДС)
														, Объект.ЦенаВключаетНДС);
		Строка_.ПроцентАвтоматическойСкидки = 100* Строка_.Сумма/Строка_.Цена * Строка_.Количество; 
	КонецЕсли;
...
//	СуммаОперации       = СуммаОплаты;	// - HVOYA 03.11.2016 16:41:21, Латышев А.А. 
	...
	СтруктрураЭквайринговыйТерминал = ПодключаемоеОборудованиеРТКлиент.ДанныеЭквайринговогоТерминала(ВыбраннаяСтрока.ЭквайринговыйТерминал, ДанныеЗаказа.ЭтоУзелИнтернетМагазина = Истина);
	ИдентификаторУстройстваЭТ       = СтруктрураЭквайринговыйТерминал.ПодключаемоеОборудование;
						...
						Если НЕ СтруктрураЭквайринговыйТерминал.ИспользоватьБезПодключенияОборудования Тогда
							...
							Если РезультатЭТ Тогда
								...
								Если ВыходныеПараметры.Количество()>1 Тогда
									Если ТипЗнч(ВыходныеПараметры[1]) = Тип("Структура") И  ВыходныеПараметры[1].Свойство("КодRRN") Тогда
										НомерСсылкиОперации = ВыходныеПараметры[1].КодRRN;
									Иначе
										НомерСсылкиОперации = "";
									КонецЕсли;
									НомерКарты          = "";
									НомерЧекаЭТ         = "";
									глПодключаемоеОборудование.Вставить("ПоследнийНомерТранзакции", НомерСсылкиОперации);
								Иначе
									НомерКарты          = "";
									НомерСсылкиОперации = "";
									НомерЧекаЭТ         = "";
...
//	Если ИспользоватьПодключаемоеОборудование Тогда
	Если Истина Тогда
		...
		//	СтрокаОплаты.Сумма = СуммаОплаты;	// - HVOYA 04.11.2016 19:26:41, Латышев А.А. 
			...
			//2025-01-29 sa сохраняем слип-чек в оплатах для последующего использования
			Если глПодключаемоеОборудование.Свойство("ПоследнийСлипЧек") Тогда
				СтрокаОплаты.СлипЧек = глПодключаемоеОборудование.ПоследнийСлипЧек;
			КонецЕсли;
			//2025-03-05 sa восстанавливаем номер транзакции из последней оплаты/возврата
			Если глПодключаемоеОборудование.Свойство("ПоследнийНомерТранзакции") Тогда
				СтрокаОплаты.СсылочныйНомер = глПодключаемоеОборудование.ПоследнийНомерТранзакции;
			КонецЕсли;
...
Процедура ОбработкаТабличнойЧастиПогашенияПодарочныхСертификатов()
	...
	Для каждого СтрокаТаблицыПогашения Из ТаблицаПогашенияПодарочныхСертификатов Цикл
		...
		Если СтрокиТекущейТаблицаОплат.Количество() = 0 Тогда
			...
			//2025-03-01 sa подарочные сертификаты теперь должны быть по-отдельности (каждый сертификат отдельный вид оплаты)
			//засунем серийный номер в номер платежной карты, чтобы дальше система его обработала как отдельную оплату.
			СтрокаОплаты.НомерПлатежнойКарты    = СтрокаТаблицыПогашения.СерийныйНомер;
...
Процедура УстановитьОформлениеКнопокУслугиБанков(ТипУслуги, ПриОткрытии = Ложь)
	...
	Иначе
		...
		Если УслугаБанкаДанные.ТипУслуги = ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетМалыша")
				Или УслугаБанкаДанные.ТипУслуги = ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетШкольника") Тогда
					
			ТекстЗаголовка = ?(УслугаБанкаДанные.ТипУслуги = ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетМалыша")
				, "малюка"
				, "щколяра");

			Элементы.КомандаУслугиОплатаЧастями.Заголовок = "Пакунок" + Символы.ПС + ТекстЗаголовка;
...
&НаКлиенте	//	LNK 08.04.2024 10:36:56
Процедура УстановитьДоступностьОплат(НаличныеДоступность = Истина, СертификатДоступность = Истина, КредитДоступность = Истина, КартаДоступность = Истина)

	//2025-03-18 sa По окончанию оплаты картой проверяем если ФР = Мария, то блокируем последующие оплаты картой
	//Элементы.КомандаКарта.Доступность		= ДоступнаКомандаКарта И КартаДоступность;
	Элементы.КомандаКарта.Доступность		= ДоступнаКомандаКарта И КартаДоступность И ПроверитьМожноЛиПроводитьПовторнуюОплатуКартой();
	

	Элементы.КомандаНаличные.Доступность	= ДоступнаКомандаНаличные И НаличныеДоступность;
	Элементы.КомандаСертификат.Доступность	= ДоступнаКомандаСертификат И СертификатДоступность;
	Элементы.КомандаКредит.Доступность		= ДоступнаКомандаКредит И КредитДоступность;
...
Функция КонтекстВозвратаПоУслугеБанка()
	...
	Возврат ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат")
		...
		И НЕ (УслугаБанкаДанные.ТипУслуги = ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетМалыша")
			Или УслугаБанкаДанные.ТипУслуги = ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетШкольника"));

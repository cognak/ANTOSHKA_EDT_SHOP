...
Процедура КомандаEnter(Команда)
	...
	Если ОплатаБанковскимКредитомПлатежнойКартой > ИтогПоОрганизации Тогда
		...
		ОчиститьПоОрганизации();
		Возврат;
	ИначеЕсли ОплатаБанковскимКредитомПлатежнойКартой < ИтогПоОрганизации тогда 
		ОстатокКопеек = (ИтогПоОрганизации - ОплатаБанковскимКредитомПлатежнойКартой) % 1;
		Если ОстатокКопеек  > 0.001 Тогда
			Сумма1 =  ОплатаБанковскимКредитомПлатежнойКартой + ОстатокКопеек;
			Сумма2 =  ОплатаБанковскимКредитомПлатежнойКартой + ОстатокКопеек - 1;
			ЗаголовокИнформации = "Помилка сплати за чеком"; 
			ТекстОшибки = "Увага! У разі змішаної оплати суму по картці треба вводити з копійками! " + Сумма1 + "грн. " + "чи "  +Сумма2 + "грн." ;
...
Процедура ОплатаПлатежнойКартойЧерезЭквайринговыйТерминалАвторизация(Результат, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ЗначениеВыбораЧисло) Тогда
			...
			Если СуммаОперации < ИтогПоОрганизации тогда 
				ОстатокКопеек = (ИтогПоОрганизации - СуммаОперации) % 1;
				Если ОстатокКопеек  > 0.001 Тогда

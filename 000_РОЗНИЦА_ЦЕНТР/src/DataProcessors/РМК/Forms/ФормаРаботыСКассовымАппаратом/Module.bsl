...
Процедура ЗакрытьСмену(КассаККМ)
	...
	//+HVOYA Mykhailo
	Если ПараметрыКассыККМ.ПечатьЧековНаПринтер = Истина
	...
	Тогда  // передаем отчет на принтер
		Попытка
		...
		Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
			...
			Если ИдентификаторУстройства <> Неопределено Тогда
				...
				Если Результат Тогда
					...
					ВходныеПараметры[2][21] = ЗначениеНастроекПовтИсп.ПолучитьПараметрыКассыККМ(КассаККМ);

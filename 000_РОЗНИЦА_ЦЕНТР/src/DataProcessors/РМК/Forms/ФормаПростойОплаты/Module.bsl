...
//	Пропускаем инициализацию, чтобы гарантировать получение формы при передаче параметра "АвтоТест".
	...
	Если Параметры.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда
		...
		//	Запрос.УстановитьПараметр("СуммаИтоговая", Параметры.СуммаИтоговая);
			...
			//2026-02-06 sa Отменили правило
...
Процедура ЗаполнитьИзменяемыеРеквизиты(ПараметрыФормы)
	...
	ИтогПоЧекПараметр = 0;
	ПараметрыФормы.Свойство("ИтогПоЧеку", ИтогПоЧекПараметр);
	ИтогПоЧекПараметр = СуммаОкругленная(ИтогПоЧекПараметр);
	СуммаОплаты = ИтогПоЧекПараметр;
	ИтогПоЧеку = ИтогПоЧекПараметр;
...
Функция СуммаОкругленная(СуммаПараметр)
	...
	Если РазрешитьОкругление Тогда
		...
		СуммаБезКопеек = Цел(СуммаПараметр);
		СуммаКопеек = (СуммаПараметр - СуммаБезКопеек) * 100;
		...
		Значение = СуммаБезКопеек + НужнаяСуммаКопеек / 100;

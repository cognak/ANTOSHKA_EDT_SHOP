...
//	Пропускаем инициализацию, чтобы гарантировать получение формы при передаче параметра "АвтоТест".
	...
	Если Параметры.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда
		...
		Заголовок = "Возврат денежных средств";
		Элементы.ОплатитьКартой.Заголовок = "ВОЗВРАТ НА КАРТУ (F4)";
		...
		Элементы.ПолеВводимоеЧисло.Заголовок = "Выдано";
		Элементы.ИтогПоЧеку.Заголовок        = "К возврату";
...
Процедура КомандаEnter(Команда)
	...
	Если СуммаОплатыЧисло = 0 Тогда
		...
		ПараметрыИнформации.ЗаголовокИнформации = "ПЛОХАЯ СУММА ОПЛАТЫ";
		ПараметрыИнформации.ТекстИнформации = "Сумма оплаты должна быть заполнена!";
	...
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Продажа") Тогда
		...
		Если ВладелецФормы.ОтменитьРучнуюОплатуБонуснымиБалламиНаСервере() Тогда
			...
			ПоказатьОповещениеПользователя("Оплата бонусными баллами"
				,
				, "Очищена сумма баллов, введенная вручную!"

...
// - HVOYA 30.09.2016 14:55:23, Латышев А.А.
&НаКлиенте	//	LNK 16.04.2024 17:35:02
Перем СанкционированноеЗакрытие;
...
//	Пропускаем инициализацию, чтобы гарантировать получение формы при передаче параметра "АвтоТест".
	...
	Параметры.Свойство("ЗапретитьОтмену", ЗапретитьОтмену);	//	LNK 16.04.2024 17:32:44
...
&НаКлиенте	//	LNK 14.04.2024 12:20:04
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)

	Если ЗапретитьОтмену И НЕ СанкционированноеЗакрытие = Истина Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Тогда
		...
		Если ИмяСобытия = "TracksData" Тогда
			...
			Иначе	ПолученКодКарты(Параметр[0], Параметр[1][1]);
...
Процедура КомандаEnter(Команда)
	...
	Иначе	СтруктураОтвета.Вставить("ВведенноеЧисло", ПривестиСтрокуКЧислу(ВводимоеЧисло, Истина));

	КонецЕсли;

	СтруктураОтвета.Вставить("ДанныеКарты", ДанныеКарты);

	СанкционированноеЗакрытие = Истина;	//	LNK 16.04.2024 17:36:34

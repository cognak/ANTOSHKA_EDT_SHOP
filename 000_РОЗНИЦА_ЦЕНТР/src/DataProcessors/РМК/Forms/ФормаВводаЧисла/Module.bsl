...
Процедура ПриОткрытии(Отказ)
	...
	Если Заголовок = "СчитатьКарту" Тогда
	...
	Если ТипЗнч(ДанныеДорожек) = Тип("Массив")
	   ...
	   И Не ПустаяСтрока(ДанныеДорожек[1]) Тогда
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Указана неверная карта или произошла ошибка при считывании карты.
...
Функция ОжиданиеОтветаТерминала(POSТерминал)
	...
	Пока 1 = 1 Цикл
		...
		Если (ТекущаяУниверсальнаяДатаВМиллисекундах() - ЧасНачала)/1000 > ЧасОжиданияPOS Тогда
			ТекстСообщения = НСтр("ru='Превышено время ожидания ответа терминала!");
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Ошибка'"), ТекстСообщения);
			Возврат -1;
		КонецЕсли; 
	КонецЦикла;

	...
	Если Источник = "ПодключаемоеОборудование" Тогда
		...
		Если ИмяСобытия = "ScanData" Тогда
			...
			Если СерийныеНомера.Количество() >= Количество Тогда
				...
				ПоказатьОповещениеПользователя("Отказано!",, "Допустимое количество серийных номеров достигнуто", БиблиотекаКартинок.Предупреждение32);
			...
			//	Проверяем номерной сертификат..
				...
				Иначе
					...
					ТекстСообщения = "Получен недопустимый код «" + Штрихкод + "»";

				КонецЕсли;

				Если НЕ ПустаяСтрока(ТекстСообщения) Тогда

					ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК("Отказ кода «" + Штрихкод + "»", ТекстСообщения);
...
Функция ПолучитьСерийныйНомерПоШтрихкоду(Штрихкод, ТекстСообщения)
	...
	Если СоставОбъекта.Найден Тогда
		...
		Если НЕ СоставОбъекта.Владелец = Номенклатура Тогда
			...
			+ "НЕ соответствует номинал полученного серийного номера";
		...
		Если НЕ СоставОбъекта.МагазинНаличия = Магазин Тогда
			...
			Отказ = Истина;
			ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", Символы.ПС)
			+ "Сертификат находится в другом магазине «" + СоставОбъекта.МагазинНаличия + "»";
		...
		Если ЗначениеЗаполнено(СоставОбъекта.Состояние) Тогда
			...
			Отказ = Истина;
			ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", Символы.ПС)
			+ "Сертификат находится в состоянии «" + СоставОбъекта.Состояние + "»";
		...
		Если НЕ СоставОбъекта.ДатаОкончанияДействия = '00010101' И СоставОбъекта.ДатаОкончанияДействия < ТекущаяДата() Тогда
			...
			Отказ = Истина;
			ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", Символы.ПС)
			+ "Сертификат просрочен (действие до " + Формат(СоставОбъекта.ДатаОкончанияДействия, "ДФ=dd.MM.yyyy") + ").";
		...
		Если НЕ ТоварыСерийныеНомера.НайтиПоЗначению(СоставОбъекта.СерийныйНомер) = Неопределено Тогда
			...
			Отказ = Истина;
			ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", Символы.ПС)
			+ "Сертификат «" + СоставОбъекта.СерийныйНомер + "» УЖЕ имеется в текущем чеке";
	...
	Иначе
		...
		ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", Символы.ПС)
		+ "Не найден сертификат по коду «" + Штрихкод + "»";

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.СкидочныйКупон Тогда
	    Заголовок = НСтр("ru = 'Ввести скидочные купоны для номенклатуры'") + " """ + Параметры.ПредставлениеНоменклатуры + """"; 
		Элементы.СерийныйНомер.Заголовок = Строка("Номер скидочного купона");
	КонецЕсли;
	...
	Если Параметры.УдалениеСертификата Тогда
		...
		Элементы.СерийныйНомер.Заголовок = Строка("Удалите лишний сертификат");
	КонецЕсли;
	// - HVOYA 30.11.2016 19:17:32, Латышев А.А. 

...
//	\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\/
	...
	Элементы.РасширенныйПериодескийОтчет.Видимость = РаботаетПРРО(РабочееМесто, Магазин);
...
Процедура ПриСозданииНаСервереЗавершение(Отказ, СтандартнаяОбработка)
	...
	КодПоЕДРПОУ = ОбщегоНазначенияРТповтИсп.РеквизитыОрганизации(ПараметрыСеанса.ТекущаяОрганизация).КодПоЕДРПОУ;
...
#КонецОбласти
 ...
 &НаСервере
 Процедура ПереустановитьПараметрыСеанса(Магазин) 
	Если ЗначениеЗаполнено(ПараметрыСеанса.ТекущаяОрганизация) Тогда
		Возврат;	
	КонецЕсли;
	...
	Если Рез_.Следующий() Тогда
		ОбщегоНазначенияРТСервер.ПереустановитьПараметрыСеанса(Магазин,Рез_.Организация);
	КонецЕсли;
КонецПроцедуры
...
Процедура МагазиныВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда
		...
		//2025-08-11 sa Это нужно только для центральной базы и то, если не установились параметры сеанса (Магазин и Организация)
		ПереустановитьПараметрыСеанса(ТекущиеДанные.Магазин);
...
Процедура КомандаПериодическийОтчетПоНомерам(Команда)
	...
	Если ПроверитьОбщуюВозможностьРаботы(ПараметрыИнформации) Тогда
		...
		Если Кассы.Количество() = 0 Тогда
			...
			ЗаголовокИнформации	= "Помилка друку періодичного звіту!";
			ТекстИнформации		= "немає визначення доступних кас.";
...
Процедура КомандаВыборПериода(Команда)
	...
	ПолучитьИнтерактивноПериод("Вкажіть період звіту", ПериодПериодическогоОтчета);
...
Функция ПроверитьОбщуюВозможностьРаботы(ПараметрыИнформации)
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ТаблицаСправочник.Владелец = &Организация
	|		КОНЕЦ"
	);
	Запрос.УстановитьПараметр("Магазин"			, Магазин);
...
Процедура ОбновитьЗаголовокФормы()
	...
	Иначе	Заголовок = "(" + Формат(НомерМагазина, "ЧЦ=3; ЧВН=; ЧГ=") + "/" + СокрЛП(КодПоЕДРПОУ) + ")РМК: " + РабочееМесто;
		...
		//	Подключение устройства
			Если НЕ ИдентификаторУстройства = Неопределено Тогда
				...
				Если Результат Тогда
					...
					ПараметрыКассыККМ.РасширенныйПериодескийОтчет = РасширенныйПериодескийОтчет;	//	LNK 03.01.2024 08:57:20

					ВходныеПараметры  = Новый Массив(3);
					ВыходныеПараметры = Неопределено;

					ВходныеПараметры[0] = ПериодПериодическогоОтчета.ДатаНачала;
					ВходныеПараметры[1] = ПериодПериодическогоОтчета.ДатаОкончания;
		...
		//	Подключение устройства
			Если ИдентификаторУстройства <> Неопределено Тогда
				...
				Если Результат Тогда
					...
					ВходныеПараметры[0] = НачальныйНомерПериодическогоОтчета;
					ВходныеПараметры[1] = КонечныйНомерПериодическогоОтчета;
					ВходныеПараметры[2] = Новый Массив(22);
					ВходныеПараметры[2][21] = ПараметрыКассыККМ;
...
&НаСервереБезКонтекста	//	LNK 03.01.2024 08:18:26
Функция РаботаетПРРО(РабочееМесто, Магазин)

	ТаблицаКассККМ = РМКСервер.ПолучитьТаблицуКассПоРабочемуМесту(РабочееМесто, Магазин);

	ЭтоПРРО = Ложь;

	Если НЕ ТаблицаКассККМ.Количество() = 0 Тогда

		ПараметрыКассыККМ = ЗначениеНастроекПовтИсп.ПолучитьПараметрыКассыККМ(ТаблицаКассККМ[0].КассаККМ);	//	LNK 03.01.2024 08:07:26

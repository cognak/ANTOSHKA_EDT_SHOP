...
Процедура ПриОткрытии(Отказ)
	...
	Иначе
		...
		Иначе
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК("Магазин не указан", "Необходимо определить магазин");
			Возврат;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ РазрешеноЗакрытие Тогда
		...
		Отказ = Истина;
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК("В действии отказано!", "Воспользуйтесь клавишами Alt+F4");
...
КонецПроцедуры // ПолучитьНазначениеКассыККМ()
...
//	LNK 02.06.2017 15:09:43
	Если НЕ ЗапретПродажПроверен Тогда
		...
		Если НельзяПродавать(СтрокаТабличнойЧасти.Номенклатура, ТекстЗапретаПродаж) Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Запрет продаж!'"), ТекстЗапретаПродаж);
...
//	УчетХарактеристик = ФлагУчетаХарактеристик(СтрокаТабличнойЧасти.Номенклатура);
	...
	Если ВыполнитьУстановкуЦены Тогда
		...
		Если НЕ ПолучитьРазрешениеИзмененияЦены(СтрокаТабличнойЧасти.Номенклатура) тогда	// -HVOYA. 24.08.2016 1:03:15, Львова Е.А.
			...
			Если СтрокаТабличнойЧасти.Цена = 0 И СтрокаТабличнойЧасти.Номенклатура <> Неопределено Тогда	
				...
				ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Цена не установлена!'"), ТекстСообщения)
		...
		ИначеЕсли СтруктураПараметров.Действие = "ПроверкаКарт" Тогда
			...
			Если СтруктураПараметров.ЗначенияПоиска.Количество() > 0 Тогда
				...
				Если РеквизитыКарты.ПометкаУдаления Тогда
                    ...
                    ТекстСообщения = НСтр("ru = 'Карта %КодКарты% недействительна!'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КодКарты%", РеквизитыКарты.ИнформационнаяКарта);
                    СтруктураПараметровКлиента.Вставить("КартаБлокирована", ТекстСообщения);
                    СтруктураПараметровКлиента.Вставить("ПрекратитьПоиск");
				...
				Если РеквизитыКарты.ТипКарты = Перечисления.ТипыИнформационныхКарт.Регистрационная Тогда
					...
					Если ТипЗнч(РеквизитыКарты.ВладелецКарты) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
						...
						Иначе
							...
							Объект.ПродавецТекущий = РеквизитыКарты.ВладелецКарты;
...
Процедура КомандаПО04ОтложитьЧек(Команда)
	...
	Если ТестовыйРежимРМК Тогда
		...
		ЗаголовокИнформации = НСтр("ru = 'РМК работает в тестовом режиме!'"); 
		ТекстИнформации = НСтр("ru = 'ОТКАЗАНО!" + Символы.ПС + "Чек, сформированный в режиме тестирования, не может быть отложен.'");
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
	...
	//	LNK 01.11.2017 10:13:10
		ЗаголовокИнформации = НСтр("ru = 'Чек отложен!'"); 
		ТекстИнформации = НСтр("ru = 'Информация для справки:" + Символы.ПС + ПредставлениеЧека + "'");
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
	...
	Иначе
		...
		ЗаголовокИнформации = НСтр("ru = '" + НазваниеЗаказа() +"'"); 
		ТекстИнформации = НСтр("ru = 'ОТКАЗАНО!" + Символы.ПС + "Чек, сформированный по внешнему документу, не может быть отложен. Допустимы пробитие либо аннуляция текущего чека.'");
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
...
Процедура КомандаПО05ВвестиАннулирование(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(НСтр("ru = 'Аннулировать чек?'"));
...
Процедура КомандаПО06ВвестиШтрихкод(Команда)
	КодЗначение = 0;
	Если ПолучитьИнтерактивноЧисло(НСтр("ru = 'Введите штрихкод.'"), КодЗначение, 0, Ложь, Ложь) Тогда
...
Процедура КомандаПО07ОткрытьДенежныйЯщик(Команда)
	...
	Если Не ИспользоватьПодключаемоеОборудование Тогда
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Подключаемое оборудование не используется.'"), 
																	НСтр("ru = 'Денежный ящик не может быть открыт."));
		Возврат;
		...
		Если ИдентификаторУстройстваФР <> Неопределено Тогда
			...
			Если РезультатФР Тогда
				...
				Если Не РезультатФР Тогда
					...
					ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'При открытии денежного ящика возникла ошибка:'"), 
																				ВыходныеПараметры[1])
			...
			Иначе
				...
				ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'При подключении фискального регистратора произошла ошибка:'"), 
																			ОписаниеОшибки)
				
			КонецЕсли;
		КонецЕсли;
	Иначе
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Фискальный регистратор не подключен'"))
	КонецЕсли;
...
Процедура КомандаПО09Печать(Команда)
	...
	Если Объект.Товары.Количество() = 0 тогда
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Чек не заполнен.'"));
		Возврат;
	...
	ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(ТекстВопроса);
...
Процедура КомандаПО10НапечататьПоследнийСлипЧек(Команда)
	...
	Если Не ИспользоватьПодключаемоеОборудование Тогда
		...
		ТекстСообщения = НСтр("ru = 'Подключаемое оборудование не используется. Слип-чек не может быть напечатан.'"); 
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда
		...
		Если ИдентификаторУстройстваФР <> Неопределено Тогда
			...
			Если РезультатФР Тогда
				...
				МенеджерОборудованияКлиент.ОтключитьОборудованиеПоИдентификатору(УникальныйИдентификатор,
				                                                                 ИдентификаторУстройстваФР);
			Иначе
				ЗаголовокИнформации = НСтр("ru = 'При подключении фискального регистратора произошла ошибка:'");
					ТекстСообщения = НСтр("ru = '""%ОписаниеОшибки%"".
		...
		КонецЕсли;
	Иначе
		ТекстСообщения = НСтр("ru = 'Фискальный регистратор не подключен'");
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ТекстСообщения);
	КонецЕсли;
...
Процедура КомандаПО12ПолучитьВес(Команда)
	Если Не ИспользоватьПодключаемоеОборудование Тогда
		...
		ЗаголовокИнформации = НСтр("ru = 'Подключаемое оборудование не используется.'");
		ТекстИнформации = НСтр("ru = 'Вес не может быть получен.'"); ;
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
	...
	ОписаниеОшибки = НСтр("ru = 'Неопознанная ошибка.'");
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	
	Если ТекущиеДанные = Неопределено Тогда
		
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru='Необходимо выбрать строку, для которой нужно получить вес.'"));
	...
	ИначеЕсли МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда // Проверка на определенность рабочего места ВО
			...
			Если Результат Тогда
				...
				Если Результат Тогда
					Если ВыходныеПараметры[0] = 0 Тогда
						...
						|Электронных весы вернули нулевой вес.'");
		...
		Иначе
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru='Весы не подключены.'"));
...
//	LNK 21.09.2017 14:33:47
	...
	ПараметрыОткрытия.Вставить("НадписьПустогоСписка", НСтр("ru = 'Список отложенных чеков пуст!'"));
	...
	Если ВыборИнформационнойКартыТолькоПоКоду Тогда	// - HVOYA 30.09.2016 14:19:19, Латышев А.А. 
		...
		Если Команда = "СчитатьКарту" или Команда.Имя = "СчитатьКарту" Тогда
			...
			Если НЕ СписокОборудования.Количество() = 0 тогда   
				...
				ПоказатьОповещениеПользователя("Проводите картой по считывателю магнитных карт!",,,,,);	

			Иначе	

				Если ПолучитьИнтерактивноЧисло(НСтр("ru = 'СчитатьКарту'"), КодЗначение, 0, Ложь, Истина) Тогда
		...
		Иначе
			...
			Если ПолучитьИнтерактивноЧисло(НСтр("ru = 'Введите магнитный код'"), КодЗначение, 0, Ложь, Истина) Тогда
...
Процедура КомандаПО20ЗаменаКарты(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ДисконтнаяКарта) Тогда
		ПараметрыОткрытия.Вставить("КартаИсточник", Объект.ДисконтнаяКарта);
	ИначеЕсли ЗначениеЗаполнено(ДисконтнаяКартаДляЗамены) Тогда
		ПараметрыОткрытия.Вставить("КартаИсточник", ДисконтнаяКартаДляЗамены);
	Иначе
		ПараметрыОткрытия.Вставить("КартаИсточник", ПредопределенноеЗначение("Справочник.ИнформационныеКарты.ПустаяСсылка"));
	КонецЕсли;
	
	Результат = ОткрытьФормуМодально(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаДокументаЗаменаОднойКарты", Объект.ИмяВнешнейОбработки), ПараметрыОткрытия, УникальныйИдентификатор);
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Объект.ДисконтнаяКарта = Результат;
	Модифицированность = Истина;
...
Процедура КомандаПО21ПробитьЧек(Команда)
	...
	ПараметрыОткрытия.Вставить("НадписьПустогоСписка", НСтр("ru = 'Список непробитых чеков пуст!'"));
	ПараметрыОткрытия.Вставить("Магазин", Объект.Магазин);
...
Процедура ПробитьНепробитыйЧекЗавершение(Результат, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Если ПроверитьОплатуИВыполнитьОкруглениеЧекаПоСсылке(ЧекККМСсылка, ПараметрыИнформации) Тогда
			...
			Отказ = НЕ ПробитьЧекККМ(ЧекККМСсылка, КассаККМ, ПараметрыЗаполненияЧекаККМ);
...
Функция ПроверитьОплатуИВыполнитьОкруглениеЧекаПоСсылке(ЧекККМСсылка, ПараметрыИнформации)
	...
	Если НЕ Результаты[1].Пустой() Тогда
		...
		Если Выборка.СуммаОплаты < Выборка.СуммаДокумента Тогда
			...
			ПараметрыИнформации.ЗаголовокИнформации = "Операция невозможна!";
			ПараметрыИнформации.ТекстИнформации     = "Сумма оплаты, указанная в «" + ЧекККМСсылка + "», МЕНЬШЕ суммы документа.";
	...
	Иначе
		...
		ПараметрыИнформации.ЗаголовокИнформации = "Данные не обнаружены!";
		ПараметрыИнформации.ТекстИнформации     = "Получение данных выбранного чека «" + ЧекККМСсылка + "» НЕ увенчалось успехом.";
...
Процедура КомандаПО24ПолучитьИзТСД(Команда)
	...
	Если Не ИспользоватьПодключаемоеОборудование Тогда
		ЗаголовокИнформации = НСтр("ru = 'Подключаемое оборудование не используется.'"); 
		ТекстИнформации     = НСтр("ru = 'Данные не могут быть получены.'"); 
...
Процедура КомандаПО25ПечатьПробитогоЧека(Команда)
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("МассивСтруктурОтбора", МассивСтруктурОтбора);
	ПараметрыОткрытия.Вставить("НадписьПустогоСписка", НСтр("ru = 'Список пробитых чеков пуст!'"));
	ПараметрыОткрытия.Вставить("Магазин", Объект.Магазин);
...
Процедура КоманднаяПанельСкидкиНажатие(Команда)
	...
	ЗаголовокСообщения = НСтр("ru = 'Ограничение применения ручных скидок.'");
	...
	Если ЗаказПокупателяОплачен(Объект.ЗаказПокупателя) Тогда	//	HVOYA Mykhailo
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокСообщения
			, НСтр("ru = 'Интернет-заказ ОПЛАЧЕН! Ручные скидки недоступны!'"));
		Возврат;
...
//	LNK 30.08.2017 10:51:43
	Если НЕ Объект.ДоговорСтрахования.Пустая() Тогда
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокСообщения
			, НСтр("ru = 'Отказано. Скидки в режиме оплаты договора страхования не предусмотрены!'"));
		Возврат;
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат") Тогда
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокСообщения
			, НСтр("ru = 'Отказано. Ручные скидки в режиме возврата товаров запрещены!'"));
		Возврат;
	...
	Если НЕ Объект.УслугаБанка.Пустая() Тогда
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокСообщения
			, НСтр("ru = 'Отказано. Скидки в режиме банковской услуги не предусмотрены!'"));
		Возврат;
...
Процедура ВыбратьПричинуРучнойСкидкиЗавершение(ПричинаРучнойСкидки, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ ПричинаРучнойСкидки  = Неопределено Тогда
		...
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("Заголовок"  , "Опишите факт.причину предоставления скидки");
		ПараметрыОткрытия.Вставить("Приглашение", "Причина скидки");
		...
		ОткрытьФорму(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаВводаСтроки", Объект.ИмяВнешнейОбработки), ПараметрыОткрытия, ЭтотОбъект, ЭтотОбъект,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДействияПослеВводаПричиныРучнойСкидки(ОписаниеПричины, ДополнительныеПараметры)	Экспорт
	...
	//	Определяем, как отмену воздействия.
		Сообщить("Отмена воздействия пользователем!");
		Возврат;
	...
	//	Булево присваивается в случае выбора пользователем пустой строки... недопустимо!
		ТекстСообщения = "Описание причины ручной скидки является обязательным! Без такого описания ручная скидка назначена быть не может - в назначении ручной скидки отказано";
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК("Контроль описания причины ручной скидки", ТекстСообщения);
		Возврат;
...
Процедура hiОткрытьИнформациюОСкидках(Команда)
	...
	Если НаборПравИНастроек.ПродажиАктивированы Тогда
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
			"Отказано! Информация о скидках недоступна",
			"Скидки назначаются во внешней системе (CRM)"
		);
...
Процедура УстановитьОформлениеЭлементов()
	...
	Элементы.ДекорацияПроцентСкидки.Заголовок = ?(НаборПравИНастроек.ПрограммаЛояльности = ПредопределенноеЗначение("Перечисление.ВидыПрограммЛояльности.БонуснаяСистемаCRM")
												, "БОНУСЫ", "Процент скидки:");
...
&НаСервере
Функция ФлагУчетаХарактеристик(Номенклатура)
	...
	//КонецЕсли;
	//
	//Возврат НЕ (ВидНоменклатуры.ИспользованиеХарактеристик = Перечисления.ВариантыВеденияДополнительныхДанныхПоНоменклатуре.НеИспользовать);

//	LNK 02.06.2017 12:37:56 - пока характеристики не используем!
	Возврат Ложь;
	
КонецФункции // ФлагУчетаХарактеристик()
...
//	Проверить необходимость объединять товары
	...
	Если НЕ МассивТоваров.Количество() = 0 Тогда
		...
		И НЕ ТекущаяСтрока.ИспользоватьСерийныеНомера	Тогда
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
				НСтр("ru = 'Купон уже указан!'"),
				"Отказано!
				|Купон Б/Н «" + Номенклатура + "» уже указан в текущем чеке.");
			Возврат;
...
Процедура УстановитьЗаголовок()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат") Тогда
		...
		ДекорацияЗаголовок = ?(ТестовыйРежимРМК, "ТЕСТОВЫЙ РЕЖИМ: ", "")
			+ "ВОЗВРАТ"
			+ ?(ЗаказПокупателяЗаполнен(Объект.ЗаказПокупателя), " по " + ПредставлениеДокумента(Объект.ЗаказПокупателя, НазваниеЗаказа())
			, ?(ЗначениеЗаполнено(Объект.ЧекККМПродажа)  , " по " + ПредставлениеДокумента(Объект.ЧекККМПродажа, "ПРОДАЖА"), ""));

	ИначеЕсли ЗаказПокупателяЗаполнен(Объект.ЗаказПокупателя) Тогда

		ДекорацияЗаголовок = ?(ТестовыйРежимРМК, "ТЕСТОВЫЙ РЕЖИМ: ", "") + ПредставлениеДокумента(Объект.ЗаказПокупателя, НазваниеЗаказа());

	Иначе

		ДекорацияЗаголовок = ?(ТестовыйРежимРМК, "ТЕСТОВЫЙ РЕЖИМ: ","")
			+ "!!! F5 - СЧИТАТЬ ДИСКОНТ.КАРТОЧКУ !!! "
			+ "ПРОДАЖА";
...
Функция НазваниеЗаказа()
	...
	Возврат ?(ТипЗнч(Объект.ЗаказПокупателя) = Тип("ДокументСсылка.ЗаказПокупателя"), "ИНТЕРНЕТ-ЗАКАЗ",
			?(ТипЗнч(Объект.ЗаказПокупателя) = Тип("ДокументСсылка.СчетНаОплатуПокупателю"), "СЧЁТ НА ОПЛАТУ", ""));
...
//	Проверимся на всякий случай - имеется ли такой ключ в структуре? Мало-ли откуда вызов прилетит ...
	...
	Если НЕ ПустаяСтрока(ПричинаПроблемы) Тогда
		...
		СтрокаСообщения = НСтр("ru = 'Не разрешено использовать «[%1%] %2%» для магазина No.%3% («%4%») по причине:" + Символы.ПС + "%5%'");
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК("Позиция не разрешена к продаже!", СтрокаСообщения);
...
Процедура ВывестиИнформациюНаДисплейПокупателя(Команда)
	...
	Если НЕ ЗначениеЗаполнено(НаборПравИНастроек.ШаблонРаботыДисплеяПокупателя) Тогда
		...
		ВыводитьИтоги = Ложь;
		ТекстПриПростое = НСтр("ru = 'ЛАСКАВО ПРОСИМО!'");
		ТекстВНерабочемСостоянии = НСтр("ru = 'КАСА НЕ ПРАЦЮЄ'");
	...
	ИначеЕсли Команда = "Оплата" Тогда
		...
		Строка2 = НСтр("ru = '  РЕШТА: '") + ДисплейПокупателяПараметры.Сдача;
	...
	ИначеЕсли Команда = "ОжиданиеОплаты" Тогда
		...
		Если ПустаяСтрока(ДисплейПокупателяПараметры.СуммаСкидки) Тогда
			...
			Строка1 = НСтр("ru = 'ВСЬОГО: '") + ДисплейПокупателяПараметры.СуммаВсего;

		Иначе

			Строка1 = НСтр("ru = 'ВСЬОГО: '") + ДисплейПокупателяПараметры.СуммаВсего;
			Строка2 = НСтр("ru = 'ЗНИЖКА: '") + ДисплейПокупателяПараметры.СуммаСкидки;
	...
	ИначеЕсли Команда = "ОтменаОплаты" Тогда
		...
		Строка1 = НСтр("ru = 'ВІДМІНА ОПЛАТИ'");
	...
	ИначеЕсли Команда = "ДобавлениеТовара" или  Команда = "УдалениеТовара" или
		...
		Если ВыводитьИтоги Тогда
			...
			Строка2 = НСтр("ru = 'ВСЬОГО: '") + ДисплейПокупателяПараметры.СуммаВсего;
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоОтрицательнымОстаткам Цикл
		...
		Если ТаблицаТоваров.НайтиСтроки(СтруктураПоиска).Количество() > 0 Тогда
			Если ТаблицаПодарков.НайтиСтроки(СтруктураПоиска).Количество() > 0 Тогда
				СтрокаПозици = НСтр("ru = 'Номенклатура и подарок'");
			Иначе
				СтрокаПозици = НСтр("ru = 'Номенклатура'");
			КонецЕсли;
		Иначе
			СтрокаПозици = НСтр("ru = 'Подарок'");
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации)
	...
	Если ТаблицаЗапроса.Количество() > 0 Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗапроса Цикл
			...
			ИначеЕсли СтрокаТаблицы.ОшибкаСумма Тогда
				...
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В строке №%1 табличной части не заполнена сумма'"),
					СтрокаТаблицы.НомерСтроки
				);
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда
		...
		Если Отказ Тогда
			ПараметрыИнформации.ЗаголовокИнформации = НСтр("ru = 'Ошибка возврата товара'");
			ПараметрыИнформации.ТекстИнформации = СтрокаОшибки;
			Возврат;
		КонецЕсли;
	...
	//	информация об этом ещё не успела доехать до ЦБ..
		...
		Если Отказ Тогда
			...
			ПараметрыИнформации.ЗаголовокИнформации = НСтр("ru = 'Ошибка продажи подарочного сертификата'");
			ПараметрыИнформации.ТекстИнформации = СтрокаОшибки;
		...
		Если Отказ Тогда	// +HVOYA. 30.08.2016 13:54:58, Львова Е.А.
			...
			ПараметрыИнформации.ЗаголовокИнформации = НСтр("ru = 'Ошибка погашения скидочного купона'");
			ПараметрыИнформации.ТекстИнформации = СтрокаОшибки;
			Возврат;
...
Функция ПроверитьДопустимостьПричинРучныхСкидок(Отказ, ПараметрыИнформации)
	...
	Если ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьЛимитыРучныхСкидок") Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			...
			ПараметрыИнформации.ЗаголовокИнформации = НСтр("ru = 'Контроль ограничения использования ручных скидок'");
			Выборка = Результат.Выбрать();
			...
			Пока Выборка.Следующий() Цикл
				...
				Отказ = Истина;
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В строке №%1 ручная скидка «%2» суммы разрешения недостаточно на %3 грн'"),
					Выборка.НомерСтроки,
...
Процедура ЗаполнитьТаблицуЧековИПроверитьСоотвествиеОрганизаций(Отказ)
	...
	ИначеЕсли ТабличноеПолеЧеков.Количество() = 0 Тогда
		...
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Не правильно заполненны склады реализации'"));
...
Функция ПроверкаСогласияПокупателяНаПотерюСуммыПоПодарочнымСертификатам(ПроверкаВРазрезеОрганизаций, ПараметрыИнформации)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Потери покупателя от недобора товара составляют - %1 грн.'"),
...
Процедура ПровестиЧекККМ(ЧекККМСсылка, ПараметрыЗаполненияЧекаККМ, МассивСсылокЧеков, ВывестиДополнительноеСообщение, ПараметрыИнформации)
	...
	Если НЕ ОбъектЧекККМ.СтатусЧекаККМ = Перечисления.СтатусыЧековККМ.Пробитый Тогда
		...
		Иначе	ПараметрыИнформации.ЗаголовокИнформации = НСтр("ru = 'Чек не пробит. '");

		КонецЕсли;

		ПараметрыИнформации.ТекстИнформации = НСтр("ru = 'Чек нужно пробить позднее или аннулировать перед закрытием смены'");
...
&НаКлиенте	//	LNK 30.09.2019 10:41:58
Процедура ВыполнитьДействияПередПробитиемКлиент(Отказ)

	Если Отказ ИЛИ НЕ НаборПравИНастроек.ПродажиАктивированы Тогда
	...
	Для каждого СтрокаУдалить Из Объект.ПогашениеСкидочныхКупонов.НайтиСтроки(Новый Структура("Принят", Ложь)) Цикл

		ТекстСообщения = ТекстСообщения + ?(ПустаяСтрока(ТекстСообщения), "", Символы.ПС)
		+ СтрокаУдалить.СерийныйНомер;
	...
	Если НЕ ПустаяСтрока(ТекстСообщения) Тогда
		...
		ПараметрыИнформации = ОбщегоНазначенияРТКлиентСервер.ПолучитьСтруктуруВыводимойВРМКИнформации();
		ПараметрыИнформации.ЗаголовокИнформации = "ВЕРНИТЕ ПОКУПАТЕЛЮ КУПОНЫ";
		ПараметрыИнформации.ТекстИнформации = ТекстСообщения;
...
//
&НаКлиенте
Функция ПробитьЧекККМ(ЧекККМСсылка, КассаККМ, ПараметрыЗаполнения, ПечатьНаПринтер = Ложь) // добавляем опцию "ПечатьНаПринер"
	...
	Если НЕ ИспользоватьПодключаемоеОборудование ИЛИ ПечатьНаПринтер Тогда // если сразу печатать на принтер чеков
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда // Проверка на определенность рабочего места ВО
		...
		//	Подключаем ФР
			...
			//	Готовим данные
				ВходныеПараметры  = ВходныеПараметрыПолучитьНаСервере(ЧекККМСсылка);
				ВыходныеПараметры = Неопределено;
			...
			//	Печатаем чек.
				...
				Иначе
					...
					ТекстСообщения = НСтр("ru = 'Чек не напечатан на фискальном регистраторе.
			...
			Иначе
				...
				ЗаголовокИнформации = НСтр("ru = 'При подключении устройства произошла ошибка.'");
				ТекстСообщения = НСтр("ru = 'Чек не напечатан на фискальном регистраторе.
				                            |Дополнительное описание:
				                            |%ДополнительноеОписание%'");
		...
		Иначе
			...
			ЗаголовокИнформации = НСтр("ru = 'Не выбран фискальный регистратор для кассы «" + СокрЛП(КассаККМ) + "».'");
	...
	Иначе
		...
		ЗаголовокИнформации = НСтр("ru = 'Предварительно необходимо выбрать рабочее место внешнего оборудования текущего сеанса.'");
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации);
...
Функция АнулироватьЧекККМ(КассаККМ, ПараметрыЗаполнения)
	...
	Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда // Проверка на определенность рабочего места ВО
		...
		Если ИдентификаторУстройстваФР <> Неопределено Тогда
			...
			Если Результат Тогда   
				...
				Если Результат Тогда
					...
					ЧекАнулирован = Истина;
				Иначе
					ЗаголовокИнформации = НСтр("ru = 'При анулировании чека произошла ошибка.'");
					
					ТекстСообщения = НСтр("ru = 'Чек не анулирован на фискальном регистраторе.
												|Дополнительное описание:
												|%ДополнительноеОписание%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения,
												 "%ДополнительноеОписание%",
		...
		Иначе
			ЗаголовокИнформации = НСтр("ru = 'Не выбран фискальный регистратор.'");
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации);
		КонецЕсли;
...
КонецФункции // АнулироватьЧекККМ()
...
Процедура СоздатьЧеки(Отказ, Печать = Ложь)
	...
	Попытка
		ПечатьНаПринтерЧеков = ОбщегоНазначенияРТСервер.ПолучитьЗначениеРеквизитаВПривилегированномРежиме(Объект.КассаККМ, "ПечатьЧековНаПринтер");
	Исключение
	...
	Если ТестовыйРежимРМК Тогда
		...
		ЗаголовокИнформации = НСтр("ru = 'Тестовый режим РМК'"); 
		ТекстИнформации = НСтр("ru = 'ОТКАЗАНО!" + Символы.ПС + "Чек, сформированный в режиме тестирования, не может быть пробит.'");
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат") Тогда
		...
		Если Отказ Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Ошибка возврата товара'"), Ответ);
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если Отказ Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Ошибки погашения подарочных сертификатов'"), Ответ);
			Возврат;
		КонецЕсли;
	...
	Если ТабличноеПолеЧеков.Количество() > 1 Тогда
		...
		Для каждого СтрокаТабличногоПоляЧеков Из ТабличноеПолеЧеков Цикл
			...
			ВыполнитьДействияПередПробитиемКлиент(Отказ);	//	LNK 30.09.2019 10:40:31
		//	LNK 30.09.2019 10:38:31	- ПЕРЕД пробитием.
			...
			Отказ = НЕ ПробитьЧекККМ(ЧекККМСсылка, СтрокаТабличногоПоляЧеков.КассаККМ, ПараметрыЗаполненияЧекаККМ, ПечатьНаПринтерЧеков); // добавлено ПечатьНаПринтерЧеков
	...
	//	LNK 30.09.2019 10:38:31	- ПЕРЕД пробитием.
		...
		Отказ = НЕ ПробитьЧекККМ(ЧекККМСсылка, ТабличноеПолеЧеков[0].КассаККМ, ПараметрыЗаполненияЧекаККМ, ПечатьНаПринтерЧеков); // добавлено ПечатьНаПринтерЧеков
		...
		Иначе
			...
			Попытка
				...
				ПроверитьАкцииНаПечатьУникальныхКупонов(ЧекККМСсылка, ТабличноеПолеЧеков[0].КассаККМ);
	...
	Если ПечатьНаПринтерЧеков Тогда	//+HVOYA Mykhailo : печать чеков на принтер
		Если НЕ Отказ Тогда
			Попытка
	...
	Исключение
		ЗафиксироватьОшибку(ОписаниеОшибки(), "СоздатьЧеки");	//	LNK 04.09.2018 14:16:54
	КонецПопытки;
	// + HVOYA 02.08.2017 22:31:56, Латышев А.А.
	hiПроверитьЛЖ();
	// - HVOYA 02.08.2017 22:31:56, Латышев А.А. 
	ВыйтиИзРежимаИЗ();
	//-HVOYA Mykhailo
	
КонецПроцедуры // СоздатьЧеки()
...
Процедура НапечататьЧекиККМ(МассивСсылокЧеков)
	...
	ОткрытьФорму(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаВыбораПечатныхФорм", Объект.ИмяВнешнейОбработки), ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	Для каждого ЧекККМСсылка Из МассивСсылокЧеков Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтруктураПараметровПечати.ПодключаемоеОборудование) 
		 ИЛИ НЕ ЗначениеЗаполнено(СтруктураПараметровПечати.ШиринаЛенты)  Тогда
			...
			ЗаголовокИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проверьте настройку кассы ККМ %1'"),
				СтруктураПараметровПечати.Ссылка,
			);
			ТекстИнформации = НСтр("ru = 'Не задана ширина ленты или подключаемое оборудование'"); 
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
		...
		Если НЕ ИдентификаторУстройстваФР = Неопределено Тогда
			...
			Если Результат Тогда
				...
				Если НЕ РезультатФР Тогда
					ЗаголовокИнформации = НСтр("ru = 'При печати товарного чека произошла ошибка.'");
					
					ТекстСообщения = НСтр("ru = 'Товарный Чек не напечатан на фискальном регистраторе.
					                            |Дополнительное описание:
					                            |%ДополнительноеОписание%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения,
					                             "%ДополнительноеОписание%",
...
Процедура ИзменитьПроцентРучнойСкидкиНаценкиВСтроке(ЭтоНаценка = Ложь)
	...
	Если ВозможностьРаботыСРучнымиСкидкамиВСтроке() Тогда
		...
		Если ПолучитьИнтерактивноЧисло(ЗаголовокФормы, 
										ЧислоВвода,
...
Процедура ПоискПоНомеруТелефонаКлиент(БуфферПоляВвода)
	...
	Если НЕ ПустаяСтрока(ТекстСообщения) Тогда	//	LNK 18.07.2019 10:19:04
		...
		Если НЕ ПустаяСтрока(ДанныеКонтрагента.НомерТелефона) И ДанныеКонтрагента.ИнтерактивныйВводНового = Истина Тогда
            ...
            ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(
				"Вы можете создать нового клиента по номеру телефона «" + ДанныеКонтрагента.НомерТелефона + "»", НСтр("ru = 'Покупатель согласен?'"), "Нет");
	...
	Элементы.ПоискШК.ПодсказкаВвода = "введите штрихкод товара";		
	ПоискШК = "";	
...
Процедура РМК_СчетНаОплатуПокупателю(Команда)
	...
	Если НЕ ЗаказПокупателяЗаполнен(Объект.ЗаказПокупателя) Тогда
		...
		Если Объект.Товары.Количество() > 0 Тогда
			...
			ЗаголовокИнформации = НСтр("ru = 'Счёт на оплату покупателю'"); 
			ТекстИнформации = НСтр("ru = 'Необходимо пробить, аннулировать или отложить текущий чек!'");
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
...
Процедура ПерейтиВРежимЗаказПокупателя(Команда)
	...
	Если НЕ ЗаказПокупателяЗаполнен(Объект.ЗаказПокупателя) Тогда
		...
		Если Объект.Товары.Количество() > 0 Тогда	//-HVOYA Mykhailo 2017-01-18
			...
			ЗаголовокИнформации = НСтр("ru = 'Интернет-заказ'"); 
			ТекстИнформации = НСтр("ru = 'Необходимо пробить, аннулировать или отложить текущий чек!'");
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
			УстановитьТекущийЭлемент();	//	LNK 13.09.2017 08:17:04
			Возврат;
...
Процедура РМК_Просмотр(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ЗаголовокИнформации = НСтр("ru = 'Просмотр чека'"); 
		ТекстИнформации = НСтр("ru = 'Перед просмотром чека необходимо пробить, аннулировать или отложить текущий чек!'");
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстИнформации);
		Возврат;
...
Процедура ВыборЧекаДляПросмотраЗавершение(Значение, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ Значение = Неопределено Тогда
		...
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ЧекДляПросмотра", Значение.ЧекККМ);

		ОписаниеОповещения = Новый ОписаниеОповещения("ПросмотрЧекаЗавершение", ЭтотОбъект);
		
		ОткрытьФорму(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаПросмотраЧека", Объект.ИмяВнешнейОбработки), ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьОформлениеКнопкиБанковскойУслуги(ТипУслуги, ВывестиСообщение = Ложь)
	...
	Если ДанныеМагазина.ИспользоватьБанковскиеУслуги = Истина Тогда
		...
		Если ВывестиСообщениеКассиру И ВывестиСообщение Тогда
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
				НСтр("ru = 'УСЛУГА БАНКА «" + ТекстЗаголовка + "»'"), ТекстИнформации);
...
Процедура РМК_Аннуляция(Команда)
	...
	ИЛИ ЗаказПокупателяОплачен(Объект.ЗаказПокупателя) Тогда
...
Процедура ВыборЧекаДляВозвратаЗавершение(Результат, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Если ЭтоВозвратныйЧек(Результат.ЧекККМ) Тогда	//+HVOYA Mykhailo : если выбран возвратный чек, выдаем предупреждение
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
				НСтр("ru = 'Ввод чека возврата'"),
				НСтр("ru = 'ОТКАЗАНО! Документ «" + Результат.ЧекККМ + "» является чеком ВОЗВРАТА. Недопустимо создание чека возврата на основании чека возврата.'"));
		...
		ИначеЕсли Не СогласованВозвратСБанком(Результат.ЧекККМ) = Неопределено И Не СогласованВозвратСБанком(Результат.ЧекККМ) Тогда	
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
				НСтр("ru = 'Ввод чека возврата'"),
				НСтр("ru = 'ОТКАЗАНО! Документ «" + Результат.ЧекККМ + "» является чеком ОПЛАТЫ ЧАСТЯМИ АЛЬФА-БАНК. Не согласована заявка возврата с банком.'"));
			УстановитьТекущийЭлемент();	//	LNK 02.03.2018 09:34:59
...
&НаКлиенте
Процедура РМК_МЕНЮ(Команда)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("РабочееМесто"  , РабочееМесто);
	ПараметрыФормы.Вставить("мИмяКомпьютера", мИмяКомпьютера);
	ФормаДопМеню = ПолучитьФорму("Обработка.РМК.Форма.ФормаДополнительногоМеню", ПараметрыФормы, УникальныйИдентификатор);
	ФормаДопМеню.ТекКассаККМ = Объект.КассаККМ;
	ФормаДопМеню.ОткрытьМодально();
...
Процедура ЗакрытьРМК()
	...
	Если НаборПравИНастроек.ПродажиАктивированы И ПустаяСтрока(ТекстИнформации) И Объект.ПогашениеСкидочныхКупонов.Количество() > 0 Тогда
		...
		ТекстИнформации = НСтр("ru = 'Перед выходом необходимо вернуть покупателю скидочные купоны, после чего аннулировать текущий чек'");
...
Процедура РМК_Отчет(Команда)
	...
	ПараметрыОткрытия = Новый Структура(
		"РабочееМесто, мИмяКомпьютера, ТекКассаККМ"
		, РабочееМесто, мИмяКомпьютера, Объект.КассаККМ);

	ОписаниеОповещения = Новый ОписаниеОповещения("РМК_Отчет_Завершение", ЭтотОбъект);
	ОткрытьФорму(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаРаботыСКассовымАппаратом", Объект.ИмяВнешнейОбработки), ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура АнулироватьВсеТовары(Компьютер)
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.Товары Цикл
		...
		ЗафиксироватьАннуляцию(СтрокаТабличнойЧасти.ПолучитьИдентификатор(), -1, Компьютер);
			...
			Если ЗначениеЗаполнено(КодЗначение)  Тогда
				...
				ИначеЕсли СтруктураПараметровКлиента.Свойство("ДобавитьТоварнуюПозицию") Тогда
					...
					Если Не ЭтоКупон Тогда
						СтрокаСообщения = НСтр("ru = 'Данный штрихкод %1% не является купоном'");
						СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "%1%", КодЗначение);
						ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(СтрокаСообщения);
						Возврат Истина;
					КонецЕсли;
					hiНапечататьСкидочныйКупон(КодЗначение, КассаККМ);
					...
					Возврат Ложь;
				Иначе 
					СтрокаСообщения = НСтр("ru = 'Данный штрихкод %1% не зарегистрирован в базе или не является скидочным купоном! Отложите его и передайте администратору!'");
					СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "%1%", КодЗначение);
					ЗаголовокИнформации = НСтр("ru = 'Ошибка выдачи скидочного купона'");
					ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации,СтрокаСообщения);
					Возврат Истина;
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли;
	...
	Если ВозможностьРаботыСРучнымиСкидкамиВЧеке() Тогда
        ...
        Иначе
            ...
            Если Результат.Отказ Тогда
                ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(НСтр("ru = 'Ошибки погашения подарочных сертификатов'"), Результат.ТекстОшибки);
            ...
            Если Разница > 0  Тогда
                ...
                ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(Текст, НСтр("ru = 'Покупатель согласен?'"), "Нет");
...
КонецПроцедуры // hiНапечататьСкидочныйКупон()()
...
Процедура hiНапечататьСкидочныйКупон(Штрихкод, КассаККМ)
	...
	Если Результат Тогда
		...
		Если Не РезультатФР Тогда
			ЗаголовокИнформации = НСтр("ru = 'При печати купона возникла ошибка:'");
			ТекстСообщения = НСтр("ru = '""%ОписаниеОшибки%"".'"); 
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, ТекстСообщения);
		КонецЕсли;
...
Функция ПечатьТоварногоЧекаДляПД(МассивОбъектов)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ШиринаЛенты = 0 Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У кассы %1 документ (%2) не задана ширина ленты';uk='У каси %1 документ (%2) не задана ширина стрічки'", КодЯзыкаПечать),
		...
		Если ВыборкаПоДокументам.ЭтоВозврат Тогда
			СинонимДокумента   = НСтр("ru='ЧЕК (Возврат)';uk='ЧЕК (Повернення)'", КодЯзыкаПечать);
		Иначе
			СинонимДокумента   = НСтр("ru='ЧЕК';uk='ЧЕК'", КодЯзыкаПечать);
		КонецЕсли;
...
// + HVOYA 13.07.2017 14:40:33, Латышев А.А.
&НаКлиенте
Процедура ОткрытьЛистЖеланий(Команда)

	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Магазин", Объект.Магазин);

	ОткрытьФорму(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаВыбораДействияСЛистомЖелания", Объект.ИмяВнешнейОбработки)
		, ПараметрыОткрытия, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

КонецПроцедуры

&НаКлиенте
Процедура hiПроверитьЛЖ()
	СтрокаИнформации = "";
	СоздатьДокументыПоЛЖ(СтрокаИнформации);
	Если НЕ СтрокаИнформации = "" Тогда
		ЗаголовокИнформации = "Информация по листам желаний!";
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(ЗаголовокИнформации, СтрокаИнформации);
	КонецЕсли; 
КонецПроцедуры // hiПроверитьЛЖ()

&НаСервере
Процедура СоздатьДокументыПоЛЖ(СтрокаИнформации)
	ТоварыКопия = Объект.Товары.Выгрузить(,"ЛистЖеланий");
	ТоварыКопия.Свернуть("ЛистЖеланий");
	Для каждого Строка Из ТоварыКопия Цикл
		Если Не ЗначениеЗаполнено(Строка.ЛистЖеланий) Тогда
			Продолжить;
		КонецЕсли; 
		
		СтрокаИнформации = СтрокаИнформации + Символы.ПС + Строка(Строка.ЛистЖеланий)+ Символы.ПС;
		Отбор = Новый Структура("ЛистЖеланий",Строка.ЛистЖеланий);
		МассивСтрок = Объект.Товары.НайтиСтроки(Отбор);
		ДокументКорректировкиЛистаЖеланий = Документы.КорректировкаЛистаЖеланий.СоздатьДокумент();
		СтруктураЛистаЖеланий = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Строка.ЛистЖеланий, "Анкета, Корзина");
		ДокументКорректировкиЛистаЖеланий.Анкета = СтруктураЛистаЖеланий.Анкета;
		ДокументКорректировкиЛистаЖеланий.Дата = ТекущаяДата();
		ДокументКорректировкиЛистаЖеланий.Магазин = Объект.Магазин;
		ДокументКорректировкиЛистаЖеланий.Корзина = СтруктураЛистаЖеланий.Корзина;
		ДокументКорректировкиЛистаЖеланий.РедактируемыйДокумент = Строка.ЛистЖеланий;
		...
		Для каждого Элемент из МассивСтрок Цикл
			...
			НоваяСтрока.Операция = Перечисления.ОперацияЛистаЖеланий.Продажа;
		КонецЦикла;
		Попытка
			ДокументКорректировкиЛистаЖеланий.Записать(РежимЗаписиДокумента.Проведение);
			СтрокаИнформации = СтрокаИнформации + "Создан документ " + ДокументКорректировкиЛистаЖеланий.Ссылка + Символы.ПС;
		Исключение
			
			ТекстОшибки = ОписаниеОшибки();
			ЗафиксироватьОшибку(ТекстОшибки, "СоздатьДокументыПоЛЖ");	//	LNK 04.09.2018 14:16:54
			ДокументКорректировкиЛистаЖеланий.Записать(РежимЗаписиДокумента.Запись);
			СтрокаИнформации = СтрокаИнформации + "Произошла ошибка создании корректировки, запишите номер листа желаний и передайте его администратору для ручной корректировки."
				+ Символы.ПС + ТекстОшибки + Символы.ПС;
			Продолжить;
		КонецПопытки;
		...
		Запрос.УстановитьПараметр("ЛистЖеланий", Строка.ЛистЖеланий);
		...
		ДокументЗакрытияЛистаЖеланий.Анкета = СтруктураЛистаЖеланий.Анкета;
		ДокументЗакрытияЛистаЖеланий.Дата = ТекущаяДата();
		ДокументЗакрытияЛистаЖеланий.Магазин = Объект.Магазин;
		ДокументЗакрытияЛистаЖеланий.Корзина = СтруктураЛистаЖеланий.Корзина;
		ДокументЗакрытияЛистаЖеланий.ЛистЖеланий = Строка.ЛистЖеланий;
		Попытка
			ДокументЗакрытияЛистаЖеланий.Записать(РежимЗаписиДокумента.Проведение);
			СтрокаИнформации = СтрокаИнформации + "Лист желаний закрыт!" + Символы.ПС;
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			ЗафиксироватьОшибку(ТекстОшибки, "СоздатьДокументыПоЛЖ");	//	LNK 04.09.2018 14:16:54
			ДокументЗакрытияЛистаЖеланий.Записать(РежимЗаписиДокумента.Запись);
			СтрокаИнформации = СтрокаИнформации +  "Закрытие листа желаний завершилось ошибкой!" + Символы.ПС;
			СтрокаИнформации = СтрокаИнформации +  "Запишите номер документа закрытия листа желаний и передайте его администратору для ручного закрытия."+ Символы.ПС 
			+ Строка(ДокументЗакрытияЛистаЖеланий.Ссылка)+ Символы.ПС + ТекстОшибки + Символы.ПС;
		КонецПопытки;
	КонецЦикла; 
...
Процедура ДоговорСтрахования(Команда)
	...
	Если НЕ (Объект.ДоговорСтрахования.Пустая() И Объект.Товары.Количество() = 0) Тогда
		...
		ПараметрыИнформации.ЗаголовокИнформации = НСтр("ru = 'Оплата по договору страхования не может быть выполнена'");
		ПараметрыИнформации.ТекстИнформации = НСтр(
			?(НЕ Объект.Товары.Количество() = 0
				, "ru = 'Таблица товаров НЕ может быть заполнена!'"
				, "ru = 'Договор страхования уже указан!'"));
		ОбщегоНазначенияРТКлиент.ОткрытьФормуИнформацииДляРМК(ПараметрыИнформации);
	...
	Если НЕ Объект.ДоговорСтрахования.Пустая() Тогда
		...
		Если Выборка.Сумма <= 0 Тогда
			...
			ТекстИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По договору «%1» страховой взнос уже оплачен!'"),
				Объект.ДоговорСтрахования
			);
			
			ПараметрыИнформации.ЗаголовокИнформации = "Ошибка подсистемы страхования";
		...
		ИначеЕсли СуммаВзноса > Выборка.Сумма Тогда
			...
			ТекстИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По договору «%1» страховой взнос превышает остаток по договору на %2 грн!'"),
				Объект.ДоговорСтрахования,
		...
		Если НЕ Отказ Тогда
			...
			Если НЕ Объект.Товары.Количество() = 1 Тогда
				...
				ПараметрыИнформации.ЗаголовокИнформации = "Ошибка подсистемы страхования";
				ПараметрыИнформации.ТекстИнформации     = НСтр("ru = 'Допускается только одна строка в таблице «Товары»!'");
			...
			ИначеЕсли НЕ Объект.Товары[0].Номенклатура = УслугаСтрахования Тогда
				...
				ПараметрыИнформации.ЗаголовокИнформации = "Ошибка подсистемы страхования";
				ПараметрыИнформации.ТекстИнформации     = НСтр("ru = 'Допустимо использовать только определённую услугу страхования!'");
...
КонецФункции // ПроверитьПоДоговоруСтрахования()
...
Процедура КомандаВыдатьКартуПредварительную(Команда)
	...
	ОтветПользователя = ОбщегоНазначенияРТКлиент.ВывестиВопросДляРМК(НСтр("ru = 'Выдать покупателю карту?'"));
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Заголовок"             , "Укажите номер предварительной карты");
//	ПараметрыОткрытия.Вставить("МаксимальноеЗначение"  , МаксимальноеЗначение);
//	ПараметрыОткрытия.Вставить("ЧислоВвода"            , ЧислоВвода);
	...
	ОткрытьФорму(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаВводаЧисла", Объект.ИмяВнешнейОбработки), ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыдатьКартуПредварительнуюЗавершение(Результат, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Иначе
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК("Карта не выдана", ТекстСообщения);
...
Функция ВыдатьКартуПредварительнуюНаСервере(КодКарты)
	...
	Если НЕ ПустаяСтрока(КодКарты) Тогда
		...
		Иначе
			...
			ТекстСообщения = "Карта «" + КодКарты + "» НЕ ОБНАРУЖЕНА!";

		КонецЕсли;

	Иначе

		ТекстСообщения = "Не указан код карты!";

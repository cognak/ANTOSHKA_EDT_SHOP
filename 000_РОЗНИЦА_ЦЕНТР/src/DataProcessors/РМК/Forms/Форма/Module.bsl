...
Процедура ЗаполнитьНаСервереПоЗаказуПокупателя(СписокДанных)
	...
	УчетнаяСистемаЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЗаказПокупателя, "УчетнаяСистема");
	ОбработкаВ1С = (УчетнаяСистемаЗаказа = Перечисления.УчетныеСистемыКомпании.Розница);

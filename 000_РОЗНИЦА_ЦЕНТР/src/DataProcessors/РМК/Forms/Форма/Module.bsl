	...
	Если НЕ РезультатЗапроса.Пустой() тогда 
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ВариантВыдачиКупона = Перечисления.ВариантыВыдачиКупона.ПечататьНаЧекеИзСписка тогда   // тут два варианта 1 печатаем изи списка загруженных, 2 генерируем скидочныйкупон
				...
				СкидочныйКупон = ПолучитьНомерДляПечати(Выборка.НоменклатураСкидочногоКупона,ЧекККМСсылка);   // Проверяем есть ли запись купона в регистре для печати из чека если в момент пробития он не распекчатался из РМК

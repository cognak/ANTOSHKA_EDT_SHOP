...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		...
		УстановитьВремяНачалаЧека();  // 20250201 Бакан RETAIL1C-1008
...
КонецПроцедуры
&НаКлиенте
Процедура УстановитьВремяНачалаЧека()  // 20250201 Бакан RETAIL1C-1008
	
	Если НачалоЧека = Дата(1,1,1) тогда 
	...
	Если НЕ СтруктураПараметров.НеизвестныеДанныеПО Тогда
		...
		ИначеЕсли СтруктураПараметров.Действие = "ПроверкаСерийныхНомеров" Тогда
			...
			Если СтруктураПараметров.ЗначенияПоиска.Количество() > 0 Тогда
				...
				ТаблицаТовараСерийногоНомера = Объект.Товары.Выгрузить();
				ТаблицаТовараСерийногоНомера.Очистить();
				НоваяСтрока = ТаблицаТовараСерийногоНомера.Добавить();
				НоваяСтрока.НомерСтроки = 1;
				НоваяСтрока.Номенклатура = ДанныеСерийногоНомера.Номенклатура;
				
				ТаблицаПогашениеСерийногоНомера = Объект.ПогашениеСкидочныхКупонов.Выгрузить();
				ТаблицаПогашениеСерийногоНомера.Очистить();
				НоваяСтрока = ТаблицаПогашениеСерийногоНомера.Добавить();
				НоваяСтрока.СкидочныйКупон = ДанныеСерийногоНомера.Номенклатура;
				НоваяСтрока.СерийныйНомер = ДанныеСерийногоНомера.СерийныйНомер;
				НоваяСтрока.НоминалКупона = 0;
				
				СтрокаОшибки = "";
				Отказ = Ложь;
				
				МаркетинговыеАкцииПривилегированный.ПроверитьСерийныеНомераПогашенияКупонов(
						ТаблицаТовараСерийногоНомера,
						ТаблицаПогашениеСерийногоНомера,
						Отказ,
						СтрокаОшибки);
		...
		ИначеЕсли СтруктураПараметров.Действие = "ПроверкаQRCode" Тогда
			
			ТекстСообщения = ПоискПоУИ(СтруктураПараметров.ЗначенияПоиска);
			
			Если ПустаяСтрока(ТекстСообщения) Тогда
				ОперативныеДанныеПокупателя(Объект.ДисконтнаяКарта, Объект.ВладелецДисконтнойКарты);
				СтруктураПараметровКлиента.Вставить("ПрекратитьПоиск");
				РежимКонтрольногоОткрытия = Истина;
			КонецЕсли;
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	ОтработкаОчисткиТоваров();			//	LNK 07.02.2024 17:46:16
...
Процедура КомандаПО04ОтложитьЧек(Команда)
	...
	ИначеЕсли НЕ ЭтоПродажаПоЗаказуПокупателя Тогда
		...
		Если Объект.Товары.Количество() = 0 Тогда
			...
			ОбновитьФорму();	//	LNK 18.10.2023 07:34:58
...
Процедура КомандаПО17ВвестиИнформационнуюКарту(Команда)
	...
	//А++ 20241114 https://awdev.atlassian.net/browse/RETAIL1C-99
	Если ЭтоЧекПоЗаказу() Тогда
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
			НСтр("ru = 'Запрет'; uk = 'Заборона'","uk")
			, НСтр("ru = 'Чек в режиме Интернет-заказа. 
                   |Изменение дисконтной карты невозможно! '; uk = 'Чек у режимі Інтернет-замовлення. 
                   |Зміна дисконтної картки неможлива!'","uk")
			);
...
Процедура ВыбратьПричинуРучнойСкидки(ИмяКоманды)
	...
	Если СертификатКакСкидка Тогда
		...
		Если ИмяКоманды = "КомандаСкидкаПроцентВЧеке" или ИмяКоманды = "КомандаСкидкаСуммаВСтроке" или ИмяКоманды = "КомандаСкидкаПроцентВСтроке" Тогда
			...
			эОтбор = фиксНастройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			эОтбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИспользованиеДляЗаказаКлиента");
...
Процедура ДействияПослеВводаПричиныРучнойСкидки(ОписаниеПричины, ДополнительныеПараметры)	Экспорт
	...
	//	LNK 21.09.2018 09:41:56 - Ручные скидки не должны давать возможность использования скидочных купонов.. любых!
		...
		Иначе // в Учетной политике прописан сертификат как оплата - в ручных скидках только стандартные варианты
...
//	Обновим флаг доступа принудительно..
	НаборПравИНастроек.РесурсWebRetailДоступен = РозничныеПродажиСлужебный.РесурсWebRetailДоступен();

	Если НаборПравИНастроек.РесурсWebRetailДоступен = Истина Тогда
...
&НаКлиенте	//	LNK 11.09.2023 04:49:30
Процедура КомандаПередатьБонусныеБаллыНаБлаготворительныйФонд(Команда)
...
//	LNK 03.01.2024 07:58:30 - код перенесен в модуль РМКСервер
	ТаблицаКассККМОрганизаций.Загрузить(РМКСервер.ПолучитьТаблицуКассПоРабочемуМесту(Объект.РабочееМесто, Объект.Магазин));
...
&НаСервере	//	LNK 16.04.2024 07:54:02
Функция ДоступныеКассыПоТекущейОрганизации()

	СписокКасс = Новый Массив;

	Для каждого СтрокаТаблицы Из ТаблицаКассККМОрганизаций Цикл

		Если СтрокаТаблицы.Организация = ПараметрыСеанса.ТекущаяОрганизация Тогда

			СписокКасс.Добавить(СтрокаТаблицы.КассаККМ);
...
//	LNK 02.04.2022 05:45:23
	...
	НаборПравИНастроек.Вставить("НовыхИзмененныхЗаказов", ЗаказыПокупателейСервер.ЗапросДоступностиНовыйИзменен());
...
&НаКлиенте	//	LNK 17.10.2023 08:15:13
Процедура УстановитьСуммуНакопленийИлиБонусов(СуммаПокупок = Неопределено, Обнулить = Ложь)

	Если НаборПравИНастроек.ИспользоватьБонуснуюСистему Тогда

		Элементы.Декорация10.Заголовок = "Бонуси активні";
		СоединениеУстановлено = УстановитьСуммуПокупок(СуммаПокупок, Обнулить);	//	LNK 01.09.2023 08:15:41

		Если СоединениеУстановлено Тогда

			Если РМК2_СуммаАктивныхБонусов > 0 Тогда
				
					Элементы.РМК2_СуммаНакоплений.ЦветТекста = WebЦвета.Черный;

			Иначе	Элементы.РМК2_СуммаНакоплений.ЦветТекста = WebЦвета.Серый;

			КонецЕсли;

		Иначе		Элементы.РМК2_СуммаНакоплений.ЦветТекста = WebЦвета.Красный;

		КонецЕсли;

		Элементы.РМК2_СуммаНакоплений.Шрифт = Новый Шрифт(,, ?(СоединениеУстановлено И РМК2_СуммаАктивныхБонусов <= 0, Ложь, Истина));

	//	LNK 11.09.2023 04:47:04
		УстановитьДоступностьЭлементаФормы("КомандаПередатьБонусныеБаллыНаБлаготворительныйФонд"
			, НаборПравИНастроек.РесурсWebRetailДоступен И РМК2_СуммаАктивныхБонусов > 0 И НЕ ПустаяСтрока(Объект.НомерТелефона)
		);

	Иначе

	//	LNK 11.09.2023 04:47:04
		УстановитьДоступностьЭлементаФормы("КомандаПередатьБонусныеБаллыНаБлаготворительныйФонд", Ложь);
...
Процедура УстановитьОформлениеЭлементов()	Экспорт	//	при передаче бонусов
	...
	Если НаборПравИНастроек.НовыхИзмененныхЗаказов = 0 Тогда
		Элементы.НовыеЗаказыГруппа.Видимость = Ложь;
	Иначе
		Элементы.НовыеЗаказыГруппа.Видимость = Истина;;
	КонецЕсли;
	
	НовыеИзмененныеЗаказы = НаборПравИНастроек.НовыхИзмененныхЗаказов;
...
&НаКлиенте
Процедура ОбновитьФорму(ЭтоПлюсОдин = Ложь, СообщатьОплатаЧастями = Ложь, ТипУслуги = Неопределено, УстановитьСуммыНакоплений = Истина)
	...
	СписокТиповУслугБанка.Добавить(ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетШкольника"));
	...
	Если УстановитьСуммыНакоплений = Истина Тогда

		УстановитьСуммуНакопленийИлиБонусов();	//	LNK 18.10.2023 06:47:02
	...
	//А++ 20241114 https://awdev.atlassian.net/browse/RETAIL1C-99
	Элементы.ПоискПоНомеру.Видимость = НЕ ЭтоЧекПоЗаказу();
	//А++ 20241114 https://awdev.atlassian.net/browse/RETAIL1C-99
	Если Объект.Товары.Количество() = 0 тогда
		НачалоЧека = Дата(1,1,1);	//	Бакан 20250128
	КонецЕсли;
КонецПроцедуры
...
Процедура РассчитатьСкидкиНаценкиНаСервере(СтруктураПараметры)
	УчетнаяПолитика = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику(ТекущаяДатаСеанса());

	Если Объект.КартаСотрудника И Объект.ИспользоватьПрограммуЛояльности и УчетнаяПолитика.ИспользоватьСкидкуСотрудника Тогда
...
Функция ПолученШтрихкодИзСШК(Штрихкод, ДополнительныйПараметрПоискаКарты = Неопределено, КолВСтроке = 1)
	...
	//А++ 202411121 https://awdev.atlassian.net/browse/RETAIL1C-99
	Если НЕ ЭтоЧекПоЗаказу() Тогда
		
		СтруктураДействий.Вставить("ПроверкаКарт", СтруктураПараметровДействия);
		СтруктураДействий.Вставить("ПроверкаQRCode");
		
	КонецЕсли;
	//А++ 20241121 https://awdev.atlassian.net/browse/RETAIL1C-99
...
Процедура ОбработатьДанныеПоКодуКлиент(СтруктураПараметровКлиента, ТекКод, КолВСтроке = 1,НеВыдавать = Ложь)
	...
	УстановитьСуммыНакоплений = Истина;
	...
	ИначеЕсли СтруктураПараметровКлиента.Свойство("ДобавитьТоварнуюПозицию") Тогда
		...
		БуфферПоляВвода = "";
		УстановитьСуммыНакоплений = Ложь;
	...
	ОбновитьФорму(,,, УстановитьСуммыНакоплений);
...
//	Проверимся на всякий случай - имеется ли такой ключ в структуре? Мало-ли откуда вызов прилетит ...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Свойство("Блокирован") И ВыбранноеЗначение.Блокирован = Истина Тогда
			...
			ПричинаПроблемы = "обрана позиція БЛОКОВАНА для продажу!";
		...
		Если ПустаяСтрока(ПричинаПроблемы) И ВыбранноеЗначение.Свойство("НазначениеТовара") Тогда
			...
			Если ВыбранноеЗначение.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Тогда
				...
				Если НЕ ДанныеМагазина.РазрешитьПродажуУслуг = Истина Тогда
					...
					ПричинаПроблемы = "продаж послуги для магазина НЕ дозволено!";
...
Функция ПолученМагнитныйКод(МагнитныйКод, ДополнительныйПараметрПоискаКарты = Неопределено)
	...
	//А++ 20241117 https://awdev.atlassian.net/browse/RETAIL1C-99
	Если НЕ ЭтоЧекПоЗаказу() Тогда
...
&НаСервере
Функция ПоискПоУИ(Знач УИКлиентаВремя)

//	Приведение номера к стандарту поиска.
	УИКлиента = Лев(УИКлиентаВремя, 36);
	...
	Если СоставДанных.Контрагент = Неопределено ИЛИ СоставДанных.Ошибка = Истина Тогда
		...
		ТекстСообщения = "Покупатель по уникальному коду «" + УИКлиента + "» НЕ ОБНАРУЖЕН!"
	...
	Иначе
		...
		Объект.ВладелецДисконтнойКарты	= СоставДанных.Контрагент;
		Объект.ДисконтнаяКарта			= СоставДанных.ДисконтнаяКарта;
		Объект.НомерТелефона				= СоставДанных.НомерТелефона;
		РежимКонтрольногоОткрытия = Истина;
...
Процедура ВывестиИнформациюНаДисплейПокупателя(Команда)	Экспорт
	...
	ИначеЕсли Команда = "ОжиданиеОплаты" Тогда
		...
		ДисплейПокупателяПараметры.СуммаВсего  = Формат(СкидкиНаценкиКлиентСервер.ОкруглитьСуммуПоФискальнымПравилам(СуммаВсего), "ЧДЦ=2;ЧГ=;ЧН=0.00");
		ДисплейПокупателяПараметры.СуммаСкидки = Формат(СуммаСкидки, "ЧДЦ=2;ЧГ=");
	...
	ИначеЕсли Команда = "ДобавлениеТовара" или  Команда = "УдалениеТовара" или
		...
		//2025-11-26 sa формируем структуру для вывода на экран
		МассивСтрок = Новый Массив;
		Для Каждого СтрокаТовара Из Объект.Товары Цикл
			СтруктураТовара = Новый Структура("SKU,SKU_NAME,QUANTITY,DISCOUNT,PRICE"
						...
						,СтрокаТовара.Цена);
			МассивСтрок.Добавить(СтруктураТовара);
		КонецЦикла;
		//2026-06-01 sa на дисплее покупателя может отображаться только 14 позиций товара. Оставляем последние из них.
		Пока МассивСтрок.Количество() > 14 Цикл
	...
	Для каждого Дисплей Из Дисплеи Цикл
		...
		//2025-11-26 sa Определяем наш подключенный дисплей (разработка Антошки, многострочный, автомасштабируемый)
		Если Дисплей.ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАнтошкаДисплейПокупателя") Тогда
			Если Команда = "ДобавлениеТовара" или  Команда = "УдалениеТовара" или
					Команда = "ИзмененияКолва"  или Команда = "ИзмененияЦены" Тогда
				
			ВходныеПараметры  = Новый Массив;
			ВходныеПараметры.Добавить(МассивСтрок);
			...
			Иначе
				ВходныеПараметры  = Новый Массив;
				ВходныеПараметры.Добавить(Строка1+"\n"+Строка2);
				ВыходныеПараметры = Новый Массив();
...
Процедура ПровестиОпрос(ПриИзмененииКарты = Ложь, Принудительно = Ложь)
	...
	//2026-06-09 sa сотрудникам запрещаем менять данные через этот механизм. Только через отдел кадров.
	Если ДКСотрудника() Тогда
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
			НСтр("ru = 'Запрет'; uk = 'Заборона'","uk")
			, НСтр("ru = 'ДК принадлежит сотруднику. 
	               |Изменение данных сотрудника только через отдел кадров! '; uk = 'ДК належить співробітнику. 
	               |Зміна персональних даних тількі через відділ кадрів!'","uk")
			);
...
Процедура ПроверитьКассовыеСмены(Отказ, ПараметрыИнформации)
	...
	ДоступныеКассыККМ = ДоступныеКассыПоТекущейОрганизации();	//ТаблицаКассККМОрганизаций.Выгрузить().ВыгрузитьКолонку("КассаККМ");
...
&НаСервере	//	LNK 19.12.2023 05:31:24
Функция ПроверитьЗаполнениеШапкиЧекаВозврата(Отказ, ПараметрыИнформации)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат") Тогда
		...
		|ГДЕ
		|	ТаблицаПродажи.Ссылка = &ЧекККМПродажа
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда	//	в чеке продажи карта указана!

			ВыборкаПродажа = РезультатЗапроса.Выбрать();
			ВыборкаПродажа.Следующий();

			Атрибуты = Новый Массив;
			Атрибуты.Добавить("ВладелецДисконтнойКарты");
			Атрибуты.Добавить("ДисконтнаяКарта");

			Для каждого Атрибут Из Атрибуты Цикл

				Если НЕ ВыборкаПродажа[Атрибут] = Объект[Атрибут] Тогда

					Если НЕ Отказ Тогда

						Отказ = Истина;
						ПараметрыИнформации.ЗаголовокИнформации =
						"Маємо помилки заповнення повернення
						|Рекомендуємо переоформити повернення";
						ПараметрыИнформации.ТекстИнформации = "";

					КонецЕсли;

					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"Реквізит '%1' НЕ відповідає значенню [%2]",
						Атрибут, ВыборкаПродажа[Атрибут]
					);
...
&НаСервере
Процедура ДобавитьТаблицуТовары(АдресТаблицы)
	времТовары = ПолучитьИзВременногоХранилища(АдресТаблицы);
	Если времТовары <> Неопределено Тогда
		Если времТовары.Количество() > 0 Тогда
			Объект.Товары.Загрузить(времТовары);	
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры                  

&НаСервере
Процедура ДобавитьТаблицуСкидкиНаценки(АдресТаблицы)
	времСкидкиНаценки = ПолучитьИзВременногоХранилища(АдресТаблицы);
	Если времСкидкиНаценки <> Неопределено Тогда
		Если времСкидкиНаценки.Количество() > 0 Тогда
			Объект.СкидкиНаценки.Загрузить(времСкидкиНаценки);	
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОплатитьТовары(ИмяФормыОплаты, ОплатаПлатежнойКартой = Неопределено, Печать = Ложь, ВозвратОплаты = Неопределено, ПроверкиУжеВыполнены = Ложь)
	...
	НачалоРасчетаСкидок = ТекущаяДата();// 20250201 Бакан RETAIL1C-1008	
	Отказ = Ложь;
	ЭтоВозврат = (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат"));
	...
	//	LNK 19.12.2023 05:29:28
		Если ПроверитьЗаполнениеШапкиЧекаВозврата(Отказ, ПараметрыИнформации) Тогда

			ПроверитьЗаполнениеТабличнойЧасти(Отказ, ПараметрыИнформации);
            УдалитьИзТоваровЧекаПогашаемыеСкидочныеКупоны();
		КонецЕсли;
...
//	\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\/
	НачалоОплаты = ТекущаяДата(); // 20250201 Бакан RETAIL1C-1008
	Если НЕ ОплатаТолькоНаличными Тогда
		...
		Для каждого СтрокаЧека Из ТабличноеПолеЧеков Цикл
			...
			ДополнительныеПараметры = Новый Структура(	//	может, чего нужно передать в процедуру завершения.
				"Магазин, Печать, НесколькоОрганизаций"
				, Объект.Магазин
				, Печать
			...
			ПараметрыОткрытия = ПолучитьПараметрыОплаты(СтрокаЧека, ОплатаПлатежнойКартой, ВозвратОплаты);
...
Процедура ОплатитьТоварыЗавершение(Результат, ДополнительныеПараметры)	Экспорт
	...
	ИначеЕсли НЕ ТестовыйРежимРМК Тогда  //+HVOYA Mykhailo
		...
		//2026-09-07 Sa Если из модуля оплаты передали таблицы Товаров и СкидкиНаценки, то нужно их загрузить
		//2026-09-10 sa пока откатываем, нужно больше времени на проверку совместимости.		
		//Если Результат.Свойство("АдресТовары") Тогда
		//	ДобавитьТаблицуТовары(Результат.АдресТовары);
		//КонецЕсли;
		//Если Результат.Свойство("АдресСкидкиНаценки") Тогда
		//	ДобавитьТаблицуСкидкиНаценки(Результат.АдресСкидкиНаценки);
		//КонецЕсли;
...
//	LNK 03.06.2020 10:31:02
	...
	//2026-09-04 sa добавим Товары и СкидкиНаценки
	УстановитьТабличныеЧасти(ПараметрыОткрытия);
...
&НаСервере
Процедура УстановитьТабличныеЧасти(ПараметрыОткрытия)
	АдресТовары = ПоместитьВоВременноеХранилище(Объект.Товары.Выгрузить());
	АдресСкидкиНаценки = ПоместитьВоВременноеХранилище(Объект.СкидкиНаценки.Выгрузить());
	ПараметрыОткрытия.Вставить("АдресТовары", АдресТовары);
	ПараметрыОткрытия.Вставить("АдресСкидкиНаценки", АдресСкидкиНаценки);
...
Процедура ЗаполнитьНаОснованииВозврата()
	...
	Если Не ЗаказПокупателяЗаполнен(Объект.ЗаказПокупателя, "ЗаказПокупателя") Тогда
		СкидкиНаценкиКлиентСервер.ОчиститьОкругленияТоваров(Объект.Товары, Ложь);	//	LNK 31.01.2020 10:31:19
	КонецЕсли;
	УправляемыеСкидки.ЗагрузитьЗначения(Объект.УправляемыеСкидки.Выгрузить().ВыгрузитьКолонку("СкидкаНаценка"));
...
Процедура ПровестиЧекККМ(ЧекККМСсылка, ПараметрыЗаполненияЧекаККМ, МассивСсылокЧеков, ВывестиДополнительноеСообщение, ПараметрыИнформации)
	...
	Если ПараметрыЗаполненияЧекаККМ.Свойство("ДанныеПРРО") Тогда	//	LNK 25.10.2023 08:45:08

		ДанныеХранилища.ДанныеПРРО = ПараметрыЗаполненияЧекаККМ.ДанныеПРРО;

		Документы.ЧекККМ.УстановитьФлагиПроведения(ОбъектЧекККМ, ПараметрыЗаполненияЧекаККМ.ДанныеПРРО);	//	LNK 07.02.2024 11:57:04
...
&НаСервере
Процедура ОтправкаПочты(ОбъектОтправки) 
	...
	Попытка
		...
		Если Не УчетныеДанныеОтправителя.Пустая() Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				...
				НомерМагазина = СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектОтправки.Магазин, "НомерМагазина"));
				
				Адрес = "shop" + НомерМагазина + "@antoshka.ua;
							|shop" + НомерМагазина + ".dir@antoshka.ua;";
							
				Тема = "Застосування ручної знижки в чеку № " + СокрЛП(ОбъектОтправки.Номер);
				
				РучныеСкидки = "";
				РучныхСкидокНет = Истина;
				Для Каждого СтрокаТовара Из ОбъектОтправки.Товары Цикл
					Если Не СтрокаТовара.ПричинаРучнойСкидки.Пустая() Тогда
						РучныеСкидки = РучныеСкидки + СтрокаТовара.ПричинаРучнойСкидки + Символы.ПС;
						РучныхСкидокНет = Ложь;
					КонецЕсли;
				КонецЦикла;
				
				Если РучныхСкидокНет Тогда 
					Возврат;
				КонецЕсли;
				
				ТелоПисьма = "Застосування ручної знижки в чеку № "
						+ СокрЛП(ОбъектОтправки.Номер)
						+ ", на касі № "
						+ СокрЛП(ОбъектОтправки.КассаККМ)
						+ ", касир "
						+ СокрЛП(ОбъектОтправки.Автор)
						+ "список ручних знижок:"
						+ Символы.ПС
						+ РучныеСкидки;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("Ручная скидка"
...
//
&НаКлиенте
Функция ПробитьЧекККМ(ЧекККМСсылка, КассаККМ, ПараметрыЗаполнения, ПараметрыКассыККМ = Неопределено, ОтветККМ = Неопределено)
		...
		//	Подключаем ФР
			...
			//	Печатаем чек.
				...
				//	Установить полученное значение номера чека реквизиту документа.
					...
					//		LNK 25.10.2023 08:23:21 - на текущий момент только ПРРО!
					//		соответствие, необходимое для печати копии чека
							ПараметрыЗаполнения.Вставить("ДанныеПРРО" , ВыходныеПараметры[2]);
							ЭтоПРРО = Истина;

					Иначе	
							ПараметрыЗаполнения.Вставить("ДанныеПРРО" , Неопределено);
							ЭтоПРРО = Ложь; 
					КонецЕсли;
...
Процедура СоздатьЧеки(Отказ, Печать = Ложь)
	...
	//	LNK 30.09.2019 10:38:31	- ПЕРЕД пробитием.
		...
		Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(ЧекККМСсылка, "ВидОперации") = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат") тогда
			...
			ЗаписатьСкидочныйКупонВРегистрПоКодуСервер(ЧекККМСсылка,КодСерийногоНомера)	

		Иначе

			Попытка
				Если ЭтоПРРО тогда
					ПроверитьАкцииНаПечатьУникальныхКупоновПРРО(ЧекККМСсылка);
				Иначе
	...
	Попытка
		ЗаписатьВремяОбработкиЧека(МассивСсылокЧеков);    // 20250201 Бакан RETAIL1C-1008
	Исключение
	КонецПопытки;
	Если НЕ Отказ Тогда
...
КонецПроцедуры
&НаСервере
Процедура ЗаписатьВремяОбработкиЧека(ЧекККМ)  // 20250201 Бакан RETAIL1C-1008
НаборЗаписей = РегистрыСведений.ВремяОбработкиЧека.СоздатьНаборЗаписей(); 

НаборЗаписей.Отбор.ЧекККМ.Установить(ЧекККМ[0]);

НоваяЗапись = НаборЗаписей.Добавить();
НоваяЗапись.ЧекККМ = ЧекККМ[0];
НоваяЗапись.ВремяОбработки = НачалоРасчетаСкидок - НачалоЧека;
НоваяЗапись.ВремяНачала = НачалоЧека; 
НоваяЗапись.НачалоРасчетаСкидок = НачалоРасчетаСкидок; 
НоваяЗапись.ВремяОкончания = ТекущаяДата(); 
НоваяЗапись.ВремяРасчетаСкидок =НачалоОплаты-НачалоРасчетаСкидок; 
НоваяЗапись.НачалоОплаты = НачалоОплаты; 
НоваяЗапись.ВремяОплаты = ТекущаяДата()-НачалоОплаты; 
НоваяЗапись.Автор = ПараметрыСеанса.ТекущийПользователь;
НаборЗаписей.Записать();
НачалоЧека = Дата(1,1,1);
КонецПроцедуры
&НаСервере
...
Процедура НапечататьТоварныйЧекНаФР(МассивСсылокЧеков)
	...
	Для каждого ЧекККМСсылка Из МассивСсылокЧеков Цикл
		...
		ПараметрыКассыККМ = ЗначениеНастроекПовтИсп.ПолучитьПараметрыКассыККМ(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЧекККМСсылка, "КассаККМ", Неопределено));
		
		Если НЕ ЗначениеЗаполнено(ПараметрыКассыККМ.ИдентификаторУстройства) 
		 ИЛИ НЕ ЗначениеЗаполнено(ПараметрыКассыККМ.ШиринаЛенты)  Тогда
		 
			ЗаголовокИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"Перевірте налаштування каси ККМ «%1»",
				ПараметрыКассыККМ.КассаККМ,
			);
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
				ЗаголовокИнформации
	...
	//	LNK 19.02.2024 11:35:46
		Если ПараметрыКассыККМ.ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор") Тогда

			СтрокаПечати = ПолучитьДанныеКопииЧекаПРРО(ЧекККМСсылка);

			Если СтрокаПечати.Пустой = Истина Тогда
		...
		Иначе
			...
			СтрокаПечати = СтрокаПечатиНаФР(МассивПечати, ПараметрыКассыККМ.ШиринаЛенты);
		...
		//	Подключаем ФР
			...
			//	Готовим данные
				...
				ВходныеПараметры[0] = СтрокаПечати;
				ВходныеПараметры[2] = Новый Массив(22);
				ВходныеПараметры[2][21] = ПараметрыКассыККМ;
...
Процедура ПОИСК_ВВЕДЕННОГО_КОДА(Элемент)
	...
	Если флПоискПоНомеру Или СтрДлина(БуфферПоляВвода) >= 36 Тогда
		...
		//	разделяем количество и штрихкод
			...
			//	а если это номер регистрационной карты ???
				Если СтруктураПараметровКлиента.Свойство("НеизвестныеДанныеПО") И СтруктураПараметровКлиента.НеизвестныеДанныеПО Тогда
					//А++ 20241115 https://awdev.atlassian.net/browse/RETAIL1C-99
					Если ЭтоЧекПоЗаказу() Тогда
						ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
							НСтр("ru = 'Запрет'; uk = 'Заборона'","uk")
							, НСтр("ru = 'Чек в режиме Интернет-заказа. 
...
Процедура ПоискПоНомеруТелефонаКлиент(БуфферПоляВвода)
	...
	Если СтрДлина(БуфферПоляВвода) >= 36 Тогда
		
		ТекстСообщения = ПоискПоУИ(БуфферПоляВвода);
	...
	ОбновитьФорму();					//	LNK 18.10.2023 07:04:19
...
Процедура ВыборЗаказаПокупателяЗавершение(ДанныеВыбора, ДополнительныеПараметры)	Экспорт
	...
	Если ТипЗнч(ДанныеВыбора) = Тип("Структура") И НЕ ОбщегоНазначенияКлиентСервер.СсылкаПустая(ДанныеВыбора.Ссылка) Тогда
		УстановитьВремяНачалаЧека();  // 20250211 Бакан RETAIL1C-1008
...
Процедура ЗаполнитьНаСервереПоЗаказуПокупателя(СписокДанных)
	...
	ДанныеЗаказаРеквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ЗаказПокупателя, "УчетнаяСистема, ДокументОснование");
...
Функция ТекстЗапросаДанныеЗаказаПокупателя(ОбработкаВ1С = Ложь)
	...
	Возврат ЗаказыПокупателейСервер.ТекстЗапросаДанныеЗаказаПокупателя(ОбработкаВ1С);
...
Функция ДанныеЗаказаПокупателяЗаполнить(ШапкаВыборка = Неопределено, ЗаказПокупателя, НаборПравИНастроек, УслугаДоставкиЗаказаПокупателя)
	...
	Если ЗаказПокупателяЗаполнен(ЗаказПокупателя, "ЗаказПокупателя") Тогда
		...
		Если ШапкаВыборка = Неопределено Тогда
			...
			ДанныеЗаказаРеквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаказПокупателя, "УчетнаяСистема, ДокументОснование, Магазин");
			ОбработкаВ1С = ДанныеЗаказаРеквизиты.УчетнаяСистема = Перечисления.УчетныеСистемыКомпании.Розница;
			Запрос = Новый Запрос(ТекстЗапросаДанныеЗаказаПокупателя(ОбработкаВ1С));
			Запрос.УстановитьПараметр("Магазин", ДанныеЗаказаРеквизиты.Магазин);
...
Процедура РМК_ПРОБИТЬ_ЧЕК(Команда)
	Если ПроверитьНазначениеКассыККМ(Истина) И ПроверитьДанныеПокупателя() Тогда	//	LNK 19.01.2018 11:52:09
		...
		Если ПроверкаАвтоПечатиЧеков() Тогда
			...
			Иначе	// расчет скидок, оплата наличными
				...
				Если ЗаказПокупателяЗаполнен(Объект.ЗаказПокупателя) Тогда
					...
					Иначе	
						...
						//А++ 20241114 https://awdev.atlassian.net/browse/RETAIL1C-99
						Если ПроверитьНоменклатуруЗаказа() Тогда
							
							ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
									Нстр("ru = 'Ошибка товарного состава!'; uk = 'Помилка товарного складу!'","uk")
									,НСтр("ru = 'Вы удалили все товары, связанные с Интернет-заказом. 
									       |Закройте данный чек и создайте новый не связанный с заказом. 
									       |Пробитие чека остановлено! '; uk = 'Ви видалили всі продукти, пов''язані з Інтернет-замовленням. 
									       |Закрийте цей чек і створіть новий, не пов''язаний із замовленням.
									       |Пробиття чека зупинено!'","uk"),
									////,НСтр("ru = 'Вы удалили товары, связанные с Интернет-заказом. 
									////       |Закройте данный чек и создайте новый не связанный с заказом. 
									////       |Пробитие чека остановлено! '; uk = 'Ви видалили продукти, пов''язані з Інтернет-замовленням. 
									////       |Закрийте цей чек і створіть новий, не пов''язаний із замовленням.
									////       |Пробиття чека зупинено!'","uk"),
									,ЭтаФорма);
		...
		Иначе
			...
			ПараметрыИнформации = ОбщегоНазначенияРТКлиентСервер.ПолучитьСтруктуруВыводимойВРМКИнформации();
			ПараметрыИнформации.ЗаголовокИнформации = "АвтоПечать чеков";
			ПараметрыИнформации.ТекстИнформации = "В данный момент работает автопечать чеков. Попробуйте через несколько минут.";
...
// Проверка авто печати чеков.
// 
// Возвращаемое значение:
//  Булево - Истина, если нет задание по авточеку
&НаСервереБезКонтекста
Функция ПроверкаАвтоПечатиЧеков()
	...
	ОтборЗаданий.Вставить("Ключ", "РЕГЛАМЕНТ_АВТО_ПЕЧАТЬ_ЧЕКОВ_ИМ");
	ОтборЗаданий.Вставить("Состояние", СостояниеФоновогоЗадания.Активно);
	ЗаданиеАвтоЧека = ФоновыеЗадания.ПолучитьФоновыеЗадания(ОтборЗаданий);
	Результат = ЗаданиеАвтоЧека.Количество() = 0;
	...
	Если НЕ НаборПравИНастроек.ПродажиАктивированы Тогда	//	не в контексте CRM
		...
		Если НЕ (Объект.ДисконтнаяКарта.Пустая() ИЛИ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЧекККМ.Возврат")) Тогда
			...
			Если НЕ ТипЗнч(ДанныеКарты) = Тип("Массив") Тогда
                 ПроверитьНаСотруника();  // + Бакан Т.С. 20250609 Аутификация сотрудников
		         ОперативныеДанныеПокупателя(Объект.ДисконтнаяКарта, Объект.ВладелецДисконтнойКарты);
...
&НаКлиенте
Процедура КомандаПакетШкольника(Команда)

	КомандаУслугаБанкаВыполнить(ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетШкольника"));
...
Процедура КомандаУслугаБанкаВыполнить(ТипУслуги)
	...
	Если НЕ УслугаБанкаДанные.ТипУслуги = ТипУслуги Тогда
		...
		Для каждого ТипУслугиСписка Из СписокТиповУслугБанка Цикл
			...
			Если Не ТипУслуги = ТипУслугиСписка Тогда
	
				УстановитьОформлениеКнопкиБанковскойУслуги(ТипУслугиСписка, Ложь);
	...
	ОбновитьФорму(, Истина, ТипУслуги, Ложь);
...
Процедура УстановитьОформлениеКнопкиБанковскойУслуги(ТипУслуги, ВывестиСообщение = Ложь)
	...
	СоответсвиеЭлементовФормы = Новый Соответствие();
	СоответсвиеЭлементовФормы.Вставить(ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетМалыша"), Элементы.КомандаПакетМалыша);
	СоответсвиеЭлементовФормы.Вставить(ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ОплатаЧастями"), Элементы.КомандаОплатаЧастями);
	СоответсвиеЭлементовФормы.Вставить(ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетШкольника"), Элементы.КомандаПакетШкольника);

	ЭлементФормы = СоответсвиеЭлементовФормы.Получить(ТипУслуги);
		...
		УслугаБанкаДанные = ФинансыВызовСервера.ДоступнаяУслугаБанка(
			...
			ЗаказПокупателяЗаполнен(Объект.ЗаказПокупателя
				, ?(ТипУслуги = ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетМалыша")
						Или ТипУслуги = ПредопределенноеЗначение("Перечисление.ТипыУслугБанка.ПакетШкольника")
...
Процедура РМК_Аннуляция(Команда)
	...
	//А++ 20241114 https://awdev.atlassian.net/browse/RETAIL1C-99
	ЗаказКлиентаИзменен = Истина;
	//А++ 20241114 https://awdev.atlassian.net/browse/RETAIL1C-99

КонецПроцедуры

&НаКлиенте	//	LNK 07.02.2024 17:46:01
Процедура ОтработкаОчисткиТоваров()

	Если Объект.Товары.Количество() = 0 Тогда	//	LNK 07.02.2024 17:41:45

		ОбнулитьНаСервере();
		ОтменаЧека();
		СоздатьНовыйНабор();

		//А++ 20241114 по задаче https://awdev.atlassian.net/browse/RETAIL1C-99
		//ПоказатьОповещениеПользователя("Текущий чек отменён!"
		//	,
		//	, "Работа продолжается по новому чеку, так как все товары были очищены!"
		//	, БиблиотекаКартинок.Предупреждение32
		//);
		
		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
			НСтр("ru = 'Текущий чек отменён!'; uk = 'Поточний чек скасовано!'","uk")
			,НСтр("ru = 'Работа продолжается по новому чеку, так как все товары были очищены!'; uk = 'Робота продовжується за новим чеком, тому що всі товари були очищені!'","uk")
			,
			,ЭтаФорма);
		//А++ 20241114 по задаче https://awdev.atlassian.net/browse/RETAIL1C-99
...
Процедура РМК_Возврат(Команда)
	...
	Иначе
		...
		СтруктураОтбора = ОбщегоНазначенияРТКлиентСервер.СтруктураОтбора();
		СтруктураОтбора.Поле          = "Организация";
		...
		КоличествоДней = ?(НаборПравИНастроек.ЭтоУзелИнтернетМагазина = Истина, 30, 2);	//	LNK 17.03.2024 08:23:13

		СтруктураОтбора = ОбщегоНазначенияРТКлиентСервер.СтруктураОтбора();
		СтруктураОтбора.Поле          = "Дата";
		СтруктураОтбора.Значение      = НачалоДня(ОбщегоНазначенияВызовСервера.ТекущаяДатаСервера() - 86400 * КоличествоДней);
		СтруктураОтбора.ВидСравнения  = ВидСравненияКомпоновкиДанных.БольшеИлиРавно;
		СтруктураОтбора.Использование = Истина;
		МассивСтруктурОтбора.Добавить(СтруктураОтбора);
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Результат.ЧекККМ)  Тогда
				УстановитьВремяНачалаЧека();  // 20250224 Бакан RETAIL1C-1008
...
&НаКлиенте
Процедура ПроверитьНаСотруника() // + Бакан Т.С. 20250609 Аутификация сотрудников
	Если ЭтоСотрдник() тогда
			// + Бакан Т.С. 202500707 Пока отключаем Аутификацию сотрудников

			//АутифицироватьСотрудника(ПолучитьНомерТелефона(Объект.ВладелецДисконтнойКарты),Объект.ВладелецДисконтнойКарты,"РМК",ЭтаФорма.УникальныйИдентификатор,ЭтаФорма);
	КонецЕсли;
КонецПроцедуры
&НаСервере
Функция ЭтоСотрдник() // + Бакан Т.С. 20250609 Аутификация сотрудников
	Сотрудник = Ложь;
		Запрос = Новый Запрос(
	"ВЫБРАТЬ
...
&НаКлиенте
Процедура АутифицироватьСотрудника(НомерТелефона, Контрагент, ИмяВнешнейОбработки, УникальныйИдентификатор, Форма = Неопределено)	
...
//	Обновим флаг доступа принудительно..
	...
	Если РесурсWebRetailДоступен = Истина И НЕ ПустаяСтрока(НомерТелефона) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьАутификацию", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ОбработатьАутификацию(ДанныеВозврата, ДополнительныеПараметры) Экспорт 	
...
&НаСервере	//	LNK 01.09.2023 08:07:37
Функция УстановитьСуммуПокупок(СуммаПокупок, Обнулить = Ложь)

	СоединениеУстановлено = Ложь;

	Если НаборПравИНастроек.ИспользоватьБонуснуюСистему Тогда

		Если РозничныеПродажиСерверПовтИсп.ЗапретитьОбновлениеБонусныхБалловРМК(Объект.Магазин) Тогда

			РМК2_СуммаАктивныхБонусов = 0;
			РМК2_СуммаНакоплений	  = "-denied-";

		ИначеЕсли Объект.ВладелецДисконтнойКарты = Неопределено ИЛИ Объект.ВладелецДисконтнойКарты.Пустая() Тогда

			РМК2_СуммаАктивныхБонусов = 0;
			РМК2_СуммаНакоплений	  = "--//--";

		Иначе

			Если Обнулить = Истина Тогда
			...
			Иначе
				...
				ДанныеОтвета = БонусныеБаллыСервер.BPS_ПолучитьОстатокБонусныхБаллов(Объект.ВладелецДисконтнойКарты, ПараметрыЗапроса);
				...
				Иначе
					...
					РМК2_СуммаАктивныхБонусов = ДанныеОтвета.ДанныеСчёта.СуммаБонусныхБаллов;

				КонецЕсли;

			КонецЕсли;

			РМК2_СуммаНакоплений = Формат(РМК2_СуммаАктивныхБонусов, "ЧДЦ=2; ЧН=0.00; ЧГ=");

		КонецЕсли;

	Иначе

		РМК2_СуммаНакоплений = ?(СуммаПокупок = Неопределено, 0, РМК2_СуммаПокупок);

	КонецЕсли;

	Если НЕ СуммаПокупок = Неопределено Тогда
		
		РМК2_СуммаПокупок  = Формат(СуммаПокупок, "ЧДЦ=2; ЧН=0.00; ЧГ=");
...
Процедура ИндикаторПодключенияНажатиеНаСервере()
	...
	ОбновитьПовторноИспользуемыеЗначения();	//	LNK 21.09.2023 06:18:09
...
//	обновим открывающую форму после очистки управляемых типов бонусной оплаты.
	...
	ПараметрыОткрытия.Вставить("РежимКонтрольногоОткрытия" , РежимКонтрольногоОткрытия);
...
Процедура РМК_ПечатьПоследнегоЧека(Команда)
	...
	НапечататьТоварныйЧекНаФР(ПолучитьПоследнйЧекНаСервере(Объект.КассаККМ));

КонецПроцедуры

&НаСервереБезКонтекста	//+HVOYA Mykhailo : печать последнего чека
Функция ПолучитьПоследнйЧекНаСервере(КассаККМ)	//	LNK 19.02.2024 11:13:56 - модифицировал процедуру в функцию
...
//	1. находим последний пробитый чек
	...
	|	Документ.ЧекККМ КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.Проведен = ИСТИНА
	|	И ТаблицаДокумента.КассаККМ = &КассаККМ
	|	И ТаблицаДокумента.СтатусЧекаККМ = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробитый)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Иначе	СписокСсылок = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка");
...
&НаСервереБезКонтекста
Функция ПолучитьДанныеКопииЧекаПРРО(ЧекККМСсылка)
	...
	ДанныеПРРО = Новый Структура(
			"Пустой, URL, ОтветПРРО, ПараметрыЗакрытия"
			, Истина
			, ""
			, Новый Соответствие
			, Новый Структура
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если ТипЗнч(ДанныеХранилища) = Тип("Структура") И ДанныеХранилища.Свойство("ДанныеПРРО") Тогда

			ЗаполнитьЗначенияСвойств(ДанныеПРРО, ДанныеХранилища.ДанныеПРРО);
			ДанныеПРРО.Пустой = (ДанныеПРРО.ОтветПРРО.Количество() = 0);
...
//	Бакан. Готовим данные
	МассивВходныеПараметры = ПолучитьТекстУникальногоКупонаСервер(ЧекККМСсылка);  // ДР Антошка в этой функции получаем данные купонов
	ОписаниеОшибки = "";
...
Возврат УчетнаяПолитика.ИнтернетМагазин;	
КонецФункции
...
&НаКлиенте 
Функция ПроверитьАкцииНаПечатьУникальныхКупоновПРРО(ЧекККМСсылка)		
	

	Если ОбщегоНазначения.ПолучитьЗначениеРеквизита(ЧекККМСсылка,"Магазин") = ПолучитьИМ() Тогда
		Возврат ЛОЖЬ;
	КонецЕсли;	
	ТабличныйДокумент  =  ПолучитьТабДокКупона(ЧекККМСсылка);
...
Процедура ОчиститьРеквизитыФормы()
	...
	РМК2_ИдентификацияКлиента = "";
	РМК2_Штрихкод             = "";
	УстановитьСуммуПокупок(0, Истина);	//	LNK 01.09.2023 08:07:37
	...
	РежимКонтрольногоОткрытия = Ложь;
	...
	Иначе
		...
		Если ЭтоЧекПоЗаказу() Тогда
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
				НСтр("ru = 'Запрет'; uk = 'Заборона'","uk")
				, НСтр("ru = 'Чек в режиме Интернет-заказа. 
	                   |Изменение клиента невозможно! '; uk = 'Чек у режимі Інтернет-замовлення. 
	                   |Зміна кліента неможлива!'","uk")
				);
...
Процедура ПроцедураУчетаРабочегоВремени(Команда)
	...
	//2025-05-28 sa временно отключим признак биосканера
	Если ИспользуетсяБиометрическийСканерСервер() Тогда
		ОткрытьФорму("Обработка.УчетРабочегоВремениБиосканер.Форма",, ЭтотОбъект, ЭтотОбъект,,,, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	Иначе
...
//А++ 20241114 https://awdev.atlassian.net/browse/RETAIL1C-99
&НаСервере
Функция ЭтоЧекПоЗаказу()
	Возврат ЗначениеЗаполнено(Объект.ЗаказПокупателя);	
КонецФункции

&НаСервере
Функция ПроверитьНоменклатуруЗаказа()
	
	Если ТипЗнч(Объект.ЗаказПокупателя) = Тип("ДокументСсылка.ЗаказПокупателя") и ЗаказКлиентаИзменен  Тогда 
		...
		|	СУММА(ЗаказПокупателяТовары.КоличествоУпаковок) КАК КоличествоУпаковок
		|ПОМЕСТИТЬ ВР_ТоварыЗаказПокупателя
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТоварыРМК.Номенклатура КАК Номенклатура,
		|	ТоварыРМК.КоличествоУпаковок КАК КоличествоУпаковок
		|ПОМЕСТИТЬ ВР_ТоварыВЧеке
		|ИЗ
		|	&ТоварыРМК КАК ТоварыРМК
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВР_ТоварыЗаказПокупателя.Номенклатура КАК Номенклатура,
		|	ВР_ТоварыЗаказПокупателя.КоличествоУпаковок КАК КоличествоУпаковок,
		|	ЕСТЬNULL(ВР_ТоварыВЧеке.КоличествоУпаковок, 0) КАК КоличествоУпаковок1,
		|	""ИТОГ"" КАК Итог
		|ПОМЕСТИТЬ ВР_Общая
		|ИЗ
		|	ВР_ТоварыЗаказПокупателя КАК ВР_ТоварыЗаказПокупателя
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВР_ТоварыВЧеке КАК ВР_ТоварыВЧеке
		|		ПО ВР_ТоварыЗаказПокупателя.Номенклатура = ВР_ТоварыВЧеке.Номенклатура
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВР_Общая.Итог КАК Итог,
		|	СУММА(ВР_Общая.КоличествоУпаковок1) КАК КоличествоУпаковок1
		|ИЗ
		|	ВР_Общая КАК ВР_Общая
		|
		|СГРУППИРОВАТЬ ПО
		|	ВР_Общая.Итог";
		
		Запрос.УстановитьПараметр("ЗаказПокупателя", Объект.ЗаказПокупателя);
		Запрос.УстановитьПараметр("ТоварыРМК", Объект.Товары.Выгрузить(,"Номенклатура,КоличествоУпаковок"));
		...
		Иначе
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				Возврат ВыборкаДетальныеЗаписи.КоличествоУпаковок1=0;
			КонецЦикла;
		КонецЕсли;
...
&НаСервере
Функция ДКСотрудника()
	Если ЗначениеЗаполнено(Объект.ВладелецДисконтнойКарты) И ТипЗнч(Объект.ВладелецДисконтнойКарты) = Тип("СправочникСсылка.Контрагенты") Тогда
		Возврат ЗначениеЗаполнено(Объект.ВладелецДисконтнойКарты.ФизЛицо);
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции
...
Функция ИспользуетсяБиометрическийСканерСервер()  
	Возврат РаботаСБиометричекимиДанными.ИспользуетсяБиометрическийСканер(Объект.Магазин);
КонецФункции


&НаКлиенте
Процедура ПечатьКупона(Команда)
	Если Объект.Товары.Количество() > 0 Тогда

		ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
			"Введення чека повернення",
			"Перед введенням чека друку купона необхідно пробити, анулювати чи відкласти поточний чек!"
		);
		УстановитьТекущийЭлемент();	//	LNK 13.09.2017 08:17:04
	...
	Иначе
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыборЧекаДляПечатиКупоанЗавершение", ЭтотОбъект);
		ОткрытьФорму(РМККлиент.КонтекстИмениФормы("Обработка.РМК.Форма.ФормаВыбораЧека", Объект.ИмяВнешнейОбработки), ПараметрыОткрытия, ЭтотОбъект, УникальныйИдентификатор,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаКлиенте	//	LNK 01.02.2019 12:46:14
Процедура ВыборЧекаДляПечатиКупоанЗавершение(Результат, ДополнительныеПараметры)	Экспорт
	...
	Если НЕ Результат = Неопределено Тогда
		...
		Если ЭтоВозвратныйЧек(Результат.ЧекККМ) Тогда	//+HVOYA Mykhailo : если выбран возвратный чек, выдаем предупреждение
			...
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМК(
				"Введення чека для купона",
				"ВІДМОВЛЕНО! Документ «" + Результат.ЧекККМ + "» є чеком ПОВЕРНЕННЯ. Неприпустимо друк купона з чека повернення."
			);
			УстановитьТекущийЭлемент();	//	LNK 02.03.2018 09:34:59
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Результат.ЧекККМ)  Тогда
           		ПроверитьАцкцииНаПечатьУникальныхКупонов(Результат.ЧекККМ);
			КонецЕсли;
...
Функция ПроверитьАцкцииНаПечатьУникальныхКупонов(ЧекККМСсылка)
	...
	// Готовим данные
	МассивВходныеПараметры  = ПолучитьТекстКупона(ЧекККМСсылка);
	ОписаниеОшибки = "";
			...
			Для каждого ВходныеПараметры из МассивВходныеПараметры	цикл
				Если НЕ ВходныеПараметры = Неопределено тогда 
					...
					Если МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента() Тогда // Проверка на определенность рабочего места ВО
						...
						Если ЭтоПРРО Тогда 
						ТабДокумент = ПолучитьМакетКупона(ЧекККМСсылка);
...
&НаСервере
Процедура ЗаписатьСкидочныйКупонВРегистрПоКодуСервер(ЧекККМСсылка,КодСерийногоНомера)
	Документы.ЧекККМ.ЗаписатьСкидочныйКупонВРегистрПоКоду(ЧекККМСсылка,КодСерийногоНомера);
КонецПроцедуры

&НаСервере
Функция ПолучитьТекстУникальногоКупонаСервер(ЧекККМСсылка)
...
&НаСервере
Функция ПолучитьТекстКупона(ЧекККМСсылка)
	Возврат Документы.ЧекККМ.ПолучитьТекстУникальногоКупона(ЧекККМСсылка);
КонецФункции

&НаСервере
Функция ПолучитьНомерДляПечати(НоменклатураСкидочногоКупона,ЧекККМ)
	...
	|ГДЕ
	|	СерийныеНомераДляАкций.ЧекККМПродажи = &ЧекККМ
	|	И СерийныеНомераДляАкций.СерийныйНомер.Владелец = &Владелец"
	);
...
&НаСервере 
Функция ПолучитьМакетКупона(ЧекККМСсылка)
	Возврат Документы.ЧекККМ.ПолучитьТабДокКупона(ЧекККМСсылка);

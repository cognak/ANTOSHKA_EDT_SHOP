...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ГруппаСвойстваВидаНоменклатуры = ЭтотОбъект.Элементы["РазмещениеСвойстваВидаНоменклатуры"];
	ГруппаСвойстваНоменклатуры = ЭтотОбъект.Элементы["РазмещениеСвойстваНоменклатуры"];
	...
	Для каждого КлючИЗначение Из СоответствиеЭлементаИТипаЗначений Цикл
		ЭтотОбъект[КлючИЗначение.Ключ].ТипЗначения = КлючИЗначение.Значение;
	КонецЦикла;
	...
	Для Каждого ОписаниеСвойства Из Массив Цикл
		...
		Если ОписаниеСвойства.ТипСвойства = 1 Тогда
			...
			Для Каждого РеквизитФормы Из ОписаниеСвойства.РеквизитыФормы Цикл
				...
				ЭлементФормы = ЭтотОбъект.Элементы.Добавить(РеквизитФормы.ИмяРеквизитаФормы, Тип("ПолеФормы"), Группа);
		...
		ИначеЕсли ОписаниеСвойства.ТипСвойства = 2 Тогда
			...
			ЭлементФормы_ = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ТаблицаФормы"), Группы.ГруппаЛево);
			ЭлементФормы_.ПутьКДанным = ОписаниеСвойства.ИмяРеквизитаФормы;
			
			ЭлементФормы = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ПолеФормы"), ЭлементФормы_);
			ЭлементФормы.ПутьКДанным = ОписаниеСвойства.ИмяРеквизитаФормы + ".Значение";
			
		ИначеЕсли ОписаниеСвойства.ТипСвойства = 4 Тогда
			
			ЭлементФормы = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ПолеФормы"), ГруппаСвойстваВидаНоменклатуры);
			ЭлементФормы.Вид                = ВидПоляФормы.ПолеФлажка;
		...
		ИначеЕсли ОписаниеСвойства.ТипСвойства = 5 Тогда
			...
			ЭлементФормы = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ПолеФормы"), ГруппаСвойстваНоменклатуры);
			ЭлементФормы.Вид                = ВидПоляФормы.ПолеФлажка;
			ЭлементФормы.ПутьКДанным        = ОписаниеСвойства.ИмяРеквизитаФормы;
			ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
		...
		ИначеЕсли ОписаниеСвойства.ТипСвойства = 3 Тогда
			...
			ЭлементФормы = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ПолеФормы"), Группы.ГруппаЛево);
			ЭлементФормы.Вид         = ВидПоляФормы.ПолеВвода;
			ЭлементФормы.Заголовок   = "Минимум";
			ЭлементФормы.ПутьКДанным = ОписаниеСвойства.ИмяРеквизитаФормыНачальноеЗначение;
			
			ЭлементФормы = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ПолеФормы"), Группы.ГруппаПраво);
			ЭлементФормы.Вид         = ВидПоляФормы.ПолеВвода;
				...
				Для Каждого Эл Из Массив Цикл
					...
					Если Эл.Свойство = СтруктураЗначенияСвойств.Свойство Тогда
						...
						Если Эл.ТипСвойства = 1 Тогда
							...
							Для каждого РеквФормы Из Эл.РеквизитыФормы Цикл
								...
								Если СтруктураЗначенияСвойств.Значение.НайтиПоЗначению(РеквФормы.Значение) <> Неопределено ТОгда
									ЭтотОбъект[РеквФормы.ИмяРеквизитаФормы] = Истина;
								КонецЕсли;
						...
						ИначеЕсли Эл.ТипСвойства = 2 Тогда
							...
							ЭтотОбъект[Эл.ИмяРеквизитаФормы] = СтруктураЗначенияСвойств.Значение;
		...
		Для каждого Категория Из Параметры.ТекущийОтбор.ОтборПоКатегориям Цикл
			...
			Для Каждого Эл Из Массив Цикл
				...
				Если Эл.Свойство = Категория Тогда
					...
					ЭтотОбъект[Эл.ИмяРеквизитаФормы] = Истина;
	...
	Для каждого Свойство Из ОписаниеФормы.Массив Цикл
		...
		Если Свойство.ТипСвойства = 1 Тогда 
			...
			Для каждого РеквизитФормы Из Свойство.РеквизитыФормы Цикл
				...
				Если ЭтотОбъект[РеквизитФормы.ИмяРеквизитаФормы] Тогда
		...
		ИначеЕсли Свойство.ТипСвойства = 2 Тогда
			...
			Список = ЭтотОбъект[Свойство.ИмяРеквизитаФормы];
		...
		ИначеЕсли Свойство.ТипСвойства = 3 Тогда
			...
			СтруктураЗначенияСвойств.Вставить("НачальноеЗначение", ЭтотОбъект[Свойство.ИмяРеквизитаФормыНачальноеЗначение]);
			СтруктураЗначенияСвойств.Вставить("КонечноеЗначение", ЭтотОбъект[Свойство.ИмяРеквизитаФормыКонечноеЗначение]);
		...
		ИначеЕсли Свойство.ТипСвойства = 4 Тогда
			...
			Если ЭтотОбъект[Свойство.ИмяРеквизитаФормы] Тогда
...
Функция СоздатьЭлементыДляСвойства(Родитель, Заголовок)
	...
	ЭлементДекорация = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ДекорацияФормы"), Родитель);
	...
	ЭлементГруппа01     = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ГруппаФормы"), Родитель);
	...
	ЭлементДекорация = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ДекорацияФормы"), ЭлементГруппа01);
	ЭлементДекорация.Вид         = ВидДекорацииФормы.Надпись;
	ЭлементДекорация.Заголовок   = " ";
	ЭлементДекорация.Ширина      = 1;
	ЭлементДекорация.РастягиватьПоГоризонтали = Ложь;
	
	ЭлементГруппа02     = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ГруппаФормы"), ЭлементГруппа01);
	...
	ЭлементГруппа03     = ЭтотОбъект.Элементы.Добавить(ПолучитьУникальныйИдентификатор(), Тип("ГруппаФормы"), ЭлементГруппа01);

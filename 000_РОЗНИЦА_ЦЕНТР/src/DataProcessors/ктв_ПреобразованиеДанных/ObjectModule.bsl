...
//
Функция ЗначениеВСтруктуру(Значение, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Если ЭтоОбъект(Значение) Тогда
		Возврат ОбъектВСтруктуру(Значение, СписокСвойств, ИсключаяСвойства);
	КонецЕсли;
	
	Если ЭтоНаборЗаписейРегистра(Значение) Тогда
		Возврат НаборЗаписейРегистраВСтруктуру(Значение, СписокСвойств, ИсключаяСвойства);
	КонецЕсли;
...
//  ДополнительныеСвойства - Структура - дополнительные параметры для записи данных
//
// Возвращаемое значение:
//   Произвольный     - результат преобразования
//
...
//
Функция ЗначениеИзСтруктуры(СтруктураЗначения, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Если ЭтоОбъект(СтруктураЗначения) Тогда
		Возврат СоздатьОбъектИзСтруктуры(СтруктураЗначения, ДополнительныеСвойства);
	КонецЕсли;
	
	Если ЭтоНаборЗаписейРегистра(СтруктураЗначения) Тогда
		Возврат СоздатьНаборЗаписейРегистраИзСтруктуры(СтруктураЗначения, ДополнительныеСвойства);
	КонецЕсли;
...
Функция ОбъектВСтруктуру(Объект, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Если ЕстьОбъект Тогда
		ЭтоСправочник = ПредставленияРавны(ОписаниеВида.Имя, "Справочник");
		Для Каждого ТекРеквизит Из Метаданные.ОбщиеРеквизиты Цикл
			Если ЭтоСправочник И Объект.ЭтоГруппа Тогда
				Продолжить;
			КонецЕсли;
			Если ТекРеквизит.Состав.Найти(Объект.Метаданные()).Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать Тогда
...
Функция СоздатьОбъектИзСтруктуры(СтруктураОбъекта,
	...
	Если ЗначениеЗаполнено(Конструктор) Тогда
		...
		Для ПопыткаСоздания = 1 По 10 Цикл
			...
			Попытка
				...
				Если УстановитьСсылку Тогда
					...
					ЭлементБлокировки = Блокировка.Добавить(МетаОбъект.ПолноеИмя);
					ЭлементБлокировки.УстановитьЗначение("Ссылка", НоваяСсылка);
				...
				Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства, "Загрузка", Ложь) Тогда
					Объект.ОбменДанными.Загрузка = Истина;
				КонецЕсли;
				
				Для Каждого СтрокаДопСвойств Из ДополнительныеСвойства Цикл
					Объект.ДополнительныеСвойства.Вставить(СтрокаДопСвойств.Ключ, СтрокаДопСвойств.Значение);
				КонецЦикла;
				
				Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства, "ЗаписыватьДанные", Ложь) Тогда
					Объект.Записать();
				КонецЕсли;
			...
			Исключение
				...
				ОтменитьТранзакцию();
				Если ПопыткаСоздания = 10 Тогда
					ВызватьИсключение;
				Иначе
					ОбщегоНазначенияКлиентСервер.Пауза(1000 + ПопыткаСоздания);
				КонецЕсли;
	...
	Если ЗначениеЗаполнено(Конструктор) Тогда
		...
		Для ПопыткаСоздания = 1 По 10 Цикл
			...
			Попытка
				...
				Для Каждого СтрокаДопСвойств Из ДополнительныеСвойства Цикл
					НаборЗаписей.ДополнительныеСвойства.Вставить(СтрокаДопСвойств.Ключ, СтрокаДопСвойств.Значение);
				КонецЦикла;
...
Функция НайтиЗначениеПоОписанию(СтруктураЗначения, Знач ПоискПоНаименованию = Ложь) Экспорт
	...
	Иначе
		...
		КонецЕсли;
		//Если СсылкаСуществует(ВремСсылка) И Не ВремСсылка = Неопределено Тогда                     
		Если Не ВремСсылка = Неопределено Тогда                     
			Возврат ВремСсылка;
		КонецЕсли;
	КонецЕсли;
...
Процедура НаборЗаписейРегистраСведенийИзСтруктуры(НаборЗаписей,
	...
	Иначе
		Если МетаОбъект.ОсновнойОтборПоПериоду И НЕ МетаОбъект.ПериодичностьРегистраСведений =
				Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
...
Процедура ЗаполнитьДанныеЗаписиРегистраВСтруктуре(СтруктураЗаписи, Запись, СписокСвойств, ИсключаяСвойства)
	...
	Для Каждого ТекРеквизит Из Метаданные.ОбщиеРеквизиты Цикл
		Если НЕ ИспользоватьСвойство(ТекРеквизит.Имя, СписокСвойств, ИсключаяСвойства) Тогда
			Продолжить;
		КонецЕсли;
		Если ТекРеквизит.Состав.Найти(МетаОбъект).Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать Тогда
			СтруктураЗаписи.Вставить(ТекРеквизит.Имя, ЗначениеВСтруктуру(Запись[ТекРеквизит.Имя]));
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьДанныеЗаписиРегистраИзСтруктуры(Запись, СтруктураЗаписи, СписокСвойств, ИсключаяСвойства)
	...
	Для Каждого ТекПоле Из Метаданные.ОбщиеРеквизиты Цикл
		Если НЕ (СтруктураЗаписи.Свойство(ТекПоле.Имя)
		   И ИспользоватьСвойство(ТекПоле.Имя, СписокСвойств, ИсключаяСвойства)) Тогда
			Продолжить;
		КонецЕсли;

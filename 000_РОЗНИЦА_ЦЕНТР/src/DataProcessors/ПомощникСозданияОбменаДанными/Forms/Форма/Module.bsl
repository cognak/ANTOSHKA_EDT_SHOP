...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		...
		Если ОбменДаннымиСервер.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			ИмяПланаОбменаРИБ = ОбменДаннымиПовтИсп.ГлавныйУзел().Метаданные().Имя;
...
Процедура ИмяФайлаНастроекОбменаДаннымиДляЗагрузкиПриИзменении(Элемент)
	...
	Если    Не Файл.Существует()
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Укажите корректное имя файла настроек.'"));
...
Процедура КомандаГотово(Команда)
	...
	Если ВключитьРегистрациюВсехСобытийВЖурналеРегистрации Тогда
		...
		Если Не ПустаяСтрока(Предупреждение) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Запись сообщений об ошибках в журнал регистрации отключена,
	...
	ОбменДаннымиКлиент.ПриЗавершенииРаботыПомощникаСозданияОбмена(ЭтотОбъект, ЗапуститьМенеджерЗаданий());
...
Процедура ВыполнитьНастройкуВыгрузкиДанных(Команда)
	...
	РезультатОткрытия = ОткрытьФормуМодально(ИмяФормыНастройкиУзла, ПараметрыФормы, ЭтотОбъект);
...
Процедура НастройкаОграниченийРегистрацииДанных(Команда)
	...
	РезультатОткрытия = ОткрытьФормуМодально(ИмяФормыНастройкиУзла, Новый Структура("НастройкаОтборовНаУзле", НастройкаОтборовНаУзле), ЭтотОбъект);
...
Процедура ПроверитьПодключениеCOM(Команда)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'При установке подключения возникла ошибка(см. Журнал регистрации).'"));
...
Процедура КакОпределитьПараметрыПодключенияКВебСервису(Команда)
	...
	ОткрытьФормуМодально("Обработка.ПомощникСозданияОбменаДанными.Форма.ДополнительноеОписание", ПараметрыФормы, ЭтотОбъект);
...
Процедура УзнатьПрефиксЭтойИнформационнойБазы(Команда)
	...
	ОткрытьФормуМодально("Обработка.ПомощникСозданияОбменаДанными.Форма.УзнатьПрефиксЭтойИнформационнойБазы", , ЭтотОбъект);
...
Процедура ПредложитьПерейтиВЖурналРегистрации(ТекстВопроса, Знач Событие)
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ОткрытьФормуМодально("Обработка.ЖурналРегистрации.Форма", Отбор, ЭтотОбъект);
...
Процедура ВыбратьФайлНастроекОбмена(Интерактивно)
	...
	Если ФайлВыбран Тогда
		...
		Если Отказ Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.'"));
			Возврат;
		КонецЕсли;

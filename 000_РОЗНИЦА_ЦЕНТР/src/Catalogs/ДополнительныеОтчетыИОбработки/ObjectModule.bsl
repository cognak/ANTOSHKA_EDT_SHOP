...
Процедура ПередЗаписью(Отказ)
	...
	ВидПоСсылке = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Вид");

	Если НЕ ЗначениеЗаполнено(ВидПоСсылке)	Тогда

		ВидПоСсылке = Вид;	//	LNK 29.05.2019 13:01:03

	КонецЕсли;

	// Предварительные проверки
	Если НЕ ЭтоНовый() И Вид <> ВидПоСсылке Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(

...
//	LNK 18.03.2019 15:05:00
	Элементы.ФормаСправочникПрофилиГруппДоступаУстановитьСлужебный.Доступность = УправлениеПользователями.ПолучитьБулевоЗначениеПраваПользователя(ПланыВидовХарактеристик.ПраваПользователей.АдминистраторПолномочный, Ложь);
...
Процедура ВидыДоступаПриАктивизацииСтроки(Элемент)
	...
	ПриИзмененииТекущегоВидаДоступа(ЭтотОбъект);
...
Процедура ВидыДоступаВидДоступаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Элементы.ПоказыватьНеиспользуемыеВидыДоступа.Пометка Тогда
		СтруктураПараметров.Добавить(Новый ПараметрВыбора("ПоказыватьНеиспользуемыеВидыДоступа", Истина));
	КонецЕсли;
	
	Элемент.ПараметрыВыбора = Новый ФиксированныйМассив(СтруктураПараметров);
...
Процедура ЗначенияДоступаПриИзменении(Элемент)
	...
	ОбновитьНомерСтрокиПоВиду(ЭтотОбъект, Элементы.ВидыДоступа.ТекущиеДанные);
	ОбновитьПредставлениеИспользованияВидаДоступа(ЭтотОбъект, Элементы.ВидыДоступа.ТекущиеДанные);
...
Процедура ЗаполнитьСвойстваВидовДоступаВФорме()
	...
	Для каждого Строка Из Объект.ВидыДоступа Цикл
		...
		КонецЕсли;
		ОбновитьПредставлениеИспользованияВидаДоступа(ЭтотОбъект, Строка);
		ОбновитьНомерСтрокиПоВиду(ЭтотОбъект, Строка);
	КонецЦикла;
...
Процедура ОбработатьИнтерфейсРолей(Действие, ОсновнойПараметр = Неопределено)
	...
	ПараметрыДействия.Вставить("СкрытьРольПолныеПрава",
		НЕ (Объект.Ссылка = Справочники.ПрофилиГруппДоступа.Администратор ИЛИ Объект.Ссылка = Справочники.ПрофилиГруппДоступа.АдминистраторДанных));

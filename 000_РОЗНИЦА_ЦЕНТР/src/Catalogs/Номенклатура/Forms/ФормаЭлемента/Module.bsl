...
//	Элементы.Марка.Видимость = ИспользоватьАссортимент;
	...
	//А++ по задаче https://awdev.atlassian.net/browse/RETAIL1C-864
	Элементы.ИсключитьИзОбменаNavision.Доступность = Пользователи.ЭтоПолноправныйПользователь();
	//А++ по задаче https://awdev.atlassian.net/browse/RETAIL1C-864
...
//А++ по задаче https://awdev.atlassian.net/browse/RETAIL1C-864
&НаСервере
Процедура ИсключитьИзОбменаNavisionПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ИсключитьИзОбменаNavisionПриИзменении(Элемент)
	ИсключитьИзОбменаNavisionПриИзмененииНаСервере();
КонецПроцедуры
//А++ по задаче https://awdev.atlassian.net/browse/RETAIL1C-864
...
&НаКлиенте
Процедура СвойстваНоменклатурыНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка) Экспорт
	СтандартнаяОбработка = Ложь; 
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ИмяЭлемента",Элемент.Имя);
	ПараметрыФормы.Вставить("ТипСвойства",ПредопределенноеЗначение("Перечисление.ТипыСвойствНоменклатуры.МодельТовара"));
	ОткрытьФорму("Справочник.СвойстваНоменклатуры.ФормаВыбора",ПараметрыФормы,ЭтаФорма,,,,Новый ОписаниеОповещения("СвойстваНоменклатурыЗавершениеВыбора",ЭтаФорма,ПараметрыФормы));
КонецПроцедуры

&НаКлиенте
Процедура СвойстваНоменклатурыЗавершениеВыбора(ДанныеВыбора, ПараметрыВыбора) Экспорт    
	Если ДанныеВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ЭтаФорма[ПараметрыВыбора.ИмяЭлемента] = ДанныеВыбора;
КонецПроцедуры

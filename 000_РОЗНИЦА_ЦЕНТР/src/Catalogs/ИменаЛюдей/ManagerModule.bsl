...
Функция ИмяКорректно(Имя, Пол = Неопределено, Ссылка = Неопределено,ИмяРебенка = ЛОЖЬ)	Экспорт  //А++ 20250205 добавил параметр ИмяРебенка для выделения из общей массы
		...
		Если НЕ Корректно И НЕ Ссылка = Неопределено И ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда
			...
			Если НЕ КонтактнаяИнформацияКлиентСервер.ЭтоПустоеНаименование(Имя) Тогда
				...
				КлючиЗаписи = Новый Структура(
					...
					, "ФИОФизЛиц"
					, ?(ИмяРебенка,"Имя ребенка «" + Запрос.Параметры.Имя + "» не найдено в справочнике.","Имя «" + Запрос.Параметры.Имя + "» не найдено в справочнике.")  //А++ 20250205 добавил параметр ИмяРебенка для выделения из общей массы
				);

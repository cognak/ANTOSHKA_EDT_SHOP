...
Процедура ОбработкаКоманды(МассивРассылок, Параметры)
	...
	ФормаИсточник = Параметры.Источник;
	
	ПредварительныеНастройки = Неопределено;
	Если ФормаИсточник.ИмяФормы = "Справочник.РассылкиОтчетов.Форма.ФормаЭлемента" Тогда
		ТекстСообщения = НСтр("ru = 'Выполняется рассылка отчетов'");
		
		Объект = ФормаИсточник.Объект;
	...
	Результат = ВыполнитьРассылкиВФоновомЗадании(ПараметрыВызоваСервера, ФормаИсточник.УникальныйИдентификатор);
	...
	ИначеЕсли Результат.Статус = "Выполняется" Тогда
		ФормаДлительнойОперации = ДлительныеОперацииКлиент.ОткрытьФормуДлительнойОперации(ФормаИсточник, Результат.Детали.ИдентификаторЗадания);
		...
		ФормаИсточник.ПараметрыФоновогоЗадания.Очистить();
		ФормаИсточник.ПараметрыФоновогоЗадания.Добавить(Результат.Детали.ИдентификаторЗадания);
		ФормаИсточник.ПараметрыФоновогоЗадания.Добавить(Результат.Детали.АдресХранилища);
		ФормаИсточник.ПараметрыФоновогоЗадания.Добавить(ПараметрыОбработчика);
		ФормаИсточник.ПараметрыФоновогоЗадания.Добавить(ФормаДлительнойОперации);
		
		ФормаИсточник.ПодключитьОбработчикОжидания("Подключаемый_ПроверитьВыполнениеФоновогоЗадания", ПараметрыОбработчика.ТекущийИнтервал, Истина);
	КонецЕсли;
КонецПроцедуры

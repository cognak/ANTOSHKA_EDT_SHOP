...
Процедура МесяцыПриИзменении(Элемент)
	Если Элемент <> Неопределено Тогда
		Расписание.Месяцы = ИзменитьИРазадресоватьМассив(ЭтотОбъект[Элемент.Имя], Кэш.Соответствия.Месяцы[Элемент.Имя], Расписание.Месяцы);
	КонецЕсли;
...
Процедура ДниНеделиПриИзменении(Элемент)
	Если Элемент <> Неопределено Тогда
		Расписание.ДниНедели = ИзменитьИРазадресоватьМассив(ЭтотОбъект[Элемент.Имя], Кэш.Соответствия.ДниНедели[Элемент.Имя], Расписание.ДниНедели);
	КонецЕсли;
...
Процедура ТипПолучателейРассылкиПриИзменении(Элемент)
	...
	Если Найденные.Количество() = 1 Тогда
		...
		Объект.ВидТелефонаПолучателей = СтрокаПолучатель.ОсновнойВидТелефон;
...
Процедура ИспользоватьЭлектроннуюПочтуПриИзменении(Элемент)
	...
	Если НЕ Публиковать И НЕ Объект.ИспользоватьЭлектроннуюПочту И НЕ Объект.ИспользоватьSMSСообщения Тогда
...
&НаКлиенте
Процедура ИспользоватьSMSСообщенияПриИзменении(Элемент)

	ВидимостьДоступностьКорректность(ЭтотОбъект, "ИспользоватьSMSСообщения");
...
Процедура ДобавитьИзменитьШаблонДатыРассылки(Команда)
	...
	ДобавитьИзменитьШаблонДаты("[ДатаВыполнения(");
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьИзменитьШаблонДатаВчерашняя(Команда)

	ДобавитьИзменитьШаблонДаты("[ДатаВчерашняя(");

КонецПроцедуры

&НаКлиенте
Процедура ДобавитьИзменитьШаблонДаты(Префикс)
	...
	Для Каждого СтрокаПолучатель Из ТаблицаТиповПолучателей Цикл
		...
		Если СтрокаПолучатель.ИОМД = Объект.ТипПолучателейРассылки Тогда
			...
			Если Объект.ВидПочтовогоАдресаПолучателей.Пустая() И ЗначениеЗаполнено(СтрокаПолучатель.ОсновнойВидКИ) Тогда
				Объект.ВидПочтовогоАдресаПолучателей = СтрокаПолучатель.ОсновнойВидКИ;
			КонецЕсли;
			Если Объект.ВидТелефонаПолучателей.Пустая() И ЗначениеЗаполнено(СтрокаПолучатель.ОсновнойВидТелефон) Тогда
				Объект.ВидТелефонаПолучателей = СтрокаПолучатель.ОсновнойВидТелефон;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаписатьНастройкиСтрокиОтчеты(ИдентификаторСтроки)
	...
	Иначе
		...
		СохраняемоеЗначение = НастройкиТекущегоОтчета.Выгрузить().Скопировать(Отбор, ИменаРеквизитов);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ВидимостьДоступностьКорректность(ЭтотОбъект, Изменения = "")
	Объект = ЭтотОбъект.Объект;
	Элементы = ЭтотОбъект.Элементы;
	
	Если Изменения = "" ИЛИ Изменения = "FTPСерверИКаталог" Тогда
		ЭтотОбъект.FTPСерверИКаталог = "ftp://"+ Объект.FTPСервер + Объект.FTPКаталог;
	КонецЕсли;
	...
	Если Изменения = "" ИЛИ Изменения = "ВидРассылки" Тогда
		...
		ИспользуетсяГруппаЛичныхРассылок = (Объект.Родитель = ЭтотОбъект.Кэш.ГруппаЛичныхРассылок);
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(ЭтотОбъект, "Родитель", , 
				НСтр("ru = 'Группа установлена в соответствии с видом рассылки'")
			);
			Объект.Родитель = ?(Объект.Личная, ЭтотОбъект.Кэш.ГруппаЛичныхРассылок, Неопределено);
		КонецЕсли;
		
		Если Объект.Личная Тогда
			РассылкаОбщая = Ложь;
			ЭтотОбъект.ВидРассылки = "Личная";
		ИначеЕсли Объект.Персонализирована Тогда
			РассылкаОбщая = Ложь;
			ЭтотОбъект.ВидРассылки = "Персонализирована";
		Иначе
			РассылкаОбщая = Истина;
			ЭтотОбъект.ВидРассылки = "Общая";
		КонецЕсли;
		...
		Если Объект.ИспользоватьПапку Тогда
			ЭтотОбъект.ДругойСпособДоставки = "ИспользоватьПапку";
			ЭтотОбъект.Публиковать = Истина;
		ИначеЕсли Объект.ИспользоватьСетевойКаталог Тогда
			ЭтотОбъект.ДругойСпособДоставки = "ИспользоватьСетевойКаталог";
			ЭтотОбъект.Публиковать = Истина;
		ИначеЕсли Объект.ИспользоватьFTPРесурс Тогда
			ЭтотОбъект.ДругойСпособДоставки = "ИспользоватьFTPРесурс";
			ЭтотОбъект.Публиковать = Истина;
		Иначе
			ЭтотОбъект.ДругойСпособДоставки = Элементы.ДругойСпособДоставки.СписокВыбора[0].Значение;
			ЭтотОбъект.Публиковать = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если Изменения = "" ИЛИ Изменения = "Отчеты" Тогда
		КоличествоОтчетов = ЭтотОбъект.Объект.Отчеты.Количество();
	...
	Если Изменения = "" ИЛИ Изменения = "ДругойСпособДоставки" ИЛИ Изменения = "Публиковать" ИЛИ Изменения = "ВидРассылки" Тогда
		Элементы.ДругойСпособДоставки.Доступность = ЭтотОбъект.Публиковать;
		Элементы.ПараметрыДоставки.Доступность = ЭтотОбъект.Публиковать;
		Элементы.ПараметрыДоставки.ТекущаяСтраница = Элементы[ЭтотОбъект.ДругойСпособДоставки];
		Элементы.ПроверитьПубликацию.Доступность = ЭтотОбъект.Публиковать;
	КонецЕсли;
	
	Если Изменения = ""
	ИЛИ  Изменения = "ВидРассылки"
	ИЛИ  Изменения = "ИспользоватьЭлектроннуюПочту"
	ИЛИ  Изменения = "ИспользоватьSMSСообщения" Тогда

		Элементы.ГруппаУчетнаяЗапись.Доступность = Объект.ИспользоватьЭлектроннуюПочту;
		Элементы.ТемаИТекстПисьма.Доступность = Объект.ИспользоватьЭлектроннуюПочту;
		Элементы.Получатели.Доступность       = Объект.ИспользоватьЭлектроннуюПочту ИЛИ Объект.ИспользоватьSMSСообщения;
		Элементы.Ответственные.Доступность    = Объект.ИспользоватьЭлектроннуюПочту;
		Элементы.ВидПочтовогоАдресаПолучателей.Доступность = Объект.ИспользоватьЭлектроннуюПочту;
		Элементы.ВидТелефонаПолучателей.Доступность        = Объект.ИспользоватьSMSСообщения;
	...
	ИЛИ  Изменения = "ВидРассылки" 
		...
		ВидимостьОпции = (Объект.ИспользоватьЭлектроннуюПочту И ЭтотОбъект.Публиковать);
	...
	Если Изменения = "" ИЛИ Изменения = "ПериодичностьРасписания" Тогда
		ИмяПеречисления = ЭтотОбъект.Кэш.Перечисления.Периодичности[Объект.ПериодичностьРасписания];
		...
		Если Изменения = "ПериодичностьРасписания"
			...
			// Общие параметры
			ЭтотОбъект.Расписание.ДатаНачала = '00010101';
			ЭтотОбъект.Расписание.ДатаКонца  = '00010101';
			ЭтотОбъект.Расписание.ВремяКонца = '00010101';
			ЭтотОбъект.Расписание.ВремяЗавершения = '00010101';
			ЭтотОбъект.Расписание.ДеньНеделиВМесяце = 0;
			ЭтотОбъект.Расписание.ДетальныеРасписанияДня = Новый Массив;
			ЭтотОбъект.Расписание.ИнтервалЗавершения = 0;
			ЭтотОбъект.Расписание.ПаузаПовтора = 0;
			ЭтотОбъект.Расписание.ПериодНедель = 0;
			ЭтотОбъект.Расписание.ПериодПовтораВТечениеДня = 0;
			
			Если ИмяПеречисления <> "Ежедневно" Тогда
				ЭтотОбъект.Расписание.ПериодПовтораДней = 1;
			КонецЕсли;
			...
			Если ИмяПеречисления <> "Еженедельно" Тогда
				...
				Для н = 1 По 7 Цикл
					ВыбранныеДниНедели.Добавить(н);
				КонецЦикла;
				ЭтотОбъект.Расписание.ДниНедели = ВыбранныеДниНедели;
			КонецЕсли;
			...
			Если ИмяПеречисления <> "Ежемесячно" Тогда
				...
				Для н = 1 По 12 Цикл
					ВсеМесяцы.Добавить(н);
				КонецЦикла;
				ЭтотОбъект.Расписание.Месяцы = ВсеМесяцы;
				ЭтотОбъект.Расписание.ДеньВМесяце = 0;
			КонецЕсли;
		КонецЕсли;
		
		// Восстанавливаем параметры на текущей закладке в соответствии с параметрами расписания
		Если ИмяПеречисления = "Ежедневно" Тогда
			ЭтотОбъект.ВремяНачала = ЭтотОбъект.Расписание.ВремяНачала;
			ЭтотОбъект.ПериодПовтораДней = ЭтотОбъект.Расписание.ПериодПовтораДней;
		ИначеЕсли ИмяПеречисления = "Еженедельно" Тогда
			ЭтотОбъект.ВремяНачала = ЭтотОбъект.Расписание.ВремяНачала;
			Для Каждого КлючИЗначение Из ЭтотОбъект.Кэш.Соответствия.ДниНедели Цикл
				ЭтотОбъект[КлючИЗначение.Ключ] = (ЭтотОбъект.Расписание.ДниНедели.Найти(КлючИЗначение.Значение) <> Неопределено);
			КонецЦикла;
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			ЭтотОбъект.ВремяНачала = ЭтотОбъект.Расписание.ВремяНачала;
			Если ЭтотОбъект.Расписание.ДеньВМесяце >= 0 Тогда
				ЭтотОбъект.ДеньВМесяце = ЭтотОбъект.Расписание.ДеньВМесяце;
			...
			КонецЕсли;
			Для Каждого КлючИЗначение Из ЭтотОбъект.Кэш.Соответствия.Месяцы Цикл
				ЭтотОбъект[КлючИЗначение.Ключ] = (ЭтотОбъект.Расписание.Месяцы.Найти(КлючИЗначение.Значение) <> Неопределено);
			КонецЦикла;
		КонецЕсли;
	...
	Если Изменения = "" ИЛИ Изменения = "НачалоКонецМесяца" Тогда
		Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = ?(ЭтотОбъект.Расписание.ДеньВМесяце >= 0, "начала", "конца");
	КонецЕсли;
	
	Если Изменения = "" ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРасписание Тогда
		ЭтотОбъект.ПредставлениеРасписания = Строка(ЭтотОбъект.Расписание);
	КонецЕсли;
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьМодифицированностьФормы(ЭтотОбъект, Поле = "", ПутьКДанным = "", Текст = "")
	Если НЕ ЭтотОбъект.Модифицированность Тогда
		ЭтотОбъект.ФормаБылаМодифицированаНаСервере = Истина;

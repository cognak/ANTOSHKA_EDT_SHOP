		...
		//	тогда нужно подготовить строку свойств объекта метаданных.
			...
			//	Обновление свойств существующих объектов метаданных, если изменились.
				...
				Если Свойства.Наименование         <> СвойстваОбъекта.Наименование
				 ...
				 ИЛИ СвойстваОбъекта.БезКлючаОбъектаМетаданных И НЕ КлючОбъектаМетаданных = Тип("Неопределено")
				 Тогда
...
Функция СвойстваКоллекцийОбъектовМетаданных() Экспорт
	...
	Строка.БезДанных       = Истина;	//	не может быть метода ".ПустаяСсылка"
...
//	Замена дублей в подчиненном РИБ-узле.
	...
	|	Справочник.ИдентификаторыОбъектовМетаданных КАК Идентификаторы
	|ГДЕ
	|	НЕ Идентификаторы.НоваяСсылка = ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка)"
	);
...
#Область ПоддержкаПолученияЗначенийНастройкиОбъектМетаданных
...
Функция СписыватьАкционныеБонусныеБаллы(ПолноеИмяМетаданных)	Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ТаблицаСправочник.Ссылка КАК Идентификатор,
	|	ТаблицаСправочник.СписыватьАкционныеБонусныеБаллы КАК СписыватьАкционныеБонусныеБаллы
	|ИЗ
	|	Справочник.ИдентификаторыОбъектовМетаданных КАК ТаблицаСправочник
	|ГДЕ
	|	ТаблицаСправочник.ПолноеИмя = &ПолноеИмя
	|	И ТаблицаСправочник.СписыватьАкционныеБонусныеБаллы"
	);

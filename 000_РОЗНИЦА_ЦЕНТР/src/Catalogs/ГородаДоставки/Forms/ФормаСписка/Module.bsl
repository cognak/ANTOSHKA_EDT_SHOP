...
Процедура ОбновитьОтделения(Команда)
	...
	Если Не ТекДанные = Неопределено Тогда
		ОбновитьОтделенияНаСервере(ТекДанные.Ссылка);
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьОтделенияНаСервере(ГородДляУлиц)

	ТаблицаМетодов = Неопределено;
	ПараметрыПоиска = Новый Структура("ГородЗагрузки", ГородДляУлиц);
	ОбменНПСервер.ЗаполнитьСправочникиНП(ТаблицаМетодов, "getWarehouses", ПараметрыПоиска);

...
Процедура ИспользоватьПериодДействияПриИзменении(Элемент)
	...
	Если Не ИспользоватьПериодДействия = 2 Тогда
		Объект.ДатаПериодаДействия = Дата(1, 1, 1);
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыНоменклатураПредоставления
...
Процедура НоменклатураПредоставленияНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НоменклатураНачалоВыбора(Элемент,
							 Элементы.НоменклатураПредоставления.ТекущиеДанные.Номенклатура,
							 ДанныеВыбора,
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ИспользоватьПериодДействия = ?(Объект.ДатаПериодаДействия = Дата(1, 1, 1), ИспользоватьПериодДействия, 2);
...
Процедура УстановитьОформлениеЭлементов()
	...
	НачислитьБонусныеБаллы = (Объект.СпособПримененияСкидки
	...
	Если НачислитьБонусныеБаллы Тогда
		...
		Элементы.НачислятьБаллыПоКредитнойПрограмме.ШрифтЗаголовка = Новый Шрифт(, ,
			Объект.НачислятьБаллыПоКредитнойПрограмме);
		Элементы.СовместноеНачислениеСкидокИБонусов.ШрифтЗаголовка = Новый Шрифт(, ,
			Объект.СовместноеНачислениеСкидокИБонусов);

		Элементы.ГруппаСрокДействияПраво.Доступность = (ИспользоватьПериодДействия = 1);
		Элементы.ДатаПериодаДействия.Доступность = (ИспользоватьПериодДействия = 2);
...
Процедура НазначитьКолонкеИндексКартинки(ТекущиеДанные)
	...
	Если Не ТекущиеДанные = Неопределено Тогда
		...
		ТекущиеДанные.КнопкаExcel = ?(ЗначениеЗаполнено(ТекущиеДанные.Номенклатура),
									  ?(ТипЗнч(ТекущиеДанные.Номенклатура)
			= Тип("СправочникСсылка.СегментыНоменклатуры"), 8, 16),
									  0);
...
Процедура НоменклатураНачалоВыбора(Элемент, Значение, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не Значение = Неопределено Тогда
		...
		ОтборПоСпособу.Добавить(ПредопределенноеЗначение("Перечисление.СпособыФормированияСегментов.ФормироватьВручную"),
								ПредопределенноеЗначение("Перечисление.СпособыФормированияСегментов.ФормироватьВручную"));
		ОтборПоСпособу.Добавить(ПредопределенноеЗначение("Перечисление.СпособыФормированияСегментов.ФормироватьДинамически"),
		...
		ПараметрыФормыВыбора.Вставить("Отбор", Новый Структура("СпособФормирования, ПометкаУдаления", ОтборПоСпособу,
			Ложь));

		ОткрытьФорму("Справочник." + ?(ТипЗнч(Значение) = Тип("СправочникСсылка.СегментыНоменклатуры"),
									   "СегментыНоменклатуры",
									   "Номенклатура") + ".ФормаВыбора", ПараметрыФормыВыбора, Элемент);
...
Процедура ВыборКомандыЗагрузкиСегментаНоменклатуры(ТекущиеДанные)
	...
	Если ЗначениеЗаполнено(СегментНоменклатуры) И ТипЗнч(СегментНоменклатуры)
		= Тип("СправочникСсылка.СегментыНоменклатуры") Тогда

		ДополнительныеПараметры = Новый Структура("СегментНоменклатуры, ФайлВыбран", СегментНоменклатуры, Ложь);

		ОписаниеОповещения = Новый ОписаниеОповещения("ВыборКомандыЗагрузкиСегментаНоменклатурыЗавершение", ЭтотОбъект,
		...
		ПоказатьВопрос(ОписаниеОповещения,
					   ...
					   "Загрузка сегмента из таблицы «Excel»",
...
Процедура ВыборКомандыЗагрузкиСегментаНоменклатурыЗавершение(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ФайлВыбран = Ложь Тогда
		...
		ДиалогВыбораФайла.Фильтр = НСтр("ru = 'Лист Excel'") + " (*.xls)|*.xls|" + НСтр("ru = 'Лист Excel2007'")
			+ " (*.xlsx)|*.xlsx";
	...
	ИначеЕсли ДополнительныеПараметры.ФайлВыбран = Истина Тогда
		...
		Если Не ФайлЗагрузки.Существует() Тогда
			...
			ПоказатьОповещениеПользователя("Загрузка невозможна",
										   ,
										   "Указанный файл не обнаружен - укажите существующий файл!",
										   БиблиотекаКартинок.Вопрос32);

		Иначе

			ПолноеИмяИмпортируемогоФайла = ДополнительныеПараметры.ПолноеИмяИмпортируемогоФайла;	//	запомним на следующую загрузку

			ПоказатьОповещениеПользователя("Загрузка данных из файла ...", , "Выполняется загрузка «"
				+ ПолноеИмяИмпортируемогоФайла + "»", БиблиотекаКартинок.Предупреждение32);

			АдресВХранилище = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ПолноеИмяИмпортируемогоФайла),
															УникальныйИдентификатор);
			ЗагрузитьСоставСегментаИзТаблицыExcel(ДополнительныеПараметры.СегментНоменклатуры,
												  АдресВХранилище,
												  ФайлЗагрузки.Расширение);
...
Процедура ЗагрузитьСоставСегментаИзТаблицыExcel(СегментНоменклатуры, АдресВХранилище, РасширениеИмени)
	...
	Запрос.УстановитьПараметр("СписокТоваров", ЗаполнениеОбъектов.ЗагрузитьДанныеВМассивИзТаблицыExcel(АдресВХранилище,
																									   РасширениеИмени,
																									   Истина));

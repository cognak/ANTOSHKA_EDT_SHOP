...
Процедура ОбновитьРолиПользователейПриИзмененииГруппыДоступа(ПарольПользователяСервиса)
	...
	Перем ОбновитьРолиПринудительно;
	
	УстановитьПривилегированныйРежим(Истина);

	ДополнительныеСвойства.Свойство("ОбновитьРолиПринудительно", ОбновитьРолиПринудительно);

	// Обновление ролей для добавленных, оставшихся и удаленных пользователей.
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ГруппаДоступа"  , Ссылка);
	Запрос.УстановитьПараметр("СтарыеУчастники", ?(ОбновитьРолиПринудительно = Истина, Новый Массив, СтарыеУчастники));
	...
	//	Добавляем в список всех пользователей, имеющих роль ПолныеПрава, но отсутствующих в текущей группе
...
Процедура ВосстановитьСоставУчастниковГруппыДоступаАдминистраторы()
	...
	//	Так уж случилось... существует общий модуль - "Пользователи"
		...
		|ИЗ
		|	Справочник.ГруппыДоступа.Пользователи КАК ТабличнаяЧасть
		|ГДЕ
		|	ТабличнаяЧасть.Ссылка = &ТекущаяСсылка
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки"
		);
		...
		ЭтотОбъект.Пользователи.Загрузить(Запрос.Выполнить().Выгрузить());

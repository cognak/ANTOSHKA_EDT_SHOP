...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если МенеджерОбъекта <> Неопределено Тогда
		...
		Для Каждого Макет Из МетаданныеОбъекта.Макеты Цикл
			Если Макет.ТипМакета = Метаданные.СвойстваОбъектов.ТипМакета.СхемаКомпоновкиДанных 
				...
				ЭтотОбъект[ИмяСтраницыКомпоновщикаНастроек].Инициализировать(ИсточникДоступныхНастроекКомпоновкиДанных);
	...
	КомпоновщикНастроек = ЭтотОбъект["СКД" + ИмяОбъектаМетаданных + "ПоляШаблона"];
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ШаблонСохраняется И Модифицированность Тогда
		...
		Оповестить("ИзмененШаблон", ПоместитьВоВременноеХранилищеШаблон(Структура), ЭтотОбъект);
	КонецЕсли;
...
Процедура ШаблонЧекаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОтветФормы = ОткрытьФормуМодально("Справочник.ХранилищеШаблонов.Форма.ФормаРедактированияПараметров", СтруктураПараметров, ЭтотОбъект);
...
Процедура ДобавитьВШаблонДоступнуюСтроку(Команда)
	Если ТипЗнч(ЭтотОбъект.ТекущийЭлемент) = Тип("ТаблицаФормы") Тогда
		Если Найти(ЭтотОбъект.ТекущийЭлемент.Имя, "КомпоновщикНастроек") > 0.00 Тогда
			Для Каждого ВыбраннаяСтрока Из ЭтотОбъект.ТекущийЭлемент.ВыделенныеСтроки Цикл
				ДобавитьРеквизитДокументаВШаблонЧека(ЭтотОбъект.ТекущийЭлемент, ВыбраннаяСтрока);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьРеквизитДокументаВШаблонЧека(Элемент, ВыбраннаяСтрока, СтандартнаяОбработка = Ложь)
	...
	текСтр = ЭтотОбъект[РеквизитКомпоновщикНастроек].Настройки.ДоступныеПоляПорядка.ПолучитьОбъектПоИдентификатору(ВыбраннаяСтрока);
...
Процедура ДобавитьСтрокуВШаблонЧека(ИмяЭлемента, ТипЭлемента, Знач КоллекцияСтрок = Неопределено, ИмяКолонки = "")
	...
	Иначе
		КоллекцияСтрок = ТекущаяСтрока.Строки;
	КонецЕсли;
	КомпоновщикНастроек = ЭтотОбъект[Элементы.ГруппаКомпоновщикиНастроек.ТекущаяСтраница.Имя];

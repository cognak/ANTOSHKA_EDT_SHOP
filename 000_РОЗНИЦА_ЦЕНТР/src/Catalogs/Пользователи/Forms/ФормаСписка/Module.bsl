...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Отбор.Свойство("Недействителен") Тогда
		...
		Параметры.Отбор.Удалить("Недействителен");	//	LNK 19.09.2017 10:51:18

	КонецЕсли;

	ОбновитьОтборНедействительныхПользователей(ЭтотОбъект);
	ОбновитьОтборСотрудниковКЦ(ЭтотОбъект);
...
//	КонецЕсли;
	...
	Элементы.ГруппыПользователей.РежимВыбора = Параметры.РежимВыбора;
...
Процедура ЗаполнитьПараметрыДинамическихСписков()
	...
	ПользователиСписок.Параметры.УстановитьЗначениеПараметра("ИсключительныйРежим", ТехническаяПоддержкаВызовСервера.ИсключительныйРежим());
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьОтборСотрудниковКЦ(Форма)
	
	Элементы = Форма.Элементы;
	
	Если Форма.Параметры.Свойство("ОтборСотрудниковКЦ") Тогда
		
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			Форма.ПользователиСписок, "СотрудникКЦ", Истина, , , Истина);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			Форма.ПользователиСписок, "СотрудникКЦ", Истина, , , Ложь);
	КонецЕсли;

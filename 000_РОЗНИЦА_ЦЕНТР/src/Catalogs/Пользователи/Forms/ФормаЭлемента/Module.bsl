...
//	LNK 07.10.2016 09:59:22
	Если Объект.Ссылка = Справочники.Пользователи.АдминистраторАвтоматов Тогда
...
&НаКлиенте	//	LNK 03.03.2024 08:00:39
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)

	Если ИмяСобытия = "ЗАПИСЬ_ЭЛЕМЕНТА" И Параметр.Объект = Объект.Ссылка Тогда
...
Процедура ДоступКИнформационнойБазеРазрешенПриИзменении(Элемент)
	...
	Если НЕ ПользовательИБСуществует
	   И ДоступКИнформационнойБазеРазрешен Тогда
		...
		//	ПользователиСлужебныйКлиентСервер.ПолучитьКраткоеИмяПользователяИБ(
		//		Объект.Наименование);
	//	LNK 06.01.2017 09:02:47
		СоставИмени = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, Объект.Наименование,, " ");
		ПользовательИнфБазыИмя = ?(СоставИмени.Количество() > 2, СоставИмени[0] + " " + СоставИмени[1], Объект.Наименование);
...
Процедура ФизЛицоПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.ФизЛицо) Тогда
		...
		ИначеЕсли Объект.Наименование <> Строка(Объект.ФизЛицо) Тогда

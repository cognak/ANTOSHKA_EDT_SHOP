...
Функция НайтиУлицу(НаименованиеУлицы, Город, ОператорДоставки) Экспорт
	...
	Иначе
		...
		Результат = ОбменНПСервер.ДобавитьУлицуНовый(Город.ИдентификаторSettlement, НаименованиеУлицы);
		
		Если Не (ТипЗнч(Результат) = Тип("СправочникСсылка.Улицы") И ЗначениеЗаполнено(Результат)) Тогда

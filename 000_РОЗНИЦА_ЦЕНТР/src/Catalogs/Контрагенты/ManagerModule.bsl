...
//  Структура - Получить реквизиты контрагента:
...
Функция ПолучитьРеквизитыКонтрагента(Контрагент) Экспорт
	...
	СтруктураРеквизитов.Вставить("НомерСвидетельства", "");
	...
	Если Выборка.Следующий() Тогда
		СтруктураРеквизитов.Представление = Выборка.Наименование;
		СтруктураРеквизитов.Наименование = ?(Не ПустаяСтрока(Выборка.НаименованиеПолное), Выборка.НаименованиеПолное, Выборка.Наименование);
		СтруктураРеквизитов.КодПоЕДРПОУ	= Выборка.КодПоЕДРПОУ;
		СтруктураРеквизитов.НомерСвидетельства = Выборка.НомерСвидетельства;
	КонецЕсли;
...
// 
// Параметры:
//  Контрагент - СправочникОбъект.Контрагенты, СправочникСсылка.Контрагенты - Контрагент
//  НомерТелефонаПараметр - Строка, Неопределено - Номер телефона параметр
// 
// Возвращаемое значение:
//  Строка - Получить номер телефона
Функция ПолучитьНомерТелефона(Контрагент, НомерТелефонаПараметр = Неопределено)	Экспорт
...
// Возвращаемое значение:
//  Строка - Получить номер телефона
Функция Получить_EMail(Контрагент, АдресЭП_Параметр = Неопределено)	Экспорт

	АдресЭП = "";

	Если ТипЗнч(АдресЭП_Параметр) = Тип("Строка") И НЕ ПустаяСтрока(АдресЭП_Параметр) Тогда

		АдресЭП = СокрЛП(АдресЭП_Параметр);

	ИначеЕсли ТипЗнч(Контрагент) = Тип("СправочникОбъект.Контрагенты") Тогда

		СтрокаАдресЭП = Контрагент.КонтактнаяИнформация.Найти(Справочники.ВидыКонтактнойИнформации.EmailКонтрагента, "Вид");

		Если НЕ СтрокаАдресЭП = Неопределено Тогда

			АдресЭП = СокрЛП(СтрокаАдресЭП.НомерТелефона);
	...
	Иначе
		...
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ТаблицаКонтакты.АдресЭП КАК АдресЭП
		|ИЗ
		|	Справочник.Контрагенты.КонтактнаяИнформация КАК ТаблицаКонтакты
		|ГДЕ
		|	ТаблицаКонтакты.Ссылка = &Контрагент
		|	И ТаблицаКонтакты.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.EmailКонтрагента)
		|	И НЕ ТаблицаКонтакты.АдресЭП = """""
		);
		Запрос.УстановитьПараметр("Контрагент", Контрагент);
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			АдресЭП = СокрЛП(Выборка.АдресЭП);
...
Функция ЯвляетсяСотрудником(Контрагент)	Экспорт
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И НЕ Контрагент = ПустаяСсылка() Тогда
...
// Есть день рождения ребенка.
// 
// Параметры:
//  Контрагент - СправочникСсылка.Контрагенты - Контрагент
// 
// Возвращаемое значение:
//  Булево - Есть день рождения ребенка
Функция ЕстьДеньРожденияРебенка(Контрагент) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДатаРожденияВЭтомГоду = Дата(ТекущийГод, МесяцРождения, ДеньРождения);
		
		// Вычисляем разницу дат на встроенном языке
		РазницаДат = (ТекущаяДатаСеанса() - ДатаРожденияВЭтомГоду) / 86400;
		
		Результат = РазницаДат >= -7 И РазницаДат <= 7;
...
// IDN_КореллируетДаннымNAV
// 
// Параметры:
//  Контрагент - СправочникСсылка.Контрагенты - Контрагент
// 
// Возвращаемое значение:
//  Булево - IDN кореллирует данным NAV
...
// Получить список по номеру телефона.
// 
// Параметры:
...
// 
// Возвращаемое значение:
//   СписокЗначений из СправочникСсылка.Номенклатура - Получить список по номеру телефона
Функция ПолучитьСписокПоНомеруТелефона(НомерТелефона, Отбор)	Экспорт

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НомерТелефона", НомерТелефона);

	Если ТипЗнч(Отбор) = Тип("Структура") И НЕ Отбор.Количество() = 0 Тогда
...
// Возвращает контрагента по номеру телефона с возможностью фильтрации по юридическому/физическому лицу, покупателю и виду контакта. 
// Поиск осуществляется среди контрагентов, не являющихся плательщиками НДС, поставщиками или производителями.
//
// Параметры:
//   НомерТелефона - Строка - номер телефона для поиска, приводится к стандартному формату
//   ЮрФизЛицо - ПеречислениеСсылка.ЮрФизЛицо, Неопределено - тип контрагента для фильтрации
//   Покупатель - Булево, Неопределено - признак покупателя для фильтрации
//   ВидКонтакта - СправочникСсылка.ВидыКонтактнойИнформации, Неопределено - вид контактной информации для фильтрации
//
// Возвращаемое значение:
//   СправочникСсылка.Контрагенты - найденный контрагент
//   Неопределено - если контрагент не найден
...
// Проверяет, указаны ли ключи поиска в данных контрагента.
//
// Параметры:
//   ДанныеКонтрагента - Структура - данные контрагента, содержащие ключи поиска
//
// Возвращаемое значение:
//   Булево - Истина, если указан хотя бы один ключ поиска
...
// Выполняет поиск контрагента в информационной базе по данным, полученным из Microsoft Dynamics CRM.
// Поиск выполняется последовательно по нескольким критериям: внутренний идентификатор (IDC),
// идентификатор CRM (IDD), номер телефона. При необходимости возвращает дисконтную карту.
//
// Параметры:
//   ДанныеКонтрагента - см. ОбменMicrosoftDynamicsСервер.ИнициализироватьСоставДанных - данные контрагента из CRM
//   ВернутьОбъекты - Булево - признак необходимости вернуть объекты вместо ссылок
//
// Возвращаемое значение:
//   Структура:
//     * Контрагент - СправочникСсылка.Контрагенты, Неопределено - найденный контрагент
//     * НомерТелефона - Строка - номер телефона контрагента
//     * IDD - Строка - идентификатор в CRM
//     * Ошибка - Булево - признак ошибки поиска
//     * ОписаниеОшибки - Строка - текстовое описание ошибки
//     * ДисконтнаяКарта - СправочникСсылка.ИнформационныеКарты, Неопределено - дисконтная карта контрагента
//     * Объекты - Структура:
//         ** Контрагент - СправочникОбъект.Контрагенты - объект контрагента (при ВернутьОбъекты = Истина)
//         ** ДисконтнаяКарта - СправочникОбъект.ИнформационныеКарты - объект дисконтной карты (при ВернутьОбъекты = Истина)
//     * ПоискВыполнен - Булево - признак выполнения поиска
...
//	1.1.	пытаемся найти по "нашему" идентификатору, если он не пустой.
		...
		ДанныеКонтрагента.УИ.IDC = ЗаменаРУСнаАнгл(ДанныеКонтрагента.УИ.IDC);
		
		Попытка

			СоставДанных.Контрагент = ПолучитьСсылку(Новый УникальныйИдентификатор(ДанныеКонтрагента.УИ.IDC));
	...
	Если СоставДанных.Контрагент.Пустая() И НЕ ПустаяСтрока(ДанныеКонтрагента.УИ.IDD) Тогда
...
//	1.3.	пытаемся найти клиента по номеру телефона
	...
	Если СоставДанных.Контрагент.Пустая() И НЕ ПустаяСтрока(ДанныеКонтрагента.НомерТелефона) Тогда
...
//	По IDN не ищем, так как не известно, как там обстоят дела с этим реквизитом в CRM... НЕ ТЕСТИРОВАНО!
	Если ЛОЖЬ И СоставДанных.Контрагент.Пустая() И НЕ ПустаяСтрока(ДанныеКонтрагента.УИ.IDN) Тогда
...
//	1.4.	пытаемся найти клиента по IDN, если он указан в сообщении.
	...
	Если СоставДанных.Контрагент.Пустая() Тогда
...
// Применяет данные из Microsoft Dynamics CRM к существующему контрагенту или создает нового.
//
// Параметры:
//   ДанныеКонтрагента - Структура - данные контрагента из CRM
//   СоставДанных - Структура - заполняется результатами работы функции:
//     * Ошибка - Булево - признак возникновения ошибки
//     * ОписаниеОшибки - Строка - описание ошибки
//     * Контрагент - СправочникСсылка.Контрагенты - созданный или обновленный контрагент
//     * ДисконтнаяКарта - СправочникСсылка.ИнформационныеКарты - присвоенная дисконтная карта (если применимо)
//     * Объекты - Структура:
//         ** Контрагент - СправочникОбъект.Контрагенты
//         ** ДисконтнаяКарта - СправочникОбъект.ИнформационныеКарты
//   ВернутьОбъекты - Булево - возвращать ли объекты в СоставДанных.Объекты
//   ВызовИсключения - Булево - вызывать исключение при ошибке записи
//
// Возвращаемое значение:
//   Структура - см. параметр СоставДанных
Функция ПрименитьКонтрагентаПоДаннымCRM(ДанныеКонтрагента, СоставДанных, ВернутьОбъекты = Ложь, ВызовИсключения = Истина)	Экспорт
	...
	Если НЕ СоставДанных.Контрагент.Пустая() Тогда
	...
	ИначеЕсли ДанныеКонтрагента.РазрешеноСозданиеНового = Истина Тогда
		...
		Если КлючиПоискаУказаны(ДанныеКонтрагента) Тогда
			...
			КонтрагентОбъект.УстановитьСсылкуНового(ПолучитьСсылку());
			...
			Попытка
				...
				Если ЭтоНовый Тогда	//	LNK 17.08.2021 13:49:27
					...
					СоставДанных.ДисконтнаяКарта = ИнформационныеКартыСлужебный.ПрисвоитьРезервнуюВиртуальнуюКарту(КонтрагентОбъект.ПолучитьСсылкуНового()
						...
						, ПолучитьНомерТелефона(КонтрагентОбъект)
...
// Откладывает данные контрагента в регистр сведений для последующей обработки
//
// Параметры:
//   КодКарты - Строка - код карты клиента
//   НомерТелефона - Строка - номер телефона клиента
//   ДанныеКонтрагента - Структура - данные клиента
//   Назначение - Строка - назначение сохраняемых данных
//   ДокументПродажи - ДокументСсылка - документ продажи
//   Магазин - СправочникСсылка.Магазины - магазин
//   Контрагент - СправочникСсылка.Контрагенты - контрагент. Неопределено, если нужно записать новые данные
...
// Очищает соответствия объектов учетных систем для контрагента при наличии пометки удаления.
//
// Параметры:
//   Контрагент - СправочникСсылка.Контрагенты - контрагент, для которого выполняется очистка соответствий
//   ПометкаУдаления - Булево
//                   - Неопределено - признак необходимости удаления соответствий
...
// Находит контрагентов в базе по данным из Magento и заполняет таблицу результатов.
//
// Параметры:
//   ТаблицаВнешняя - ТаблицаЗначений - таблица с данными из Magento. Заполняется результатами поиска.
//     Входные колонки: IDC (Строка), НомерТелефона (Строка), КодКарты (Строка)
//     Выходные колонки: Контрагент (СправочникСсылка.Контрагенты), Найден (Булево),
//       EmailТекущий (Строка), ОписаниеОшибки (Строка), ДисконтнаяКарта (СправочникСсылка.ИнформационныеКарты)
Процедура НайтиКонтрагентовПоСпискуMagento(ТаблицаВнешняя)	Экспорт
	...
	Для каждого СтрокаВнешняя Из ТаблицаВнешняя Цикл
		...
		Если НЕ ПустаяСтрока(СтрокаВнешняя.IDC) Тогда
			...
			СтрокаВнешняя.Контрагент = ПолучитьСсылку(Новый УникальныйИдентификатор(СтрокаВнешняя.IDC));
			
			Если НЕ ОбщегоНазначенияКлиентСервер.СсылкаПустая(СтрокаВнешняя.Контрагент) Тогда
				
				ГруппаКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаВнешняя.Контрагент, "Родитель");
				
				Если Не ГруппаКонтрагента = Покупатели Тогда
					
					СтрокаВнешняя.Контрагент = ПустаяСсылка();
	...
	|			И ТаблицаЗаголовки.Родитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.Покупатели)
...
// Ищет контрагента в информационной базе по данным из системы Magento.
//
// Параметры:
//   ДанныеВнешние - Структура - данные контрагента из Magento
//
// Возвращаемое значение:
//   Структура - найденные данные контрагента, колонки соответствуют результату поиска
Функция НайтиКонтрагентаПоДаннымMagento(ДанныеВнешние)	Экспорт
	...
	НайтиКонтрагентовПоСпискуMagento(ТаблицаВнешняя);

	Возврат ОбщегоНазначенияКлиентСервер.СтрокаТаблицыКакСтруктура(ТаблицаВнешняя.Колонки, ТаблицаВнешняя[0]);

КонецФункции

// Создает или обновляет контрагента по данным из интернет-магазина Magento, заполняет контактную информацию и персональные данные.
//
// Параметры:
//   СтрокаВнешняя - Структура - данные из Magento (НомерТелефона, Найден, Контрагент, ДанныеКонтрагента, EmailТекущий, ДисконтнаяКарта)
//
// Возвращаемое значение:
//   СправочникСсылка.Контрагенты - ссылка на созданного или обновленного контрагента
...
//			Блокиратор.Заблокировать();
			...
			КонтрагентОбъект.Родитель   = Покупатели;
			КонтрагентОбъект.Покупатель = Истина;
			КонтрагентОбъект.ЮрФизЛицо  = Перечисления.ЮрФизЛицо.ФизЛицо;	
...
Процедура УстановитьКонтактнуюИнформацию(КонтрагентОбъект, ДанныеКонтрагента)
	...
	КлючиСтруктуры = "Реквизит, ИмяВнешнее, ВидКонтакта, Телефон";
	
	СоставКонтактов.Добавить(Новый Структура(КлючиСтруктуры, "НомерТелефона", "НомерТелефона", "ТелефонКонтрагента", Истина));
	СоставКонтактов.Добавить(Новый Структура(КлючиСтруктуры, "НомерТелефона", "НомерТелефонаСлужебный", "ТелефонКонтрагентаСлужебный", Истина));
	СоставКонтактов.Добавить(Новый Структура(КлючиСтруктуры, "Представление", "Email", "EmailКонтрагента", Ложь));
	СоставКонтактов.Добавить(Новый Структура(КлючиСтруктуры, "Представление", "EmailСлужебный", "EmailКонтрагентаСлужебный", Ложь));
	СоставКонтактов.Добавить(Новый Структура(КлючиСтруктуры, "Представление", "АдресПочтовый", "ПочтовыйАдресКонтрагента", Ложь));

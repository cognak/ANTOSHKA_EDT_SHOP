...
Функция ЕстьДеньРожденияРебенка(Контрагент) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("БольшаяДата", ТекущаяДатаСеанса());
		Запрос.УстановитьПараметр("МеньшаяДата", ДатаРожденияВЭтомГоду);
		Запрос.Текст= "ВЫБРАТЬ РАЗНОСТЬДАТ(&МеньшаяДата, &БольшаяДата, ДЕНЬ) КАК Разница";

		Выборка = Запрос.Выполнить().Выбрать(); 
			
		РазностДат = ?(Выборка.Следующий(), ?(Выборка.Разница <> Null, Выборка.Разница, 100), 100);
		
		Результат = РазностДат >= -7 И РазностДат <= 7;

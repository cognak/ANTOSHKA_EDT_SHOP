...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.Организации") Тогда
		...
		ДанныеСсылки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, 
			"Наименование, НаименованиеПолное, ОсновнойБанковскийСчет, ЮрФизЛицо");
		
		Наименование           = ДанныеСсылки.Наименование;
		ЮрФизЛицо              = ДанныеСсылки.ЮрФизЛицо;
		НаименованиеПолное     = ДанныеСсылки.НаименованиеПолное;
		ОсновнойБанковскийСчет = ДанныеСсылки.ОсновнойБанковскийСчет;
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() Тогда
		...
		Для Каждого СтрокаМаркетинговаяГруппа Из МаркетинговыеГруппы Цикл
			
			СтрокаМаркетинговаяГруппа.ДатаДобавления = ДатаСоздания;
	...
	Иначе
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаМаркетинговыхГрупп.Группа КАК Группа
		|ПОМЕСТИТЬ ВременнаяТаблица
		|ИЗ
		|	&ТаблицаМаркетинговыхГрупп КАК ТаблицаМаркетинговыхГрупп
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА КонтрагентыМаркетинговыеГруппы.Ссылка ЕСТЬ NULL
		|			ТОГДА &ТекущаяДата
		|		ИНАЧЕ КонтрагентыМаркетинговыеГруппы.ДатаДобавления
		|	КОНЕЦ КАК ДатаДобавления
		|ИЗ
		|	ВременнаяТаблица КАК ВременнаяТаблица
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.МаркетинговыеГруппы КАК КонтрагентыМаркетинговыеГруппы
		|		ПО ВременнаяТаблица.Группа = КонтрагентыМаркетинговыеГруппы.Группа
		|		И КонтрагентыМаркетинговыеГруппы.Ссылка = &Ссылка";
		
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
		Запрос.УстановитьПараметр("ТаблицаМаркетинговыхГрупп", МаркетинговыеГруппы.Выгрузить());
		Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДатаСеанса());
		
		РезультатЗапроса = Запрос.Выполнить();

		МаркетинговыеГруппы.Загрузить(РезультатЗапроса.Выгрузить());
	КонецЕсли;
	
	//А++ 20250116 по задаче  https://awdev.atlassian.net/browse/RETAIL1C-1055
	//Очищаем пустые строки с детьми
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить("Имя", "");
	СтруктураОтбора.Вставить("Отчество", "");
	СтруктураОтбора.Вставить("Фамилия", "");
	СтруктураОтбора.Вставить("ДатаРождения", Дата(1, 1, 1));
	СтруктураОтбора.Вставить("Комментарий", "");
	СтруктураОтбора.Вставить("Пол", Перечисления.ПолФизическогоЛица.ПустаяСсылка());
	
	ПустыеСтроки = Дети.НайтиСтроки(СтруктураОтбора); 

	Для Каждого СледСтрока Из ПустыеСтроки Цикл 
		Дети.Удалить(СледСтрока); 
	КонецЦикла; 	
	
	//2026-03-30 sa Если ДР клиента не задано, то ставим по-умолчанию 01.01.1901
	Если Не ЗначениеЗаполнено(ДатаРождения) Тогда
		ДатаРождения = Дата("19010101");
	КонецЕсли;
...
//	LNK 13.11.2016 15:10:41
	...
	Если НЕ ЭтоГруппа Тогда
		...
		Если Поставщик И ПустаяСтрока(IDN) Тогда
			...
			Сообщение.Текст = "Отказано. Поставщик не может иметь пустой IDN (идентификатор Navision).";
			Сообщение.Сообщить();
...
Процедура ПриЗаписи(Отказ)
	...
	// Нужно, что бы определить, почему записывается новый элемент без виртуальной карты
	Если ДополнительныеСвойства.ЭтоНовый Тогда
		...
		Исключение
			...
			ЖурналСобытий.Регистрация("Запись нового контрагента"
...
// Проверяет, является ли переданное значение перечисления признаком юридического лица
//
// Параметры:
//  ЮрФизЛицо - ПеречислениеСсылка.ЮрФизЛицо
//
// Возвращаемое значение:
//  - Булево - Истина, если юридическое лицо или юридическое лицо-нерезидент, Ложь, если физическое лицо или индивидуальный предприниматель
//  - Неопределено - если передано другое значение перечисления
...
Процедура ВыполнитьПроверкиКорректностиИнформации()
	...
	Наименование = СокрЛП(СтрЗаменить(Наименование, "null", ""));
	...
	Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(Наименование) Тогда
	...
	Если НЕ ЭтоГруппа Тогда
		...
		Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(НаименованиеПолное) Тогда
		...
		Для каждого СтрокаДети Из Дети Цикл
			...
			Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(СтрокаДети.Фамилия) Тогда

				СтрокаДети.Фамилия = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтрокаДети.Фамилия, "");

			КонецЕсли;

			Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(СтрокаДети.Имя) Тогда

				СтрокаДети.Имя = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыXML(СтрокаДети.Имя, "");

			КонецЕсли;

			Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(СтрокаДети.Отчество) Тогда
		...
		Для каждого СтрокаКонтакты Из КонтактнаяИнформация Цикл
			...
			Для каждого Реквизит Из Метаданные.Справочники.Контрагенты.ТабличныеЧасти.КонтактнаяИнформация.Реквизиты Цикл
				...
				Если ТипЗнч(СтрокаКонтакты[Реквизит.Имя]) = Тип("Строка")
						И ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(СтрокаКонтакты[Реквизит.Имя]) Тогда
...
// Выполняет нормализацию наименования физического лица. Если наименование пустое, то очищает полное наименование.
//
// Параметры:
//   Отказ - Булево - признак отмены записи объекта
...
Процедура УстановитьИдентификаторВнешнихИсточников()
	...
	Если ПустаяСтрока(IDN) Или СтрДлина(IDN) > 10 Тогда
		...
		Если СтрДлина(НовыйIDN) > 10 Тогда
		
			НовыйIDN = СтрЗаменить(НовыйIDN, ".", "");
		...
		Пока Истина Цикл
			...
			Запрос.Текст =
				...
				|	И Контрагенты.IDN = &IDN";
			
			Запрос.УстановитьПараметр("Ссылка", Ссылка);
			Запрос.УстановитьПараметр("IDN", НовыйIDN);
			...
			Если Не Выборка.Количество() = 0 Тогда
				...
				НовыйIDN = Строка(КодПрефикса) + Прав(НовыйIDN, 7);
				КодПрефикса = КодПрефикса + 1;

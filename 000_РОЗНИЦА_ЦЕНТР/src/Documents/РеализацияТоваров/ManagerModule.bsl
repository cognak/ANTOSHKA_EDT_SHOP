...
Функция ПечатьАктаОбОказанииУслуг(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Сумма          = Сумма       + ВыборкаПоСтрокамТЧ.Сумма - ВыборкаПоСтрокамТЧ.Скидка;
...
Функция ПечатьСчет(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	|	(ВЫБРАТЬ
	|		МАКСИМУМ(ВЫБОР
	|				КОГДА РеализацияТоваровТовары.Цена * РеализацияТоваровТовары.Количество = РеализацияТоваровТовары.Сумма
	|					ТОГДА ЛОЖЬ
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ) КАК ЕстьСкидки
	|	ИЗ
	|		Документ.РеализацияТоваров.Товары КАК РеализацияТоваровТовары
	|	ГДЕ
	|		РеализацияТоваровТовары.Ссылка В(&МассивОбъектов)) КАК ВложенныйЗапрос
	|ГДЕ
	|	Документ.Ссылка В(&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	ОбластьШапкаДанных         = Макет.ПолучитьОбласть("ШапкаТаблицы|Данные");
	ОбластьШапкаСуммы          = Макет.ПолучитьОбласть("ШапкаТаблицы|Сумма");
	ОбластьШапкаСкидки         = Макет.ПолучитьОбласть("ШапкаТаблицы|Скидки");
	ОбластьКолонкаТовар = Макет.Область("Товар");
	ОбластьКолонкаТовар.ШиринаКолонки = ОбластьКолонкаТовар.ШиринаКолонки
	                                  + Макет.Область("КолонкаКодов").ШиринаКолонки;
	ОбластьСтрокаНомера         = Макет.ПолучитьОбласть("Строка|НомерСтроки");
	...
	ОбластьСтрокаДанных         = Макет.ПолучитьОбласть("Строка|Данные");
	ОбластьСтрокаСуммы          = Макет.ПолучитьОбласть("Строка|Сумма");
	ОбластьСтрокаСкидки          = Макет.ПолучитьОбласть("Строка|Скидки");
	
	ОбластьИтогоНДСНомера = Макет.ПолучитьОбласть("ИтогоНДС|НомерСтроки");
	ОбластьИтогоНДСКодов  = Макет.ПолучитьОбласть("ИтогоНДС|КолонкаКодов");
	ОбластьИтогоНДСДанных = Макет.ПолучитьОбласть("ИтогоНДС|Данные");
	ОбластьИтогоНДССуммы  = Макет.ПолучитьОбласть("ИтогоНДС|Сумма");
	ОбластьИтогоНДССкидки  = Макет.ПолучитьОбласть("ИтогоНДС|Скидки");
	// Вывести Итого.
	ОбластьИтогоНомера         = Макет.ПолучитьОбласть("Итого|НомерСтроки");
	ОбластьИтогоКодов          = Макет.ПолучитьОбласть("Итого|КолонкаКодов");
	ОбластьИтогоДанных         = Макет.ПолучитьОбласть("Итого|Данные");
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ЕстьСкидки тогда
			ТабличныйДокумент.Присоединить(ОбластьШапкаСкидки);
		КонецЕсли;	
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			ТабличныйДокумент.Присоединить(ОбластьСтрокаДанных);
			Если ЕстьСкидки тогда
				ОбластьСтрокаСкидки.Параметры.Заполнить(ВыборкаПоСтрокамТЧ);
				ТабличныйДокумент.Присоединить(ОбластьСтрокаСкидки);
			КонецЕсли;
			ОбластьСтрокаСуммы.Параметры.Заполнить(ВыборкаПоСтрокамТЧ);
			ТабличныйДокумент.Присоединить(ОбластьСтрокаСуммы);
...
&НаСервере
Функция СформироватьСертификаты(ДокументСсылка) Экспорт   
	...
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер.Владелец КАК СерийныйНомерВладелец,
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер.КодСерийногоНомера КАК СерийныйНомерКодСерийногоНомера,
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер.ДатаСоздания КАК СерийныйНомерДатаСоздания,
	|	СостоянияСерийныхНомеров.ДатаОкончанияДействия КАК ДатаОкончанияДействия,
	|	СерийныеНомераКлючи.КлючНомера КАК КлючНомера
	|ИЗ
	|	РегистрСведений.ДвиженияСерийныхНомеров.СрезПоследних(, Регистратор = &Документ) КАК ДвиженияСерийныхНомеровСрезПоследних
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияСерийныхНомеров КАК СостоянияСерийныхНомеров
	|		ПО ДвиженияСерийныхНомеровСрезПоследних.Номенклатура = СостоянияСерийныхНомеров.Номенклатура
	|			И ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер = СостоянияСерийныхНомеров.СерийныйНомер
	|			И ДвиженияСерийныхНомеровСрезПоследних.Регистратор = СостоянияСерийныхНомеров.Регистратор
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СерийныеНомераКлючи КАК СерийныеНомераКлючи
	|		ПО ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер = СерийныеНомераКлючи.СерийныйНомер
	|
	|СГРУППИРОВАТЬ ПО
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер,
	|	СостоянияСерийныхНомеров.ДатаОкончанияДействия,
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер.Владелец,
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер.КодСерийногоНомера,
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер.ДатаСоздания,
	|	СерийныеНомераКлючи.КлючНомера
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДвиженияСерийныхНомеровСрезПоследних.СерийныйНомер.КодСерийногоНомера";
	Запрос.УстановитьПараметр("Документ", ДокументСсылка); //Реализация товаров, Возврат товаров поставщику, Поступление товаров, Чек ККМ, Возврат товаров от покупателя, Списание товаров, Уценка товаров, Оприходование товаров, Отчет о розничных продажах, Перемещение товаров, Передача товаров, Ордер на перемещение товаров, Расходный ордер, Корректировка регистров, Приходный ордер
	
	Результат = Запрос.Выполнить().Выбрать();
	
	Пока Результат.Следующий() Цикл 
		
		ТабличныйДокумент =	Документы.РеализацияТоваров.ПечатьИзображенийСертификатов(Результат.СерийныйНомер, Результат.ДатаОкончанияДействия, Результат.КлючНомера);  
		...
		СтруктураСертификата.Вставить("ИмяФайла", СокрЛП(Результат.СерийныйНомер.Наименование));
		СтруктураСертификата.Вставить("АдресХранилища", АдресХранилища);
		
		МассивВозврата.Добавить(СтруктураСертификата);
...
Функция ПечатьИзображенийСертификатов(СерийныйНомер, ДатаОкончанияДействия, КлючНомера) Экспорт 
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.РеализацияТоваров.ИзображениеСертификата", КодЯзыкаПечать);
	ОбластьМакета = Макет.ПолучитьОбласть("СертификатСтрока|Сертификат");         
	
	ОбластьМакета.Параметры.СерияСертификата 	= СерийныйНомер.КодСерийногоНомера;
	
	ОбластьМакета.Параметры.НоминалСертификата 	= СтрЗаменить(СерийныйНомер.Владелец.Наименование, "Подарунковий сертифікат-", "") + " грн";
	
	СтруктураШтрихкода = ПодключаемоеОборудованиеРТ.ПолучитьШтрихкод(СерийныйНомер); 
		
	Если СтруктураШтрихкода <> Неопределено Тогда
				
		Штрихкод          = ПодарочныеСертификатыСервер.РасшифроватьКлючНомера(КлючНомера);//СтруктураШтрихкода.Штрихкод;
		ТипШтрихкода      = СтруктураШтрихкода.ТипШтрихкода; 
		
		Эталон = Макет.Области.ШтрихКод.Высота;//Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("Эталон");
		КоличествоМиллиметровВПикселе = Эталон / 100;
		ПараметрыШтрихкода = Новый Структура;
		ПараметрыШтрихкода.Вставить("Ширина",          Окр(ОбластьМакета.Области.ШтрихКод.Ширина / КоличествоМиллиметровВПикселе));
		ПараметрыШтрихкода.Вставить("Высота",          Окр(ОбластьМакета.Области.ШтрихКод.Высота / КоличествоМиллиметровВПикселе));
		...
		ОбластьМакета.Параметры.КодСертификата	= Штрихкод;	

	КонецЕсли;
	
	ОбластьМакета.Рисунки.СрокДействия.Текст = Формат(ДатаОкончанияДействия,"ДФ=dd.MM.yyyy");  
	
	ТабличныйДокумент.АвтоМасштаб = Истина;
	ТабличныйДокумент.ВысотаСтраницы = 185;
	ТабличныйДокумент.ШиринаСтраницы = 110;

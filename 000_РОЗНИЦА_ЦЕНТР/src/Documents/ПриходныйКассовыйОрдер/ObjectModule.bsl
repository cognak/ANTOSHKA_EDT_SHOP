...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//А++ 20241204 по задаче занес проверку на дату закрытия счёта
...
//	Заполним данные шапки документа.
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.Магазин.КассаМагазина.Владелец = ДанныеДокумента.Организация
	|			ТОГДА ДанныеДокумента.Магазин.КассаМагазина
	...
	ДанныеЗаполнения = ОбщегоНазначенияКлиентСервер.СтрокаТаблицыКакСтруктура(РезультатЗапроса.Колонки);

	Если РезультатЗапроса.Пустой() Тогда

		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			  "Не требуется вводить приходный кассовый ордер на основании документа %1"
			, ДокументОснование
		);

...
Процедура ЗаполнитьПоДаннымДругогоФормата(Команда)
	Если Объект.ТоварныеКатегории.Количество()>0 Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ВыбранныйФорматМагазина = ОткрытьФормуМодально("Справочник.ФорматыМагазинов.ФормаВыбора");
...
Процедура ДобавитьМарку(Команда)
	...
	Если ТекСтрока=Неопределено Тогда // Нет строк совсем
		ПоказатьПредупреждение(, "Марку можно добавить только в подчинение какой-либо категории.");
	Иначе
		ТипСтроки=0;
		...
		Если ТипСтроки>0 Тогда
			ВыбраннаяМарка = ОткрытьФормуМодально("Справочник.Марки.ФормаВыбора", , ЭтотОбъект);
			Если ВыбраннаяМарка<>Неопределено Тогда
				...
				Если ТекСтрокаКоллекция<>Неопределено Тогда
					...
					Если МаркаУжеДобавлена(СтрокаРодителя.ПолучитьИдентификатор(), ВыбраннаяМарка) Тогда
						ПоказатьПредупреждение(, "Марка уже добавлена в данной категории.");
					Иначе
...
//	LNK 04.01.2017 13:54:53
	...
	Если НЕ ЭтотОбъект.ТолькоПросмотр Тогда
		ЭтотОбъект.ТолькоПросмотр = НЕ ДоступностьПравилаЦенообразования;
	КонецЕсли;

...
Процедура ОформитьПриНеобходимостиПоПеремещению(ПеремещениеСсылка)	Экспорт
	...
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() ИЛИ НЕ ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИспользоватьПрименениеЦенПриПеремещении") = Истина Тогда
...
Процедура ВыполнитьПрименениеЦенНаОснованииПеремещения_NAV_чтобы_не_переделывать(ШапкаВыборка)

	Перем ТаблицаNavision;
	...
	|	ТаблицаТовары.Магазин.СкладПродажи.IDN КАК КодСклада,
	|	ТаблицаТовары.Номенклатура,
	|	ТаблицаТовары.Характеристика,
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ПредставлениеДаты = Формат(ТекущаяДатаСеанса(), "ДФ=yyyyMMdd");
		ТоварыВыборка     = Результат.Выбрать();
		ТаблицаДокумента  = Результат.Выгрузить().СкопироватьКолонки();
		
		Пока ТоварыВыборка.Следующий() Цикл

			ТаблицаТовара = ВнешниеИсточники.ПолучитьДанныеОтNavision("SELECT * FROM dbo.PL_GetItemPriceRec('" + СокрЛП(ТоварыВыборка.IDN) + "', '" + ПредставлениеДаты + "')",,, Истина);

			Если ТаблицаNavision = Неопределено Тогда

				ТаблицаNavision = ТаблицаТовара.СкопироватьКолонки();
				ТаблицаNavision.Колонки.Добавить("Номенклатура"  , Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
				ТаблицаNavision.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));

			КонецЕсли;

			СтрокаСклада = ТаблицаТовара.Найти(ТоварыВыборка.КодСклада, "КодСклада");

			Если НЕ СтрокаСклада = Неопределено Тогда

			//	Пытаемся получить цену от Navision... 
				СтрокаNavision = ТаблицаNavision.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаNavision, СтрокаСклада);
				СтрокаNavision.Номенклатура   = ТоварыВыборка.Номенклатура;
				СтрокаNavision.Характеристика = ТоварыВыборка.Характеристика;

			ИначеЕсли НЕ ТоварыВыборка.Цена = 0 Тогда

			//	Цены в Navision нет. А в документе есть - будем устанавливать цену, указанную в перемещении.
				СтрокаТовары = ТаблицаДокумента.Добавить();
		...
		Если НЕ ТаблицаNavision.Количество() = 0 Тогда
			...
			|	ВЫРАЗИТЬ(ТаблицаNavision.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
			|	ТаблицаNavision.Характеристика КАК Характеристика,
			|	ТаблицаNavision.КОДЕДИНИЦЫИЗМЕРЕНИЯ КАК КодУпаковки,
			|	ТаблицаNavision.ДатаНачала,
			|	ТаблицаNavision.Акция,
			|	ТаблицаNavision.ЦенаВключаетНДС,
			|	ТаблицаNavision.КодВалюты,
			...
			|	ТаблицаNavision.Характеристика,
			...
			|				И ТаблицаNavision.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
			|			ТОГДА 1.2
			|		ИНАЧЕ 1
			|	КОНЕЦ КАК Цена,
			|	ТаблицаNavision.Номенклатура.Наименование КАК КлючПорядка
			|ИЗ
			|	ТаблицаNavision КАК ТаблицаNavision
			...
			Запрос.УстановитьПараметр("ТаблицаNavision", ТаблицаNavision);

			ОбщегоНазначенияРТСервер.ЗагрузитьВТаблицуЗначений(Запрос.Выполнить().Выгрузить(), ТаблицаДокумента, Ложь);

		КонецЕсли;

		Если НЕ ТаблицаДокумента.Количество() = 0 Тогда

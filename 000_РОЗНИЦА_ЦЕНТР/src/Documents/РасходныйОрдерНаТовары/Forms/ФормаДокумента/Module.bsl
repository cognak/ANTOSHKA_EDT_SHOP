...
//	LNK 14.09.2016 12:56:35
&НаСервере
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)

//	LNK 14.09.2016 12:56:31
//	NAV:БлокировкаДанныхУчестьДокументВNavision
...
&НаКлиенте	//	LNK 18.10.2021 11:05:39
...
Процедура ВвестиПодарочныеСертификаты(Команда)
	...
	Если ТолькоПросмотр И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда

		ОписаниеОповещения = Новый ОписаниеОповещения("ВвестиПодарочныеСертификатыЗавершение", ЭтотОбъект, ДополнительныеПараметры);

		ПоказатьВопрос(ОписаниеОповещения,
			"Вы можете изменить состав серийных номеров без изменения
			|общего количества (будут изменены все документы в подчинении).
			|Вам нужно выбрать одно из действий:
			|[Да] - сертификаты для изменения
			|[Нет] - сертификаты для просмотра
			|[Отмена] - отказаться",
	...
	Иначе
		...
		ВвестиПодарочныеСертификатыЗавершение(КодВозвратаДиалога.Да, ДополнительныеПараметры);

	КонецЕсли;

КонецПроцедуры

&НаКлиенте	//	LNK 12.03.2020 13:47:35
Процедура ВвестиПодарочныеСертификатыЗавершение(КодВозврата, ДополнительныеПараметры)	Экспорт

	Перем ПересчетКоличества, ТекстСообщения;

	Если КодВозврата = КодВозвратаДиалога.Да ИЛИ КодВозврата = КодВозвратаДиалога.Нет Тогда

		ПересчетКоличества   = Ложь;
		КоличествоНеИзменять = ТолькоПросмотр И КодВозврата = КодВозвратаДиалога.Да;

		ОбработкаТабличнойЧастиТоварыКлиент.ВвестиСерийныеНомераНоменклатурыВТЧ(ЭтотОбъект,
			Объект.СерийныеНомера,
			Элементы.Товары.ТекущиеДанные, 
			ПересчетКоличества,,
			КоличествоНеИзменять
		);
		...
		Если КоличествоНеИзменять И Модифицированность Тогда

		//	Сбрасываем для того, чтобы пользователь не смог записать объект!
		//	Ведь сюда попадаем только в режиме просмотра.
			Модифицированность = Ложь;

			Если ПрименитьИзменениеСерийныхНомеровНаСервере(Элементы.Товары.ТекущиеДанные.КлючСвязиСерийныхНомеров, ТекстСообщения) Тогда

				ПоказатьОповещениеПользователя("Операция выполнена!",, ТекстСообщения, БиблиотекаКартинок.Информация32);

			Иначе

				ПоказатьОповещениеПользователя("Операция НЕ выполнена!",, ТекстСообщения, БиблиотекаКартинок.Ошибка32);
				Закрыть();
...
&НаСервере	//	LNK 13.03.2020 10:22:17
Функция ПрименитьИзменениеСерийныхНомеровНаСервере(КлючСвязиСерийныхНомеров, ТекстСообщения)

	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	Выполнено = ОбработкаОбъект.ПрименитьИзменениеСерийныхНомеров(КлючСвязиСерийныхНомеров, ТекстСообщения);
...
&НаКлиенте	//	LNK 18.10.2021 11:03:51
...
&НаСервере	//	LNK 18.10.2021 11:06:38

...
//	NAV:БлокировкаДанныхУчестьДокументВNavision
	...
	Если РольДоступна("ДобавлениеИзменениеНачислениеИСписаниеБонусныхБаллов") Тогда
		Если НЕ НачалоДня(Дата) = НачалоДня(ТекущаяДата()) ТОгда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Документ «" + СокрЛП(Ссылка) + "» запрещено проводить датой ранее текущей. Отказано.", Ссылка,,, Отказ);
			Возврат;
		КонецЕсли;
		
		Если СтрДлина(Комментарий) < 6 ТОгда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Поле ""Комментарий"" обязательно к заполнению! Отказано.", Ссылка,,, Отказ);
			Возврат;
		КонецЕсли;

...
//	LNK 21.09.2023 06:24:38
Функция СформироватьДокументПередачиБонусныхБаллов(Контрагент, СуммаБонусныхБаллов, ФондБлаготворительный)	Экспорт
...
//	---------------------------------------------------------------------------------------
	...
	ДокументОбъект.ВидОперации	= Перечисления.ВидыОперацийНачислениеИСписаниеБонусныхБаллов.ПередачаНаБлаготворительныйФонд;

	ДокументОбъект.Магазин		= ?(ПараметрыСеанса.ТекущийМагазин.Пустая(), ОбменДаннымиПовтИсп.ПолучитьДанныеУзла().Магазин, ПараметрыСеанса.ТекущийМагазин);
	ДокументОбъект.КодБюджета	= РозничныеПродажиСерверПовтИсп.РеквизитыМагазина(ДокументОбъект.Магазин).КодБюджета;
	ДокументОбъект.ПрограммаЛояльности = РозничныеПродажиСерверПовтИсп.РеквизитыФонда(ФондБлаготворительный).ПрограммаЛояльности;

	ДокументОбъект.Комментарий	= "Переказ суми бонусів на благодійний фонд";

	СтрокаТабличнойЧасти = ДокументОбъект.Списание.Добавить();
	СтрокаТабличнойЧасти.Контрагент = Контрагент;
	СтрокаТабличнойЧасти.СуммаБонусныхБаллов  = СуммаБонусныхБаллов;
	СтрокаТабличнойЧасти.КонтрагентПолучатель = РозничныеПродажиСерверПовтИсп.РеквизитыФонда(ФондБлаготворительный).Контрагент;

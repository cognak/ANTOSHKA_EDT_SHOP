...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		ГородОтправитель = Выборка.Город;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если (СтатусПосылки.СтатусСлужбыДоставки = Перечисления.СтатусыПосылокСлужбыДоставки.Новая
			...
			И Реестр.Пустая()
			И Не ПустаяСтрока(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ВнешняяСсылка"))
		...
		Запрос.Текст =
			...
			|	И РеестрЭН.ПосылкиЛокал = ЭлектроннаяНакладная.Локал
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		Если ПустаяСтрока(НомерДомаОтправитель) Тогда
...
Процедура ЗаполнитьДокумент(ДокументДляЗаполнения, ДанныеЗаполнения) Экспорт
	...
	Запрос.Текст =
		...
		|	ТаблицаЗаказа.КонтрагентПолучатель КАК КонтрагентПолучатель
		|ИЗ
		|	Документ.ЗапросДоступностиТоваров КАК ТаблицаЗапроса
	...
	Если ВыборкаЗаполнения.Следующий() Тогда
		...
		Иначе
			...
			ДокументДляЗаполнения.КонтрагентПолучатель = ВыборкаЗаполнения.КонтрагентПолучатель;
		...
		Если Выборка.Следующий() Тогда
			...
			Если Не СтороннийОтправитель Тогда
				...
				ДокументДляЗаполнения.ЭтажОтправителя = Выборка.Этаж;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьМагазинПолучатель(ДокументДляЗаполнения, МагазинПолучатель, ОператорДоставки)
	...
	Запрос.Текст =
		...
		|	КонтактныеЛицаСлужбыДоставки.КонтактноеЛицо КАК КонтактноеЛицо1,
		|	КонтактныеЛицаСлужбыДоставки.Телефон КАК Телефон,
		|	КонтактныеЛицаСлужбыДоставки.Email КАК Email,
		|	КонтактныеЛицаСлужбыДоставки.Контрагент КАК Контрагент1,
		|	КонтактныеЛицаСлужбыДоставки.Фамилия КАК Фамилия,
		|	КонтактныеЛицаСлужбыДоставки.Имя КАК Имя,
		|	КонтактныеЛицаСлужбыДоставки.Отчество КАК Отчество,
		|	КонтактныеЛицаСлужбыДоставки.Действие КАК Действие,
		|	КонтактныеЛицаСлужбыДоставки.Предопределенный КАК Предопределенный,
		|	КонтактныеЛицаСлужбыДоставки.ИмяПредопределенныхДанных КАК ИмяПредопределенныхДанных,
		|	КонтактныеЛицаСлужбыДоставки.Представление КАК Представление,
		|	АдресаСлужбыДоставки.ИДГорода КАК ИДГорода,
		|	АдресаСлужбыДоставки.ИДУлицы КАК ИДУлицы,
		|	АдресаСлужбыДоставки.Дом КАК Дом,
		|	АдресаСлужбыДоставки.Адрес КАК Адрес,
		|	НастройкиСлужбДоставки.Адрес КАК АдресПолучателя,
		|	НастройкиСлужбДоставки.Этаж КАК Этаж
		|ИЗ
		|	РегистрСведений.НастройкиСлужбДоставкиМагазинов КАК НастройкиСлужбДоставки
	...
	Если Выборка.Следующий() Тогда
		...
		ДокументДляЗаполнения.АдресПолучателя = Выборка.АдресПолучателя;
		ДокументДляЗаполнения.ЭтажПолучателя = Выборка.Этаж;

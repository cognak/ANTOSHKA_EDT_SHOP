...
Процедура ОтменитьНакладнуюНаСервере()
	...
	Если ПустаяСтрока(Объект.Номер) Тогда
		...
		РегистрыСведений.СтатусыЭН.ОтменитьПосылку(Объект.Ссылка); 
	
	Иначе 
			
		Если РегистрыСведений.СтатусыЭН.ПосылкаВДороге(Объект.Ссылка) Тогда
...
Процедура ОтображениеЭлементов()
	...
	Элементы.КонтрагентПолучатель.Видимость = Не Объект.КонтрагентПолучатель.Пустая();
	...
	Если Не ТехническаяПоддержкаВызовСервера.ИсключительныйРежим() Тогда
		...
		Элементы.ВозвратнаяНакладная.ТолькоПросмотр = Истина;
		Элементы.ПереадресацияНакладная.ТолькоПросмотр = Истина;
		
	Иначе
		
		Элементы.ВозвратнаяНакладная.ТолькоПросмотр = Ложь;
		Элементы.ПереадресацияНакладная.ТолькоПросмотр = Ложь;
	...
	ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.СтатусыЭН"));
...
Процедура ТТННаСервере()
	...
	Если Выборка.ПрошлаБезнальнаяОплата Тогда
		...
		ЖурналСобытий.Регистрация("ЭлекстроннаяНакладная.Запрос"
							, УровеньЖурналаРегистрации.Информация
							, Метаданные.Документы.ЭлектроннаяНакладная
							, Объект.Ссылка
							, Новый ХранилищеЗначения(ОтветЭН.ТекстЗапроса)
							, 
							, СокрЛП(ОтветЭН.ТекстЗапроса)
						);
		
		ЖурналСобытий.Регистрация("ЭлекстроннаяНакладная.Ответ"
							, УровеньЖурналаРегистрации.Информация
							, Метаданные.Документы.ЭлектроннаяНакладная
							, Объект.Ссылка
							, Новый ХранилищеЗначения(ОтветЭН.ТекстОтвета)
							, 
							, СокрЛП(ОтветЭН.ТекстОтвета)
						);
		...
		Если ОтветЭН.Успех Тогда
			...
			Объект.Локал = ОтветЭН.Локал;
			Записать(ПараметрыЗаписи);
			РегистрыСведений.СтатусыЭН.СозданаПосылка(Объект.Ссылка);
		...
		Иначе
			...
			ЖурналСобытий.Регистрация("ЭлекстроннаяНакладная.ОшибкаСоздания"
								, УровеньЖурналаРегистрации.Ошибка
								, Метаданные.Документы.ЭлектроннаяНакладная
								, Объект.Ссылка
								, Новый ХранилищеЗначения(ОтветЭН.ТексОшибки)
								, ОтветЭН.ТексОшибки
								, СокрЛП(ОтветЭН.ТексОшибки)
							);

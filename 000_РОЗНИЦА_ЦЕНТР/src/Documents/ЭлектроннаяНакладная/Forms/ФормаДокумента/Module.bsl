...
Процедура ОтображениеЭлементов()
	...
	Элементы.ФормаСоздатьНакладную.Видимость = ПустаяСтрока(Объект.ВнешняяСсылка);

	Элементы.ПерезаполнитьДокумент.Видимость = ПустаяСтрока(Объект.ВнешняяСсылка);

	СтатусПосылки = ПолучитьСтатусЭН();
	СтатусДокумента = СтатусПосылки.СтатусЭН;
	Элементы.ФормаПолучитьДокументы.Видимость =
		 Не (СтатусПосылки.СтатусСлужбыДоставки = ПредопределенноеЗначение("Перечисление.СтатусыПосылокСлужбыДоставки.Отменена"));

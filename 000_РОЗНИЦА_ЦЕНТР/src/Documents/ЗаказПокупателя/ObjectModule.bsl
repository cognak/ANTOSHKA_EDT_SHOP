...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Не Отказ тогда 
		ПроверитьЗаписьВРССтатусы(Ссылка,ЭтотОбъект.Ответственный,ЭтотОбъект.Статус);	
	КонецЕсли;
КонецПроцедуры
	...
	Если Статус = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности Тогда 
		Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
			...
			Если (ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз
				...
				И НЕ ЗначениеЗаполнено(Товары[ТекИндекс].МагазинПолучатель) Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения) 
	...
	Если УчетнаяСистема.Пустая()
		...
		Если СтатусИМ = Перечисления.СтатусыИнтернетМагазина.pending Тогда
...
//	Справочники.Магазины.НайтиПоРеквизиту("НомерМагазина", 35).СкладПродажи
//	Перечисления.СтатусОплаты.Оплачен
//	Перечисления.СтатусыЗаказовПокупателей.Согласован
//	Перечисления.СтатусыИнтернетМагазина.in_moving
//	Перечисления.СтатусыИнтернетМагазина.delivered
//	Перечисления.ТипДоставкиЗаказПокупателя.НоваяПочтаДоставкаВОтделение
//	Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз
...
//	В режиме "ЗагрузкаДокументаОтMagento" эта процедура вызывается из записывающего модуля.
	...
	Для Каждого СтрокаОплаты Из ОплатаЗаказа Цикл
		...
		Если СтрокаОплаты.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.ОплатаКартойПриват
				И СтрокаОплаты.СтатусОплаты = Перечисления.СтатусОплаты.Оплачен
				И СтрокаОплаты.ВидОплаты = Перечисления.ВидыОплатыЗаказаПокупателя.Оплата Тогда
		...
		ИначеЕсли СтрокаОплаты.ВидОплаты = Перечисления.ВидыОплатыЗаказаПокупателя.Возврат Тогда
			СуммаОплат = СуммаОплат - СтрокаОплаты.Сумма;
		ИначеЕсли Не СтрокаОплаты.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.Наличные
				И СтрокаОплаты.ВидОплаты = Перечисления.ВидыОплатыЗаказаПокупателя.Оплата Тогда
			СуммаОплат = СуммаОплат + СтрокаОплаты.Сумма;
		КонецЕсли;
...
//	LNK 24.10.2018 14:47:58
	...
	Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЗначениеКлючаСтруктуры(ДополнительныеСвойства, "ПередатьВЦентр", Ложь) = Истина ИЛИ УчетнаяПолитика.ИнтернетМагазин =  ОбменДаннымиПовтИсп.ПолучитьДанныеУзла().Магазин Тогда
	...
	Если ДополнительныеСвойства.Свойство("МассивКомментарийСтрок") Тогда 
		Для Каждого СтрокаКомментария Из ДополнительныеСвойства.МассивКомментарийСтрок Цикл 
			...
			РегистрыСведений.КомментарийПоСтрокам.ЗаписьКомментария(СтрокаКомментария);
			
		КонецЦикла;
	КонецЕсли;
	
	//Если Статус = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности
...
Процедура ВыполнитьДвиженияКлючевыеАтрибутыЗаказовПокупателей(Отказ)
	...
	И НЕ КлючевыеАтрибуты = Неопределено	Тогда
		...
		Для каждого КлючЗначение Из КлючевыеАтрибуты Цикл
			...
			Если КлючЗначение.Ключ = "Состояние" И КлючЗначение.Значение = НовоеСостояние.Состояние Тогда 

				АтрибутыРавны = Ложь;
				Прервать;
				
			ИначеЕсли КлючЗначение.Значение = ЭтотОбъект[КлючЗначение.Ключ] Тогда
	...
	Если ВыполнитьДвижение Тогда
		...
		НаборЗаписей.Отбор.Период.Установить(?(ДанныеПолучения = Неопределено, ТекущаяДата(), ДанныеПолучения.ДатаПередачи));
...
Процедура ЗаписьКомментарияПоДокументу()
	...
	Если ЭтоНовый() Тогда // Объект еще не был записан   
		...
		РегистрыСведений.КомментарийИнтернетЗаказа.ЗаписьКомментария(Ссылка,
																	...
																	"Создание нового заказа в КЦ");

		
	Иначе 
		ТекстДляЗаписи = "Изменение шапки документа:" + Символы.ПС;
		ШапкаИзменена = Ложь;
		Для Каждого Реквизит Из ЭтотОбъект.Ссылка.Метаданные().Реквизиты Цикл
		...
		Для Каждого Реквизит Из ЭтотОбъект.Ссылка.Метаданные().СтандартныеРеквизиты Цикл
			ТекстИзменения = ТекстИзменениеРеквизита(ЭтотОбъект, Реквизит.Имя); 
			Если Не ПустаяСтрока(ТекстИзменения) Тогда 
				ШапкаИзменена = Истина;
				ТекстДляЗаписи = ТекстДляЗаписи + ТекстИзменения + "----------------------" + Символы.ПС; 
		...
		ТекстДляЗаписи = ТекстДляЗаписи + "Изменение шапки документа:" + Символы.ПС;
...
Функция ТекстИзменениеРеквизитаТЧ(Данные, ИмяРеквизита, ИмяТЧ)
	...
	Для Счетчик = 0 По МаксИндекс - 1 Цикл
		...
		Если ТекФлагИзменения Тогда
			Результат = "Имя реквизита: " + ИмяТЧ + ":" + ИмяРеквизита + " (" + НомерСтроки + ")" + Символы.ПС;
...
Процедура ИзменениеОтветственного()
	...
	Если ДополнительныеСвойства.Свойство("ИзмененОтветственный") И
		...
		ЗаказыПокупателейСервер.СотрудникИзменениеОтветственного(
			ЗаказыПокупателейСервер.ЯвляетсяСотрудникомКоллЦентра(ЭтотОбъект.Ссылка.Ответственный).СотрудникКоллЦентра,
			ЗаказыПокупателейСервер.ЯвляетсяСотрудникомКоллЦентра(ЭтотОбъект.Ответственный).СотрудникКоллЦентра,
			Дата);
	...
	Если ЭтоНовый() Тогда 
		...
		ЗаказыПокупателейСервер.СотрудникНовыйЗаказ(ЗаказыПокупателейСервер.ЯвляетсяСотрудникомКоллЦентра(ЭтотОбъект.Ответственный).СотрудникКоллЦентра);
...
Процедура ЗаполнитьОбеспечениеЗаказа()
	...
	НовоеСостояние = Документы.ЗаказПокупателя.ПолучитьСостояниеЗаказаПокупателя(ЭтотОбъект.Ссылка);

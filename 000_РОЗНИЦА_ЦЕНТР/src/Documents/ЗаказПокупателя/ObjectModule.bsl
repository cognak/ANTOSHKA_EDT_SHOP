...
Процедура КонтрольОплаты()
	...
	Если Статус = Перечисления.СтатусыЗаказовПокупателей.Отменён Тогда 
		...
		Для Каждого СтрокаОплаты Из ОплатаЗаказа Цикл 
			...
			Если Не ПустаяСтрока(СтрокаОплаты.ИДОплаты) И СтрокаОплаты.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.ОплатаКартойПриват Тогда 
	...
	Если УчетнаяСистема = Перечисления.УчетныеСистемыКомпании.Розница Тогда
		...
		КонецЕсли;
		// +мав проверяем на переплату   
		СтруктураОплат = ЗаказыПокупателейКлиентСервер.СтруктураПодсчетаОплат();
		СтруктураОплат.СуммаВсего = СуммаДокумента;
		ПодсчетОплаты = ЗаказыПокупателейКлиентСервер.ПодсчетОплаты(ЭтотОбъект, СтруктураОплат);
        СтатусОплаты = ПодсчетОплаты.СтатусОплаты;
		// -мав

...
Процедура ОтразитьДвиженияСерийныхНомеровКупонов(Отказ)
	...
	|	ДвиженияСерийныхНомеровКупонов.Магазин КАК Магазин,
	|	Купоны.Количество
	|ИЗ
	...
	ВыборкаУстановлена = Ложь;
	
	Пока КупоныВыборка.Следующий() Цикл

		Если Не ВыборкаУстановлена Тогда
			Если КупоныВыборка.ЧекККМ = Неопределено Тогда
				СсылкаНаЧек = Документы.ЧекККМ.ПолучитьСсылку();
			Иначе
				СсылкаНаЧек = КупоныВыборка.ЧекККМ;
			КонецЕсли;

			НаборЗаписей.Отбор.ЧекККМ.Установить(СсылкаНаЧек);
			ВыборкаУстановлена = Истина;;

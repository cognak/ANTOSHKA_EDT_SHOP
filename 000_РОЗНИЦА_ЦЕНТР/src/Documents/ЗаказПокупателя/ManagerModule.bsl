...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства) Экспорт
	...
	|	ТаблицаТовары.СуммаАвтоматическойСкидки + ТаблицаТовары.СуммаРучнойСкидки + ТаблицаТовары.СуммаБонусныхБалловСписано КАК СуммаСкидки
	|ИЗ
...
Функция ПолучитьЗапросПроведенияИЗаполнитьРеквизитыШапки(ДокументСсылка, Реквизиты = Неопределено, ДополнительныеСвойства = Неопределено)
	...
	|				ТОГДА ТаблицаДокумента.СтатусИМ
	|			ИНАЧЕ ТаблицаКлючей.СтатусИМ
	|		КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.СтатусыИнтернетМагазина.delivered) КАК НужныДвиженияБонусов,
...
Функция ПолучитьСостояниеЗаказаПокупателя(ДокументСтатуса) Экспорт 
	...
	ИначеЕсли ДокументСтатуса.Статус = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности Тогда 
		...
		Запрос.Текст = 
			...
			|		КОНЕЦ) КАК ВМагазине,
			|	СУММА(ВЫБОР
			|			КОГДА ЗапросДоступностиТоваров.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.Согласован)
			|				ТОГДА 1
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК Согласовано,
			|	СУММА(ВЫБОР
			|			КОГДА ЗапросДоступностиТоваров.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.ЧастичноСогласован)
			|				ТОГДА 1
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК Частично,
			|	СУММА(ВЫБОР
			...
			|						ИЛИ ЗапросДоступностиТоваровТовары.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваров.Отменён))) КАК ВложенныйЗапрос
	...
	ИначеЕсли ДокументСтатуса.Статус = Перечисления.СтатусыЗаказовПокупателей.Продажа Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	КОЛИЧЕСТВО(ЧекККМТовары.Ссылка) КАК КоличествоЧек
			|ПОМЕСТИТЬ СтрокВЧеке
			|ИЗ
			|	Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Товары КАК ЧекККМТовары
			|		ПО ЗаказПокупателяТовары.Ссылка = ЧекККМТовары.Ссылка.ЗаказПокупателя
			|			И (ЧекККМТовары.Ссылка.Проведен)
			|			И (ЗаказПокупателяТовары.Ссылка.Проведен)
			|			И ЗаказПокупателяТовары.КлючСвязи = ЧекККМТовары.КлючСвязи
			|ГДЕ
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	КОЛИЧЕСТВО(ЗаказПокупателяТовары.Ссылка) КАК КоличествоЗаказ
			|ПОМЕСТИТЬ СтрокВЗаказе
			|ИЗ
			|	Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
			|ГДЕ
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	СтрокВЗаказе.КоличествоЗаказ = СтрокВЧеке.КоличествоЧек КАК ВсеПродано
			|ИЗ
			|	СтрокВЧеке КАК СтрокВЧеке,
			|	СтрокВЗаказе КАК СтрокВЗаказе";

	...
	ИначеЕсли ДокументСтатуса.Статус = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности Тогда 
		...
		Иначе
			...
			Запрос.Текст = 
				...
				|	СУММА(ВЫБОР
				|		КОГДА
				|			ТаблицаЗапросов.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.ЧастичноСогласован)
				|			ТОГДА 1
				|		ИНАЧЕ 0
				|	КОНЕЦ) КАК Частично,
				|	СУММА(ВЫБОР
				...
				|			ИЛИ ТаблицаТоваров.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваров.Отменён))) КАК
				|			ТаблицаАктивных

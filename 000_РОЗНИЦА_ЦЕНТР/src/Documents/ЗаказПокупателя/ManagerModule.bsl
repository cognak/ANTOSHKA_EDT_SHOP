...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства) Экспорт
	...
	ТаблицыДляДвижений.Вставить("ТаблицаСостоянияСерийныхНомеров"			, Результат[12].Выгрузить());
КонецПроцедуры
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "СчетФактура", 
			"Счет-Фактура", 
			ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,Истина);
		
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РазностьДляСклада") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "РазностьДляСклада", 
			"Разность для склада", 
			ПечатьРазностьДляСклада(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,Истина);
...
//	То есть, эти значения получаем по самому первому (раннему) событию.
	...
	Иначе
		...
		Запрос.УстановитьПараметр("ПериодБонусов", ПолучитьДатуИзмененияСтатусаИМ(ДокументСсылка));
...
Функция ТекстЗапросаТаблицаСостоянияСерийныхНомеров()
	...
	"ВЫБРАТЬ
	|	&Период КАК Период,
	|	ПогашениеСертификатов.ПодарочныйСертификат КАК Номенклатура,
	|	ПогашениеСертификатов.СерийныйНомер КАК СерийныйНомер,
	|	&Магазин КАК Магазин,
	|	ЗНАЧЕНИЕ(Перечисление.СостоянияСерийныхНомеров.Блокирован) КАК Состояние,
	|	-2 КАК КлючПорядка
	|ПОМЕСТИТЬ ТаблицаДвижений
	|ИЗ
	|	Документ.ЗаказПокупателя.ПогашениеПодарочныхСертификатов КАК ПогашениеСертификатов
...
Функция РеквизитыДанныхВыбора(Ссылка)	Экспорт
	...
	РеквизитыВыбора.Вставить("Ссылка", Документы.ЗаказПокупателя.ПустаяСсылка()); 
	РеквизитыВыбора.Вставить("Номер", ""); 
	РеквизитыВыбора.Вставить("Дата", "00010101"); 
	РеквизитыВыбора.Вставить("СкидкиРассчитаны", Ложь); 
	РеквизитыВыбора.Вставить("Статус", Перечисления.СтатусыЗаказовПокупателей.ПустаяСсылка()); 
	РеквизитыВыбора.Вставить("СтатусИМ", Перечисления.СтатусыИнтернетМагазина.ПустаяСсылка()); 
	РеквизитыВыбора.Вставить("СтатусОплаты", Перечисления.СтатусОплаты.ПустаяСсылка()); 
	РеквизитыВыбора.Вставить("ТипДоставки", Перечисления.ТипДоставкиЗаказПокупателя.ПустаяСсылка()); 
	РеквизитыВыбора.Вставить("ТипОплаты", Перечисления.ТипОплатыЗаказПокупателя.ПустаяСсылка()); 
	РеквизитыВыбора.Вставить("Отгружен", Ложь); 
	РеквизитыВыбора.Вставить("ОтгруженПредставление", ""); 
	РеквизитыВыбора.Вставить("КоличествоЧеков", 0); 
	РеквизитыВыбора.Вставить("КоличествоПеремещений", 0); 
	РеквизитыВыбора.Вставить("УчетнаяСистема", Перечисления.УчетныеСистемыКомпании.ПустаяСсылка());
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		|	ВЫБОР
		|		КОГДА ТаблицаЗаказы.УчетнаяСистема = ЗНАЧЕНИЕ(Перечисление.УчетныеСистемыКомпании.Розница)
		|			ТОГДА ТаблицаЗаказы.СкидкиРассчитаны
		|		ИНАЧЕ ВЫБОР
		...
		|	ТаблицаЗаказы.УчетнаяСистема КАК УчетнаяСистема
		|ПОМЕСТИТЬ Фильтр
		|ИЗ
		...
		|	ТаблицаОбщая.УчетнаяСистема КАК УчетнаяСистема
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|			И ТаблицаПодчиненные.СтатусЧекаККМ В (ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Архивный),
		...
		|		Ведущая.ТипДоставки,
		|		Ведущая.ТипОплаты,
		|		Ведущая.УчетнаяСистема) КАК ТаблицаОбщая
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаОбщая.Ссылка,
...
Функция ПолучитьСостояниеЗаказаПокупателя(ДокументСтатуса) Экспорт 
	...
	ИначеЕсли ДокументСтатуса.Статус = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности Тогда 
		...
		Иначе
			...
			Запрос.Текст = 
				"ВЫБРАТЬ РАЗРЕШЕННЫЕ
				|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаЗапросов.Ссылка) КАК ВсегоЗапросов,
				|	СУММА(ВЫБОР
				|			КОГДА ТаблицаЗапросов.МагазинОтправитель = ТаблицаЗапросов.МагазинПолучатель
				|				ТОГДА 1
				|			ИНАЧЕ 0
				|		КОНЕЦ) КАК ВМагазине,
				|	СУММА(ВЫБОР
				|			КОГДА ТаблицаЗапросов.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.Новый)
				|					ИЛИ ТаблицаЗапросов.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.Отменён)
				|					ИЛИ ТаблицаЗапросов.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.Изменён)
				|					ИЛИ ТаблицаЗапросов.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.ВРаботе)
				|				ТОГДА 0
				|			ИНАЧЕ 1
				|		КОНЕЦ) КАК Согласовано,
				|	СУММА(ВЫБОР
				|			КОГДА ТаблицаЗапросов.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.ЧастичноСогласован)
				|				ТОГДА 1
				|			ИНАЧЕ 0
				|		КОНЕЦ) КАК Частично,
				|	СУММА(ВЫБОР
				|			КОГДА ТаблицаЗапросов.ТипДоставки = ЗНАЧЕНИЕ(Перечисление.ТипДоставкиЗаказПокупателя.Самовывоз)
				|				ТОГДА 1
				|			ИНАЧЕ 0
				|		КОНЕЦ) КАК ЕстьСамовывоз
				|ИЗ
				|	Документ.ЗапросДоступностиТоваров КАК ТаблицаЗапросов
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ РАЗЛИЧНЫЕ
				|			ТаблицаТоваров.Ссылка КАК Ссылка
				|		ИЗ
				|			Документ.ЗапросДоступностиТоваров.Товары КАК ТаблицаТоваров
				|		ГДЕ
				|			ТаблицаТоваров.Ссылка.ДокументОснование = &ДокументОснование
				|			И НЕ(ТаблицаТоваров.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваров.Удалён)
				|						ИЛИ ТаблицаТоваров.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваров.Отменён))) КАК ТаблицаАктивных
				|		ПО ТаблицаЗапросов.Ссылка = ТаблицаАктивных.Ссылка
				|ГДЕ
				|	ТаблицаЗапросов.ДокументОснование = &ДокументОснование";
				...
				Если Выборка.ВсегоЗапросов = (Выборка.Частично + Выборка.Согласовано) Тогда
					...
					Иначе
						...
						Иначе
							...
							Если Выборка.ВсегоЗапросов = Выборка.ЕстьСамовывоз Тогда 
	...
	ИначеЕсли ДокументСтатуса.Статус = Перечисления.СтатусыЗаказовПокупателей.Перемещение Тогда
		...
		Запрос.Текст = 
			...
			|	ТаблицаРегистра.Получен КАК Получен,
			|	ТаблицаРегистра.Отправлен КАК Отправлен
			|ИЗ
			|	РегистрСведений.СостояниеСтрокЗаказаПокупателя КАК ТаблицаРегистра
			|ГДЕ
			|	ТаблицаРегистра.ЗаказПокупателя = &ЗаказПокупателя";
		
		Запрос.УстановитьПараметр("ЗаказПокупателя", ДокументСтатуса);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Выборка = РезультатЗапроса.Выбрать();
		
		Если Не Выборка.НайтиСледующий(Ложь, "Отправлен") Тогда
		...
		Если Не Выборка.НайтиСледующий(Ложь, "Получен") Тогда
			
			Результат.Состояние = Перечисления.СостоянияЗаказовПокупателей.ГотовКПродаже;

		КонецЕсли;

	ИначеЕсли ДокументСтатуса.Статус = Перечисления.СтатусыЗаказовПокупателей.Продажа Тогда
		...
		Запрос.Текст = 
			...
			|	ЕСТЬNULL(СостояниеСтрокЗаказаПокупателя.Выдано, ЛОЖЬ) КАК Выдано
			|ИЗ
		...
		Если Выборка.Количество() = 1 Тогда
			...
			Если Выборка.Продано И Выборка.Выдано Тогда

				Если Выборка.СогласованоВNavision Тогда
				
					Результат.Состояние = Перечисления.СостоянияЗаказовПокупателей.ГотовКЗакрытию;
					
				Иначе
					
					Результат.Состояние = Перечисления.СостоянияЗаказовПокупателей.СогласованиеNavision;
	...
	ИначеЕсли ДокументСтатуса.Статус = Перечисления.СтатусыЗаказовПокупателей.Доставка Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ЗаказПокупателяТовары.Ссылка,
			|	ЕСТЬNULL(СостояниеСтрокЗаказаПокупателя.Доставлено, ЛОЖЬ) КАК Доставлено,
			|	ЕСТЬNULL(СостояниеСтрокЗаказаПокупателя.СогласованоВNavision, ЛОЖЬ) КАК СогласованоВNavision,
			|	ЕСТЬNULL(СостояниеСтрокЗаказаПокупателя.Продано, ЛОЖЬ) КАК Продано,
			|	ЗаказПокупателяТовары.Ссылка.ТипДоставки = ЗНАЧЕНИЕ(Перечисление.ТипДоставкиЗаказПокупателя.Самовывоз) КАК Самовывоз
			|ИЗ
			|	Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеСтрокЗаказаПокупателя КАК СостояниеСтрокЗаказаПокупателя
			|		ПО ЗаказПокупателяТовары.Ссылка = СостояниеСтрокЗаказаПокупателя.ЗаказПокупателя
		...
		Если Выборка.Количество() = 1 Тогда
			...
			Если Выборка.Доставлено Тогда
				
				Результат.Состояние = Перечисления.СостоянияЗаказовПокупателей.ОжидаетсяЧекКЦ;
...
Процедура РаспределениеЗаказов(Ответственный = Неопределено, МассивДокументов = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА &ТолькоНовые
		|			ТОГДА ЗаказПокупателя.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПокупателей.Новый)
		|			ИЛИ СотрудникиКоллЦентра.ЗаказыПоУмолчанию
		|		ИНАЧЕ
		...
		|	ИЛИ ЗаказПокупателя.УчетнаяСистема = ЗНАЧЕНИЕ(Перечисление.УчетныеСистемыКомпании.Розница))
	...
	Пока Выборка.Следующий() Цикл 
		...
		ОтветственныйЗаЗаказ = ЗаказыПокупателейСервер.ОтветственныйЗаЗаказ(ДокументЗаказа, Ложь, ПериодРаспределения, Ответственный);
		Если ОтветственныйЗаЗаказ.Сотрудник.Пустая() Тогда 
			Продолжить;
		КонецЕсли;
		ДокументЗаказа.Ответственный = ОтветственныйЗаЗаказ.Пользователь;
...
// Заполненость всех строк.
// 
// Параметры:
//  ОбъектСтрок - ДокументОбъект.ЗаказПокупателя
//  Автовыполнение - Булево - Автовыполнение
// 
// Возвращаемое значение:
//  Структура - Заполненость всех строк:
// * ВсеОтменены - Булево - Истина, если все строки отменены
// * СтрокиЗаполнены - Булево - Истина, если все склады указаны
// * ПараметрыЗаписи - Структура
// * ТекстВопроса - Строка - текст вопроса для клиента
Функция ЗаполненостьВсехСтрок(ОбъектСтрок, Автовыполнение = Ложь) Экспорт
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ВсеОтменены", Ложь);
	ДополнительныеПараметры.Вставить("СтрокиЗаполнены", Ложь);
	ДополнительныеПараметры.Вставить("ПараметрыЗаписи", Новый Структура);
	ДополнительныеПараметры.Вставить("ТекстВопроса", "");

	СтрокОтменено = 0;
	СтрокЗаполнено = 0;
	
	Для Каждого СтрокаТЧ Из ОбъектСтрок.Товары Цикл 
		
		Если СтрокаТЧ.Отменено Тогда 
			
			СтрокОтменено = СтрокОтменено + 1;
			
		ИначеЕсли Не СтрокаТЧ.Склад.Пустая() Тогда 
			Если НЕ ОбъектСтрок.ТипДоставки =  Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз 
	...
	Если ОбъектСтрок.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.БРПредоплата
			Или ОбъектСтрок.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.БРПостоплата Тогда
		
		ПереводитьСтатус = (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектСтрок.Контрагент, "ЮрФизЛицо")
					= Перечисления.ЮрФизЛицо.ЮрЛицо);
		
	Иначе
		
		ПереводитьСтатус = Истина;
		
	КонецЕсли;

	Если ОбъектСтрок.Товары.Количество() = СтрокОтменено
			И Не ОбъектСтрок.Статус = Перечисления.СтатусыЗаказовПокупателей.Отменён Тогда 
		
		
		ДополнительныеПараметры.ВсеОтменены = Истина;
		ДополнительныеПараметры.ТекстВопроса = "В документе отменены все строки." + Символы.ПС + "Изменить статус на «Отменено»?";
		
	ИначеЕсли ОбъектСтрок.Товары.Количество() = (СтрокЗаполнено + СтрокОтменено)
			И (ОбъектСтрок.Статус = Перечисления.СтатусыЗаказовПокупателей.ВРаботе
				Или Автовыполнение)
			И ПереводитьСтатус
			И (ОбъектСтрок.Проведен
				Или Автовыполнение) Тогда 

		ДополнительныеПараметры.СтрокиЗаполнены = Истина;
		ДополнительныеПараметры.ТекстВопроса = "В документе заполнены все строки." + Символы.ПС + "Создать «Запрос доступности»?";
...
Функция ПечатьТоварнаяНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Запрос.Текст = ТекстЗапросДляПечати();
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.ЗаказПокупателя.ТоварнаяНакладная", КодЯзыкаПечать);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Иначе
			 Если ВыборкаПоДокументам.ДоставкаНаАдрес Тогда
			 ...
			 Иначе
			 	ОбластьШапка.Параметры.АдресДоставки = Строка(ВыборкаПоДокументам.Город)
													+ ", " + Строка(ВыборкаПоДокументам.Отделение);
			КонецЕсли;			 	
		КонецЕсли;
		ОбластьШапка.Параметры.НазваниеТоварнойНакладной = 
				НСтр("ru='Товарная накладная №" + СокрЛП(ВыборкаПоДокументам.Номер) + " от " + Формат(ВыборкаПоДокументам.Дата,"Л=ru; ДЛФ=DD") 
					+ "';uk='Товарна накладна №" + СокрЛП(ВыборкаПоДокументам.Номер) + " від " + Формат(ВыборкаПоДокументам.Дата,"Л=uk_UA; ДЛФ=DD") 
					+ "'", КодЯзыкаПечать);										
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			ОбластьСтрокаТаблицы.Параметры.ЦенаСоСкидкой = ВыборкаПоСтрокамТЧ.Цена * (100 - ВыборкаПоСтрокамТЧ.ПроцентРучнойСкидки) / 100;
...
Функция ПечатьРазностьДляСклада(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
		
	КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;

	ТабличныйДокумент  = Новый ТабличныйДокумент;
	СинонимДокумента   = НСтр("ru='Разность для склада';uk='Різниця для складу'", КодЯзыкаПечать);
	
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РазностьДляСклада_РазностьДляСклада"; 
	
	Запрос = Новый Запрос;
	
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	ТаблицаЗП.Ссылка КАК ЗаказПокупателя,
	|	ТаблицаЗДТ.Ссылка КАК ЗапросДоступности,
	|	ТаблицаЗДТТовары.Номенклатура КАК Номенклатура,
	|	ТаблицаЗДТТовары.КоличествоТребование КАК КоличествоТребование,
	|	ТаблицаЗДТТовары.КлючСвязи КАК КлючСвязи,
	|	ТаблицаЗДТТовары.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваров.Отменён)
	|	ИЛИ ТаблицаЗДТТовары.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваров.Удалён) КАК
	|		СтрокаУдалена
	|ПОМЕСТИТЬ ТоварыВЗаказе
	|ИЗ
	|	Документ.ЗаказПокупателя КАК ТаблицаЗП
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросДоступностиТоваров КАК ТаблицаЗДТ
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|			ПО ТаблицаЗДТ.МагазинОтправитель = Склады.Ссылка
	|			И Склады.НазначениеСклада = ЗНАЧЕНИЕ(Перечисление.НазначенияСкладов.УправляющаяСистема)
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗапросДоступностиТоваров.Товары КАК ТаблицаЗДТТовары
	|			ПО ТаблицаЗДТ.Ссылка = ТаблицаЗДТТовары.Ссылка
	|		ПО ТаблицаЗП.Ссылка = ТаблицаЗДТ.ДокументОснование
	|ГДЕ
	|	ТаблицаЗП.Ссылка В (&МассивОбъектов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	ТоварыВЗаказе.ЗапросДоступности.Ссылка КАК ЗапросДоступности,
	|	ТоварыВЗаказе.ЗапросДоступности.Номер КАК Номер
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТоварыВЗаказе.ЗаказПокупателя КАК ЗаказПокупателя,
	|	ТоварыВЗаказе.ЗапросДоступности КАК ЗапросДоступности,
	|	ТоварыВЗаказе.Номенклатура КАК Номенклатура,
	|	ТоварыВЗаказе.КоличествоТребование КАК Количество,
	|	ТоварыВЗаказе.КлючСвязи КАК КлючСвязи,
	|	ТоварыВЗаказе.СтрокаУдалена КАК СтрокаУдалена,
	|	ЕСТЬNULL(СостояниеСтрокЗаказаПокупателя.СтароеКоличество, 0) КАК СтароеКоличество,
	|	ТоварыВЗаказе.Номенклатура.IDN КАК Код,
	|	ТоварыВЗаказе.Номенклатура.Артикул КАК Артикул,
	|	ТоварыВЗаказе.Номенклатура.Представление КАК НоменклатураПредставление
	|ИЗ
	|	ТоварыВЗаказе КАК ТоварыВЗаказе
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеСтрокЗаказаПокупателя КАК СостояниеСтрокЗаказаПокупателя
	|		ПО ТоварыВЗаказе.ЗаказПокупателя = СостояниеСтрокЗаказаПокупателя.ЗаказПокупателя
	|		И ТоварыВЗаказе.ЗапросДоступности = СостояниеСтрокЗаказаПокупателя.ЗапросДоступностиТоваров
	|		И ТоварыВЗаказе.КлючСвязи = СостояниеСтрокЗаказаПокупателя.КлючСвязи
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЗапросДоступности
	|ИТОГИ
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.ЗаказПокупателя.РазностьДляСклада", КодЯзыкаПечать);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если НЕ ВыборкаПоТабличнымЧастям.НайтиСледующий(Новый Структура("ЗапросДоступности",ВыборкаПоДокументам.ЗапросДоступности)) Тогда
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Если ВыборкаПоСтрокамТЧ.Количество = ВыборкаПоСтрокамТЧ.СтароеКоличество Тогда
				Продолжить;
			КонецЕсли;

			ОбластьСтрокаТаблицы.Параметры.Заполнить(ВыборкаПоСтрокамТЧ);
			
			Если ВыборкаПоСтрокамТЧ.СтрокаУдалена Тогда
				
				СостояниеТовара = "Удалить позицию";
				ОбластьСтрокаТаблицы.Параметры.Количество = 0;
				
			ИначеЕсли ВыборкаПоСтрокамТЧ.СтароеКоличество = 0 Тогда 
				
				СостояниеТовара = "Добавить позицию";
				
			Иначе
					
				СостояниеТовара = "Изменить позицию";
				
			КонецЕсли;
			
			ОбластьСтрокаТаблицы.Параметры.СостояниеТовара = СостояниеТовара;
			ТабличныйДокумент.Вывести(ОбластьСтрокаТаблицы);

		КонецЦикла;

		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НомерСтрокиНачало, ОбъектыПечати, ВыборкаПоДокументам.ЗапросДоступности);
...
Функция ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
		
	КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;

	ТабличныйДокумент  = Новый ТабличныйДокумент;

	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СчетФактура_СчетФактура"; 
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.ЗаказПокупателя.СчетФактура", КодЯзыкаПечать);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		//ШАПКА
		ОбластьШапка.Параметры.Заполнить(ВыборкаПоДокументам);
		//СЮП 07.02.2024 15.59.23 
		//https://awdev.atlassian.net/browse/RETAIL1C-637
		Если ВыборкаПоДокументам.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз Тогда 
			ОбластьШапка.Параметры.АдресДоставки = ВыборкаПоДокументам.АдресМагазина;
		Иначе
			 Если ВыборкаПоДокументам.ДоставкаНаАдрес Тогда
		...
		ОбластьШапка.Параметры.НазваниеТоварнойНакладной = 
				НСтр("ru='Счет-фактура №" + СокрЛП(ВыборкаПоДокументам.Номер) + "(" + СокрЛП(ВыборкаПоДокументам.НомерЗапроса) + ") от " + Формат(ВыборкаПоДокументам.Дата,"Л=ru; ДЛФ=DD") 
					+ "';uk='Рахунок-фактура №" + СокрЛП(ВыборкаПоДокументам.Номер) + " від " + Формат(ВыборкаПоДокументам.Дата,"Л=uk_UA; ДЛФ=DD") 
					+ "'", КодЯзыкаПечать);										
		ТаблицаСистемыНалогообложения = 
			РегистрыСведений.СистемыНалогообложенияОрганизаций.СрезПоследних(ВыборкаПоДокументам.Дата,
									Новый Структура("Организация", ВыборкаПоДокументам.Организация));
		...
		//ПОДПИСИ
		ОбластьПодписей.Параметры.КтоВыписал = ПараметрыСеанса.ТекущийПользователь;
		ТабличныйДокумент.Вывести(ОбластьПодписей);
...
Функция ПечатьСообщениеДляМагазинов(МассивОбъектов, ОбъектыПечати, ПараметрыВывода) Экспорт
		
	КодЯзыкаПечать = ПараметрыВывода.КодЯзыкаДляМногоязычныхПечатныхФорм;

	ТабличныйДокумент  = Новый ТабличныйДокумент;
	СинонимДокумента   = НСтр("ru='Подтвердить наличие';uk='Підтвердити наявність'", КодЯзыкаПечать);
	
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СообщениеДляМагазинов_СообщениеДляМагазинов"; 
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаказПокупателя.Ссылка КАК Ссылка,
	|	ЗаказПокупателя.Номер КАК НомерЗапроса,
	|	ЗаказПокупателя.Дата КАК ДатаЗапроса,
	...
	|	Документ.ЗаказПокупателя КАК ЗаказПокупателя
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
	|		ПО Штрихкоды.Владелец = ЗаказПокупателя.Ссылка
	|ГДЕ
	...
	|			ТОГДА ЗаказПокупателяТовары.Номенклатура.Представление
	...
	|	ЗаказПокупателяТовары.Номенклатура.Марка КАК НоменклатураМарка,
	...
	|		ПО ЗаказПокупателяТовары.Номенклатура = ТабШтрихкодыСрез.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатурнаяИнформация КАК НоменклатурнаяИнформация
	|		ПО ЗаказПокупателяТовары.Номенклатура = НоменклатурнаяИнформация.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КомментарийПоСтрокам КАК КомментарийПоСтрокам
	|		ПО ЗаказПокупателяТовары.Ссылка = КомментарийПоСтрокам.Документ
	|		И ЗаказПокупателяТовары.КлючСвязиЗапросаДоступности = КомментарийПоСтрокам.КлючСвязи
	|ГДЕ
	|	ЗаказПокупателяТовары.Ссылка В (&МассивОбъектов)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.ЗаказПокупателя.СообщениеДляМагазинов", КодЯзыкаПечать);
	...
	ОбластьШапка.Параметры.АвторПисьма = ПараметрыВывода.АвторПисьма;
	ОбластьШапка.Параметры.ВремяОтправления = Формат(ТекущаяДатаСеанса(),"Л=uk_UA; ДФ=HH:mm;");
	ТабличныйДокумент.Вывести(ОбластьШапка);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			ОбластьСтрокаТаблицы.Параметры.КомментарийПоСтроке = ВыборкаПоСтрокамТЧ.Комментарий;

			ОбластьСтрокаТаблицы.Параметры.Заполнить(ВыборкаПоСтрокамТЧ);

			ОбластьСтрокаТаблицы.Параметры.АдресКартинки = "Фото" + СокрЛП(ВыборкаПоСтрокамТЧ.Номенклатура.IDN);
			ТабличныйДокумент.Вывести(ОбластьСтрокаТаблицы);
...
Функция ТекстЗапросДляПечати()
	
	ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	ЗаказПокупателяТовары.Ссылка КАК Ссылка,
		|	ЗаказПокупателяТовары.МагазинПолучатель КАК МагазинПолучатель,
		|	МагазиныАдрес.Представление КАК Представление
		|ПОМЕСТИТЬ ПолучательМагазин
		|ИЗ
		|	Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Магазины.КонтактнаяИнформация КАК МагазиныАдрес
		|		ПО ЗаказПокупателяТовары.МагазинПолучатель = МагазиныАдрес.Ссылка
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ТаблицаЗаказа.Ссылка КАК Ссылка,
		|	ТаблицаЗаказа.Номер КАК НомерЗапроса,
		|	ТаблицаЗаказа.Дата КАК ДатаЗапроса,
		|	ТаблицаЗаказа.ТипДоставки КАК ТипДоставки,
		...
		|	ПолучательМагазин.Представление КАК АдресМагазина,
		...
		|	ТаблицаЗаказа.СуммаДоставки КАК Доставка,
		...
		|		ПО КонтрагентыТелефон.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.Телефон)
		|		И ТаблицаЗаказа.Контрагент = КонтрагентыТелефон.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.КонтактнаяИнформация КАК КонтрагентыПочта
		|		ПО КонтрагентыПочта.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты)
		|		И ТаблицаЗаказа.Контрагент = КонтрагентыПочта.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПолучательМагазин КАК ПолучательМагазин
		|		ПО ТаблицаЗаказа.Ссылка = ПолучательМагазин.Ссылка
		...
		|ГДЕ
		|	ТаблицаЗаказа.Ссылка В (&МассивОбъектов)

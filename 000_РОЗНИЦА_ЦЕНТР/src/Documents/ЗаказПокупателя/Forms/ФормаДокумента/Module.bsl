...
Процедура СтатусОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	...
	Если ЕстьПродажи(Объект.Ссылка) Тогда
		СтандартнаяОбработка = Ложь;
...
Функция ЕстьПродажи(ДокументПроверки)
	Возврат РегистрыСведений.СостояниеСтрокЗаказаПокупателя.ЕстьПродажи(ДокументПроверки);
КонецФункции

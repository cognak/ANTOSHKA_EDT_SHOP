...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ИначеЕсли Объект.Товары.Количество() = (СтрокЗаполнено + СтрокОтменено)
...
//	СтруктураОтбора.Вставить("СкидочныйКупон",МассивНомеров.Владелец); 
...
//		ТекущаяСтрока.СкидочныйКупон = МассивНомеров.Владелец;  
//		ТекущаяСтрока.СерийныйНомер = МассивНомеров.Ссылка; 
//		ТекущаяСтрока.НоминалКупона = МассивНомеров.Владелец.Номинал;
//		
//		ТекущаяСтрока = Объект.СерийныеНомера.Добавить();
//		ТекущаяСтрока.СерийныйНомер = МассивНомеров.Ссылка; 
//		ТекущаяСтрока.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.СкидочныйКупон;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	Цена = 0;
	Магазин = ОбщегоНазначенияРТСервер.ОпределитьТекущийМагазин();
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	...
	Иначе  
		...
		ТекстДляЗаписи = "Получена цена " + Строка(Цена) +  " грн. от Competera на товар " + IDN;
		ЗаписатьКомментарий(ТекстДляЗаписи);
	КонецЕсли;	
	СтруктураПараметровСтавкиНДС = Новый Структура;
	СтруктураПараметровСтавкиНДС.Вставить("Дата"       , Объект.Дата);
	СтруктураПараметровСтавкиНДС.Вставить("Организация", Объект.Организация);
...
&НаСервере
Процедура  ЗаписатьКомментарий(ТекстДляЗаписи)   
	Значение = РеквизитФормыВЗначение("Объект");
	Ссылка = УправлениеДоступомСлужебный.СсылкаОбъекта(Значение);
	РегистрыСведений.КомментарийИнтернетЗаказа.ЗаписьКомментария(Ссылка, ПараметрыСеанса.ТекущийПользователь, ТекстДляЗаписи);
КонецПроцедуры	
...
Процедура ОплатаЗаказаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ИнформацияОбОплате.АктивныеСтроки <= 1 Тогда
		...
		Элементы.ОплатаЗаказаТипОплаты.РежимВыбораИзСписка = Ложь;
		Элементы.ОплатаЗаказаТипОплаты.СписокВыбора.Очистить();
		
	Иначе
		
		Элементы.ОплатаЗаказаТипОплаты.РежимВыбораИзСписка = Истина;
		Элементы.ОплатаЗаказаТипОплаты.СписокВыбора.Очистить();
...
Процедура ПоискШКПриИзменении(Элемент)
	БуфферПоляВвода = СокрЛП(ПоискШК);	 
...
Процедура УстановкаТипаДоставки()
	Если НЕ Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.Новый") Тогда
...
Функция ГородАвтоПодборНаСервере(Текст)
    ...
    "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	    |	Отделения.Город КАК Город
	    |ПОМЕСТИТЬ ОтделенияГорода
	    |ИЗ
	    ...
	    |////////////////////////////////////////////////////////////////////////////////
	    |ВЫБРАТЬ
	    |	Почтоматы.Город КАК Город
	    |ПОМЕСТИТЬ ПочтоматыГорода
	    |ИЗ
	    ...
	    |	ГородаДоставки.НаименованиеRU
	    |ИЗ
	    |	Справочник.ГородаДоставки КАК ГородаДоставки
	    |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтделенияГорода КАК ОтделенияГорода
	    |		ПО (ГородаДоставки.Ссылка = ОтделенияГорода.Город)
	    |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПочтоматыГорода КАК ПачтоматыГорода
	    |		ПО (ГородаДоставки.Ссылка = ПачтоматыГорода.Город)
	    |ГДЕ
    ...
    Пока Выборка.Следующий() Цикл
    	...
    	ПредставлениеКонтрагента = Выборка.Наименование+" ("+Выборка.НаименованиеRU+")";
        спВыбора.Добавить(Выборка.Ссылка, ПредставлениеКонтрагента);    

	...
	Если ВнешниеИсточникиПовтИсп.ИспользоватьЦеныCompetera(Магазин,ТекущаяДата()) тогда 
		Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) тогда 
			Если ЗначениеЗаполнено(IDN) тогда 
				Цена = ПолучитьЦенуGraphQL(IDN, "reprice");
			КонецЕсли;
		КонецЕсли;	
	КонецЕсли;
...
Процедура ВозвратБезналичнойОплаты(Команда)
	...
	Иначе
		...
		Иначе
			...
			Элементы.ОплатаЗаказа.ТекущиеДанные.ИДОплаты = ОбменСБанкамиВызовСервера.ВозвратБезналичнойОплатыИнтернетЗаказа(СуммаВозврата, ИДОплаты, ДанныеОтвета);
...
&НаКлиенте
Процедура ПересчитатьСуммуДоставки()

	СтоимостьДоставки = ЗаказПокупателяВызовСервера.СтоимостьДоставки(Объект.Дата
		, Объект.ОператорДоставки
		, Объект.ДоставкаНаАдрес);
	СуммаДоставкиСлужбойДоставки = СтоимостьДоставки.СуммаДоставкиСлужбойДоставки;
	СуммаБесплатнойДоставки = СтоимостьДоставки.СуммаБесплатнойДоставки;
	
	Если Объект.УчетнаяСистема = ПредопределенноеЗначение("Перечисление.УчетныеСистемыКомпании.Розница")
			И Не Объект.СуммаДоставкиВРучную
			И Не Объект.СуммаДоставкиОплачена Тогда
		Если Объект.ТипДоставки = ПредопределенноеЗначение("Перечисление.ТипДоставкиЗаказПокупателя.Самовывоз") Тогда
			Если Не Объект.СуммаДоставки = 0 Тогда
				Объект.СуммаДоставки = 0;
				Модифицированность = Истина;
			КонецЕсли;
		Иначе
			Если СуммаВсего >= СуммаБесплатнойДоставки Тогда
				Если Не Объект.СуммаДоставки = 0 Тогда
					Объект.СуммаДоставки = 0;
					Модифицированность = Истина;
				КонецЕсли;
			Иначе
				Если Не Объект.СуммаДоставки = СуммаДоставкиСлужбойДоставки Тогда
					Объект.СуммаДоставки = СуммаДоставкиСлужбойДоставки;
					Модифицированность = Истина;
				КонецЕсли;
			КонецЕсли;

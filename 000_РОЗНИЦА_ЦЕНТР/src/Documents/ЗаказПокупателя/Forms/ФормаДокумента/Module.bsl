...
Процедура ОбновитьИтоговыеПоказатели(Форма, ЕстьСкидкаНаДОставку = Ложь)
	...
	СтоимостьДоставки = ЗаказПокупателяВызовСервера.СтоимостьДоставки(Форма.Объект.Дата
		, Форма.Объект.ОператорДоставки
		, Форма.Объект.ДоставкаНаАдрес);
	...
	Если Форма.Объект.УчетнаяСистема = ПредопределенноеЗначение("Перечисление.УчетныеСистемыКомпании.Розница")
		...
		Иначе
			Если Форма.СуммаВсего >= СуммаБесплатнойДоставки Тогда
				...
				КонецЕсли;
			Иначе
				Если Не Форма.Объект.СуммаДоставки = СуммаДоставкиСлужбойДоставки Тогда

...
Функция ПроверитьОплатуНаСервере(НомерОплаты)
	...
	Данные.Вставить("data", ДанныеДляОтправки.Данные);
	Данные.Вставить("signature", ДанныеДляОтправки.Сигнатура);

	ОтветСервера = КоннекторHTTP.Post("https://liqpay.ua/api/request", Данные);
	Результат = КоннекторHTTP.КакJson(ОтветСервера);
	
	РегистрыСведений.КомментарийИнтернетЗаказа.ЗаписьКомментария(Объект.Ссылка, ПараметрыСеанса.ТекущийПользователь, Результат);
	
	Если (ОтветСервера.КодСостояния = 200 Или ОтветСервера.КодСостояния = 201) Тогда
		
		Если Результат.Получить("status") = "success" Тогда 
			
			СтруктураОтвета.ОплатаПроизведена = Истина;
			СтруктураОтвета.Сумма = Результат.Получить("amount");
		...
		Иначе
			...
			СтруктураОтвета.ТекстОшибки = "Нет оплаты. Статус: " + Результат.Получить("err_description");

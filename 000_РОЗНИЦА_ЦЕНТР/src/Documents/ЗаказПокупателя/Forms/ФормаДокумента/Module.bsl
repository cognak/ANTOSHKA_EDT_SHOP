...
Процедура ОбновитьИтоговыеПоказатели(Форма)
	...
	Форма.СуммаОплачено = 0; 
	Форма.СуммаОжидаетОплаты = 0; 
	Форма.СуммаСформированныхОплат = 0;
	Форма.СуммаСформированныхВозвратов = 0;
	...
	Для Каждого СтрокаОплаты Из Форма.Объект.ОплатаЗаказа Цикл 
		...
		Если СтрокаОплаты.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыОплатыЗаказаПокупателя.Оплата") Тогда
			...
			Форма.СуммаСформированныхОплат = Форма.СуммаСформированныхОплат + СтрокаОплаты.Сумма;
			
			Если СтрокаОплаты.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.Оплачен") Тогда 
				Форма.СуммаОплачено = Форма.СуммаОплачено + СтрокаОплаты.Сумма;
			КонецЕсли;

			Если СтрокаОплаты.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.ОтправленаСсылка") Тогда
				Форма.СуммаОжидаетОплаты = Форма.СуммаОжидаетОплаты + СтрокаОплаты.Сумма;
			КонецЕсли;
			
		Иначе
			
			Форма.СуммаСформированныхВозвратов = Форма.СуммаСформированныхВозвратов + СтрокаОплаты.Сумма;
	...
	Если Форма.СуммаВсегоДляОплаты = Форма.СуммаВсегоОплаты Тогда 
		Если Не Форма.СуммаОжидаетОплаты = 0 Или Не Форма.СуммаСформированныхОплат = Форма.СуммаОплачено Тогда 
			Форма.Элементы.СостояниеОплатыЗаказа.Заголовок = "Удалите не нужные оплаты или сделайте возврат!!!"; 
		Иначе 
			Форма.Элементы.СостояниеОплатыЗаказа.Заголовок = "Заказ оплачен!!!"; 
		КонецЕсли;
	ИначеЕсли Форма.СуммаВсегоДляОплаты > Форма.СуммаВсегоОплаты Тогда
		Форма.Элементы.СостояниеОплатыЗаказа.Заголовок = "Недостаточно оплат!!!";
	Иначе
		Форма.Элементы.СостояниеОплатыЗаказа.Заголовок = "Переплата заказа!!!";
	КонецЕсли;

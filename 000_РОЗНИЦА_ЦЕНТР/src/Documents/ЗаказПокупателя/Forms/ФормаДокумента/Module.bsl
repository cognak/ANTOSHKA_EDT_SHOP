...
Процедура ВозвратБезналичнойОплаты(Команда)
	...
	Иначе
		...
		Иначе
			...
			ДанныеОтвета = "";
			Элементы.ОплатаЗаказа.ТекущиеДанные.ИДОплаты = ВозвратБезналичнойОплатыНаСервере(Элементы.ОплатаЗаказа.ТекущиеДанные.Сумма, ДанныеОтвета);
...
&НаСервере
Функция ВозвратБезналичнойОплатыНаСервере(СуммаКредитаВозврат, ДанныеОтвета)
	...
	Если ОбменСБанкамиМонобанкСервер.ПроверитьНазначениеСоединения(ДанныеСоединения, КредитнаяПрограмма, ДанныеОтвета.ТекстСообщения, ДанныеОтвета.Отказ) Тогда
		...
		ДанныеЗапросаВБанк.order_id = Объект.ОплатаЗаказах[0].ИДОплаты;

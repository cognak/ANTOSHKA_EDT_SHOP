...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.ЗапросДоступности")
			И Не Объект.СуммаБонусныхБалловСписано = Объект.Товары.Итог("СуммаБонусныхБалловСписано") Тогда
	...
	УстановитьКлючСвязиДляЗапросаДоступности();
	
КонецПроцедуры 

&НаСервере
Процедура УстановитьКлючСвязиДляЗапросаДоступности()

	ЭтоНовый = Объект.Ссылка.Пустая();

	ЗаказыПокупателейСервер.ЗаполнитьКлючСвязиЗапросаДоступности(Объект.Товары, ЭтоНовый);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если Объект.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.БезналичныйРасчет") Тогда
		
		ПереводитьСтатус = ЮрЛицо;
...
Процедура ТоварыОтмененоПриИзменении(Элемент)
	...
	Если ((Объект.ТипДоставки = ПредопределенноеЗначение("Перечисление.ТипДоставкиЗаказПокупателя.Смешаный")
		...
		ТекущаяСтрока.Отменено = Не ТекущаяСтрока.Отменено;
...
Процедура УстановитьДоступностьЭлементовНаСервере(ТолькоПросмотрЭлементов)
   ...
   Если НЕ ТехническаяПоддержкаВызовСервера.ИсключительныйРежим() Тогда
   		...
   		РедактированиеКонтрагента = Объект.Ссылка.Пустая()
   								 Или (Объект.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.БезналичныйРасчет
		...
		Для Каждого СтрокаСклада Из СтрокаТовара.stock Цикл
			...
			Если ВыборДляМассива Тогда 
				...
				Иначе
					...
					Если Не МассивВыбора.Найти(СтрокаСклада.wh_code) = Неопределено 
							И СвободныйОстаток > 0 Тогда
...
Процедура ВидимостьЭлементов()
	...
	Элементы.ДекорацияЮрЛицо.Видимость = Не ЮрЛицо 
			И (Объект.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.БезналичныйРасчет"));
...
Процедура ОплатаЗаказаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ИнформацияОбОплате.АктивныеСтроки <= 1 Тогда
		...
		Элементы.ОплатаЗаказаТипОплаты.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.БезналичныйРасчет"));
...
Функция ГородАвтоПодборНаСервере(Текст)
    ...
    Пока Выборка.Следующий() Цикл
    	...
    	ПредставлениеКонтрагента = Выборка.Наименование+" (Область - "+Выборка.Область+")";
...
Функция ОтделениеАвтоПодборНаСервере(Текст,город)
    ...
    Пока Выборка.Следующий() Цикл
    	...
    	ПредставлениеОтделения = Выборка.Наименование;//+" ("+Выборка.НаименованиеRU+")";
        спВыбора.Добавить(Выборка.Ссылка, ПредставлениеОтделения);    
...
Процедура УлицаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
    СтандартнаяОбработка = Ложь;
    ДанныеВыбора = УлицаАвтоПодборНаСервере(Объект.Город, Текст);
КонецПроцедуры

&НаСервереБезКонтекста
Функция УлицаАвтоПодборНаСервере(ГородПоиска, Текст)
    ...
    Запрос.УстановитьПараметр("Текст", "%"+Текст+"%");
    Запрос.УстановитьПараметр("Ссылка", ГородПоиска);
    
    Выборка = Запрос.Выполнить().Выбрать();
    Пока Выборка.Следующий() Цикл
    	
    	ПредставлениеУлицы = Выборка.Наименование+" (Область - "+Выборка.Область+")";
        спВыбора.Добавить(Выборка.Ссылка, ПредставлениеУлицы);    

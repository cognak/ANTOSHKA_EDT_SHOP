...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка) 
	...
	Если Элемент.ТекущийЭлемент = Элементы.ТоварыНоменклатураКод Тогда 
		Если ПодключитьРасширениеРаботыСФайлами() Тогда 
			...
			ЗапуститьПриложение("https://antoshka.ua/uk/catalogsearch/result/?q=" + СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Элемент.ТекущиеДанные.Номенклатура, "Код")));  
...
Процедура ВвестиПодарочныеСертификатыПогашения(Команда)
	...
	ИначеЕсли Объект.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.Наличные")
			И Объект.ТипДоставки = ПредопределенноеЗначение("Перечисление.ТипДоставкиЗаказПокупателя.Самовывоз") Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'В заказе с типом оплаты - наличные и типом доставки - самовывоз вводить сертификат нельзя!!!';
				|uk = 'У замовленні з типом оплати - готівкою та типом доставки - самовивіз вводити сертифікат не можна!'"));
	
	Иначе
		Оповещение = Новый ОписаниеОповещения("ВвестиПодарочныеСертификатыПогашенияЗавершение", ЭтотОбъект);
		
		ОткрытьФорму("Документ.ЗаказПокупателя.Форма.ФормаВводаСертификата"
			,
			,
			,
			,
...
&НаКлиенте
Процедура ВвестиПодарочныеСертификатыПогашенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		
		СтрокаСертификата = Объект.ПогашениеПодарочныхСертификатов.Добавить();
		СтрокаСертификата.Номинал = Результат.Номинал;
		СтрокаСертификата.ПодарочныйСертификат = Результат.Владелец;
		СтрокаСертификата.СерийныйНомер = Результат.СерийныйНомер;
		ОбновитьИтоговыеПоказатели(ЭтотОбъект, ЕстьСкидкаНаДОставку());
		Модифицированность = Истина;
...
&НаКлиенте
Процедура ВозвратБезналичнойОплаты(Команда)

	ТекстСообщения = "";
	Если Модифицированность или Не ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		ТекстСообщения = "Сначало запишите документ";
	Иначе
		СуммаВозврата = Элементы.ОплатаЗаказа.ТекущиеДанные.Сумма;
		
		Если СуммаВозврата = 0 Тогда 
			ТекстСообщения = "Не указанна сумма";
		Иначе
			
			ДанныеОтвета = "";
			ИДОплаты = Объект.ОплатаЗаказа[0].ИДОплаты;
			Элементы.ОплатаЗаказа.ТекущиеДанные.ИДОплаты = ВозвратБезналичнойОплатыНаСервере(СуммаВозврата, ИДОплаты, ДанныеОтвета);
			Элементы.ОплатаЗаказа.ТекущиеДанные.СтруктураОтвета = ДанныеОтвета.ТекстОтвета;
...
&НаСервере
Процедура ЗаполнитьТелефонКлиента(ИзменятьТелефонПолучателя = Ложь)
	...
	Если ЗначениеЗаполнено(Объект.Контрагент)
			И НЕ Объект.ПолучательТретееЛицо
			И ИзменятьТелефонПолучателя тогда
...
Процедура КонтрагентПриИзмененииНаСервере()
	...
	Объект.ВладелецДисконтнойКарты = Объект.Контрагент;
	ДанныеКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Контрагент
		, "ЮрФизЛицо
			|, Блокирован");
	ЮрЛицо = Не (ДанныеКонтрагента.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо);
	КонтрагентЗаблокирован = ДанныеКонтрагента.Блокирован;
	Объект.КонтрагентПолучатель = ?(ЮрЛицо, Объект.Контрагент, Справочники.Контрагенты.ПустаяСсылка());
	
	ЗаполнитьТелефонКлиента(Истина);
	УстановитьДоступностьЭлементовПоСтатусуСервер();
...
//	КонецЕсли;
	...
	ЭтоСотрудник = Справочники.Контрагенты.ЯвляетсяСотрудником(Объект.Контрагент);
	ИспользоватьПрограммуЛояльности = ЭтоСотрудник;
	Элементы.СотрудникАнтошки.Видимость = ЭтоСотрудник;
...
Процедура ОбновитьИтоговыеПоказатели(Форма, ЕстьСкидкаНаДОставку = Ложь)
	...
	КоличествоТоваров = 0;
	Для Каждого СтрокаТовара Из Форма.Объект.Товары.НайтиСтроки(СтруктураОтбора) Цикл
		
		КоличествоТоваров = КоличествоТоваров + СтрокаТовара.Количество
		
	КонецЦикла;

	СуммаПодарочныхСертификатов = Форма.Объект.ПогашениеПодарочныхСертификатов.Итог("Номинал");

	Если СтруктураОплат.СуммаВсего > СуммаПодарочныхСертификатов Тогда
		СтруктураОплат.СуммаВсегоДляОплаты = СтруктураОплат.СуммаВсего + Форма.Объект.СуммаДоставки - СуммаПодарочныхСертификатов; 
	Иначе
		СтруктураОплат.СуммаВсегоДляОплаты = КоличествоТоваров + Форма.Объект.СуммаДоставки;
	КонецЕсли;

	Форма.СуммаВсегоДляОплаты = СтруктураОплат.СуммаВсегоДляОплаты; 
...
Процедура ТоварыСкладНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка) 
	...
	Если  ЗначениеЗаполнено(Элементы.Товары.ТекущиеДанные.Номенклатура) Тогда 
		...
		СтруктураТовара.Вставить("Товар", Элементы.Товары.ТекущиеДанные.Номенклатура);
		СтруктураТовара.Вставить("Количество", Элементы.Товары.ТекущиеДанные.Количество);
		СтруктураТовара.Вставить("Самовывоз", Элементы.Товары.ТекущиеДанные.Самовывоз);
		СтруктураТовара.Вставить("Доставка", Не Элементы.Товары.ТекущиеДанные.Самовывоз);
...
Процедура ВидимостьЭлементов()
	...
	Элементы.КонтрагентПолучатель.Видимость = ЮрЛицо;
...
Процедура ОбновитьПредставлениеНоменклатуры()
	...
	Соответствие = ЗаказыПокупателейСервер.ПолучитьСоответствиеПредставленийТовара(Массив);
...
&НаСервереБезКонтекста
Функция ВозвратБезналичнойОплатыНаСервере(СуммаКредитаВозврат, ИДОплаты, ДанныеОтвета)

	КредитнаяПрограмма = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику().КредитнаяПрограммаИнтернетМагазина;
	...
	ИДВозврата = СтрЗаменить(СокрЛП(Новый УникальныйИдентификатор()), "-", "");

	Если ОбменСБанкамиМонобанкСервер.ПроверитьНазначениеСоединения(ДанныеСоединения, КредитнаяПрограмма, ДанныеОтвета.ТекстСообщения, ДанныеОтвета.Отказ) Тогда

		ДанныеЗапросаВБанк = ОбменСБанкамиМонобанкСлужебный.СтруктураДанныхДляЗапросаВБанк();
		ДанныеЗапросаВБанк.order_id = ИДОплаты;
		ДанныеЗапросаВБанк.return_money_to_card = Истина;
		ДанныеЗапросаВБанк.store_return_id = ИДВозврата;
		ДанныеЗапросаВБанк.sum = СуммаКредитаВозврат;
		ДанныеЗапросаВБанк.additional_params.nds = 0;
	...
	Если  Не Объект.Товары.Количество() = 0 Тогда 
		...
		Для Каждого СтрокаТЧ Из Объект.Товары Цикл
			...
			Если Не СтрокаТЧ.Отменено Тогда
				...
				СтруктураТовара = Новый Структура;
				СтруктураТовара.Вставить("Товар", СтрокаТЧ.Номенклатура);
				СтруктураТовара.Вставить("Количество", СтрокаТЧ.Количество);
				СтруктураТовара.Вставить("Самовывоз",
					Объект.ТипДоставки = ПредопределенноеЗначение("Перечисление.ТипДоставкиЗаказПокупателя.Самовывоз"));
				СтруктураТовара.Вставить("Доставка",
					Не Объект.ТипДоставки = ПредопределенноеЗначение("Перечисление.ТипДоставкиЗаказПокупателя.Самовывоз"));
...
Процедура ВидимостьКнопкиОплаты() 
	...
	Если Не Элементы.ОплатаЗаказа.ТекущиеДанные = Неопределено Тогда
		...
		ТипОплатыСтроки = Элементы.ОплатаЗаказа.ТекущиеДанные.ТипОплаты;
		СтатусОплатыСтроки = Элементы.ОплатаЗаказа.ТекущиеДанные.СтатусОплаты;
		
		Если Элементы.ОплатаЗаказа.ТекущиеДанные.ВидОплаты = ПредопределенноеЗначение("Перечисление.ВидыОплатыЗаказаПокупателя.Оплата") Тогда
		
			ОплатаПриват = (ТипОплатыСтроки = ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.ОплатаКартойПриват"));
			ОплатаНаличные = (ТипОплатыСтроки = ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.Наличные"));
			НеОплачено = (СтатусОплатыСтроки = ПредопределенноеЗначение("Перечисление.СтатусОплаты.НеОплачен"));
			СсылкаОтправлена = (СтатусОплатыСтроки = ПредопределенноеЗначение("Перечисление.СтатусОплаты.ОтправленаСсылка"));
			Отменён = Элементы.ОплатаЗаказа.ТекущиеДанные.Отменён;
		
		Иначе
			
			ВозвратОплата = (ТипОплатыСтроки = ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.monobank"));
			
		КонецЕсли;
	
	КонецЕсли;
	
	Элементы.Оплатить.Видимость = 								ОплатаПриват И НеОплачено И Не СсылкаОтправлена И Не Отменён;
	Элементы.ПроверитьОплату.Видимость =						ОплатаПриват;
	Элементы.ОплатаЗаказаРедактироватьСуммуОплаты.Видимость = 	ОплатаПриват И Не НеОплачено И СсылкаОтправлена И Не Отменён;
	Элементы.ОплатаЗаказаВозвратБезналичнойОплаты.Видимость = 	ВозвратОплата И Не НеОплачено; 
	
	Элементы.ОплатаЗаказаСтатусОплаты.ТолькоПросмотр = ОплатаПриват Или ОплатаНаличные;
	Элементы.ОплатаЗаказаВозвратБезналичнойОплаты.Доступность = ЯвляетсяСуперВайзером;
...
Процедура ОплатаЗаказаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Иначе
		...
		Иначе
			...
			Если (СуммаОплаченныхСтрок = СуммаВсегоДляОплаты И Не Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.Отменён"))
					Или (СуммаОплаченныхСтрок = 0 И Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.Отменён")) Тогда
...
Процедура ОплатаЗаказаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПокупателей.Отменён")  Тогда
			СуммаСтрокиДляОплаты = - СуммаВсегоДляОплаты;
		ИначеЕсли СуммаОплаченныхСтрок > СуммаВсегоДляОплаты Тогда 

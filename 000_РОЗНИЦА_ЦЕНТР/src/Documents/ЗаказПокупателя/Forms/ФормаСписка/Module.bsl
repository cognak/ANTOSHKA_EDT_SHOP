...
Процедура УстановитьОтборДинамическихСписков()
	...
	Для Каждого ДинамическийСписок Из ПолучитьМассивДинамическихСписковНаСервере() Цикл
		...
		// +мав
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(ДинамическийСписок, "СтатусОплаты"   	 , СтатусыОплаты, ЗначениеЗаполнено(СтатусыОплаты), ВидСравненияКомпоновкиДанных.Равно);		
		// -мав
	КонецЦикла; 
...
Процедура ПовторноПередатьВNAVНаСервере()
	...
	Если Не ТекущаяСтрока = Неопределено Тогда
		...
		Попытка
			...
			ДокументЗапроса.ДополнительныеСвойства.Вставить("ПовторнаяОтправкаВNavision");

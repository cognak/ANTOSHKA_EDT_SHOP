...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВнутреннийНомерСотрудника = СотрудникКоллЦентра.ВнутреннийНомер;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Входящий_Звонок" Тогда	//	Белоконь  
		...
		ИначеЕсли ВидУведомления = ПредопределенноеЗначение("Перечисление.ВидыУведомленийВходящегоЗвонка.ОткрыватьОкноЗвонка") Тогда 
			ОткрытьФорму("ОбщаяФорма.ФормаВходящегоЗвонкаКлиента", Параметр, ЭтаФорма); 
		...
		//ПоказатьЗначение(,Параметр.Объект);

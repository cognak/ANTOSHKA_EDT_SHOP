...
Процедура УстановитьОтборДинамическихСписков()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаСтатусов.Количество) КАК Количество,
		|	ТаблицаСтатусов.Статус,
		|	СУММА(ТаблицаСтатусов.ЕстьСообщение) КАК ЕстьСообщение
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЗаказПокупателя.Ссылка КАК Количество,
		|		ЗаказПокупателя.Статус КАК Статус,
		|		ЕСТЬNULL(ВложенныйЗапрос.ЕстьСообщение, 0) КАК ЕстьСообщение
		|	ИЗ
		|		Документ.ЗаказПокупателя КАК ЗаказПокупателя
		|			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|				КОЛИЧЕСТВО(ВложенныйЗапрос.ЕстьСообщение) КАК ЕстьСообщение,
		|				ВложенныйЗапрос.Ссылка
		|			ИЗ
		|				(ВЫБРАТЬ
		|					КомментарийПоСтрокам.Документ КАК ЕстьСообщение,
		...
		|					КомментарийПоСтрокам.Документ.ДокументОснование
		|				ИЗ
		|					РегистрСведений.КомментарийПоСтрокам КАК КомментарийПоСтрокам
		|				ГДЕ
		|					ТИПЗНАЧЕНИЯ(КомментарийПоСтрокам.Документ) = ТИП(Документ.ЗапросДоступностиТоваров)
		|					И НЕ КомментарийПоСтрокам.Просмотрен) КАК ВложенныйЗапрос
		|			СГРУППИРОВАТЬ ПО
		...
		|		И ЗаказПокупателя.Дата >= &НачальнаяДата
		|		ИЛИ ЗаказПокупателя.УчетнаяСистема = ЗНАЧЕНИЕ(Перечисление.УчетныеСистемыКомпании.Розница))) КАК ТаблицаСтатусов
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаСтатусов.Статус";
...
Процедура СделатьЗаписьВрегистрПриходУход(ТипВремени)
	...
	ОрганизацияРаботы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МагазинРаботы, "Организация");

...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЧекККМ") Тогда
		...
		Если ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Продажа Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДанныеЗаполнения.НачислениеБонусныхБаллов	, НачислениеБонусныхБаллов);
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДанныеЗаполнения.ОплатаБонуснымиБаллами	, ОплатаБонуснымиБаллами);
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Если Шапка.Следующий() Тогда
			...
			Пока ТоварыВыборка.Следующий() Цикл
				...
				СтрокаТовары.Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УчетнаяПолитика.ИнтернетМагазин, "СкладПродажи");
			...
			ТаблицаСертификатов = РезультатыЗапроса[8].Выгрузить();
			ПогашениеПодарочныхСертификатов.Загрузить(ТаблицаСертификатов);
			
			СуммаСертификатов = 0;
			Для Каждого СтрокаСертификата Из ТаблицаСертификатов Цикл
				
				СуммаСертификатов = СуммаСертификатов + СтрокаСертификата.Номинал;
				
				НоваяОплата = Оплата.Добавить();
				НоваяОплата.ВидОплаты = Справочники.ВидыОплатЧекаККМ.ОплатаПодарочнымСертификатом;
				НоваяОплата.НомерПлатежнойКарты = СтрокаСертификата.СерийныйНомер;
				НоваяОплата.Сумма = СтрокаСертификата.Номинал;
			...
			НоваяОплата.Сумма = Шапка.СуммаДокумента + Шапка.СуммаДоставки - СуммаСертификатов
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//	Обработки.ЗакрытиеКассовойСмены.Создать()
		...
		Если ДополнительныеСвойства.УчетнаяПолитика.ИспользоватьЧекиКакРегистраторыНачисленияБонусныхБаллов = Истина Тогда
			...
			//КоллекцияДвижений.Добавить(Новый Структура("Индекс, НаборЗаписей, Номер", 6, Движения.РасшифровкаНачисленияБонусовПоЧекам, "НомерСтроки"));
	...
	//	Бакан 19.04.2026
		ПроведениеСервер.ОтразитьДвиженияПоРегистру("РасшифровкаНачисленияБонусовПоЧекам", ДополнительныеСвойства, Движения,Отказ);
...
Процедура ПриЗаписи(Отказ)
	...
	Если Проведен Тогда
		СтруктураЗаписи = РегистрыСведений.ДанныеПластиковыхКарт.СтрукураДанныхДляЗаписи();
		СтруктураЗаписи.ЧекККМ = Ссылка;
		РегистрыСведений.ДанныеПластиковыхКарт.ОчиститьРегистр(СтруктураЗаписи);
		
		СтруктураЗаписи.Контрагент = ВладелецДисконтнойКарты;
		Для Каждого СтрокаОплаты Из Оплата Цикл
			
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаОплаты.ВидОплаты, "ТипОплаты") = Перечисления.ТипыОплатЧекаККМ.ПлатежнаяКарта Тогда
				
				СлипЧекВерх = ВРег(СтрокаОплаты.СлипЧек);
				Если Не СтрНайти(СлипЧекВерх, "VISA") = 0
						Или СтрНачинаетсяС(СтрокаОплаты.НомерПлатежнойКарты, "4") Тогда
					СтруктураЗаписи.ТипПластиковойКарты = Перечисления.ТипыПластиковыхКарт.VISA;
				ИначеЕсли Не СтрНайти(СлипЧекВерх, "MASTERCARD") = 0
						Или СтрНачинаетсяС(СтрокаОплаты.НомерПлатежнойКарты, "5") Тогда
					СтруктураЗаписи.ТипПластиковойКарты = Перечисления.ТипыПластиковыхКарт.MASTERCARD;
				ИначеЕсли Не СтрНайти(СлипЧекВерх, "MAESTRO") = 0 Тогда
					СтруктураЗаписи.ТипПластиковойКарты = Перечисления.ТипыПластиковыхКарт.MAESTRO;
				ИначеЕсли Не СтрНайти(СлипЧекВерх, "PROSTIR") = 0 Тогда
					СтруктураЗаписи.ТипПластиковойКарты = Перечисления.ТипыПластиковыхКарт.PROSTIR;
				Иначе
					СтруктураЗаписи.ТипПластиковойКарты = Перечисления.ТипыПластиковыхКарт.Другие;
				КонецЕсли;
				
				СтруктураЗаписи.СуммаРозница = СтрокаОплаты.Сумма;
				
				РегистрыСведений.ДанныеПластиковыхКарт.ЗаписатьДанныеОплаты(СтруктураЗаписи);
...
//	При этом неважно, где мы сейчас находимся - ЦБ или ПБ.
	...
	Если ДополнительныеСвойства.УчетнаяПолитика.ДвиженияБонусныхБалловТолькоВЦентре = Истина Тогда	//	LNK 08.11.2022 06:37:19
		...
		ИменаРегистров.Добавить("РасшифровкаНачисленияБонусовПоЧекам");
	КонецЕсли;

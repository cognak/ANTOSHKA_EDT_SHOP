...
Процедура ПриЗаписи(Отказ)
	...
	//
	//А++ 20250102 по задаче оптимизации работы журнала. 
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаВедущая.Ссылка КАК ЧекККМ,
		...
		|			Документ.ЧекККМ.Оплата КАК ТаблицаОплата
		|		ГДЕ
		|			ТаблицаОплата.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплатЧекаККМ.БанковскийКредит)
		|		{ГДЕ
		|			ТаблицаОплата.Ссылка.*}
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		...
		|			Документ.ЧекККМ.Оплата КАК ТаблицаОплата
		|		ГДЕ
		|			ТаблицаОплата.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплатЧекаККМ.Предоплата)
		|		{ГДЕ
		|			ТаблицаОплата.Ссылка.*}
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		...
		|			Документ.ЧекККМ.Оплата КАК ТаблицаОплата
		|		ГДЕ
		|			ТаблицаОплата.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплатЧекаККМ.Послеплата)
		|		{ГДЕ
		|			ТаблицаОплата.Ссылка.*}
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		...
		|			Документ.ЧекККМ.Оплата КАК ТаблицаОплата
		|		ГДЕ
		|			ТаблицаОплата.ВидОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплатЧекаККМ.ПодарочныйСертификат)
		|		{ГДЕ
		|			ТаблицаОплата.Ссылка.*}) КАК ТаблицаДеление
		|		ПО ТаблицаВедущая.Ссылка = ТаблицаДеление.Ссылка
		|ГДЕ
		|	ТаблицаВедущая.Ссылка = &Ссылка
		|{ГДЕ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НаборЗаписей = РегистрыСведений.СостояниеОплатЧеков.СоздатьНаборЗаписей();
		
		НаборЗаписей.Отбор.ЧекККМ.Установить(ВыборкаДетальныеЗаписи.ЧекККМ); 

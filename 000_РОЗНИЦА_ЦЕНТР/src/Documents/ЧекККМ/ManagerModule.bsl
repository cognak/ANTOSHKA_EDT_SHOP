...
Функция ПолучитьПлатежныеРеквизиты(ЧекККМСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(ЧекККМСсылка.ЗаказПокупателя) Тогда
		Результат = РегистрыСведений.ОплатаЗаказаКлиента.СтруктураОплатыЗаказа(ЧекККМСсылка.ЗаказПокупателя);
		...
		//Liqpay не возвращает bank_id и term_id, поэтому получим его из справочника эквайринговые терминалы с привязкой к кассе.
		    РеквизитыЭТ = ПолучитьИдентификаторТерминала(ЧекККМСсылка.КассаККМ);
		...
		Если Результат.Свойство("amount") Тогда
			 СтруктураОтвета.sum = Результат.amount;
		КонецЕсли;

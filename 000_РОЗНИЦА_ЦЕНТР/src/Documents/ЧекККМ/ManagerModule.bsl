...
Функция ПолучитьПлатежныеРеквизиты(ЧекККМСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(ЧекККМСсылка.ЗаказПокупателя) Тогда
		...
		//Liqpay не возвращает bank_id и term_id, поэтому получим его из справочника эквайринговые терминалы с привязкой к кассе.
			 СтруктураОтвета.term_id = ПолучитьИдентификаторТерминала(ЧекККМСсылка.КассаККМ);
		//КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЧекККМОплата.ЭквайринговыйТерминал.МерчантИД КАК ТерминалИД,
...
Функция  РазобратьСлипЧек(СлипЧек)
	 СтруктураСлипЧека = Новый Структура("bank_name,bank_id,term_id,paysys,cardmask,auth_code,rrn","","","","","");
...
Функция ПолучитьИдентификаторТерминала(КассаККМ)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЭквайринговыеТерминалы.МерчантИД КАК ТерминалИД
	               |ИЗ

...
Функция ВходныеПараметрыПолучить(ДокументСсылка, ДополнительныеПараметры)	Экспорт
	...
	|	ЕСТЬNULL(ТаблицаДокумента.КассаККМ.ПРРО, Ведущая.КассаККМ.ПРРО) КАК ПРРО,
	|	ЕСТЬNULL(ТаблицаДокумента.КассаККМ.НоваяФормаЧека, Ведущая.КассаККМ.НоваяФормаЧека) КАК НоваяФормаЧека,
	|	ЕСТЬNULL(ТаблицаДокумента.КассаККМ.ШиринаЛенты, Ведущая.КассаККМ.ШиринаЛенты) КАК ШиринаЛенты,
	|	ЕСТЬNULL(ТаблицаДокумента.ДисконтнаяКарта, Ведущая.ТекущийДокумент.ДисконтнаяКарта) КАК ДисконтнаяКарта,
	...
	|	ЕСТЬNULL(ТаблицаДокумента.ПодакцизныеТоварыДляКоммерческогоИспользования, ЛОЖЬ) КАК ПодакцизныеТоварыДляКоммерческогоИспользования,
	...
	|	ЕСТЬNULL(ТаблицаДокумента.КассаККМ.ПодключаемоеОборудование, Ведущая.КассаККМ.ПодключаемоеОборудование) КАК ФР_Идентификатор,
	|	ЕСТЬNULL(ТаблицаДокумента.КассаККМ.ПодключаемоеОборудование.ОбработчикДрайвера, Ведущая.КассаККМ.ПодключаемоеОборудование.ОбработчикДрайвера) КАК ФР_ОбработчикДрайвера
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		ВЫРАЗИТЬ(&Кассир КАК Справочник.Пользователи) КАК Кассир) КАК Ведущая

...
//	Подготовка таблицы общих параметров
	...
	ИнформацияПодвала = ИнформацияПодвала
		...
		+ ПодготовитьСтроку("КАСИР "  +  РеквизитыЧека.Продавец) + Символы.ПС;
...
Функция ПечатьТоварногоЧекаДляПД(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.ЧекККМ.ПФ_MXL_ТоварныйЧекДляПД", КодЯзыкаПечать);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ВыборкаПоДокументам.ЭтоВозврат Тогда
			СинонимДокумента   = НСтр("ru='Чек (повернення)';uk='Чек (повернення)'", КодЯзыкаПечать);
		Иначе
...
#Область ОбеспечениеПечатиКопииЧекаНаФискальномРегистраторе
...
//	Получение макета
	МакетЧекаККМ      = Документы.ЧекККМ.ПолучитьМакет("ЧекПокупателяУкр");
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Иначе
				...
				ОбластьТелоЧека.Параметры.ЗагСкидка = ?(КонечнаяСкидка > 0,  НСтр("ru='Скидка:';uk='Знижка:'", КодЯзыкаПечать), НСтр("ru='Надбавка:';uk='Надбавка:'", КодЯзыкаПечать));
		...
		ДокументЧекККМ.Вывести(ОбластьПогашения);
	...
	Для Индекс = 1 По КоличествоСтрок Цикл
		...
		ТекстПечати = ТекстПечати + ДокументЧекККМ.ПолучитьСтроку(Индекс); 
...
Функция ПолучитьДанныеХранилища(Хранилище, Очистить = Ложь)	Экспорт
	...
	КлючиДанных.Вставить("ОтветПРРО"	 , Новый Структура);

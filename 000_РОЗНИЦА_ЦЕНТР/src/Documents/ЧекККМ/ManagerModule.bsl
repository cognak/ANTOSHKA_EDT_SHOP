...
Функция ПолучитьЗапросПроведенияИЗаполнитьРеквизитыШапки(ДокументСсылка, Реквизиты = Неопределено)	Экспорт
	...
	|	ЕСТЬNULL(ТаблицаДокумента.ЗаказПокупателя.ТипДоставки,
	|		ЗНАЧЕНИЕ(Перечисление.ТипДоставкиЗаказПокупателя.ПустаяСсылка)) КАК ЗаказПокупателяТипДоставки,
	|	ЕСТЬNULL(ТаблицаДокумента.ЗаказПокупателя.ТипОплаты,
	|		ЗНАЧЕНИЕ(Перечисление.ТипОплатыЗаказПокупателя.ПустаяСсылка)) КАК ЗаказПокупателяТипОплаты,
	...
	|	УчетнаяПолитикаСрезПоследних.ИнтернетМагазин = ТаблицаДокумента.Магазин КАК ИнтернетЗаказДоставка
	|ИЗ
...
Функция ВходныеПараметрыПолучить(ДокументСсылка, ДополнительныеПараметры)	Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(Товары.НаименованиеПолное КАК СТРОКА(3)) = ""   ""
	|			ТОГДА Товары.Наименование
	|		ИНАЧЕ Товары.НаименованиеПолное
	...
	|	Товары.КодПоУКТЗЕД КАК КодПоУКТЗЕД,
	...
	|	ВЫБОР
	|		КОГДА Товары.Артикул = """"
	|			ТОГДА Товары.Код
	|		ИНАЧЕ Товары.Артикул
	|	КОНЕЦ КАК Артикул,
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.Штрихкод = """"
	|			ТОГДА ЕСТЬNULL(Товары.Код, """")
	...
	|	Товары.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера
	|ПОМЕСТИТЬ Товары
	|ИЗ
	|	Документ.ЧекККМ.Товары КАК ТаблицаТовары
	...
	|		ПО ТаблицаТовары.Характеристика = ХарактеристикиНоменклатуры.Ссылка
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &ТекущийДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ТаблицаДокумента.Номер, """") КАК Номер,
	|	ЕСТЬNULL(ТаблицаДокумента.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК Дата,
	|	ЕСТЬNULL(ТаблицаДокумента.Проведен, ЛОЖЬ) КАК Проведен,
	|	ЕСТЬNULL(ТаблицаДокумента.ВидОперации, ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЧекККМ.Продажа)) КАК ВидОперации,
	|	ЕСТЬNULL(ТаблицаДокумента.Магазин, Ведущая.Магазин) КАК Магазин,
	|	ЕСТЬNULL(ТаблицаДокумента.КассаККМ, Ведущая.КассаККМ) КАК КассаККМ,
	|	ЕСТЬNULL(Кассы.ПРРО, Ведущая.ПРРО) КАК ПРРО,
	|	ЕСТЬNULL(Кассы.НоваяФормаЧека, Ведущая.НоваяФормаЧека) КАК НоваяФормаЧека,
	|	ЕСТЬNULL(Кассы.ШиринаЛенты, Ведущая.ШиринаЛенты) КАК ШиринаЛенты,
	|	ЕСТЬNULL(ТаблицаДокумента.ДисконтнаяКарта, Ведущая.ДисконтнаяКарта) КАК ДисконтнаяКарта,
	|	ЕСТЬNULL(ТаблицаДокумента.ЗаказПокупателя, НЕОПРЕДЕЛЕНО) КАК ЗаказПокупателя,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ВладелецДисконтнойКарты ЕСТЬ NULL
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ВЫБОР
	|			КОГДА ТаблицаДокумента.ВладелецДисконтнойКарты В (НЕОПРЕДЕЛЕНО, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|				ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка))
	|				ТОГДА ЕСТЬNULL(ИнформационныеКарты.ВладелецКарты, НЕОПРЕДЕЛЕНО)
	|			ИНАЧЕ ТаблицаДокумента.ВладелецДисконтнойКарты
	|		КОНЕЦ
	|	КОНЕЦ КАК ВладелецКарты,
	|	ЕСТЬNULL(ТаблицаКонтакты.НомерТелефона, ЕСТЬNULL(КонтрагентыКонтактнаяИнформация.НомерТелефона, """")) КАК
	|		НомерТелефона,
	|	ЕСТЬNULL(ТаблицаДокумента.ЦенаВключаетНДС, ИСТИНА) КАК ЦенаВключаетНДС,
	|	ЕСТЬNULL(ТаблицаДокумента.ПодакцизныеТоварыДляКоммерческогоИспользования, ЛОЖЬ) КАК
	|		ПодакцизныеТоварыДляКоммерческогоИспользования,
	|	ЕСТЬNULL(ТаблицаДокумента.СуммаДокумента, 0) КАК СуммаДокумента,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Ответственный ЕСТЬ NULL
	|			ТОГДА Ведущая.Кассир
	|		ИНАЧЕ ВЫБОР
	...
	|		КОНЕЦ
	|	КОНЕЦ КАК Продавец,
	|	ЕСТЬNULL(Заказ.НомерЗаказаНаСайте, """") КАК ИнтернетЗаказ,
	|	ЕСТЬNULL(Кассы.ПодключаемоеОборудование, Ведущая.ПодключаемоеОборудование) КАК ФР_Идентификатор,
	|	ЕСТЬNULL(Оборудование.ОбработчикДрайвера, Ведущая.ОбработчикДрайвера) КАК ФР_ОбработчикДрайвера
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВЫРАЗИТЬ(&ТекущийДокумент КАК Документ.ЧекККМ) КАК ТекущийДокумент,
	|		ВЫРАЗИТЬ(&Магазин КАК Справочник.Магазины) КАК Магазин,
	|		ВЫРАЗИТЬ(&КассаККМ КАК Справочник.КассыККМ) КАК КассаККМ,
	|		ВЫРАЗИТЬ(&Кассир КАК Справочник.Пользователи) КАК Кассир,
	|		КассыККМ.ПРРО КАК ПРРО,
	|		КассыККМ.НоваяФормаЧека КАК НоваяФормаЧека,
	...
	|		Оборудование.ОбработчикДрайвера КАК ОбработчикДрайвера
	|	ИЗ
	...
	|			ПОЛНОЕ СОЕДИНЕНИЕ Документ.ЧекККМ КАК ЧекККМ
	|			ПО ЧекККМ.Ссылка = &ТекущийДокумент
	|	ГДЕ
	|		КассыККМ.Ссылка = &КассаККМ) КАК Ведущая
	...
	|			ПО ТаблицаКонтакты.Ссылка = ТаблицаДокумента.ВладелецДисконтнойКарты
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПодключаемоеОборудование КАК Оборудование
	|				ПО Кассы.ПодключаемоеОборудование = Оборудование.Ссылка
	|			ПО ТаблицаДокумента.КассаККМ = Кассы.Ссылка
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК Заказ
	|			ПО ТаблицаДокумента.ЗаказПокупателя = Заказ.Ссылка
	|		ПО Ведущая.ТекущийДокумент = ТаблицаДокумента.Ссылка
	...
	|		ПО Ведущая.ДисконтнаяКарта = ИнформационныеКарты.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ТекущийДокумент", ?(ДокументСсылка = Неопределено, Документы.ЧекККМ.ПустаяСсылка(), ДокументСсылка));
//	LNK 08.03.2024 08:43:36 - Нижеуказанные параметры исользуются для получения данных для печати нулевого чека на ПРРО
	Запрос.УстановитьПараметр("Магазин"	, ДополнительныеПараметры.ПараметрыКассыККМ.Магазин);
	Запрос.УстановитьПараметр("КассаККМ", ДополнительныеПараметры.ПараметрыКассыККМ.КассаККМ);
	Запрос.УстановитьПараметр("Кассир"	, ДополнительныеПараметры.ПараметрыКассыККМ.Кассир);
	...
	ИЛИ
		 ТипЗнч(ДополнительныеПараметры) = Тип("Структура") И НЕ ДополнительныеПараметры.Свойство("БонусныйСчет")
	Тогда
...
//	Подготовка таблицы товаров
	...
	Пока ТоварыВыборка.Следующий() Цикл
		...
		СтрокаТаблицыТоваров.Добавить(ТоварыВыборка.Штрихкод);				//  1 - Штрихкод
...
//	Подготовка таблицы оплат
	...
	ПараметрыДанных = Новый Структура(
		"СуммаДокумента, ТипыОплат, КодТранзакции,НомерПлатежнойКарты"
		, СуммаВсего
...
//	Подготовка таблицы общих параметров
	...
	//2025-02-28 sa еще нужны использованные в чеке сертификаты
	МассивПогашенныеСертификаты = Новый Массив;
	Если НЕ ДокументСсылка.ПогашениеПодарочныхСертификатов.Количество() = 0 Тогда
		ИнформацияПодвала = ИнформацияПодвала + Символы.ПС
		+ "ПОГАШЕНI СЕРТИФIКАТИ:" + Символы.ПС;
		Для каждого СтрокаТаблицы Из ДокументСсылка.ПогашениеПодарочныхСертификатов Цикл
			ПогашенныйСертификат = Новый Структура("Номер,Номинал",СтрокаТаблицы.СерийныйНомер,СтрокаТаблицы.ПодарочныйСертификат.Номинал);
			МассивПогашенныеСертификаты.Добавить(ПогашенныйСертификат);
			ИнформацияПодвала = ИнформацияПодвала + СтрокаТаблицы.СерийныйНомер.КодСерийногоНомера + " (" + Формат(СтрокаТаблицы.ПодарочныйСертификат.Номинал, "ЧГ=") + ")" + Символы.ПС;
		КонецЦикла;
	КонецЕсли;
	...
	//2025-03-04 sa Данные эквайринга устанавливаются в фискальной части чека
	...
	УстановитьСуммуНаличнойОплатыИСдачи(ИнформацияПодвала, РеквизитыЧека.СуммаДокумента, ТаблицаПоТипамОплат, ПРРО, РеквизитыЧека.ШиринаЛенты);	//	LNK 01.02.2024 06:48:26
...
//	LNK 30.03.2023 07:24:01	- используется при печати в ПРРО
	...
	//2024-09-05 sa добавим еще нужные поля
	СтруктураДопПараметров = Новый Структура;
	СтруктураДопПараметров.Вставить("УИД",Строка(ДокументСсылка.УникальныйИдентификатор()));
	СтруктураДопПараметров.Вставить("ЭтоВозврат",ТипЧека=1);
	СтруктураДопПараметров.Вставить("ПлатежныеРеквизиты", ПолучитьПлатежныеРеквизиты(ДокументСсылка));
	СтруктураДопПараметров.Вставить("ФискальныйРегистратор", ДополнительныеПараметры.ПараметрыКассыККМ.РегистрационныйНомер);
	СтруктураЧекаОснования = ПолучитьФискальныйНомер(ДокументСсылка.ЧекККМПродажа);
	СтруктураЧекаОснования.Вставить("УИД",?(ЗначениеЗаполнено(ДокументСсылка.ЧекККМПродажа),Строка(ДокументСсылка.ЧекККМПродажа.УникальныйИдентификатор()),""));
	СтруктураДопПараметров.Вставить("ЧекККМПродажа",СтруктураЧекаОснования);
	СтруктураДопПараметров.Вставить("ПогашенныеСертификаты",МассивПогашенныеСертификаты);
	
	//СтруктураДопПараметров.Вставить("ЭтоИнтернетЗаказ",ЗначениеЗаполнено(ДокументСсылка.ЗаказПокупателя));
	//СтруктураДопПараметров.Вставить("ПлатежныеРеквизиты",РегистрыСведений.ОплатаЗаказаКлиента.СтруктураОплатыЗаказа(ДокументСсылка.ЗаказПокупателя));
	
	
	ОбщиеПараметры.Добавить(СтруктураДопПараметров);                 // 22 - Дополнительные параметры для печати чека
	...
	Для каждого ОписаниеТипаОплаты Из ПараметрыДанных.ТипыОплат Цикл
		...
		Если ОписаниеТипаОплаты.ТипОплаты = Перечисления.ТипыОплатЧекаККМ.ПодарочныйСертификат Тогда
			...
			Если СуммаСертификатов > 0 Тогда
				...
				Если СуммаСертификатов > 0 Тогда
					//2025-03-01 sa теперь каждый сертификат должен идти отдельной оплатой	
					Для каждого СтрокаТаблицы Из СтрокиТаблицыОплата Цикл
						СтрокаОплаты = Новый СписокЗначений();
						СтрокаОплаты.Добавить(ОписаниеТипаОплаты.ID);
						//если остаток погашения по сертификату меньше чем номинал сертификата, то в оплату идет только оставлшаяся часть 
						//остальное пойдет в сдачу
						Если СуммаСертификатов - СтрокаТаблицы.Сумма < 0 Тогда
						...
						СтрокаОплаты.Добавить(СтрокаТаблицы.НомерПлатежнойКарты);
						СтрокаОплаты.Добавить(СтрокаТаблицы.КодТранзакции);
...
//	LNK 01.02.2024 06:48:08
Процедура УстановитьСуммуНаличнойОплатыИСдачи(ИнформацияПодвала, СуммаДокумента, ТаблицаПоТипамОплат, ПРРО, ШиринаЛенты = 25)

	Если НЕ ПРРО = Истина Тогда
	...
	СуммаНаличных  = 0;
	СтрокиНаличных = ТаблицаПоТипамОплат.НайтиСтроки(Новый Структура("ТипОплаты", Перечисления.ТипыОплатЧекаККМ.Наличные));
	...
	Сдача = Макс(ТаблицаПоТипамОплат.Итог("Сумма") - СуммаДокумента, 0);
	Сдача = Мин(Сдача, СуммаНаличных);

	Если Сдача > 0 Тогда

		ИнформацияПодвала = ИнформацияПодвала + Символы.ПС
		+ ПодготовитьСтроку("Отримано готівкою " + Символы.Таб + Формат(СуммаНаличных, "ЧДЦ=2; ЧГ=") + " грн", ШиринаЛенты) + Символы.ПС
		+ ПодготовитьСтроку("Решта готівкою " + Символы.Таб + Формат(Сдача, "ЧДЦ=2; ЧГ=") + " грн", ШиринаЛенты);
...
//	Сичкаренко 10.12.2020
	...
	ТаблицыДляДвижений.Вставить("ТаблицаРасшифровкаНачисленияБонусовПоЧекам", Результат[43].Выгрузить());
...
Функция ТекстЗапросаТаблицаПродажПоЧекамИСкидкамНаценкам()
	...
	|					СкидкиНаценки КАК СкидкиНаценки
	|				ГДЕ
	|					СкидкиНаценки.СкидкаНаценка.СпособПримененияСкидки = ЗНАЧЕНИЕ(Перечисление.СпособыПримененияСкидокНаценок.ПрименитьВМоментРасчетаСкидокНаценок)
	|				
	|				СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаТаблицаБонусныеБаллы()
	...
	|		КОГДА &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЧекККМ.Возврат)
	|			ТОГДА ВЫБОР
	|				КОГДА &ЗаказПокупателяЗаполнен И &ИнтернетЗаказДоставка
	|					ТОГДА &ЗаказПокупателя
	|				ИНАЧЕ ВЫРАЗИТЬ(&ЧекККМПродажа КАК Документ.ЧекККМ)
	|			КОНЕЦ
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ЧекККМ.ПустаяСсылка)
	...
	|				ИЛИ &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЧекККМ.Возврат)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
	|		ИНАЧЕ ИСТИНА
	...
	|		КОГДА ТаблицаБонусы.ВидОперации В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийБонусныхБаллов.Начисление),
	|			ЗНАЧЕНИЕ(Перечисление.ВидыОперацийБонусныхБаллов.Аннуляция))
...
Функция ТекстЗапросаБонусыРасшифровка()	Экспорт	//	LNK 12.07.2022 06:02:32
	...
	|	ТабличнаяЧасть.Ссылка КАК Ссылка,
	|	ТабличнаяЧасть.НомерСтроки КАК НомерСтроки,
	|	ТабличнаяЧасть.СкидкаНаценка КАК АвтоматическаяСкидкаНаценка,
	|	ТабличнаяЧасть.Сумма КАК СуммаНачисленныхБонусов,
	|	ЧекККМТовары.Номенклатура КАК Номенклатура,
	...
	|	ЧекККМТовары.Ссылка.ВладелецДисконтнойКарты КАК ВладелецДисконтнойКарты,
	|	ЧекККМТовары.Ссылка КАК ДокументПродажи,
	|	ЧекККМТовары.Ссылка.Магазин КАК Магазин,
	|	ЧекККМТовары.Продавец КАК Продавец
	|ИЗ
	|	Документ.ЧекККМ.СкидкиНаценки КАК ТабличнаяЧасть
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ.Товары КАК ЧекККМТовары
	|		ПО ТабличнаяЧасть.Ссылка = ЧекККМТовары.Ссылка
	|			И ТабличнаяЧасть.КлючСвязи = ЧекККМТовары.НомерСтроки
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	|	И ТабличнаяЧасть.СпособПримененияСкидки = ЗНАЧЕНИЕ(Перечисление.СпособыПримененияСкидокНаценок.НачислитьБонусныеБаллы)" + "
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|";
...
Процедура ВыполнитьДвиженияРасширеннойАналитикиПродаж(ДокументСсылка, Период, СтатусЧека, РежимЗагрузки = Ложь, СкидкиНаценки = Истина, ПродажиПоЧекам = Истина, БонусныеБаллы = Истина)	Экспорт
    ...
    ИспользоватьРасшифровкуБонусов = (БонусныеБаллы = Истина И (УчетнаяПолитика.ИспользоватьЧекиКакРегистраторыНачисленияБонусныхБаллов ИЛИ СтатусЧека = Перечисления.СтатусыЧековККМ.Пробитый));
//	здесь реквизиты шапки документа применяются как параметры запроса.
	Запрос = ПолучитьЗапросПроведенияИЗаполнитьРеквизитыШапки(ДокументСсылка, Реквизиты);
	...
	+ ?(ИспользоватьРасшифровкуБонусов
			, ТекстЗапросаБонусыРасшифровка()
			, ""
		)
	;
	...
	Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДата());
//	LNK 10.01.2024 12:48:43	
	Запрос.УстановитьПараметр("ЭтоВозврат", Реквизиты.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат);
	Запрос.УстановитьПараметр("ФормироватьДвижения", Истина);
...
//	LNK 08.03.2023 04:46:29
	...
	Если ПродажиПоЧекам = Истина Тогда
		...
		ВыполнитьДвиженияПродажиПоЧекам(ДокументСсылка, РезультатыЗапроса, РежимЗагрузки);
	...
	И	(УчетнаяПолитика.ИспользоватьЧекиКакРегистраторыНачисленияБонусныхБаллов ИЛИ Запрос.Параметры.СтатусЧекаККМ = Перечисления.СтатусыЧековККМ.Пробитый) Тогда
		...
		КоллекцияДвижений.Добавить(Новый Структура("Индекс, НаборЗаписей, Номер", 25, РегистрыНакопления.БонусныеБаллы.СоздатьНаборЗаписей(), "НомерСтроки"));
		КоллекцияДвижений.Добавить(Новый Структура("Индекс, НаборЗаписей, Номер", 26, РегистрыНакопления.БонусныеБаллыВРезерве.СоздатьНаборЗаписей(), "НомерСтроки"));
		КоллекцияДвижений.Добавить(Новый Структура("Индекс, НаборЗаписей, Номер", 27, РегистрыСведений.СписанныеБонусныеБаллы.СоздатьНаборЗаписей(), "НомерСтрокиДокумента"));
		КоллекцияДвижений.Добавить(Новый Структура("Индекс, НаборЗаписей, Номер", 28, РегистрыНакопления.РасшифровкаНачисленияБонусовПоЧекам.СоздатьНаборЗаписей(), "НомерСтроки"));
...
//	LNK 10.01.2024 12:25:27
Процедура ВыполнитьДвиженияПродажиПоЧекам(ДокументСсылка, РезультатыЗапроса, РежимЗагрузки = Ложь)
...
//	---------------------------------------------------------------------------------------
//	LNK 10.01.2024 12:30:15

	НаборЗаписей = РегистрыСведений.ДвиженияСерийныхНомеров.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Регистратор.Установить(ДокументСсылка);
	НаборЗаписей.Загрузить(РезультатыЗапроса[13].Выгрузить());
	НаборЗаписей.ОбменДанными.Загрузка = РежимЗагрузки;
	...
	НаборЗаписей = РегистрыСведений.СостоянияСерийныхНомеров.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Регистратор.Установить(ДокументСсылка);
	НаборЗаписей.Загрузить(РезультатыЗапроса[20].Выгрузить());
	НаборЗаписей.ОбменДанными.Загрузка = РежимЗагрузки;
...
Функция ЕстьДругойТипОплаты(ДокСсылка)
	...
	Запрос.Текст = 
		...
		|	И ЧекККМОплата.ВидОплаты <> &ВидОплаты2
		|			И ЧекККМОплата.Сумма > 0";
	Запрос.УстановитьПараметр("ВидОплаты", Справочники.ВидыОплатЧекаККМ.ОплатаПодарочнымСертификатом);  
	Запрос.УстановитьПараметр("ВидОплаты2", Справочники.ВидыОплатЧекаККМ.Предоплата);
	Запрос.УстановитьПараметр("Ссылка", ДокСсылка);
...
Функция ПечатьНаПринтерЧеков(ВыборкаОбщая, РезультатыЗапроса, ОбъектыПечати, ПараметрыВывода, ПервыйДокумент)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		//2025-06-30 sa добавим в подвал чека сумму покупок для контроля.
		ОбластьДопПараметр        = Макет.ПолучитьОбласть("ДопПараметр|_"        + ШиринаЛенты);
		ОбластьДопПараметр.Параметры.Параметр = "РЕШТА: ";
		Если ВыборкаПоДокументам.ЭтоВозврат Тогда
			ОбластьДопПараметр.Параметры.Значение = "0.00";
		Иначе
			ОбластьДопПараметр.Параметры.Значение = Формат(СуммаОплат - ВыборкаПоДокументам.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧРД=.; ЧН=; ЧГ=0");
		КонецЕсли;
		ТабличныйДокумент.Вывести(ОбластьДопПараметр);

		ОбластьДопПараметр        = Макет.ПолучитьОбласть("ДопПараметр|_"        + ШиринаЛенты);
		ТабличныйДокумент.Вывести(ОбластьДопПараметр);
		
		ДанныеКарты = РозничныеПродажиСервер.ПолучитьДанныеКарты(ВыборкаПоДокументам.Ссылка, ВыборкаПоДокументам.Ссылка.ВладелецДисконтнойКарты, ВыборкаПоДокументам.Ссылка.ДисконтнаяКарта,,,ВыборкаПоДокументам.Ссылка.ЗаказПокупателя);
		ОбластьДопПараметр        = Макет.ПолучитьОбласть("ДопПараметр|_"        + ШиринаЛенты);
		ОбластьДопПараметр.Параметры.Параметр = "СУММА ПОКУПОК: ";
		ОбластьДопПараметр.Параметры.Значение = Формат(ДанныеКарты.СуммаПокупокОбычная,"ЧДЦ=2; ЧН=; ЧГ=")+" грн.";
		ТабличныйДокумент.Вывести(ОбластьДопПараметр);
...
//	LNK 07.02.2024 11:56:32
//	Длина с запасом. При записи дополняется справа "0".
//	1. "0" или "1" - отправлен е-чек (ПРРО)
//	2. "0", "1", "2" - нет печати, есть печать, есть печать по запросу клиента (ПРР)
//
Процедура УстановитьФлагиПроведения(Объект, ДанныеПРРО)	Экспорт

	Перем ФлагиПечати;	//	= "Напечатан, ТребованиеПечати, SMS"

	Если ПустаяСтрока(Объект.ФлагиПроведения) Тогда

		Объект.ФлагиПроведения = ОбщегоНазначенияКлиентСервер.PAD(Объект.ФлагиПроведения, Метаданные.Документы.ЧекККМ.Реквизиты.ФлагиПроведения.Тип.КвалификаторыСтроки.Длина, Ложь, "0");

	КонецЕсли;

	Если ТипЗнч(ДанныеПРРО) = Тип("Структура") И ДанныеПРРО.Свойство("ФлагиПечати", ФлагиПечати) Тогда

		УстановитьФлагПроведения(Объект.ФлагиПроведения, 1, ?(ФлагиПечати.SMS = Истина, "1", "0"));
		УстановитьФлагПроведения(Объект.ФлагиПроведения, 2
			, ?(ФлагиПечати.Напечатан = Истина И ФлагиПечати.ТребованиеПечати = Истина
				, "2"
			, ?(ФлагиПечати.Напечатан = Истина
				, "1"
				, "0")
			)
		);
...
//	LNK 07.02.2024 13:45:26
Процедура УстановитьФлагПроведения(ФлагиПроведения, Позиция, Значение)

	СтрокаСлева	 = Лев(ФлагиПроведения , Позиция - 1);
	СтрокаСправа = Сред(ФлагиПроведения, Позиция + 1);

	ФлагиПроведения = СтрокаСлева + Значение + СтрокаСправа;

КонецПроцедуры



//2024-09-03 sa фиксируем номер чека в базе. Новые ФН текстовые, поэтому ЧекККМ.НомерЧекаККМ не подходит. Делать рестуктуризацию никто не хочет.
Функция ЗаписатьФискальныйНомер(ЧекСсылка,ФН,ФР,ОтветПРРО="") Экспорт
	СтруктураОтвета = Новый Структура("Успешно,ОписаниеОшибки",Ложь,"");
	
	Если Не ЗначениеЗаполнено(ФН) Тогда
		СтруктураОтвета.Вставить("ОписаниеОшибки","Не указан фискальный номер");
	...
	Запрос.УстановитьПараметр("ЧекККМ",ЧекСсылка);
	Рез_ = Запрос.Выполнить().Выбрать();
	Если Рез_.Следующий() Тогда
		СтруктураОтвета.Вставить("ОписаниеОшибки","Фискальный номер уже существует в базе");
		Возврат СтруктураОтвета;
	КонецЕсли;
	
	РС = РегистрыСведений.ФискальныеНомераЧеков.СоздатьМенеджерЗаписи();
	РС.Активность = Истина;
	РС.ЧекККМ = ЧекСсылка;
	РС.ФискальныйНомер = ФН;
	РС.ФискальныйРегистратор = ФР;
	//2026-07-16 sa Приняли решение не хранить ответ ПРРО. Т.к. он занимает много места.
	//(якобы есть механизм получения данных ПРРО из DeviceManager)
	РС.ОтветПРРО = "";
	Попытка
		РС.Записать(Истина);
...
Функция ПолучитьФискальныйНомер(ЧекСсылка) Экспорт
	СтруктураОтвета = Новый Структура("ФискальныйНомер,ФискальныйРегистратор,Дата","","");
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ФискальныеНомераЧеков.ФискальныйНомер КАК ФискальныйНомер,
	               |	ФискальныеНомераЧеков.ФискальныйРегистратор КАК ФискальныйРегистратор,
	               |	ФискальныеНомераЧеков.ЧекККМ.Дата КАК Дата
	               |ИЗ
	               |	РегистрСведений.ФискальныеНомераЧеков КАК ФискальныеНомераЧеков
	               |ГДЕ
	               |	ФискальныеНомераЧеков.ЧекККМ = &ЧекККМ";
	...
	Рез_ = Запрос.Выполнить().Выбрать();
	Если Рез_.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(СтруктураОтвета,Рез_);	
	КонецЕсли;
...
Функция ПолучитьПлатежныеРеквизиты(ЧекККМСсылка) Экспорт
	ДанныеЧека = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЧекККМСсылка, "ЗаказПокупателя, КассаККМ, СуммаДокумента, ВидОперации");
	ПлатежныеРеквизиты = Новый Соответствие;
	СтруктураОтвета = Новый Структура ("bank_name,bank_id,term_id,paysys,rrn,cardmask,oper_type,auth_code,sum, commission","","","","","","","","",0,0);
	
	//2024-10-25 sa Для интернет-заказов структура платежных реквизитов определяется особым способом)
	Если ЗначениеЗаполнено(ДанныеЧека.ЗаказПокупателя) Тогда
		Результат = РегистрыСведений.ОплатаЗаказаКлиента.СтруктураОплатыЗаказа(ДанныеЧека.ЗаказПокупателя);
		
		Если Результат.Свойство("sender_card_bank") Тогда
			 СтруктураОтвета.bank_name = Результат.sender_card_bank;
		КонецЕсли;
		//Если Результат.Свойство("bank_id") Тогда
		//	 СтруктураОтвета.bank_id = Результат.sender_card_bank;
		//КонецЕсли;
		//Если Результат.Свойство("term_id") Тогда
		//Liqpay не возвращает bank_id и term_id, поэтому получим его из справочника эквайринговые терминалы с привязкой к кассе.
		    РеквизитыЭТ = ПолучитьИдентификаторТерминала(ДанныеЧека.КассаККМ);
			СтруктураОтвета.term_id = РеквизитыЭТ.ТерминалИД;
			СтруктураОтвета.bank_id = РеквизитыЭТ.МерчантИД;
			
		//КонецЕсли;
		Если Результат.Свойство("sender_card_type") Тогда
			 СтруктураОтвета.paysys = Результат.sender_card_type;
		КонецЕсли;
		Если Результат.Свойство("rrn_debit") Тогда
			 СтруктураОтвета.rrn = Результат.rrn_debit;
		КонецЕсли;
		Если Результат.Свойство("sender_card_mask2") Тогда
			 СтруктураОтвета.cardmask = Результат.sender_card_mask2;
		КонецЕсли;
		Если Результат.Свойство("action") Тогда
			 СтруктураОтвета.oper_type = Результат.action;
		КонецЕсли;
		Если Результат.Свойство("authcode_debit") Тогда
			 СтруктураОтвета.auth_code = Результат.authcode_debit;
		КонецЕсли;
		Если Результат.Свойство("amount") Тогда
			 СтруктураОтвета.sum = ?(ДанныеЧека.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат, ДанныеЧека.СуммаДокумента, Результат.amount);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтруктураОтвета.rrn) Тогда
			ПлатежныеРеквизиты.Вставить(СтруктураОтвета.rrn,СтруктураОтвета);
			Возврат ПлатежныеРеквизиты;
		КонецЕсли;
		//иначе пробуем стандартным методом
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЧекККМОплата.Ссылка.КассаККМ.РегистрационныйНомер КАК РегистрационныйНомер,
	               |	ЧекККМОплата.ЭквайринговыйТерминал КАК ЭквайринговыйТерминал,
	               |	ЧекККМОплата.Сумма КАК Сумма,
	               |	ЧекККМОплата.СсылочныйНомер КАК НомерТранзакции,
	               |	ЧекККМОплата.НомерЧекаЭТ КАК НомерЧекаЭТ,
	               |	ЧекККМОплата.ЭквайринговыйТерминал.МерчантИД КАК МерчантИД,
	               |	ЧекККМОплата.ЭквайринговыйТерминал.ТерминалИД КАК ТерминалИД,
	               |	ЧекККМОплата.НомерПлатежнойКарты КАК НомерКарты,
	               |	ЧекККМОплата.СлипЧек КАК СлипЧек,
	               |	ВЫБОР
	               ...
	               |	КОНЕЦ КАК ВидОперации,
	               |	ЧекККМОплата.ВидОплаты КАК ВидОплаты,
	               |	ЧекККМОплата.Ссылка.ДоговорЭквайринга КАК ДоговорЭквайринга,
	               |	ЧекККМОплата.СуммаТорговойУступки КАК commission
	               |ИЗ
	               |	Документ.ЧекККМ.Оплата КАК ЧекККМОплата
	               |ГДЕ
	               |	ЧекККМОплата.Ссылка = &Ссылка
	...
	Пока Результат.Следующий() Цикл
		СтруктураОтвета = Новый Структура ("bank_name,bank_id,term_id,paysys,rrn,cardmask,oper_type,auth_code,sum, commission","","","","","","","","",0,0);
		СтруктураСлипЧека = РазобратьСлипЧек(Результат.СлипЧек);
		ЗаполнитьЗначенияСвойств(СтруктураОтвета,СтруктураСлипЧека);
		
		СтруктураОтвета.rrn = ?(ЗначениеЗаполнено(СтруктураОтвета.rrn),СтруктураОтвета.rrn, Результат.НомерТранзакции);
		СтруктураОтвета.cardmask = ?(ЗначениеЗаполнено(СтруктураОтвета.cardmask),СтруктураОтвета.cardmask, Результат.НомерКарты);
		СтруктураОтвета.oper_type = ?(ЗначениеЗаполнено(СтруктураОтвета.oper_type),СтруктураОтвета.oper_type, Результат.ВидОперации);
		Если Не ЗначениеЗаполнено(СтруктураОтвета.term_id) Тогда
			СтруктураОтвета.term_id = Результат.ТерминалИД;
		КонецЕсли;
		//выяснилось что мерчант ИД в национальном кэшбеке не используется
		//2025-01-28 sa выяснилось что Мерчант ИД используется банком для передачи в национальный кешбек                   
		Если Не ЗначениеЗаполнено(СтруктураОтвета.bank_id) Тогда
			СтруктураОтвета.bank_id = Результат.МерчантИД;
		КонецЕсли;
		СтруктураОтвета.sum = Результат.Сумма;
...
Функция  РазобратьСлипЧек(СлипЧек) Экспорт
	 СтруктураСлипЧека = Новый Структура("bank_name,bank_id,term_id,paysys,cardmask,auth_code,rrn,oper_type","","","","","");
	 Поля = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СлипЧек,Символы.ПС,Ложь);
	 
	 Если Поля.Количество()>0 Тогда
	 	СтруктураСлипЧека.Вставить("bank_name",Поля[0]);
	 КонецЕсли;
	
	 Если Поля.Количество()>1 Тогда
		 СтруктураСлипЧека.Вставить("paysys",Поля[1]);
	 КонецЕсли;
	 
	 Если Поля.Количество()>2 Тогда
		 СтруктураСлипЧека.Вставить("cardmask",Поля[2]);
	 КонецЕсли;
	 
	 Если Поля.Количество()>3 Тогда
		вПоз = Найти(Поля[3],":");
		Если вПоз > 0 Тогда
	 		СтруктураСлипЧека.Вставить("auth_code",СокрЛП(Сред(Поля[3],вПоз+1)));
		Иначе
	 		СтруктураСлипЧека.Вставить("auth_code",Поля[3]);
		КонецЕсли;
	 КонецЕсли;
	 ...
	 Если Поля.Количество()>4 Тогда
		вПоз = Найти(Поля[4],":");
		Если вПоз > 0 Тогда
	 		СтруктураСлипЧека.Вставить("rrn",СокрЛП(Сред(Поля[4],вПоз+1)));
		Иначе
	 		СтруктураСлипЧека.Вставить("rrn",Поля[4]);
		КонецЕсли;
	 КонецЕсли;
	 ...
	 Если Поля.Количество()>7 Тогда
		вПоз = Найти(Поля[7],":");
		Если вПоз > 0 Тогда
	 		СтруктураСлипЧека.Вставить("bank_id",СокрЛП(Сред(Поля[7],вПоз+1)));
		Иначе
	 		СтруктураСлипЧека.Вставить("bank_id",Поля[7]);
		КонецЕсли;
	 КонецЕсли;
	 Если Поля.Количество()>8 Тогда
		вПоз = Найти(Поля[8],":");
		Если вПоз > 0 Тогда
	 		СтруктураСлипЧека.Вставить("term_id",СокрЛП(Сред(Поля[8],вПоз+1)));
		Иначе
	 		СтруктураСлипЧека.Вставить("term_id",Поля[8]);
		КонецЕсли;
	 КонецЕсли;
	 ...
	 Если Поля.Количество()>9 Тогда
		вПоз = Найти(Поля[9],":");
		Если вПоз > 0 Тогда
	 		СтруктураСлипЧека.Вставить("oper_type",СокрЛП(Сред(Поля[9],вПоз+1)));
		Иначе
	 		СтруктураСлипЧека.Вставить("oper_type",Поля[9]);
		КонецЕсли;
	 КонецЕсли;
	 
	 Возврат СтруктураСлипЧека;	
КонецФункции

Функция ПолучитьИдентификаторТерминала(КассаККМ)
	СтруктураОтвета = Новый Структура("МерчантИД,ТерминалИД","","");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЭквайринговыеТерминалы.МерчантИД КАК МерчантИД,
	               |	ЭквайринговыеТерминалы.ТерминалИД КАК ТерминалИД
	               |ИЗ
...
//ПЕЧАТЬ КУПОНОВ
#Область ПЕЧАТЬ_КУПОНОВ
&НаСервере 
Функция ПолучитьТабДокКупона(ЧекККМСсылка) Экспорт
	...
	Если НЕ МассивВходныеПараметры = Неопределено тогда 
		...
		ПечатьQR = ПроверитьНужноЛиПечататьQR(ЧекККМСсылка);
		Если ПечатьQR Тогда
			ОбластьШтрихкод 		= Макет.ПолучитьОбласть("ОбластьQR");   
		Иначе
			ОбластьШтрихкод 		= Макет.ПолучитьОбласть("ОбластьШтрихкод");
		КонецЕсли;
		
		Для каждого ВходныеПараметры из МассивВходныеПараметры	цикл
			Если ВходныеПараметры[5] = Перечисления.ВариантыВыдачиКупона.ПечататьНаЧеке Тогда
				//ВходныеПараметры[7] - здесь должны быть массив строк для печати купона из настроек акции. Одна из строк может быть QR кодом
				ШаблонЧека = ВходныеПараметры[7];
				Если ЗначениеЗаполнено(ШаблонЧека) Тогда
					Для Каждого СтрокаШаблона Из  ШаблонЧека.СтрокиПодвала Цикл
						Если СтрокаШаблона.QRКод Тогда
							ГенераторQRКода = ПолучитьГенераторQRкода(); 
							Попытка
								ДанныеQRКода = ГенераторQRКода.GenerateQRCode(СтрокаШаблона.Строка, 0, 190);
								КартинкаQRКода = Новый Картинка(ДанныеQRКода);
								ОбластьШтрихкод.Рисунки.КРКод.Картинка = КартинкаQRКода;
							Исключение
						...
						Иначе
							ОбластьСтрока.Параметры.Содержание = СтрокаШаблона.Строка;
			...
			Иначе
				...
				Если НЕ ПустаяСтрока(ВходныеПараметры[1]) Тогда	//	имеется ШК
					...
					Если ПечатьQR Тогда
						ГенераторQRКода = ПолучитьГенераторQRкода(); 
						Попытка
							ДанныеQRКода = ГенераторQRКода.GenerateQRCode(ВходныеПараметры[1], 0, 190);
							КартинкаQRКода = Новый Картинка(ДанныеQRКода);
							ОбластьШтрихкод.Рисунки.КРКод.Картинка = КартинкаQRКода;
						Исключение
							Сообщить(ОписаниеОшибки());
...
Функция ПолучитьГенераторQRкода()
    ...
    //лМакетКомпоненты = Обработка.ПолучитьМакет("КомпонентаПечатиQRКода");
...
&НаСервере 
Функция ПроверитьНужноЛиПечататьQR(ЧекККМСсылка)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЧекККМ.Ссылка КАК Ссылка,
	               |	ЧекККМ.КассаККМ КАК КассаККМ,
	               |	ЧекККМ.КассаККМ.ПечатьКодQR КАК КассаККМПечатьКодQR
	               |ИЗ
	               |	Документ.ЧекККМ КАК ЧекККМ
	               |ГДЕ
	               |	ЧекККМ.Ссылка = &Ссылка";
	...
	Рез = Запрос.Выполнить().Выбрать();
	Если Рез.Следующий() Тогда
		Возврат Рез.КассаККМПечатьКодQR;
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции
...
&НаСервере
Функция ПолучитьТекстУникальногоКупона(ЧекККМСсылка) Экспорт
...
&НаСервере
Процедура СделатьЗаписьВРССерийныеНомераДляАкций(ЧекККМСсылка, СкидкаНаценка, КодСерийногоНомера, СерийныйНомер, СуммаКупона=0, ТекстСообщенияКассиру="") Экспорт
...
&НаСервере
Процедура ЗаписатьСкидочныйКупонВРегистрПоКоду(ЧекККМСсылка, КодСерийногоНомера) Экспорт  // Аннуляция купона при возврате

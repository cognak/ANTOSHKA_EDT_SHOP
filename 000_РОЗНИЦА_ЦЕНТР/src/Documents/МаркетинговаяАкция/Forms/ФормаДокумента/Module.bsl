...
&НаКлиенте
Процедура ДобавитьАкцииДляСотрудников(Команда)
	ОткрытьФорму("Обработка.КопированиеАкций.Форма", , ЭтаФорма);
КонецПроцедуры

 &НаСервере
Функция  ПолучитьУчетнуюПолитику();
	Возврат	ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику();
КонецФункции
&НаКлиенте
Процедура ИспользоватьИнтернетМагазинПриИзменении(Элемент)  
	УчетнаяПолитика = ПолучитьУчетнуюПолитику();
	ИнтернетМагазин = УчетнаяПолитика.ИнтернетМагазин;
	
	Если ЗначениеЗаполнено(ИнтернетМагазин) Тогда
		
		НайденныеСтроки = Объект.Магазины.НайтиСтроки(Новый Структура("Магазин", ИнтернетМагазин));
		
		Если Объект.ИспользоватьИнтернетМагазин Тогда
			// Флаг установлен, добавляем магазин, если его еще нет в списке
			Если НайденныеСтроки.Количество() = 0 Тогда
				НоваяСтрока = Объект.Магазины.Добавить();
				НоваяСтрока.Магазин = ИнтернетМагазин;
			КонецЕсли;
		Иначе
			// Флаг снят, удаляем магазин из списка, если он там есть
			Для Каждого Стр Из НайденныеСтроки Цикл
				Объект.Магазины.Удалить(Стр);
			КонецЦикла;
		КонецЕсли;

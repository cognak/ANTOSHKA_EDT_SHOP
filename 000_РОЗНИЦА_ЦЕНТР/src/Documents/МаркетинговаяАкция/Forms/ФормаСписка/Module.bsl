...
&НаСервере
Процедура КопироватьАкциюНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура КопироватьАкцию(Команда)
	ТекущаяАкция = Элементы.Список.ТекущаяСтрока;
	
	Если Не ЗначениеЗаполнено(ТекущаяАкция) Тогда
		Возврат; // Ничего не делаем, если строка не выбрана
	КонецЕсли;
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("МаркетинговаяАкция", ТекущаяАкция);
	
	ОткрытьФорму("Обработка.КопированиеАкций.Форма", ПараметрыОткрытия);
КонецПроцедуры

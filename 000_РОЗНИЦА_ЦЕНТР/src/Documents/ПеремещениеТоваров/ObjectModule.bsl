...
//	LNK 16.02.2017 14:33:05 - здесь ИсключительныйРежим будет отлючен для бесправного пользователя
	...
	ИначеЕсли Товары.Количество() = 0 тогда
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Проведение невозможно! Пустой документ можно только записать.", Ссылка,,, Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
//	NAV:БлокировкаДанныхУчестьДокументВNavision
	...
	ИначеЕсли ВнешниеИсточникиСобытия.ПередачаNavision(Ссылка) Тогда	//	LNK 28.12.2023 05:52:24

	//	.. в условии выше версия и так будет установлена актуальная!
		РегистрыСведений.ОбъектУчтенВNavision.УстановитьВерсиюДанных(ЭтотОбъект);	//	LNK 28.12.2023 05:48:07
...
//	---------------------------------------------------------------------------------------
	...
	Если ВнешниеИсточникиСобытия.ПередачаNavision(Ссылка) Тогда
		...
		Если ОбменДаннымиПовтИсп.ПолучитьДанныеУзла().Магазин = ПолучитьМагазинДокумента() Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗапросДоступностиТоваров") Тогда 
		...
		Если Не (ДанныеЗаполнения.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.КОтправке
				...
				Или ДанныеЗаполнения.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Отправлен
		...
		Запрос.Текст = 
			...
			|		И (ТоварыПеремещение.Ссылка.МагазинПолучатель.СкладУправляющейСистемы)
			|ГДЕ
			...
			|	И НЕ ТоварыЗапрос.Количество = 0"; //СЮП 07.02.2024 22.53.09 https://awdev.atlassian.net/browse/RETAIL1C-635
		...
		Если Не ПеремещениеУжеЕсть Тогда
			...
			|	ВЫБОР
			|		КОГДА ТаблицаДокумента.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.Выдан)
			|		ИЛИ Не ТаблицаДокумента.ОператорДоставки = ЗНАЧЕНИЕ(Перечисление.ОператорыДоставки.ВнутренняяЛогистика)
			|			ТОГДА ЕСТЬNULL(УчетнаяПолитика.СкладЦентральныйИнтернетМагазина, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))
			|		ИНАЧЕ ЕСТЬNULL(УчетнаяПолитика.СкладЦентральныйИнтернетМагазина.Магазин.СкладПереброски,
...
//			Иначе
		...
		//	КонецЕсли;
			...
			//СЮП 15.02.2024 11.14.04 https://awdev.atlassian.net/browse/RETAIL1C-652
			ПозицияЗД = СтрНайти(РеквизитыДанных.Номер, "-");
			Если Не ПозицияЗД = 0 Тогда
				
				НомерВнешнегоДокумента = СокрЛП(Лев(РеквизитыДанных.Номер, ПозицияЗД - 1));

			КонецЕсли;
			
			РеквизитыМагазина = ОбщегоНазначенияРТповтИсп.РеквизитыМагазина(РеквизитыДанных.МагазинОтправитель, Дата);
	...
	//	В этом месте производится заполнение "конечного" перемещения по схеме "Переброска между магазинами".
		...
		|	ТаблицаВедущая.МагазинПолучатель КАК МагазинОтправитель,
		|	ТаблицаВедущая.МагазинПолучательКонечный КАК МагазинПолучатель,
		...
		|		ПО ТаблицаВедущая.МагазинПолучательКонечный = ТаблицаОрганизаций.Владелец
		|ГДЕ
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения)
	...
	|	Документ.ЗапросДоступностиТоваров.Товары КАК ТоварыЗапрос
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров.Товары КАК ТоварыПеремещение
	|		ПО (НЕ ТоварыПеремещение.Ссылка.ПометкаУдаления)
	|		И ТоварыЗапрос.Ссылка = ТоварыПеремещение.Ссылка.ДокументОснование
	|		И (ТоварыПеремещение.Номенклатура = ТоварыЗапрос.Номенклатура)
	|		И (ТоварыПеремещение.Ссылка.МагазинПолучатель.СкладУправляющейСистемы)
	|ГДЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВсеСтрокиЗапроса КАК ВсеСтрокиЗапроса
	|		ПО Ведущая.Ссылка = ВсеСтрокиЗапроса.Ссылка
	|		И Ведущая.Номенклатура = ВсеСтрокиЗапроса.Номенклатура
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(&ДанныеЗаполнения) = ТИП(Документ.ЗапросДоступностиТоваров)
	...
	|	И НЕ ВсеСтрокиЗапроса.ЕстьПеремещение
	|	И НЕ Ведущая.Количество = 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки"
	); //СЮП 07.02.2024 22.53.09 https://awdev.atlassian.net/browse/RETAIL1C-635
	//СЮП 02.04.2024 23.00.13 
	Запрос.УстановитьПараметр("ДанныеЗаполнения", ДанныеЗаполнения);
	Запрос.УстановитьПараметр("ГраницаСреза", ?(ЭтоНовый(), ТекущаяДатаСеанса(), Новый Граница(МоментВремени(), ВидГраницы.Исключая)));

...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗапросДоступностиТоваров") Тогда 
		...
		Если Не РезультатЗапроса.Пустой() Тогда 
			...
			ДополнительныеСвойства.Вставить("ПеремещениеУжеЕсть", Истина);
			
			//ТекстОшибки = НСтр("ru='Отказано! Для документа «%1» уже есть перемещение. Ввод на основании такого документа невозможен.'");
			//ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ДанныеЗаполнения);
			//
			//ВызватьИсключение ТекстОшибки;   
		...
		Иначе
			...
			|	ТаблицаДокумента.МагазинОтправитель КАК МагазинОтправитель,
			|	ТаблицаДокумента.МагазинОтправитель.СкладПродажи КАК СкладОтправитель,
			|	ЕСТЬNULL(УчетнаяПолитика.СкладЦентральныйИнтернетМагазина.Магазин, ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка)) КАК МагазинПолучатель,
			|	ЕСТЬNULL(УчетнаяПолитика.СкладЦентральныйИнтернетМагазина, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК СкладПолучатель,
			|	ВЫБОР
			|		КОГДА ТаблицаДокумента.МагазинПолучатель = ЕСТЬNULL(УчетнаяПолитика.ИнтернетМагазин, ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка))
...
Процедура ЗаполнитьПоЗаказуПокупателя(ДанныеЗаполнения)
	...
	|	ТаблицаЗаказы.КоличествоОстаток,
	|	ТаблицаЗаказы.КоличествоОстаток
	|ИЗ
	|	Документ.ЗапросДоступностиТоваров.Товары КАК Ведущая
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(&ГраницаСреза, Заказ = &ДанныеЗаполнения) КАК ТаблицаЗаказы
	|		ПО Ведущая.Ссылка = ТаблицаЗаказы.Заказ
	|			И Ведущая.Номенклатура = ТаблицаЗаказы.Номенклатура
	|			И Ведущая.Характеристика = ТаблицаЗаказы.Характеристика
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(&ДанныеЗаполнения) = ТИП(Документ.ЗапросДоступностиТоваров)
	|	И Ведущая.Ссылка = &ДанныеЗаполнения
	|	И ТаблицаЗаказы.КоличествоОстаток > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки"
	);

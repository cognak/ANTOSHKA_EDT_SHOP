...
Процедура ВыгрузитьВТСД(Команда)
	...
	Иначе    
		Если РозничныеПродажиСервер.НовыеСтикеры() тогда
			ТранспортТерминаловНовый.ВыгрузитьПакетДокументов(ПакетДокумнтов);   
		Иначе
...
Процедура УдалитьИзТСДНаСервере()
	...
	КонецЕсли;
		Если РозничныеПродажиСервер.НовыеСтикеры() тогда
			ТранспортТерминаловНовый.УстановитьСтатусОтмена(ТипДок,Объект.Номер,Объект.Дата);   
		Иначе

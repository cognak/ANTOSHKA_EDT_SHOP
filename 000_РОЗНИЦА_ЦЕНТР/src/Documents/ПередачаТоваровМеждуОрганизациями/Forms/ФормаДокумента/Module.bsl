...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПерерасчетПроизведен = НЕ НДСИсходящийСервер.НуженАвторасчетНДС(Объект.Товары, Объект.ЦенаВключаетНДС);
...
Процедура АдресДоставкиНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	СоответствиеАдресов = ПолучитьСписокАдресовОрганизации(Объект.ОрганизацияПолучатель);
...
Процедура ПодборПоОстаткамТоваровОтправителя(Команда)
	...
	Если ОбщегоНазначенияРТКлиент.ПроверитьВозможностьЗаполненияТабличнойЧасти(ЭтотОбъект, Неопределено, СтруктураРеквизитов) Тогда
...
Процедура ЗаполнитьЦеныПоВидуЦен(Команда)
	...
	Иначе	
		...
		ВидЦены = ОткрытьФормуМодально("Справочник.ВидыЦен.ФормаВыбора", Новый Структура("Отбор", СтруктураОтбора));
...
Процедура УстановитьТекущуюСтраницуСуммПодвала()
	...
	Если Объект.УчитыватьНДС Тогда
		Элементы.Итоги.ТекущаяСтраница = Элементы.ИтогоСНДС;
	Иначе
		Элементы.Итоги.ТекущаяСтраница = Элементы.ИтогоБезНДС;
	КонецЕсли;
...
Функция ЗаполнитьЦеныПоВидуЦенСервер(ВидЦены)
	...
	ЦеныРассчитаны = ЗапасыСервер.ЗаполнитьЦеныПоВидуЦен(Объект, "Товары", ВидЦены, Истина, Истина, Истина);
...
Функция ЗаполнитьЦеныВыделенныхСтрокПоВидуЦенСервер(ВидЦены)
	...
	ЦеныРассчитаны = ЗапасыСервер.ЗаполнитьЦеныВыделенныхСтрокПоВидуЦен(Объект, "Товары", ВидЦены, Элементы.Товары.ВыделенныеСтроки, Истина, Истина, Истина);
...
Функция ПолучитьСписокАдресовОрганизации(Организация)
...
Процедура ПолучитьТоварыИзХранилища(АдресПлатежейВХранилище)
	...
	ОбработкаТабличнойЧастиТоварыСервер.ЗаполнитьИспользоватьСерийныеНомераВТЧСервер(Объект.Товары);
	ОбработкаТабличнойЧастиТоварыСервер.ЗаполнитьПризнакИспользованияХарактеристик(Объект.Товары);
	ЗапасыСервер.ЗаполнитьЦеныПоВидуЦен(Объект, "Товары", Объект.ВидЦены, Истина, Истина, Истина);
...
Процедура ПерезаполнитьНалогообложениеНДС()
	...
	НовоеНалогообложение = НДСОбщегоНазначенияСервер.ПолучитьИОбработатьДоступностьНалогообложениеНДС(Объект.Организация, Объект.ОрганизацияПолучатель, Объект.Дата, Истина, Элементы, МассивЭлементов);
...
Процедура ВыполнитьАвторасчетНДССервер()
	...
	НДСИсходящийСервер.ПересчитатьНДСсУчетомПогрешностиОкругления(Объект.Товары, Объект.Ссылка, Объект.ЦенаВключаетНДС, ПогрешностиОкругления, "Товары", "грн");

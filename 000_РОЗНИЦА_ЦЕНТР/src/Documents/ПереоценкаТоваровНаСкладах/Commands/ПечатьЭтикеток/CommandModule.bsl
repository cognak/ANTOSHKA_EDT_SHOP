...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если МассивНепроведенныхДокументов.Количество() > 0 Тогда
		...
		Если МассивНепроведенныхДокументов.Количество() = 1 Тогда
			ТекстСообщения = НСтр("ru = 'Документ помечен к удалению! Печать этикеток невозможна.'")
		Иначе
			ТекстСообщения = НСтр("ru = 'Документы помечены к удалению! Печать этикеток невозможна.'")
		КонецЕсли;
		
		ПоказатьОповещениеПользователя("В печати этикеток отказано",, ТекстСообщения, БиблиотекаКартинок.Предупреждение32);
		Возврат;

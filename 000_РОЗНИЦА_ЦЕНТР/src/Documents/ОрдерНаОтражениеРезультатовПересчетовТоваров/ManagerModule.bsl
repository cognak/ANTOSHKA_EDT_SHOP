...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОрдерНаОтражениеРезультатовПересчетовТоваров") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПечатьОтраженияИзлишковИНедостачНаСкладе(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),,,Истина);

	КонецЕсли;
КонецПроцедуры

Функция ПечатьОтраженияИзлишковИНедостачНаСкладе(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	|	ОрдерНаОтражениеРезультатовПересчетовТоваров.Ответственный.ФизЛицо КАК Кладовщик,
	|	ОрдерНаОтражениеРезультатовПересчетовТоваров.Ответственный.ФизЛицо КАК Ответственный,
	...
	СинонимДокумента = НСтр("ru = 'Ордер на отражение результатов пересчетов товаров';uk='Ордер на відбиття результатів перерахувань товарів'", КодЯзыкаПечать);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Макет = УправлениеПечатью.ПолучитьМакет("Документ.ОрдерНаОтражениеРезультатовПересчетовТоваров.ПФ_MXL_ОрдерНаОтражениеРезультатовПересчетов", КодЯзыкаПечать);
		...
		Если ЗначениеЗаполнено(ВыборкаПоДокументам.Приказ) Тогда
			...
			СинонимПриказа = НСтр("ru='Приказ на пересчет товаров';uk='Наказ на перерахунок товарів'", КодЯзыкаПечать);
			РеквизитыДокумента.Номер = ВыборкаПоДокументам.Приказ.Номер;
			РеквизитыДокумента.Дата  = ВыборкаПоДокументам.Приказ.Дата;
			ОбластьПриказ.Параметры.ПриказПредставление = ФормированиеПечатныхФормСервер.СформироватьЗаголовокДокумента(РеквизитыДокумента, СинонимПриказа, КодЯзыкаПечать);

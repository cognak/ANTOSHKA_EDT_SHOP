...
Процедура ЗаполнитьКорректировкиНаСервере()
	...
	Пока Результат.Следующий() Цикл
		...
		Для Каждого Строка_ Из МассивСтрок Цикл
			...
			Сообщить("Внесена корректировка по "+Результат.Сотрудник +"["+Результат.Должность+"] за "+Формат(Результат.ДатаК,"ДФ=д")+" число.");
		КонецЦикла;
...
Процедура ПодписатьИОтправить(Команда)
	Если НЕ Объект.Проведен тогда 
		Ответ = Вопрос("Перед отправкой нужно провести документ. Вы хотите провести его сейчас?",РежимДиалогаВопрос.ДаНет);
		Если Ответ = КодВозвратаДиалога.Нет Тогда
			Возврат;
		КонецЕсли;

...
Процедура ОтметитьУстановитьУволенныхСотрудников()
	Для Каждого Строка_ Из Объект.ДанныеОВремени Цикл
		Строка_.Уволен = Не ОбменWebRetailСервер.ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(Строка_.Сотрудник,Строка_.Должность, Объект.Магазин,Объект.ДатаОкончанияПериода);

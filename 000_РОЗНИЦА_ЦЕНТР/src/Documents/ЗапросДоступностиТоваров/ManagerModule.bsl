...
Процедура СозданиеИзменениеЗапросовДоступности(ДокументСсылкаЗаказКлиента) Экспорт
	...
	СуммаДоставкаЗаказа = ДокументСсылкаЗаказКлиента.СуммаДоставки;
	ДоставкаУчтена = Ложь;

	Если Не (ДокументСсылкаЗаказКлиента.Статус = Перечисления.СтатусыЗаказовПокупателей.Новый
		Или ДокументСсылкаЗаказКлиента.Статус = Перечисления.СтатусыЗаказовПокупателей.ВРаботе
		Или ДокументСсылкаЗаказКлиента.Статус = Перечисления.СтатусыЗаказовПокупателей.Отменён
		Или ДокументСсылкаЗаказКлиента.Статус = Перечисления.СтатусыЗаказовПокупателей.Закрыт) Тогда
		...
		Пока ВыборкаТоварыЗДСсылка.Следующий() Цикл
			...
			Если НовыйЗД И ДокументСсылкаЗаказКлиента.Статус = Перечисления.СтатусыЗаказовПокупателей.Отменён Тогда
				Продолжить;
			КонецЕсли;
				...
				Пока ВыборкаТоварыЗПОператорДоставки.Следующий() Цикл 
					...
					Пока ВыборкаТоварыЗПМагазинПолучатель.Следующий() Цикл 
						...
						Пока Выборка.Следующий() Цикл 
							...
							Если Не (Выборка.ТоварыЗПСамовывоз
								И Выборка.ТоварыЗПТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.Наличные) Тогда
									
									ВыгружаетсяВNavision = Истина;
									ДокументИзменен = Выборка.ИзменилсяТипОплаты;
...
//							КонецЕсли;
						...
						Если ДокументСсылкаЗаказКлиента.Статус = Перечисления.СтатусыЗаказовПокупателей.Отменён

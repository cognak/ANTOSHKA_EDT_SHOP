...
Функция ПечатьЛистаОтбора(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, ИмяМакета)
	...
	|	ЕСТЬNULL(ТаблицаЗаказа.Город = ТаблицаНастроек.Город, ЛОЖЬ) КАК Локал,
	|	ЕСТЬNULL(ТаблицаПолитикМагазина.ИспользоватьЛокалЕкспрес, ЛОЖЬ) КАК ИспользоватьЛокалЕкспрес
	|ИЗ
	|	Документ.ЗапросДоступностиТоваров КАК ТаблицаЗапроса
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
	...
	|	ЗапросДоступностиТоваровТовары.Номенклатура.Марка КАК НоменклатураМарка,
	|	ЗапросДоступностиТоваровТовары.КоличествоТребование КАК КоличествоТребование,
	|	ВТ_ЩтрихКод.Штрихкод КАК Штрихкод,
	|	0 КАК КоличествоНаСкладе,
	|	ЗапросДоступностиТоваровТовары.Характеристика КАК Характеристика,
	|	ЗапросДоступностиТоваровТовары.Номенклатура.IDN КАК НоменклатураIDN
	|ИЗ
	|	Документ.ЗапросДоступностиТоваров.Товары КАК ЗапросДоступностиТоваровТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЩтрихКод КАК ВТ_ЩтрихКод
	|		ПО ЗапросДоступностиТоваровТовары.Номенклатура = ВТ_ЩтрихКод.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатурнаяИнформация КАК НоменклатурнаяИнформация
	|		ПО ЗапросДоступностиТоваровТовары.Номенклатура = НоменклатурнаяИнформация.Номенклатура
	|ГДЕ
	|	ЗапросДоступностиТоваровТовары.Ссылка В (&МассивОбъектов)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ВыборкаПоДокументам.Локал И ВыборкаПоДокументам.ИспользоватьЛокалЕкспрес Тогда
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Пока ВыборкаШтрихКод.Следующий() Цикл
				...
				Если ПерваяВыборка Тогда
					...
					СтруктураКодаКартинки = Новый Структура("sku", ВыборкаШтрихКод.НоменклатураIDN);
					
					ДанныеОтвета = ОбменMagentoСервер.ПолучитьДанныеНоменклатуры(СтруктураКодаКартинки);

...
Функция СтикерДляВнутреннейЛогистики(ДокументДляСтикера) Экспорт
	...
	Запрос.УстановитьПараметр("Владелец", ДокументДляСтикера);
	...
	Иначе
		...
		ШтрихКод = ДокументДляСтикера.Номер+Формат(Год(ТекущаяДатаСеанса()),"ЧГ=0");
		НаборЗаписей.Отбор.Штрихкод.Установить(ШтрихКод);
		НаборЗаписей.Отбор.Владелец.Установить(ДокументДляСтикера); 
		...
		НоваяЗапись.Владелец = ДокументДляСтикера; 
	...
	Область.Параметры.ГородДоставки = СокрЛП(ДокументДляСтикера.МагазинПолучатель.СкладПродажи.IDN);
	Область.Параметры.НомерЗапроса = СокрЛП(ДокументДляСтикера.Номер);
	...
	КоличествоМест = ДокументДляСтикера.Упаковки.Количество();

...
// Создание изменение запросов доступности.
// 
// Параметры:
//  ДокументСсылкаЗаказКлиента - ДокументСсылка.ЗаказПокупателя - Документ ссылка заказ клиента
Процедура СозданиеИзменениеЗапросовДоступности(ДокументСсылкаЗаказКлиента) Экспорт
	...
	Если Не (ДокЗаказа.Статус = Перечисления.СтатусыЗаказовПокупателей.Новый
		...
		Запрос.Текст = 
			...
			|	ЗаказПокупателяТовары.Ссылка.ТипОплаты КАК ТипОплаты,
			|	ЗаказПокупателяТовары.СуммаДоставки КАК СуммаДоставкиТовара,
			|	ЗаказПокупателяТовары.Ссылка.СтатусОплаты КАК СтатусОплаты
			|ПОМЕСТИТЬ ТоварыЗП
			|ИЗ
			...
			|	ЗапросДоступностиТоваровТовары.Ссылка.ТипОплаты КАК ТипОплаты,
			|	ЗапросДоступностиТоваровТовары.Ссылка.СтатусОплаты КАК СтатусОплаты
			|ПОМЕСТИТЬ ТоварыЗД
			|ИЗ
			...
			|	ТоварыЗП.СтатусОплаты КАК ТоварыЗПСтатусОплаты,
			|	ВЫБОР
			|		КОГДА ТоварыЗД.СтатусОплаты ЕСТЬ NULL
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ
			|		НЕ ТоварыЗД.СтатусОплаты = ТоварыЗП.СтатусОплаты
			|	КОНЕЦ КАК ИзменилсяСтатусОплаты
			|ИЗ
				...
				Пока ВыборкаТоварыЗПОператорДоставки.Следующий() Цикл 
					...
					Пока ВыборкаТоварыЗПМагазинПолучатель.Следующий() Цикл 
						...
						Пока Выборка.Следующий() Цикл 
							...
							Если Не Выборка.ТоварыЗПСтатусОплаты = ДокументОбъект.СтатусОплаты Тогда
								
								ДокументОбъект.СтатусОплаты = Выборка.ТоварыЗПСтатусОплаты;
								ДокументИзменен = Истина;
								
							КонецЕсли;
							
							Если ДокументОбъект.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ПродажаБезнал
							...
							ИначеЕсли ДокументОбъект.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ПродажаНаКассе
									И Не ДокументОбъект.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.Наличные Тогда 
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЛистОтбораА4") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ЛистОтбораА4", 
			"Лист отбора", 
			ПечатьЛистаОтбора(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, "ЛистОтбораА4"),,,Истина);
		
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЛистОтбораА5") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ЛистОтбораА5", 
			"Лист отбора", 
			ПечатьЛистаОтбора(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, "ЛистОтбораА5"),,,Истина);
...
Функция ПечатьТоварнаяНакладная(МассивОбъектов, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	|	ТаблицаЗапроса.СуммаОплатыДоставки КАК Доставка,
...
Функция СтикерДляВнутреннейЛогистики(ДокументДляСтикера) Экспорт
	...
	Запрос.УстановитьПараметр("Владелец", ДокументДляСтикера.СсылкаНаЗапрос);
	...
	Иначе
		...
		ШтрихКод = ДокументДляСтикера.НомерЗапроса+Формат(Год(ТекущаяДатаСеанса()),"ЧГ=0");
		НаборЗаписей.Отбор.Штрихкод.Установить(ШтрихКод);
		НаборЗаписей.Отбор.Владелец.Установить(ДокументДляСтикера.СсылкаНаЗапрос); 
		...
		НоваяЗапись.Владелец = ДокументДляСтикера.СсылкаНаЗапрос; 
    ...
    // Получаем макет по его имени
    Макет = УправлениеПечатью.ПолучитьМакет("Документ.ЗапросДоступностиТоваров.СтикерВнутреннейЛогистики");

    // Дальнейшая работа с макетом...	

	// 1. Создаем табличный документ
	ТабДок = Новый ТабличныйДокумент;

	// 2. Устанавливаем нестандартный размер страницы (Custom) и задаем его размеры в мм
	ТабДок.РазмерСтраницы = "Custom"; 
	ТабДок.ШиринаСтраницы = 100;
	ТабДок.ВысотаСтраницы = 100;

	// 3. Убираем отступы (поля) для экономии места на стикере
	ТабДок.ПолеСлева = 0;
	ТабДок.ПолеСправа = 0;
	ТабДок.ПолеСверху = 0;
	ТабДок.ПолеСнизу = 0;
	
	// 4. Заполняем макет данными (например, берем ваш подготовленный макет)
	// Макет = ПолучитьМакет("МакетСтикеров");
	Область = Макет.ПолучитьОбласть("СтрокаСтикера"); 
	Область.Параметры.ГородДоставки = СокрЛП(ДокументДляСтикера.СсылкаНаЗапрос.МагазинПолучатель.СкладПродажи.IDN);
	Область.Параметры.НомерЗапроса = СокрЛП(ДокументДляСтикера.НомерЗапроса);
	Область.Параметры.НомераПеремещений = СокрЛП(ДокументДляСтикера.НомераПеремещений);
	Область.Параметры.ДатаСоздания = ТекущаяДатаСеанса();
	...
	КоличествоМест = ДокументДляСтикера.СсылкаНаЗапрос.Упаковки.Количество();
	
	Для НомерМеста = 1 По КоличествоМест Цикл
		
			Область.Параметры.Места = СтрШаблон("%1 из %2 мест", НомерМеста, КоличествоМест);
			ТабДок.Вывести(Область);
			
			Если НомерМеста < КоличествоМест Тогда
...
//
Функция ПечатьЛистаОтбора(МассивОбъектов, ОбъектыПечати, ПараметрыВывода, ИмяМакета)
	...
	|	ТаблицаЗапроса.Дата КАК ДатаЗапроса,
	|	ТаблицаЗапроса.ТипДоставки КАК ТипДоставки,
	|	ТаблицаЗапроса.ДокументОснование.Номер КАК Номер,
	|	ТаблицаЗапроса.ДокументОснование.Дата КАК Дата,
	|	ТаблицаЗапроса.Комментарий КАК Комментарий,
	|	Штрихкоды.Штрихкод КАК Штрихкод,
	|	ТаблицаЗапроса.МагазинПолучатель = ТаблицаПолитики.ИнтернетМагазин КАК ПолучательИМ,
	|	ТаблицаЗапроса.МагазинПолучатель.Представление КАК МагазинПолучательПредставление,
	|	ЕСТЬNULL(ТаблицаЗаказа.Город = ТаблицаНастроек.Город, ЛОЖЬ)
	|	И ЕСТЬNULL(ТаблицаПолитикМагазина.ИспользоватьЛокалЕкспрес, ЛОЖЬ) КАК ИспользоватьЛокалЕкспрес
	|ИЗ
	|	Документ.ЗапросДоступностиТоваров КАК ТаблицаЗапроса
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
	|		ПО Штрихкоды.Владелец = ТаблицаЗапроса.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитика.СрезПоследних(&ДатаДокумента,) КАК ТаблицаПолитики
	|		ПО ИСТИНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК ТаблицаЗаказа
	|		ПО ТаблицаЗапроса.ДокументОснование = ТаблицаЗаказа.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиСлужбДоставкиМагазинов КАК ТаблицаНастроек
	|		ПО ТаблицаЗапроса.МагазинОтправитель = ТаблицаНастроек.Магазин
	|		И ТаблицаЗапроса.ОператорДоставки = ТаблицаНастроек.ОператорДоставки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаМагазинов.СрезПоследних(&ДатаДокумента,) КАК ТаблицаПолитикМагазина
	|		ПО ТаблицаЗапроса.МагазинОтправитель = ТаблицаПолитикМагазина.Магазин
	|ГДЕ
	|	ТаблицаЗапроса.Ссылка В (&МассивОбъектов)
	|	И ТаблицаЗапроса.Проведен
	|
	...
	|		ПО ЗапросДоступностиТоваровТовары.Номенклатура = ВТ_ШтрихКод.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатурнаяИнформация КАК НоменклатурнаяИнформация
	|		ПО ЗапросДоступностиТоваровТовары.Номенклатура = НоменклатурнаяИнформация.Номенклатура
	|ГДЕ
	|	ЗапросДоступностиТоваровТовары.Ссылка В (&МассивОбъектов)
	...
	|	ВТ_ШтрихКод.Номенклатура КАК Номенклатура
	|ИЗ
	|	ВТ_ШтрихКод КАК ВТ_ШтрихКод";
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Документ.ЗапросДоступностиТоваров." + СокрЛП(ИмяМакета), КодЯзыкаПечать);
	
	ОбластьЗаголовок 		= Макет.ПолучитьОбласть("Заголовок");
	ОбластьШапка 			= Макет.ПолучитьОбласть("Шапка");
	ОбластьШапкаЛокал		= Макет.ПолучитьОбласть("ШапкаЛокал");
	...
	ТаблицаНоменклатуры = Результаты[3].Выгрузить();
	ТаблицаНоменклатуры.Индексы.Добавить("Номенклатура");
	МассивНоменклатуры = общегоназначения.ВыгрузитьКолонку(ТаблицаНоменклатуры, "Номенклатура", Истина);
	
	КартинкиНоменклатуры = ЗаказыПокупателейСервер.ПолучитьСоответствиеКартинокТовара(МассивНоменклатуры, Истина);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ВыборкаПоДокументам.ИспользоватьЛокалЕкспрес Тогда
			ТабличныйДокумент.Вывести(ОбластьШапкаЛокал);
		КонецЕсли;
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			ВыборкаШтрихКод = ВыборкаПоСтрокамТЧ.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			ПерваяВыборка = Истина;
			ТекстШрихКода = "";
			
			Пока ВыборкаШтрихКод.Следующий() Цикл
				
				Если ПерваяВыборка Тогда

					НомерВМассиве = МассивКомметариев.Найти(СокрЛП(ВыборкаШтрихКод.НоменклатураКод));
					...
					ОбластьСтрокаТаблицы.Параметры.Заполнить(ВыборкаШтрихКод);
					...
					Если ТипЗнч(ВыборкаПоДокументам.Ссылка.МагазинОтправитель) = Тип("СправочникСсылка.Магазины") Тогда
						...
						Фильтр.Вставить("Номенклатура",ВыборкаШтрихКод.Номенклатура);
						Фильтр.Вставить("Характеристика",ВыборкаШтрихКод.Характеристика);
					...
					КартинкаНоменклатуры = КартинкиНоменклатуры.Получить(ВыборкаШтрихКод.Номенклатура);
					
					Если КартинкаНоменклатуры = Неопределено Тогда
						
						Картинка = Новый Картинка();

					Иначе
						
						Картинка = Новый Картинка(КартинкаНоменклатуры);
				...
				ТекстШрихКода = ТекстШрихКода + СокрЛП(ВыборкаШтрихКод.ШтрихКод) + Символы.ПС;
				
			КонецЦикла;
			
			ОбластьСтрокаТаблицы.Параметры.Штрихкод = ТекстШрихКода;

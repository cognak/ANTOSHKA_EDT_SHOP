...
Процедура НапечататьРеестр(Команда)
	...
	РеестрНПСтруктура = НапечататьРеестрНаСервере(ТекущийРеестр);
...
Процедура ДобавитьТЗСтатусовЗапросаДоступности(ТЗСтатусов, Статус)
	...
	СтрокаСтатусов = СтатусыЗапросовДоступности.НайтиСтроки(Новый Структура("СтатусЗапросаПеречисление", Статус));

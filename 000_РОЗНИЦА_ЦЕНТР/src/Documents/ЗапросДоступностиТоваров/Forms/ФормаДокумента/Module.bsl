...
Процедура ТТННаСервере()
	Успех =	ОбменНПСервер.РегистрацияЕН(Объект.Ссылка);	// Вставить содержимое обработчика.
	Если	Успех тогда
		Сообщить("Создана ЭН");
	Иначе
...
Функция ВыполнитьПроверкуОбязательныхРеквезитов() 
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) тогда   
		...
		Если НЕ ПроверитьИмяФизЛица(Имя) Тогда
			Сообщить("Имя «" + Имя + "» НЕ найдено в справочнике имён людей.");
			Имя = "";
	        Возврат Ложь; 
		КонецЕсли;    
...
Функция ПолучитьСтатус() 
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.СтатусыТТН.СрезПоследних(, ДокументРегистратор = &ДокументРегистратор) КАК СтатусыТТНСрезПоследних
		|
		|УПОРЯДОЧИТЬ ПО
		|	Период УБЫВ";

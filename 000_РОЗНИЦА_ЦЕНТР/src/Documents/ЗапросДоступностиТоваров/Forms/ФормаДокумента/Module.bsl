...
Процедура ОтредактироватьСтрокуЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.ИзменилиКоличество Тогда
			...
			Если Не ПустаяСтрока(НайденнаяСтрока[0].Примечание) Тогда
				
				ТекстИзменения = НайденнаяСтрока[0].Примечание + Символы.ПС + "--------------------" + Символы.ПС; 

			КонецЕсли;
			
			ТекстИзменения = ТекстИзменения + "Изменено количество: было - " + НайденнаяСтрока[0].Количество
		...
		ИначеЕсли Не ПустаяСтрока(Результат.СообщениеМагазину) Тогда
			...
			ТекстИзменения = ТекстИзменения + Результат.СообщениеМагазину;

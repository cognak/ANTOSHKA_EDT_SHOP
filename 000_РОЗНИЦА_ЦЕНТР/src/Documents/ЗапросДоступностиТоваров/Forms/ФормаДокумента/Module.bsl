...
//	Обработчик механизма "ВерсионированиеОбъектов"
	...
	НазначениеСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.МагазинОтправитель, "НазначениеСклада", Перечисления.НазначенияСкладов.ПустаяСсылка(), Ложь);
	
	ПополнениеМагазина = ((НазначениеСклада = Перечисления.НазначенияСкладов.УправляющаяСистема)
			И Не (УчетнаяПолитика.ИнтернетМагазин = Объект.МагазинПолучатель))
		Или УчетнаяПолитика.СкладЗакупки = Объект.МагазинОтправитель;
...
Функция ОтобразитьОбсуждениеНаСервере(ТекстОбсуждения, ДокСсылка)
	...
	Пока ВыборкаСообщений.Следующий() Цикл
		...
		ТекстРасшифровки = 	"> " + ВыборкаСообщений.Автор + " " + ВыборкаСообщений.Дата + Символы.ПС + 
							"> " + ВыборкаСообщений.УзелСоздания + Символы.ПС + 					
...
Процедура ПометитьПрочтение()
	...
	Запрос.УстановитьПараметр("ДокументОснование", 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ДокументОснование"));

...
//	Обработчик механизма "ВерсионированиеОбъектов"
	...
	Элементы.ФормаПовторноОтправитьВNAV.Видимость = ТехническаяПоддержкаВызовСервера.ИсключительныйРежим();
...
Процедура ОтредактироватьСтрокуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.ИзменилиКоличество Тогда
			...
			СтруктураТекстИзменения.Вставить("Дата", ОбщегоНазначенияВызовСервера.ТекущаяДатаСервера());
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Элементы.ДекорацияЛокал.Видимость = ВидимостьЛокал();

КонецПроцедуры

&НаСервере
Функция ВидимостьЛокал()
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ТаблицаЗапроса.Ссылка = &МассивОбъектов";
	
	Запрос.УстановитьПараметр("МассивОбъектов", Объект.Ссылка);
	Запрос.УстановитьПараметр("ДатаДокумента", Объект.Дата);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		Результат = Выборка.ИспользоватьЛокалЕкспрес;
	КонецЕсли;
...
Функция ПроверитьАдрес()
	Город = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Город");
	Улица = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Улица");
	Дом = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Дом");
	Квартира = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Квартира");
...
Функция ВыполнитьПроверкуОбязательныхРеквезитов()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		Если ВГХНеЗаполнены Тогда
			...
			Возврат Ложь;
		КонецЕсли;
		Если Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "ДоставкаНаАдрес") Тогда
			Отделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Отделение");
			Если Отделение = "" Или Отделение = Справочники.Отделения.ПустаяСсылка() Или Отделение
				...
				Возврат Ложь;
			КонецЕсли;
			ОбъемныйВесПревышен = ОбменНПСервер.ПроверитьОбъемныйВес(Объект.ДокументОснование, Отделение);
		...
		КонецЕсли;
		Имя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Имя");
		Если ПустаяСтрока(Имя) Тогда
			Сообщить("Не заполнено имя");
			Возврат Ложь;
		КонецЕсли;

		Фамилия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Фамилия");
		Если ПустаяСтрока(Фамилия) Тогда
			Сообщить("Не заполнена фамилия!");
			Возврат Ложь;
		КонецЕсли;

		Телефон = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Телефон");
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Элемент.ТекущийЭлемент = Элементы.ТоварыНоменклатураКод Тогда
		Если ПодключитьРасширениеРаботыСФайлами() Тогда
			...
			ЗапуститьПриложение("https://antoshka.ua/uk/catalogsearch/result/?q="

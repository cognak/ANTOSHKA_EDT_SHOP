...
// 
// Параметры:
//  СписокОшибок - Строка - Список ошибок
// 
// Возвращаемое значение:
//  Булево - Проверить заполнение документа
...
//			КонецЕсли;   
	...
	ИначеЕсли СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.КОтправке
			И ОператорДоставки = Перечисления.ОператорыДоставки.НоваяПочта Тогда 
	...
	ДокументОснованиеРеквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "Статус, ТипОплаты, ДокументОснование");
	
	Если ДокументОснованиеРеквизиты.Статус = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности
			И ДокументОснованиеРеквизиты.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.Наличные
			И Не ОператорДоставки = Перечисления.ОператорыДоставки.ВнутренняяЛогистика
			И Не ЗначениеЗаполнено(ДокументОснованиеРеквизиты.ДокументОснование) Тогда
	...
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда
		...
		Если ДополнительныеСвойства.Свойство("ЗаписатьСтароеКоличество") Тогда
			...
			Для Каждого СтрокаТовара Из Товары Цикл
				...
				Если Не (СтрокаТовара.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваров.Отменён
...
Процедура ЗаписатьОтправкуВNavisionВРегистр()
	...
	Если СтатусОтправкиЗапроса = Перечисления.СтатусыЗапросаДоступностиВNavision.ПустаяСсылка()
		...
		Если ДополнительныеСвойства.Свойство("ОтправлятьВNavision")
				И ДополнительныеСвойства.Свойство("ПовторнаяОтправкаВNavision") Тогда
		...
		ИначеЕсли СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ЧекВКЦ
...
Процедура ЗаписьКлючевыхАтребутовЗаказаКлиента(ЗаказПокупателя) 
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.КлючевыеАтрибутыЗаказовПокупателей.СрезПоследних(, ЗаказПокупателя = &ЗаказПокупателя) КАК

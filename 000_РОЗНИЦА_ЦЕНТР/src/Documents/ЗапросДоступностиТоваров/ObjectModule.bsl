...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если (ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПустаяСсылка()
				Или ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПередатьВNavision)
			И ((СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ПродажаБезнал
					И ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз)
		...
		ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПроверкаВозможностиЗакрытияВNavision;
...
//			КонецЕсли;   
	...
	Если ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПустаяСсылка()
			Или ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПередатьВNavision Тогда
		...
		Иначе 
			ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Не (СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Отменён 
		Или СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ПодтвердитьОтмену
...
Процедура ПриЗаписи(Отказ) 
	...
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда
		...
		Если Не ПустаяСтрока(ОтветОтправкиПочты) Тогда

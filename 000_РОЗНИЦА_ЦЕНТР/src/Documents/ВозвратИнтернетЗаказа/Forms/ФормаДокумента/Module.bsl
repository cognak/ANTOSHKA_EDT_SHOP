...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ВидВозврата.ТолькоПросмотр = ЗначениеЗаполнено(Объект.ВидВозврата) И Не ИсключительныйРежим;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ВозвратДоставки Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ВозвратИнтернетЗаказа.ВозвратДоставки
			|ИЗ
			|	Документ.ВозвратИнтернетЗаказа КАК ВозвратИнтернетЗаказа
			|ГДЕ
			|	ВозвратИнтернетЗаказа.ВозвратДоставки
			|	И НЕ ВозвратИнтернетЗаказа.Ссылка = &Ссылка
			|	И ВозвратИнтернетЗаказа.ДокументОснование = &ДокументОснование
			|	И НЕ ВозвратИнтернетЗаказа.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВозвратаЗаказаПокупателя.Отменён)";
		
		Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
		Запрос.УстановитьПараметр("ДокументОснование", Объект.ДокументОснование);
		
		Если Не Запрос.Выполнить().Пустой() Тогда
			ТекстОшибки = НСтр("ru='Доставка уже была возвращена'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстОшибки,
				Объект,
				"ВозвратДоставки",
...
Процедура ВидВозвратаПриИзменении(Элемент)
	...
	Если Объект.ВидВозврата = ПредопределенноеЗначение("Перечисление.ВидыВозвратаИнтернетЗаказа.Обмен") Тогда
		Объект.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.Оплачен");
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура ВозвратДоставкиПриИзменении(Элемент)
	ОтображениеЭлементов();
КонецПроцедуры
#КонецОбласти
...
&НаКлиенте
Процедура ОтменаВозврата(Команда)
	
	Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыВозвратаЗаказаПокупателя.Отменён");
	ОтображениеЭлементов();
...
Процедура ВозвратБезналичнойОплаты(Команда)
	...
	Иначе
		...
		СуммаВозврата = Объект.СуммаДокумента + ?(Объект.ВозвратДоставки, Объект.СуммаДоставки, 0);
		...
		Иначе
			...
			ДанныеОтвета = "";
			ИДОплаты = Объект.ИДОплаты;
			Объект.ИДВозврата = ОбменСБанкамиВызовСервера.ВозвратБезналичнойОплатыИнтернетЗаказа(СуммаВозврата, ИДОплаты, ДанныеОтвета);
			ЗаписатьКомментарий(ДанныеОтвета.ТекстОтвета);
...
Процедура ОтображениеЭлементов()
	...
	Элементы.ОтменаВозврата.Видимость = НовыйСтатус;
	...
	Элементы.СуммаДоставки.Видимость = Объект.ВозвратДоставки;
    ...
    Объект.СуммаДокумента = Объект.Товары.Итог("Сумма") + ?(Объект.ВозвратДоставки, Объект.СуммаДоставки, 0);
    Объект.СуммаБонусныхБалловСписано = Объект.Товары.Итог("СуммаБонусныхБалловСписано");
    
	ОплатаМоноБанк = (Объект.ТипОплаты = ПредопределенноеЗначение("Перечисление.ТипОплатыЗаказПокупателя.monobank"));
	ВовзратНеОплачен = (Объект.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.НеОплачен"));
	Элементы.ВозвратБезналичнойОплаты.Видимость = ОплатаМоноБанк И ВовзратНеОплачен И (ЯвляетсяСуперВайзером Или ИсключительныйРежим);
	Элементы.СтатусОплаты.Доступность = ИсключительныйРежим Или Не ОплатаМоноБанк;
	Элементы.ГруппаОплатаМоно.Видимость = ОплатаМоноБанк;
	Элементы.ВозвратДоставки.Доступность = ВовзратНеОплачен;
	Элементы.СуммаДоставки.Доступность = ВовзратНеОплачен;
...
Процедура ПослеОтметкиЭлементов(Элементы, Параметры) Экспорт
    ...
    Объект.СуммаДоставки = Объект.Товары.Итог("СуммаДоставки");
...
Процедура ЗавершениеВводаНомерЭН(Результат, Параметры) Экспорт 
	...
	Если ТипЗнч(Результат) = Тип("ДокументСсылка.ЭлектроннаяНакладная") И Не Результат.Пустая() Тогда
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЭлектроннаяНакладная, "ВозвратОтКлиента") Тогда
			
			ЗарегистрироватьОбъект(Объект.Ссылка);
...
Процедура ЗарегистрироватьОбъект(ОбъектДляРегистрации)
	
	ПланыОбмена.ОбменСЕСпутником.ЗарегистрироватьОбъект(ОбъектДляРегистрации);
...
&НаСервере
Процедура ЗаписатьКомментарий(ТекстСообщения)

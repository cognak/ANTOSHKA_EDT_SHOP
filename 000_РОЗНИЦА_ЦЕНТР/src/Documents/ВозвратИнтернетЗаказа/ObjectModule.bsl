...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЭлектроннаяНакладная") Тогда
		...
		Запрос.Текст =
			...
			|	ТаблицаОплаты.ИДОплаты КАК ИДОплаты
			|ИЗ
			|	Документ.ЗаказПокупателя КАК ТаблицаЗаказа
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДляВозврата КАК ДанныеДляВозврата
			|		ПО ДанныеДляВозврата.ЗаказПокупателя = ТаблицаЗаказа.Ссылка
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.ОплатаЗаказа КАК ТаблицаОплаты
			|		ПО ТаблицаОплаты.Ссылка = ТаблицаЗаказа.Ссылка
			|		И НЕ ТаблицаОплаты.Отменён
			|		И ТаблицаОплаты.ВидОплаты = ЗНАЧЕНИЕ(Перечисление.ВидыОплатыЗаказаПокупателя.Оплата)
			|		И ТаблицаОплаты.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипОплатыЗаказПокупателя.monobank)";
	...
	Для Каждого СтрокаВозврата Из Товары Цикл
		...
		СтрокаВозврата.КоличествоВЗаказе = СтрокаВозврата.КоличествоУпаковок;
	...
	Если Статус = Перечисления.СтатусыВозвратаЗаказаПокупателя.Чек
		...
		Если Не УчетныеДанныеОтправителя.Пустая() Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				...
				Адрес = "im@antoshka.ua; myhailo.avramenko@antoshka.ua";

				ПараметрыПисьма = Новый Структура;
				ПараметрыПисьма.Вставить("Кому", Адрес);
				ПараметрыПисьма.Вставить("Тема", "ТТН возврата #"
								 + СокрЛП(НомерТТН));
				ТелоПисьма = "Поступила ТТН возврата #"
					+ СокрЛП(НомерТТН)
					+ " на возврат #"
					+ СокрЛП(Номер)
					+ " по заказу #"

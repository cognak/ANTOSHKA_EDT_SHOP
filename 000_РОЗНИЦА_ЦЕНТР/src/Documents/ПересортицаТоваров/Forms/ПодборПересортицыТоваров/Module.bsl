...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗакрытиеПриСозданииПересортицы Тогда
		...
		Если ТаблицаВыбора.Количество() > 0 Тогда
			...
			ТекстВопроса = НСтр("ru = 'Закрыть форму без создания пересортицы товаров?'"); 
...
Процедура ЗаполнитьПараметрыЗапроса(Запрос, МассивПараметров, НомерПозицииНаименованиеВключение)
	...
	Для каждого СтрокаПараметра Из МассивПараметров Цикл
		...
		ИначеЕсли СтрокаПараметра = НСтр("ru = 'Принадлежность к одной группе номенклатуры'") Тогда
			...
			|			ТОГДА " + Формат(НомерУпорядочивания, "ЧДЦ=0; ЧГ=0");
			НомерУпорядочивания = НомерУпорядочивания + 1;
		ИначеЕсли СтрокаПараметра = НСтр("ru = 'Принадлежность к одной номенклатурной группе'") Тогда
			УсловиеСортировки = УсловиеСортировки + "
			|		КОГДА ПолнаяТаблица.НоменклатурнаяГруппаОприходование = ПолнаяТаблица.НоменклатурнаяГруппаСписание
			|			ТОГДА " + Формат(НомерУпорядочивания, "ЧДЦ=0; ЧГ=0");
			НомерУпорядочивания = НомерУпорядочивания + 1;
...
Процедура ЗаполнитьТаблицуСортировкиСервер()
	...
	|	ТаблицаОприходование.Номенклатура.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	...
	|	ТаблицаСписание.Номенклатура.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	...
	|	ТаблицаОприходованиеСРеквизитами.НоменклатурнаяГруппа КАК НоменклатурнаяГруппаОприходование,
	|	ТаблицаСписаниеСРеквизитами.НоменклатурнаяГруппа КАК НоменклатурнаяГруппаСписание,
...
Процедура ЗаполнитьТекущейТаблицыВыбораСервер()
	...
	ТекстТекущегоПоложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обработно №%1 из %2 позиций'"),

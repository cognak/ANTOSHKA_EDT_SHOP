...
//	LNK 13.02.2024 05:45:23
	ПодключитьОбработчикОжидания("ПроверитьСоответствиеIDN", 1, Истина);
...
&НаКлиенте	//	LNK 13.02.2024 05:43:06
Процедура ПроверитьСоответствиеIDN()

	ПроверитьСоответствиеIDN_Сервер(Истина);

КонецПроцедуры

&НаСервере	//	LNK 13.02.2024 05:45:10
Процедура ПроверитьСоответствиеIDN_Сервер(НеЗаписывать = Ложь)

	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ТаблицаОснования.IDN КАК IDN,
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если НЕ Объект.Номер = Выборка.Номер ИЛИ НЕ Объект.IDN = Выборка.IDN Тогда

			Объект.Номер = Выборка.Номер;
			Объект.IDN	 = Выборка.IDN;

			Если НеЗаписывать = Истина Тогда
...
//	Оформление элементов в обработчике подсистемы "NAV"
	...
	Если ПровестиИЗакрыть И ЭтотОбъект["РегистрацияПередачиВNavision"] Тогда
		...
		ПроверитьСоответствиеIDN_Сервер(Истина);	//	LNK 13.02.2024 06:18:41

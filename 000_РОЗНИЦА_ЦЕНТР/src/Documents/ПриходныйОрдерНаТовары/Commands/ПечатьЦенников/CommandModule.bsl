...
Функция ПодготовитьСтруктуруДанных(МассивДокументов)
	...
	|ИЗ
	|	Документ.ПриходныйОрдерНаТовары КАК Шапка
	|ГДЕ
	...
	|ГДЕ
	|	ТаблицаДокументы.Ссылка В(&МассивДокументов)"
	);
...
//
&НаКлиенте	//	LNK 08.11.2017 13:17:17	- разрешаем печатать не только проведенные, а все.
...
//   Адрес   - адрес структуры данных во временном хранилище
//
Функция ПодготовитьСтруктуруДанных_ТИПОВАЯ(МассивДокументов, МассивНепроведенныхДокументов)
	...
	|ГДЕ
	|	ТаблицаДокументы.Ссылка В(&МассивДокументов)
	|	И НЕ ТаблицаДокументы.Ссылка В (&МассивНепроведенныхДокументов)"
	);
...
//
&НаКлиенте
Процедура ОбработкаКоманды_ТИПОВАЯ(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если ПараметрКоманды.Количество() - МассивНепроведенныхДокументов.Количество() > 0 Тогда
		...
		АдресВХранилище = ПодготовитьСтруктуруДанных_ТИПОВАЯ(ПараметрКоманды, МассивНепроведенныхДокументов);

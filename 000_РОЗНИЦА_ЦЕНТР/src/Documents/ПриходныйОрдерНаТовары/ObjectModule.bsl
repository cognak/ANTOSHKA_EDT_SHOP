...
//	Заполняет ТЧ Товары, товарами к поступлению.
//	Вызывается из модуля формы и модуля объекта
//
Процедура ЗаполнитьТоварыПоТоварамКПоступлению(ВидЗаполнения) Экспорт
	...
	//	для док.основания ПеремещениеТоваров и ПоступлениеТоваров.
		...
		|ВЫБРАТЬ
		|	МИНИМУМ(ВремДокТЧ.НомерСтроки) КАК НомерСтроки,
		|	ВремДокТЧ.Номенклатура КАК Номенклатура,
		|	ВремДокТЧ.Характеристика КАК Характеристика
		|ПОМЕСТИТЬ ДокТЧ
		|ИЗ
		|	ВремДокТЧ КАК ВремДокТЧ
		|
		...
		|	СУММА(ВЫБОР
		|			КОГДА &ЗаполнятьКоличество
		|				ТОГДА ТоварыКПоступлению.КоличествоОборот
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК Количество
		|ПОМЕСТИТЬ ТоварыКПоступлению
		|ИЗ
		|	РегистрНакопления.ТоварыКПоступлению.Обороты(, , Регистратор, ДокументОснование = &ДокументОснование) КАК ТоварыКПоступлению
		|ГДЕ
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(ТоварыКПоступлению.КоличествоОборот) > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	Товары.КоличествоОснования КАК КоличествоОснования,
		|	ВЫБОР
		...
		Иначе
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрЗаменить(
					"Немає даних для заповнення на підставі «%ДокументОснование%»."
					, "%ДокументОснование%"
					, ДокументОснование
				)
				, ЭтотОбъект
...
//	LNK 16.02.2017 14:33:05 - здесь ИсключительныйРежим будет отлючен для бесправного пользователя
	...
	РеквизитыСсылки = Документы.ПриходныйОрдерНаТовары.РеквизитыСсылки(Ссылка);	//	LNK 04.06.2019 10:48:22
	...
	ДополнительныеСвойства.Вставить("АтрибутыОснования", Новый Структура(
		"Дата, Номер, Проведен, Склад, ВидПеремещения, ЭтоПеремещение, ПереданNAV"
			...
			, РеквизитыСсылки.ПереданNAV_Основание
...
//	LNK 14.05.2024 17:01:23
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения ИЛИ НЕ Проведен Тогда

		Если РеквизитыСсылки.Проведен = Истина И РеквизитыСсылки.ПереданNAV = Истина Тогда

		//	Безусловный отказ.
		//	Делаем безо всяких обходов и калиток во избежание "левых" действий
		//	каких-то неизвестных процессов.
		//	Концепция: "Хороший стук наружу вылезет".
			Отказ = Истина;

			Сообщить("Отказано!" + Символы.ПС + "Невозможно отменить проведение отправленного в NAV ордера.");
...
//	NAV:БлокировкаДанныхУчестьДокументВNavision
	Если НЕ ЭтоНовый() Тогда
		...
		Если ВнешниеИсточникиСобытия.ПередачаNavision(Ссылка) Тогда
			...
			Если ПустаяСтрока(ДополнительныеСвойства.IDN) И НЕ ПустаяСтрока(IDN) Тогда	//	LNK 07.05.2024 05:54:40

			//	это установка IDN по перемещению. Пропускаем.

			ИначеЕсли ДополнительныеСвойства.ИсключительныйРежимВключен ИЛИ ОбщегоНазначенияКлиентСервер.ЗначениеКлючаСтруктуры(ДополнительныеСвойства, "СлужебнаяЗапись", Ложь) Тогда
...
//	NAV:БлокировкаДанныхУчестьДокументВNavision
	Если ОбщегоНазначенияКлиентСервер.ЗначениеКлючаСтруктуры(ДополнительныеСвойства, "РегистрацияПередачиВNavision", Ложь) Тогда
		...
		ПринудительноПриОтладке = Ложь;	//	LNK 23.04.2024 09:02:16

		Если КлючевыеСвойства.СозданВ1СРозница = Истина И НЕ ВнешниеИсточникиСобытия.ПередачаNavision(ДокументОснование)
		ИЛИ	 ПринудительноПриОтладке = Истина
		Тогда
	...
	ИначеЕсли ВнешниеИсточникиСобытия.ПередачаNavision(Ссылка) Тогда	//	LNK 28.12.2023 05:50:40
...
Процедура УстановитьДатуПеремещенияПереброскиПриНеобходимости(Отказ)	//	LNK 29.03.2021 06:19:30
	...
	Если НЕ (МесяцСоответствует()
		...
		Попытка
			...
			ДокументОбъект.ДополнительныеСвойства.Вставить("ФлагИсключенияРекурсии", Истина);

...
Процедура ЗаписатьИЗакрытьВыполнить()
	...
	Параметры.ПараметрыНастройки.Добавить(Истина	, "МногострочныйДисплей");	
...
Функция ПолучитьНастройки()
	...
	ПараметрыДрайвера.Вставить("МногострочныйДисплей", Истина);
...
&НаКлиенте
Процедура КомандаУстановитьДисплейПокупателя(Команда)
	СсылкаНаФайл = МенеджерОборудованияСервер.ПолучитьМакетССервера("МониторПокупателяRedHead");
	ИмяФайла = КаталогВременныхФайлов()+"\ClientMonitor_inst.exe";
    ВременныйКаталог = КаталогВременныхФайлов();
	// Распаковка архива дистрибутива во временный каталог
	Результат = ПолучитьФайл(СсылкаНаФайл, ИмяФайла, Ложь);

	// Запуск инсталлятора
	ЗапуститьПриложение(ИмяФайла, ВременныйКаталог, Истина);

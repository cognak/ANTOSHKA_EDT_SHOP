	...
	Если ТипЗнч(ДополнительныеНастройки) = Тип("Структура") Тогда
		...
		Для каждого ИмяРеквизитаЗначение Из ДополнительныеНастройки Цикл
			Если ИмяРеквизитаЗначение.Ключ <> "НеИспользоватьПроксиДляАдресов" Тогда
				ЭтотОбъект[ИмяРеквизитаЗначение.Ключ] = ИмяРеквизитаЗначение.Значение;
			КонецЕсли;
		КонецЦикла;
...
Процедура СохранитьНастройкиПроксиВФайлеКонфигурацииСетиПоУмолчанию()
	...
	ПоказатьПредупреждение(, РезультатЗаписиВФайл);
...
Процедура ДобавитьПротоколКСтрокеПротоколов(Протокол, СтрокаПротоколов)
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект[ИмяРеквизитаАдреса]) Тогда
		
		СтрокаПротоколов = СтрокаПротоколов
			+ ?(ПустаяСтрока(СтрокаПротоколов), "", " ")
			+ Протокол
			+ ?(ПустаяСтрока(Протокол), "", "=")
			+ НормализованныйАдресПроксиСервера(ЭтотОбъект[ИмяРеквизитаАдреса])
			+ ?(ЭтотОбъект[ИмяРеквизитаПорта] = 0,
				"",
				":" + СтрЗаменить(Строка(ЭтотОбъект[ИмяРеквизитаПорта]), Символ(160), ""));
...
Функция ПутьКФайлуКонфигурацииСети()
	...
	Попытка
		...
		Пока ПрочитаннаяСтрока <> Неопределено Цикл
			...
			Если СтрНайти(ВРег(СтрокаКонфигурации), "CONFLOCATION") = 1 Тогда

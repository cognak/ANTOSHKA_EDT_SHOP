...
Процедура СформироватьПечатныеФормы(КоллекцияПечатныхФорм, ИменаМакетов, Отказ)
	...
	// При создании коллекции извне, данная колонка может отсутствовать
	//Если КоллекцияПечатныхФорм.Колонки.Найти("ЭтоМногоязычнаяПечатнаяФорма") = Неопределено Тогда
	//	КоллекцияПечатныхФорм.Колонки.Добавить("ЭтоМногоязычнаяПечатнаяФорма", Новый ОписаниеТипов("Булево"));
	//КонецЕсли;
...
//	Элементы.КодЯзыкаДляМногоязычныхПечатныхФорм.ТолькоПросмотр = Не Пользователи.РолиДоступны("ФормированиеПечатныхФормНаВсехДоступныхЯзыках");
	Элементы.КодЯзыкаДляМногоязычныхПечатныхФорм.ТолькоПросмотр = Истина;	//	LNK 25.09.2017 11:17:40 Временно, нужно повесить на какое право

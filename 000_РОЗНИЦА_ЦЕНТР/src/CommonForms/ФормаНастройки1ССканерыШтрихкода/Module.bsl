...
Процедура ОбновитьИнформациюОДрайвере()
	...
	Если ВерсияИзБПОСтр > ВерсияСтр И ВерсияСтр > 8000000 Тогда
		Сообщить(НСтр("ru='Установленная на компьютере версия драйвера устарела! Рекомендуется обновление до версии:'") + Символы.НПП + ВерсияИзБПО);
	КонецЕсли;
...
Функция СПВТекст(Номер) 
	Результат = Номер;	
	ПервоеВхождение = СтрНайти(Результат, "#");
	Если ПервоеВхождение = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	Врем = Сред(Результат, ПервоеВхождение+1);
	ВтороеВхождение = СтрНайти(Врем, "#") + ПервоеВхождение;

...
Процедура ПолучитьЧекИзЦентральногоУзлаЗавершение(КодВозврата, ДополнительныеПараметры)	Экспорт
	...
	Если КодВозврата = КодВозвратаДиалога.ОК Тогда
		...
		Если ДанныеОтвета.Ошибка Тогда
			...
			Элементы.ТекстОшибки.Заголовок = ДанныеОтвета.ТекстОшибки;

		Иначе

			ПоказатьОповещениеПользователя("Выполнено",, "Документ получен «" + ДанныеОтвета.Документ + "»", БиблиотекаКартинок.ПлюсЗеленый_128);
			Элементы.ТекстОшибки.Заголовок = НСтр("ru = 'Документ получен!'; uk = 'Документ отримано!'");
	...
	Иначе
		...
		Элементы.ТекстОшибки.Заголовок = НСтр("ru = 'Действие выполнено не будет'; uk = 'Дія виконана не буде!'");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Элементы.ТекстОшибки.Заголовок = "";
	Номер = "";
	Дата = Дата(1,1,1);
КонецПроцедуры

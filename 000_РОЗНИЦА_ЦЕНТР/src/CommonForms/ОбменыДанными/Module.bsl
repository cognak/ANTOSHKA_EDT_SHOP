...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПлановОбмена.Количество() = 0 Тогда
		ТекстСообщения = "Можливості налаштування синхронізації даних не передбачено.";
...
Процедура СписокСостоянияУзловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОбменДаннымиКлиент.ВыполнитьОбменДаннымиОбработкаКоманды(Элементы.СписокСостоянияУзлов.ТекущиеДанные.УзелИнформационнойБазы, ЭтотОбъект);
...
Процедура ВыполнитьОбменДанными(Команда)
	...
	Если КоличествоСинхронизаций = 1 Тогда
		...
		СписокУзлов.Добавить(СписокСостоянияУзлов[0].УзелИнформационнойБазы);
		
	Иначе
		
		Для каждого ИдентификаторСтроки Из Элементы.СписокСостоянияУзлов.ВыделенныеСтроки Цикл

			ТекущиеДанные = СписокСостоянияУзлов.НайтиПоИдентификатору(ИдентификаторСтроки);
			
			Если НЕ ТекущиеДанные = Неопределено Тогда
				
				СписокУзлов.Добавить(ТекущиеДанные.УзелИнформационнойБазы);
	...
	ЗадатьВопрос = СписокУзлов.Количество() = 1;
	
	Для каждого УзелОбмена Из СписокУзлов Цикл

		ОбменДаннымиКлиент.ВыполнитьОбменДаннымиОбработкаКоманды(УзелОбмена, ЭтотОбъект, ЗадатьВопрос);
...
Процедура НастроитьСценарииОбменаДаннымиОдна(Команда)
	...
	ОткрытьФормуМодально("Справочник.СценарииОбменовДанными.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
...
Процедура ОбновитьКоличествоСинхронизаций()
	...
	ЕстьПравоОбновления = Ложь;//ПравоДоступа("ОбновлениеКонфигурацииБазыДанных", Метаданные);
	...
	Иначе
		...
		НадписьОбменДаннымиПриостановлен.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НадписьОбменДаннымиПриостановлен.Заголовок, ОбменДаннымиПовтИсп.ГлавныйУзел());
...
Процедура ОбновитьКомандыРезультатовСинхронизации()
	...
	Если КоличествоСинхронизаций <> 0 Тогда
		...
		Если ЕстьПроблемы Тогда
			НадписьПроблемы = "Проблеми: %Количество%";
		...
		Если ИспользуетсяВерсионирование Тогда
			...
			Если ЗначениеЗаполнено(ДатаПоследнейКоллизии) Тогда
				НадписьКоллизии = "Останній конфлікт %Дата%";
			...
			Если ЗначениеЗаполнено(ДатаПоследнегоЗапретаЗагрузки) Тогда
				НадписьНепринятые = "Остання заборона завантаження %Дата%";
...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	Если НастроеннаяСинхронизация.УзелИнформационнойБазы = Неопределено Тогда
		...
		ВызватьИсключение "Робота монітора синхронізації даних у нерозділеному режимі не підтримується";
		
	КонецЕсли;
	
	ЗаголовокФормы = "Синхронізація даних з «%Программа%»";
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойВыгрузки <> НастроеннаяСинхронизация.ДатаПоследнейВыгрузки
		...
		ПодсказкаВыгрузки = "Дані надіслані: %ДатаОтправки%
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойЗагрузки <> НастроеннаяСинхронизация.ДатаПоследнейЗагрузки
		...
		ПодсказкаЗагрузки = "Дані отримані: %ДатаПолучения%
							|Остання спроба: %ДатаПопытки%";
	...
	Иначе
		...
		РасписаниеСинхронизацииДанных = "Розклад синхронізації не налаштований";
		...
		Если МенеджерПланаОбмена.ИспользоватьПомощникСозданияОбменаДанными() 
			...
			Иначе
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					...
					Команды[ИмяКоманды].Заголовок = ЗаголовокКоманды + " (у сервісі)...";
...
#Область КомандыПереходаНаСвязанныеРесурсы

&НаКлиенте	//	LNK 24.06.2021 07:11:03
Процедура ПерейтиВСписокСостоянияОбменовДанными(Команда)

	ПерейтиВСписокСвязанногоРесурса("СостоянияОбменовДанными");

КонецПроцедуры

&НаКлиенте	//	LNK 24.06.2021 07:11:08
Процедура ПерейтиВСписокСостоянияУспешныхОбменовДанными(Команда)

	ПерейтиВСписокСвязанногоРесурса("СостоянияУспешныхОбменовДанными");

КонецПроцедуры

&НаКлиенте	//	LNK 24.06.2021 07:13:01
Процедура ПерейтиВСписокСвязанногоРесурса(ИмяРегистра)

	ОткрытьФорму("РегистрСведений." + ИмяРегистра + ".ФормаСписка",, ЭтотОбъект, ЭтотОбъект,,,, РежимОткрытияОкнаФормы.Независимый);

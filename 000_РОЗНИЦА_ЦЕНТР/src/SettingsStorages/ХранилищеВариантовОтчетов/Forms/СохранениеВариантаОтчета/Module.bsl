...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ УправлениеПользователями.ПолучитьБулевоЗначениеПраваПользователя(ПланыВидовХарактеристик.ПраваПользователей.РазрешитьСохранениеВариантаДляВсехПользователей, Ложь) Тогда
	//	LNK 29.01.2019 13:13:19

		Элементы.Доступен.СписокВыбора.Удалить(1);
...
Процедура ВариантыОтчетаОбработчикАктивизацииСтроки(ЭтотОбъект)
	ЭтотОбъект.ИдентификаторТекущейСтроки = ЭтотОбъект.Элементы.ВариантыОтчета.ТекущаяСтрока;
	
	Вариант = ЭтотОбъект.ВариантыОтчета.НайтиПоИдентификатору(ЭтотОбъект.ИдентификаторТекущейСтроки);
	Если Вариант = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПравоНастройкиВарианта = ПравоНастройкиВарианта(Вариант, ЭтотОбъект.ПолныеПраваНаВарианты);
	ПравоЗаписиВарианта    = ПравоЗаписиВарианта(Вариант, ПравоНастройкиВарианта);
	Если ПравоЗаписиВарианта Тогда
		ЭтотОбъект.ВариантСсылка = Вариант.Ссылка;
		ЭтотОбъект.ВариантАвтор  = Вариант.Автор;
		ЭтотОбъект.ВариантНаименование = Вариант.Наименование;
		ЭтотОбъект.ВариантОписание     = Вариант.Описание;
		ЭтотОбъект.ВариантВидимостьПоУмолчанию = Вариант.ВидимостьПоУмолчанию;
		ЭтотОбъект.ВариантТолькоДляАвтора      = Вариант.ТолькоДляАвтора;
	Иначе
		ЭтотОбъект.ВариантСсылка = Неопределено;
		ЭтотОбъект.ВариантАвтор  = ЭтотОбъект.ТекущийПользователь;
		ЭтотОбъект.ВариантНаименование = СформироватьСвободноеНаименование(Вариант, ЭтотОбъект.ВариантыОтчета);
		ЭтотОбъект.ВариантОписание     = "";
		ЭтотОбъект.ВариантВидимостьПоУмолчанию = Ложь;
		ЭтотОбъект.ВариантТолькоДляАвтора      = Истина;
	КонецЕсли;
	
	ЭтотОбъект.Доступен = ?(ЭтотОбъект.ВариантТолькоДляАвтора, "1", "2");
	
	УстановитьЧтоБудетДальше(ЭтотОбъект, ЗначениеЗаполнено(ЭтотОбъект.ВариантСсылка));
...
Процедура ВидимостьДоступность(ЭтотОбъект, Изменения = "")
	...
	Если Изменения = "" ИЛИ Изменения = "ТолькоДляАвтора" Тогда
		Если НЕ ЭтотОбъект.ОтчетИнформация.ЭтоВнешний Тогда
			Элементы.ВидимостьПоУмолчанию.Доступность = НЕ ЭтотОбъект.ВариантТолькоДляАвтора;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЧтоБудетДальше(ЭтотОбъект, Перезапись)
	Если Перезапись Тогда
		ЭтотОбъект.Элементы.ПерезаписьИлиНовый.ТекущаяСтраница = ЭтотОбъект.Элементы.Перезапись;
	Иначе
		ЭтотОбъект.Элементы.ПерезаписьИлиНовый.ТекущаяСтраница = ЭтотОбъект.Элементы.Новый;
	КонецЕсли;
КонецПроцедуры

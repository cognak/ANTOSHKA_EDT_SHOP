Процедура СформироватьТабличныйДокумент(ТабличныйДокумент)	Экспорт
	...
	Запрос.УстановитьПараметр("ДатаТекущая"  , ТекущаяДатаСеанса());
	...
	Пока ВладелецВыборка.Следующий() Цикл
		...
		Пока КлючиВыборка.Следующий() Цикл
			...
			КлючНомера   = ПодарочныеСертификатыСервер.РасшифроватьКлючНомера(КлючиВыборка.КлючНомераСекретный);
			ОбластьСтрока.Параметры.КлючНомера   = КлючНомера;
			КлючНомераПодШтрихкодом = Сред(КлючНомера, 1, 4) + " ";
			КлючНомераПодШтрихкодом = КлючНомераПодШтрихкодом + Сред(КлючНомера, 5, 4) + " ";
			КлючНомераПодШтрихкодом = КлючНомераПодШтрихкодом + Сред(КлючНомера, 9, 4) + " ";
			КлючНомераПодШтрихкодом = КлючНомераПодШтрихкодом + Сред(КлючНомера, 13, 4);
			ОбластьСтрока.Параметры.КлючНомераПодШтрихкодом   = КлючНомераПодШтрихкодом;

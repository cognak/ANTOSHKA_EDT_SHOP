...
Функция ИмяФормыСозданияНачальногоОбраза() Экспорт
	...
	Возврат "ПланОбмена.ПоМагазину.Форма.ФормаСозданияНачальногоОбраза";
...
Функция ПрименитьОбъектПриКоллизииИзменений(УзелИнформационнойБазы, Объект) Экспорт
	...
	Разрешение = Истина;	//	полученное изменение должно примениться
	...
	//	Тут главное - не перепутать, что и когда.
		Разрешение = ОбменДаннымиПовтИсп.ЭтоГлавныйУзел();

	ИначеЕсли НазначенПриоритетПериферии(Объект) Тогда

	//	Изменение однозначно зарегистрировано в получателе!
	//	Для некоторых объектов ("Документы.ЧекККМ", "Документы.ОтчетОРозничныхПродажах" и коллекция их движений)
	//	изменяем приоритет - приоритет у периферийного узла! Периферия в смысле фронта первична!

	//	Когда заезжаем в главный узел - принимаем изменения безоговорочно!
	//	Когда заезжаем в периферию - отказ в получении.. ох, тока б не промахнуться.
		Если НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда

			Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЧекККМ") И НЕ Объект.ВладелецДисконтнойКарты = Справочники.Контрагенты.РозничныйПокупатель Тогда

			//	Из ЦБ могут поступить "исправления" контрагента и дисконтной карты.
			//	Поэтому подменим полученные данные объектом этой БД (периферии), сохранив
			//	при этом полученные значение реквизитов "ВладелецДисконтнойКарты" и "ДисконтнаяКарта"
				...
				|	И ТаблицаДокумента.ВладелецДисконтнойКарты = ЗНАЧЕНИЕ(Справочник.Контрагенты.РозничныйПокупатель)"
				);
				...
				Если Выборка.Следующий() Тогда
					...
					ДокументОбъект.ВладелецДисконтнойКарты = Объект.ВладелецДисконтнойКарты;
					ДокументОбъект.ДисконтнаяКарта = Объект.ДисконтнаяКарта;
	...
	Иначе
		...
		Разрешение = НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел();
...
//	LNK 31.10.2018 10:51:25
Функция НазначенПриоритетПериферии(Объект)

	ПриоритетНазначен = Ложь;
	...
	Попытка
		...
		Если ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных) Тогда
			...
			ПриоритетНазначен = (ОбъектМетаданных = Метаданные.Документы.ОтчетОРозничныхПродажах)
							ИЛИ (ОбъектМетаданных = Метаданные.Документы.ЧекККМ)
							ИЛИ (ОбъектМетаданных = Метаданные.Документы.ПересчетТоваровСамостоятельный);

		ИначеЕсли ОбщегоНазначения.ЭтоРегистрНакопления(ОбъектМетаданных) Тогда

			ПриоритетНазначен = (ТипЗнч(Объект.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах"))
							ИЛИ (ТипЗнч(Объект.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ЧекККМ"))
							ИЛИ (ТипЗнч(Объект.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ПересчетТоваровСамостоятельный"))
							ИЛИ ОбъектМетаданных = Метаданные.РегистрыНакопления.БонусныеБаллы
							ИЛИ ОбъектМетаданных = Метаданные.РегистрыНакопления.БонусныеБаллыВРезерве
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных) Тогда
			...
			Если ОбъектМетаданных.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
				...
				ПриоритетНазначен = (ТипЗнч(Объект.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах"))
								ИЛИ (ТипЗнч(Объект.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ЧекККМ"))
								ИЛИ (ТипЗнч(Объект.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ПересчетТоваровСамостоятельный"))
								ИЛИ ОбъектМетаданных = Метаданные.РегистрыСведений.СписанныеБонусныеБаллы

...
#Область ОбработчикиСобытийТабличногоПоляОбъектыИсключения

&НаКлиенте	//	LNK 23.03.2024 15:22:06
Процедура ОбъектыИсключенияПолноеИмяНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;

	ТекущиеДанные = Элементы.ОбъектыИсключения.ТекущиеДанные;

	Если НЕ ТекущиеДанные = Неопределено Тогда

		ДополнительныеПараметры	= Новый Структура(
			"ТекущиеДанные"
			, Элементы.ОбъектыИсключения.ТекущиеДанные
		);
		ПараметрыОткрытия = Новый Структура(
			"Заголовок, ПолноеИмя"
			, "Выберите полное имя объекта метаданных"
			, Элементы.ОбъектыИсключения.ТекущиеДанные.ПолноеИмя
		);

		ОписаниеОповещения = Новый ОписаниеОповещения("ОбъектыИсключенияПолноеИмяНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		ОткрытьФорму("ОбщаяФорма.ФормаВыбораМетаданных"
			, ПараметрыОткрытия
			, Элемент, Элемент
			,,
			, ОписаниеОповещения
...
&НаКлиенте	//	LNK 24.03.2024 10:18:36
Процедура ОбъектыИсключенияПолноеИмяНачалоВыбораЗавершение(Значение, ДополнительныеПараметры)	Экспорт

	Если ТипЗнч(Значение) = Тип("Структура") Тогда

		ДополнительныеПараметры.ТекущиеДанные.ПолноеИмя = Значение.ПолноеИмя;

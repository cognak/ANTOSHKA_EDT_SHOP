...
Функция ПолучитьДанныеУзла(УзелИнформационнойБазы)	Экспорт
	...
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаНачалаВыгрузкиДокументов,
	|	ЗНАЧЕНИЕ(Перечисление.РежимыВыгрузкиИнформативныхОстатков.НеВыгружать) КАК РежимВыгрузкиИнформативныхОстатков,
	|	ИСТИНА КАК РегистрироватьИзменения,
	|	ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка) КАК Магазин,
	|	0 КАК НомерМагазина,
	|	ВЫБОР
	|		КОГДА ТаблицаУзлов.УчетнаяСистема = ЗНАЧЕНИЕ(Перечисление.УчетныеСистемыКомпании.РозницаТестовая)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыУзлов.ТестовыйУзел)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыУзлов.ПустаяСсылка)
	|	КОНЕЦ КАК ВидУзла,
	|	ЗНАЧЕНИЕ(Справочник.СтруктураУзлов.ПустаяСсылка) КАК ЭлементСтруктуры
	|ИЗ
	|	ПланОбмена.Полный КАК ТаблицаУзлов
	|ГДЕ
...
//	в этом обмене "едем" только в одну сторону.
//	Поэтому коллизии разрешаем однозначно.
	Разрешение = Истина;

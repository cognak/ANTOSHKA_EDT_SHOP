...
Функция PrintPOST(ЗапросСервиса)
	...
	Иначе
		...
		ИначеЕсли Результат.type = "sticker" Тогда 
			...
			Если ДанныеОЗапросе.ВнутренняяЛогистика Тогда
				
				СоздатьВнутреннийСтикер(ДанныеОЗапросе, Результат.order);
				СтруктураДанных.Вставить("doc", "https://cbone.antoshka.ua/np/InternalLogistics" + СокрЛП(ДанныеОЗапросе.НомерЗапроса) + ".pdf");
...
Функция ParcelPOST(ЗапросСервиса)
	...
	ИначеЕсли ДанныеОЗапросе.Отменён Тогда  
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("Центральный склад. ВГХ", УровеньЖурналаРегистрации.Ошибка
				, Метаданные.Документы.ЗапросДоступностиТоваров
				, ДанныеОЗапросе.СсылкаНаЗапрос
				, ТекстОшибки
			);
...
Функция ParcelGET(ЗапросСервиса)
	...
	Если СтруктураОтвета.КодОтвета = 0 Тогда
		...
		Запрос.Текст =
			...
			|		ПО ТаблицаЗапросаТовары.Ссылка = ЗДТ.Ссылка
			|ГДЕ
		...
		Иначе
			...
			Попытка
				...
				Если СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Новый
						...
						Или СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Изменён Тогда
...
Функция TTNPOST(ЗапросСервиса)
	...
	ИначеЕсли Не ДанныеОЗапросе.ВнутренняяЛогистика И (ДанныеОЗапросе.СсылкаНаЭН = Документы.ЭлектроннаяНакладная.ПустаяСсылка()) Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("Центральный склад. Создание электронной накладной", УровеньЖурналаРегистрации.Ошибка
	...
	ИначеЕсли ДанныеОЗапросе.ВнутренняяЛогистика Тогда

		СоздатьВнутреннийСтикер(ДанныеОЗапросе, Результат.order);
		
		ДанныеОЗапросе.Номер = "Внутреняя логистика";
	...
	Если СтруктураОтвета.КодОтвета = 0 Тогда
		...
		Попытка
			...
			Если ПустаяСтрока(ДанныеОЗапросе.ВнешняяСсылка)
					И Не ДанныеОЗапросе.СсылкаНаЭН.Пустая() Тогда
				...
				Если ОтветЭН.Успех Тогда
					...
					ДокументОбъект.Записать();
					РегистрыСведений.СтатусыЭН.СозданаПосылка(ДокументОбъект.Ссылка);
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("Центральный склад. Заполнение электронной накладной и создание ТТН", УровеньЖурналаРегистрации.Ошибка
				, Метаданные.Документы.ЭлектроннаяНакладная
				, НовыйДокумент
				, ТекстОшибки
			);
...
Функция LiqPayPOST(ЗапросСервиса)
	...
	ОбменСБанкамиLiqPayСервер.ОбработкаОтветаПоЗаказуLiqPay(СтрокаЗапроса);
...
Функция EsputnikPOST(ЗапросСервиса)
	...
	Для Каждого СтрокаДанных Из ДанныеОтвета Цикл
		...
		Если СтрокаДанных.mediaType = "viber" И СтрокаДанных.Свойство("sourceEventKey") Тогда
...
Функция ДанныеОЗапросеДоступности(НомерЗапроса)
	...
	Результат.Вставить("ВнутренняяЛогистика", Ложь);
	Результат.Вставить("НомераПеремещений", "");

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЗДТ.Ссылка КАК СсылкаНаЗапрос,
		|	ЕСТЬNULL(ЭН.Ссылка, ЗНАЧЕНИЕ(Документ.ЭлектроннаяНакладная.ПустаяСсылка)) КАК СсылкаНаЭН,
		...
		|	ЕСТЬNULL(РеестрЭН.Дата, ДАТАВРЕМЯ(1900, 1, 1)) КАК РеестрДата,
		...
		|	КОНЕЦ КАК Отменён,
		|	ВЫБОР
		|		КОГДА ЭН.Ссылка ЕСТЬ NULL
		|			ТОГДА ЗДТ.ОператорДоставки = ЗНАЧЕНИЕ(Перечисление.ОператорыДоставки.ВнутренняяЛогистика)
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ВнутренняяЛогистика,
		|	ЕСТЬNULL(ПеремещениеТоваров.Номер, """") КАК НомерПеремещения,
		...
		|		ПО ПеремещениеТоваров.ДокументОснование = ЗДТ.Ссылка
		|ГДЕ
		|	ЗДТ.Номер = &Номер
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивПеремещений.Добавить(Выборка.НомерПеремещения);
//		
//		Результат.ЕстьЗаказ = Истина;
//		Результат.СсылкаНаЗапрос = Выборка.СсылкаНаЗапрос;
//		Результат.СсылкаНаЭН = Выборка.СсылкаНаЭН;
...
//		Результат.Отменён = Выборка.Отменён;
		
	КонецЦикла;
	
	НомераПеремещений = "";
	
	Для Каждого НомерПеремещения Из МассивПеремещений Цикл
		
		НомераПеремещений = НомераПеремещений + ?(НомераПеремещений = "", "", ", ") + НомерПеремещения;
		
	КонецЦикла;
	
	Результат.НомераПеремещений = НомераПеремещений;
...
Функция ОтветСервиса(СтруктураОтвета)
	...
	Иначе
		...
		ИначеЕсли СтруктураОтвета.КодОтвета = 14 Тогда
			КодОшибки = "NO_14";
			ТекстОшибки = "Заказ отменён или закрыт";
		КонецЕсли;  
		
		ТекстОшибкиСтруктура = Новый Структура;
		ТекстОшибкиСтруктура.Вставить("message", ТекстОшибки);
		ТекстОшибкиСтруктура.Вставить("message_code", КодОшибки);
...
Процедура СоздатьВнутреннийСтикер(ДанныеОЗапросе, НомерДокумента)

	ДокументСтикер = Документы.ЗапросДоступностиТоваров.СтикерДляВнутреннейЛогистики(ДанныеОЗапросе);
	...
	ДокументСтикер.Записать(ИмяФайлаЗаписи, ТипФайлаТабличногоДокумента.PDF); 
	ИмяФайлаЗаписиНакладной = "InternalLogistics"+СокрЛП(НомерДокумента)+".pdf";
	ОбменНПСервер.ПередатьФайл(ИмяФайлаЗаписи, ИмяФайлаЗаписиНакладной);

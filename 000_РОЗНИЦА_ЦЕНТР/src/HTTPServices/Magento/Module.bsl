...
Функция RegisterClient(ДанныеЗапроса)
	...
	Для Каждого СтрокаТаблицы Из ДанныеВходящие.ТаблицаКлиентов Цикл
			КонтрагентСсылка = Справочники.Контрагенты.НайтиПоТелефону(СтрокаТаблицы.НомерТелефона, Перечисления.ЮрФизЛицо.ФизЛицо, Истина, Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента);
			...
			Если КонтрагентСсылка.Пустая() Тогда
				...
				КонтрагентСсылка = СоздатьКонтрагента(УИ
					, СтрокаТаблицы
					, ДанныеВходящие.ТаблицаДетей.НайтиСтроки(Новый Структура("ID_Parent", СтрокаТаблицы.ID_Parent))
				);
				СтрокаТаблицы.УспешноСоздан = Истина;
				ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
			Иначе
				ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
				
				ОписаниеКлиента.Error		 = Истина;
				ОписаниеКлиента.Message = "A client with the same phone number "+СтрокаТаблицы.НомерТелефона+" already exists";

		...
		//	Например: "2009-02-15T00:00:00Z".
			...
			КонецЕсли;
				...
				ИначеЕсли Действие = "receipt" Тогда		
					ТекстОтвета = Receipt(RequestData);	
					HTTPОтвет = СформироватьHTTPОтвет(ТекстОтвета);
				ИначеЕсли Действие = "sendclientmessage" Тогда		
					ТекстОтвета = SendClientMessage(RequestData);	
					HTTPОтвет = СформироватьHTTPОтвет(ТекстОтвета);
				Иначе
...
Функция GetChanges(ДанныеЗапроса)
	...
	Если КэшКлиентов = Неопределено Тогда
		...
		КэшКлиентов = Новый Структура("МассивКлиентов,Дата,УИДКеша",ИзмененныеКлиенты,ТекущаяДатаСеанса(),Строка(Новый УникальныйИдентификатор));
...
Функция SendClientMessage(ДанныеЗапроса)
	...
	СтруктураОтвета.Вставить("Provider", "TurboSMS");  //Пока только один
	...
	ТекстСообщения = ДанныеЗапроса.MessageData.MessageText;
	Попытка

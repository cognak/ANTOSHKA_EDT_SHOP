...
#Область КомплексПроцедуры_GetChangesOrder

Функция ПолучитьИзмененияПоЗаказам()
	...
	|		ПО ЗаказПокупателяИзменения.Ссылка = ЗаказПокупателя.Ссылка
	|ГДЕ
	|	ЗаказПокупателяИзменения.Узел В (&Узлы)";
	Получатели = ОбменMagentoСервер.ОпределитьПолучателейМаженто2();
	Запрос.УстановитьПараметр("Узлы",Получатели);
...
Функция GetChangesOrder(ДанныеЗапроса)
	...
	ИзмененныеЗаказы = ПолучитьИзмененияПоЗаказам();

	ВсегоЗаказов = ИзмененныеЗаказы.Количество();
	ВсегоСтраниц = Цел(ВсегоЗаказов/ЧислоЗаписейНаСтранице) + ?(Цел(ВсегоЗаказов/ЧислоЗаписейНаСтранице)<>ВсегоЗаказов/ЧислоЗаписейНаСтранице,1,1);
	Если НомерСтраницы > ВсегоСтраниц Тогда
		НомерСтраницы = ВсегоСтраниц;
	КонецЕсли;
	...
	Если ПерваяЗапись < 0 Тогда
		ПерваяЗапись = 0;
	КонецЕсли;
	ПоследняяЗапись = Мин(ПерваяЗапись + ЧислоЗаписейНаСтранице, ВсегоЗаказов);
	Если ПоследняяЗапись < 0 Тогда
		ПоследняяЗапись = 1;
	КонецЕсли;
	Для НомерЗаказа = ПерваяЗапись По ПоследняяЗапись - 1 Цикл
		
		//СтрокаЗаказа = ИзмененныеЗаказы[]

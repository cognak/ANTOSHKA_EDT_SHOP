...
Функция CustomerDo(HTTPЗапрос)	Экспорт
	...
	Действие = Нрег(HTTPЗапрос.ПараметрыURL["Action"]);
		...
		//	Например: "2009-02-15T00:00:00Z".
			...
			Если ДанныеЗапроса.IDBASE <> ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы") Тогда
				HTTPОтвет.КодСостояния	= 400;	//	400 Bad Request
												...
												//	что воспринимается как ошибка клиента (например, неправильный синтаксис,
												//	формат или маршрутизация запроса).
				HTTPОтвет.Заголовки.Вставить("ErrorStatus"	, "WRONG_DESTINATION");
				HTTPОтвет.Заголовки.Вставить("ErrorMessage"	, СтроковыеФункцииКлиентСервер.СтрокаЛатиницей("Wrong IDBASE!"));
			...
			КонецЕсли;
				...
				ИначеЕсли Действие = "update" Тогда	//	/Customer/{Action} - последняя лексема
					...
					HTTPОтвет = СформироватьHTTPОтвет(ТекстОтвета);
				ИначеЕсли Действие = "deletekid" Тогда	//	/Customer/{Action} - последняя лексема
					ТекстОтвета = DeleteClientKid(ДанныеЗапроса);	//	
					HTTPОтвет = СформироватьHTTPОтвет(ТекстОтвета);
				...
				ИначеЕсли Действие = "getchanges" Тогда	//	/Customer/{Action} - последняя лексема
					...
					//	НомерСтраницы = 0;
					//Иначе
					//	НомерСтраницы = Число(HTTPЗапрос.ПараметрыЗапроса.Получить("Page"));
					//КонецЕсли;
				...
				ИначеЕсли Действие = "getdiscountcalcv2" Тогда	//	/Customer/{Action} - последняя лексема
					...
					HTTPОтвет = СформироватьHTTPОтвет(ТекстОтвета);
				ИначеЕсли Действие = "receipt" Тогда		
					ТекстОтвета = Receipt(ДанныеЗапроса);	
					HTTPОтвет = СформироватьHTTPОтвет(ТекстОтвета);
				ИначеЕсли Действие = "sendclientmessage" Тогда		
					ТекстОтвета = SendClientMessage(ДанныеЗапроса);	
					HTTPОтвет = СформироватьHTTPОтвет(ТекстОтвета);
				Иначе
...
Функция Латиница(Текст)
	...
	Для н = 1 По ДлинаТекста Цикл
		...
		ТрансСимвол = СокрЛП(Сред(ЛатинскийАлфавит,т + 1,ы - т));
...
//	IDC является приоритетным ключём! Если указан - принимаем эту ссылку.
	...
	ТекстJSON = ПолучитьКлиентовВФорматеJSON(Запрос,ТаблицаНовых);
	...
	//РаботаСJSONСервер.ЗаписатьСвойство("DIFF"	, СокрЛП(Новый УникальныйИдентификатор), ЗаписьJSON);
	...
	//			РаботаСJSONСервер.ЗаписатьСвойство("IDC"				, ОбменMagentoСлужебный.IDC_Строкой(СтрокаКлиенты.Контрагент), ЗаписьJSON);
	//			РаботаСJSONСервер.ЗаписатьСвойство("ClientPhone"		, СтрокаКлиенты.НомерТелефона, ЗаписьJSON);
	//			РаботаСJSONСервер.ЗаписатьСвойство("ClientEmail"		, СокрЛП(СтрокаКлиенты.Электропочта), ЗаписьJSON);

	//			РаботаСJSONСервер.ЗаписатьСвойство("ClientFirstName" , СтрокаКлиенты.Имя, ЗаписьJSON);
	//			РаботаСJSONСервер.ЗаписатьСвойство("ClientMiddleName", СтрокаКлиенты.Отчество, ЗаписьJSON);
	//			РаботаСJSONСервер.ЗаписатьСвойство("ClientLastName"  , СтрокаКлиенты.Фамилия, ЗаписьJSON);

	//			РаботаСJSONСервер.ЗаписатьСвойство("ClientDateBorn"	, ОбменMagentoСервер.ФорматДаты(СтрокаКлиенты.ДатаРождения, ФорматДатыJSON.ISO), ЗаписьJSON);
	//			РаботаСJSONСервер.ЗаписатьСвойство("ClientGender"	, СтрокаКлиенты.Пол, ЗаписьJSON);

	//			РаботаСJSONСервер.ЗаписатьСвойство("Message"			, СтрокаКлиенты.ТекстСообщения, ЗаписьJSON);
	//			РаботаСJSONСервер.ЗаписатьСвойство("Error"			, ?(СтрокаКлиенты.Ошибка, 1, 0), ЗаписьJSON);
	...
	//					РаботаСJSONСервер.ЗаписатьСвойство("ChildId"			, СтрокаРебёнок.РебёнокUUID, ЗаписьJSON);
	//					РаботаСJSONСервер.ЗаписатьСвойство("ChildFirstName"	, СтрокаРебёнок.РебёнокИмя, ЗаписьJSON);
	//					РаботаСJSONСервер.ЗаписатьСвойство("ChildGender"		, СтрокаРебёнок.РебёнокПол, ЗаписьJSON);
	//					РаботаСJSONСервер.ЗаписатьСвойство("ChildDateBorn"	, ОбменMagentoСервер.ФорматДаты(СтрокаРебёнок.РебёнокДатаРождения, ФорматДатыJSON.ISO), ЗаписьJSON);
...
Функция ПолучитьКлиентовВФорматеJSON(Запрос, ТаблицаНовых)
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
		...
		Для каждого СтрокаТаблицы Из ТаблицаНовых Цикл
			...
			//	Предварительный контроль дублей!
				...
				Если КонтрагентСсылка.Пустая() Тогда
					КонтрагентСсылка = СоздатьКонтрагента_Старый(УИ
						, СтрокаТаблицы
...
//	AddressWarehouse	- ID почтового отделения.
	...
	Если НЕ СоставАдреса.Количество() = 0 Тогда
		...
		РаботаСJSONСервер.ЗаписатьСвойство("AddressName"		, "Actual Home Address", ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("AddressCity"		, "", ЗаписьJSON);	//	Код КОАТУУ населённого пункта.
		РаботаСJSONСервер.ЗаписатьСвойство("AddressStreet"	, ЭлементАдреса("Улица", СоставАдреса), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("AddressHouse"	, ЭлементАдреса("Дом", СоставАдреса)
			+ ?(ПустаяСтрока(ЭлементАдреса("Корпус", СоставАдреса))
					, ""
					, "/" + ЭлементАдреса("Корпус", СоставАдреса))
		, ЗаписьJSON
		);
		РаботаСJSONСервер.ЗаписатьСвойство("AddressFlat"		, ЭлементАдреса("Квартира", СоставАдреса), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("AddressWarehouse", ЭлементАдреса("Индекс", СоставАдреса), ЗаписьJSON);
...
Функция ПолучитьТаблицуВходящихДанных(ДанныеЗапроса)
	...
	Для каждого Состав Из ДанныеЗапроса.Clients Цикл
		...
		Если Состав.Свойство("IDC") Тогда
			СтрокаТаблицы.IDC			= СокрЛП(Состав.IDC);
		Иначе
			СтрокаТаблицы.IDC			= Строка(Новый УникальныйИдентификатор);
		КонецЕсли;
		...
		СтрокаТаблицы.ID_Parent		= СтрокаТаблицы.IDC;

		СтрокаТаблицы.Имя			= СокрЛП(Состав.ClientFirstName);
		СтрокаТаблицы.Фамилия		= СокрЛП(Состав.ClientLastName);
		
		Если Состав.Свойство("ClientMiddleName") Тогда
			СтрокаТаблицы.Отчество		= СокрЛП(Состав.ClientMiddleName);
		КонецЕсли;

		Если Состав.Свойство("ClientDateBorn") Тогда
			СтрокаТаблицы.ДатаРождения	= Состав.ClientDateBorn;
		КонецЕсли;
		
		Если Состав.Свойство("ClientGender") Тогда
			СтрокаТаблицы.Пол			= Состав.ClientGender;
		КонецЕсли;
		Если Состав.Свойство("ClientEmail") Тогда
			СтрокаТаблицы.Электропочта	= Состав.ClientEmail;
		КонецЕсли;
		
		//добавим возможность через вебсервис регистриовать клиента для последующей отдачи в мадженту (для целей тестирования)
		Если Состав.Свойство("Register") Тогда
			СтрокаТаблицы.Регистрировать = Состав.Register = Истина;
		КонецЕсли;

		Если Состав.Свойство("Children") Тогда
...
Функция ПолучитьСтруктуруТаблицыКлиентов(Режим, ТаблицаКлиентов = Неопределено)
	...
	//	такая длина в "РегистрыСведений.ФИОФизЛиц"
		...
		КолонкиДобавить("УспешноСоздан"	, Новый ОписаниеТипов("Булево"), ТаблицаКлиентов.Колонки);
		КолонкиДобавить("Регистрировать", Новый ОписаниеТипов("Булево"), ТаблицаКлиентов.Колонки);
...
Функция GetBuyerBonusInfo(RequestData)
	...
	РаботаСJSONСервер.ЗаписатьСвойство("IDBASE", Константы.ИдентификаторУчетнойСистемы.Получить(), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("Number_PackMGT", -1, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("DIFF", СокрЛП(Новый УникальныйИдентификатор), ЗаписьJSON);
	...
	//	Инициализируем пользователя административной учетной записью.
		...
		Если Command = "rest" Тогда
			...
			Пока ДанныеВыборка.Следующий() Цикл
				...
				РаботаСJSONСервер.ЗаписатьСвойство("IDC"				, ОбменMagentoСлужебный.IDC_Строкой(ДанныеВыборка.Контрагент), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("phoneNumber"		, СтрокаВнешняя.НомерТелефона, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("BonusesActive"	, ДанныеВыборка.СуммаБонусныхБалловАктивно, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("BonusesReserved"	, ДанныеВыборка.СуммаБонусныхБалловРезерв, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("BonusesPromo"	, ДанныеВыборка.СуммаАкционныхБонусовАктивно, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("AmountPurchases"	, ДанныеВыборка.СуммаПокупок, ЗаписьJSON);

				Если НЕ ПустаяСтрока(СтрокаВнешняя.ОписаниеОшибки) Тогда

					РаботаСJSONСервер.ЗаписатьСвойство("ErrorMessage", СтрокаВнешняя.ОписаниеОшибки, ЗаписьJSON);
	...
	РаботаСJSONСервер.ЗаписатьСвойство("Error", РаботаСJSONСервер.БулевоКакЧисло(СоставОшибки.Ошибка), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("ErrorMessage", СоставОшибки.ОписаниеОшибки, ЗаписьJSON);
...
#Область КомплексПроцедуры_RegisterClient
Функция RegisterClient(ДанныеЗапроса)
	//ДанныеВходящие = ПолучитьТаблицуВходящихДанных(ДанныеЗапроса);

	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("IDBASE"	, ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"));
	СтруктураОтвета.Вставить("DIFF"	, СокрЛП(Новый УникальныйИдентификатор));
	
	МассивКлиентов = Новый Массив;
	Для Каждого Клиент Из ДанныеЗапроса.Clients Цикл
		ОписаниеКлиента = Новый Структура("Error,Message",Ложь,"");
		Если Не Клиент.Свойство("ClientPhone") Тогда
			ОписаниеКлиента.Error		 = Истина;
			ОписаниеКлиента.Message = "Field ClientPhone requid";
			МассивКлиентов.Вставить(ОписаниеКлиента);
			Продолжить;
		КонецЕсли;
			КонтрагентСсылка = Справочники.Контрагенты.НайтиПоТелефону(Клиент.ClientPhone, Перечисления.ЮрФизЛицо.ФизЛицо, Истина, Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента);
			ТаблицаДетей = Новый ТаблицаЗначений;
			КолонкиДобавить("ID_Parent"				, Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(60)), ТаблицаДетей.Колонки);
			КолонкиДобавить("РебёнокUUID"			, Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(36)), ТаблицаДетей.Колонки);
			КолонкиДобавить("РебёнокИмя"			, Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(50)), ТаблицаДетей.Колонки);
			КолонкиДобавить("РебёнокДатаРождения"	, Новый ОписаниеТипов("Дата"), ТаблицаДетей.Колонки);
			КолонкиДобавить("РебёнокПол"			, Новый ОписаниеТипов("ПеречислениеСсылка.ПолФизическогоЛица") , ТаблицаДетей.Колонки);
			Если КонтрагентСсылка.Пустая() Тогда
				
				КонтрагентСсылка = СоздатьКонтрагента(Клиент);
				Если Клиент.Свойство("Error") И Клиент.Error = Истина Тогда
					ОписаниеКлиента.Вставить("Error",Истина);
					ОписаниеКлиента.Вставить("Message",Клиент.Message);
				Иначе
					ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
				КонецЕсли;
			Иначе
				//2024-09-29 sa По многочисленным просьбам, если клиент существует и найден по номеру телефона 
				//переписываем его данные на переданные в запросе.
				Клиент.Вставить("IDC", Строка(КонтрагентСсылка.УникальныйИдентификатор()));
				ОбновитьКлиента(Клиент);
				ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
				
				//ОписаниеКлиента.Error		 = Истина;
				ОписаниеКлиента.Message = "A client with the same phone number "+Клиент.ClientPhone+" already exists";

			КонецЕсли;
			МассивКлиентов.Добавить(ОписаниеКлиента);
	КонецЦикла;
	СтруктураОтвета.Вставить("Clients",МассивКлиентов);
	
	ТекстДЖСОН = ОбменWebRetailСервер.СтруктуруВТекстДЖСОН(СтруктураОтвета);
	
	//ТекстДЖСОН = ПолучитьКлиентовВФорматеJSONНов(ДанныеВходящие);
	
	Возврат ТекстДЖСОН;
КонецФункции

Функция СоздатьДисконтнуюКарту(КонтрагентСсылка,ТелефонКлиента)
	...
	Запрос.УстановитьПараметр("ВладелецКарты", КонтрагентСсылка);
	
	РезультатЗапроса = Запрос.Выполнить().Выбрать();
	
	Если НЕ РезультатЗапроса.Следующий() Тогда 
		
		ДК = ИнформационныеКартыСлужебный.ПрисвоитьРезервнуюВиртуальнуюКарту(КонтрагентСсылка,,ТелефонКлиента,"");
	Иначе
		ДК = РезультатЗапроса.Ссылка;
...
Функция СоздатьКонтрагента(ДанныеКлиента)
...
//		Блокиратор.Заблокировать();
		...
		Попытка
			...
			Если ДанныеКлиента.Свойство("IDC") Тогда
				УИД = Новый УникальныйИдентификатор(ДанныеКлиента.IDC);
			Иначе
				УИД = Новый УникальныйИдентификатор();
			КонецЕсли;
			КонтрагентОбъект.УстановитьСсылкуНового(Справочники.Контрагенты.ПолучитьСсылку(УИД));
			...
			МаркетинговаяГруппа = КонтрагентОбъект.МаркетинговыеГруппы.Добавить();
			МаркетинговаяГруппа.Группа = Справочники.МаркетинговыеГруппы.М2;
		//	---------------------------------------------------------------------------------------
			//Если Не СтрокаТаблицы.Регистрировать Тогда
			...
			ОбменДаннымиСервер.УстановитьВсехПолучателей(КонтрагентОбъект.ОбменДанными.Получатели, "ОбменСЕСпутником");
			...
			Если ДанныеКлиента.Свойство("ClientGender") Тогда
				КонтрагентОбъект.Пол		= ?(НРег(ДанныеКлиента.ClientGender) = "male"	, Перечисления.ПолФизическогоЛица.Мужской,
											  ?(НРег(ДанныеКлиента.ClientGender) = "female"	, Перечисления.ПолФизическогоЛица.Женский
										  	  , Перечисления.ПолФизическогоЛица.ПустаяСсылка()));
			КонецЕсли;
			КонтрагентОбъект.Покупатель		= Истина;
			Если ДанныеКлиента.Свойство("ClientDateBorn") Тогда
				КонтрагентОбъект.ДатаРождения	= Дата(ДанныеКлиента.ClientDateBorn);
			КонецЕсли;


		//	---------------------------------------------------------------------------------------
			НужнаЗаписьИмени = Ложь;
			Имя_ = "";Фамилия_ = ""; Отчество_ = "";
			Если ДанныеКлиента.Свойство("ClientFirstName") Тогда
				СтроковыеФункцииКлиентСервер.ПривестиНаименование(ДанныеКлиента.ClientFirstName, Истина, Истина);
				НужнаЗаписьИмени = Истина;
				Имя_ = ДанныеКлиента.ClientFirstName;
			КонецЕсли;
			Если ДанныеКлиента.Свойство("ClientMiddleName") Тогда
				СтроковыеФункцииКлиентСервер.ПривестиНаименование(ДанныеКлиента.ClientMiddleName, Истина, Истина);
				Отчество_ = ДанныеКлиента.ClientMiddleName;
			КонецЕсли;
			Если ДанныеКлиента.Свойство("ClientLastName") Тогда
				СтроковыеФункцииКлиентСервер.ПривестиНаименование(ДанныеКлиента.ClientLastName, Истина, Истина);
				Фамилия_ = ДанныеКлиента.ClientLastName;
			КонецЕсли;
			...
			МенеджерЗаписи.Фамилия	= Фамилия_;
			МенеджерЗаписи.Имя		= Имя_;
			МенеджерЗаписи.Отчество	= Отчество_;

			МенеджерЗаписи.Записать();

			КонтрагентОбъект.Наименование		= СтроковыеФункцииКлиентСервер.ПривестиНаименование(СокрЛП(Фамилия_ + " " + Имя_ + " " + Отчество_), Ложь, Истина);
		...
		//	---------------------------------------------------------------------------------------
			...
			СтрокаКонтакты.НомерТелефона = СокрЛП(ДанныеКлиента.ClientPhone);
			...
			Если ДанныеКлиента.Свойство("ClientEmail") Тогда
				Если ЗначениеЗаполнено(СокрЛП(ДанныеКлиента.ClientEmail)) Тогда
					СтрокаКонтакты = КонтрагентОбъект.КонтактнаяИнформация.Добавить();
					СтрокаКонтакты.Вид = Справочники.ВидыКонтактнойИнформации.EmailКонтрагента;
					СтрокаКонтакты.Тип = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаКонтакты.Вид, "Тип");
					СтрокаКонтакты.АдресЭП = СокрЛП(ДанныеКлиента.ClientEmail);
					СтрокаКонтакты.Представление = СокрЛП(ДанныеКлиента.ClientEmail);
				КонецЕсли;			
			КонецЕсли;			
			Если ДанныеКлиента.Свойство("Children") Тогда
				Для каждого СтрокаРебенок Из ДанныеКлиента.Children Цикл

					СтрокаТабличнойЧасти = КонтрагентОбъект.Дети.Добавить();
					//Если Не СтрокаРебенок.Свойство("ChildUUID") Тогда
					//	СтрокаРебенок.ChildUUID = Новый УникальныйИдентификатор;
					//КонецЕсли;
					СтрокаТабличнойЧасти.УникальныйИдентификатор = Новый УникальныйИдентификатор;
					СтрокаТабличнойЧасти.ДатаРождения	= Дата(СтрокаРебенок.ChildDateBorn);
					СтрокаТабличнойЧасти.Имя			= СтрокаРебенок.ChildFirstName;
					СтрокаТабличнойЧасти.Пол			= ?(НРег(СтрокаРебенок.ChildGender) = "male"	, Перечисления.ПолФизическогоЛица.Мужской,
												 		  ?(НРег(СтрокаРебенок.ChildGender) = "female"	, Перечисления.ПолФизическогоЛица.Женский
												  										, Перечисления.ПолФизическогоЛица.ПустаяСсылка()));
				КонецЦикла;
			КонецЕсли;
			
			//2026-03-11 sa Из-за того что в отделе аналитики не умеют пользоваться Power BI будем заполнять ДР клиента
			//данными по-умолчанию. 
			Если Не ЗначениеЗаполнено(КонтрагентОбъект.ДатаРождения) Тогда
				КонтрагентОбъект.ДатаРождения = Дата("19010101");
			КонецЕсли;
			
			КонтрагентОбъект.Записать();
			КонтрагентСсылка = КонтрагентОбъект.Ссылка;
			
			//2024-11-20 sa добавляем создание виртуальной ДК
			СоздатьДисконтнуюКарту(КонтрагентСсылка,СокрЛП(ДанныеКлиента.ClientPhone));
			ЗафиксироватьТранзакцию();

		Исключение

			ТекстОшибки = "Ошибка при создании [" + ДанныеКлиента.ClientPhone + "]" + ОписаниеОшибки();
			ОтменитьТранзакцию();
...
//	Исключение
//
//		ТекстОшибки = "Отказано в создании клиента [" + ДанныеКлиента.ClientPhone + "]. Не удалось получить управление процессом.";
//
//	КонецПопытки;

	Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
		Если Не ДанныеКлиента.Свойство("Error") Тогда
			ДанныеКлиента.Вставить("Error", Истина);
			ДанныеКлиента.Вставить("Message","");
		КонецЕсли;
		ДанныеКлиента.Error		 = Истина;
		ДанныеКлиента.Message = ДанныеКлиента.Message + ?(ПустаяСтрока(ДанныеКлиента.Message), "", Символы.ПС)
			+ ТекстОшибки;
		...
		ЖурналСобытий.Регистрация("MAG2.Клиенты.Создание", УровеньЖурналаРегистрации.Предупреждение
			...
			,
			, ТекстОшибки
			, "Клиент [" + ДанныеКлиента.ClientPhone + "]"
			, Истина
			, Ложь
		);
...
Функция СоздатьКонтрагента_Старый(Знач УИ, СтрокаТаблицы, ТаблицаДетей)
...
//		Блокиратор.Заблокировать();
		...
		//	---------------------------------------------------------------------------------------
			...
			КонтрагентОбъект.Пол			= ?(НРег(СтрокаТаблицы.Пол) = "male"	, Перечисления.ПолФизическогоЛица.Мужской,
											  ?(НРег(СтрокаТаблицы.Пол) = "female"	, Перечисления.ПолФизическогоЛица.Женский
											  										, Перечисления.ПолФизическогоЛица.ПустаяСсылка()));
			КонтрагентОбъект.Покупатель		= Истина;
		...
		//	---------------------------------------------------------------------------------------
			...
			Для каждого СтрокаРебенок Из ТаблицаДетей Цикл

				СтрокаТабличнойЧасти = КонтрагентОбъект.Дети.Добавить();

				СтрокаРебенок.РебёнокUUID = Новый УникальныйИдентификатор;
				СтрокаТабличнойЧасти.УникальныйИдентификатор = СтрокаРебенок.РебёнокUUID;
				СтрокаТабличнойЧасти.ДатаРождения	= СтрокаРебенок.РебёнокДатаРождения;
				СтрокаТабличнойЧасти.Имя			= СтрокаРебенок.РебёнокИмя;
				СтрокаТабличнойЧасти.Пол			= СтрокаРебенок.РебёнокПол;
...
Функция ПолучитьОписаниеКлиента(Контрагент)
	ОписаниеКлиента = Новый Структура("IDC,Наименование,ДатаРождения,Пол,Фамилия,Имя,Отчество,ЭлектроннаяПочта,НомерТелефона,ВерсияДанных,Ошибка,ТекстСообщения,Ссылка");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(КонтактнаяИнформацияЕмайл.АдресЭП, """") КАК ЭлектроннаяПочта,
	               |	ЕСТЬNULL(КонтактнаяИнформацияТелефон.НомерТелефона, """") КАК НомерТелефона,
	               |	ЛОЖЬ КАК Ошибка,
	               |	"""" КАК ТекстСообщения,
	               ...
	               |		ПО Контрагенты.Ссылка = КонтактнаяИнформацияТелефон.Ссылка
	               |			И (КонтактнаяИнформацияТелефон.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.Телефон))
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.КонтактнаяИнформация КАК КонтактнаяИнформацияЕмайл
	               |		ПО Контрагенты.Ссылка = КонтактнаяИнформацияЕмайл.Ссылка
	               |			И (КонтактнаяИнформацияЕмайл.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
	               |ГДЕ
	...
	Если Рез.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(ОписаниеКлиента,Рез);		
	КонецЕсли;
	ОписаниеКлиента.IDC = Контрагент.УникальныйИдентификатор();	
	
	Возврат ОписаниеКлиента;
КонецФункции

Функция ПолучитьОписаниеКлиентаДЖСОН(Контрагент)
	ОписаниеКлиента = Новый Структура("IDC,ClientDateBorn,ClientGender,ClientFirstName,ClientLastName,ClientMiddleName,ClientEmail,ClientPhone,VersionNo,Error,Message, Inactive");
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	"""" КАК IDC,
	               |	Контрагенты.ДатаРождения КАК ClientDateBorn,
	               |	ВЫБОР
	               ...
	               |				ИНАЧЕ """"
	               |			КОНЕЦ
	               |	КОНЕЦ КАК ClientGender,
	               |	ЕСТЬNULL(ФИОФизЛицСрезПоследних.Фамилия, """") КАК ClientLastName,
	               |	ЕСТЬNULL(ФИОФизЛицСрезПоследних.Имя, """") КАК ClientFirstName,
	               |	ЕСТЬNULL(ФИОФизЛицСрезПоследних.Отчество, """") КАК ClientMiddleName,
	               |	ЕСТЬNULL(КонтактнаяИнформацияЕмайл.АдресЭП, """") КАК ClientEmail,
	               |	ЕСТЬNULL(КонтактнаяИнформацияТелефон.НомерТелефона, """") КАК ClientPhone,
	               |	ЛОЖЬ КАК Error,
	               |	"""" КАК Message,
	               |	Контрагенты.Ссылка КАК Ссылка,
	               |	Контрагенты.ВерсияДанных КАК VersionNo,
	               |	Контрагенты.ПометкаУдаления КАК Inactive
	               |ИЗ
	               |	Справочник.Контрагенты КАК Контрагенты
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних КАК ФИОФизЛицСрезПоследних
	               |		ПО (ФИОФизЛицСрезПоследних.ФизЛицо = Контрагенты.Ссылка)
	...
	Если Рез.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(ОписаниеКлиента,Рез);
		ОписаниеКлиента.IDC = Строка(Контрагент.УникальныйИдентификатор());
		МассивДетей = Новый Массив;
		Для Каждого Строка_ Из Контрагент.Дети Цикл
			ПолРебенка = ?(Строка_.Пол = Перечисления.ПолФизическогоЛица.Мужской,"male",
							?(Строка_.Пол = Перечисления.ПолФизическогоЛица.Женский,"female",""));
			СтруктураРебенка = Новый Структура("ChildID,ChildFirstName,ChildGender,ChildDateBorn",
									Строка(Строка_.УникальныйИдентификатор),
									Строка_.Имя,
									ПолРебенка,
									Строка_.ДатаРождения);
			МассивДетей.Добавить(СтруктураРебенка);
				
		КонецЦикла;
		ОписаниеКлиента.Вставить("Children", МассивДетей);
	Иначе
		ОписаниеКлиента.Error = Истина;
		ОписаниеКлиента.Message = "Client with IDC " + Строка(Контрагент.УникальныйИдентификатор())+ " not found in database";
	КонецЕсли;
...
Функция СформироватьОписаниеКлиентаДЖСОН(ЗаписьJSON,СтрокаКлиенты)
	//ОписаниеКлиента = ПолучитьОписаниеКлиента(Контрагент);
	//Если ОписаниеКлиента.Количество() = 0 Тогда
	//	Возврат;
	//Иначе
	//	СтрокаКлиенты = ОписаниеКлиента[0];
	//КонецЕсли;
	...
	ИДКлиента = Строка(СтрокаКлиенты.Ссылка.УникальныйИдентификатор());
	РаботаСJSONСервер.ЗаписатьСвойство("IDC"				, ИДКлиента, ЗаписьJSON);
	//РаботаСJSONСервер.ЗаписатьСвойство("VersionNo"		, СтрокаКлиенты.ВерсияДанных, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("ClientPhone"		, СтрокаКлиенты.НомерТелефона, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("ClientEmail"		, СокрЛП(СтрокаКлиенты.ЭлектроннаяПочта), ЗаписьJSON);
	...
	РаботаСJSONСервер.ЗаписатьСвойство("ClientDateBorn"	, ОбменMagentoСервер.ФорматДаты(СтрокаКлиенты.ДатаРождения, ФорматДатыJSON.ISO), ЗаписьJSON);
	Если СтрокаКлиенты.Пол = Перечисления.ПолФизическогоЛица.Мужской Тогда
		ПолКлиента = "male";
	ИначеЕсли СтрокаКлиенты.Пол = Перечисления.ПолФизическогоЛица.Женский Тогда
		ПолКлиента = "female";
	Иначе
		ПолКлиента = "";
	КонецЕсли;
	
	РаботаСJSONСервер.ЗаписатьСвойство("ClientGender"	, ПолКлиента, ЗаписьJSON);

	РаботаСJSONСервер.ЗаписатьСвойство("Message"			, СтрокаКлиенты.ТекстСообщения, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("Error"			, СтрокаКлиенты.Ошибка, ЗаписьJSON);

	ЗаписьJSON.ЗаписатьИмяСвойства("Children");
	ЗаписьJSON.ЗаписатьНачалоМассива();
//	2. ---------------------------------------------------------------------------------------

	//Дети = ТаблицаДетей.НайтиСтроки(Новый Структура ("ID_Parent",ИДКлиента));
	Для каждого СтрокаРебенок Из СтрокаКлиенты.Ссылка.Дети Цикл
	
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
		РаботаСJSONСервер.ЗаписатьСвойство("ChildId"			, СтрокаРебенок.УникальныйИдентификатор, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("ChildFirstName"	, СтрокаРебенок.Имя, ЗаписьJSON);
		Если СтрокаРебенок.Пол = Перечисления.ПолФизическогоЛица.Мужской Тогда
			ПолРебенка = "male";
		ИначеЕсли СтрокаРебенок.Пол = Перечисления.ПолФизическогоЛица.Женский Тогда
			ПолРебенка = "female";
		Иначе
			ПолРебенка = "";
		КонецЕсли;
		РаботаСJSONСервер.ЗаписатьСвойство("ChildGender"		, ПолРебенка, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("ChildDateBorn"	, ОбменMagentoСервер.ФорматДаты(СтрокаРебенок.ДатаРождения, ФорматДатыJSON.ISO), ЗаписьJSON);
...
Функция ПолучитьКлиентовВФорматеJSONНов(ДанныеВходящие)
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
	...
	РаботаСJSONСервер.ЗаписатьСвойство("DIFF"	, СокрЛП(Новый УникальныйИдентификатор), ЗаписьJSON);
		...
		ТаблицаКлиентов = ДанныеВходящие.ТаблицаКлиентов;
		ТаблицаДетей = ДанныеВходящие.ТаблицаДетей;
		Для каждого СтрокаКлиенты Из ТаблицаКлиентов Цикл
		//	СформироватьОписаниеКлиентаДЖСОН(ЗаписьJSON,СтрокаКлиенты.Контрагент);
			
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
		//	1. ---------------------------------------------------------------------------------------
				ИДКлиента = ОбменMagentoСлужебный.IDC_Строкой(СтрокаКлиенты.Контрагент);
				РаботаСJSONСервер.ЗаписатьСвойство("IDC"				, ИДКлиента, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("ClientPhone"		, СтрокаКлиенты.НомерТелефона, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("ClientEmail"		, СокрЛП(СтрокаКлиенты.Электропочта), ЗаписьJSON);
			...
			//	2. ---------------------------------------------------------------------------------------
				...
				Дети = ТаблицаДетей.НайтиСтроки(Новый Структура ("ID_Parent",ИДКлиента));
				Для каждого СтрокаРебёнок Из Дети Цикл
				
					ЗаписьJSON.ЗаписатьНачалоОбъекта();
					РаботаСJSONСервер.ЗаписатьСвойство("ChildId"			, СтрокаРебёнок.РебёнокUUID, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("ChildFirstName"	, СтрокаРебёнок.РебёнокИмя, ЗаписьJSON);
					Если СтрокаРебёнок.РебёнокПол = Перечисления.ПолФизическогоЛица.Мужской Тогда
						ПолРебенка = "male";
					ИначеЕсли СтрокаРебёнок.РебёнокПол = Перечисления.ПолФизическогоЛица.Женский Тогда
						ПолРебенка = "female";
					Иначе
						ПолРебенка = "";
					КонецЕсли;
...
#Область КомплексПроцедуры_UpdateClient
Функция ОбновитьКлиента(Клиент)
		СтруктураКлиента = Новый Структура;
		КонтрагентСсылка = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(Клиент.IDC));
		//СтароеОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
		//ОписаниеКлиента = Новый Структура("IDC,Error,Message",Клиент.IDC,Ложь,"");
		ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
		СтароеОписаниеКлиента = ОписаниеКлиента;
		МассивДетей = Новый Массив;
		
		Если ОписаниеКлиента.Error Тогда
			//МассивКлиентов.Добавить(ОписаниеКлиента);
			Возврат ОписаниеКлиента;
		КонецЕсли;
		...
		Если КонтрагентОбъект = Неопределено Тогда
			ОписаниеКлиента.Error = Истина;
			ОписаниеКлиента.Message = "The Client has not found in database";
			Возврат ОписаниеКлиента;
		КонецЕсли;
		Если НЕ (Клиент.Свойство("Register") И  Клиент.Register = Истина) Тогда
			КонтрагентОбъект.ОбменДанными.Загрузка = Истина;
			КонтрагентОбъект.ОбменДанными.Получатели.АвтоЗаполнение = Ложь;
			КонтрагентОбъект.ДополнительныеСвойства.Вставить("ОтключитьМеханизмРегистрацииОбъектов", Истина);
		КонецЕсли;
		...
		Если Клиент.Свойство("ClientPhone")Тогда
			ДругойКлиент = Справочники.Контрагенты.НайтиПоТелефону(Клиент.ClientPhone, Перечисления.ЮрФизЛицо.ФизЛицо, Истина, Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента);
			Если ЗначениеЗаполнено(ДругойКлиент) И КонтрагентОбъект.Ссылка <> ДругойКлиент Тогда
				ОписаниеКлиента.Error = Истина;
				ОписаниеКлиента.Message = "The ClientPhone number is already registered to another client";
				//МассивКлиентов.Добавить(ОписаниеКлиента);
				Возврат ОписаниеКлиента;
			КонецЕсли;
			
		    ТекТелефон = КонтрагентОбъект.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид",Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента));
			Для Каждого СтрокаТелефон Из ТекТелефон Цикл
				Если СтрокаТелефон.НомерТелефона = СокрЛП(Клиент.ClientPhone) Тогда
					Продолжить;
				КонецЕсли;
				ИндексСтроки = КонтрагентОбъект.КонтактнаяИнформация.Индекс(СтрокаТелефон);
				КонтрагентОбъект.КонтактнаяИнформация.Удалить(ИндексСтроки);
			КонецЦикла;
			//Если этот номер уже зарегестрирован за клиентом, то просто пропускаем, ошибку не даем.
			Если КонтрагентОбъект.КонтактнаяИнформация.Найти(СокрЛП(Клиент.ClientPhone),"НомерТелефона") = Неопределено Тогда
			//	---------------------------------------------------------------------------------------
				СтрокаКонтакты = КонтрагентОбъект.КонтактнаяИнформация.Добавить();
				СтрокаКонтакты.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента;
				СтрокаКонтакты.Тип = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаКонтакты.Вид, "Тип");
				СтрокаКонтакты.НомерТелефона = СокрЛП(Клиент.ClientPhone);
				СтрокаКонтакты.Представление = СтрокаКонтакты.НомерТелефона;
				...
				Представление = Лев( СтрокаКонтакты.НомерТелефона,  3)
						...
						+ " " + Сред(СтрокаКонтакты.НомерТелефона, 11, 2);
				СтрокаКонтакты.ЗначенияПолей = ВерификацияКлиентовСервер.СформироватьДанныеТелефона(Представление, СтрокаКонтакты.НомерТелефонаБезКодов);
				ОписаниеКлиента.Вставить("ClientPhone",СтрокаКонтакты.НомерТелефона);
			Иначе	//если в контрагенте есть несколько телефонов, то дается первый из них. Если первым телефоном пустая строка, то выдается она. Поправим
				
			КонецЕсли;
			
			//СтруктураОтвета.Вставить("ClientPhone",Клиент.ClientPhone);
		КонецЕсли;
		Если Клиент.Свойство("ClientEmail")Тогда
		    ТекАдресЭП = КонтрагентОбъект.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид",Справочники.ВидыКонтактнойИнформации.EmailКонтрагента));
			Для Каждого СтрокаЭП Из ТекАдресЭП Цикл
				Если СтрокаЭП.АдресЭП = СокрЛП(Клиент.ClientEmail) Тогда
					Продолжить;
				КонецЕсли;
				ИндексСтроки = КонтрагентОбъект.КонтактнаяИнформация.Индекс(СтрокаЭП);
				КонтрагентОбъект.КонтактнаяИнформация.Удалить(ИндексСтроки);
			КонецЦикла;
			//Если этот емайл уже зарегестрирован за клиентом, то просто пропускаем, ошибку не даем.
			Если КонтрагентОбъект.КонтактнаяИнформация.Найти(СокрЛП(Клиент.ClientEmail),"АдресЭП") = Неопределено Тогда
			    ТекАдресЭП = КонтрагентОбъект.КонтактнаяИнформация.Найти(Справочники.ВидыКонтактнойИнформации.EmailКонтрагента,"Вид");
				Если ТекАдресЭП <>Неопределено Тогда
					ИндексСтроки = КонтрагентОбъект.КонтактнаяИнформация.Индекс(ТекАдресЭП);
					КонтрагентОбъект.КонтактнаяИнформация.Удалить(ИндексСтроки);
				КонецЕсли;
				СтрокаКонтакты = КонтрагентОбъект.КонтактнаяИнформация.Добавить();
				СтрокаКонтакты.Вид = Справочники.ВидыКонтактнойИнформации.EmailКонтрагента;
				СтрокаКонтакты.Тип = ОбщегоНазначения.ПолучитьЗначениеРеквизита(СтрокаКонтакты.Вид, "Тип");
				СтрокаКонтакты.АдресЭП = СокрЛП(Клиент.ClientEmail);
				СтрокаКонтакты.Представление = СокрЛП(Клиент.ClientEmail);
				
				ОписаниеКлиента.Вставить("ClientEmail",СтрокаКонтакты.АдресЭП);
			КонецЕсли;
			//СтруктураОтвета.Вставить("ClientEmail",Клиент.ClientEmail);
		КонецЕсли;
		Если Клиент.Свойство("ClientGender")Тогда
			КонтрагентОбъект.Пол = ?(НРег(Клиент.ClientGender) = "male"		, Перечисления.ПолФизическогоЛица.Мужской,
								   ?(НРег(Клиент.ClientGender) = "female"	, Перечисления.ПолФизическогоЛица.Женский
									  										, Перечисления.ПолФизическогоЛица.ПустаяСсылка()));
			ОписаниеКлиента.Вставить("ClientGender",Клиент.ClientGender);
			//СтруктураОтвета.Вставить("ClientGender",Клиент.ClientGender");
		КонецЕсли;
		
		НужнаЗаписьФИО = Ложь;
		Если Клиент.Свойство("ClientFirstName")Тогда
			Имя_ = СтроковыеФункцииКлиентСервер.ПривестиНаименование(Клиент.ClientFirstName, Истина, Истина);
			НужнаЗаписьФИО = Истина;
			//СтруктураОтвета.Вставить("ClientFirstName",Имя_);
		Иначе
			Имя_ = СтароеОписаниеКлиента.ClientFirstName;
		КонецЕсли;
		Если Клиент.Свойство("ClientLastName")Тогда
			Фамилия_ = СтроковыеФункцииКлиентСервер.ПривестиНаименование(Клиент.ClientLastName, Истина, Истина);
			НужнаЗаписьФИО = Истина;
			//СтруктураОтвета.Вставить("ClientLastName",Фамилия_);
		Иначе
			//Клиент.Вставить("ClientLastName",ОписаниеКлиента.ClientLastName);
			Фамилия_ = СтароеОписаниеКлиента.ClientLastName;
		КонецЕсли;
		Если Клиент.Свойство("ClientMiddleName")Тогда
			Отчество_ = СтроковыеФункцииКлиентСервер.ПривестиНаименование(Клиент.ClientMiddleName, Истина, Истина);
			НужнаЗаписьФИО = Истина;
			//СтруктураОтвета.Вставить("ClientMiddleName",Отчество_);
		Иначе
			Отчество_ = СтароеОписаниеКлиента.ClientMiddleName;
		КонецЕсли;
		Если Клиент.Свойство("ClientDateBorn")Тогда
			ДатаРождения_ =	 ОбменMagentoСервер.ПолучитьДату(Клиент.ClientDateBorn);
			//2024-11-21 sa хотят иметь возможность удалять дату рождения клиента.
			//Если Не ЗначениеЗаполнено(ДатаРождения_) И ЗначениеЗаполнено(КонтрагентОбъект.ДатаРождения) Тогда
			//	ОписаниеКлиента.Error = Истина;
			//	ОписаниеКлиента.Message = ОписаниеКлиента.Message+Символы.ПС+"ClientDateBorn is not valid format. Date must be in YYYY-MM-DD format";
			//	//МассивКлиентов.Добавить(ОписаниеКлиента);
			//	Возврат ОписаниеКлиента;
			//КонецЕсли;
			КонтрагентОбъект.ДатаРождения	= ДатаРождения_;
		КонецЕсли;
		
		Если НужнаЗаписьФИО Тогда
			МенеджерЗаписи = РегистрыСведений.ФИОФизЛиц.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.Период	= ТекущаяДата();
			МенеджерЗаписи.ФизЛицо	= КонтрагентСсылка;
			МенеджерЗаписи.Фамилия	= Фамилия_;
			МенеджерЗаписи.Имя		= Имя_;
			МенеджерЗаписи.Отчество	= Отчество_;
			МенеджерЗаписи.Записать();

			КонтрагентОбъект.Наименование		= СтроковыеФункцииКлиентСервер.ПривестиНаименование(Фамилия_ + " " + Имя_ + " " + Отчество_, Ложь, Истина);
			КонтрагентОбъект.НаименованиеПолное	= КонтрагентОбъект.Наименование;
			
			ОписаниеКлиента.Вставить("ClientFirstName",		Имя_);
			ОписаниеКлиента.Вставить("ClientLastName",		Фамилия_);
			ОписаниеКлиента.Вставить("ClientMiddleName",	Отчество_);
		КонецЕсли;
		
		Если Клиент.Свойство("Children")Тогда
			МассивДетей = Новый Массив;
			Дети = Клиент.Children;
			Для Каждого Ребенок Из Дети Цикл
				ОписаниеРебенка = Новый Структура;
				//2024-08-27 sa для изменения данных по ребенку нужно указать его УИД.
				//Если это добавление нового ребенка, то УИД указывать не надо, но имя ребенка должно отличаться от уже существующих (чтобы 1С не наплодило
				//детей пока кто-то тыкает запросом в сервис.) Вероятностью что у клиента несколько детей с одинаковыми именами пренебрегаем.
				Если Ребенок.Свойство("ChildId")Тогда
					СтрокаРебенок = КонтрагентОбъект.Дети.Найти(Ребенок.ChildId,"УникальныйИдентификатор");
					Если СтрокаРебенок = Неопределено Тогда
							ОписаниеКлиента.Error = Истина;
							ОписаниеКлиента.Message = ОписаниеКлиента.Message+Символы.ПС+"Children with id "+Ребенок.ChildId+" not found. To change data you must specify the ChildId.";
							//МассивКлиентов.Добавить(ОписаниеКлиента);
							//Продолжить;
							Возврат ОписаниеКлиента;
					КонецЕсли;
					ОписаниеРебенка.Вставить("ChildId",Ребенок.ChildId);	
				Иначе
						//2024-11-04 sa Если не задан ChildId, то обязательные поля: Имя, Дата Рождения, Пол.
						//Если не задано, досвидос.
					Если (Ребенок.Свойство("ChildFirstName") И Ребенок.Свойство("ChildDateBorn") И Ребенок.Свойство("ChildGender")) Тогда
					//Если Ребенок.Свойство("ChildFirstName")Тогда
						ПолРебенка =  ?(НРег(Ребенок.ChildGender) = "male"		, Перечисления.ПолФизическогоЛица.Мужской,
										   ?(НРег(Ребенок.ChildGender) = "female"	, Перечисления.ПолФизическогоЛица.Женский
											  										, Перечисления.ПолФизическогоЛица.ПустаяСсылка()));
						Попытка															
							ДатаРебенка = Дата(Ребенок.ChildDateBorn);
						Исключение
							ДатаРебенка = Дата("00010101");
						КонецПопытки;
						
						ОтборПоРебенку = Новый Структура("Имя,Пол,ДатаРождения",Ребенок.ChildFirstName,ПолРебенка,ДатаРебенка);
						НайденныеДети = КонтрагентОбъект.Дети.НайтиСтроки(ОтборПоРебенку);
						//если ребенок с таким именем уже есть, то для изменения данных по нему нужно указать его УИД.
						Если НайденныеДети.Количество()>0 Тогда
							ОписаниеКлиента.Error = Истина;
							ОписаниеКлиента.Message = ОписаниеКлиента.Message+Символы.ПС+"Children "+Ребенок.ChildFirstName+" already exists. To change data you must specify the ChildId.";
							//МассивКлиентов.Добавить(ОписаниеКлиента);
							Возврат ОписаниеКлиента;
						Иначе  //это новый ребенок, добавляем
							СтрокаРебенок = КонтрагентОбъект.Дети.Добавить();
							СтрокаРебенок.УникальныйИдентификатор = Новый УникальныйИдентификатор;
						КонецЕсли;
					Иначе
						ОписаниеКлиента.Error = Истина;
						ОписаниеКлиента.Message = ОписаниеКлиента.Message+Символы.ПС+"Your must specify the ChildId to change data or specify ChildFirstName, ChildDateBorn, ChildGender to add new child";
						Возврат ОписаниеКлиента;
					КонецЕсли;
				КонецЕсли;

				Если Ребенок.Свойство("ChildFirstName")Тогда
					СтрокаРебенок.Имя = СтроковыеФункцииКлиентСервер.ПривестиНаименование(Ребенок.ChildFirstName, Истина, Истина);
					ОписаниеРебенка.Вставить("ChildFirstName",СтрокаРебенок.Имя);	
				КонецЕсли;
				Если Ребенок.Свойство("ChildLastName")Тогда
					СтрокаРебенок.Фамилия = СтроковыеФункцииКлиентСервер.ПривестиНаименование(Ребенок.ChildLastName, Истина, Истина);
					ОписаниеРебенка.Вставить("ChildLastName",СтрокаРебенок.Фамилия);	
				КонецЕсли;
				Если Ребенок.Свойство("ChildMiddleName")Тогда
					СтрокаРебенок.Отчество = СтроковыеФункцииКлиентСервер.ПривестиНаименование(Ребенок.ChildMiddleName, Истина, Истина);
					ОписаниеРебенка.Вставить("ChildMiddleName",СтрокаРебенок.Отчество);	
				КонецЕсли;
				Если Ребенок.Свойство("ChildGender")Тогда
					СтрокаРебенок.Пол = ?(НРег(Ребенок.ChildGender) = "male"		, Перечисления.ПолФизическогоЛица.Мужской,
										   ?(НРег(Ребенок.ChildGender) = "female"	, Перечисления.ПолФизическогоЛица.Женский
											  										, Перечисления.ПолФизическогоЛица.ПустаяСсылка()));
					ОписаниеКлиента.Вставить("ClientGender",Ребенок.ChildGender);
					ОписаниеРебенка.Вставить("ClientGender",Ребенок.ChildGender);	
				КонецЕсли;
				Если Ребенок.Свойство("ChildDateBorn")Тогда
					ДатаРождения_ =	 ОбменMagentoСервер.ПолучитьДату(Ребенок.ChildDateBorn);
					Если Не ЗначениеЗаполнено(ДатаРождения_) И ЗначениеЗаполнено(СтрокаРебенок.ДатаРождения) Тогда
						ОписаниеКлиента.Error = Истина;
						ОписаниеКлиента.Message = ОписаниеКлиента.Message+Символы.ПС+"ChildDateBorn is not valid format. Date must be in YYYY-MM-DD format";
						//МассивКлиентов.Добавить(ОписаниеКлиента);
						Возврат ОписаниеКлиента;
					КонецЕсли;
					СтрокаРебенок.ДатаРождения	= ДатаРождения_;
					ОписаниеРебенка.Вставить("ChildDateBorn",Ребенок.ChildDateBorn);	
				КонецЕсли;
				
				МассивДетей.Добавить(ОписаниеРебенка);	
				
			КонецЦикла;
		КонецЕсли;
		Если ОписаниеКлиента.Error Тогда  //где-то на этапе изменения данных детей возникла ошибка. Запись отменяем.
			Возврат ОписаниеКлиента;
		КонецЕсли; 
		
		Если Клиент.Свойство("MarketingGroup") Тогда
			Если НЕ Клиент.MarketingGroup.Найти("emalaytko") = Неопределено Тогда
				Если КонтрагентОбъект.МаркетинговыеГруппы.Найти(Справочники.МаркетинговыеГруппы.Е_Малятко,"Группа") = Неопределено Тогда
					СтрокаГруппа = КонтрагентОбъект.МаркетинговыеГруппы.Добавить();
					СтрокаГруппа.Группа = Справочники.МаркетинговыеГруппы.Е_Малятко;
				КонецЕсли;
			КонецЕсли;	
		КонецЕсли;	
		
		//2024-09-29 sa хотят чтобы все клиенты к которым прикоснулась Маджента превращалось в ..клиентов мадженты
		Если КонтрагентОбъект.МаркетинговыеГруппы.Найти(Справочники.МаркетинговыеГруппы.М2,"Группа") = Неопределено Тогда
			СтрокаГруппа = КонтрагентОбъект.МаркетинговыеГруппы.Добавить();
			СтрокаГруппа.Группа = Справочники.МаркетинговыеГруппы.М2;
		КонецЕсли;
		Попытка
			Для Каждого Ребенок Из КонтрагентОбъект.Дети Цикл
				Если Не ЗначениеЗаполнено(Ребенок.УникальныйИдентификатор) Тогда
					Ребенок.УникальныйИдентификатор = Новый УникальныйИдентификатор;	
				КонецЕсли;
			КонецЦикла;
			...
			КонтрагентОбъект.Записать();
			//2024-11-20 sa добавляем создание виртуальной ДК (если ДК еще не создано)
			Если Клиент.Свойство("ClientPhone") Тогда
				ТЛФ = Клиент.ClientPhone;
			Иначе
				ТЛФ = СтароеОписаниеКлиента.ClientPhone;
			КонецЕсли;
			СоздатьДисконтнуюКарту(КонтрагентОбъект.Ссылка,ТЛФ);
			
		Исключение
			ОписаниеКлиента.Error = Истина;
			ОписаниеКлиента.Message = ОписаниеКлиента.Message+Символы.ПС+ОписаниеОшибки();
			//Возврат ОписаниеКлиента;
		КонецПопытки;
		//Еще раз прочитаем актуальное описание клиента
		ОписаниеКлиентаНов = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентОбъект.Ссылка);
		
		//2024-12-02 sa после долгих раздумий решили, что нужно возвращать все поля.
		Возврат ОписаниеКлиентаНов;
		
		//Возвращаем только те поля которые изменились (были в исходном запросе)
		ОписаниеКлиента = Новый Структура("IDC,Error,Message","",ОписаниеКлиента.Error,ОписаниеКлиента.Message);
		Для Каждого КлючИЗначение Из Клиент Цикл
			НовоеЗначение = "";
			Если ТипЗнч(КлючИЗначение.Значение) = Тип("Структура") Тогда
				ВложеннаяСтруктура = Новый Структура;
				ВложеннаяСтруктураНов = Новый Структура;
				ОписаниеКлиентаНов.Свойство(КлючИЗначение.Ключ,ВложеннаяСтруктураНов);
				Если ВложеннаяСтруктураНов = Неопределено Тогда
					ВложеннаяСтруктураНов = Новый Структура;
				КонецЕсли;
				Для Каждого КлючИЗначение2 Из КлючИЗначение.Значение Цикл
					НовоеЗначение = "";
					ОписаниеКлиентаНов.Свойство(КлючИЗначение2.Ключ,НовоеЗначение);
					ВложеннаяСтруктура.Вставить(КлючИЗначение2.Ключ,НовоеЗначение);
				КонецЦикла;
				ОписаниеКлиента.Вставить(КлючИЗначение.Ключ,ВложеннаяСтруктура);
			ИначеЕсли КлючИЗначение.Ключ = "Children" Тогда
				ОписаниеКлиента.Вставить(КлючИЗначение.Ключ,МассивДетей);
			Иначе
				ОписаниеКлиентаНов.Свойство(КлючИЗначение.Ключ,НовоеЗначение);
				ОписаниеКлиента.Вставить(КлючИЗначение.Ключ,НовоеЗначение);
			КонецЕсли;
		КонецЦикла;
...
Функция UpdateClient(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	...
	МассивКлиентов = Новый Массив;
	Для Каждого Клиент Из ДанныеЗапроса.Clients Цикл
		ОписаниеКлиента = ОбновитьКлиента(Клиент);
		МассивКлиентов.Добавить(ОписаниеКлиента);
	КонецЦикла;
	СтруктураОтвета.Вставить("Clients",МассивКлиентов);
...
#Область КомплексПроцедуры_DeleteClientKid
Функция DeleteClientKid(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	...
	Для Каждого Клиент Из ДанныеЗапроса.Clients Цикл
		...
		Если Клиент.Свойство("Children")Тогда
			...
			Для Каждого Ребенок Из Дети Цикл
				Если Ребенок.Свойство("ChildId")Тогда
					...
					Если СтрокаРебенок = Неопределено Тогда
						ОписаниеКлиента.Error = Истина;
						ОписаниеКлиента.Message = СокрЛП(ОписаниеКлиента.Message+Символы.ПС+"Children with ChildID "+Ребенок.ChildId+" not found in client card");
					Иначе
						ИндексСтроки = КонтрагентОбъект.Дети.Индекс(СтрокаРебенок);
						КонтрагентОбъект.Дети.Удалить(ИндексСтроки);
					КонецЕсли;
				Иначе
					ОписаниеКлиента.Error = Истина;
					ОписаниеКлиента.Message = СокрЛП(ОписаниеКлиента.Message+Символы.ПС+"ChildID is requid");
				КонецЕсли;
		...
		Исключение
			ОписаниеКлиента.Error = Истина;
			ОписаниеКлиента.Message = СокрЛП(ОписаниеКлиента.Message+Символы.ПС+ОписаниеОшибки());
			МассивКлиентов.Добавить(ОписаниеКлиента);
			Продолжить;
		КонецПопытки;
		//Еще раз прочитаем актуальное описание клиента
		ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентОбъект.Ссылка);
		МассивКлиентов.Добавить(ОписаниеКлиента);
	КонецЦикла;
	СтруктураОтвета.Вставить("Clients",МассивКлиентов);
...
#Область КомплексПроцедуры_DeleteClient
Функция DeleteClient(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	...
	КлиентИД = ДанныеЗапроса.IDC;
	СтруктураКлиента = Новый Структура;
	КонтрагентСсылка = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(КлиентИД));
	ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
	Если ОписаниеКлиента.Error Тогда
		СтруктураОтвета.Вставить("Error", Истина);
		СтруктураОтвета.Вставить("Message", "Client with ID "+КлиентИД+" was not found in database");
		ТекстДЖСОН = ОбменWebRetailСервер.СтруктуруВТекстДЖСОН(СтруктураОтвета);
		Возврат ТекстДЖСОН;
	КонецЕсли;
	
	ОтветУдаления = КонтактнаяИнформацияСервер.ОчиститьДанныеКлиента(КонтрагентСсылка);
	Если ОтветУдаления.Успешно Тогда
		СтруктураОтвета.Вставить("Message", "Client with ID "+КлиентИД+" was successfuly deleted from database");
	Иначе
		СтруктураОтвета.Вставить("Error", Истина);
		СтруктураОтвета.Вставить("Message", ОтветУдаления.ОписаниеОшибки);
	КонецЕсли;
...
#Область КомплексПроцедуры_AcceptedClient
Функция AcceptedClient(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	...
	МассивКлиентов = Новый Массив;
	Для Каждого Клиент Из ДанныеЗапроса.Clients Цикл
		СтруктураКлиента = Новый Структура("IDC,Error,Message");
		
		КонтрагентСсылка = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(Клиент.IDC));
		ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
		ЗаполнитьЗначенияСвойств(СтруктураКлиента,ОписаниеКлиента);
		Если Не ОписаниеКлиента.Error Тогда
			СтруктураКлиента.Message = "SUCCESS_ACCEPTED";
			МассивПринятых.Добавить(КонтрагентСсылка);
		КонецЕсли;
		МассивКлиентов.Добавить(ОписаниеКлиента);
	КонецЦикла;
	ОбменMagentoСервер.УдалитьРегистрациюОбъектов(МассивПринятых);	
	СтруктураОтвета.Вставить("Clients",МассивКлиентов);
...
Функция GetClients(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	...
	МассивКлиентов = Новый Массив;
	Для Каждого СтрокаТаблицы Из ДанныеЗапроса.Clients Цикл
		КонтрагентСсылка = Неопределено;
		КлиентНайден = Ложь;
		Если ЗначениеЗаполнено(СтрокаТаблицы.IDC) Тогда
			КонтрагентСсылкаПоИД = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрокаТаблицы.IDC));
			ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылкаПоИД);
			Если ОписаниеКлиента.Error = Ложь Тогда
				МассивКлиентов.Добавить(ОписаниеКлиента);
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
		Если СтрокаТаблицы.Свойство("ClientPhone") И ЗначениеЗаполнено(СтрокаТаблицы.ClientPhone) Тогда
			КонтрагентСсылка = Справочники.Контрагенты.НайтиПоТелефону(СтрокаТаблицы.ClientPhone, Перечисления.ЮрФизЛицо.ФизЛицо, Истина, Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента);
			ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
			Если ОписаниеКлиента.Error Тогда
				//Даем ошибку
				ОписаниеКлиента.Message = ОписаниеКлиента.Message + ?(ПустаяСтрока(ОписаниеКлиента.Message), "", Символы.ПС)
					+ "Client with Phone " + СтрокаТаблицы.ClientPhone + " not found in database.";
			КонецЕсли;
		КонецЕсли;
		МассивКлиентов.Добавить(ОписаниеКлиента);
		//СформироватьОписаниеКлиентаДЖСОН(ЗаписьJSON,ОписаниеКлиента);
	КонецЦикла;
	СтруктураОтвета.Вставить("Clients",МассивКлиентов);
...
#Область КомплексПроцедуры_GetChanges
Функция ПолучитьИзмененияПоКлиентам()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 10000
	               |	КонтрагентыИзменения.Ссылка КАК Ссылка
	               |ИЗ
	               |	Справочник.Контрагенты.Изменения КАК КонтрагентыИзменения
	               |ГДЕ
	               |	КонтрагентыИзменения.Узел В (&Узлы)";
	Получатели = ОбменMagentoСервер.ОпределитьПолучателейМаженто2();
	Запрос.УстановитьПараметр("Узлы",Получатели);
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
КонецФункции

Процедура СохранитьЗначениеКеша(КэшКлиентов)
	
	ИмяФайла_ = КаталогВременныхФайлов()+"\GetChanges_cache.xml";
	ЗаписьДЖСОН = Новый ЗаписьJSON;
	ПараметрыЗаписиДЖСОН = Новый ПараметрыЗаписиJSON(,Символы.Таб);
	ЗаписьДЖСОН.ОткрытьФайл(ИмяФайла_,"UTF-8",,ПараметрыЗаписиДЖСОН);
	СериализаторXDTO.ЗаписатьJSON(ЗаписьДЖСОН,КэшКлиентов,НазначениеТипаXML.Явное);
	ЗаписьДЖСОН.Закрыть();
	
КонецПроцедуры

Функция ВосстановитьЗначениеКеша()
	ИмяФайла_ = КаталогВременныхФайлов()+"\GetChanges_cache.xml";
	ФайлКэша = Новый Файл(ИмяФайла_);
	Если Не ФайлКэша.Существует() Тогда
		Возврат Неопределено;
	КонецЕсли;

	ЧтениеДЖСОН = Новый ЧтениеJSON;
	ЧтениеДЖСОН.ОткрытьФайл(ИмяФайла_,"UTF-8");
	КэшКлиентов = СериализаторXDTO.ПрочитатьJSON(ЧтениеДЖСОН);
	ЧтениеДЖСОН.Закрыть();
	
	Если ТипЗнч(КэшКлиентов) <> Тип("Структура") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция GetChanges(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	Если ДанныеЗапроса.Свойство("Page") Тогда
		Попытка
			НомерСтраницы = Число(ДанныеЗапроса.Page);
		Исключение
			НомерСтраницы = 1;
		КонецПопытки;
	Иначе
	...
	Если ДанныеЗапроса.Свойство("PageSize") Тогда
		Попытка
			ЧислоЗаписейНаСтранице = Число(ДанныеЗапроса.PageSize);
		Исключение
			ЧислоЗаписейНаСтранице = 10;
		КонецПопытки;
	Иначе
		ЧислоЗаписейНаСтранице = 10;
	КонецЕсли;
	Если ДанныеЗапроса.Свойство("DIFF") Тогда
		Дифф = ДанныеЗапроса.DIFF;
		КэшКлиентов = ВосстановитьЗначениеКеша();
		Если КэшКлиентов.УИДКеша <> Дифф Тогда  //Переданный DIFF не совпадает с УИД кеша, даем ошибку
			СтруктураОтвета.Вставить("IDBASE"	, ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"));
			СтруктураОтвета.Вставить("DIFF"		, Строка(Новый УникальныйИдентификатор));
			СтруктураОтвета.Вставить("Error"	, Истина);
			СтруктураОтвета.Вставить("Message"	, "The specified DIFF does not exist!");
			ТекстДЖСОН = ОбменWebRetailСервер.СтруктуруВТекстДЖСОН(СтруктураОтвета);
			
			Возврат ТекстДЖСОН;
		КонецЕсли;
	Иначе
		КэшКлиентов = Неопределено;
	КонецЕсли;
	//Делаем пагинацию. Получаем клиентов и кэшируем ее между сеансами. Если с момента создания кэша прошло больше часа, получаем его снова.
	Если КэшКлиентов = Неопределено Тогда
		ИзмененныеКлиенты = ПолучитьИзмененияПоКлиентам();
		КэшКлиентов = Новый Структура("МассивКлиентов,Дата,УИДКеша",ИзмененныеКлиенты,ТекущаяДата(),Строка(Новый УникальныйИдентификатор));
		СохранитьЗначениеКеша(КэшКлиентов);
	КонецЕсли;
	ВсегоКлиентов = КэшКлиентов.МассивКлиентов.Количество();
	ВсегоСтраниц = Цел(ВсегоКлиентов/ЧислоЗаписейНаСтранице) + ?(Цел(ВсегоКлиентов/ЧислоЗаписейНаСтранице)<>ВсегоКлиентов/ЧислоЗаписейНаСтранице,1,1);
	Если НомерСтраницы > ВсегоСтраниц Тогда
		НомерСтраницы = ВсегоСтраниц;
	КонецЕсли;
	//ДанныеВходящие = ПолучитьТаблицуВходящихДанных(ДанныеЗапроса);
	СтруктураОтвета.Вставить("IDBASE"	, ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"));
	СтруктураОтвета.Вставить("DIFF"	, КэшКлиентов.УИДКеша);
	СтруктураОтвета.Вставить("Page"	, НомерСтраницы);
	СтруктураОтвета.Вставить("TotalPages"	, ВсегоСтраниц);

	МассивКлиентов = Новый Массив;
	ПерваяЗапись = (НомерСтраницы-1) * ЧислоЗаписейНаСтранице;
	Если ПерваяЗапись < 0 Тогда
		ПерваяЗапись = 0;
	КонецЕсли;
	ПоследняяЗапись = Мин(ПерваяЗапись + ЧислоЗаписейНаСтранице,ВсегоКлиентов);
	Если ПоследняяЗапись < 0 Тогда
		ПоследняяЗапись = 1;
	КонецЕсли;
	Для н = ПерваяЗапись По ПоследняяЗапись-1 Цикл
		КонтрагентСсылка = КэшКлиентов.МассивКлиентов.Получить(н);
		ОписаниеКлиента = ПолучитьОписаниеКлиентаДЖСОН(КонтрагентСсылка);
		МассивКлиентов.Добавить(ОписаниеКлиента);
	КонецЦикла;
	СтруктураОтвета.Вставить("Clients",МассивКлиентов);
...
Функция GetClients_НЕ_Используется(ДанныеЗапроса)
	...
	Для Каждого СтрокаТаблицы Из ДанныеЗапроса.Clients Цикл
		...
		КлиентНайден = Ложь;
		Если ЗначениеЗаполнено(СтрокаТаблицы.IDC) Тогда
			КонтрагентСсылка = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрокаТаблицы.IDC));
			ОписаниеКлиента = ПолучитьОписаниеКлиента(КонтрагентСсылка);
			Если Не ЗначениеЗаполнено(ОписаниеКлиента.Ссылка) Тогда
				//Даем ошибку
				ОписаниеКлиента.Ошибка		 = Истина;
				ОписаниеКлиента.ТекстСообщения = "Client with IDC " + СтрокаТаблицы.IDC + " not found in database.";
			Иначе
				КлиентНайден = Истина;
			КонецЕсли;
		КонецЕсли;	
		Если КлиентНайден = Ложь И  СтрокаТаблицы.Свойство("ClientPhone") И ЗначениеЗаполнено(СтрокаТаблицы.ClientPhone) Тогда
			КонтрагентСсылка = Справочники.Контрагенты.НайтиПоТелефону(СтрокаТаблицы.ClientPhone, Перечисления.ЮрФизЛицо.ФизЛицо, Истина, Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента);
			ОписаниеКлиента = ПолучитьОписаниеКлиента(КонтрагентСсылка);
			Если Не ЗначениеЗаполнено(ОписаниеКлиента.Ссылка) Тогда
				//Даем ошибку
				ОписаниеКлиента.Ошибка		 = Истина;
				ОписаниеКлиента.ТекстСообщения = ОписаниеКлиента.ТекстСообщения + ?(ПустаяСтрока(ОписаниеКлиента.ТекстСообщения), "", Символы.ПС)
					+ "Client with Phone " + СтрокаТаблицы.ClientPhone + " not found in database.";
			Иначе    //значит клиент найден не по переданному IDC а по номеру телефона. Уведомим об этом клиента
				ОписаниеКлиента.Ошибка		 = Истина;
				ОписаниеКлиента.ТекстСообщения = "Client with IDC " + СтрокаТаблицы.IDC + " not found in database. But found by phone number";
			КонецЕсли;
		КонецЕсли;
...
Функция GetRegisteredBalances(RequestData)
	...
	Если ОбменMagentoПовтИсп.ОбменАктивен() Тогда
					...
					Если НЕ СоставОшибки.Ошибка = Истина Тогда
						...
						Если ДанныеЗапроса.Command = "full_registration" Тогда
							...
							//	Выполнить полную регистрацию всех товарных остатков и резервов по заказам покупателей.
								...
								РаботаСJSONСервер.ЗаписатьСвойство("IDP", ДанныеЗапроса.IDP, ЗаписьJSON);
		...
		//	Например, номер сообщения какой "левый"
			РаботаСJSONСервер.ЗаписатьСвойство("Error", РаботаСJSONСервер.БулевоКакЧисло(СоставОшибки.Ошибка), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("ErrorCode", СоставОшибки.КодОшибки, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("ErrorMessage", СоставОшибки.ОписаниеОшибки, ЗаписьJSON);
...
Функция GetDistributedBonus(RequestData)	//	LNK 06.11.2021 05:35:13
	...
	//	ДокументОбъект = ПолучитьДокументMagento(ДанныеЗапроса.OrderData, "ЧекККМ", Истина, УчетнаяПолитика, СоставОшибки);
		...
		//Если ДанныеЗапроса.OrderData.Свойство("SummaBonusPay") И ДанныеЗапроса.OrderData.SummaBonusPay > 0 Тогда
		Если ДанныеЗапроса.OrderData.Свойство("SummaBonusPay") Тогда
			...
			ДанныеРаспределения = БонусныеБаллыВызовСервера.РаспределитьСуммуБонусныхБалловМадженто(ДокументОбъект
			...
			//2024-11-12 Sa если есть погашение скидочных купонов, то нужно пересчитать скидки.
			Если НЕ ДокументОбъект.ОплатаБонуснымиБаллами.Количество() = 0
					ИЛИ ДокументОбъект.ПогашениеСкидочныхКупонов.Количество()>0 
					ИЛИ ДокументОбъект.ПогашениеПодарочныхСертификатов.Количество()>0 
					Тогда
		...
		ЗаписатьПодарочныеСертификатыВJSON(ДанныеЗапроса.OrderData, ЗаписьJSON);
	...
	РаботаСJSONСервер.ЗаписатьСвойство("Error"		, РаботаСJSONСервер.БулевоКакЧисло(СоставОшибки.Отказ), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("ErrorMessage", СоставОшибки.ОписаниеОшибки, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("ErrorType"	, СоставОшибки.СтатусОшибки, ЗаписьJSON);
...
Функция GetBuyerExtendedInfo(RequestData)	//	LNK 27.09.2022 08:12:07
	...
	ДанныеОтвета.Error			= РаботаСJSONСервер.БулевоКакЧисло(СоставОшибки.Отказ);
...
Функция GetDiscountCalc(RequestData)	//	LNK 08.06.2021 07:07:47
	...
	Попытка
		...
		ЧтениеJSON = Новый ЧтениеJSON;
		ЧтениеJSON.УстановитьСтроку(RequestData);
		ДанныеДЖСОН = ПрочитатьJSON(ЧтениеJSON, Ложь,,, "ПолучитьДатуОтMagento", ОбменMagentoСервер,, ОбменMagentoСервер.ИменаТипаДата());

		//2026-05-06 sa небольшой патч для КСО. Они вместо IDN передают GUID Товара. Нужно преобразовать его в IDN
		ЭтоКСО = Ложь;
		Для Каждого Строка_ Из ДанныеДЖСОН.OrderData.Products Цикл
			Если СтрДлина(Строка_.IDN) = 36 Тогда
				Товар = Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(Строка_.IDN));
				Строка_.IDN = Товар.IDN;
				//2026-06-22 sa Выяснилась, что признак акционного товара внешняя система обязана присылать сама
				//(это должно ханится в карточке товара внешней системы). В КСО такого признака нет, поэтому нам нужно
				//его вычислять походу из базы.
				Строка_.Action = ОбменДаннымиКСОСервер.ПолучитьПризнакАкционностиТовара(Товар); 
				ЭтоКСО = Истина;
			КонецЕсли;
		КонецЦикла;

		ДанныеЗапроса = УточнитьДанныеВнешнегоЗапроса(ДанныеДЖСОН);
		
		//2026-08-31 sa нужно отличать источник данных КСО или Интернет-магазин
		Если ЭтоКСО Тогда
			ДанныеЗапроса.OrderData.Вставить("SourceData","KSO");
		КонецЕсли;
		...
		Попытка
			...
			КупонАктуален = Истина; //если купона нет, то считаем, что по этой теме все валидно.
			//2026-02-04 sa проверяем валидность купона.
			Если ДанныеЗапроса.OrderData.Свойство("CouponsRedeemed") И ДанныеЗапроса.OrderData.CouponsRedeemed.Количество() > 0 Тогда
				//2026-02-06 sa у команды М2 купон может быть задан, но вместо номера купона передается null
				//КупонАктуален = Ложь;  //если купон задан, то по умолчанию он не валиден
				//М2 говорит, что у них всегда один купон. Но если вдруг назначат несколько, то применять будут только ПОСЛЕДНИЙ
				Для Каждого СтруктураКупона Из ДанныеЗапроса.OrderData.CouponsRedeemed Цикл
					Если СтруктураКупона.Свойство("Type") 
							И СтруктураКупона.Type = "Coupon" 
							И СтруктураКупона.Свойство("SerialNumber") 
							И ЗначениеЗаполнено(СтруктураКупона.SerialNumber) Тогда
						КупонАктуален = ПроверитьКупон(СтруктураКупона.SerialNumber);
					КонецЕсли;
				КонецЦикла;
				//тут же М2 просят вообще НЕ возвращать купон, если он не активен. 
				Если КупонАктуален = Ложь Тогда
					ДанныеЗапроса.OrderData.CouponsRedeemed.Очистить();		
				КонецЕсли;
			КонецЕсли;
			...
			//	ДокументОбъект = ПолучитьДокументMagento(ДанныеЗапроса.OrderData, "ЧекККМ", Истина, УчетнаяПолитика, СоставОшибки);
				...
				ИспользоватьПрограммуЛояльности = Справочники.Контрагенты.ЯвляетсяСотрудником(ДокументОбъект.ВладелецДисконтнойКарты);
				...
				//	и вот переходим к расчёту скидок!
                    ...
                    //2025-07-07 sa включаем программу лояльности для сотрудников в расчет
					...
					Попытка
						...
						//2026-02-04 sa Если купон существует, но на данный момент он невалиден, то договорились, что дальше расчет скидок не проводим.
						//Если купона нет, то переменная КупонАктуален = Истина.  
						//2026-02-06 sa команда рукожопов переиграла задачу, теперь будем пересчитывать купоны всегда.
						//Если КупонАктуален Тогда 
						...
						ЗаписьJSON.ЗаписатьНачалоОбъекта();
							Если ДанныеЗапроса.OrderData.Свойство("SummaBonusPay") Тогда
								РаботаСJSONСервер.ЗаписатьСвойство("SummaBonusPay" , ДанныеЗапроса.OrderData.SummaBonusPay,ЗаписьJSON);
							Иначе	
								РаботаСJSONСервер.ЗаписатьСвойство("SummaBonusPay" , 0,ЗаписьJSON);
							КонецЕсли;
							ЗаписатьРасчётСкидкиВJSON(ДокументОбъект, ЗаписьJSON, Ложь);
							//Запись данных о подарочных сертификатах
							ЗаписатьПодарочныеСертификатыВJSON(ДанныеЗапроса.OrderData, ЗаписьJSON);
			...
			Иначе
				...
				Если ДанныеЗапроса.OrderData.BuyerSpecified Тогда
					...
					ЗаписьJSON.ЗаписатьНачалоОбъекта();
						...
						//РаботаСJSONСервер.ЗаписатьСвойство("cardNumber"	, ДанныеЗапроса.OrderData.Buyer.cardNumber, ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("phoneNumber"	, ДанныеЗапроса.OrderData.Buyer.phoneNumber, ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("email"		, ДанныеЗапроса.OrderData.Buyer.email, ЗаписьJSON);

						РаботаСJSONСервер.ЗаписатьСвойство("IDC"   , ДанныеЗапроса.OrderData.Buyer.IDC, ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("IDMGT" , ДанныеЗапроса.OrderData.Buyer.IDMGT, ЗаписьJSON);
						//РаботаСJSONСервер.ЗаписатьСвойство("EDRPOU", ДанныеЗапроса.OrderData.Buyer.EDRPOU, ЗаписьJSON);
...
//	так и обычным объектом.. например, при выгрузке. Учитываем такой факт.
	...
	РаботаСJSONСервер.ЗаписатьСвойство("Number" , ДокументОбъект.Номер, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("Date"   , Формат(ДокументОбъект.Дата, "ДФ='yyyy/MM/dd ЧЧ:мм:сс'"), ЗаписьJSON);
	

	Если НЕ СокращенныйВариант = Истина Тогда

		РаботаСJSONСервер.ЗаписатьСвойство("Company", СокрЛП(РеквизитыШапки.КодПоЕДРПОУ), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("Depot"  , СокрЛП(РеквизитыШапки.IDNМагазина), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("TaxInclud" 	 , РаботаСJSONСервер.БулевоКакЧисло(ДокументОбъект.ЦенаВключаетНДС), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("CreditProgram", ОбменMagentoСлужебный.IDC_Строкой(ДокументОбъект.УслугаБанка), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("CreditDepth"  , ?(ДокументОбъект.УслугаБанка.Пустая(), 0, ДокументОбъект.УслугаБанкаПериодПредоставления), ЗаписьJSON);
	...
	//	---------------------------------------------------------------------------------------
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			//РаботаСJSONСервер.ЗаписатьСвойство("cardNumber"	, СокрЛП(РеквизитыШапки.КодКарты), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("phoneNumber"	, СокрЛП(РеквизитыШапки.НомерТелефона), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("email"		, СокрЛП(РеквизитыШапки.Почта), ЗаписьJSON);

			РаботаСJSONСервер.ЗаписатьСвойство("IDC"   , ОбменMagentoСлужебный.IDC_Строкой(РеквизитыШапки.Контрагент), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("IDMGT" , "", ЗаписьJSON);
			//РаботаСJSONСервер.ЗаписатьСвойство("EDRPOU", СокрЛП(РеквизитыШапки.КонтрагентКодПоЕДРПОУ), ЗаписьJSON);
	...
	//	---------------------------------------------------------------------------------------
		...
		Для каждого СтрокаТабличнойЧасти Из ДокументОбъект.Товары Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
				...
				РаботаСJSONСервер.ЗаписатьСвойство("KeyLink"		, СтрокаТабличнойЧасти.КлючСвязи, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("KeyLinkBonus", СтрокаТабличнойЧасти.КлючСвязиБонусныхБаллов, ЗаписьJSON);

				РаботаСJSONСервер.ЗаписатьСвойство("Action"  , РаботаСJSONСервер.БулевоКакЧисло(СтрокаТабличнойЧасти.АкционнаяЦена), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("IDN"		, СокрЛП(РеквизитыТоваров.Получить(СтрокаТабличнойЧасти.Номенклатура).IDN), ЗаписьJSON);

				РаботаСJSONСервер.ЗаписатьСвойство("Price"	, СтрокаТабличнойЧасти.Цена, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("Count"	, СтрокаТабличнойЧасти.КоличествоУпаковок, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("Summa"	, СтрокаТабличнойЧасти.Сумма, ЗаписьJSON);

				РаботаСJSONСервер.ЗаписатьСвойство("SummaAutoDiscount"	, СтрокаТабличнойЧасти.СуммаАвтоматическойСкидки, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("SummaManualDiscount"	, СтрокаТабличнойЧасти.СуммаРучнойСкидки, ЗаписьJSON);

				РаботаСJSONСервер.ЗаписатьСвойство("SummaBonusAccrue", СтрокаТабличнойЧасти.СуммаБонусныхБалловНачислено, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("SummaBonusPay"	, СтрокаТабличнойЧасти.СуммаБонусныхБалловСписано, ЗаписьJSON);

				Если НЕ СокращенныйВариант = Истина Тогда

					РаботаСJSONСервер.ЗаписатьСвойство("TaxSumma", СтрокаТабличнойЧасти.СуммаНДС, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("TaxRate"	, НДСОбщегоНазначенияСервер.ПолучитьСтавкуНДСЧислом(СтрокаТабличнойЧасти.СтавкаНДС) * 100, ЗаписьJSON);

					ДанныеУпаковки = РеквизитыТоваров.Получить(СтрокаТабличнойЧасти.Упаковка);
					РаботаСJSONСервер.ЗаписатьСвойство("PackName", ДанныеУпаковки.Наименование, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("PackRate", ДанныеУпаковки.Коэффициент, ЗаписьJSON);
	...
	//	---------------------------------------------------------------------------------------
		...
		Для каждого СтрокаТабличнойЧасти Из ПодготовитьСкидкиНаценкиДляВыгрузки(ДокументОбъект.СкидкиНаценки) Цикл
			...
			РаботаСJSONСервер.ЗаписатьСвойство("KeyLink"			, СтрокаТабличнойЧасти.КлючСвязи, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("DiscountType"	, ОбщегоНазначенияКлиентСервер.РеквизитПеречисления(СтрокаТабличнойЧасти.СпособПредоставления), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("Discount"		, ОбменMagentoСлужебный.IDC_Строкой(СтрокаТабличнойЧасти.СкидкаНаценка), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("DiscountName"	, СокрЛП(СтрокаТабличнойЧасти.СкидкаНаценка), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("TypeApplication"	, ОбменMagentoПовтИсп.СпособПримененияСкидкиИдентификатор(СтрокаТабличнойЧасти.СпособПримененияСкидки), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("LoyaltyProgram"	, ОбменMagentoСлужебный.IDC_Строкой(СтрокаТабличнойЧасти.ПрограммаЛояльности), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("Summa"			, СтрокаТабличнойЧасти.Сумма, ЗаписьJSON);
	...
	//	---------------------------------------------------------------------------------------
		...
		Для каждого СтрокаТабличнойЧасти Из ДокументОбъект.НачислениеБонусныхБаллов Цикл
			...
			РаботаСJSONСервер.ЗаписатьСвойство("LoyaltyProgram"		, ОбменMagentoСлужебный.IDC_Строкой(СтрокаТабличнойЧасти.ПрограммаЛояльности), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("LoyaltyProgramName"	, СокрЛП(СтрокаТабличнойЧасти.ПрограммаЛояльности), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("TypeBonus"			, ОбменMagentoПовтИсп.ТипБонусовИдентификатор(СтрокаТабличнойЧасти.ПрограммаЛояльности), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("DateAccrual"			, Формат(СтрокаТабличнойЧасти.ДатаНачисления, "ДФ=yyyy/MM/dd"), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("DateCancel"			, Формат(СтрокаТабличнойЧасти.ДатаСписания, "ДФ=yyyy/MM/dd"), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("Summa"				, СтрокаТабличнойЧасти.СуммаБонусныхБаллов, ЗаписьJSON);
	...
	//	---------------------------------------------------------------------------------------
		...
		Для каждого СтрокаТабличнойЧасти Из ДокументОбъект.ВыдачаСкидочныхКупонов Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();	//	Элемент массива: Начало
				...
				РаботаСJSONСервер.ЗаписатьСвойство("Type"		, ТипКупона, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("Coupon"		, СкидочныйКупон, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("CouponName"	, Представление, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("SerialNumber", СерийныйНомер, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("Nominal"		, СтрокаТабличнойЧасти.НоминалКупона, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("Count"		, СтрокаТабличнойЧасти.Количество, ЗаписьJSON);
		...
		Для каждого СтрокаТабличнойЧасти Из ДокументОбъект.ПогашениеСкидочныхКупонов Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();	//	Элемент массива: Начало
				...
				РаботаСJSONСервер.ЗаписатьСвойство("SerialNumber", СерийныйНомер, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("Nominal"		, СтрокаТабличнойЧасти.НоминалКупона, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("Accepted"	, РаботаСJSONСервер.БулевоКакЧисло(СтрокаТабличнойЧасти.Принят), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("ErrorMessage", "", ЗаписьJSON);	//	если купон не был применён.. как это сделать, ещё не знаю.
...
Процедура ЗаписатьПодарочныеСертификатыВJSON(ДанныеЗапросаСертификата, ЗаписьJSON)
	
	ЗаписьJSON.ЗаписатьИмяСвойства("Certificates");
	ЗаписьJSON.ЗаписатьНачалоМассива();
	
	Если ДанныеЗапросаСертификата.Свойство("Certificates") Тогда

		Для Каждого СтрокаСертификата Из ДанныеЗапросаСертификата.Certificates Цикл
			
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
			
			СостояниеСерийногоНомера = ПодарочныеСертификатыСервер.ПолучитьСоставСерийногоНомера(СтрокаСертификата.SerialNumber);
			
			ЗаписьJSON.ЗаписатьИмяСвойства("SerialNumber");
			ЗаписьJSON.ЗаписатьЗначение(СтрокаСертификата.SerialNumber);
			...
			Если СостояниеСерийногоНомера.Найден Тогда
				...
				ЗаписьJSON.ЗаписатьИмяСвойства("IsFound");
				ЗаписьJSON.ЗаписатьЗначение(ДанныеПоСертификату.Найден);

				ЗаписьJSON.ЗаписатьИмяСвойства("EndDateAction");
				ЗаписьJSON.ЗаписатьЗначение(Формат(ДанныеПоСертификату.ДатаОкончанияДействия, "ДФ=yyyy/MM/dd"));

				ЗаписьJSON.ЗаписатьИмяСвойства("SerialNumberCode");
				ЗаписьJSON.ЗаписатьЗначение(ДанныеПоСертификату.КодСерийногоНомера);

				ЗаписьJSON.ЗаписатьИмяСвойства("Price");
				ЗаписьJSON.ЗаписатьЗначение(Формат(ДанныеПоСертификату.Номинал, "ЧГ=;"));

				ЗаписьJSON.ЗаписатьИмяСвойства("State");
				ЗаписьJSON.ЗаписатьЗначение(ОбщегоНазначенияКлиентСервер.РеквизитПеречисления(ДанныеПоСертификату.Состояние)); //ПеречислениеСсылка.СостоянияСерийныхНомеров
...
Функция ПроверитьКупон(НомерКупона)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДвиженияСерийныхНомеровКупонов.СерийныйНомер, 0) = 0
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|		ПО (ДвиженияСерийныхНомеровКупонов.СерийныйНомер = СерийныеНомера.Ссылка)
	|ГДЕ
	|	СерийныеНомера.КодСерийногоНомера = &НомерКупона
	|	И СерийныеНомера.Владелец.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.СкидочныйКупон)
	...
	|	Штрихкоды.Штрихкод,
	|	ИСТИНА,
	|	Штрихкоды.Блокирован
	|ИЗ
	|	РегистрСведений.Штрихкоды КАК Штрихкоды
	|ГДЕ
	|	Штрихкоды.Штрихкод = &НомерКупона
	|	И Штрихкоды.Владелец.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.СкидочныйКупон)"
	);
	Запрос.УстановитьПараметр("НомерКупона", НомерКупона);

	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		
		Возврат (НЕ Выборка.Использован И Выборка.Существует); //возвращаем НЕ Использован проверяем что он вообще существует.	
	Иначе
		Возврат Ложь; //купона не существует
	КонецЕсли;
...
#Область Receipt 
Функция Receipt(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("IDBASE"	, ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"));
	СтруктураОтвета.Вставить("DIFF"	, СокрЛП(Новый УникальныйИдентификатор));
	СтруктураОтвета.Вставить("Error"	, 0);
	СтруктураОтвета.Вставить("ErrorMessage"	, "");

	Успешно =  ОбменДаннымиКСОСервер.СоздатьЧекПоПараметрам(ДанныеЗапроса);
	Если Не Успешно Тогда
		Если ДанныеЗапроса.Свойство("КодОшибки") Тогда
			СтруктураОтвета.Вставить("Error"	, ДанныеЗапроса.КодОшибки);
		Иначе
			СтруктураОтвета.Вставить("Error"	, 1);
		КонецЕсли;
		СтруктураОтвета.Вставить("ErrorMessage"	, ДанныеЗапроса.ОписаниеОшибки);
	КонецЕсли;
	ТекстДЖСОН = ОбменWebRetailСервер.СтруктуруВТекстДЖСОН(СтруктураОтвета);
...
#КонецОбласти //Receipt

#Область SendClientMessage 
Функция SendClientMessage(ДанныеЗапроса)
	СтруктураОтвета = Новый Структура;
	СтруктураОтвета.Вставить("IDBASE"	, ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"));
	СтруктураОтвета.Вставить("DIFF"	, СокрЛП(Новый УникальныйИдентификатор));
	СтруктураОтвета.Вставить("MessageId", СокрЛП(Новый УникальныйИдентификатор)); //теоретически надо логировать, но пока нет необходимости.
	СтруктураОтвета.Вставить("Channel", "SMS");
	СтруктураОтвета.Вставить("Status", "SENT");
	СтруктураОтвета.Вставить("Provider", "GMSU");  //Пока только один
	...
	Если НЕ РозничныеПродажиСлужебный.РесурсWebRetailДоступен() Тогда
		...
		СтруктураОтвета.Вставить("Error", 1);
		СтруктураОтвета.Вставить("ErrorMessage", "SMS аутентификация: НЕТ доступа к серверу ЦБ. Попытайтесь выполнить позже.");
	...
	Если НЕ (ДанныеЗапроса.Свойство("MessageData") 
				И ДанныеЗапроса.MessageData.Свойство("phoneNumber")
				И ДанныеЗапроса.MessageData.Свойство("MessageText")
				И ДанныеЗапроса.MessageData.Свойство("ClientIDC")) Тогда
	 	СтруктураОтвета.Вставить("Status", "ERROR");
		СтруктураОтвета.Вставить("Error", 1);
		СтруктураОтвета.Вставить("ErrorMessage", "SMS аутентификация: Неправильная структура запроса. Не обнаружено одно из обязательных полей");
	...
	НомерТелефона = ДанныеЗапроса.MessageData.phoneNumber;
	ТекстСообщения = Латиница(ДанныеЗапроса.MessageData.MessageText);
	Попытка
		КонтрагентСсылка = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(ДанныеЗапроса.MessageData.ClientIDC));
		Если ЗначениеЗаполнено(КонтрагентСсылка.Ссылка) Тогда
		КонецЕсли;
	Исключение
		СтруктураОтвета.Вставить("Error", 1);
		СтруктураОтвета.Вставить("ErrorMessage", "SMS аутентификация: Невалидный идентификтор клиента "+ДанныеЗапроса.MessageData.ClientIDC);
	 	СтруктураОтвета.Вставить("Status", "ERROR");
		ТекстДЖСОН = ОбменWebRetailСервер.СтруктуруВТекстДЖСОН(СтруктураОтвета);
	...
	Если Результат.Ошибка Тогда
		СтруктураОтвета.Вставить("Error", 1);
		СтруктураОтвета.Вставить("ErrorMessage", "SMS аутентификация: "+Результат.ОписаниеОшибки);
	 	СтруктураОтвета.Вставить("Status", "ERROR");

...
Функция СформироватьОписаниеКлиентаДЖСОН(ЗаписьJSON,СтрокаКлиенты)
	...
	JSON.ЗаписатьСвойство("IDC"				, ИДКлиента, ЗаписьJSON);
	//JSON.ЗаписатьСвойство("VersionNo"		, СтрокаКлиенты.ВерсияДанных, ЗаписьJSON);
	JSON.ЗаписатьСвойство("ClientPhone"		, СтрокаКлиенты.НомерТелефона, ЗаписьJSON);
	JSON.ЗаписатьСвойство("ClientEmail"		, СокрЛП(СтрокаКлиенты.ЭлектроннаяПочта), ЗаписьJSON);
	...
	JSON.ЗаписатьСвойство("ClientGender"	, ПолКлиента, ЗаписьJSON);

	JSON.ЗаписатьСвойство("Message"			, СтрокаКлиенты.ТекстСообщения, ЗаписьJSON);
	JSON.ЗаписатьСвойство("Error"			, СтрокаКлиенты.Ошибка, ЗаписьJSON);
...
//	2. ---------------------------------------------------------------------------------------
	...
	Для каждого СтрокаРебенок Из СтрокаКлиенты.Ссылка.Дети Цикл
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
		JSON.ЗаписатьСвойство("ChildId"			, СтрокаРебенок.УникальныйИдентификатор, ЗаписьJSON);
		JSON.ЗаписатьСвойство("ChildFirstName"	, СтрокаРебенок.Имя, ЗаписьJSON);
		...
		Иначе
			ПолРебенка = "";
		КонецЕсли;
		JSON.ЗаписатьСвойство("ChildGender"		, ПолРебенка, ЗаписьJSON);
		JSON.ЗаписатьСвойство("ChildDateBorn"	, ОбменMagentoСервер.ФорматДаты(СтрокаРебенок.ДатаРождения, ФорматДатыJSON.ISO), ЗаписьJSON);
				...
				//	и вот переходим к расчёту скидок!
					...
					Попытка
						...
						ЗаписьJSON.ЗаписатьНачалоОбъекта();
							...
							Иначе	
								JSON.ЗаписатьСвойство("SummaBonusPay" , 0,ЗаписьJSON);
							КонецЕсли;
...
Функция Receipt(ДанныеЗапроса)
	...
	Успешно =  СоздатьЧекПоПараметрам(ДанныеЗапроса);
...
Функция СоздатьЧекПоПараметрам(СтруктураЧека) Экспорт               
	...
	Попытка
		...
		Если СтруктураЧека.Свойство("Buyer") И ЗначениеЗаполнено(СтруктураЧека.Buyer) Тогда
			ЧекОбъект.ДисконтнаяКарта = Справочники.ИнформационныеКарты.ПолучитьСсылку(Новый УникальныйИдентификатор(СтруктураЧека.Buyer.IDC)); 
			ЧекОбъект.ВладелецДисконтнойКарты = ЧекОбъект.ДисконтнаяКарта.Владелец; 
		КонецЕсли;
		...
		Для Каждого СтруктураСтроки Из СтруктураЧека.Products Цикл
			...
			Строка_.КоличествоУпаковок = СтруктураСтроки.PackRate;
			Строка_.Сумма = Строка_.Количество * Строка_.Цена * Строка_.КоличествоУпаковок;
		КонецЦикла;

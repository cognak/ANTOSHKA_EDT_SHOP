...
Функция ПрочитатьОтветТерминала(ОбъектДрайвера,ПараметрыДрайвера)
	...
	Пока Истина Цикл
		...
		Если (ОбъектДрайвера.LastErrorMessage <> "") ИЛИ (КодСимвола(Прав(Ответ,1)) = 0) Тогда
			Прервать;	
		КонецЕсли;  
	...
	Иначе                                        
		...
		СообщитьОтладка(""+ТекущаяДата()+" ПоследнийШанс: "+ПоследнийШанс, ПараметрыДрайвера.Отладка);
		Если Прав(ПоследнийШанс,1) = Символ(00) Тогда
			ПоследнийШанс = Лев(ПоследнийШанс,СтрДлина(ПоследнийШанс) - 1); 
		Иначе //2025-06-17 sa иногда получаем только половинку ответа. Пробуем прочитать остаток
			Пауза(1000);
			ПоследнийШанс = ПоследнийШанс+ ОбъектДрайвера.ReadMessage(); //дочитываем кусок ответа
			СообщитьОтладка(""+ТекущаяДата()+" Дочитываем остаток: "+ПоследнийШанс, ПараметрыДрайвера.Отладка);
			Если Прав(ПоследнийШанс,1) = Символ(00) Тогда
				ПоследнийШанс = Лев(ПоследнийШанс,СтрДлина(ПоследнийШанс) - 1); 
			КонецЕсли;						
		КонецЕсли; 
...
Функция ПродажаДЖСОН(ПараметрыПлатежа)
	РезультатОплаты = Новый Структура("Успешно,Сумма,Скидка,ОписаниеОшибки,Банк,НомерКарты,КодRRN,НомерЧека,КодАвторизации,ДействительнаДо,Клиент,Эмитент,ТерминалИД,МерчантИД,СлипЧек",Ложь,0,0);
	...
	РезультатыПлатежа = СтруктураОтвета.params;	
	РезультатОплаты.Успешно = Не СтруктураОтвета.error;
	РезультатОплаты.Сумма = Число(РезультатыПлатежа.amount);
	РезультатОплаты.Банк = РезультатыПлатежа.bankAcquirer;
	РезультатОплаты.НомерКарты = РезультатыПлатежа.pan;
	РезультатОплаты.КодRRN = РезультатыПлатежа.rrn;
	РезультатОплаты.НомерЧека = РезультатыПлатежа.invoiceNumber;
	...
	РезультатОплаты.ТерминалИД = РезультатыПлатежа.terminalId;
	РезультатОплаты.МерчантИД = РезультатыПлатежа.merchant;
	//sa добавим еще скидку
	РезультатОплаты.Скидка = Число(РезультатыПлатежа.discount);
	//РезультатОплаты.Скидка = Окр(РезультатОплаты.Сумма * 0.50,2);
	...
	//2025-06-18 sa Ловушка. Если в ответе терминала нет обязательных заполненных полей, то логируем эту ситуацию, чтобы потом разобраться с Приватом
	Если Не ЗначениеЗаполнено(РезультатОплаты.МерчантИД)
			ИЛИ Не ЗначениеЗаполнено(РезультатОплаты.Банк)
			ИЛИ Не ЗначениеЗаполнено(РезультатОплаты.НомерКарты)
			ИЛИ Не ЗначениеЗаполнено(РезультатОплаты.КодRRN)
			ИЛИ Не ЗначениеЗаполнено(РезультатОплаты.НомерЧека)
			ИЛИ Не ЗначениеЗаполнено(РезультатОплаты.КодАвторизации)
			ИЛИ Не ЗначениеЗаполнено(РезультатОплаты.Эмитент)
			ИЛИ Не ЗначениеЗаполнено(РезультатОплаты.ТерминалИД)
			Тогда 
			СтруктураДанных = Новый Структура;
			СтруктураДанных.Вставить("Комментарий", "Ответ терминала не содержит всех данных");
			СтруктураДанных.Вставить("Пользователь", ПользователиКлиентСервер.АвторизованныйПользователь());
			СтруктураДанных.Вставить("Событие","ОШИБКА_ПРИ_ОПЛАТЕ");
			СтруктураДанных.Вставить("ОбъектМетаданных", "Документ.ЧекККМ");
			СтруктураДанных.Вставить("Уровень",ПредопределенноеЗначение("Перечисление.УровеньЖурналаРегистрации.Ошибка"));
			СтруктураДанных.Вставить("ОтветТерминала",ОтветТерминала);
			СтруктураДанных.Вставить("РезультатОплаты",РезультатОплаты);
			РМКСервер.Логирование(СтруктураДанных);  //Стандартную функцию записи в журнал логирования нужно вызывать с сервера.  
			//Пусть ответ регистрируется, а мы пока поставим заплатку. Если не заполнен Эмитент, то проставим его вручную
			Если  Не ЗначениеЗаполнено(РезультатОплаты.Эмитент) Тогда
				РезультатОплаты.СлипЧек = ""+РезультатОплаты.Банк + Символы.ПС + 
				  Символы.ПС +
				  "UNKNOWN" + Символы.ПС +
				  //"ДЕЙСТВ. ДО: " + Прав(ДействительнаДо,2) + "/" + Лев(ДействительнаДо,2) + Символы.ПС +
...
Функция ВозвратДЖСОН(ПараметрыПлатежа)
	...
	Если Не ЗначениеЗаполнено(РезультатОплаты.МерчантИД)
			...
			СтруктураДанных.Вставить("Комментарий", "Ответ терминала не содержит всех данных");
			СтруктураДанных.Вставить("Пользователь", ПользователиКлиентСервер.АвторизованныйПользователь());
			СтруктураДанных.Вставить("Событие","ОШИБКА_ПРИ_ВОЗВРАТЕ");
			СтруктураДанных.Вставить("ОбъектМетаданных", "Документ.ЧекККМ");
			СтруктураДанных.Вставить("Уровень",ПредопределенноеЗначение("Перечисление.УровеньЖурналаРегистрации.Ошибка"));
			СтруктураДанных.Вставить("ОтветТерминала",ОтветТерминала);
			СтруктураДанных.Вставить("РезультатОплаты",РезультатОплаты);

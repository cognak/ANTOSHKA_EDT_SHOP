...
Функция ПрочитатьОтветТерминала(ОбъектДрайвера,ПараметрыДрайвера)
	...
	Пока Истина Цикл
		...
		Если (ОбъектДрайвера.LastErrorMessage <> "") ИЛИ (СтрДлина(Ответ)>1) Тогда
			Прервать;	
		КонецЕсли;  
	...
	Иначе                                        
		...
		ПоследнийСимвол = ОбъектДрайвера.ReadMessage();
		Если ПоследнийСимвол <> Символ(00) И ПоследнийСимвол <>"" Тогда 
			Сообщить("Неожидаемый ответ: "+ПоследнийСимвол); 
			...
			ОписаниеОшибкиКоманды = ?(ОбъектДрайвера.LastErrorMessage="","Транзакцію скасовано","Помилка скасування транзакції: "+ОбъектДрайвера.LastErrorMessage);
			Пауза(2000);
			ОтветОтмены = ОбъектДрайвера.ReadMessage();
			СообщитьОтладка(""+ОтветОтмены,ПараметрыДрайвера.Отладка);
			Если Не Успешно Тогда

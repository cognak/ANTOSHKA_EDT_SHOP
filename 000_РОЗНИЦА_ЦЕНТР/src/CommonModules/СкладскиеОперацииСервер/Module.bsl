...
Процедура ПроверитьУчтеноNavisionДляОрдеров()	Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ТаблицаУчетаОрдеры.Объект.Дата КАК Период,
	|	ТаблицаУчетаОрдеры.Объект КАК Объект,
	|	ТаблицаУчетаОрдеры.Магазин КАК Магазин,
	|	ТаблицаУчетаОрдеры.Объект.ДокументОснование КАК ДокументОснование
	|ИЗ
	...
	|		ПО ТаблицаУчетаОрдеры.Объект.ДокументОснование = ТаблицаУчетаПеремещения.Объект
	|			И ТаблицаУчетаОрдеры.Магазин = ТаблицаУчетаПеремещения.Магазин
	|			И (ТаблицаУчетаПеремещения.УчтеноNavision)
	|ГДЕ
	|	(ТаблицаУчетаОрдеры.Объект ССЫЛКА Документ.ПриходныйОрдерНаТовары
	|			ИЛИ ТаблицаУчетаОрдеры.Объект ССЫЛКА Документ.РасходныйОрдерНаТовары)
	|	И НЕ ТаблицаУчетаОрдеры.УчтеноNavision
	|	И ТаблицаУчетаОрдеры.Объект.Дата >= ДАТАВРЕМЯ(2022, 10, 1)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период,
	|	Объект"

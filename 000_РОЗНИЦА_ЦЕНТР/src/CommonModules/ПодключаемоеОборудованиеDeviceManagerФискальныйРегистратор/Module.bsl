	...
	//	Снятие X-отчёта ФР
		...
		Если Успешно = Истина Тогда
			...
			//	ТабличныйДокумент = ПолучитьТабличныйДокументОтчет(ТекстЗаголовка, res.Получить("info"));
				...
				Если ВходныеПараметры[2][21].ПечатьЧековНаПринтер = Истина
				Тогда
	...
	//	Снятие Z-отчёта ФР
		...
		Если Успешно = Истина Тогда
			...
			//	ТабличныйДокумент = ПолучитьТабличныйДокументОтчет(ТекстЗаголовка, res.Получить("info"));
				...
				Если ВходныеПараметры[2][21].ПечатьЧековНаПринтер = Истина И ВходныеПараметры[2][21].ПРРО_ПД_Печать_Z_Отчета = Истина
				Тогда		//ОбъектДрайвера.Параметры.НаПринтер
...
Функция НапечататьПериодическийОтчетПоДатам(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Попытка
		...
		reqData.Вставить("dt_from"	, ВходныеПараметры[0]);
		reqData.Вставить("dt_to"	, ВходныеПараметры[1]);

		res = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистратор.ОтправитьПакет(ОбъектДрайвера, ОбъектДрайвера.Параметры.enFiscalTaskType.Z_REPORT_BY_DATE, reqData);
		Успешно = (res.get("res") = 0);
		...
		Если Успешно = Истина Тогда
			...
			Если ЗначениеЗаполнено(errortxt) Тогда
				...
				Успешно = Ложь;
				ОбъектДрайвера.ОписаниеОшибки = "Помилка друку періодичного звіту щодо дат:" + Символы.ПС
					+ ПодключаемоеОборудованиеDeviceManagerФискальныйРегистратор.ПолучитьКодИТекстОшибки(ОбъектДрайвера);
			...
			Иначе
				...
				ТекстЗаголовка	  = "Періодичний Z-звіт(и) щодо дат";
				ТабличныйДокумент = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистраторСервер.ПолучитьТабличныйДокументОтчет(ТекстЗаголовка, res.Получить("info"));
				#Если _ Тогда
				ТабличныйДокумент = Новый ТабличныйДокумент;
				#КонецЕсли
				...
				Тогда
					...
					ПоказатьОповещениеПользователя("Періодичний звіт щодо дат надруковано",	//	LNK 16.05.2023 15:15:15
						, "Періодичний звіт щодо дат сформовано та надіслано на принтер за замовчуванням"
						, БиблиотекаКартинок.Информация32
					);
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить("Помилка виклику методу <ОбъектДрайвера.НапечататьПериодическийОтчетПоДатам>: " + ОписаниеОшибки());
...
Функция НапечататьПериодическийОтчетПоНомерам(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить("Помилка виклику методу <ОбъектДрайвера.НапечататьПериодическийОтчетПоНомерам>: " + ОписаниеОшибки());
...
Функция Инкассация(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТипИнкассации, Сумма, ВыходныеПараметры)
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить("Помилка виклику методу <ОбъектДрайвера.НапечататьЧекВнесенияВыемки>: " + ОписаниеОшибки());
	КонецПопытки;
...
Функция НапечататьНулевойЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Исключение
		...
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить("Помилка виклику методу <ОбъектДрайвера.НапечататьНулевойЧек>: " + ОписаниеОшибки());
	КонецПопытки;
...
Функция ОтправитьПакет(ОбъектДрайвера, Задание = "", reqData = Неопределено) Экспорт
	...
	Если resData = Неопределено Тогда
		...
		Исключение
			...
			resData = СтруктураВМассив(new Structure("res, errortxt", -2, "Error getting response from DeviceManager:"
			+ " " + ErrorDescription()));
...
Функция СоздатьПакет(ОбъектДрайвера, Задание, reqData = Неопределено)
	...
	packet.Вставить("dt"		, format(currentDate(), "ДФ=yyyyMMddHHmmss") 

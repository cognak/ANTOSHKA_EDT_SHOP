		...
		Иначе
			...
			Если НЕ ПустаяСтрока(ПараметрыЗакрытия.НомерТелефона) И НЕ ПустаяСтрока(URL)
				...
				Исключение
					...
					СообщитьПользователю(НСтр("ru='Ошибка сообщения клиенту ';uk='Помилка повідомлення клієнту '") + Символы.ПС + ОписаниеОшибки());
	...
	//	Снятие X-отчёта ФР
		...
		Если Успешно = Истина Тогда
			...
			Если ЗначениеЗаполнено(errortxt) Тогда
				...
				Успешно = Ложь;
				ОбъектДрайвера.ОписаниеОшибки = НСтр("ru='Ошибка печати Х-отчета! ';uk='Помилка друку Х-звіту! '")
...
#Область УдалитьПозже	//	LNK 08.04.2022 09:23:39

&НаКлиенте
Функция НапечататьОтчетБезГашения_УДАЛИТЬ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	//	Снятие X-отчёта ФР
		res = ОтправитьПакет(ОбъектДрайвера, ОбъектДрайвера.Параметры.enFiscalTaskType.X_REPORT);
		Успешно = (res.get("res") = 0);

		Если НЕ Успешно Тогда       

			ОбъектДрайвера.ОписаниеОшибки = НСтр("ru='Ошибка печати Х-отчета! ';uk='Помилка друку Х-звіту! '") + ПолучитьКодИТекстОшибки(ОбъектДрайвера);
		...
		Иначе 
			...
			Иначе
				...
				сИнфо = res.Получить("info");
			//	ВывестиИнформациюПоОтчету("Х отчет", сИнфо);		

			КонецЕсли;

		КонецЕсли;
		
		//Ответ = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистраторИнтерфейсВК.НапечататьОтчетБезГашения(ОбъектДрайвера);
		Если НЕ Успешно Тогда
...
// Функция осуществляет снятие отчета с гашением 
//
Функция НапечататьОтчетСГашением_УДАЛИТЬ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Попытка
		...
		res = ОтправитьПакет(ОбъектДрайвера, ОбъектДрайвера.Параметры.enFiscalTaskType.Z_REPORT);
		Успешно = (res.get("res") = 0);
		Если НЕ Успешно Тогда 
			ОбъектДрайвера.ОписаниеОшибки = НСтр("ru='Ошибка печати Z-отчета! ';uk='Помилка друку Z-звіту! '") + ПолучитьКодИТекстОшибки(ОбъектДрайвера);
			ДобавитьСтрокуЛога(ОбъектДрайвера.ОписаниеОшибки, СтрокаЛога);
		Иначе
			СтрокаЛога = СтрЗаменить(СтрокаЛога, "#DEV#", ПолучитьЗначениеСоответствия(res, "device",""));
			errortxt = ПолучитьЗначениеСоответствия(res, "errortxt", "");
			Если ЗначениеЗаполнено(errortxt) Тогда
				ОбъектДрайвера.ОписаниеОшибки = НСтр("ru='Ошибка печати Z-отчета! ';uk='Помилка друку Z-звіту! '") + ПолучитьКодИТекстОшибки(ОбъектДрайвера);
				ДобавитьСтрокуЛога(ОбъектДрайвера.ОписаниеОшибки, СтрокаЛога);
				ДобавитьСтрокуЛога(errortxt, СтрокаЛога);
			Иначе
				сИнфо = res.Получить("info");
			//	ВывестиИнформациюПоОтчету("Z отчет", сИнфо);		
			КонецЕсли;
		...
		Если НЕ Успешно Тогда
			...
			ВыходныеПараметры.Добавить("");
			ВыходныеПараметры[1] = res.get("errortxt");
			ДобавитьСтрокуЛога(ВыходныеПараметры[1], СтрокаЛога);
		Иначе
			МенеджерОборудованияСервер.СохранитьПараметрыУстройства(Параметры.Идентификатор, Параметры);
			//clearLastArticle(Параметры.УникальныйИдентификатор);
	...
	Исключение
		Результат = Ложь;
		ОписаниеОшибки = НСтр("ru='Ошибка вызова метода <ОбъектДрайвера.НапечататьОтчетСГашением>:';uk='Помилка виклику методу <ОбъектДрайвера.НапечататьОтчетСГашением>:'") + ОписаниеОшибки();
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ОписаниеОшибки);
		ДобавитьСтрокуЛога(ОписаниеОшибки, СтрокаЛога);
	КонецПопытки;
	
	Если Результат = Истина Тогда

			ДобавитьСтрокуЛога("----- " + НСтр("ru='Смена на устройстве закрыта';uk='Зміна на пристрої закрита'") + " -----", СтрокаЛога);

	Иначе	ДобавитьСтрокуЛога(НСтр("ru='При закрытии кассовой смены возникли ошибки';uk='При закритті касової зміни виникли помилки'"), СтрокаЛога);
...
&НаКлиенте
Процедура ВывестиИнформациюПоОтчету_УДАЛИТЬ(НаименованиеОтчета, сИнфо)

	СтрокаЛога = СокрЛП(НаименованиеОтчета);
	...
	ДобавитьСтрокуЛога("------Внесення\винесення:------", СтрокаЛога);

	Для каждого СтрокаОплат ИЗ money Цикл
		sum_p = Формат(ПолучитьЗначениеСоответствия(СтрокаОплат, "sum_p", ""),"ЧДЦ=2");
		sum_m = Формат(ПолучитьЗначениеСоответствия(СтрокаОплат, "sum_m", ""),"ЧДЦ=2");
		
		Если ЗначениеЗаполнено(sum_p) Тогда
			ДобавитьСтрокуЛога("Внесення " + sum_p, СтрокаЛога);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(sum_m) Тогда
			ДобавитьСтрокуЛога("Винесення " + sum_m, СтрокаЛога);
		КонецЕсли;
		
	КонецЦикла;

	ДобавитьСтрокуЛога("------Оплати:------", СтрокаЛога);
	...
	Для каждого СтрокаОплат ИЗ pays Цикл
		...
		Если ЗначениеЗаполнено(sum_p) Тогда
			ДобавитьСтрокуЛога("Cума продажів: " + ПолучитьЗначениеСоответствия(СтрокаОплат, "name", "") + "  " + sum_p, СтрокаЛога);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(sum_m) Тогда
			ДобавитьСтрокуЛога("Cума повернення: " + ПолучитьЗначениеСоответствия(СтрокаОплат, "name", "") + "  " + sum_m, СтрокаЛога);
		КонецЕсли;
		
	КонецЦикла;

	ДобавитьСтрокуЛога("------Податки:------", СтрокаЛога);

	Для каждого СтрокаНалогов ИЗ taxes Цикл

		base_sum_p =  Формат(ПолучитьЗначениеСоответствия(СтрокаНалогов, "base_sum_p", ""),"ЧДЦ=2");  
		tax_sum = Формат(ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_sum_p", ""),"ЧДЦ=2");  

		Если ЗначениеЗаполнено(base_sum_p) Тогда
			ДобавитьСтрокуЛога("Cума продажів " + base_sum_p + "   " + ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_name", "") + "  =  " + Формат(tax_sum, "ЧДЦ=2"), СтрокаЛога);
		КонецЕсли;
		
		base_sum_m =  Формат(ПолучитьЗначениеСоответствия(СтрокаНалогов, "base_sum_m", ""),"ЧДЦ=2");  
		tax_sum = Формат(ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_sum_m", ""),"ЧДЦ=2");  

		Если ЗначениеЗаполнено(base_sum_m) Тогда
			ДобавитьСтрокуЛога("Cума повернення "+base_sum_m+"   "+ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_name", "")+"  =  "+Формат(tax_sum,"ЧДЦ=2"), СтрокаЛога);
		КонецЕсли;

		ex_sum = ПолучитьЗначениеСоответствия(СтрокаНалогов, "ex_sum_p", "");
		Если ЗначениеЗаполнено(ex_sum) Тогда				
			ДобавитьСтрокуЛога("Cума "+base_sum_p+"   "+ПолучитьЗначениеСоответствия(СтрокаНалогов, "ex_name", "")+"  =  "+Формат(ex_sum,"ЧДЦ=2"), СтрокаЛога);
		КонецЕсли		

	КонецЦикла;
	                                                           
	ДобавитьСтрокуЛога("------Сейф:------", СтрокаЛога);
	safe =  ПолучитьЗначениеСоответствия(сИнфо, "safe", 0);
	ДобавитьСтрокуЛога("Залишок у каcі: "+Формат(safe,"ЧДЦ=2; ЧН=0.00"), СтрокаЛога);

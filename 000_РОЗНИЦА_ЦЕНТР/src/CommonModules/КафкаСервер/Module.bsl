...
// 
// Параметры:
//  МассивСобытий - Массив - событий
//  КаналОтправки - Строка - Канал отправки
// 
// Возвращаемое значение:
//  Неопределено, Произвольный - Отправить по каналу
Функция ОтправитьПоКаналу(МассивСобытий, КаналОтправки) Экспорт
	...
	Для Каждого ОблачноеСобытие Из МассивСобытий Цикл
	...
	Попытка
		ВозвращаемоеЗначение = КоннекторHTTP.Post(
	    		"http://" + Константы.АдресБрокераКафка.Получить() + ":8082/topics/" + КаналОтправки,
...
Функция ПолучитьСообщения(КаналПолучения) Экспорт
	...
	Попытка
		...
		//получаем записи
		ОтветСервере = КоннекторHTTP.Get(БазовыйURL + "/records",ПараметрыЗапроса,Новый Структура("Заголовки", ЗаголовкиСериализацииЗаписей));
		ЗаписиПотока = КоннекторHTTP.КакJson(ОтветСервере , ПараметрыПреобразованияJSON);
		...
		Попытка
			...
			МенеджерЗаписи.ОтветСервера = Истина;
			МенеджерЗаписи.ПолученныеДанные = Новый ХранилищеЗначения(ЗаписиПотока);

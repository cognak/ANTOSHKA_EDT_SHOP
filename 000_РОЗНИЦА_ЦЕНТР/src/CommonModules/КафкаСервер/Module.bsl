...
// 
// Параметры:
//  Объект - Массив из ЛюбаяСсылка - массив ссылок или ссылка 
...
Процедура ЗаписьСправочникаДокументаПриЗаписи(Источник, Отказ) Экспорт
	...
	МассивОбъектовВыгрузки = ОбщегоНазначения.ТаблицаЗначенийВМассив(Запрос.Выполнить().Выгрузить());
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			Если Выборка.ТипСодержимого = Перечисления.КафкаТипСодержимого.application_json Тогда
				ДанныеДляОтправки=ОбъектВСтрокуJSON(МассивОбъектовВыгрузки); 
			Иначе
				ДанныеДляОтправки=ОбъектВСтрокуXML(МассивОбъектовВыгрузки, Выборка.ПравилаОбмена); 
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого ЭлементОправки Из МассивКаналовОтправки Цикл
			...
			МенеджерЗаписи.Представление = Источник.Ссылка;
			МенеджерЗаписи.ПромежуточнаяЗапись = Выборка.ПромежуточнаяЗапись;
			МенеджерЗаписи.СсылкаНаОбъект = МассивОбъектовВыгрузки[0].Ссылка;

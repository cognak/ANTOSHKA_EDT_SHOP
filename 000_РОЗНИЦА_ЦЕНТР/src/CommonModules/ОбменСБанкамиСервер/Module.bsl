...
Функция СформироватьДанныеLiqPay(СтруктураДанных) Экспорт
	...
	private_key = "sandbox_Xdb5toppjWbxKl5tn4AKm8VLRk79WpI8R58lxWWE";
		...
		Если Выборка.Следующий() Тогда 
			...
			Если ДанныеОтвета.status = "success" Тогда  
				...
				Для Каждого СтрокаОплаты Из ДокументЗаказа.ОплатаЗаказа Цикл 
					Если СтрокаОплаты.НомерОплаты = НомерОплаты Тогда
						СтрокаОплаты.Сумма = ДанныеОтвета.amount;
						СтрокаОплаты.СтатусОплаты = Перечисления.СтатусОплаты.Оплачен;
						СтрокаОплаты.Отменён = Ложь;
						Прервать;
					КонецЕсли;  
				КонецЦикла;

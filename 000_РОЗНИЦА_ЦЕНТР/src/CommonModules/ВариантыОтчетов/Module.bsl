...
//           |- (Строка)       Текст ошибки
//
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя, Пропустить = Ложь) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		ИначеЕсли НЕ ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Пропустить = Истина;	//	LNK 07.02.2020 08:53:52
...
Процедура ЗаписьВРегистрИспользованияОтчета(Вариант) Экспорт
	...
	НаборЗаписей = РегистрыСведений.СтатистикаИспользованиеОтчетов.СоздатьНаборЗаписей();
	
	ТекущаяДата = ТекущаяДатаСеанса();

	НаборЗаписей.Отбор.Период.Установить(ТекущаяДата);
	НаборЗаписей.Отбор.ВариантОтчета.Установить(Вариант);
	НаборЗаписей.Отбор.Магазин.Установить(ПараметрыСеанса.ТекущийМагазин);
	НаборЗаписей.Отбор.Пользователь.Установить(ПараметрыСеанса.ТекущийПользователь);
	
	НовЗапись = НаборЗаписей.Добавить();

	НовЗапись.Период = ТекущаяДата;
	НовЗапись.ВариантОтчета  = Вариант;
	НовЗапись.Магазин = ПараметрыСеанса.ТекущийМагазин;
...
Процедура ФормаОтчетаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.РассылкаОтчетов") Тогда
		...
		Модуль.ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш, ОбщиеДанные)
	...
	Иначе
		...
		|	ДанныеСправочника.Ссылка
		|ИЗ
		|	Справочник.ВариантыОтчетов КАК ДанныеСправочника
		|ГДЕ
		|	ДанныеСправочника.ПометкаУдаления = ЛОЖЬ
		|	И (ДанныеСправочника.Отчет.ПометкаУдаления = ИСТИНА
		|			ИЛИ ВЫБОР
		|				КОГДА ТИПЗНАЧЕНИЯ(ДанныеСправочника.Отчет) = ТИП(СТРОКА)
		|					ТОГДА ДанныеСправочника.Отчет = """"
		|				КОГДА ТИПЗНАЧЕНИЯ(ДанныеСправочника.Отчет) = ТИП(Справочник.ДополнительныеОтчетыИОбработки)
		|					ТОГДА ДанныеСправочника.Отчет.Наименование ЕСТЬ NULL 
		|				КОГДА ТИПЗНАЧЕНИЯ(ДанныеСправочника.Отчет) = ТИП(Справочник.ИдентификаторыОбъектовМетаданных)
		|					ТОГДА ДанныеСправочника.Отчет.Имя ЕСТЬ NULL 
		|				ИНАЧЕ ДанныеСправочника.Отчет = НЕОПРЕДЕЛЕНО
		|			КОНЕЦ)"
		;

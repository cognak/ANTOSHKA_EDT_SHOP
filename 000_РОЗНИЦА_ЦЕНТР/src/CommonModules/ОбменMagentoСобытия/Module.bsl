...
// Регистрирует изменения набора записей регистра сведений для обмена с Magento.
//
// Параметры:
//   Источник - РегистрНаборЗаписей - Набор записей регистра, изменения которого регистрируются
//   Отказ - Булево - Признак отказа от выполнения стандартной обработки
...
// Регистрирует изменения контрагента для обмена с Magento2 при выполнении условий:
//  объект является контрагентом, принадлежит маркетинговой группе М2,
//   не является юридическим лицом и не отключена регистрация изменений.
//
// Параметры:
//   Источник - СправочникОбъект.Контрагенты, СправочникСсылка.Контрагенты - измененный объект
//   Отказ - Булево - признак отмены записи объекта
Процедура ОбменРозницаMagento2ЗарегистрироватьИзменение(Источник, Отказ) Экспорт
	...
	Если НЕ (ТипЗнч(Источник) = Тип("СправочникОбъект.Контрагенты") Или ТипЗнч(Источник)=Тип("СправочникСсылка.Контрагенты")) Тогда
		Возврат;
	КонецЕсли;

	ЕстьМ2 = Источник.МаркетинговыеГруппы.Найти(ПредопределенноеЗначение("Справочник.МаркетинговыеГруппы.М2"),"Группа");
	Если ЕстьМ2 = Неопределено Тогда //если это не клиент мадженты 2, то не регистрируем.
		Возврат;
	КонецЕсли; 

	//2026-04-23 sa Задача https://awdev.atlassian.net/browse/RETAIL1C-1137.
	//Виключити з реєстрації в плані обміну контрагентів юридичних осіб для нового сайту
	Если Источник.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо ИЛИ Источник.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
		Возврат;
	КонецЕсли;

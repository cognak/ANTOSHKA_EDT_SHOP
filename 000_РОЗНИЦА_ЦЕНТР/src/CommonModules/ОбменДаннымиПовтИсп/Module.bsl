...
//	Следует поддерживать соответствие имён в классах и структуре.
	...
	КлассификаторСостава.Классы.Добавить("ЗаказыПокупателей");			//	LNK 17.05.2023 09:53:55
	КлассификаторСостава.Классы.Добавить("ЗакрытиеЗаказовПокупателей");	//	LNK 18.05.2023 10:07:21
...
//	Нижеуказанные классы будем ограничивать по количеству элементов в пакете.
	...
	КлассификаторСостава.Классы.Добавить("СоответствияОбъектовУчетныхСистем");		//	выгружаем "частями"	LNK 18.11.2023 07:22:59
	КлассификаторСостава.Классы.Добавить("КлючевыеАтрибутыЗаказовПокупателей");		//	выгружаем "частями"	//	LNK 26.11.2023 10:24:43

//	LNK 28.12.2023 06:13:09 - выгружаем "частями"
//	Важно выгрузить перед "ОбъектУчтенВNavision", так как должны совпасть версии данных.
//	Часть механизма создания "КорректировкаПеремещения".
	КлассификаторСостава.Классы.Добавить("ПеремещениеТоваров");
	КлассификаторСостава.Классы.Добавить("ПриходныйОрдерНаТовары");
	КлассификаторСостава.Классы.Добавить("РасходныйОрдерНаТовары");
...
//	В класс "Прочее" загоняем все остальные объекты метаданных, не вошедшие в предыдущие классы
	...
	Запрос.Текст =
		...
		|	КафкаНастройки.Наименование КАК Наименование
		|ИЗ
		...
		|ГДЕ
		|	НЕ КафкаНастройки.ПометкаУдаления
		|	И КафкаНастройки.ИсключитьИзОбменов
		|	И (КафкаНастройкиТопики.ОтправлятьИзЦентра
		|	И &ЦентральнаяБаза
		|	ИЛИ КафкаНастройкиТопики.ОтправлятьИзПереферии
		|	И НЕ &ЦентральнаяБаза)";
		
	Запрос.УстановитьПараметр("ЦентральнаяБаза", ЭтоГлавныйУзел());
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ИсключаемыеИзОбмена = РезультатЗапроса.Выгрузить();

	Для каждого ЭлементСостава Из УзелИнформационнойБазы.Метаданные().Состав Цикл
		
		ИскомаяСтрока = ИсключаемыеИзОбмена.Найти(ЭлементСостава.Метаданные.ПолноеИмя(), "Наименование");
		Если ИскомаяСтрока <> Неопределено Тогда
			Индекс = КлассификаторСостава.Классы.Найти(ЭлементСостава.Метаданные.Имя);
			Если Индекс <> Неопределено Тогда
			    КлассификаторСостава.Классы.Удалить(Индекс);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьРазмерПакетаВыгрузки(Идентификатор)	Экспорт
	...
	Иначе
		...
		ИначеЕсли Идентификатор = "ЗаказПокупателя"
		Тогда	ЗначениеВозврата = 500;	//	LNK 24.04.2023 07:23:50

		ИначеЕсли Идентификатор = "ЗакрытиеЗаказовПокупателей"
		Тогда	ЗначениеВозврата = 500;	//	LNK 19.05.2023 06:03:13

		ИначеЕсли Идентификатор = "ЗаказыПокупателей"
		Тогда	ЗначениеВозврата = 1000;	//	LNK 17.05.2023 09:50:31
		...
		ИначеЕсли Идентификатор = "СоответствияОбъектовУчетныхСистем"
		Тогда	ЗначениеВозврата = 1000;	//	LNK 18.11.2023 07:27:36

		ИначеЕсли Идентификатор = "КлючевыеАтрибутыЗаказовПокупателей"
		Тогда	ЗначениеВозврата = 1000;	//	LNK 26.11.2023 10:29:29

...
Процедура ВыгрузитьДокументПеремещение(Ссылка,РазрешитьДобавлятьСтрокиВТСД) ЭКСПОРТ
	...
	|	ВЫБОР
	|		КОГДА РСцены.ТипПереоценки = ЗНАЧЕНИЕ(Перечисление.ТипыПереоценок.ПромоНачало)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК DSCN,
	|	ВЫБОР
	...
	|		ПО Товары.Номенклатура.Марка = Марки.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, ) КАК РСцены
	|		ПО Товары.Магазин.ПравилоЦенообразования.ВидЦен = РСцены.ВидЦены
	|			И Товары.Упаковка = РСцены.Упаковка
	|			И Товары.Номенклатура = РСцены.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОпорныеЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, Магазин = &Магазин) КАК ОпорныеЦеныНоменклатурыСрезПоследних
...
Процедура ОбновитьСправочникиВТСД(ТолькоИзменения)	Экспорт
	Если РозничныеПродажиСервер.НовыеСтикеры() тогда
		ТранспортТерминаловНовый.ОбновитьСправочникиВТСД(ТолькоИзменения);  
	Иначе
		ОбновитьСправочникиВТСДстарыеСтикеры(ТолькоИзменения);	
	КонецЕсли;
КонецПроцедуры
 //	LNK 12.09.2018 08:09:25
Процедура ОбновитьСправочникиВТСДстарыеСтикеры(ТолькоИзменения)	Экспорт
...
Процедура ЗаписатьТоварВоВнешнийИсточник(ТоварыВыборка)
	...
	Если НЕ ТоварыВыборка.VERSION = ТоварВнешний.VERSION ИЛИ НЕ ТоварыВыборка.PRICE = ТоварВнешний.PRICE ИЛИ НЕ ТоварыВыборка.MARGA = ТоварВнешний.MARGA тогда
...
Процедура ВыгрузкаЦенВТСД() Экспорт  
	...
	Если РозничныеПродажиСервер.НовыеСтикеры() тогда
		ТранспортТерминаловНовый.ВыгрузкаЦенВТСД();  
	КонецЕсли;
	//УстановитьПривилегированныйРежим(Истина);
	//
	//Запрос = Новый Запрос;
...
Процедура ОбновлдениеЦенВТСД() Экспорт

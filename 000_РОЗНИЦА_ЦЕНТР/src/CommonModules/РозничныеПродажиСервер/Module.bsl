...
Функция ЗакрытьКассовыйДень(Магазин)	Экспорт
	...
	ДанныеКассовогоДня = ПолучитьСостояниеКассовогоДня(Магазин, Истина);	//	LNK 19.11.2022 06:21:14 - ТолькоОткрытый = Истина
	...
	Если ДанныеКассовогоДня.СтатусКассовогоДня = Перечисления.СтатусыКассовойСмены.Открыта Тогда
		...
		Попытка
			...
			Если ПроверитьОтчетыОПродажахЗаКассовыйДень(Магазин, ДанныеКассовогоДня.КассовыйДень, ДанныеКассовогоДня.Сообщение) Тогда
				...
				КассовыйДеньОбъект.Дата                  = ТекущаяДатаСеанса();
...
Функция КонтрольнаяСуммаКассовогоДняКасса(КассовыйДень)	Экспорт
	...
	СуммаДокумента = Результаты[4].Выгрузить().Итог("Сумма");
...
Функция ПолучитьСуммуНакоплений(ВладелецКарты, ВидКарты)	Экспорт
	...
	//	ВидКарты - истина значит карта сотрудника
		...
		Иначе
			...
			//2025-06-25 sa решили использовать в качестве суммы покупок оборот по регистру. Т.к. в магазинных базах остаток стал отличаться от оборота.
			//В центральной базе оборот совпадает с остатком.
			Запрос = Новый Запрос(
			//"ВЫБРАТЬ
			...
			|ИЗ
			|	РегистрНакопления.ПродажиПоДисконтнымКартам.Обороты(, , , ВладелецДисконтнойКарты = &ВладелецКарты) КАК ПродажиПоДисконтнымКартамОбороты"
			);
			Запрос.УстановитьПараметр("ВладелецКарты", ВладелецКарты);
...
Функция НовыеСтикеры()	Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	УчетнаяПолитикаМагазиновСрезПоследних.НовыеСтикеры КАК НовыеСтикеры
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаМагазинов.СрезПоследних(&Период, Магазин = &Магазин) КАК УчетнаяПолитикаМагазиновСрезПоследних"
	);
	...
	Пока выборка.Следующий() цикл
		...
		НовыеСтикеры = выборка.НовыеСтикеры;
	КонецЦикла;	
	Возврат НовыеСтикеры;

...
Функция ПреобразоватьТабельВJSON(ДокументТабель) Экспорт
	...
	Для Каждого СтрокаТЧ ИЗ ДокументТабель.ДанныеОВремени Цикл   
		...
		СотрудникРаботает = ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(СтрокаТЧ.Сотрудник,СтрокаТЧ.Должность,ДокументТабель.Магазин,КонецДня(ДокументТабель.ДатаОкончанияПериода)-2);
		...
		Если ЕстьОтработанноеВремя Тогда

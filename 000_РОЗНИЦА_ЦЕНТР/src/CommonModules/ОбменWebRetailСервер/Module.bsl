...
Функция ПреобразоватьТабельВJSON(ДокументТабель) Экспорт
	...
	Для Каждого СтрокаТЧ ИЗ ДокументТабель.ДанныеОВремени Цикл   
		...
		СотрудникРаботает = ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(СтрокаТЧ.Сотрудник,ДокументТабель.Магазин,ДокументТабель.ДатаОкончанияПериода);
...
Функция ПроверитьЧтоСотрудникЭтогоМагазина(Сотрудник, Магазин,Дата = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&КонецПериода, Подразделение = &Магазин) КАК РаботникиОрганизацийСрезПоследних

...
Функция ПреобразоватьТабельВJSON(ДокументТабель) Экспорт
	...
	Для Каждого СтрокаТЧ ИЗ ДокументТабель.ДанныеОВремени Цикл   
		...
		СотрудникРаботает = ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(СтрокаТЧ.Сотрудник,ДокументТабель.ДатаОкончанияПериода);
...
Функция ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(Сотрудник,Дата) Экспорт

	//Возврат Не Сотрудник.Уволен;
	//2025-09-02 sa  таки нужно проверять на дату
	Запрос = Новый Запрос;

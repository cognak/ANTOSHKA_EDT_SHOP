...
Функция ПреобразоватьТабельВJSON(ДокументТабель) Экспорт
	...
	Для Каждого СтрокаТЧ ИЗ ДокументТабель.ДанныеОВремени Цикл   
		//2025-07-02 sa договорились что уволенных сотрудников не выгружаем в кадровую базу. 
		//2026-02-18 sa на собрании с Дубинкиной приняли решение, что сотрудник увольняющийся в последний день текущего периода, не должен попадать в этот табель.
		//если же сотрудник переводится с этого магазина 15 или 31 числа, то он должен быть табелирован в старом магазине.
		//2026-08-17 sa по факту работает так: если сотрудник уволен в любой день, он уже не должен попадать в табель.
		СотрудникРаботает = ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(СтрокаТЧ.Сотрудник,ДокументТабель.Магазин,КонецДня(ДокументТабель.ДатаОкончанияПериода));
		Если Не СотрудникРаботает Тогда
			Продолжить;
		КонецЕсли;
		//2025-09-04 sa договорились о возможности табелирования сотрудников в любом магазине, но выгружать в кадровую базу можно только тех
		//сотрудников которые приписаны к этому магазину. Остальные нужны в табеле для информации.
		Если НЕ ПроверитьЧтоСотрудникЭтогоМагазина(СтрокаТЧ.Сотрудник,ДокументТабель.Магазин,ДокументТабель.ДатаОкончанияПериода) Тогда
			Продолжить;	
		КонецЕсли;
		...
		ЕстьОтработанноеВремя = Ложь;
		// 2026-03-16 sa Иногда надо добавить сотрудника в табель. Чтобы это сделать удобно, примем, что если на сотрудника задан план, то он выгружается в кадровую базу.
		ЕстьПланРабот = Ложь; 
		Для н = 1 По 31 Цикл
			...
			Если ЧасовФакт = 0 Тогда
				ЧасовФакт = ЧасовАвто;
			КонецЕсли;
			ЧасовПлан = СтрокаТЧ["Часов"+н];
			Если ЧасовПлан > 0 Тогда
				ЕстьПланРабот = Истина;
			КонецЕсли;
		...
		КонецЕсли;
		//2026-02-17 sa если сотрудник беременный, мобилизованный и просто числится работающим, но при этом
		//ни разу не сканировался в табель, то таких сотрудников не выгружаем в табельную базу. Договорено с Дубинкиной 18.02.2026
		// 2026-03-16 sa Иногда надо добавить сотрудника в табель. Чтобы это сделать удобно, примем, что если на сотрудника задан план, то он выгружается в кадровую базу.
		Если ЕстьОтработанноеВремя ИЛИ ЕстьПланРабот Тогда
...
//2025-12-04 sa попробуем опираться только на регистр РаботникиОрганизаций и на занятость сотрудника. По идее в синхронизированной базе, это должно работать.
Функция ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(Сотрудник,Магазин,Дата) Экспорт
	
	//2026-03-16 sa Есть проблема, когда сотрудника уволили с Основного места работы, но переоформили как совместителя или внешнего совместителя (ФОП)
	//Если увольнение было последним документом, то проблема. Поэтому проверяем по виду занятости по-отдельности.
	Если ПроверитьЧтоСотрудникРаботаетНаКонецМесяцаУниверсальный(Сотрудник,Магазин,Дата,Перечисления.ВидыЗанятости.ОсновноеМестоРаботы) Тогда
		Возврат Истина;
	ИначеЕсли ПроверитьЧтоСотрудникРаботаетНаКонецМесяцаУниверсальный(Сотрудник,Магазин,Дата,Перечисления.ВидыЗанятости.ВнутреннееСовместительство) Тогда
		Возврат Истина;
	ИначеЕсли ПроверитьЧтоСотрудникРаботаетНаКонецМесяцаУниверсальный(Сотрудник,Магазин,Дата,Перечисления.ВидыЗанятости.Совместительство) Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;	
...
//2025-12-04 sa попробуем опираться только на регистр РаботникиОрганизаций и на занятость сотрудника. По идее в синхронизированной базе, это должно работать.
Функция ПроверитьЧтоСотрудникРаботаетНаКонецМесяцаУниверсальный(Сотрудник,Магазин,Дата,ВидЗанятости) 

	//2025-09-15 sa все-таки проблема комплексная. С одной стороны если табель за прошлый месяц и сотрудник проработал его полностью - он должен быть в табеле и на выгрузке.
	//с другой, если сотрудника уволили в текущем месяце, то его уже расчитали и в табеле его быть не должно.
	//с треьтей стороны, в декабре ЗП считают 25 декабря и если у сотрудника стоит дата увольнения с 01.01.2026, то он неправильно считатется.
	//Попробуем еще раз определять заботает ли сотрудник на дату окончания периода.
	//Если НачалоМесяца(Дата) = НачалоМесяца(ТекущаяДата()) Тогда
	//	Возврат Не Сотрудник.Уволен;
	//Иначе 
	//2026-01-15 sa сотрудники прыгают между должностями их увольняют и из-за этого проблема.       
	//2026-02-10 sa Решили, что должности теперь не влияют на формирование табеля, поэтому теперь нам важно только то, чтобы сотрудник работал в этом магазине
	//неважно на какой должности
		//если сотрудник числится в этом подразделении, то у него должно быть Занимаемых ставок > 0 
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	ЕСТЬNULL(РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок, 0) КАК ЗанимаемыхСтавок
		               |ИЗ
		               ...
		               |					И ВидЗанятости = &ВидЗанятости) КАК РаботникиОрганизацийСрезПоследних
		               |		ПО (РаботникиОрганизацийСрезПоследних.Сотрудник = ФизическиеЛица.Ссылка)
		               |ГДЕ
...
//2025-09-04 sa Проверяет, что сотрудник является сотрудником этого магазина.
Функция ПроверитьЧтоСотрудникЭтогоМагазина(Сотрудник, Магазин,Дата = Неопределено) Экспорт
	
	Возврат ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(Сотрудник,Магазин,Дата);
	
	Если НЕ ЗначениеЗаполнено(Сотрудник) Тогда
		Возврат Ложь; //неясно что возвращать в этом случае, ну пусть будет ложь.	
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОрганизацииПодразделений.СрезПоследних(&КонецПериода, Владелец = &Магазин) КАК ОрганизацииПодразделенийСрезПоследних
	               |		ПО РаботникиОрганизацийСрезПоследних.Подразделение = ОрганизацииПодразделенийСрезПоследних.Владелец
	               |			И РаботникиОрганизацийСрезПоследних.Организация = ОрганизацииПодразделенийСрезПоследних.Организация
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Период УБЫВ";
	...
	//Возврат Сотрудник.Магазин = Магазин; 
КонецФункции

 Функция ПолучитьОрганизацию(Магазин) Экспорт

...
Функция ПреобразоватьТабельВJSON(ДокументТабель) Экспорт
	...
	Для Каждого СтрокаТЧ ИЗ ДокументТабель.ДанныеОВремени Цикл   
		...
		СотрудникРаботает = ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(СтрокаТЧ.Сотрудник,СтрокаТЧ.Должность,ДокументТабель.Магазин,КонецДня(ДокументТабель.ДатаОкончанияПериода));
...
Функция ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(Сотрудник,Должность,Магазин,Дата) Экспорт
	...
	//2026-01-15 sa сотрудники прыгают между должностями их увольняют и из-за этого проблема.
		...
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		               ...
		               |	И РаботникиОрганизацийСрезПоследних.Должность = &Должность
		               |
		               |УПОРЯДОЧИТЬ ПО
...
Функция ПроверитьЧтоСотрудникРаботаетНаКонецМесяца_СТАРЫЙ(Сотрудник,Магазин,Дата) Экспорт
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ЕСТЬNULL(ВложенныйЗапрос.СостояниеСотрудника, ЗНАЧЕНИЕ(Перечисление.ПричиныИзмененияСостояния.Увольнение)) КАК Поле1
		               |ИЗ
		               |	Справочник.ФизическиеЛица КАК ФизическиеЛица
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеРаботниковОрганизаций.СрезПоследних(
		               |				&КонецПериода,
		               |				Сотрудник = &Сотрудник
		               |					И Подразделение = &Магазин) КАК СостояниеРаботниковОрганизацийСрезПоследних
		               |		ПО (СостояниеРаботниковОрганизацийСрезПоследних.Сотрудник = ФизическиеЛица.Ссылка),
		               |	(ВЫБРАТЬ
		               |		ВложенныйЗапрос.Организация КАК Организация,
		               |		ВложенныйЗапрос.Сотрудник КАК Сотрудник,
		               |		ВложенныйЗапрос.ПериодНачала КАК ПериодНачала,
		               ...
		               |				И РаботникиОрганизаций.Подразделение = &Магазин
		               |			
		               |			СГРУППИРОВАТЬ ПО
		               |				РаботникиОрганизаций.Организация,
		               |				РаботникиОрганизаций.Сотрудник,
		               |				РаботникиОрганизаций.Подразделение) КАК ВложенныйЗапрос
		               |			ПО РаботникиОрганизаций.Организация = ВложенныйЗапрос.Организация
		               |				И РаботникиОрганизаций.Сотрудник = ВложенныйЗапрос.Сотрудник
		               |				И РаботникиОрганизаций.Подразделение = ВложенныйЗапрос.Подразделение) КАК ВложенныйЗапрос
		               |ГДЕ
		               |	(СостояниеРаботниковОрганизацийСрезПоследних.Состояние В (&СостоянияРаботающих)
		               |			ИЛИ ВложенныйЗапрос.СостояниеСотрудника = ЗНАЧЕНИЕ(Перечисление.ПричиныИзмененияСостояния.ПриемНаРаботу))"; 
		...
		МассивСостоянийРаботающихСотрудников.Добавить(Перечисления.СостоянияРаботникаОрганизации.Заболевание);

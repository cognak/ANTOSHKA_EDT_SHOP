...
Функция ПреобразоватьТабельВJSON(ДокументТабель) Экспорт
	...
	Для Каждого СтрокаТЧ ИЗ ДокументТабель.ДанныеОВремени Цикл   
		...
		Если НЕ ПроверитьЧтоСотрудникЭтогоМагазина(СтрокаТЧ.Сотрудник,ДокументТабель.Магазин) Тогда
			Продолжить;	
		КонецЕсли;
...
Функция ПроверитьЧтоСотрудникРаботаетНаКонецМесяца(Сотрудник,Магазин,Дата) Экспорт
	...
	Иначе
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеРаботниковОрганизаций.СрезПоследних(&КонецПериода, Сотрудник = &Сотрудник) КАК СостояниеРаботниковОрганизацийСрезПоследних
		               ...
		               |				И РаботникиОрганизаций.Сотрудник = ВложенныйЗапрос.Сотрудник) КАК ВложенныйЗапрос
		               |ГДЕ
...
Функция ПроверитьЧтоСотрудникЭтогоМагазина(Сотрудник, Магазин) Экспорт

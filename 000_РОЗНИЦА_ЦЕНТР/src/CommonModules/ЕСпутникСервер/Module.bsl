...
Функция ОтправитьЗаказы(СтруктураВыгрузки, Использовать_V3 = Ложь)
	...
	Если Не (HTTPОтвет.КодСостояния = 200 Или HTTPОтвет.КодСостояния = 201) Тогда 
		...
		ДанныеДляПочты = "Ошибка отправки заказа";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + "/////Запрос/////";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + СтруктураВыгрузки.ТекстЗапроса;
		...
		НастройкиОповещения.Вставить("ТелоСообщения"   , ДанныеДляПочты);
...
Функция ОтправитьСобытие(СтруктураВыгрузки, Использовать_V3 = Ложь)
	...
	Если Не (HTTPОтвет.КодСостояния = 200 Или HTTPОтвет.КодСостояния = 201) Тогда 
		...
		ДанныеДляПочты = "Ошибка отправки события";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + "/////Запрос/////";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + СтруктураВыгрузки.ТекстЗапроса;
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + "/////Ответ/////";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + ТекстОтвета;
	...
	Если СтруктураВыгрузки.УзелОбмена.ЛогироватьОтветыСервера Тогда     
		
		ПредставлениеДанных = "================ЗАГОЛОВКИ================" + Символы.ПС;
		
		Для Каждого Эл Из HTTPОтвет.Заголовки Цикл 
			ПредставлениеДанных = ПредставлениеДанных + Эл.Ключ + " : " + Эл.Значение + Символы.ПС;	
		КонецЦикла;  
		
		ПредставлениеДанных = ПредставлениеДанных + Символы.ПС + "================ОТВЕТ================" + Символы.ПС;
		
		ПредставлениеДанных = ПредставлениеДанных + HTTPОтвет.ПолучитьТелоКакСтроку();	
		
		Если ТипЗнч(СтруктураВыгрузки.ДанныеДляОтправки) = Тип("РегистрСведенийНаборЗаписей.СтатусыЭН") Тогда
        	ДанныеДляОтправки = СтруктураВыгрузки.ДанныеДляОтправки;
		Иначе 
        	ДанныеДляОтправки = СтруктураВыгрузки.ДанныеДляОтправки.Ссылка;
		КонецЕсли;	
		
		ЖурналСобытий.Регистрация(СокрЛП(СтруктураВыгрузки.УзелОбмена.Наименование),, СтруктураВыгрузки.УзелОбмена.Метаданные().Имя, ДанныеДляОтправки,, СтруктураВыгрузки.ТекстЗапроса, ПредставлениеДанных);
...
Функция ОтправитьКонтакты(СтруктураВыгрузки)
	...
	Если Не (HTTPОтвет.КодСостояния = 200 Или HTTPОтвет.КодСостояния = 201) Тогда 
		...
		ДанныеДляПочты = "Ошибка создания клиента";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + "/////Запрос/////";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + СтруктураВыгрузки.ТекстЗапроса;
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + "/////Ответ/////";
		ДанныеДляПочты = ДанныеДляПочты + Символы.ПС + ТекстОтвета;
...
Функция ДобавитьКонтакт(СтруктураВыгрузки)
	...
	РаботаСJSONСервер.ЗаписатьСвойство("dedupeOn"			, "sms", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("restoreDeleted"		, true, ЗаписьJSONКонтакта);
	...
	РаботаСJSONСервер.ЗаписатьСвойство("externalCustomerId"	, Строка(ДанныеОКонтакте.Контрагент.УникальныйИдентификатор()), ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("firstName"			, СокрЛП(ДанныеОКонтакте.Имя), ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("lastName"			, СокрЛП(ДанныеОКонтакте.Фамилия), ЗаписьJSONКонтакта);

	ЗаписьJSONКонтакта.ЗаписатьИмяСвойства("channels");
	ЗаписьJSONКонтакта.ЗаписатьНачалоМассива();
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("type"				, "sms", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, Строка(ДанныеОКонтакте.НомерТелефона), ЗаписьJSONКонтакта);
	...
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "26369", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, Формат(ДанныеОКонтакте.ДатаРождения, "ДФ=гггг-ММ-дд"), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "26370", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, ?(ДанныеОКонтакте.Пол = Перечисления.ПолФизическогоЛица.Женский,"ж","м"), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	Если Не КоличествоДетей = 0 Тогда 
		ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
		РаботаСJSONСервер.ЗаписатьСвойство("id"					, "35558", ЗаписьJSONКонтакта);
		РаботаСJSONСервер.ЗаписатьСвойство("value"				, КоличествоДетей, ЗаписьJSONКонтакта);
		ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	КонецЕсли;
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "46892", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, "да", ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	//ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	//РаботаСJSONСервер.ЗаписатьСвойство("id"					, "52471", ЗаписьJSONКонтакта);
	//РаботаСJSONСервер.ЗаписатьСвойство("value"				, ?(ДанныеОКонтакте.Пол = Перечисления.ПолФизическогоЛица.Женский,"ж","м"), ЗаписьJSONКонтакта);
	//ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	...
	Пока Выборка.Следующий() Цикл
		ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
		РаботаСJSONСервер.ЗаписатьСвойство("id"					, Строка(Формат(СтруктураДетей[ТекущийРебенок].День, "ЧГ=")), ЗаписьJSONКонтакта);
		РаботаСJSONСервер.ЗаписатьСвойство("value"				, Формат(Выборка.ДатаРождения, "ДФ=гггг-ММ-дд"), ЗаписьJSONКонтакта);
		ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
		ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
		РаботаСJSONСервер.ЗаписатьСвойство("id"					, Строка(Формат(СтруктураДетей[ТекущийРебенок].Пол, "ЧГ=")), ЗаписьJSONКонтакта);
		РаботаСJSONСервер.ЗаписатьСвойство("value"				, ?(Выборка.Пол = Перечисления.ПолФизическогоЛица.Женский,"ж","м"), ЗаписьJSONКонтакта);
		ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
		ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
		РаботаСJSONСервер.ЗаписатьСвойство("id"					, Строка(Формат(СтруктураДетей[ТекущийРебенок].Имя, "ЧГ=")), ЗаписьJSONКонтакта);
		РаботаСJSONСервер.ЗаписатьСвойство("value"				, СокрЛП(Выборка.Имя), ЗаписьJSONКонтакта);
		ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
...
Функция ДобавитьОбновитьКонтрагента(СтруктураВыгрузки, Контрагент) Экспорт 
	...
	|	ИнформационныеКарты.ИнформационнаяКарта КАК ДисконтнаяКарта,
	|	Контрагенты.Пол КАК Пол,
	|	КонтрагентыКонтактнаяИнформация.НомерТелефона КАК НомерТелефона,
	|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Контрагенты.Ссылка) КАК IDC
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(, ) КАК ФИОФизЛицСрезПоследних
	|		ПО Контрагенты.Ссылка = ФИОФизЛицСрезПоследних.ФизЛицо
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИнформационныеКарты КАК ИнформационныеКарты
	...
	РаботаСJSONСервер.ЗаписатьСвойство("externalCustomerId"	, Строка(ДанныеОКонтакте.IDC), ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("firstName"			, СокрЛП(ДанныеОКонтакте.Имя), ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("lastName"			, СокрЛП(ДанныеОКонтакте.Фамилия), ЗаписьJSONКонтакта);
	...
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("type"				, "email", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, Строка(Справочники.Контрагенты.Получить_EMail(ДанныеОКонтакте.Контрагент)), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	ЗаписьJSONКонтакта.ЗаписатьКонецМассива();		
	...
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "196510", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, СписокМагазинов, ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "196515", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, СписокМагазинов, ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "196513", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, ПоследнийМагазин, ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "196514", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, МакПосящаемыйМагазин, ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "201552", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, ПолучитьПроцентСкидкиКлиента(Контрагент.Ссылка), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта(); 
	
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "201552", ЗаписьJSONКонтакта);
	//РаботаСJSONСервер.ЗаписатьСвойство("value"				, ВыборкаСсылка.ПроцентСкидки, ЗаписьJSONКонтакта);  
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, ПолучитьПроцентСкидкиКлиента(Контрагент.Ссылка), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();  
	...
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("id"					, "291161", ЗаписьJSONКонтакта);
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, ?(НЕ Контрагент.МаркетинговыеГруппы.Найти(Справочники.МаркетинговыеГруппы.Е_Малятко,"Группа") = Неопределено И Контрагент.Дети.Количество() > 0, "Так", "Ні"), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
	...
	Заголовки.Вставить("Content-Type", "application/json");
	HTTPЗапрос = Новый HTTPЗапрос("/api/v1/contact", Заголовки);
	HTTPЗапрос.УстановитьТелоИзСтроки(ТекстЗапроса, КодировкаТекста.UTF8);
	HTTPОтвет = HTTP.ОтправитьДляОбработки(HTTPЗапрос);
...
Функция ЭтоКонтрагентРозницы(Контрагент)
	...
	|	Справочник.Контрагенты КАК Контрагенты
	|ГДЕ
	|	Контрагенты.Родитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.Покупатели)
...
Функция ПолучитьПроцентСкидкиКлиента(Контрагент)
	...
	|	ВЫБОР
	|		КОГДА СУММА(ПродажиПоДисконтнымКартамОбороты.СуммаОборот) МЕЖДУ 0 И 7000
	|			ТОГДА 3
	|		КОГДА СУММА(ПродажиПоДисконтнымКартамОбороты.СуммаОборот) МЕЖДУ 7000 И 15000
	|			ТОГДА 5
	|		КОГДА СУММА(ПродажиПоДисконтнымКартамОбороты.СуммаОборот) МЕЖДУ 15000 И 25000
	|			ТОГДА 7
	|		КОГДА СУММА(ПродажиПоДисконтнымКартамОбороты.СуммаОборот) > 25000
	|			ТОГДА 10
	|	КОНЕЦ КАК ПроцентСкидки
	|ИЗ
	|	РегистрНакопления.ПродажиПоДисконтнымКартам.Обороты(, , Авто, ВладелецДисконтнойКарты = &Контрагент) КАК ПродажиПоДисконтнымКартамОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ПродажиПоДисконтнымКартамОбороты.ВладелецДисконтнойКарты";
	Запрос.УстановитьПараметр("Контрагент", Контрагент); //Контрагент, Физическое лицо
	
	Результат = Запрос.Выполнить().Выбрать();
	
	Если Результат.Следующий() Тогда  
		Возврат Результат.ПроцентСкидки;	
	Иначе 
		Возврат 0;	
	КонецЕсли;	
...
Функция ВыгрузкаЕСпутник(СтруктураВыгрузки) 
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		РаботаСJSONСервер.ЗаписатьСвойство("externalOrderId"		, Строка(ВыборкаСсылка.Ссылка.УникальныйИдентификатор()), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("externalCustomerId"	, Строка(ВыборкаСсылка.НомерТелефона), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("totalCost"			, СуммаДокумента, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("status"				, СтатусДокумента, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("date"				, Формат(ВыборкаСсылка.Дата,"ДФ=yyyy-MM-ddTHH:mm:ss.0000000+02:00"), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("phone"				, Строка(ВыборкаСсылка.НомерТелефона), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("firstName"			, СокрЛП(ВыборкаСсылка.Имя), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("lastName"			, СокрЛП(ВыборкаСсылка.Фамилия), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("currency"			, "UAH", ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("shipping"			, СуммаДоставки, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("source"				, ИсточникДокумента, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("storeId"				, Строка(СтрЗаменить(ВыборкаСсылка.МагазинНаименование, ",", "")), ЗаписьJSON);
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
			...
			РаботаСJSONСервер.ЗаписатьСвойство("externalItemId"			, Строка(ВыборкаДетальныеЗаписи.IDN), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("name"					, СокрЛП(ВыборкаДетальныеЗаписи.Наименование), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("category"				, СокрЛП(ВыборкаДетальныеЗаписи.ВидНоменклатуры), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("quantity"				, ВыборкаДетальныеЗаписи.Количество, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("cost"					, ВыборкаДетальныеЗаписи.Цена, ЗаписьJSON);
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		РаботаСJSONСервер.ЗаписатьСвойство("firstName"			, СокрЛП(ВыборкаСсылка.Имя), ЗаписьJSONКонтакта);
		РаботаСJSONСервер.ЗаписатьСвойство("lastName"			, СокрЛП(ВыборкаСсылка.Фамилия), ЗаписьJSONКонтакта);
		РаботаСJSONСервер.ЗаписатьСвойство("externalCustomerId"	, СокрЛП(ВыборкаСсылка.Контрагент.УникальныйИдентификатор()), ЗаписьJSONКонтакта);
		
		ЗаписьJSONКонтакта.ЗаписатьИмяСвойства("channels");
		ЗаписьJSONКонтакта.ЗаписатьНачалоМассива();
		ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
		РаботаСJSONСервер.ЗаписатьСвойство("type"				, "sms", ЗаписьJSONКонтакта);
		РаботаСJSONСервер.ЗаписатьСвойство("value"				, Строка(ВыборкаСсылка.НомерТелефона), ЗаписьJSONКонтакта);
		ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();
		ЗаписьJSONКонтакта.ЗаписатьКонецМассива();		
...
Функция ВыгрузкаЕСпутник_V3(СтруктураВыгрузки) Экспорт 
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		Если (ОтветСервера.КодСостояния = 200 Или ОтветСервера.КодСостояния = 201) Тогда 
			...
			Если ТипЗнч(ДанныеОтвета) = Тип("Массив") И ДанныеОтвета.Количество() = 0 Тогда 
				...
				СтруктураВыгрузки.ТекстЗапроса = ВыборкаСсылка;
				//Если Не ДобавитьКонтакт(СтруктураВыгрузки) Тогда 
				Если ДобавитьОбновитьКонтрагента(СтруктураВыгрузки, ВыборкаСсылка.Контрагент) Тогда 	
					ЗаписьЖурналаРегистрации("Ошибка добовления нового контакта в ЕСпутник", УровеньЖурналаРегистрации.Ошибка,,, "Ответ от серверане 200");
					Возврат Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		РаботаСJSONСервер.ЗаписатьСвойство("eventTypeKey"		, ТипСобытия, ЗаписьJSON);   
		...
		РаботаСJSONСервер.ЗаписатьСвойство("externalOrderId"		, Строка(ВыборкаСсылка.Ссылка.УникальныйИдентификатор()), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("externalCustomerId"	, Строка(ВыборкаСсылка.Контрагент.УникальныйИдентификатор()), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("totalCost"			, СуммаДокумента, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("status"				, СтатусДокумента, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("date"				, Формат(ВыборкаСсылка.Дата,"ДФ=yyyy-MM-ddTHH:mm:ss+02:00"), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("phone"				, Строка(ВыборкаСсылка.НомерТелефона), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("firstName"			, СокрЛП(ВыборкаСсылка.Имя), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("lastName"			, СокрЛП(ВыборкаСсылка.Фамилия), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("currency"			, "UAH", ЗаписьJSON);
		...
		РаботаСJSONСервер.ЗаписатьСвойство("source"				, ИсточникДокумента, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("storeId"				, Строка(СтрЗаменить(ВыборкаСсылка.МагазинНаименование, ",", "")), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("paymentMethod"		, Строка(СтрЗаменить(ВыборкаСсылка.ТипОплаты, ",", "")), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("deliveryMethod"		, Строка(СтрЗаменить(ВыборкаСсылка.ТипДоставки, ",", "")), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("storeName"			, Строка(СтрЗаменить(ВыборкаСсылка.МагазинНаименование, ",", "")), ЗаписьJSON);
		...
		//2026-06-25 sa дополнительно отправляем в М2. По договоренности отправляем только заказ
		Если ТипЗнч(СтруктураВыгрузки.ДанныеДляОтправки) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда
			//Обернем в попытку, т.к. проблемы на стороне М2 валят отправку уведомлений в ЕСпутник
			Попытка
				ВнешниеИсточникиGraphQL.ОтправитьСтатусЗаказа(ТекстЗапроса); 
			Исключение
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		ДобавитьОбновитьКонтрагента(СтруктураВыгрузки, ВыборкаСсылка.Контрагент);
...
Функция ВыгрузкаЕСпутникСобытия(СтруктураВыгрузки)
	...
	Если Не ПустаяСтрока(СтруктураСобытия.ТипСобытия) Тогда
		...
		Пока ВыборкаСсылка.Следующий() Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
			РаботаСJSONСервер.ЗаписатьСвойство("externalOrderId"		, СокрЛП(Строка(СтруктураСобытия.externalOrderId)), ЗаписьJSON); 
			РаботаСJSONСервер.ЗаписатьСвойство("CustomerId"			, СокрЛП(ВыборкаСсылка.Имя), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("numTTN"				, СокрЛП(Строка(СтруктураСобытия.numTTN)), ЗаписьJSON); 
			РаботаСJSONСервер.ЗаписатьСвойство("shopAddress"			, СокрЛП(Строка(СтруктураСобытия.shopAddress)), ЗаписьJSON); 
			РаботаСJSONСервер.ЗаписатьСвойство("skus"				, СтруктураСобытия.skus, ЗаписьJSON); 
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
			РаботаСJSONСервер.ЗаписатьСвойство("name"				, "json", ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("value"				, ТекстЗапросаJSON, ЗаписьJSON);
			ЗаписьJSON.ЗаписатьКонецОбъекта(); 
			
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
			РаботаСJSONСервер.ЗаписатьСвойство("name"				, "PhoneNumber", ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("value"				, СокрЛП(Строка(ВыборкаСсылка.НомерТелефона)), ЗаписьJSON);
			ЗаписьJSON.ЗаписатьКонецОбъекта(); 

			ЗаписьJSON.ЗаписатьКонецМассива();
			
			РаботаСJSONСервер.ЗаписатьСвойство("eventTypeKey"		, СтруктураСобытия.ТипСобытия, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("keyValue"			, СокрЛП(Строка(СтруктураСобытия.keyValue)), ЗаписьJSON); 
...
Функция ВыгрузкаЕСпутникСобытия_V3(СтруктураВыгрузки) Экспорт 
	...
	Если ТипЗнч(ОбъектСобытия) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда
		...
		Если ОбъектСобытия.Статус = Перечисления.СтатусыЗаказовПокупателей.Новый ИЛИ ОбъектСобытия.Статус = Перечисления.СтатусыЗаказовПокупателей.ВРаботе Тогда 
	...
	ИначеЕсли ТипЗнч(ОбъектСобытия) = Тип("РегистрСведенийНаборЗаписей.СтатусыЭН") Тогда 
		...
		|	СтатусыЭНСрезПоследних.ДокументРегистратор.ЗаказПокупателя КАК ЗаказПокупателя,
		|	СтатусыЭНСрезПоследних.ДокументРегистратор.ДокументОснование.ТипДоставки КАК ТипДоставки,
		|	СтатусыЭНСрезПоследних.ДокументРегистратор.ДокументОснование.Номер КАК Номер,
		|	СтатусыЭНСрезПоследних.ДокументРегистратор.Номер КАК СсылкаНаЭННомер,
		|	СтатусыЭНСрезПоследних.СтатусЭН.СтатусПосылки КАК СтатусПосылки
		|ИЗ
		|	РегистрСведений.СтатусыЭН.СрезПоследних(, ДокументРегистратор = &СсылкаНаЭН) КАК СтатусыЭНСрезПоследних
		|ГДЕ
		|	СтатусыЭНСрезПоследних.ДокументРегистратор.ВозвратнаяНакладная = ЛОЖЬ
		|			И СтатусыЭНСрезПоследних.ДокументРегистратор.ПереадресацияНакладная = ЛОЖЬ";
	...
	ИначеЕсли ТипЗнч(ОбъектСобытия) = Тип("СправочникОбъект.Контрагенты") Тогда 
		
		Если ОбъектСобытия.МаркетинговыеГруппы.Найти(Справочники.МаркетинговыеГруппы.Е_Малятко,"Группа") = Неопределено И НЕ ОбъектСобытия.Дети.Количество() > 0 Тогда 
			Возврат Истина;	
		КонецЕсли;	
		
		СтруктураСобытия.ТипСобытия 		= "eMalyatko";
		СтруктураСобытия.externalCustomerId = ОбъектСобытия.Ссылка.УникальныйИдентификатор();
		СтруктураСобытия.keyValue 			= СтруктураСобытия.externalCustomerId;
	
	КонецЕсли;

	Если Не ПустаяСтрока(СтруктураСобытия.ТипСобытия) И НЕ СтруктураСобытия.ТипСобытия = "eMalyatko" Тогда
		...
		Пока ВыборкаСсылка.Следующий() Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта(); 
			РаботаСJSONСервер.ЗаписатьСвойство("phone"				, ВыборкаСсылка.НомерТелефона, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("externalCustomerId"	, СокрЛП(Строка(СтруктураСобытия.ЗаказПокупателя.Контрагент.УникальныйИдентификатор())), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("externalOrderId"		, СокрЛП(Строка(СтруктураСобытия.externalOrderId)), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("CustomerId"			, СокрЛП(ВыборкаСсылка.Имя), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("numTTN"				, СокрЛП(Строка(СтруктураСобытия.numTTN)), ЗаписьJSON); 
			РаботаСJSONСервер.ЗаписатьСвойство("shopAddress"			, СокрЛП(Строка(СтруктураСобытия.shopAddress)), ЗаписьJSON); 
			...
			//2026-06-05 sa выгружаем данные в М2
			//Обернем в попытку, т.к. проблемы на стороне М2 валят отправку уведомлений в ЕСпутник
			Попытка
				//2026-06-16 sa М2 не готовы принимать статусы по запросам доступности и т.д. Ограничимся только заказами. 
				Если ТипЗнч(ОбъектСобытия) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда				
					ВнешниеИсточникиGraphQL.ОтправитьСтатусЗаказа(ТекстЗапроса); 
				ИначеЕсли СтруктураСобытия.ТипСобытия = "order_delivered" Тогда
					ВнешниеИсточникиGraphQL.ОтправитьСтатусЗаказа(ТекстЗапроса); 
				КонецЕсли;
			Исключение
				ЗаписьЖурналаРегистрации("Ошибка отправки статуса в M2", УровеньЖурналаРегистрации.Ошибка,,, "Ошибка: "+ОписаниеОшибки());
	...
	ИначеЕсли СтруктураСобытия.ТипСобытия = "eMalyatko" Тогда  
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта(); 
		РаботаСJSONСервер.ЗаписатьСвойство("phone"				, Справочники.Контрагенты.ПолучитьНомерТелефона(ОбъектСобытия.Ссылка), ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("externalCustomerId"	, СокрЛП(Строка(СтруктураСобытия.externalCustomerId)), ЗаписьJSON);
		...
		Если Не ОтправитьСобытие(СтруктураВыгрузки, Истина) Тогда 
			ЗаписьЖурналаРегистрации("Ошибка отправки данных в ЕСпутник", УровеньЖурналаРегистрации.Ошибка,,, "Ответ от сервера не 200");
			Возврат Ложь;
		КонецЕсли;
...
Процедура ВыполнитьОбмен(УзелОбмена, ИнтерактивныйОбмен = Ложь, ТипОбмена = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ИначеЕсли ВРег(ТипОбмена) = "ЗАКАЗЫ" И (ТипЗнч(ДанныеДляОтправки) = Тип("СправочникОбъект.Контрагенты")) Тогда 			
				
				Если ЭтоКонтрагентРозницы(ДанныеДляОтправки.Ссылка) Тогда 
					СообщениеОтправлено = ДобавитьОбновитьКонтрагента(СтруктураВыгрузки, ДанныеДляОтправки.Ссылка);
				КонецЕсли;	
			...
			Если ВРег(ТипОбмена) = "СОБЫТИЯ" 
				...
				Или ТипЗнч(ДанныеДляОтправки) = Тип("ДокументОбъект.ВозвратИнтернетЗаказа")) 
				Или ТипЗнч(ДанныеДляОтправки) = Тип("СправочникОбъект.Контрагенты")  Тогда
				СтруктураВыгрузки.ДанныеДляОтправки = ДанныеДляОтправки;  
				
				Если ТипЗнч(ДанныеДляОтправки) = Тип("СправочникОбъект.Контрагенты") Тогда //Проверка: Если нет контрагента в системе подождем пока появится
					
					СтруктураВыгрузки.ТекстЗапроса = СтруктураВыгрузки.ДанныеДляОтправки.Ссылка.УникальныйИдентификатор();
					
					Ответ = ПроверитьКонтакт(СтруктураВыгрузки, Истина);
					
					КонтрагентМассив = РаботаСJSONСервер.JSONВЗначение(Ответ.ПолучитьТелоКакСтроку(), ,Ложь);
					
					Если КонтрагентМассив.Количество() = 0 Тогда 
						Продолжить;
					КонецЕсли;	

...
Процедура ВыгрузкаЕСпутникСобытия(СтруктураВыгрузки)
	...
	ДокументСобытия = СтруктураВыгрузки.ДанныеДляОтправки.Ссылка;
	
	СтруктураСобытия = Новый Структура;
	СтруктураСобытия.Вставить("ТипСобытия", "");
	СтруктураСобытия.Вставить("externalOrderId", "");
	
	Если ДокументСобытия.Статус = Перечисления.СтатусыЗаказовПокупателей.Новый Тогда 
		
		СтруктураСобытия.ТипСобытия = "order_new";
		СтруктураСобытия.externalOrderId = ДокументСобытия.Номер;

	ИначеЕсли ДокументСобытия.Статус = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности Тогда 
		
		СтруктураСобытия.ТипСобытия = "order_complectation";
		СтруктураСобытия.externalOrderId = ДокументСобытия.Номер;

	ИначеЕсли ДокументСобытия.Статус = Перечисления.СтатусыЗаказовПокупателей.Доставка Тогда 

//		Запрос = Новый Запрос;
//		Запрос.Текст =
//			"ВЫБРАТЬ
//			|	ЕСТЬNULL(СтатусыЭНСрезПоследних.СтатусЭН, ЗНАЧЕНИЕ(Справочник.КодыСтатусовТТН.ПустаяСсылка)) КАК СтатусЭН,
//			|	ЕСТЬNULL(ЭН.Номер, """") КАК НомерЭН,
//			|	ЗП.Номер КАК НомерЗП
//			|ИЗ
//			|	Документ.ЗаказПокупателя КАК ЗП
...
//			|			ПО СтатусыЭНСрезПоследних.ДокументРегистратор = ЭН.Ссылка
//			|		ПО ЗП.Ссылка = ЭН.ЗаказПокупателя
//			|ГДЕ
//			|	ЗП.Ссылка = &Ссылка";
//		
//		Запрос.УстановитьПараметр("Ссылка", ДокументСобытия);
//		
//		РезультатЗапроса = Запрос.Выполнить();
//		
//		Выборка = РезультатЗапроса.Выбрать();
//		
//		Пока Выборка.Следующий() Цикл
//			Если ПустаяСтрока(Выборка.НомерЭН) Тогда
//				
...
//				
//			ИначеЕсли Не Выборка.СтатусЭН.Пустая() Тогда 
//				
...
//		ТипСобытия = "order_delievery";
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		JSON.ЗаписатьСвойство("keyValue"			, СокрЛП(Строка(ВыборкаСсылка.Номер)), ЗаписьJSON); 
...
Процедура ВыполнитьОбмен(УзелОбмена, ИнтерактивныйОбмен = Ложь, ТипОбмена = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВРег(ТипОбмена) = "ЗАКАЗЫ" И (ТипЗнч(ДанныеДляОтправки) = Тип("ДокументОбъект.ЧекККМ") Или ТипЗнч(ДанныеДляОтправки) = Тип("ДокументОбъект.ЗаказПокупателя")) Тогда
			СтруктураВыгрузки.ДанныеДляОтправки = ДанныеДляОтправки;
			ВыгрузкаЕСпутник(СтруктураВыгрузки);
		КонецЕсли;
		
		//Выгрузка событий
		Если ВРег(ТипОбмена) = "СОБЫТИЯ" И ТипЗнч(ДанныеДляОтправки) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда
			СтруктураВыгрузки.ДанныеДляОтправки = ДанныеДляОтправки;

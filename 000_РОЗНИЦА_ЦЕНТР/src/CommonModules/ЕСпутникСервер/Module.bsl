	...
	Если Не ПустаяСтрока(СтруктураСобытия.ТипСобытия) Тогда
		...
		Если ТипЗнч(ОбъектСобытия) = Тип("ДокументОбъект.ЗаказПокупателя")
			...
			Для Каждого СтрокаСТоваром Из ОбъектСобытия.Товары Цикл
				...
				ОтменаВЗаказе = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСТоваром
																			, "Отменено"
																			, Ложь
																			, Ложь);
				
				ОтменаВЗапросе = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСТоваром
																			, "СтатусЗапроса"
																			, Перечисления.СтатусыЗапросовДоступностиТоваров.ПустаяСсылка()
																			, Ложь);

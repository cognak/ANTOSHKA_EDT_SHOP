...
Функция ДобавитьОбновитьКонтрагента(СтруктураВыгрузки, Контрагент) Экспорт 
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц КАК ФИОКонтрагента
	|		ПО (Контрагенты.Ссылка = ФИОКонтрагента.ФизЛицо)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц КАК ФИОФизЛица
	|		ПО (Контрагенты.Ссылка = ФИОФизЛица.ФизЛицо)
...
Функция ЭтоКонтрагентРозницы(Контрагент)
	...
	|	Справочник.Контрагенты КАК Контрагенты
	|ГДЕ
	|	Контрагенты.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(Справочник.Контрагенты.Покупатели))
	...
	Если Результат.Следующий() Тогда 
		Возврат Результат.Ссылка = Контрагент;	
	Иначе 
		Возврат Ложь;	
	КонецЕсли;	
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ИначеЕсли ВРег(ТипОбмена) = "ЗАКАЗЫ" И (ТипЗнч(ДанныеДляОтправки) = Тип("СправочникОбъект.Контрагенты")) Тогда 	
				...
				//|ГДЕ
				//|	Контрагенты.Ссылка = &Контрагент";
				//Запрос.УстановитьПараметр("Контрагент", ДанныеДляОтправки.Ссылка); //Контрагент
				//
				//Результат = Запрос.Выполнить().Выбрать();
				//
				//Если Результат.Следующий() Тогда 
				//	
				//	СтруктураВыгрузки.Вставить("ТекстЗапроса", Результат);
				//	
				//КонецЕсли;		

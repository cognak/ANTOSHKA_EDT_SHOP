...
Процедура ВыгрузкаЕСпутникСобытия(СтруктураВыгрузки)
	...
	ИначеЕсли ТипЗнч(ОбъектСобытия) = Тип("ДокументОбъект.ЗапросДоступностиТоваров") Тогда 
		...
		Если Выборка.Следующий() Тогда
			...
			СтруктураСобытия.shopAddress = СтруктураАдреса.Представление;
...
//	ИначеЕсли ТипЗнч(ОбъектСобытия) = Тип("ДокументОбъект.ЭлектроннаяНакладная") Тогда 
//	
//		СтруктураСобытия.ЗаказПокупателя = ОбъектСобытия.ЗаказПокупателя;
//
//		Если Не ПустаяСтрока(ОбъектСобытия.Номер)
//				И Не ОбъектСобытия.ЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка() Тогда 
//
//			СтруктураСобытия.ТипСобытия = "order_readynp";
//			СтруктураСобытия.externalOrderId = СтруктураСобытия.ЗаказПокупателя.Номер;
//			СтруктураСобытия.keyValue = ОбъектСобытия.Номер;
//			СтруктураСобытия.numTTN = ОбъектСобытия.Номер;
//
//		КонецЕсли;
//		
	ИначеЕсли ТипЗнч(ОбъектСобытия) = Тип("РегистрСведенийНаборЗаписей.СтатусыЭН") Тогда 
		...
		Если Не ТаблицаСтатусов.Количество() = 0 
				И Не ТаблицаСтатусов[0].ДокументРегистратор.ЗаказПокупателя = Документы.ЗаказПокупателя.ПустаяСсылка() Тогда

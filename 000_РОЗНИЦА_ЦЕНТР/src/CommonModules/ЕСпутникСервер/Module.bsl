...
Процедура ВыполнитьОбмен(УзелОбмена, ИнтерактивныйОбмен = Ложь, ТипОбмена = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ДанныеДляОтправки) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда
			...
			Запрос.Текст =
				"ВЫБРАТЬ
				|	ВозвратИнтернетЗаказа.ВидВозврата = ЗНАЧЕНИЕ(Перечисление.ВидыВозвратаИнтернетЗаказа.КорректировкаЦены) КАК
				|		КорректировкаЦены
				|ИЗ
				|	Документ.ЗаказПокупателя КАК ЗаказПокупателя
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратИнтернетЗаказа КАК ВозвратИнтернетЗаказа
				|		ПО ЗаказПокупателя.ДокументОснование = ВозвратИнтернетЗаказа.Ссылка
				|ГДЕ

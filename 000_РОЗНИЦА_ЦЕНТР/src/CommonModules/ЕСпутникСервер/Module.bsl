...
Функция ДобавитьОбновитьКонтрагента(СтруктураВыгрузки, Контрагент) Экспорт 
	...
	JSON.ЗаписатьСвойство("externalCustomerId"	, Строка(ДанныеОКонтакте.IDC), ЗаписьJSONКонтакта);
	JSON.ЗаписатьСвойство("firstName"			, СокрЛП(ДанныеОКонтакте.Имя), ЗаписьJSONКонтакта);
	JSON.ЗаписатьСвойство("lastName"			, СокрЛП(ДанныеОКонтакте.Фамилия), ЗаписьJSONКонтакта);
	...
	JSON.ЗаписатьСвойство("value"				, ПолучитьПроцентСкидкиКлиента(Контрагент.Ссылка), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта(); 
	...
	ЗаписьJSONКонтакта.ЗаписатьНачалоОбъекта();
	JSON.ЗаписатьСвойство("id"					, "291161", ЗаписьJSONКонтакта);
	JSON.ЗаписатьСвойство("value"				, ?(НЕ Контрагент.МаркетинговыеГруппы.Найти(Справочники.МаркетинговыеГруппы.Е_Малятко,"Группа") = Неопределено И Контрагент.Дети.Количество() > 0, "Так", "Ні"), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();

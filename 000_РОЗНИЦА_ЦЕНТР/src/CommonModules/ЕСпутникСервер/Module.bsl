...
Процедура ВыгрузкаЕСпутникСобытия(СтруктураВыгрузки)
	...
	Пока ВыборкаСсылка.Следующий() Цикл
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
		JSON.ЗаписатьСвойство("externalOrderId"		, Строка(ВыборкаСсылка.Номер), ЗаписьJSON); 
		...
		JSON.ЗаписатьСвойство("value"				, Строка(ВыборкаСсылка.НомерТелефона), ЗаписьJSON);
		ЗаписьJSON.ЗаписатьКонецОбъекта(); 
		
		ЗаписьJSON.ЗаписатьКонецМассива();
		
		JSON.ЗаписатьСвойство("eventTypeKey"		, ТипСобытия, ЗаписьJSON);
		JSON.ЗаписатьСвойство("keyValue"			, Строка(ВыборкаСсылка.НомерТелефона), ЗаписьJSON); 
...
Процедура ВыполнитьОбмен(УзелОбмена, ИнтерактивныйОбмен = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		// Выгрузка заказов 
		Если ТипЗнч(ДанныеДляОтправки) = Тип("ДокументОбъект.ЧекККМ") Или ТипЗнч(ДанныеДляОтправки) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда

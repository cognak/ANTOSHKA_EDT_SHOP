...
Функция ОтправитьСобытие(СтруктураВыгрузки, Использовать_V3 = Ложь)
	...
	Если СтруктураВыгрузки.УзелОбмена.ЛогироватьОтветыСервера Тогда     
		...
		ЖурналСобытий.Регистрация(СокрЛП(СтруктураВыгрузки.УзелОбмена.Наименование),, СтруктураВыгрузки.УзелОбмена.Метаданные().Имя, СтруктураВыгрузки.ДанныеДляОтправки.Ссылка,, СтруктураВыгрузки.ТекстЗапроса, ПредставлениеДанных);
...
Функция ДобавитьОбновитьКонтрагента(СтруктураВыгрузки, Контрагент) Экспорт 
	...
	РаботаСJSONСервер.ЗаписатьСвойство("value"				, ?(Контрагент.МаркетинговыеГруппы.Найти(Справочники.МаркетинговыеГруппы.Е_Малятко,"Группа") = Неопределено, "Ні", "Так"), ЗаписьJSONКонтакта);
	ЗаписьJSONКонтакта.ЗаписатьКонецОбъекта();

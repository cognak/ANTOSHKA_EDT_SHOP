...
Функция ПолучитьСостояниеБонусногоСчета(Контрагент, Период, ТипыБонусов)
	...
	|	ТаблицаБаллы.ТипБонусов КАК ТипБонусов
	|ПОМЕСТИТЬ Бонусы
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТаблицаАктивные.СуммаБонусныхБалловОстаток КАК СуммаБонусныхБаллов,
	|		0 КАК СуммаБонусныхБалловВРезерве,
	|		Фильтр.ТипБонусов КАК ТипБонусов
	|	ИЗ
	|		РегистрНакопления.БонусныеБаллы.Остатки(ДОБАВИТЬКДАТЕ(&Период, СЕКУНДА, 1), Контрагент = &Контрагент) КАК
	|			ТаблицаАктивные
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Фильтр КАК Фильтр
	|			ПО ТаблицаАктивные.ПрограммаЛояльности = Фильтр.ПрограммаЛояльности
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		Фильтр.ТипБонусов
	|	ИЗ
	|		РегистрНакопления.БонусныеБаллыВРезерве.Остатки(ДОБАВИТЬКДАТЕ(&Период, СЕКУНДА, 1), Контрагент = &Контрагент) КАК
	|			ТаблицаВРезерве
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Фильтр КАК Фильтр
	|			ПО ТаблицаВРезерве.ПрограммаЛояльности = Фильтр.ПрограммаЛояльности
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		Фильтр.ТипБонусов
	|	ИЗ
	|		РегистрНакопления.БонусныеБаллыПоЗаказамПокупателей.Остатки(ДОБАВИТЬКДАТЕ(&Период, СЕКУНДА, 1),
	|			Контрагент = &Контрагент) КАК ТаблицаПоЗаказам
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Фильтр КАК Фильтр
	|			ПО ТаблицаПоЗаказам.ПрограммаЛояльности = Фильтр.ПрограммаЛояльности) КАК ТаблицаБаллы
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаБаллы.ТипБонусов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(ТаблицаДанных.СуммаБонусныхБалловАкционных) КАК СуммаБонусныхБалловАкционных,
	|	СУММА(ТаблицаДанных.СуммаБонусныхБалловВРезервеАкционных) КАК СуммаБонусныхБалловВРезервеАкционных,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		0 КАК СуммаБонусныхБаллов,
	|		0 КАК СуммаБонусныхБалловВРезерве,
	|		Бонусы.СуммаБонусныхБаллов КАК СуммаБонусныхБалловАкционных,
	|		Бонусы.СуммаБонусныхБалловВРезерве КАК СуммаБонусныхБалловВРезервеАкционных,
	...
	|		Бонусы КАК Бонусы
	|	ГДЕ
	|		Бонусы.ТипБонусов = ЗНАЧЕНИЕ(Перечисление.ТипыБонусов.Акционный)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		Бонусы.СуммаБонусныхБаллов,
	|		Бонусы.СуммаБонусныхБалловВРезерве,
	|		0,
	|		0,
	|		ДАТАВРЕМЯ(1, 1, 1),
	|		НЕОПРЕДЕЛЕНО,
	...
	|		Бонусы КАК Бонусы
	|	ГДЕ
	|		НЕ Бонусы.ТипБонусов = ЗНАЧЕНИЕ(Перечисление.ТипыБонусов.Акционный)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
...
Функция BPS_GetBonusInfo(RequestData)	Экспорт
	...
	ИначеЕсли ДанныеЗапроса.Команда = "ОбщееСостояниеСчёта" Тогда
		...
		ДанныеОтвета.Вставить("СуммаБонусныхБалловАкционных", 			ДанныеСчёта.СуммаБонусныхБалловАкционных);			//	для совместимости
		ДанныеОтвета.Вставить("СуммаБонусныхБалловВРезервеАкционных", 	ДанныеСчёта.СуммаБонусныхБалловВРезервеАкционных);	//	для совместимости
...
Процедура ЗаполнитьКлючСвязиБонусныхБаллов(ТабличнаяЧасть, Перезаполнить = Ложь)	Экспорт
	...
	Иначе
		...
		ДанныеКлючей = ОбщегоНазначения.ПолучитьДанныеКлючейСвязи(ТабличнаяЧасть, "КлючСвязиБонусныхБаллов");
...
Процедура ПолучитьФильтрыЗапроса(Запрос)
	...
	|	Таблица.АкционныйТовар КАК АкционнаяЦена,
	...
	|	И
	|		ТаблицаПрограммы.СпособПримененияСкидки = ЗНАЧЕНИЕ(Перечисление.СпособыПримененияСкидокНаценок.НачислитьБонусныеБаллы)
	|	И ВЫБОР
	...
	|	КОНЕЦ
	|	И ВЫБОР
	|		КОГДА ТаблицаПрограммы.СписыватьБаллыНаАкционныйТовар
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ
	|		НЕ ТоварыПоСегментам.АкционнаяЦена
	|	КОНЕЦ
	|
	|ОБЪЕДИНИТЬ
	|
...
Функция ТаблицаБонусныеБаллы(ТаблицаСкидкиНаценки, Период)	Экспорт
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА НЕ ПрограммыЛояльности.ДатаПериодаДействия = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ПрограммыЛояльности.ДатаПериодаДействия
	|		КОГДА ПрограммыЛояльности.ПериодДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.День)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ДЕНЬ, ПрограммыЛояльности.КоличествоПериодовДействия)
	|		КОГДА ПрограммыЛояльности.ПериодДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Неделя)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, НЕДЕЛЯ, ПрограммыЛояльности.КоличествоПериодовДействия)
	|		КОГДА ПрограммыЛояльности.ПериодДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Месяц)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, ПрограммыЛояльности.КоличествоПериодовДействия)
	|		КОГДА ПрограммыЛояльности.ПериодДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Квартал)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, КВАРТАЛ, ПрограммыЛояльности.КоличествоПериодовДействия)
	|		КОГДА ПрограммыЛояльности.ПериодДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Год)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ГОД, ПрограммыЛояльности.КоличествоПериодовДействия)
	|		КОГДА ПрограммыЛояльности.ПериодДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Декада)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ДЕКАДА, ПрограммыЛояльности.КоличествоПериодовДействия)
	|		КОГДА ПрограммыЛояльности.ПериодДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Полугодие)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ПОЛУГОДИЕ, ПрограммыЛояльности.КоличествоПериодовДействия)
	|		ИНАЧЕ &Период
	|	КОНЕЦ КАК Период,
	|	ТабличнаяЧасть.КлючСвязи КАК КлючСвязи,
	|	СкидкиНаценки.Ссылка КАК СкидкаНаценка,
	|	ПрограммыЛояльности.Ссылка КАК ПрограммаЛояльности,
	|	ТабличнаяЧасть.Сумма КАК Сумма,
	|	ПрограммыЛояльности.ДатаПериодаДействия КАК ДатаПериодаДействия,
	|	ПрограммыЛояльности.СовместноеНачислениеСкидокИБонусов КАК СовместноеНачислениеСкидокИБонусов,
	|	ПрограммыЛояльности.НачислятьБаллыПоКредитнойПрограмме КАК НачислятьБаллыПоКредитнойПрограмме,
	|	ПрограммыЛояльности.ПериодОтсрочкиНачалаДействия КАК ПериодОтсрочкиНачалаДействия,
	|	ПрограммыЛояльности.КоличествоПериодовОтсрочкиНачалаДействия КАК КоличествоПериодовОтсрочкиНачалаДействия,
	|	ПрограммыЛояльности.КоличествоПериодовДействия КАК КоличествоПериодовДействия
	|ПОМЕСТИТЬ ПериодДействия
	|ИЗ
	|	СкидкиНаценки КАК ТабличнаяЧасть
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СкидкиНаценки КАК СкидкиНаценки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПрограммыЛояльности КАК ПрограммыЛояльности
	|			ПО СкидкиНаценки.ПрограммаЛояльности = ПрограммыЛояльности.Ссылка
	|		ПО ТабличнаяЧасть.СкидкаНаценка = СкидкиНаценки.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТабличнаяЧасть.СовместноеНачислениеСкидокИБонусов КАК СовместноеНачислениеСкидокИБонусов,
	|	ТабличнаяЧасть.НачислятьБаллыПоКредитнойПрограмме КАК НачислятьБаллыПоКредитнойПрограмме,
	|	ВЫБОР
	|		КОГДА НЕ ТабличнаяЧасть.ДатаПериодаДействия = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ТабличнаяЧасть.ДатаПериодаДействия
	|		КОГДА ТабличнаяЧасть.КоличествоПериодовДействия > 0
	|			ТОГДА ВЫБОР
	|				КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.День)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ТабличнаяЧасть.Период, ДЕНЬ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|				КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Неделя)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ТабличнаяЧасть.Период, НЕДЕЛЯ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|				КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Месяц)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ТабличнаяЧасть.Период, МЕСЯЦ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|				КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Квартал)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ТабличнаяЧасть.Период, КВАРТАЛ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|				КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Год)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ТабличнаяЧасть.Период, ГОД, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|				КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Декада)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ТабличнаяЧасть.Период, ДЕКАДА, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|				КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Полугодие)
	|					ТОГДА ДОБАВИТЬКДАТЕ(ТабличнаяЧасть.Период, ПОЛУГОДИЕ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	...
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.День)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ДЕНЬ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|		КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Неделя)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, НЕДЕЛЯ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|		КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Месяц)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|		КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Квартал)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, КВАРТАЛ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|		КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Год)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ГОД, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|		КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Декада)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ДЕКАДА, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|		КОГДА ТабличнаяЧасть.ПериодОтсрочкиНачалаДействия = ЗНАЧЕНИЕ(Перечисление.Периодичность.Полугодие)
	|			ТОГДА ДОБАВИТЬКДАТЕ(&Период, ПОЛУГОДИЕ, ТабличнаяЧасть.КоличествоПериодовОтсрочкиНачалаДействия)
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(&Период, ЧАС, 1)
	|	КОНЕЦ КАК ДатаНачисления,
	|	ТабличнаяЧасть.Сумма КАК СуммаБонусныхБаллов
	|ИЗ
	|	ПериодДействия КАК ТабличнаяЧасть"
	);
...
Функция ТаблицаНакопленийРазвернутаяПустая()	Экспорт
	...
	//2024-11-29 sa  еще нужно количество для правильного распределения бонусов
...
//	---------------------------------------------------------------------------------------
	...
	КолонкиЭталона.Добавить(Новый Структура("Имя, Тип", "АкционныйТовар", Новый ОписаниеТипов("Булево")));

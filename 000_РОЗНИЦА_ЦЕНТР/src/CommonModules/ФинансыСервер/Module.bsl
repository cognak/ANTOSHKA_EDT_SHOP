...
Функция СписокДоступныхРасчетныхСчетов(Организация, КассаИлиМагазин, ВернутьСписокЗначений = Ложь, ДатаДокумента= Неопределено)	Экспорт
	...
	Если НЕ ДатаДокумента= Неопределено 
		//и не Пользователи.ЭтоПолноправныйПользователь() //А++ Убрал т.к. у нас обычные пользователи с адм.правами.
		Тогда
		Запрос.Текст = Запрос.Текст +" И 
						|		ВЫБОР
						|			КОГДА НЕ БанковскиеСчета.ДатаЗакрытия = ДАТАВРЕМЯ(1,1,1)
						|				ТОГДА НАЧАЛОПЕРИОДА(БанковскиеСчета.ДатаЗакрытия, ДЕНЬ) > КОНЕЦПЕРИОДА(&ДатаЗакрытия, ДЕНЬ)
						|			ИНАЧЕ Истина
						|		КОНЕЦ";

...
//  ТолькоСМС          - Булево - Истина, если требуется отправка только СМС.
//
...
//
Функция ОтправитьSMS(НомераПолучателей, Знач Текст, ИмяОтправителя = Неопределено, ПеревестиВТранслит = Ложь, ТолькоСМС = Ложь)	Экспорт
	...
	Результат = ОписаниеРезультата();
	
	Попытка
		
		УчетнаяПолитика = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику();
			
		Если УчетнаяПолитика.ИспользоватьСтаруюСистемуОтпракиСМС Или ТолькоСМС Тогда
		...
		Иначе
			...
			Для Каждого НомерПолучателя Из НомераПолучателей Цикл
				...
				СтруктураСообщенияКлиенту.ТекстСообщения 			= Текст;
				...
				СтруктураСообщенияКлиенту.Телефон 					= ПодготовитьНомерТелефона(НомерПолучателя);

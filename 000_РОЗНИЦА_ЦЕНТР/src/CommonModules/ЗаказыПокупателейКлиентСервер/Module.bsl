...
Функция ПодсчетОплаты(ДокументОбъект, СтруктураОплат) Экспорт
	...
	КоличествоТоваров = 0;
	Для Каждого СтрокаТовара Из ДокументОбъект.Товары.НайтиСтроки(СтруктураОтбора) Цикл
	...
	// +мав
	СтруктураОтвета.Вставить("СтатусОплаты",  ДокументОбъект.СтатусОплаты);  
	// -мав
	...
	СуммаПодарочныхСертификатов = ДокументОбъект.ПогашениеПодарочныхСертификатов.Итог("Номинал");

	Если СтруктураОплат.СуммаВсего > СуммаПодарочныхСертификатов Тогда
		СтруктураОплат.СуммаВсегоДляОплаты = СтруктураОплат.СуммаВсего + ДокументОбъект.СуммаДоставки - СуммаПодарочныхСертификатов; 
	Иначе
		СтруктураОплат.СуммаВсегоДляОплаты = КоличествоТоваров + ДокументОбъект.СуммаДоставки;
	КонецЕсли;
	...
	Если СтруктураОплат.СуммаВсегоДляОплаты = СтруктураОплат.СуммаВсегоОплаты Тогда 
		...
		Иначе 
			...
			// +мав
			СтруктураОтвета.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.Оплачен"); 
			// -мав
		КонецЕсли;
	ИначеЕсли СтруктураОплат.СуммаВсегоДляОплаты > СтруктураОплат.СуммаВсегоОплаты Тогда
		...
		// +мав
		СтруктураОтвета.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.НеОплачен");  
		// -мав
	Иначе
		СтруктураОтвета.ЗаголовокДляФормы = "Переплата заказа!!!";    
		// +мав
		СтруктураОтвета.СтатусОплаты = ПредопределенноеЗначение("Перечисление.СтатусОплаты.Переплата");   
		// -мав
	КонецЕсли;
...
Процедура ПодсчетСтрокиВозврата(Объект, СтрокаВозврата) Экспорт
	...
	СтрокаВозврата.СуммаДоставки = СтруктураОтвета.СуммаДоставки;
	СтрокаВозврата.СуммаАвтоматическойСкидки = СтруктураОтвета.СуммаАвтоматическойСкидки * СтрокаВозврата.КоличествоУпаковок / СтрокаВозврата.КоличествоВЗаказе;
	СтрокаВозврата.СуммаРучнойСкидки = СтруктураОтвета.СуммаРучнойСкидки * СтрокаВозврата.КоличествоУпаковок / СтрокаВозврата.КоличествоВЗаказе;
	СтрокаВозврата.СуммаБонусныхБалловНачислено = СтруктураОтвета.СуммаБонусныхБалловНачислено * СтрокаВозврата.КоличествоУпаковок / СтрокаВозврата.КоличествоВЗаказе;
	СтрокаВозврата.СуммаБонусныхБалловСписано = СтруктураОтвета.СуммаБонусныхБалловСписано * СтрокаВозврата.КоличествоУпаковок / СтрокаВозврата.КоличествоВЗаказе;
	СтрокаВозврата.СуммаОкругления = СтруктураОтвета.СуммаОкругления * СтрокаВозврата.КоличествоУпаковок / СтрокаВозврата.КоличествоВЗаказе;

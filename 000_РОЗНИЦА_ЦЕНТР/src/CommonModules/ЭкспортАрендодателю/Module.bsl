...
Процедура ВыгрузитьКассовыйДень(ДокументСсылка)	Экспорт
	...
	Попытка
		...
		//	ФайлXML = Новый Файл("D:\TEMP" + "\" + Формат(ОбщаяВыборка.ДатаВыгрузки, "ДФ=ггггММддЧЧмм") + ".xml");
			...
			Пока ЧекиВыборка.Следующий() Цикл
				...
				ЗаписьXML.ЗаписатьТекст(ЧекиВыборка.НомерРегистратора);
				ЗаписьXML.ЗаписатьКонецЭлемента();
			...
			Если ФайлXML.Существует() Тогда
				...
				Исключение
					...
					ВызватьИсключение "Ошибка записи на FTP: " + ОписаниеОшибки();
...
//	недостатньо для покриття суми товару, тоді переходимо до наступного виду оплати.
	...
	Иначе
		...
		Иначе
			...
			СуммаДокумента = ТаблицаТовары.Итог("Сумма");
			...
			Для каждого СтрокаОплаты Из ТаблицаОплаты Цикл
				...
				Для каждого СтрокаТовары Из ТаблицаТовары Цикл
					...
					И	 СтрокаТовары.СуммаОплаты < СтрокаТовары.Сумма
					И	 ПустаяСтрока(СтрокаТовары.ВидОплаты)
					Тогда

						СуммаОплаты = Мин(СтрокаТовары.Сумма, СтрокаОплаты.Сумма);

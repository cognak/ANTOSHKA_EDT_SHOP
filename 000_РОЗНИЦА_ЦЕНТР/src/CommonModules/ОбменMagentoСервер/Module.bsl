...
//	Получим определяющие реквизиты поиска документа.
	...
	Если НЕ РежимЭмуляции = Истина И (Дата = НачалоДня(Дата) ИЛИ НачалоДня(Дата) = '00010101') Тогда
		...
		Иначе	Дата = Дата + (ТекущаяДатаСеанса() - НачалоДня(ТекущаяДатаСеанса()));

		КонецЕсли;

	КонецЕсли;
	
	//2026-08-31 sa вставляем костыль, чтобы если это КСО, то акции применялись как в магазине, а не как в Интернет-магазине
	//нужно чтобы КСОшники передавали УИД кассы по которому можно будет найти магазин. Пока пропишем хардкод.
	Если ДанныеВнешние.Свойство("SourceData") И ДанныеВнешние.SourceData = "KSO" Тогда
		Магазин = Справочники.Магазины.НайтиПоКоду("00-000035");
	Иначе		
	...
	Если ИмяОбъекта = "ЗаказПокупателя" Тогда	//	специфика "Документы.ЗаказПокупателя"
		...
		Если ДокументОбъект.УчетнаяСистема = Перечисления.УчетныеСистемыКомпании.Розница Тогда
			...
			//	Блок "Оплата" ("Payment")
			ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("СтатусОплаты"			, ?(РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(ДанныеВнешние.Payment, "Status", Тип("Число"))), Перечисления.СтатусОплаты.Оплачен, Перечисления.СтатусОплаты.НеОплачен), ДокументОбъект, Ложь);
		...
		//	.. проблемные
			...
			// Если заказ отменен в MAGENTO, то не записываем его
			
			//СоставОшибки.ОбрабатываетсяВ1С = 
			//	(ОбменMagentoСлужебный.СтатусИнтернетМагазина(ЗначениеПоТипу(ДанныеВнешние, "OrderStatus", Тип("Строка")), СоставОшибки)
			//		= Перечисления.СтатусыИнтернетМагазина.canceled);
		...
		//	Блок "Доставка" ("Delivery")
			ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("СуммаДоставкиОплачена"	, РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(ДанныеВнешние.Delivery, "AmountPaidClient", Тип("Число"))), ДокументОбъект, Ложь);
		...
		//	.. проблемные
			...
			ГородДоставки = Справочники.ГородаДоставки.НайтиГородRef(ЗначениеПоТипу(ДанныеВнешние.Delivery, "CityRef", Тип("Строка")), ДокументОбъект.ОператорДоставки);
			Если Не ГородДоставки = Неопределено Тогда

				ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("Город"					, ГородДоставки, ДокументОбъект, Ложь);
				
				Отделение = Справочники.Отделения.НайтиОтделение(ЗначениеПоТипу(ДанныеВнешние.Delivery, "Warehouse", Тип("Строка")),ГородДоставки ,ДокументОбъект.ОператорДоставки);
				Если Отделение.Пустая() Тогда
					Почтомат = Справочники.Почтоматы.НайтиПочтомат(ЗначениеПоТипу(ДанныеВнешние.Delivery, "Warehouse", Тип("Строка")),ГородДоставки ,ДокументОбъект.ОператорДоставки);
					Если Почтомат.Пустая() Тогда
						ОтделениеДоставки = Справочники.Отделения.ПустаяСсылка();
					Иначе
						ОтделениеДоставки = Почтомат;
					КонецЕсли;
				Иначе
					ОтделениеДоставки = Отделение;
				КонецЕсли;
				ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("Отделение"				, ОтделениеДоставки, ДокументОбъект, Ложь);
	            Если Не ЗначениеЗаполнено(Отделение) Тогда
					УлицаДоставки = Справочники.Улицы.НайтиУлицу(ЗначениеПоТипу(ДанныеВнешние.Delivery, "Street", Тип("Строка")),ГородДоставки ,ДокументОбъект.ОператорДоставки);
					ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("Улица"					, УлицаДоставки, ДокументОбъект, Ложь);
			...
			ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("Телефон"					, СтрЗаменить(ЗначениеПоТипу(ДанныеВнешние.Delivery, "RecipientPhoneNumber", Тип("Строка")), "+", ""), ДокументОбъект, Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("ФинансовыйТелефон"		, СтрЗаменить(ЗначениеПоТипу(ДанныеВнешние.Payment, "FinancialPhoneNumber", Тип("Строка")), "+", ""), ДокументОбъект, Ложь);
			//	Блок "Оплата" ("Payment")
			ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("СтатусОплаты"			, ?(РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(ДанныеВнешние.Payment, "Status", Тип("Число"))), Перечисления.СтатусОплаты.Оплачен, Перечисления.СтатусОплаты.НеОплачен), ДокументОбъект, Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("СуммаОплаты"				, ЗначениеПоТипу(ДанныеВнешние.Payment, "Summa", Тип("Число")), ДокументОбъект, Ложь);
		//	.. проблемные
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЗначениеЕслиОтличается("ЦенаВключаетНДС"				, РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(ДанныеВнешние, "TaxInclud", Тип("Число"))), ДокументОбъект, Ложь);
		...
		Если НЕ ПустаяСтрока(ДанныеВнешние.Buyer.phoneNumber) Тогда
			...
			Если СтрокаВнешняя.Найден Тогда
				...
				Если ИмяОбъекта = "ЗаказПокупателя" Тогда	//	в чеке такого реквизита нет.
					...
					ДокументОбъект.КодПоЕДРПОУ = СтрокаВнешняя.Контрагент.КодПоЕДРПОУ;
				...
				//2024-08-16 sa отказались от поля EDRPOU в HTTP-запросе. Проверку на юр.лицо перепишем.
				//Если ТипЗнч(ДанныеВнешние.Buyer.EDRPOU) = Тип("Строка") И НЕ ПустаяСтрока(ДанныеВнешние.Buyer.EDRPOU) Тогда
				Если (СтрокаВнешняя.Контрагент.ЮрФизЛицо <> Перечисления.ЮрФизЛицо.ФизЛицо) Тогда
			...
			//	Но для заказа всегда даём отказ!
				...
				Если ИмяОбъекта = "ЗаказПокупателя" Тогда
					...
					Запрос.Текст =
						"ВЫБРАТЬ
						|	ЖурналСобытий.КодУзлаОбмена
						|ИЗ
						|	РегистрСведений.ЖурналСобытий КАК ЖурналСобытий
						|ГДЕ
						|	ВЫРАЗИТЬ(ЖурналСобытий.ДанныеПредставление КАК СТРОКА(150)) = &ДанныеПредставление";
					
					Запрос.УстановитьПараметр("ДанныеПредставление", СокрЛП(Номер));
					...
					Если Запрос.Выполнить().Пустой() Тогда
						...
						Если Не УчетныеДанныеОтправителя.Пустая() Тогда
							Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
								...
								Адрес = "service.team@antoshka.ua;
								|im@antoshka.ua;
								|igor.vitchenko@antoshka.ua;";
								
								Тема = "Ошибка в заказе №" + Номер;
								ТелоПисьма = "Не найден клиент по телефону " + ДанныеВнешние.Buyer.phoneNumber;
								...
								ЖурналСобытий.Регистрация("Ошибка.НетКлиента"
	...
	Если НЕ СоставОшибки.Отказ = Истина Тогда
		...
		ИначеЕсли ЗаполнитьТабличныеЧасти Тогда
			...
			Если ИмяОбъекта = "ЗаказПокупателя" Тогда
				...
				ПолучитьСоставПогашениеСертификатов(ДанныеВнешние, ДокументОбъект, РежимЭмуляции, ИмяОбъекта, СоставОшибки);
				ПолучитьСоставОплатыЗаказа(ДанныеВнешние, ДокументОбъект, РежимЭмуляции, ИмяОбъекта, СоставОшибки);
...
Функция ПроверитьДокументMagento(ДокументОбъект, СоставОшибки)	Экспорт
	...
	Если НЕ ДокументОбъект.ЭтоНовый() Тогда
		...
		|	Источник.Статус КАК Статус
		|ПОМЕСТИТЬ Фильтр
		|ИЗ
		|	Источник КАК Источник
		|ГДЕ
		|	НЕ Источник.Магазин = &Магазин
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЗаказПокупателя
		|;
		...
		|ВЫБРАТЬ
		|	Источник.ЗаказПокупателя КАК ЗаказПокупателя,
		|	КлючевыеАтрибуты.ДатаИзмененияСтатуса КАК ДатаИзмененияСтатуса
		|ИЗ
		|	Источник КАК Источник
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КлючевыеАтрибутыЗаказовПокупателей КАК КлючевыеАтрибуты
		|		ПО Источник.ЗаказПокупателя = КлючевыеАтрибуты.ЗаказПокупателя
		|			И (КлючевыеАтрибуты.СтатусИМ = ЗНАЧЕНИЕ(Перечисление.СтатусыИнтернетМагазина.delivered))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ Источник
		...
		Если НЕ РезультатыЗапроса[4].Пустой() Тогда	//	LNK 27.06.2023 05:41:26
			...
			СоставОшибки.УжеОформлен  = Истина;
			СоставОшибки.Отказ		  = Истина;
			СоставОшибки.СтатусОшибки = "ЗАКАЗ_УЖЕ_ДОСТАВЛЕН";
			ОбменMagentoСлужебный.ОшибкаMagento(СоставОшибки.ОписаниеОшибки
				, "статус «delivered» был установлен [" + Формат(Выборка.ДатаИзмененияСтатуса, "ДФ='dd.MM.yyyy ЧЧ:мм:сс'") + "]"
			);
	...
	Если ДокументОбъект.ОплатаЗаказа.Количество() = 0 Тогда

		ОбменMagentoСлужебный.ОшибкаMagento(СоставОшибки.ОписаниеОшибки
			, "[ОплатаЗаказа]: нет строк оплаты"
			, СоставОшибки.Отказ
			);
...
#Область ПроцедурыКомплексаОбменРозницаMagento_МодульОбменMagentoСервер
...
Функция ИменаТипаДата(Вариант = 1)	Экспорт

	ИменаДата = Новый Массив;

	Если Вариант = 2 Тогда

		ИменаДата.Добавить("ClientDateBorn");
		ИменаДата.Добавить("ChildDateBorn");
...
Функция ФорматДаты(ДатаЗначение, ЗначениеФормата)	Экспорт
...
//	Дата = XMLЗначение(Тип("Дата"), "2022-01-01T23:59:59")
	...
	Если НЕ ТипЗнч(ЗначениеФормата) = Тип("ФорматДатыJSON") Тогда

		ДатаТекст = "0001-01-01T00:00:00";	//	в ФорматДатыJSON.ISO

	ИначеЕсли ЗначениеФормата = ФорматДатыJSON.JavaScript Тогда


	ИначеЕсли ЗначениеФормата = ФорматДатыJSON.Microsoft Тогда


	Иначе	//	Если ЗначениеФормата = ФорматДатыJSON.ISO

		ДатаТекст = Формат(ДатаЗначение, "ДФ=гггг-ММ-ддTЧЧ:мм:сс; ДП=0001-01-01T00:00:00");
...
Функция ПолучитьДату(ДатаСтр) Экспорт
		ЗначениеВозврата = СтрЗаменить(ДатаСтр, "/", "");
		ЗначениеВозврата = СтрЗаменить(ДатаСтр, " ", "");
		ЗначениеВозврата = СтрЗаменить(ДатаСтр, ":", "");
		ЗначениеВозврата = СтрЗаменить(ДатаСтр, "T", "");
		ЗначениеВозврата = СтрЗаменить(ДатаСтр, "-", "");
		Попытка
			ДатаЗнач = Дата(ЗначениеВозврата);
		Исключение
			ДатаЗнач = Дата("00010101");
		КонецПопытки;
...
#Область СлужебныеПроцедурыИФункцииMagento
...
Функция СоздатьJSON(НомерСообщения = -1, ДанныеЗапроса, СоставОшибки = Неопределено)	Экспорт
	...
	РаботаСJSONСервер.ЗаписатьСвойство("Number_PackMGT", НомерСообщения, ЗаписьJSON);
	...
	Если ТипЗнч(ДанныеЗапроса) = Тип("Структура") И ДанныеЗапроса.Свойство("Command") Тогда
			...
			РаботаСJSONСервер.ЗаписатьСвойство("Command", ДанныеЗапроса.Command, ЗаписьJSON);

	Иначе	РаботаСJSONСервер.ЗаписатьСвойство("Command", "Undef", ЗаписьJSON);
...
// Получить склад magento по IDN.
// 
// Параметры:
//  IDN - Строка - IDN
// 
// Возвращаемое значение:
//  Структура - Получить склад magento по IDN:
//   * СкладВыбора - СправочникСсылка.Склады - 
//   * МагазинВыбора - СправочникСсылка.Магазины - 
Функция ПолучитьСкладMagentoПоIDN(IDN) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("СкладВыбора", Справочники.Склады.ПустаяСсылка());
	Результат.Вставить("МагазинВыбора", Справочники.Магазины.ПустаяСсылка());
	...
	Если Выборка.Следующий() Тогда 
		Результат.СкладВыбора = Выборка.Склад;
		Результат.МагазинВыбора = Выборка.Магазин;
	КонецЕсли;
	
	Возврат Результат;
КонецФункции 

Функция ПолучитьОстатокПоставщика(ДанныеЗапроса) Экспорт
	
	МакетЗапросов = ПолучитьОбщийМакет("ЗапросыКСтороннимСистемам");

	// +мав
	//ОбластьЗапроса = МакетЗапросов.ПолучитьОбласть("Склад_поставщика_GraphQL");
	ОбластьЗапроса = МакетЗапросов.ПолучитьОбласть("Склад_поставщика_GraphQL_new");
	// -мав
	...
	МассивКодов = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, ДанныеЗапроса.SKU);
	СтрокаКодов = "";
	Для Каждого СтрокаМассива Из МассивКодов Цикл
		РазделительПодстрок = ?(ПустаяСтрока(СтрокаКодов), "", """,""");
		СтрокаКодов = СтрокаКодов + РазделительПодстрок + СтрокаМассива;
	КонецЦикла;
	ТекстGraphQL = СтрЗаменить(ТекстGraphQL, "#sku#" , СокрЛП(СтрокаКодов));
	...
	Результат = ВнешниеИсточникиGraphQL.ВыполнитьЗапросДляGraphQL(ТекстЗапроса);
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(Результат.ТекстJSON);
	Возврат ПрочитатьJSON(ЧтениеJSON, Ложь);

КонецФункции

// Выполняет GraphQL запрос к Magento для получения данных о номенклатуре по списку SKU.
//
// Параметры:
//   ДанныеЗапроса - Структура - данные запроса, должны содержать ключ SKU с массивом кодов товаров
//
// Возвращаемое значение:
//   Структура, Произвольный - результат разбора JSON-ответа от GraphQL
Функция ПолучитьДанныеНоменклатуры(ДанныеЗапроса) Экспорт
	
	МакетЗапросов = ПолучитьОбщийМакет("ЗапросыКСтороннимСистемам");

	ОбластьЗапроса = МакетЗапросов.ПолучитьОбласть("Данные_о_товаре_GraphQL");
	ОбластьЗапроса.УдалитьСтроку(1);
	ОбластьЗапроса.УдалитьСтроку(ОбластьЗапроса.КоличествоСтрок());
	...
	Если Не Результат.Ошибка Тогда
		ЧтениеJSON = Новый ЧтениеJSON;
		ЧтениеJSON.УстановитьСтроку(Результат.ТекстJSON);
		ОтветНаЗапрос = ПрочитатьJSON(ЧтениеJSON, Ложь);
	КонецЕсли;
...
#Область Работа_с_Magento_2

// Возвращает список активных узлов обмена данными с Magento 2 для отправки сообщений.
//
// Возвращаемое значение:
//   Массив из ПланОбменаСсылка.ОбменМаженто2 - список узлов обмена
Функция ОпределитьПолучателейМаженто2()  Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ОбменМаженто2.ПометкаУдаления = ЛОЖЬ
	               |	И ОбменМаженто2.ЭтотУзел = ЛОЖЬ";
	УзлыОбмена = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	Возврат УзлыОбмена;
КонецФункции

// Удаляет регистрацию изменений указанных объектов для узлов плана обмена Мадженто2.
//
// Параметры:
//   МассивОбъектов - Массив из <Тип> - массив объектов, для которых удаляется регистрация изменений
//   Получатели - Массив из ПланыОбменаСсылка - массив узлов плана обмена, для которых выполняется удаление регистрации
Процедура УдалитьРегистрациюОбъектов(МассивОбъектов, Получатели = Неопределено) Экспорт
	Если Получатели = Неопределено Тогда
		Получатели = ОпределитьПолучателейМаженто2();
	КонецЕсли;
...
// Регистрация маркетенговой группы клиента.
// 
// Параметры:
//  КлиентСайта - СправочникОбъект.Контрагенты, СправочникСсылка.Контрагенты - Клиент сайта
Процедура РегистрацияМаркетенговойГруппыКлиента(КлиентСайта) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если КлиентСайта.Ссылка = Справочники.Контрагенты.ПустаяСсылка() Тогда
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КонтрагентыМаркетинговыеГруппы.Ссылка
		|ИЗ
		|	Справочник.Контрагенты.МаркетинговыеГруппы КАК КонтрагентыМаркетинговыеГруппы
		|ГДЕ
		|	КонтрагентыМаркетинговыеГруппы.Ссылка = &Ссылка
		|	И КонтрагентыМаркетинговыеГруппы.Группа = ЗНАЧЕНИЕ(Справочник.МаркетинговыеГруппы.М2)";
	
	Запрос.УстановитьПараметр("Ссылка", КлиентСайта.Ссылка);
	
	Если Запрос.Выполнить().Пустой() Тогда
		
		Если ТипЗнч(КлиентСайта) = Тип("СправочникСсылка.Контрагенты") Тогда
		
			КлиентСайтаОбъект = КлиентСайта.ПолучитьОбъект();
		
		Иначе
			
			КлиентСайтаОбъект = КлиентСайта;
		
		КонецЕсли;
		
		НоваяСтрока = КлиентСайтаОбъект.МаркетинговыеГруппы.Добавить();
		НоваяСтрока.Группа = Справочники.МаркетинговыеГруппы.М2;
		КлиентСайтаОбъект.Записать();
...
Функция ТекстЗапросаПоРегистрации(ПоСкладуIDN = Ложь)
	...
	|	ТаблицаПриоритета.Приоритет КАК Приоритет
	|ПОМЕСТИТЬ Склады
	|ИЗ
	|	Справочник.Склады КАК ТаблицаСправочник
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады.ДополнительныеРеквизиты КАК ТаблицаРеквизиты
	|		ПО ТаблицаСправочник.Ссылка = ТаблицаРеквизиты.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПриоритетСкладовОтгрузки КАК ТаблицаПриоритета
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПриоритетСкладовОтгрузки.ГородОтгрузки КАК ТаблицаЛокал
	|			ПО ТаблицаПриоритета.Ссылка = ТаблицаЛокал.Ссылка
	|			И ТаблицаЛокал.Город = &ГородДоставки
	|		ПО ТаблицаСправочник.Ссылка = ТаблицаПриоритета.Склад
	|		И НЕ ТаблицаПриоритета.ПометкаУдаления
	|ГДЕ
	...
	|	И (ТаблицаПриоритета.Доставка
	|	И &Доставка
	|	ИЛИ ТаблицаПриоритета.Самовывоз
	|	И &Самовывоз)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЗаказа.Номер ЕСТЬ NULL
	|			ТОГДА ТаблицаЗаказаЗапроса.Номер
	|		ИНАЧЕ ТаблицаЗаказа.Номер
	|	КОНЕЦ КАК НомерЗаказа,
	|	СУММА(ТаблицаОстатков.КоличествоОстаток) КАК КоличествоЗаказы,
	|	РеквизитыСклады.Склад КАК Склад,
	|	РеквизитыСклады.Приоритет КАК Приоритет
	|ПОМЕСТИТЬ Заказы
	|ИЗ
	|	РегистрНакопления.ЗаказыПокупателей.Остатки(, Номенклатура В
	|		(ВЫБРАТЬ
	|			Товары.Номенклатура
	|		ИЗ
	|			Товары)
	...
	|		ПО ТаблицаОстатков.Заказ = ТаблицаЗаказа.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗапросДоступностиТоваров КАК ТаблицаЗапроса
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК ТаблицаЗаказаЗапроса
	|			ПО ТаблицаЗапроса.ДокументОснование = ТаблицаЗаказаЗапроса.Ссылка
	|		ПО ТаблицаОстатков.Заказ = ТаблицаЗапроса.Ссылка
	|СГРУППИРОВАТЬ ПО
	|	РеквизитыСклады.КодСклада,
	|	РеквизитыТовары.КодТовара,
	|	РеквизитыСклады.Склад,
	|	РеквизитыСклады.Приоритет,
	|	ВЫБОР
	|		КОГДА ТаблицаЗаказа.Номер ЕСТЬ NULL
	|			ТОГДА ТаблицаЗаказаЗапроса.Номер
	|		ИНАЧЕ ТаблицаЗаказа.Номер
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(ДанныеТоваров.КодСклада, """") КАК КодСклада,
	|	Товары.КодТовара КАК КодТовара,
	|	СУММА(ЕСТЬNULL(ДанныеТоваров.КоличествоСклад, 0)) КАК КоличествоСклад,
	|	СУММА(ЕСТЬNULL(ДанныеТоваров.КоличествоЗаказы, 0)) КАК КоличествоЗаказы,
	|	ЕСТЬNULL(ДанныеТоваров.Склад, """") КАК Склад,
	|	ЕСТЬNULL(ДанныеТоваров.Приоритет, 0) КАК Приоритет
	|ИЗ
	|	Товары КАК Товары
	...
	|					Склады.Склад
	|				ИЗ
	|					Склады КАК Склады)) КАК ТоварыНаСкладах
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Склады КАК РеквизитыСклады
	...
	|		ИЗ
	|			Товары КАК Товары,
	|			Склады КАК Склады) КАК ДанныеТоваров
	|		ПО ДанныеТоваров.КодТовара = Товары.КодТовара
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ДанныеТоваров.КодСклада, """"),
	|	Товары.КодТовара,
	|	ЕСТЬNULL(ДанныеТоваров.Склад, """"),
	|	ЕСТЬNULL(ДанныеТоваров.Приоритет, 0)
	|
	|УПОРЯДОЧИТЬ ПО
	|	КодТовара,
	|	КодСклада
			...
			Попытка
				...
				Иначе
					...
					Пока ДанныеРегистрации.Следующий() Цикл
						...
						ЗаписьНабора.Значение		= ТекущаяДатаСеанса();
...
Процедура ПолучитьДанныеРегистрации(СписокТоваров, ДанныеЗапроса, ЗаписьJSON, СоставОшибки)
	...
	Доставка = Истина;
	Если Не ДанныеЗапроса.Свойство("Delivery", Доставка) Тогда
		Доставка = Истина;
	КонецЕсли;
	
	Самовывоз = Истина;
	Если Не ДанныеЗапроса.Свойство("Pickup", Самовывоз) Тогда
		Самовывоз = Истина;
	КонецЕсли;
	...
	//СЮП 03.05.2024 20.43.55 
	Запрос.УстановитьПараметр("СкладПоставщика", ДанныеЗапроса.Свойство("SUPPLIER"));
	
	УчетнаяПолитика = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику();
	Запрос.УстановитьПараметр("ИнтернетМагазин", УчетнаяПолитика.ИнтернетМагазин);
	Запрос.УстановитьПараметр("СкладЦентральныйИнтернетМагазина", УчетнаяПолитика.СкладЦентральныйИнтернетМагазина);
	Запрос.УстановитьПараметр("ГородДоставки", Справочники.ГородаДоставки.ПустаяСсылка());
	...
	Пока ТоварыВыборка.Следующий() Цикл
		...
		РаботаСJSONСервер.ЗаписатьСвойство("sku", ТоварыВыборка.КодТовара, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("qty", ТоварыВыборка.КоличествоСклад, ЗаписьJSON);
		РаботаСJSONСервер.ЗаписатьСвойство("qty_r1", ТоварыВыборка.КоличествоЗаказы, ЗаписьJSON);
		...
		Пока СкладыВыборка.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(СкладыВыборка.КодСклада) Тогда
			...
			РаботаСJSONСервер.ЗаписатьСвойство("wh_code", СкладыВыборка.КодСклада, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("qty", СкладыВыборка.КоличествоСклад, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("qty_r1", СкладыВыборка.КоличествоЗаказы, ЗаписьJSON);

			СтруктураОтбора = Новый Структура("КодСклада, КодТовара", СкладыВыборка.КодСклада, СкладыВыборка.КодТовара);
			...
			Пока ДеталиВыборка.НайтиСледующий(СтруктураОтбора) Цикл
				...
				РаботаСJSONСервер.ЗаписатьСвойство("number", СокрЛП(ДеталиВыборка.НомерЗаказа), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("qty_r1", ДеталиВыборка.КоличествоЗаказы, ЗаписьJSON);
	...
	РаботаСJSONСервер.ЗаписатьСвойство("count_results", ТоварыВыборка.Количество(), ЗаписьJSON);
...
Функция СоставХранилищаПротокола(IDP = "", Статус = "", Атрибуты = Неопределено, ТекстСообщения = "")
	...
	ДанныеПротокола.Вставить("IDP", IDP);
	ДанныеПротокола.Вставить("Период", ТекущаяДатаСеанса());
	ДанныеПротокола.Вставить("Статус", Статус);
	ДанныеПротокола.Вставить("Атрибуты", Атрибуты);
	ДанныеПротокола.Вставить("ТекстСообщения", ТекстСообщения);
...
// Получить данные остатка по складу.
// 
// Параметры:
//  СписокТоваров - СправочникСсылка.Номенклатура - Список товаров
//  СкладIDN - Строка - Склад IDN
//  Самовывоз - Булево - Самовывоз
//  НужныйОстаток - Число - Нужный остаток
// 
// Возвращаемое значение:
//  Структура - Получить данные остатка по складу:
//   ДоступныйОстаток - Число - 
//   Склад - СправочникСсылка.Склады - 
//   Приоритет - Число - 
//   ЕстьОстаток - Булево - 
Функция ПолучитьДанныеОстаткаПоСкладу(СписокТоваров, ГородДоставки, СкладIDN, Самовывоз, НужныйОстаток)

	Результат = Новый Структура;
	Результат.Вставить("ДоступныйОстаток", 0);
	Результат.Вставить("Склад", Справочники.Склады.ПустаяСсылка());
	Результат.Вставить("Приоритет", 9999);
	Результат.Вставить("ЕстьОстаток", Ложь);
	
	Запрос = Новый Запрос(ТекстЗапросаПоРегистрации(Самовывоз));

	Запрос.УстановитьПараметр("СписокТоваров", СписокТоваров);

	УчетнаяПолитика = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику();
	//СЮП 09.05.2024 12.16.46 
	Запрос.УстановитьПараметр("СкладПоставщика", Истина);
	Запрос.УстановитьПараметр("ИнтернетМагазин", УчетнаяПолитика.ИнтернетМагазин);
	Запрос.УстановитьПараметр("СкладЦентральныйИнтернетМагазина", УчетнаяПолитика.СкладЦентральныйИнтернетМагазина);
	Запрос.УстановитьПараметр("ПоСкладуIDN", СкладIDN);
	Запрос.УстановитьПараметр("ГородДоставки", ГородДоставки);
	Запрос.УстановитьПараметр("Самовывоз", Самовывоз);
	Запрос.УстановитьПараметр("Доставка", Не Самовывоз);
	...
	Пока ТоварыВыборка.Следующий() Цикл
		...
		Пока СкладыВыборка.Следующий() Цикл
			...
			Если Самовывоз Тогда
				Результат.ДоступныйОстаток = ДоступныйОстаток;
				Результат.ЕстьОстаток = (ДоступныйОстаток >= НужныйОстаток);
			Иначе
				Если Результат.Приоритет > СкладыВыборка.Приоритет
						И ДоступныйОстаток >= НужныйОстаток Тогда
					Результат.ДоступныйОстаток = ДоступныйОстаток;
					Результат.Склад = СкладыВыборка.Склад;
					Результат.ЕстьОстаток = Истина;
					Результат.Приоритет = СкладыВыборка.Приоритет;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ПолучитьСоставТовары(ДанныеВнешние, ДокументОбъект, РежимЭмуляции, ИмяОбъекта, СоставОшибки)
	...
	Если ДанныеВнешние.Свойство("Products") Тогда
		...
		Если ДанныеВнешние.Свойство("Delivery") Тогда
			...
			ДанныеПоIDN = ПолучитьСкладMagentoПоIDN(СкладIDN);
			СкладДоставки = ДанныеПоIDN.СкладВыбора;
			МагазинПолучатель = ДанныеПоIDN.МагазинВыбора;
		...
		Для каждого ДанныеСтроки Из ДанныеВнешние.Products Цикл
			...
			Если ИмяОбъекта = "ЗаказПокупателя" Тогда
				...
				СтрокаВнешняя.МагазинПолучатель  = МагазинПолучатель;
			КонецЕсли; 
		...
		ОдинСклад = Истина;
		СсылкаНаОдинСклад = Справочники.Склады.ПустаяСсылка();
		...
		Для каждого СтрокаВнешняя Из ТаблицаВнешняя Цикл
			...
			СтрокаТабличнойЧасти.АкционнаяЦена = РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(СтрокаВнешняя.ДанныеСтроки, "Action", Тип("Число")));
			СтрокаТабличнойЧасти.АкционнаяЦенаБезСкидки = СтрокаТабличнойЧасти.АкционнаяЦена;
			...
			Если ИмяОбъекта = "ЗаказПокупателя" Тогда
				...
				ДанныеПоОстаткам = ПолучитьДанныеОстаткаПоСкладу(СтрокаВнешняя.Номенклатура
						, ДокументОбъект.Город
						, СкладIDN
						, СтрокаВнешняя.Самовывоз
						, СтрокаТабличнойЧасти.КоличествоУпаковок);
				
				Если ДанныеПоОстаткам.ЕстьОстаток Тогда
					Если СтрокаТабличнойЧасти.Самовывоз Тогда  
						СтрокаТабличнойЧасти.Склад	   = СкладДоставки;
					Иначе 
						СтрокаТабличнойЧасти.Склад	   = ДанныеПоОстаткам.Склад; 
					КонецЕсли; 
				КонецЕсли; 
				...
				Если СтрокаТабличнойЧасти.Склад = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику().СкладПродажиИнтернетМагазина Тогда
					
					СтрокаТабличнойЧасти.Склад = Справочники.Склады.ПустаяСсылка();
					
				КонецЕсли;
				
				Если СсылкаНаОдинСклад.Пустая() Тогда
					
					СсылкаНаОдинСклад = СтрокаТабличнойЧасти.Склад;
					
				Иначе
					
					Если ОдинСклад И Не СсылкаНаОдинСклад = СтрокаТабличнойЧасти.Склад Тогда
				...
				СтруктураТовара.Вставить("Товар", СтрокаТабличнойЧасти.Номенклатура);
				СтруктураТовара.Вставить("Количество", СтрокаТабличнойЧасти.Количество);
				СтруктураТовара.Вставить("Самовывоз", СтрокаТабличнойЧасти.Самовывоз);
				СтруктураТовара.Вставить("Доставка", Не СтрокаТабличнойЧасти.Самовывоз);
		...
		Если Не ОдинСклад
				И Не СсылкаНаОдинСклад.Пустая()
				И Не ДокументОбъект.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз Тогда
			
			ДанныеВыбора = Новый СписокЗначений;
			ЗаказыПокупателейСервер.ТоварыСкладВыбор(МассивТоваров, ДанныеВыбора);
			Если Не ДанныеВыбора.Количество() = 0 Тогда

				Запрос = Новый Запрос;
				Запрос.Текст =
					"ВЫБРАТЬ
					|	ПриоритетСкладовОтгрузки.Склад
					|ИЗ
					|	Справочник.ПриоритетСкладовОтгрузки КАК ПриоритетСкладовОтгрузки
					|ГДЕ
					|	НЕ ПриоритетСкладовОтгрузки.ПометкаУдаления
					|	И ПриоритетСкладовОтгрузки.Доставка
					|
					|УПОРЯДОЧИТЬ ПО
					|	ПриоритетСкладовОтгрузки.Приоритет";
				...
				Пока ВыборкаПриоритет.Следующий() Цикл
					Если Не ДанныеВыбора.НайтиПоЗначению(ВыборкаПриоритет.Склад) = Неопределено Тогда
						
						Для Каждого СтрокаТЧ Из ДокументОбъект.Товары Цикл
							
							СтрокаТЧ.Склад = ВыборкаПриоритет.Склад;
...
Процедура ПолучитьСоставОплатаБонуснымиБаллами(ДанныеВнешние, ДокументОбъект, РежимЭмуляции, ИмяОбъекта, СоставОшибки)
	...
	ДокументОбъект.СуммаБонусныхБалловСписано = 0;
	...
	Если ДанныеВнешние.Свойство("PayBonusPoints") Тогда
		...
		Для каждого СтрокаВнешняя Из ДанныеВнешние.PayBonusPoints Цикл
			...
			СтрокаТабличнойЧасти.СуммаБонусныхБаллов	 = СтрокаВнешняя.Summa;
			ДокументОбъект.СуммаБонусныхБалловСписано	 = ДокументОбъект.СуммаБонусныхБалловСписано + СтрокаВнешняя.Summa;
...
Процедура ПолучитьСоставПогашениеСертификатов(ДанныеВнешние, ДокументОбъект, РежимЭмуляции, ИмяОбъекта, СоставОшибки)

	Если ДанныеВнешние.Свойство("Certificates") Тогда

		ДокументОбъект.ПогашениеПодарочныхСертификатов.Очистить();

		Для каждого СтрокаВнешняя Из ДанныеВнешние.Certificates Цикл

			Если СтрокаВнешняя.IsFound Тогда
				
				СостояниеСерийногоНомера = ПодарочныеСертификатыСервер.ПолучитьСоставСерийногоНомера(СтрокаВнешняя.SerialNumber);
				
				СтрокаТабличнойЧасти = ДокументОбъект.ПогашениеПодарочныхСертификатов.Добавить();
				СтрокаТабличнойЧасти.ПодарочныйСертификат	= СостояниеСерийногоНомера.Владелец;
				СтрокаТабличнойЧасти.СерийныйНомер	= СостояниеСерийногоНомера.СерийныйНомер;
				СтрокаТабличнойЧасти.Номинал	= СостояниеСерийногоНомера.Номинал;
...
Процедура ПолучитьСоставОплатыЗаказа(ДанныеВнешние, ДокументОбъект, РежимЭмуляции, ИмяОбъекта, СоставОшибки)
	...
	СтрокаТабличнойЧасти.СтатусОплаты	= ?(РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(ДанныеВнешние.Payment, "Status", Тип("Число"))), Перечисления.СтатусОплаты.Оплачен, Перечисления.СтатусОплаты.НеОплачен);
	СтрокаТабличнойЧасти.Сумма			= ЗначениеПоТипу(ДанныеВнешние.Payment, "Summa", Тип("Число"));
	СтрокатабличнойЧасти.ИДОплаты		= ЗначениеПоТипу(ДанныеВнешние.Payment, "MonoOrderId", Тип("Строка"));		
	...
	Если Не ДокументОбъект.Ссылка.Пустая() Тогда	
		...
		СтруктураЗаписи.Сумма 			= ЗначениеПоТипу(ДанныеВнешние.Payment, "Summa", Тип("Число"));
		СтруктураЗаписи.СтатусОплаты 	= ?(РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(ДанныеВнешние.Payment, "Status", Тип("Число"))), Перечисления.СтатусОплаты.Оплачен, Перечисления.СтатусОплаты.НеОплачен);
		СтруктураЗаписи.Отменён 		= Ложь;
		СтруктураЗаписи.ДатаОплаты 		= ?(РаботаСJSONСервер.ЧислоКакБулево(ЗначениеПоТипу(ДанныеВнешние.Payment, "Status", Тип("Число"))), ТекущаяДатаСеанса(), Дата(1,1,1));
		СтруктураЗаписи.ИДОплаты 		= ЗначениеПоТипу(ДанныеВнешние.Payment, "MonoOrderId", Тип("Строка"));;
...
// +мав
Процедура РегламентноеЗаданиеЗагрузитьИнформациюНоменклатуры() экспорт  
	...
	ДанныеТоваров.Вставить("date", Константы.ДатаПоследнейЗагрузкиНоменклатуры.Получить());
	
	ДанныеОтвета = ОбменMagentoСервер.ПолучитьДанныеПоследнегоИзмененияНоменклатуры(ДанныеТоваров);
	...
	Если ДанныеОтвета.Свойство("data")
		...
		Для Каждого СтрокаТовара Из ДанныеОтвета.data.contentM2Products.data Цикл   
			...
			текНоменклатура = Справочники.Номенклатура.НайтиПоКоду(СтрокаТовара.sku);
			НаборЗаписей = РегистрыСведений.НоменклатурнаяИнформация.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Номенклатура.Установить(текНоменклатура);
			НаборЗаписей.Прочитать();
			Если НаборЗаписей.Количество()=0 Тогда
				текЗапись = НаборЗаписей.Добавить(); 
				текЗапись.Номенклатура = текНоменклатура; 
			Иначе
				текЗапись = НаборЗаписей[0];
			КонецЕсли;
			текЗапись.URL = СтрокаТовара.uk_url;
			текЗапись.АдресКартинки = СтрокаТовара.image_link;
			текЗапись.Наименование = СтрокаТовара.uk_name;
			текЗапись.ДатаИзменения = ОбщегоНазначения.СтрокаВДатуВремя(СтрокаТовара.updated_at,"yyyy-MM-dd hh:nn:ss'");
			НаборЗаписей.Записать();
		КонецЦикла;
	...
	Если ДанныеОтвета.Свойство("data")
			И ДанныеОтвета.data.Свойство("contentM2Products")
			И ДанныеОтвета.data.contentM2Products.Свойство("paginatorInfo")
			И ДанныеОтвета.data.contentM2Products.paginatorInfo.Свойство("lastDate") Тогда
	
       Константы.ДатаПоследнейЗагрузкиНоменклатуры.Установить( ОбщегоНазначения.СтрокаВДатуВремя(ДанныеОтвета.data.contentM2Products.paginatorInfo.lastDate,"yyyy-MM-dd hh:nn:ss'"));

	КонецЕсли;
	
конецПроцедуры 

// Выполняет GraphQL запрос к Magento для получения данных о последнем изменении номенклатуре .
//
// Параметры:
//   ДанныеЗапроса - Структура - данные запроса, должны содержать ключ date с датой последнего изменения
//
// Возвращаемое значение:
//   Структура, Произвольный - результат разбора JSON-ответа от GraphQL
Функция ПолучитьДанныеПоследнегоИзмененияНоменклатуры(ДанныеЗапроса) Экспорт
	
	МакетЗапросов = ПолучитьОбщийМакет("ЗапросыКСтороннимСистемам");

	ОбластьЗапроса = МакетЗапросов.ПолучитьОбласть("Данные_о_последнем_изменении_товара_GraphQL");
	ОбластьЗапроса.УдалитьСтроку(1);
	ОбластьЗапроса.УдалитьСтроку(ОбластьЗапроса.КоличествоСтрок());

	ТекстGraphQL = ОбластьЗапроса.ПолучитьТекст();
	ТекстGraphQL = СтрЗаменить(ТекстGraphQL, "#DATE#" , Формат(ДанныеЗапроса.date,"ДФ='yyyy-MM-dd hh:mm:ss'")); 
	ТекстGraphQL = СтрЗаменить(ТекстGraphQL, "#COUNT#" , 300);

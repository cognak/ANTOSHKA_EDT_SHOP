	...
	Если ДанныеВнешние.Свойство("Products") Тогда
		...
		Для каждого СтрокаВнешняя Из ТаблицаВнешняя Цикл
			...
			Если ИмяОбъекта = "ЗаказПокупателя" Тогда
				...
				ДанныеПоОстаткам = ПолучитьДанныеОстаткаПоСкладу(СтрокаВнешняя.Номенклатура, СкладIDN, СтрокаВнешняя.Самовывоз, СтрокаТабличнойЧасти.КоличествоУпаковок);
...
Функция ТекстЗапросаПоРегистрации(ПоСкладуIDN = Ложь)
	...
	|	ЕСТЬNULL(ТаблицаПриоритета.Приоритет, 99) КАК Приоритет
	|ПОМЕСТИТЬ Склады
	|ИЗ
...
//   Приоритет - Число - 
//   ЕстьОстаток - Булево - 
Функция ПолучитьДанныеОстаткаПоСкладу(СписокТоваров, СкладIDN, Самовывоз, НужныйОстаток)

	Результат = Новый Структура;
	Результат.Вставить("ДоступныйОстаток", 0);
	Результат.Вставить("Склад", Справочники.Склады.ПустаяСсылка());
	Результат.Вставить("Приоритет", 99);

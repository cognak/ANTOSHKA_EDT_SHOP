			...
			//	Но для заказа всегда даём отказ!
				...
				Если ИмяОбъекта = "ЗаказПокупателя" Тогда
					...
					Если Запрос.Выполнить().Пустой() Тогда
						...
						Если Не УчетныеДанныеОтправителя.Пустая() И Ложь Тогда
	...
	Если ДанныеВнешние.Свойство("Products") Тогда
		...
		Для каждого СтрокаВнешняя Из ТаблицаВнешняя Цикл
			...
			Если ИмяОбъекта = "ЗаказПокупателя" Тогда
				...
				СтруктураТовара = Новый Структура("Товар, Количество", СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.Количество);
...
Функция ТекстЗапросаПоРегистрации(ПоСкладуIDN = Ложь)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЛокал.Ссылка ЕСТЬ NULL
	|			ТОГДА 1000 + ЕСТЬNULL(ТаблицаПриоритета.Приоритет, 99)
	|		ИНАЧЕ ТаблицаПриоритета.Приоритет
	|	КОНЕЦ КАК Приоритет
	|ПОМЕСТИТЬ Склады
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПриоритетСкладовОтгрузки КАК ТаблицаПриоритета

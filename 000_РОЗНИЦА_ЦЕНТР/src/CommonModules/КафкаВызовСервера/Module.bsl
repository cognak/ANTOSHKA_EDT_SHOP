...
Процедура ОтправитьСообщения() Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 250
		...
		|ГДЕ
		|	НЕ ДанныеДляБрокераСообщений.Ошибка
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерЗаписи
		|ИТОГИ
			...
			Если Не ОтветСервера = Неопределено Тогда
				...
				Для ИндекДляМассива = 0 По МассивСообщений.Количество() - 1 Цикл
					...
					Иначе
						...
						Набор.Прочитать();
						Набор[0].Ошибка = Истина;
						ЗаписьJSON = Новый ЗаписьJSON;
						ЗаписьJSON.УстановитьСтроку();
						ЗаписатьJSON(ЗаписьJSON, ОтветСервера[ИндекДляМассива]);
						Набор[0].ТекстОшибки = ЗаписьJSON.Закрыть();
						Набор.Записать(); 
...
Процедура ПолучитьСообщение(КаналПолучения) Экспорт
	...
	Попытка
		...
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ДанныеПотокаОбмена.ВремяПолучения КАК ВремяПолучения,
			|	ДанныеПотокаОбмена.ПолученныеДанные
			|ИЗ
			|	РегистрСведений.КафкаДанныеПотокаОбмена КАК ДанныеПотокаОбмена
			|ГДЕ

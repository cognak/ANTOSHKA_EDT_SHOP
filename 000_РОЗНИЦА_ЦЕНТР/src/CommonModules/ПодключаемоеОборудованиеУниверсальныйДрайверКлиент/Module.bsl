...
Функция ОплатитьПлатежнойКартой(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(ОбъектДрайвера) = ТИП("AddIn.IngenicoECR.a_ingenicopb1c82") Тогда
			Ответ = Продажа(Сумма, НомерКарты, НомерЧека, КодRRN, КодАвторизации, ДействительнаДо, Клиент, Эмитент, СлипЧек, Параметры);
		Иначе
		...
		Если Ответ Тогда
			...
			Если ТипЗнч(СлипЧек)= Тип("Строка") Тогда
				ВыходныеПараметры[3].Добавить(СлипЧек);
			Иначе	
				ВыходныеПараметры[3].Добавить(СлипЧек.ТекстЧека);
				ВыходныеПараметры[3].Добавить(СлипЧек);
			КонецЕсли;
			...
			// - HVOYA 04.10.2016 12:27:03, Латышев А.А. 
			ВыходныеПараметры.Добавить(СлипЧек.МерчантИД);
			ВыходныеПараметры.Добавить(СлипЧек.ТерминалИД);
		Иначе
			Результат = Ложь;
			ПараметрыПодключения.ТипТранзакции = НСтр("ru='Отказ'");
			ВыходныеПараметры.Очистить();
...
Функция ОтменитьПлатежПоПлатежнойКарте(ОбъектДрайвера, Параметры, ПараметрыПодключения,
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(ОбъектДрайвера) = ТИП("AddIn.IngenicoECR.a_ingenicopb1c82") Тогда
			Ответ = ВозвратПоПривату(Сумма, СлипЧек, Параметры.P_MerchantNum, КодRRN,  Параметры);
		Иначе
		...
		// - HVOYA 24.10.2016 20:45:47, Латышев А.А. 
		Если Ответ Тогда
			СтруктураОтвета = Новый Структура("НомерЧека,КодRRN,КодАвторизации,НомерКарты",НомерЧека,КодRRN,КодАвторизации,НомерКарты);
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(Новый Массив());
			ВыходныеПараметры[0].Добавить("СлипЧек");
			ВыходныеПараметры[0].Добавить(СлипЧек);
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Попытка
		...
		ИначеЕсли ТипЗнч(ОбъектДрайвера) = ТИП("AddIn.IngenicoECR.a_ingenicopb1c82") Тогда
			Ответ = ПроверитьЭТ(РезультатТеста, АктивированДемоРежим, Параметры);
		Иначе
			Ответ = ОбъектДрайвера.ТестУстройства(РезультатТеста, АктивированДемоРежим);
		КонецЕсли; 
		// - HVOYA 30.09.2016 16:07:45, Латышев А.А. 
...
Функция ПолучитьОписаниеДрайвера(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	Попытка
		...
		Иначе
			...
			ВыходныеПараметры[9] = ОбщегоНазначенияВызовСервера.ПолучитьМакетДляПриватБанка();
...
&НаКлиенте
Функция ПроверитьЭТ(РезультатТеста, АктивированДемоРежим, Параметры)
	
	//Initialize library
	POSТерминал = Новый COMОбъект("ECRCommX.BPOS1Lib");
	Если Параметры.Свойство("P_NetConnect")И(Параметры.P_NetConnect) Тогда
		POSТерминал.CommOpenTCP(Параметры.P_IPAddress,Параметры.P_NetPort);
	Иначе
		POSТерминал.CommOpen(Параметры.P_PortNumber,Параметры.P_BaudRate);
	КонецЕсли;
...
&НаКлиенте
Функция Продажа(Сумма, НомерКарты, НомерЧека, КодRRN, КодАвторизации, ДействительнаДо, Клиент, Эмитент, СлипЧек, Параметры)
	Перем POSТерминал;
	
	//Если Не РМКСервер.ЭтоРабочаяБаза() Тогда
	//	Сумма = 0.5;
	//КонецЕсли;
	
	//Initialize library
	POSТерминал = Новый COMОбъект("ECRCommX.BPOS1Lib");
	//2025-01-27 sa добавил возможность подключаться по сети.
	Если Параметры.Свойство("P_NetConnect")И(Параметры.P_NetConnect) Тогда
		POSТерминал.CommOpenTCP(Параметры.P_IPAddress,Параметры.P_NetPort);
	Иначе
		POSТерминал.CommOpen(Параметры.P_PortNumber,Параметры.P_BaudRate);
	...
	Если НЕ LastResult=0 Тогда
		...
		Возврат Ложь;
	КонецЕсли;          
	//2025-07-29 sa при оплате нужно указывать номер мерчанта из настроек драйвера
	//POSТерминал.Purchase(Сумма*100,0,0); 
	Если Параметры.Свойство("P_MerchantNum") Тогда
		НомерМерчанта = Параметры.P_MerchantNum;
	Иначе
		НомерМерчанта = 0;
	КонецЕсли;
	POSТерминал.Purchase(Сумма*100,0,НомерМерчанта);
	mErr = ОжиданиеОтветаТерминала(POSТерминал);
	Если Не mErr = 0 ИЛИ Не ЗначениеЗаполнено(POSТерминал.RRN) Тогда
		POSТерминал.CommClose();
		POSТерминал = Неопределено;
		Возврат Ложь;
	КонецЕсли; 
	...
	//2024-10-08 sa добавляем недостающие свойства
	МерчантИД = POSТерминал.MerchantID;
	ТерминалИД = POSТерминал.TerminalID;
		
	POSТерминал.Confirm();
	mErr = ОжиданиеОтветаТерминала(POSТерминал);
	POSТерминал.CommClose();
	POSТерминал = Неопределено;
	ТекстЧека = "ПРИВАТБАНК" + Символы.ПС + 
			  Символы.ПС +
			...
			//2024-10-08 sa добавляем недостающие свойства
			  "МЕРЧАНТ ID: "+МерчантИД + Символы.ПС+
			  "ТЕРМИНАЛ ID: "+ТерминалИД;
			  
	СлипЧек = Новый Структура("Банк,МерчантИД,ТерминалИД,Эмитент,НомерКарты,НомерТранзакции,КодАвторизации,Клиент,ТекстЧека",
								"ПРИВАТБАНК",МерчантИД,ТерминалИД,Эмитент,НомерКарты,КодRRN,КодАвторизации,Клиент,ТекстЧека);
...
&НаКлиенте
Функция  ВозвратПоПривату(Сумма, СлипЧек, P_MerchantNum, КодRRN, Параметры)
	...
	Если КодАвторизации = "" Тогда
		КодАвторизации= "00000";	//для фискального регистратора код авторизации - обязательное поле.
	КонецЕсли;
	
	СлипЧек = "ПРИВАТБАНК" + Символы.ПС + 
			  //"ДЕЙСТВ. ДО: " + Прав(ДействительнаДо,2) + "/" + Лев(ДействительнаДо,2) + Символы.ПС +
			  Строка(Эмитент) + Символы.ПС +
			  Строка(НомерКарты) + Символы.ПС +
			  "КОД АВТОРИЗАЦИИ: " + КодАвторизации + Символы.ПС +
			  "RRN: " + КодRRN + Символы.ПС +
			  "СУММА: " + СТРОКА(Сумма) + " ГРН" + Символы.ПС  +
			  "КЛИЕНТ: " + Клиент + Символы.ПС+
			//2024-10-08 sa добавляем недостающие свойства
			  "МЕРЧАНТ ID: "+МерчантИД + Символы.ПС+
			  "ТЕРМИНАЛ ID: "+ТерминалИД;
	...
	//СлипЧек = Новый Структура("Банк,МерчантИД,ТерминалИД,Эмитент,НомерКарты,НомерТранзакции,КодАвторизации,Клиент,ТекстЧека",
	//							"ПРИВАТ",МерчантИД,ТерминалИД,Эмитент,НомерКарты,КодRRN,КодАвторизации,Клиент,ТекстЧека);
...
Функция ПродажаРайфайзен(Сумма, НомерКарты, НомерЧека, КодRRN, КодАвторизации, ДействительнаДо, Клиент, Эмитент, СлипЧек, Порт)
	...
	ТекстЧека = "РАЙФФАЙЗЕН БАНК АВАЛЬ" + Символы.ПС + 
			  Строка(Эмитент) + Символы.ПС +
			  //"ДЕЙСТВ. ДО: " + Прав(ДействительнаДо,2) + "/" + Лев(ДействительнаДо,2) + Символы.ПС +
			  Строка(НомерКарты) + Символы.ПС +
			  "КОД АВТОРИЗАЦИИ: " + КодАвторизации + Символы.ПС +
	...
	СлипЧек = Новый Структура("Банк,МерчантИД,ТерминалИД,Эмитент,НомерКарты,НомерТранзакции,КодАвторизации,Клиент,ТекстЧека",
								"РАЙФФАЙЗЕН БАНК АВАЛЬ","","",Эмитент,НомерКарты,КодRRN,КодАвторизации,Клиент,ТекстЧека);

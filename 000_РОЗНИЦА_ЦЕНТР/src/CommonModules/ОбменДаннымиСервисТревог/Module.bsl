...
Процедура ЗаписатьКартуТревог(СтруктураОтвета) Экспорт 
	...
	Для Каждого СтрокаОтвета Из СтруктураОтвета.alerts Цикл
		...
		Если СтруктураТекЗаписи = Неопределено Тогда
			Рег_.Отбор.Период.Установить(МестноеВремя(ПреобразоватьВДату(СтрокаОтвета.started_at),ЧасовойПояс()));
		Иначе
		...
		СтрокаКартыТревог.Период = МестноеВремя(ПреобразоватьВДату(СтрокаОтвета.started_at),ЧасовойПояс());

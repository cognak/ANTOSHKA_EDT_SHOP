...
//	LNK 13.04.2018 13:18:01
Процедура УстановитьТолькоПросмотрДляФормы(КоллекцияЭлементов, КомандыФормы, ТолькоПросмотр, ИменаИсключения = "")	Экспорт

	Перем СписокИсключений;

	Если ТипЗнч(ИменаИсключения) = Тип("Соответствие") Тогда

		СписокИсключений = ИменаИсключения;

	ИначеЕсли ТипЗнч(ИменаИсключения) = Тип("Строка") Тогда

		СписокИсключений = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, ИменаИсключения, "Map", ",;");
...
//	---------------------------------------------------------------------------------------
	...
	Для каждого ЭлементКоллекции Из КоллекцияЭлементов Цикл
		...
		Тогда
			...
			УстановитьТолькоПросмотрЭлементаФормы(ЭлементКоллекции, "ТолькоПросмотр", ТолькоПросмотр, СписокИсключений);
		//	ЭлементКоллекции.ТолькоПросмотр = ТолькоПросмотр;

		ИначеЕсли ТипЭлемента = Тип("КнопкаФормы")
		Тогда

			Если ПустаяСтрока(ЭлементКоллекции.ИмяКоманды) Тогда
				
				УстановитьТолькоПросмотрЭлементаФормы(ЭлементКоллекции, "Доступность", НЕ ТолькоПросмотр, СписокИсключений);
			//	ЭлементКоллекции.Доступность = НЕ ТолькоПросмотр;
			
			Иначе

				КомандаФормы = КомандыФормы.Найти(ЭлементКоллекции.ИмяКоманды);

				Если НЕ КомандаФормы = Неопределено И КомандаФормы.ИзменяетСохраняемыеДанные Тогда
		...
		//	И (ЭлементКоллекции.Вид = ВидГруппыФормы.КонтекстноеМеню ИЛИ ЭлементКоллекции.Вид = ВидГруппыФормы.КоманднаяПанель)
		//Тогда
		//
		//	УстановитьТолькоПросмотрДляФормы(ЭлементКоллекции.ПодчиненныеЭлементы, КомандыФормы, ТолькоПросмотр, СписокИсключений);

		ИначеЕсли ТипЭлемента = Тип("ДекорацияФормы")
			ИЛИ ТипЭлемента = Тип("ГруппаФормы")
		Тогда	//	пропускаем

		ИначеЕсли ТипЭлемента = Тип("ДополнениеЭлементаФормы") Тогда
			
			Отладочная = 22;	//	не знаю, что делать

		Иначе
			
			Отладочная = 22;	//	контроль на будущее
...
//	LNK 13.04.2018 13:18:11
Процедура УстановитьТолькоПросмотрЭлементаФормы(ЭлементКоллекции, Свойство, Значение, СписокИсключений)

	Если СписокИсключений.Получить(ЭлементКоллекции.Имя) = Неопределено Тогда

		ЭлементКоллекции[Свойство] = Значение;
...
Процедура ОткрытьФормуВходящегоЗвонка(Параметр) Экспорт 

	ОткрытьФорму("ОбщаяФорма.ФормаВходящегоЗвонкаКлиента", Параметр); 

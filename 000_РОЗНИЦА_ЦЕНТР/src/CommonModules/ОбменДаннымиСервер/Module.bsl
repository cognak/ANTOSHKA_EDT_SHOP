...
Процедура ПриОбновленииИдентификаторовОбъектовМетаданных(Отказ) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") = Истина Тогда
		...
		УзелИнформационнойБазы = ОбменДаннымиПовтИсп.ГлавныйУзел();
...
Процедура ВыполнитьОбновлениеПравилДляОбменаДанными() Экспорт
	...
	Если Отказ Тогда
		ВызватьИсключение "При оновленні правил обміну даними виникли помилки (див. Журнал реєстрації).";
	КонецЕсли;
...
Процедура ОбновитьРегламентныеЗаданияСценариевОбменовДанными() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Отказ Тогда
			ВызватьИсключение "Помилка під час оновлення регламентного завдання для сценарію обміну даними.";
		КонецЕсли;
...
Процедура ОбновитьКонстантуНастройкаПодчиненногоУзлаРИБЗавершена() Экспорт
	...
	Если  ЭтоПодчиненныйУзелРИБ()
		И РегистрыСведений.НастройкиТранспортаОбмена.НастройкиТранспортаДляУзлаЗаданы(ОбменДаннымиПовтИсп.ГлавныйУзел()) Тогда
...
Процедура ПередОбновлениемИнформационнойБазы() Экспорт
	...
	Попытка
		...
		//	LNK 19.11.2022 12:24:28
			Если ОбщегоНазначенияРТ.ПолучитьУчетнуюПолитику().ВыполнятьЗагрузкуОбменаПередОбновлением = Истина Тогда

				УзелИнформационнойБазы = ОбменДаннымиПовтИсп.ГлавныйУзел();
				
				Если НЕ УзелИнформационнойБазы = Неопределено Тогда
...
Процедура ПриДобавленииПараметровРаботыКлиентскойЛогикиСтандартныхПодсистемПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить("ИмяПланаОбменаРИБ", ?(ЭтоПодчиненныйУзелРИБ(), ОбменДаннымиПовтИсп.ГлавныйУзел().Метаданные().Имя, ""));
	Параметры.Вставить("ГлавныйУзел", ОбменДаннымиПовтИсп.ГлавныйУзел());
...
Процедура ПриОбновленииЗаголовковКомандМонитораОбменаДанными(ДатаСобытия, ПолучитьДатуПоследнейКоллизии) Экспорт
	...
	Если ЛОЖЬ И ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ВерсионированиеОбъектов") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ВерсионированиеОбъектов");
...
//	LNK 22.06.2017 11:39:43 - отключил запреты при загрузке
	Если ЛОЖЬ И НЕ ОбменДаннымиПовтИсп.ТипыИсключенийЗапрещенияЗагрузки().Получить(ТипЗнч(Данные)) = Истина
		И ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ДатыЗапретаИзменения") Тогда
		...
		//	LNK 28.12.2016 08:23:28 - рассинхронизация данных недопустима!
		//	Отказ = Истина;
			ЖурналСобытий.Регистрация("ЗагрузкаДанных.ЗапретЗагрузки", УровеньЖурналаРегистрации.Предупреждение
				,
				, Данные
				,
				, "Фиксация отклонённого отказа:" + Символы.ПС + СообщениеОбОшибке
				, СокрЛП(Данные)
			);
...
//	LNK 21.05.2019 12:44:34
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел(УзелПланаОбмена) Тогда

	//	Получаем данные от главного узла! То есть находимся в периферии.
	//	---------------------------------------------------------------------------------------

		Если ТипЗнч(Данные) = Тип("СправочникОбъект.СегментыНоменклатуры") Тогда

		//	кешировать не будем.. сегментов, собственно, не так и много
			Если Данные.СпособФормирования = Перечисления.СпособыФормированияСегментов.ПериодическиОбновлять
				И СегментыСервер.РазрешеноФормированиеНоменклатурыСегмента() Тогда

			//	Необходимо сохранить отметку времени последнего обновления сегмента, так как он формировался в этой ИБ.
				Запрос = Новый Запрос(
				"ВЫБРАТЬ
				|	СегментыНоменклатуры.ДатаОбновления КАК ДатаОбновления,
				|	СегментыНоменклатуры.ДатаОчистки КАК ДатаОчистки
				|ИЗ
				...
				Если Выборка.Следующий() Тогда
					...
					Данные.ДатаОбновления = Выборка.ДатаОбновления;
					Данные.ДатаОчистки    = Выборка.ДатаОчистки;
	...
	//	Получаем данные от периферии! То есть находимся в главном узле.
	//	---------------------------------------------------------------------------------------
...
//	LNK 31.10.2019 10:03:42
Процедура УдалитьРегистрациюВТестовомУзле(УзелИнформационнойБазы, ВыполнятьВыгрузку)	Экспорт

	Если ВыполнятьВыгрузку И ОбменДаннымиПовтИсп.ЭтоТестовыйУзел() Тогда

		Если ОбменДаннымиПовтИсп.УзлыИсключить().Получить(УзелИнформационнойБазы) = Истина Тогда

			ПланыОбмена.УдалитьРегистрациюИзменений(УзелИнформационнойБазы);

			Если НЕ (ТипЗнч(УзелИнформационнойБазы) = Тип("ПланОбменаСсылка.Полный") ИЛИ ОбменДаннымиПовтИсп.ПолучитьТекущийЭлементСтруктурыУзлов().Пустая()) Тогда

				ПланыОбмена.ЗарегистрироватьИзменения(УзелИнформационнойБазы, ОбменДаннымиПовтИсп.ПолучитьТекущийЭлементСтруктурыУзлов());
...
//	LNK 19.09.2016 10:36:30
	УдалитьРегистрациюВТестовомУзле(УзелИнформационнойБазы, ВыполнятьВыгрузку);
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА НастройкиВыполненияОбменаНастройкиОбмена.ВидТранспортаОбмена = ЗНАЧЕНИЕ(Перечисление.ВидыТранспортаСообщенийОбмена.COM)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОбменЧерезВнешнееСоединение,
	|	ВЫБОР
	|		КОГДА НастройкиВыполненияОбменаНастройкиОбмена.ВидТранспортаОбмена = ЗНАЧЕНИЕ(Перечисление.ВидыТранспортаСообщенийОбмена.WS)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &НомерСтроки = 0", УсловиеПоНомеруСтроки);
	...
	Пока Выборка.Следующий() Цикл
		...
		//	.. если настройка содержит ошибки, то обмен не производим; статус "Отменено"
			...
			СтрокаСообщения = "Початок процесу обміну даними з налаштування %1";
...
//	Если ПустаяСтрока(ИмяПользователя()) Тогда
	Если НЕ ПривилегированныйРежим() Тогда	//	LNK 24.11.2022 07:33:11
	...
	Если НЕ ЗначениеЗаполнено(КодСценарияОбмена) Тогда
		...
		ВызватьИсключение "Не визначено ідентифікатор сценарію обміну даними!";
	...
	Иначе
		...
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"Сценарій обміну даними з кодом %1 не знайдено."
			, КодСценарияОбмена
		);
...
//	LNK 13.12.2016 10:07:43
//	Проведём очистку временного каталога. Обычно там очень много файлов и не все они есть результаты неудачного обмена.
//	Предполагаем, что не всё можно чистить... мало-ли! Чистим то, что точно можно убить (как я думаю).

	СписокМасок = Новый Массив;
	СписокМасок.Добавить("Exchange*.*");
	СписокМасок.Добавить("1Cv8Update*.*");
	СписокМасок.Добавить("IB*.*");
	СписокМасок.Добавить("*.xml");
	СписокМасок.Добавить("*.zip");
	СписокМасок.Добавить("*.rar");
	СписокМасок.Добавить("*.tmp");

	Для каждого Маска Из СписокМасок Цикл

		ФайлыВременные = НайтиФайлы(КаталогВременногоХранилищаФайлов(), Маска);

		Для каждого ФайлВременный Из ФайлыВременные Цикл

			Попытка

				УдалитьФайлы(ФайлВременный.ПолноеИмя);

			Исключение

				ТекстОшибки = ОписаниеОшибки();	//	отладочная строка
...
//	LNK 29.12.2016 15:28:22
Процедура УдалениеНеактуальныхДанных()

	Если НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда

	//	Чистку данных выполняем только в центральном узле.
		Возврат;

	КонецЕсли;

	ЖурналСобытий.РегламентнаяОчистка();

	РегистрыСведений.ОбъектыДляОбработки.РегламентнаяОчистка();

	РегистрыСведений.СтатистикаРегистрацииИзменений.УдалениеНеактуальныхЗаписейСтатистикиРегистрации();
...
Процедура ВыполнитьОбменДаннымиЧерезФайловыйРесурс(СтруктураНастроекОбмена, Знач ТолькоПараметры = Ложь)
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		...
		//Если обмен через FTP, то удаляем файл обмена с сервера
		Если СтруктураНастроекОбмена.РезультатВыполненияОбмена = Неопределено
				И СтруктураНастроекОбмена.ВидТранспортаОбмена
					= Перечисления.ВидыТранспортаСообщенийОбмена.FTP Тогда
				 	
			СтруктураНастроекОбмена.ОбработкаТранспортаСообщенийОбмена.УдалитьФайлОбмена();
...
Процедура ПередЧтениемСообщенияОбмена(Знач Получатель, СообщениеОбмена, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ()
		И ТипЗнч(ОбменДаннымиПовтИсп.ГлавныйУзел()) = ТипЗнч(Получатель) Тогда
...
Процедура ПослеЧтенияСообщенияОбмена(Знач Получатель, Знач СообщениеОбмена, Знач СообщениеПрочитано, СтандартнаяОбработка, Знач УдалитьСообщение = Истина)
	...
	//	LNK 09.11.2016 14:14:11 - Отключим до подробного разбирательства
		Если ЛОЖЬ И КонфигурацияИзменена() Тогда
...
//	LNK 12.12.2016 15:46:09
	Если СтруктураНастроекОбмена.ПроизводитьОтправкуТолькоПриУспешномПриеме Тогда

		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	МАКСИМУМ(ЕСТЬNULL(Результаты.ДатаУспешнойЗагрузки, Ведущая.ПустаяДата)) КАК ДатаУспешнойЗагрузки,
		|	МАКСИМУМ(ЕСТЬNULL(Результаты.ДатаУспешнойВыгрузки, Ведущая.ПустаяДата)) КАК ДатаУспешнойВыгрузки
		|ИЗ
		|	(ВЫБРАТЬ
		|		ДАТАВРЕМЯ(1, 1, 1) КАК ПустаяДата) КАК Ведущая
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			УспешныеОбмены.ДатаОкончания КАК ДатаУспешнойЗагрузки,
		|			ДАТАВРЕМЯ(1, 1, 1) КАК ДатаУспешнойВыгрузки
		|		ИЗ
		|			РегистрСведений.СостоянияУспешныхОбменовДанными КАК УспешныеОбмены
		|		ГДЕ
		|			УспешныеОбмены.УзелИнформационнойБазы = &УзелИнформационнойБазы
		|			И УспешныеОбмены.ДействиеПриОбмене = ЗНАЧЕНИЕ(Перечисление.ДействияПриОбмене.ЗагрузкаДанных)
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		|		
		|		ВЫБРАТЬ
		|			ДАТАВРЕМЯ(1, 1, 1),
		|			УспешныеОбмены.ДатаОкончания
		|		ИЗ
		|			РегистрСведений.СостоянияУспешныхОбменовДанными КАК УспешныеОбмены
		|		ГДЕ
		|			УспешныеОбмены.УзелИнформационнойБазы = &УзелИнформационнойБазы
		|			И УспешныеОбмены.ДействиеПриОбмене = ЗНАЧЕНИЕ(Перечисление.ДействияПриОбмене.ВыгрузкаДанных)) КАК Результаты
		|		ПО (ИСТИНА)"
		);
		Запрос.УстановитьПараметр("УзелИнформационнойБазы", СтруктураНастроекОбмена.УзелИнформационнойБазы);

		Выборка = Запрос.Выполнить().Выбрать();

	//	Если не было вообще никаких движений в регистре, то запрос ничего и не вернёт...

		Если Выборка.Следующий() Тогда

			МаксимальныйИнтервал     = СтруктураНастроекОбмена.ИнтервалОграниченияВыгрузкиСообщенияОбмена * 3600;	//	определено в общей учетной политике!
			РазницаВыгрузкиЗагрузки  = Выборка.ДатаУспешнойВыгрузки - Выборка.ДатаУспешнойЗагрузки;
			РазницаПоследнейВыгрузки = ТекущаяДатаСеанса() - Выборка.ДатаУспешнойВыгрузки;

		//	Получили какие-то даты последних успешных выгрузок и загрузок.
		//	Определяем разницу времени между последней успешной выгрузкой и загрузкой.
		//	Если эта разница отрицательная - выгрузки давно не было, то работу продолжаем в штатном режиме.
		//	Если эта разница положительна (то есть загрузка была ранее выгрузки) и меньше фиксированного
		//	интервала времени (абонент мог не успеть принять пакет), то следует подумать о необходимости
		//	выгрузки. "Подумать" - сравниваем время последней выгрузки с текущим... если эта разница
		//	тоже меньше фиксированного интервала, то выгружать данные НЕ следует.

		//	Базовое условие. Загрузки не было, то есть загрузка была ранее выгрузки!
			НужноОграничить = (РазницаВыгрузкиЗагрузки > 0);

		//	Разнесем все требования по отдельным условиям ...

			Если НужноОграничить Тогда

			//	Проверяем, что не достигли ещё ограничения интервала.
				НужноОграничить = (РазницаВыгрузкиЗагрузки < МаксимальныйИнтервал) И (РазницаПоследнейВыгрузки < МаксимальныйИнтервал);

			//	Разница может быть больше интервала ограничения, но, после последней выгрузки
			//	еще не прошло нужное время после последней выгрузки!
				Если НЕ НужноОграничить Тогда

					НужноОграничить = (РазницаПоследнейВыгрузки < МаксимальныйИнтервал);
				...
				Если НужноОграничить Тогда
					...
					ЗаписьЖурналаРегистрации(СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации, УровеньЖурналаРегистрации.Предупреждение
						, СтруктураНастроекОбмена.УзелИнформационнойБазы.Метаданные(), СтруктураНастроекОбмена.УзелИнформационнойБазы
						, "Ограничение преждевременной выгрузки (нет нужного входящего пакета).");
...
//	LNK 29.10.2016 12:33:30	Процедура записи изменений для сообщения обмена.
...
//	LNK 13.04.2017 15:09:36
	Если ПустаяСтрока(ИмяПользователя()) И НЕ ПривилегированныйРежим() Тогда
...
//	LNK 18.07.2018 10:17:09
	УдалитьРегистрациюВТестовомУзле(УзелИнформационнойБазы, Истина);

	ТехническаяПоддержка.ВыполнитьАлгоритмПоНазначению(Перечисления.ВыполнятьАлгоритм.ПередОтправкойДанных, УзелИнформационнойБазы);

//	LNK 20.04.2017 11:09:56
	...
	ОтправкаДанныхПодчиненному = ОбменДаннымиПовтИсп.ЭтоГлавныйУзел(ПланыОбмена[ИмяПланаОбмена].ЭтотУзел());

	Попытка

				РазрешитьВыгрузкуУдаленияОбъектов = МенеджерПланаОбмена.РазрешитьВыгрузкуУдаленияОбъектов();

	Исключение	РазрешитьВыгрузкуУдаленияОбъектов = Ложь;
	...
	Иначе	ЗаписьXML.УстановитьСтроку();
...
//	считаем количество записанных объектов
	...
	КоличествоОбъектовОбработано = 0;
	ДатаНачалаОперации = ТекущаяДата();	//	замер производительности

//	ИспользоватьТранзакции = КоличествоЭлементовВТранзакции <> 1;
	ИспользоватьТранзакции = Ложь;	//	LNK 10.11.2018 13:24:38

	ОбменДаннымиВызовСервера.ПроверитьКэшМеханизмаРегистрацииОбъектов();

	КлассификаторСостава = ОбменДаннымиПовтИсп.ПолучитьКлассификаторСоставаПланаОбмена(УзелИнформационнойБазы);

	ПредставлениеОшибки = "";
//	LNK 26.11.2016 08:54:53
	ЛокальныйКешДокументов = Новый Соответствие;

	ПолучательОбъект = ЗаписьСообщения.Получатель.ПолучитьОбъект();

	Если ИмяПланаОбмена = "ПоМагазину" И ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда

	//	Записываем данные в сообщение
		ЗаписатьXML(ЗаписьXML, ПолучательОбъект, "ДанныеУзлаПолучателя", НазначениеТипаXML.Явное);

	КонецЕсли;

	Для каждого ИмяКласса Из КлассификаторСостава.Классы Цикл

		Если ИмяКласса = "НоменклатураСегмента" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетНоменклатураСегмента(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "НоменклатурноеСостояние" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетНоменклатурноеСостояние(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ИнформативныеОстаткиТоваровПоМагазинам" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетИнформативныеОстаткиТоваровПоМагазинам(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "КлючевыеАтрибутыКонтрагентов" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетКлючевыеАтрибутыКонтрагентов(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ВерификацияКлиентовНазначение" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетВерификацияКлиентовНазначение(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ПродажиПоДисконтнымКартам" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетПродажиПоДисконтнымКартам(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "СебестоимостьНоменклатуры" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетСебестоимостьНоменклатуры(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "СерийныеНомера" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетСерийныеНомера(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "Номенклатура" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетНоменклатура(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "Контрагенты" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетКонтрагенты(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ИнформационныеКарты" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетИнформационныеКарты(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ФизическиеЛица" Тогда	//	LNK 12.07.2022 05:54:32

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетФизическиеЛица(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "Штрихкоды" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетШтрихкоды(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ВерсииОбъектов" Тогда

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетВерсииОбъектов(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ДействующиеЦеныНоменклатуры" Тогда	//	LNK 23.10.2021 12:22:40

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетДействующиеЦеныНоменклатуры(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ОпорныеЦеныНоменклатуры" Тогда	//	LNK 23.10.2021 12:22:40

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетОпорныеЦеныНоменклатуры(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ПредыдущиеЦеныНоменклатуры" Тогда	//	LNK 02.12.2021 06:10:07

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетПредыдущиеЦеныНоменклатуры(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ЦеныНоменклатуры" Тогда	//	LNK 23.10.2021 12:22:40

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетЦеныНоменклатуры(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ЗаказПокупателя" Тогда	//	LNK 24.04.2023 07:22:29

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетЗаказПокупателя(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ЗаказыПокупателей" Тогда	//	LNK 17.05.2023 09:48:19

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетЗаказыПокупателей(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "ЗакрытиеЗаказовПокупателей" Тогда	//	LNK 18.05.2023 10:06:27

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетЗакрытиеЗаказовПокупателей(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "СоответствияОбъектовУчетныхСистем" Тогда	//	LNK 18.11.2023 07:24:13

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетСоответствияОбъектовУчетныхСистем(ЗаписьСообщения.Получатель, ИмяКласса);

		ИначеЕсли ИмяКласса = "КлючевыеАтрибутыЗаказовПокупателей" Тогда	//	LNK 18.11.2023 07:24:13

			ФильтрВыборки = ОбменДаннымиСерверПакеты.ПолучитьПакетКлючевыеАтрибутыЗаказовПокупателей(ЗаписьСообщения.Получатель, ИмяКласса);
	...
	//	Получаем выборку изменённых данных. При этом в таблицах регистрации устанавливается номер
	//	отправленного сообщения. Видимо, из-за этого и происходит блокировка. Во избежание блокировки
	//	выборку проведем в несколько попыток, разделённых паузой. Должно помочь.
		...
		Исключение
			...
			ТекстОшибки = СокрЛП(ЗаписьСообщения.Получатель) + ": Ошибка (класс[" + ИмяКласса + "]): " + Символы.ПС + ОписаниеОшибки();

			ЗаписьЖурналаРегистрации("ОбменДанными.ВыбратьИзменения", УровеньЖурналаРегистрации.Предупреждение
				, ЗаписьСообщения.Получатель.Метаданные(), ЗаписьСообщения.Получатель
				, ТекстОшибки
		...
		МетаДанных  = Неопределено;
		БылаВыборка = Ложь;

		Пока ВыборкаИзменений.Следующий() Цикл

			Данные = ВыборкаИзменений.Получить();
		//	Флаги для замера производительности
			Попытка

				МетаДанных = Данные.Метаданные();
			...
			ФиксацияПоказателейПроизводительности("Выгрузка",, ДатаНачалаОперации, КоличествоОбъектовОбработано, ЗаписьСообщения.Получатель, МетаДанных);
		...
		//	наборы выгружаем всегда, даже пустые (аналог удаления объекта)
			...
			ВыполнитьСтандартнуюПроверку = ОтправкаДанныхПодчиненному
									И НЕ ОбменДаннымиПовтИсп.ИсключенияПроверкиПриВыгрузке().Получить(ТипЗнч(Данные)) = Истина;

		//	LNK 26.11.2016 09:19:52
			Если ВыполнитьСтандартнуюПроверку И ИмяКласса = "ЦеныНоменклатуры" Тогда

				Отсутствует = "<Объект не найден>";
				Регистратор = Данные.Отбор.Регистратор.Значение;

			//	Набор записей регистра может не иметь регистратора... будет "битая" ссылка
				Если НЕ НРег(Лев(СокрЛП(Регистратор), СтрДлина(Отсутствует))) = НРег(Отсутствует) Тогда

					СоставПроверки = ЛокальныйКешДокументов.Получить(Регистратор);

					Если СоставПроверки = Неопределено Тогда

					//	Возможно, регистраторы выгрузились в предыдущем пакете... а в этой выгрузке
					//	их не было (всегда документы выгружаются до этого регистра - так прописано в классификаторе!).
					//	Отработаем регистратор так, как если бы выгружался объект. А дальше будем делать выводы.
						ДанныеПроверки = Регистратор.ПолучитьОбъект();
						СтандартныеПодсистемыСервер.ПриОтправкеДанныхПодчиненному(ДанныеПроверки, ОтправкаЭлемента, НачальнаяВыгрузкаДанных, ПолучательОбъект);

						УстановитьЛокальныйКешДокументов(ДанныеПроверки, ОтправкаЭлемента, ЛокальныйКешДокументов);
						СоставПроверки   = ЛокальныйКешДокументов.Получить(Регистратор);

						ОтправкаЭлемента = ОтправкаЭлементаДанных.Авто;	//	восстановим, на всякий случай.

					КонецЕсли;

					Если НЕ СоставПроверки = Неопределено Тогда

						Если СоставПроверки.КоличествоВидовЦен = 1 Тогда

							ОтправкаЭлемента = СоставПроверки.ОтправкаЭлемента;
							ВыполнитьСтандартнуюПроверку = Ложь;
			...
			Если ВыполнитьСтандартнуюПроверку Тогда
			...
			//	LNK 26.11.2016 09:07:50
			//	Отработка кешируемых типов данных. На текущий момент используется только для ускорения
			//	выгрузки наборов записей "РегистрыСведений.ЦеныНоменклатуры".
				УстановитьЛокальныйКешДокументов(Данные, ОтправкаЭлемента, ЛокальныйКешДокументов);
			...
			//	ИначеЕсли ОбменДаннымиПовтИсп.РазрешенаПередачаУдаленияОбъекта().Получить(Данные.Метаданные()) = Истина Тогда
			//	LNK 26.06.2018 09:47:30
				ИначеЕсли РазрешитьВыгрузкуУдаленияОбъектов Тогда
		...
		Если БылаВыборка = Истина Тогда

			ФиксацияПоказателейПроизводительности("Выгрузка", Истина, ДатаНачалаОперации, КоличествоОбъектовОбработано, ЗаписьСообщения.Получатель, МетаДанных);
...
//	---------------------------------------------------------------------------------------
	...
	ТехническаяПоддержка.ВыполнитьАлгоритмПоНазначению(Перечисления.ВыполнятьАлгоритм.ПослеОтправкиДанных, УзелИнформационнойБазы);

КонецПроцедуры

//	LNK 26.11.2016 09:08:49
Процедура УстановитьЛокальныйКешДокументов(Данные, ОтправкаЭлемента, ЛокальныйКешДокументов)

	Если ТипЗнч(Данные) = Тип("ДокументОбъект.УстановкаЦенНоменклатуры")
		И ЛокальныйКешДокументов.Получить(Данные.Ссылка) = Неопределено	Тогда

		ТаблицаВидовЦен = Данные.ВидыЦен.Выгрузить(, "ВидЦены");
		ТаблицаВидовЦен.Свернуть("ВидЦены");

		ЛокальныйКешДокументов.Вставить(
			Данные.Ссылка
			, Новый Структура(
				"ОтправкаЭлемента, КоличествоВидовЦен"
				, ОтправкаЭлемента, ТаблицаВидовЦен.Количество()))
...
//	LNK 20.04.2017 11:09:56
	...
	Попытка
		...
		Иначе	ЧтениеXML.ОткрытьФайл(ИмяФайла);
	...
	Если НЕ ЧтениеСообщения.Отправитель = УзелИнформационнойБазы Тогда // Сообщение предназначено не для этого узла
	...
	ТехническаяПоддержка.ВыполнитьАлгоритмПоНазначению(Перечисления.ВыполнятьАлгоритм.ПередПолучениемДанных, УзелИнформационнойБазы);
...
//	Удаляем регистрацию изменений для узла отправителя сообщения
	...
	УчетнаяПолитика = ОбщегоНазначенияРТ.ПолучитьУчетнуюПолитику();	//	LNK 16.11.2021 11:09:21

//	считаем сколько объектов прочитали
	КоличествоЗаписанныхОбъектов = 0;
	КоличествоОбъектовОбработано = 0;
	КоличествоОбъектовОбщее      = 0;
	КлючМетаданных				 = Неопределено;

	Попытка

				РазрешитьУдалениеОбъектов = МенеджерПланаОбмена.РазрешитьУдалениеОбъектов();

	Исключение	РазрешитьУдалениеОбъектов = Ложь;

	КонецПопытки;
	
	ИспользоватьТранзакции = КоличествоЭлементовВТранзакции <> 1;
	БылаКонстантаВерсия              = Ложь;
	БылаКонстантаКонфигурация        = Ложь;
	БылРегистрПрименениеКонфигурации = Ложь;
	...
	//	Читаем данные из сообщения
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			ИмяДанных = ЧтениеXML.Имя;	//	после чтения данных это уже будет имя следующих данных
		//	Читаем очередное значение
			Данные = ПрочитатьXML(ЧтениеXML);

			Если ИмяДанных = "ДанныеУзлаПолучателя" Тогда

				Если ИмяПланаОбмена = "ПоМагазину" И ОбменДаннымиПовтИсп.ЭтоГлавныйУзел(ЧтениеСообщения.Отправитель) Тогда

					УстановитьУправляемыеДанныеЭтогоУзла(Данные, МенеджерПланаОбмена.ЭтотУзел());
			...
			ПриПолученииДанныхОтПодчиненного(Данные, ПолучениеЭлемента, УчетнаяПолитика);
			
			Если НЕ ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать Тогда
		...
		//	Проверяем на наличие коллизии изменений
			Если ПланыОбмена.ИзменениеЗарегистрировано(ЧтениеСообщения.Отправитель, Данные) Тогда
				...
				Если ПрименятьИзменения Тогда
					...
					Если НЕ ИмяПланаОбмена = "Полный" Тогда
		...
		//	Выполняем проверку на коллизии дат запрета загрузки.
			...
			Если ЭтоУдалениеОбъекта	Тогда
				...
				Если РазрешитьУдалениеОбъектов Тогда
				...
				//	выполняем запись ссылки в регистр заданий для последующего регламентного
				//	удаления с контролем ссылочной целостности.

					Попытка

						ВнешниеИсточникиСобытия.УстановитьОбъектДляОбработки("", "УДАЛЕНИЕ_ОБЪЕКТА", Данные.Ссылка,, Истина);
						КоличествоВОбъекте = 1;
					...
					Исключение
						...
						ЗаписьЖурналаРегистрации(КлючСообщенияЖурналаРегистрации, УровеньЖурналаРегистрации.Ошибка,
							, Строка(Данные.Ссылка), ОписаниеОшибки);
						Продолжить;
			...
			Иначе
				...
				Данные.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения", Истина);
			...
			//	Выполняем попытку записи объекта
				Попытка
					...
					Если НЕ КлючМетаданных = Данные.Метаданные() ИЛИ (КоличествоОбъектовОбщее % 10000 = 0) Тогда

					//	LNK 22.11.2020 10:54:52 - пытаемся как-то экономить память процесса.
						ОбновитьПовторноИспользуемыеЗначения();

					КонецЕсли;

					КлючМетаданных = Данные.Метаданные();
				...
				Если ИмяПланаОбмена = "ПоМагазину" И ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда
					...
					Если ТипЗнч(Данные) = Тип("ДокументОбъект.ПеремещениеТоваров") Тогда
						...
						Попытка
							...
							Документы.ПеремещениеТоваров.ПроверитьСерийныеНомераПеремещенияПереброскиПолучатель(Данные);

						Исключение

							ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
							ЖурналСобытий.Регистрация("ОбменДанными.СерийныеНомера", УровеньЖурналаРегистрации.Ошибка
								, Данные.Метаданные()
								, Данные.Ссылка
								,
								, ОписаниеОшибки
			...
			//	LNK 28.12.2016 09:29:46
				КоличествоВОбъекте = ПолучитьРазмерЗагружаемогоОбъекта(Данные);
				
			КонецЕсли;

			КоличествоЗаписанныхОбъектов = КоличествоЗаписанныхОбъектов + КоличествоВОбъекте;
			КоличествоОбъектовОбщее      = КоличествоОбъектовОбщее + 1;

			Если НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда	//	LNK 17.07.2017 13:35:23

			//	Нужные для управления обновлениями объекты выгружаются первыми.
			//	Поэтому условия ниже имеют смысл до трёх первых полученных объектов.
				Если ИспользоватьТранзакции И КоличествоЭлементовВТранзакции > 1 И КоличествоОбъектовОбщее <= 3 Тогда

				//	На периферии необходимо обязательно зафиксировать данные для применения конфигурации.
				...
				//	КлассификаторСостава.Классы.Добавить("ПрименениеКонфигурации");
					...
					Если НЕ(БылаКонстантаВерсия И БылаКонстантаКонфигурация И БылРегистрПрименениеКонфигурации) Тогда

						ТипЗначения = ТипЗнч(Данные);
						НужноЗафиксироватьТранзакцию = Ложь;

						Если ТипЗначения = Тип("КонстантаМенеджерЗначения.МиграцияВерсия") Тогда

							БылаКонстантаВерсия          = Истина;
							НужноЗафиксироватьТранзакцию = Истина;

						ИначеЕсли ТипЗначения = Тип("КонстантаМенеджерЗначения.МиграцияДанные") Тогда

							БылаКонстантаКонфигурация    = Истина;
							НужноЗафиксироватьТранзакцию = Истина;

						ИначеЕсли ТипЗначения = Тип("РегистрСведенийНаборЗаписей.ПрименениеКонфигурации") Тогда

							БылРегистрПрименениеКонфигурации = Истина;
							НужноЗафиксироватьТранзакцию     = Истина;

						КонецЕсли;

						Если НужноЗафиксироватьТранзакцию Тогда
	...
	Иначе
		...
		ЧтениеСообщения.ЗакончитьЧтение();	//	увеличиваем номер полученного!
		
	КонецЕсли;
	
	ЧтениеXML.Закрыть();

	ТехническаяПоддержка.ВыполнитьАлгоритмПоНазначению(Перечисления.ВыполнятьАлгоритм.ПослеПолученияДанных, УзелИнформационнойБазы);
	
КонецПроцедуры

//	LNK 13.04.2017 14:33:06
Процедура УстановитьУправляемыеДанныеЭтогоУзла(Данные, УзелПолучатель)

	УзелОбъект	= УзелПолучатель.ПолучитьОбъект();

	Если НЕ УзелОбъект.Магазин = Данные.Магазин Тогда

		УзелОбъект.Магазин = Данные.Магазин;

	КонецЕсли;

	Если НЕ УзелОбъект.ОсновнаяОрганизация = Данные.ОсновнаяОрганизация Тогда

		УзелОбъект.ОсновнаяОрганизация = Данные.ОсновнаяОрганизация;

	КонецЕсли;

	ИменаКолонок = "Организация";

	Если НЕ ОбщегоНазначения.КоллекцииИдентичны(УзелОбъект.Организации, Данные.Организации, ИменаКолонок,, Истина) Тогда

		УзелОбъект.Организации.Загрузить(Данные.Организации.Выгрузить());

	КонецЕсли;

	Если УзелОбъект.Модифицированность() Тогда
...
Процедура ПриПолученииДанныхОтПодчиненного(Данные, ПолучениеЭлемента, УчетнаяПолитика)

	ПолучениеЭлемента = ПолучениеЭлементаДанных.Авто;
	ТипЗначения		  = ТипЗнч(Данные);

	Если ТипЗначения = Тип("РегистрСведенийНаборЗаписей.НоменклатураСегмента") Тогда

		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(ТаблицаРегистра.Номенклатура) КАК Размер
		|ИЗ
		|	РегистрСведений.НоменклатураСегмента КАК ТаблицаРегистра
		|ГДЕ
		|	ТаблицаРегистра.Номенклатура = &Номенклатура
		|	И ТаблицаРегистра.Сегмент = &Сегмент
		|	И ТаблицаРегистра.Характеристика = &Характеристика"
		);
		Запрос.УстановитьПараметр("Номенклатура", Данные.Отбор.Номенклатура.Значение);
		Запрос.УстановитьПараметр("Сегмент", Данные.Отбор.Сегмент.Значение);
		Запрос.УстановитьПараметр("Характеристика", Данные.Отбор.Характеристика.Значение);

		Выборка = Запрос.Выполнить().Выбрать();
		Выборка.Следующий();

		ПолучениеЭлемента = ?(Данные.Количество() = Выборка.Размер, ПолучениеЭлементаДанных.Игнорировать, ПолучениеЭлемента);

	ИначеЕсли ТипЗначения = Тип("РегистрНакопленияНаборЗаписей.БонусныеБаллы")
		ИЛИ	  ТипЗначения = Тип("РегистрНакопленияНаборЗаписей.БонусныеБаллыВРезерве")
		ИЛИ	  ТипЗначения = Тип("РегистрНакопленияНаборЗаписей.БонусныеБаллыПогашение")
		ИЛИ	  ТипЗначения = Тип("РегистрНакопленияНаборЗаписей.БонусныеБаллыПоЗаказамПокупателей")
		ИЛИ	  ТипЗначения = Тип("РегистрСведенийНаборЗаписей.СписанныеБонусныеБаллы")
	Тогда

		Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() И УчетнаяПолитика.ДвиженияБонусныхБалловТолькоВЦентре = Истина Тогда
...
//	LNK 26.06.2018 13:07:30
Функция ПолучитьРазмерЗагружаемогоОбъекта(Данные)

	КоличествоВОбъекте = 1;

	Попытка

		Если ТипЗнч(Данные) = Тип("ДокументОбъект.ВводОстатковПоНакопительнымСкидкам") Тогда

			КоличествоВОбъекте = Данные.Остатки.Количество();

		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(Данные.Метаданные()) Тогда

			КоличествоВОбъекте = Данные.Количество();

		ИначеЕсли ОбщегоНазначенияРТ.ЕстьТЧОбъекта("Товары", Данные.Метаданные()) Тогда

			КоличествоВОбъекте = Данные.Товары.Количество();

		КонецЕсли;

	Исключение

		ТекстОшибки = ОписаниеОшибки();	//	отладочная строка, кол-во так и останется равным 1.

	КонецПопытки;

	Возврат КоличествоВОбъекте;

КонецФункции

//	LNK 15.11.2018 11:23:22
Процедура ФиксацияПоказателейПроизводительности(НаименованиеОперации, Принудительно = Ложь, ДатаНачалаОперации, КоличествоОбъектовОбработано, Получатель, МетаДанных)
	...
	Если КоличествоОбъектовОбработано > 0 И (Принудительно = Истина ИЛИ КоличествоОбъектовОбработано % 5000 = 0) Тогда

		КоличествоСекунд = Макс(1, ТекущаяДата() - ДатаНачалаОперации);

		Показатель = КоличествоОбъектовОбработано / КоличествоСекунд;

		ТекстСообщения = ?(Принудительно = Истина, "#", "")
		+ "Скорость = " + Формат(Показатель, "ЧДЦ=3; ЧН=0,00; ЧГ=") + " об./с.; "
		+ "Всего " + Формат(КоличествоОбъектовОбработано, "ЧДЦ=; ЧГ=")
		+ " объектов за время " + ОбщегоНазначенияКлиентСервер.ВРЕМЯ(КоличествоСекунд) + " чч:мм:сс."
		+ Символы.ПС + " (последний тип «" + ?(МетаДанных = Неопределено, "нет определения", МетаДанных.ПолноеИмя()) + "»)";
		ЗаписьЖурналаРегистрации("ОбменДанными." + НаименованиеОперации
		, ?(Принудительно = Истина, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Примечание)
		, ?(МетаДанных = Неопределено, Получатель.Метаданные(), МетаДанных), Получатель, ТекстСообщения, РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
...
//Перечисления.РезультатыВыполненияОбмена.Отменено
	ТекстЗапроса =
	"ВЫБРАТЬ
	...
	|		КОГДА СостоянияОбменовДанными.РезультатВыполненияОбмена = ЗНАЧЕНИЕ(Перечисление.РезультатыВыполненияОбмена.Предупреждение_СообщениеОбменаБылоРанееПринято)
	|			ТОГДА 9
	|		КОГДА СостоянияОбменовДанными.РезультатВыполненияОбмена = ЗНАЧЕНИЕ(Перечисление.РезультатыВыполненияОбмена.Отменено)
	|			ТОГДА 10
...
Процедура ПроверитьИспользованиеОбменаДанными() Экспорт
	...
	Если НЕ ПроверитьСертификатИнформационнойБазы() Тогда

		ВызватьИсключение "Проверка сертификата завершилась неудачно! Проверьте установленный сертификат текущей информационной базы.";
...
Процедура ПроверитьВозможностьВыполненияОбменов() Экспорт
	...
	Если Не Пользователи.РолиДоступны("ВыполнениеСинхронизацииДанных, НастройкаСинхронизацииДанных") Тогда
		...
		ВызватьИсключение "Немає прав на виконання обміну даними.";
...
Функция УстановленПризнакНачальнойВыгрузкиДанных(УзелИнформационнойБазы) Экспорт
	...
	Возврат Ложь;	//	LNK 10.11.2018 12:48:42
	УстановитьПривилегированныйРежим(Истина);
...
Функция ЭтоПодчиненныйУзелРИБ() Экспорт
	...
	Возврат ОбменДаннымиПовтИсп.ГлавныйУзел() <> Неопределено;
	
КонецФункции

//	LNK 11.05.2021 12:24:10
Функция ЭтоУзелРасчетаСкидок(УзелИнформационнойБазы)	Экспорт
	...
	Если ТипЗнч(УзелИнформационнойБазы) = Тип("ПланОбменаОбъект.ПоМагазину") Тогда

		ЗначениеВозврата = (УзелИнформационнойБазы.ВидУзла = Перечисления.ВидыУзлов.УзелРасчёта);

	ИначеЕсли ТипЗнч(УзелИнформационнойБазы) = Тип("ПланОбменаСсылка.ПоМагазину") Тогда
		...
		|	И ТаблицаУзлы.ВидУзла = ЗНАЧЕНИЕ(Перечисление.ВидыУзлов.УзелРасчёта)"
		);
		Запрос.УстановитьПараметр("УзелИнформационнойБазы", УзелИнформационнойБазы);
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбмена(СтруктураНастроекОбмена, НастройкаВыполненияОбмена, НомерСтроки)
	...
	|	НастройкиОбмена.УзелИнформационнойБазы.Код КАК УзелИнформационнойБазыКод,
	|	НастройкиОбмена.ВидТранспортаОбмена КАК ВидТранспортаОбмена,
	|	НастройкиОбмена.ВыполняемоеДействие КАК ДействиеПриОбмене,
	|	НастройкиОбмена.КоличествоЭлементовВТранзакции КАК КоличествоЭлементовВТранзакции,
	|	НастройкиОбмена.Ссылка КАК НастройкаВыполненияОбмена,
	|	НастройкиОбмена.Ссылка.Наименование КАК НастройкаВыполненияОбменаНаименование,
	|	НастройкиОбмена.Ссылка.ПроизводитьОтправкуТолькоПриУспешномПриеме КАК ПроизводитьОтправкуТолькоПриУспешномПриеме,
	|	ВЫБОР
	|		КОГДА НастройкиОбмена.ВыполняемоеДействие = ЗНАЧЕНИЕ(Перечисление.ДействияПриОбмене.ЗагрузкаДанных)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ПроизводитьЗагрузкуДанных,
	|	ВЫБОР
	|		КОГДА НастройкиОбмена.ВыполняемоеДействие = ЗНАЧЕНИЕ(Перечисление.ДействияПриОбмене.ВыгрузкаДанных)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|ГДЕ
	|	НастройкиОбмена.Ссылка = &НастройкаВыполненияОбмена
	|	И НастройкиОбмена.НомерСтроки = &НомерСтроки"
	);
	Запрос.УстановитьПараметр("НастройкаВыполненияОбмена", НастройкаВыполненияОбмена);
	Запрос.УстановитьПараметр("НомерСтроки",               НомерСтроки);
...
//	LNK 24.11.2022 08:43:58
	УчетнаяПолитика = ОбщегоНазначенияРТ.ПолучитьУчетнуюПолитику();
	СтруктураНастроекОбмена.ИнтервалОграниченияВыгрузкиСообщенияОбмена = УчетнаяПолитика.ИнтервалОграниченияВыгрузкиСообщенияОбмена;
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбменаДляУзлаИнформационнойБазы(
	...
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда
		...
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	МАКСИМУМ(НастройкиОбмена.Ссылка.ПроизводитьОтправкуТолькоПриУспешномПриеме) КАК ПроизводитьОтправкуТолькоПриУспешномПриеме
		|ИЗ
		|	Справочник.СценарииОбменовДанными.НастройкиОбмена КАК НастройкиОбмена
		|ГДЕ
		|	НЕ НастройкиОбмена.Ссылка.ПометкаУдаления
		|	И НастройкиОбмена.УзелИнформационнойБазы = &УзелИнформационнойБазы"
		);
		Запрос.УстановитьПараметр("УзелИнформационнойБазы", СтруктураНастроекОбмена.УзелИнформационнойБазы);
		...
		Если Выборка.Следующий() Тогда
			...
			СтруктураНастроекОбмена.ПроизводитьОтправкуТолькоПриУспешномПриеме = Выборка.ПроизводитьОтправкуТолькоПриУспешномПриеме;
...
Функция СтруктураНастроекОбменаБазовая()
	...
	СтруктураНастроекОбмена.Вставить("ПроизводитьОтправкуТолькоПриУспешномПриеме", Ложь);
	СтруктураНастроекОбмена.Вставить("ИнтервалОграниченияВыгрузкиСообщенияОбмена", 5);
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	ИначеЕсли Интервал < 60 * 1 Тогда // 1 мин
		
		Результат = "Наразі";
		
	ИначеЕсли Интервал < 60 * 2 Тогда // 2 мин
		
		Результат = "2 хвилини тому";
		
	ИначеЕсли Интервал < 60 * 3 Тогда // 3 мин
		
		Результат = "3 хвилини тому";
		
	ИначеЕсли Интервал < 60 * 10 Тогда // 10 мин
		
		Результат = "10 хвилин тому";

	ИначеЕсли Интервал < 60 * 15 Тогда // 15 мин
		
		Результат = "5 хвилин тому";
		
	ИначеЕсли Интервал < 60 * 30 Тогда // 30 мин
		
		Результат = "15 хвилин тому";
		
	ИначеЕсли Интервал < 60 * 60 * 1 Тогда // 1 час
		
		Результат = "30 хвилин тому";
		
	ИначеЕсли Интервал < 60 * 60 * 2 Тогда // 2 часа
		
		Результат = "1 година тому";
		
	ИначеЕсли Интервал < 60 * 60 * 3 Тогда // 3 часа
		
		Результат = "2 години тому";
	...
	Иначе
		...
		Если КоличествоДнейРазницы = 0 Тогда // сегодня
			...
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Сьогодні, %1", Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 1 Тогда // вчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Вчора, %1", Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 2 Тогда // позавчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Позавчора, %1", Формат(ДатаСинхронизации, "ДЛФ=T"));
...
//	LNK 22.08.2016 12:50:35
	Возврат "FileChecksConnections.tmp";
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису() Экспорт
	...
	Возврат "Обмін даними.Установка підключення до web-сервісу";
...
Функция СобытиеЖурналаРегистрацииЗагрузкаПравилДляОбменаДанными() Экспорт
	...
	Возврат "Обмін даними.Завантаження правил";
...
Функция СобытиеЖурналаРегистрацииСозданиеОбменаДанными() Экспорт
	...
	Возврат "Обмін даними.Створення обміну даними";
...
Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	...
	Возврат "Обмін даними.Видалення тимчасового файлу";
...
Функция ТребуетсяУстановкаОбновления() Экспорт
	...
	Возврат Ложь;	//	LNK 02.01.2018 14:06:46
...
Функция СобытиеЖурналаРегистрацииОбменДанными() Экспорт
	...
	Возврат "Обмін даними";
	
КонецФункции

//	LNK 31.08.2016 14:10:15
Функция ПроверитьСертификатИнформационнойБазы()	Экспорт

	СертификатДействителен = ОбменДаннымиПовтИсп.ПроверитьСертификатИнформационнойБазы(Истина);

	Если СертификатДействителен = 0 Тогда

		ЗаписьЖурналаРегистрации("Проверка сертификата ИБ", УровеньЖурналаРегистрации.Предупреждение
		,,, "Получен недопустимый сертификат информационной базы. Выполнение любых обменов невозможно!", РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);

	КонецЕсли;

	Возврат СертификатДействителен;

КонецФункции // ПроверитьСертификатИнформационнойБазы()

//	LNK 06.09.2016 13:55:24
Процедура БезопасностьУправленияОбменаДанными(ВерсияПроверялась)	Экспорт

	УстановитьПривилегированныйРежим(Истина);

	Если ПустаяСтрока(Константы.КаталогФайловОбновления.Получить()) Тогда

		Константы.КаталогФайловОбновления.Установить("C:\EXCHANGE");

	КонецЕсли;

//	LNK 06.01.2017 08:32:06
	Если РольДоступна("АдминистраторСистемы") Тогда

	//	Лишаем простых пользователей этой задержки при загрузке. Только под администратором.
		УправлениеДоступомСлужебный.ПроверитьАдминистратораАвтоматов();
		УправлениеДоступомСлужебный.ПроверитьУчетнуюЗаписьWebПосредника();
		УправлениеДоступомСлужебный.ПроверитьУчетнуюЗаписьВнешнегоСоединения();

		ПроверитьТекущийУзелПоУстройствамАнтошка();

		Если НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда

			УправлениеДоступомСлужебный.ПроверитьОператораВиртуальнойПолки();

			ПроверитьКлючиРегламентныхЗаданий();

		КонецЕсли;

	//	Отключим использование итогов для некоторых регистров, для которых периодичность
	//	не используется - виртуальными таблицами не работаем!
		СписокМенеджеров = Новый Массив;
		СписокМенеджеров.Добавить(РегистрыСведений.ЖурналСобытий);
		СписокМенеджеров.Добавить(РегистрыСведений.ЦеныТребующиеОбновления);

		ОтключитьИспользованиеИтогов(СписокМенеджеров);	//	LNK 03.01.2018 10:39:55

	КонецЕсли;

	Если НЕ Пользователи.АвторизованныйПользователь().Пустая() Тогда

		КонтрольСоставаСтруктурыУзлов(ВерсияПроверялась);

		РегистрыСведений.РолиПользователейВУзлах.ОбновитьИнформациюОРоляхПользователя();

		БезопасностьПараметровСоединенияСВнешнимиИсточниками();

		Если НЕ ПараметрыСеанса.КонтекстЦентральногоУзла Тогда
		...
		Если СтрДлина(СокрЛП(ПрефиксУзла)) < 3 Тогда

			Константы.ПрефиксУзлаРаспределеннойИнформационнойБазы.Установить(Прав("000" + СокрЛП(ПрефиксУзла), 3));

		КонецЕсли;

		РегистрыСведений.ПрисутствиеПользователя.РегистрацияПользователя();
...
//	LNK 31.08.2016 14:10:18
Процедура КонтрольСоставаСтруктурыУзлов(ВерсияПроверялась)

//	LNK 26.12.2016 12:46:44 - надеюсь, что не промахнулся
	...
	Если НЕ ПустаяСтрока(ДанныеЭтогоУзла.Код) И ДанныеЭтогоУзла.ЭлементСтруктуры.Пустая() Тогда

		Если НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел(ПланыОбмена.ПоМагазину.ЭтотУзел()) Тогда

			ДанныеГлавногоУзла = ОбменДаннымиПовтИсп.ПолучитьДанныеУзла(ОбменДаннымиПовтИсп.ГлавныйУзел());

			Если НЕ ДанныеГлавногоУзла.ЭлементСтруктуры.Пустая() Тогда

				ЭлементОбъект = Справочники.СтруктураУзлов.СоздатьЭлемент();
				Справочники.СтруктураУзлов.ЗаполнитьЭлементПоДаннымУзла(ЭлементОбъект, ДанныеЭтогоУзла);
				ЭлементОбъект.Родитель = ДанныеГлавногоУзла.ЭлементСтруктуры;
	...
	ИначеЕсли НЕ ПустаяСтрока(ДанныеЭтогоУзла.Код) Тогда

		Справочники.СтруктураУзлов.ОбновитьЭлементПоДаннымУзла(ДанныеЭтогоУзла.ЭлементСтруктуры, ДанныеЭтогоУзла);
		ВерсияПроверялась = Истина;

	КонецЕсли;

КонецПроцедуры

//	LNK 06.09.2016 14:01:35
Процедура БезопасностьПараметровСоединенияСВнешнимиИсточниками()

	Если ОбменДаннымиПовтИсп.ПроверитьСертификатИнформационнойБазы() = 0 Тогда
	...
	//	Такие действия производим во избежание случайных обменов с рабочей ИБ КСУ Navision.
	//	Беда может случиться из каких копий разработчиков или ещё каким образом.
	//	Перекинем флаг активности с рабочей базы на первую попавшуюся... не важно, так как рабочая
	//	на текущий момент одна. Не забыть бы, если что изменится!
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ПараметрыСоединения.Источник КАК Источник,
		|	ПараметрыСоединения.ServerAddress КАК ServerAddress,
		|	ПараметрыСоединения.BaseName КАК BaseName,
		|	ПараметрыСоединения.Активен КАК Активен,
		|	ВЫБОР
		|		КОГДА ПараметрыСоединения.ServerAddress = &IP
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК АктивенПравильно,
		|	ВЫБОР
		|		КОГДА ПараметрыСоединения.ServerAddress = &IP
		|			ТОГДА 2
		|		КОГДА ПараметрыСоединения.Активен
		|			ТОГДА 1
		|		ИНАЧЕ 0
		...
		|ГДЕ
		|	ПараметрыСоединения.Источник = ЗНАЧЕНИЕ(Перечисление.ВнешниеИсточники.Navision)
		|
		|УПОРЯДОЧИТЬ ПО
		|	КлючПорядка УБЫВ"
		);
		Запрос.УстановитьПараметр("IP", "10.0.0.121");
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.КлючПорядка = 2 И Выборка.Активен Тогда

				Активность = 0;

			ИначеЕсли Выборка.КлючПорядка < 2 Тогда

				Если Выборка.Активен Тогда

				//	Уже есть активная запись... выходим.
					Прервать;

				Иначе

				//	Ниже будет установлена активность и выход из цикла.
					Активность = 1;
			...
			Если НЕ Активность = -1 Тогда

				МенеджерЗаписи = РегистрыСведений.ПараметрыСоединения.СоздатьМенеджерЗаписи();
				...
				Если МенеджерЗаписи.Выбран() Тогда
					...
					Попытка
						...
						Если Активность = 0 И НЕ ПустаяСтрока(ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы")) Тогда

							ПараметрыСеанса.ВыполняютсяСлужебныеДействия = Истина;
							Константы.ИдентификаторУчетнойСистемы.Установить("");
							ПараметрыСеанса.ВыполняютсяСлужебныеДействия = Ложь;

						ИначеЕсли Активность = 1 Тогда
					...
					Исключение
						...
						ТекстОшибки = ОписаниеОшибки();
						Сообщить("Возникла ошибка: " + ТекстОшибки);
...
//	LNK 10.10.2016 16:55:09
//	Используется в Правилах Регистрации Объекта
//
Процедура ДобавитьВСписокГлавныйУзел(Получатели)	Экспорт

	Если НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда

		Если ЗначениеЗаполнено(ОбменДаннымиПовтИсп.ГлавныйУзел()) Тогда

			Получатели.Добавить(ОбменДаннымиПовтИсп.ГлавныйУзел());
...
//	LNK 03.01.2018 10:39:38
Процедура ОтключитьИспользованиеИтогов(СписокМенеджеров)

	Для каждого МенеджерРегистра Из СписокМенеджеров Цикл
...
//	LNK 27.02.2018 12:05:23
Процедура УстановитьПолучателейМагазина(Получатели, Магазин, ДобавитьПоУстройствам = Ложь)	Экспорт

	СписокУзлов = ОбменДаннымиПовтИсп.ПолучитьСоответствиеУзловМагазинуПоМагазину(ДобавитьПоУстройствам).Получить(Магазин);
...
//	LNK 21.05.2019 10:08:58
Процедура УстановитьПолучателейМагазинов(Получатели, СписокМагазинов, ДобавитьПоУстройствам = Ложь)	Экспорт

	СоответствиеУзлов = ОбменДаннымиПовтИсп.ПолучитьСоответствиеУзловМагазинуПоМагазину(ДобавитьПоУстройствам);

	Для каждого Магазин Из СписокМагазинов Цикл

		СписокУзлов = СоответствиеУзлов.Получить(Магазин);
...
//	LNK 06.04.2018 15:33:57
Процедура УстановитьВсехПолучателей(Получатели, ИмяПланаОбмена)	Экспорт

	Для каждого Узел Из ОбменДаннымиПовтИсп.ПолучитьМассивУзловПланаОбмена(ИмяПланаОбмена) Цикл
...
//	LNK 28.02.2018 14:30:18
Процедура ПроверитьТекущийУзелПоУстройствамАнтошка()
	...
	|ГДЕ
	|	НЕ (ВЫРАЗИТЬ(ТаблицаУзлы.Код КАК СТРОКА(2))) = ""U.""
	...
	Если НЕ Результат.Пустой() Тогда
		...
		УзелОбъект = ПланыОбмена.ПоУстройствамАнтошка.ЭтотУзел().ПолучитьОбъект();

		УзелОбъект.Код = "U.000";
		УзелОбъект.Наименование = "Центр обслуживания мобильных устройств.";
...
//	LNK 02.04.2018 11:05:59
Процедура ПроверитьКлючиРегламентныхЗаданий()	Экспорт

	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() ИЛИ НЕ РольДоступна(Метаданные.Роли.АдминистраторСистемы) Тогда
...
//	---------------------------------------------------------------------------------------
	...
	ОбъектыКонтроля.Добавить(Новый Структура(
		"Ключ, Метаданные", "EXCHANGE_KEY", Метаданные.РегламентныеЗадания.СинхронизацияДанных));
	ОбъектыКонтроля.Добавить(Новый Структура(
		"Ключ, Метаданные", "REG_OBJECTS_KEY", Метаданные.РегламентныеЗадания.РегламентРегистрацииОбъектов));

	Для каждого ОбъектКонтроля Из ОбъектыКонтроля Цикл

		СписокЗаданий = РегламентныеЗадания.ПолучитьРегламентныеЗадания(Новый Структура(
			"Метаданные, Ключ", ОбъектКонтроля.Метаданные, ""));

		Для каждого РегламентноеЗадание Из СписокЗаданий Цикл

			РегламентноеЗадание.Ключ = ОбъектКонтроля.Ключ;
...
//	LNK 17.05.2018 14:50:43 - вызывать только фоновым заданием!
Процедура НачатьПроверкуСертификатаИнформационнойБазы(Идентификатор)	Экспорт
	...
	МенеджерЗаписи.СертификатИнформационнойБазы = НРег(ОбщегоНазначения.ПолучитьПредставлениеИнформационнойБазы());

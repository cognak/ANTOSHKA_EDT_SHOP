...
#Область ПолученияИнформацииОНоменклатуреИзВнешнегоИсточника
...
КонецФункции // ПолучитьИнформациюОНоменклатуре()
...
Функция ПолучитьНоменклатуру_СТАРАЯ(IDN, ПоискПоШтрихкоду = Ложь)
	...
	Если НЕ ПустаяСтрока(IDN) Тогда
		...
		Если СтрДлина(IDN) < 8 И НЕ ПоискПоШтрихкоду = Истина Тогда
			...
			Иначе	ДанныеТовара = ПолучитьНоменклатуру(IDN, Истина);
...
Функция ПолучитьДанныеМагазина(НомерМагазина)
	...
	|	ЕСТЬNULL(Магазины.Ссылка, Ведущая.Магазин) КАК Магазин,
	|	ЕСТЬNULL(Магазины.Город, Ведущая.Город) КАК Город,
	|	ЕСТЬNULL(Магазины.ОбособленноеПодразделениеОрганизации, Ведущая.Регион) КАК Регион,
	|	ЕСТЬNULL(Магазины.Организация, Ведущая.Организация) КАК Организация,
	...
	|		ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Организация) КАК Ведущая
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Магазины КАК Магазины
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|			ПО Магазины.Ссылка = Склады.Магазин
	|		ПО (Магазины.НомерМагазина = &НомерМагазина)
	|ИТОГИ
...
КонецФункции // ПолучитьДанныеМагазина()

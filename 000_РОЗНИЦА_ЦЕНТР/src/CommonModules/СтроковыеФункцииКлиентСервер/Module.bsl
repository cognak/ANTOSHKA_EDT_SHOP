...
Функция ПодставитьПараметрыВСтроку(Знач СтрокаПодстановки,
	...
	Возврат СтрЗаменить(СтрокаПодстановки, "  ", " ");
...
//	LNK 29.03.2017 15:01:19
Функция ПолучитьЧислоИзСтроки(ЛюбаяСтрока, ВернутьКакСтроку = Ложь, ТочкуУбрать = Ложь)	Экспорт

	Значение = "";

	Для Позиция = 1 По СтрДлина(ЛюбаяСтрока) Цикл

		КодСимвола = КодСимвола(Сред(ЛюбаяСтрока, Позиция, 1));

		Если КодСимвола >= 48 И КодСимвола <= 57 ИЛИ ?(ВернутьКакСтроку И ТочкуУбрать, Ложь, КодСимвола = 46) Тогда	// ИЛИ точка
	...
	Возврат ?(ВернутьКакСтроку = Истина, Значение, ОбщегоНазначенияКлиентСервер.СтрокаКакЧисло(Значение));
...
Функция СтрокаЛатиницей(Знач Значение) Экспорт
	...
	Соответствие = СтроковыеФункцииПовтИсп.СоответствиеНациональногоАлфавитаИЛатиницы();
...
#Область КонтрольКорректностиКириллическихНаименований

//	LNK 17.01.2019 14:15:00
Функция ПривестиНаименование(Наименование, ОднаЛексема = Ложь, ТолькоКириллица = Ложь, ИспользоватьТРег = Истина)	Экспорт

	Наименование = СокрЛП(Наименование);

	Если ТолькоКириллица = Истина Тогда

		СимволыУдалить = Новый Массив;

		Для Позиция = 1 По СтрДлина(Наименование) Цикл

			Символ = Сред(Наименование, Позиция, 1);

			Если СтроковыеФункцииПовтИсп.Кириллица(2, Истина, "'").Получить(Символ) = Неопределено Тогда

				СимволыУдалить.Добавить(Символ);

			КонецЕсли;

		КонецЦикла;

		Для каждого Символ Из СимволыУдалить Цикл

			Наименование = СтрЗаменить(Наименование, Символ, "");

		КонецЦикла;

	КонецЕсли;

	Если ОднаЛексема = Истина Тогда
	...
	Если ИспользоватьТРег = Истина Тогда
...
КонецФункции // ПривестиНаименование()

//	LNK 30.01.2019 11:57:17
Функция ПроверитьФамилиюИмяОтчество(ФамилияИмяОтчество, ТекстЗамечанийПроверки)	Экспорт

	Отказ = Ложь;

	Для Позиция = 1 По СтрДлина(ФамилияИмяОтчество) Цикл

		Символ = Сред(ФамилияИмяОтчество, Позиция, 1);

		Если СтроковыеФункцииПовтИсп.Кириллица(2, Истина).Получить(Символ) = Неопределено Тогда

			ТекстЗамечанийПроверки = ТекстЗамечанийПроверки + ?(ПустаяСтрока(ТекстЗамечанийПроверки), "", "; ")
			+ Формат(Позиция, "ЧГ=");
			Отказ = Истина;
...
КонецФункции // ПроверитьФамилиюИмяОтчество()
...
Процедура ВыполнитьОперациюСФункциейF(a, b, c, d, X, s, t)
	ВыполнитьОперацию(ПобитовоеИли_старая(ПобитовоеИ_старая(b, c), ПобитовоеИ_старая(ПобитовоеНе_старая(b), d)), a, b, X, s, t);
КонецПроцедуры

Процедура ВыполнитьОперациюСФункциейG(a, b, c, d, X, s, t)
	ВыполнитьОперацию(ПобитовоеИли_старая(ПобитовоеИ_старая(b, d), ПобитовоеИ_старая(ПобитовоеНе_старая(d), c)), a, b, X, s, t);
КонецПроцедуры
...
Процедура ВыполнитьОперациюСФункциейI(a, b, c, d, X, s, t)
	ВыполнитьОперацию(ПобитовоеИсключающееИли(ПобитовоеИли_старая(ПобитовоеНе_старая(d), b), c), a, b, X, s, t);
КонецПроцедуры
...
Функция ПобитовоеИ_старая(БинарныйМассив1, БинарныйМассив2)
	Результат = Новый Массив;
...
Функция ПобитовоеИли_старая(БинарныйМассив1, БинарныйМассив2)
	Результат = Новый Массив;
	Для Индекс = 0 По БинарныйМассив1.Количество()-1 Цикл
...
Функция ПобитовоеНе_старая(БинарныйМассив)
	Результат = Новый Массив;

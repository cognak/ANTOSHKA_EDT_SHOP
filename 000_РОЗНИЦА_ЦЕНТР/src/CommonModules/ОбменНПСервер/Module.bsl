...
Процедура ПолучитьСтатусТТН(КлючAPI = Неопределено, НомерЭН = "", НоваяТТН = Ложь) Экспорт
	...
	Запрос.Текст =
		...
		|	И (ЭлектроннаяНакладная.Номер = &НомерЭН
		|	ИЛИ &ВсеДокументы)
...
Процедура СделатьЗапиьВРССтатусТТН(ПараметрыДляЗаписи)
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ПолучитьСтатусТТН(ПараметрыДляЗаписи.КлючАПИ, ПараметрыДляЗаписи.IntDocNumber, Истина);

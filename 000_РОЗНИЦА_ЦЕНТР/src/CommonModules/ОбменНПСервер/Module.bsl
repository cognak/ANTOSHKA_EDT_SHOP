...
Функция Обработать_getWarehouses(ЭлементДом,Результат, Владелец, Параметры)
	...
	ИначеЕсли Отделение.Наименование <>  Строка.Наименование
			...
			ИЛИ Строка(Отделение.МаксимальныйВесМеста) <>  Строка.МаксимальныйВесМеста 
			ИЛИ Отделение.Работает <> (СтруктураПараметров.WarehouseStatus = "Working")
			ИЛИ Строка(Отделение.МаксимальнаяВысота) <>  Строка.МаксимальнаяВысота
			ИЛИ Строка(Отделение.МаксимальнаяШирина) <>  Строка.МаксимальнаяШирина
			ИЛИ Строка(Отделение.МаксимальнаяДлинна) <>  Строка.МаксимальнаяДлинна
			ИЛИ Строка(Отделение.ОбщийМаксимальныйВес) <>  Строка.ОбщийМаксимальныйВес тогда
...
Процедура ПолучитьСтатусТТН(КлючAPI = Неопределено, ВнешняяСсылка = "") Экспорт
	...
	Запрос.Текст =
		...
		|	И (ЭлектроннаяНакладная.ВнешняяСсылка = &ВнешняяСсылка
		|			ИЛИ &ВсеДокументы)";
	

	Запрос.УстановитьПараметр("ВнешняяСсылка", ВнешняяСсылка);
	Запрос.УстановитьПараметр("ВсеДокументы", ПустаяСтрока(ВнешняяСсылка));
	РезультатЗапроса = Запрос.Выполнить();

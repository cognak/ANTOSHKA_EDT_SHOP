...
Функция РегистрацияЕН(ДокССылка) Экспорт
	...
	Если СтрЧислоВхождений(Ответтекст,"<success>true</success>") then 
		...
		Если Не ПустаяСтрока(СтруктураОтветаЭН.НомерТТН) Тогда
			...
			СохранитьТТНвPDF(СтруктураОтветаЭН);
		Иначе  
...
Функция СохранитьТТНвPDF(СтруктураДляЗаписи) Экспорт
	...
	ДокументЭН = СтруктураДляЗаписи.ЭлектроннаяНакладная.ПолучитьОбъект();
	ДокументЭН.ПутьКФайлуТТН = ИмяФайлаЗаписиТТН;
	ДокументЭН.ПутьКФайлуСтикера = ИмяФайлаЗаписиСтикер;
	ДокументЭН.Записать();
...
Процедура ПолучитьСписокТТН(КлючAPI = Неопределено) Экспорт
	...
	ДатаС 	= ТекущаяДатаСеанса() - 30*24*60*60;  
...
Процедура СделатьЗапиьВРССтатусТТН(ПараметрыДляЗаписи)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЭлектроннаяНакладная.Ссылка,
		|	СтатусыТТНСрезПоследних.СтатусЭН
		|ИЗ
		|	Документ.ЭлектроннаяНакладная КАК ЭлектроннаяНакладная
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыТТН.СрезПоследних(, Регистратор.ОператорДоставки = &ОператорДоставки
		|		И Регистратор.ВнешняяСсылка = &ВнешняяСсылка) КАК СтатусыТТНСрезПоследних
		|		ПО ЭлектроннаяНакладная.Ссылка = СтатусыТТНСрезПоследних.Регистратор
		|ГДЕ

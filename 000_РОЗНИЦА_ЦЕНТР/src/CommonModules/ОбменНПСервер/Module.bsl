...
Процедура ПолучениеПечатныхДокументовДоставки() Экспорт
	...
	Запрос.Текст =
		...
		|	ЭлектроннаяНакладная.ДокументОснование,
		|	ЭлектроннаяНакладная.Номер,
...
Процедура ЗаполнитьСправочникиНП(Знач ТаблицаМетодов, вхМетод = "") Экспорт
...
Функция Обработать_getWarehouses(ЭлементДом,Результат, Владелец, Параметры)
	...
	Пока не ЭлементДом = неопределено цикл
		Если не СтруктураПараметров.Свойство(ЭлементДом.ИмяУзла)  тогда     
			...
			//Если ЭлементДом.ИмяУзла = "SendingLimitationsOnDimensions" тогда   
				Попытка
					ширина  =  ЭлементДом.ПолучитьЭлементыПоИмени("width")[0].ТекстовоеСодержимое;
					Длинна  =  ЭлементДом.ПолучитьЭлементыПоИмени("lenght")[0].ТекстовоеСодержимое;
					Высота  =  ЭлементДом.ПолучитьЭлементыПоИмени("Height")[0].ТекстовоеСодержимое;
				    СтруктураПараметров.Вставить("ширина",Число(ширина));  
				    СтруктураПараметров.Вставить("Длинна",Число(Длинна));
				    СтруктураПараметров.Вставить("Высота",Число(Высота));
				Исключение	
				КонецПопытки;
			Иначе	
				СтруктураПараметров.Вставить(Строка(ЭлементДом.ИмяУзла),ЭлементДом.ТекстовоеСодержимое);  
	...
	Строка.МаксимальныеГабариты = СтруктураПараметров.SendingLimitationsOnDimensions;
	...
	ИначеЕсли Отделение.Наименование <>  Строка.Наименование
			...
			ИЛИ Отделение.МаксимальныйВесМеста <>  Строка.МаксимальныйВесМеста 
			ИЛИ Отделение.Работает <> (СтруктураПараметров.WarehouseStatus = "Working")
			ИЛИ Отделение.МаксимальнаяВысота <>  Строка.МаксимальнаяВысота
			ИЛИ Отделение.МаксимальнаяШирина <>  Строка.МаксимальнаяШирина
			ИЛИ Отделение.МаксимальнаяДлинна <>  Строка.МаксимальнаяДлинна
		...
		СпрОтделение.ТелефонОтделения =  Строка.ТелефонОтделения;	
		СпрОтделение.Работает = СтруктураПараметров.WarehouseStatus = "Working"; 
		СпрОтделение.МаксимальныеГабариты =  Строка.МаксимальныеГабариты;	
		СпрОтделение.Записать();
	КонецЕсли;
КонецФункции
...
Функция РегистрацияЕН(ДокССылка) Экспорт
	...
	Если СтрЧислоВхождений(Ответтекст,"<success>true</success>") then 
		...
		Если Не ПустаяСтрока(СтруктураОтветаЭН.НомерТТН) Тогда
			ФайлыДляНакладной = СохранитьТТНвPDF(СтруктураОтветаЭН);
			СтруктураОтветаЭН.ПутьКФайлуТТН = ФайлыДляНакладной.ПутьКФайлуТТН;
			СтруктураОтветаЭН.ПутьКФайлуСтикера = ФайлыДляНакладной.ПутьКФайлуСтикера;
...
Функция  СоздатьДокументЭН(СтруктураДляЗаписи)
...
Функция ЗапросДляПочты(ДокументЗапроса)
	...
	|	ТаблицаЭН.ДомПолучатель,
	|	ТаблицаЭН.КвартираПолучатель,
	|	ТаблицаЭН.ФамилияПолучатель,
	|	ТаблицаЭН.ИмяПолучатель,
	|	ТаблицаЭН.ОтчествоПолучатель КАК ОтчествоПолучатель,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаЭН.ГородПолучатель) = ТИП(Справочник.ГородаДоставки)
	...
	|	ТаблицаЗД.Номер КАК НомерЗаказа,
	|	ТаблицаЗД.СуммаДокумента,
	|	ТаблицаЗД.СуммаОплатыДоставки,
	|	ТаблицаЭН.НашаСуммаДоставки КАК НашаСуммаДоставки,
	|	ТаблицаЗП.КомментарийДляДоставки
	|ИЗ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросДоступностиТоваров КАК ТаблицаЗД
...
Функция ТТНАдресАдрес(ДокССылка,КлючAPI)  
	...
	Если Выборка.Следующий() Тогда
		...
		ДопУслуги = "<AdditionalInformation>
			|Тест
			|" + Символы.ПС + СокрЛП(Выборка.КомментарийДляДоставки) + "
			|</AdditionalInformation>";
		ДопУслуги = ДопУслуги + "<InfoRegClientBarcodes>" + Выборка.НомерЗаказа + "</InfoRegClientBarcodes>";
		Если Выборка.СтатусОплаты = Перечисления.СтатусОплаты.НеОплачен тогда
			ДопУслуги = ДопУслуги + "
...
Функция ПолучитьВес(ДокССылка)  
	...
	Запрос.УстановитьПараметр("Ссылка", ДокССылка.ДокументОснование);
...
Функция СохранитьТТНвPDF(СтруктураДляЗаписи) Экспорт
	...
	СформироватьФайл(ТекущийКлюч, СтруктураДляЗаписи.ТТНRef, ИмяФайлаЗаписиТТН, "printDocument"); //ТТН 
	ИмяФайлаЗаписиСтикер = "STICKER"+СокрЛП(СтруктураДляЗаписи.НомерТТН)+".pdf";	
	СформироватьФайл(ТекущийКлюч, СокрЛП(СтруктураДляЗаписи.ТТНRef), ИмяФайлаЗаписиСтикер, "printMarkings");  // Стикер
...
//	ФайлыДляНакладной.Вставить("ПутьКФайлуТТН", "https://roznicabkp.antoshka.ua/ttn/" + ИмяФайлаЗаписиТТН);
//	ФайлыДляНакладной.Вставить("ПутьКФайлуСтикера", "https://roznicabkp.antoshka.ua/ttn/" + ИмяФайлаЗаписиСтикер);
...
//					FTPСоединение = Новый FTPСоединение(АдресСайта,Порт,Логин,Пароль,,Истина,1800);
...
Процедура ПолучитьСписокТТН(КлючAPI = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	ЭлектроннаяНакладная.ТелефонПолучатель КАК ТелефонПолучатель,
		|	СтатусыЭНСрезПоследних.СтатусЭН.СтатусПосылки КАК СтатусЭНСтатусПосылки,
		...
		|ГДЕ
		|	ЭлектроннаяНакладная.ОператорДоставки = ЗНАЧЕНИЕ(Перечисление.ОператорыДоставки.НоваяПочта)
		|	И (НЕ(СтатусыЭНСрезПоследних.СтатусЭН.СтатусПосылки = ЗНАЧЕНИЕ(Перечисление.СтатусыПосылокСлужбыДоставки.Получена)
		|					ИЛИ СтатусыЭНСрезПоследних.СтатусЭН.СтатусПосылки = ЗНАЧЕНИЕ(Перечисление.СтатусыПосылокСлужбыДоставки.Отменена))
		|			ИЛИ СтатусыЭНСрезПоследних.СтатусЭН.СтатусПосылки ЕСТЬ NULL)";
...
//			+ СокрЛП(ОтправкаSMS.ПодготовитьНомерТелефона(Выборка.ТелефонПолучатель))
//			+ "</Phone></item>" + Символы.ПС;
			+ "<item><DocumentNumber>"
			+ СокрЛП(Выборка.Номер)
...
Функция Обработать_getDocumentList(ЭлементДом, Результат, Владелец, Параметры)
	...
	Пока не ЭлементДом = неопределено цикл
		...
		ЭлементДом = ЭлементДом.СледующийСоседний; 
	КонецЦикла;
	СделатьЗапиьВРССтатусТТН(СтруктураПараметров);	
...
Функция Обработать_getStatusDocuments(ЭлементДом, Результат, ТаблицаРегистра,Параметры)
	...
	СтруктураНакладной.Вставить("Номер");
	СтруктураНакладной.Вставить("Статус");
	СтруктураНакладной.Вставить("КодСтатуса");
	СтруктураНакладной.Вставить("НомерВозратнойНакладной");
	
	Пока Не ЭлементДом = Неопределено Цикл

		Если ЭлементДом.ИмяУзла = "Number" Тогда
			СтруктураНакладной.Номер = ЭлементДом.ТекстовоеСодержимое;
		ИначеЕсли ЭлементДом.ИмяУзла = "Status" Тогда
			СтруктураНакладной.Статус = ЭлементДом.ТекстовоеСодержимое;
		ИначеЕсли ЭлементДом.ИмяУзла = "StatusCode" Тогда
			СтруктураНакладной.КодСтатуса = ЭлементДом.ТекстовоеСодержимое;
		ИначеЕсли ЭлементДом.ИмяУзла = "LastCreatedOnTheBasisNumber" Тогда
			СтруктураНакладной.НомерВозратнойНакладной = ЭлементДом.ТекстовоеСодержимое;
		КонецЕсли;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЭлектроннаяНакладная.Ссылка,
		|	СтатусыЭНСрезПоследних.СтатусЭН
		|ИЗ
		|	Документ.ЭлектроннаяНакладная КАК ЭлектроннаяНакладная
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыЭН.СрезПоследних КАК СтатусыЭНСрезПоследних
		|		ПО СтатусыЭНСрезПоследних.ДокументРегистратор = ЭлектроннаяНакладная.Ссылка
		|ГДЕ
		|	ЭлектроннаяНакладная.ОператорДоставки = &ОператорДоставки
		|	И ЭлектроннаяНакладная.Номер = &Номер";
	...
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Не Выборка.СтатусЭН = СтатусТТН Тогда
			...
			Попытка
				...
				НаборЗаписей.Отбор.ДокументРегистратор.Установить(Выборка.Ссылка); 
				НаборЗаписей.Отбор.Период.Установить(ТекДата); 
	
				НоваяЗапись 					= НаборЗаписей.Добавить();
				НоваяЗапись.ДокументРегистратор	= Выборка.Ссылка; 
				НоваяЗапись.Период 				= ТекДата;
				НоваяЗапись.СтатусЭН   			= СтатусТТН; 
				НаборЗаписей.Записать();  
				
				ДокументЭН = Выборка.Ссылка.ПолучитьОбъект();
				ДокументЭН.Статус = СтатусТТН.СтатусПосылки;
				ДокументЭН.Записать();
		...
		Если Не ПустаяСтрока(СтруктураНакладной.НомерВозратнойНакладной) Тогда
			...
			СделатьЗапиьВРССтатусТТН(ПараметрыДляЗаписи);
...
Процедура СделатьЗапиьВРССтатусТТН(ПараметрыДляЗаписи)
		
//	СтруктураСтатуса = Новый Структура;
//	СтруктураСтатуса.Вставить("КодСтатуса", ПараметрыДляЗаписи.StateId);
//	СтруктураСтатуса.Вставить("Наименование", ПараметрыДляЗаписи.StateName);
//	СтруктураСтатуса.Вставить("ОператорДоставки", Перечисления.ОператорыДоставки.НоваяПочта);
//	
//	СтатусТТН = Справочники.КодыСтатусовТТН.ПолучитьСтатус(СтруктураСтатуса);
//	
	...
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() И ПараметрыДляЗаписи.Тестовый Тогда
		...
		СоздатьДокументЭН(СтруктураОтветаЭН);

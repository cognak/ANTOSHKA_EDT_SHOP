...
// Заполнить рус название улиц НП.
...
// 
// Параметры:
//  ДокСсылка - ДокументСсылка.ЭлектроннаяНакладная - Док с сылка
// 
// Возвращаемое значение:
//  Структура - Регистрация ЕН:
// * ДокументДляТТН - ДокументСсылка.ЗапросДоступностиТоваров - 
// * НомерТТН - Строка - 
// * ТТНRef - Строка - 
// * ДатаДоставки - Дата - 
// * СуммаОтОператора - Число - 
// * ОператорДоставки - ПеречислениеСсылка.ОператорыДоставки - 
// * ЭлектроннаяНакладная - ДокументСсылка.ЭлектроннаяНакладная - 
// * ПутьКФайлуТТН - Строка - 
// * ПутьКФайлуТТННакладная - Строка - 
// * ПутьКФайлуСтикера - Строка - 
// * Успех - Булево - 
// * ТексОшибки - Строка - 
// * ОтказОтПолучения - Булево - 
...
Функция РегистрацияЕН(ДокСсылка) Экспорт
	...
	ДанныеЭН = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокСсылка
			, "ДокументОснование
				|, ОтправкаИзМагазина
				|, ДоставкаНаАдрес" );
	
	ДокЗаказ 			= ДанныеЭН.ДокументОснование;
	СтруктураОтветаЭН = СтруктураОтветаПоЭНИнициализация(ДокЗаказ);

	ОтправкаИзМагазина 	= ДанныеЭН.ОтправкаИзМагазина;
	ОтпраккаНаАдрес     = ДанныеЭН.ДоставкаНаАдрес;
	...
	КонецЕсли;
	//Локал
	СтруктураОтветаЭН.Локал = Не СтрНайти(Запрос, "<LocalExpress>1</LocalExpress>") = 0;
	...
	СтруктураОтветаЭН.ТекстЗапроса = Запрос;
	СтруктураОтветаЭН.ТекстОтвета = Ответтекст;
...
// 
// Параметры:
//  ДокСсылка - ДокументСсылка.ЗапросДоступностиТоваров - Док ссылка
// 
// Возвращаемое значение:
//  Булево - ТТНУже создана
...
// 
// Параметры:
//  СтруктураДляЗаписи - см. РегистрацияЕН - Структура для записи
// 
// Возвращаемое значение:
//  Структура - Сохранить ТТНв PDF:
// * ПутьКФайлуТТН - Строка - 
// * ПутьКФайлуТТННакладная - Строка - 
// * ПутьКФайлуСтикера - Строка - 
...
// 
// Параметры:
//  КлючAPI - Строка, Неопределено - Ключ API
...
// Проверить объемный вес.
// 
// Параметры:
//  ДокументПроверки - ДокументСсылка.ЗапросДоступностиТоваров - Документ проверки
//  Отделение - СправочникСсылка.Отделения, СправочникСсылка.Почтоматы - Отделение, почтоматы
// 
// Возвращаемое значение:
//  Булево - Истина - Если вес посылки может быть обработан отделением
Функция ПроверитьОбъемныйВес(ДокументПроверки, Отделение) Экспорт 
	...
	Запрос.УстановитьПараметр("Ссылка", ДокументПроверки);
...
// 
// Параметры:
//  НомерДок - Строка - Номер док
//  Телефон Строка - Телефон
// 
// Возвращаемое значение:
//  Число - есди 1 тогда посылка еще не уехала.
...
// 
// Параметры:
//  НомерНакладнойRef - строка - Номер накладной ref
// 
// Возвращаемое значение:
//  Строка - Удалить накладную
...
// 
// Параметры:
//  СсылкаНаЭН - ДокументСсылка.ЭлектроннаяНакладная - Ссылка на ЭН
// 
// Возвращаемое значение:
//  Структура - Получить статус ЭН:
// * СтатусЭН - СправочникСсылка.КодыСтатусовТТН - 
// * СтатусСлужбыДоставки - ПеречислениеСсылка.СтатусыПосылокСлужбыДоставки - 
...
// Добавить в реестр ЕН.
// 
// Параметры:
//  СсылкаЕН - ДокументСсылка.ЭлектроннаяНакладная - Ссылка на ЭН
//  ЕНРеф  - Строка - ЕНРеф
//  РефРеестра - Строка, Неопределено - Реф реестра
...
Процедура ВызовМашины() Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрСведений.НастройкиСлужбДоставкиМагазинов КАК ТаблицаНастроек
...
Функция ДобавитьГородНовый(Знач ГородИдентификатор) Экспорт

	Результат = Справочники.ГородаДоставки.ПустаяСсылка();
	ТочкаВхода = ОбменНПСлужебный.ПолучитьТочкуВхода();
	...
	ПараметрыМетода.Вставить("SettlementRef", ГородИдентификатор);
	
	ТекстЗапроса = ПолучитьТекстЗапроса("getSettlements", ПараметрыМетода);
	...
	Иначе
		лВыполняемаяобработка = ПолучитьПроцедуруПоМетоду("getSettlements");
		РаспарситьОтвет(лВыполняемаяобработка, ДокументДом, Результат, "getSettlements", ПараметрыМетода);
	КонецЕсли;
...
Функция ДобавитьУлицуНовый(Знач ГородИдентификатор, Знач НаименованиеУлицы) Экспорт

	Результат = Справочники.Улицы.ПустаяСсылка();
	ТочкаВхода = ОбменНПСлужебный.ПолучитьТочкуВхода();
	...
	ПараметрыМетода.Вставить("КлючАПИ", Ключ);
	ПараметрыМетода.Вставить("SettlementRef", ГородИдентификатор);
	ПараметрыМетода.Вставить("НаименованиеУлицы", НаименованиеУлицы);

	ТекстЗапроса = ПолучитьТекстЗапроса("searchSettlementStreets", ПараметрыМетода);
	...
	Если не  ЭтоУспех(ДокументДом) тогда
		...
		Сообщить(ТекстИсключения);
	Иначе
		лВыполняемаяобработка = ПолучитьПроцедуруПоМетоду("searchSettlementStreets");
		РаспарситьОтвет(лВыполняемаяобработка, ДокументДом, Результат, "searchSettlementStreets", ПараметрыМетода);
	КонецЕсли;
...
Процедура ЗаполнитьСправочникиНП(Знач ТаблицаМетодов, ВходМетод = "", ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для каждого Строка Из ТаблицаМетодов Цикл
		...
		Если Строка.Метод = "getStreet" Или Строка.Метод = "getWarehouses" Или Строка.Метод = "getTimeIntervals" Тогда
			...
			Пока Города.Следующий() Цикл
				...
				Пока Истина Цикл
					Результат = новый Структура();
					Для Повторы = 1 По 5 Цикл 
						ПараметрыМетода.Вставить("Page", Страницы);
						...
						КонецЕсли;
						ДокументДом  = ПолучитьОтветДОМ(ТочкаВхода,ТекстЗапроса);
						ОбщегоНазначенияКлиентСервер.Пауза(2000);
						Если  ДокументДом = Неопределено тогда
					...
					Конеццикла;
					Страницы = Страницы + 1;
					Если Результат.Свойство("ПустыеДанные") Тогда
						Прервать;	
					КонецЕсли;
//					Если Строка.Метод <> "getStreet" И Строка.Метод <> "GETSTREET" Тогда
//						Прервать;
//					КонецЕсли;
				Конеццикла;
			Конеццикла;
...
Функция СтруктураОтветаПоЭНИнициализация(ДокуменДляТТН) Экспорт
	...
	Результат.Вставить("Локал", Ложь);
	Результат.Вставить("ЗаказПокупателя", Документы.ЗаказПокупателя.ПустаяСсылка());
...
Функция  СоздатьДокументЭН(СтруктураДляЗаписи) Экспорт
	...
	Попытка
		...
		НовыйДокумент.ЗаказПокупателя = СтруктураДляЗаписи.ЗаказПокупателя;  
...
Функция ПолучитьТекстЗапроса(знач ИмяМетода, ПараметрыМетода = неопределено)
	...
	ИначеЕсли ИмяМетода = "searchSettlementStreets" тогда
		Текст = ПолучитьЗапрос_searchSettlementStreets(ПараметрыМетода);
	ИначеЕсли ИмяМетода = "getSettlements" тогда
		Текст = ПолучитьЗапрос_getSettlements(ПараметрыМетода);
...
Функция ПолучитьЗапрос_searchSettlementStreets(ПараметрыМетода)
	ТекстЗапроса = 
	"<?xml version=""1.0"" encoding=""UTF-8""?>
	|<file>
	|	<apiKey>"+ПараметрыМетода.КлючАПИ+"</apiKey>
	|	<modelName>AddressGeneral</modelName>
	|	<calledMethod>searchSettlementStreets</calledMethod>
	|	<methodProperties>
	|		<StreetName>"+ПараметрыМетода.НаименованиеУлицы+"</StreetName>
	|		<SettlementRef>"+ПараметрыМетода.SettlementRef+"</SettlementRef>
	|	</methodProperties>
	|</file>";
...
Функция ПолучитьЗапрос_getSettlements(ПараметрыМетода)
	...
	|	<methodProperties>
	|		<Ref>"+ПараметрыМетода.SettlementRef+"</Ref>
	|	</methodProperties>
	|</file>";
...
Процедура РаспарситьОтвет(лВыполняемаяобработка,ДокументДом, Результат, Владелец = неопределено,Параметры = Неопределено)
	...
	Если КоличествоЭлементов > 0 тогда
		...
		Если лВыполняемаяобработка = "Обработать_searchSettlementStreets" Тогда
			Item = Узел.Элемент(1);
		Иначе
			Item = Узел.Элемент(0);
		конецЕсли;
	...
	Иначе
		...
		Результат.Вставить("ПустыеДанные", Истина);
...
Функция ПолучитьПроцедуруПоМетоду(ИмяМетода)
	...
	ИначеЕсли ИмяМетода = ВРег("searchSettlementStreets") тогда
		Текст = "Обработать_searchSettlementStreets";
	ИначеЕсли ИмяМетода = "GETSTREET" тогда
		Текст = "Обработать_getStreet";
	ИначеЕсли ИмяМетода = "GETSETTLEMENTS" тогда
		Текст = "Обработать_getSettlements";
...
Функция Обработать_getCities(ЭлементДом,Результат, Владелец, Параметры)
	...
	//СтрокаХеша = ПолучитьХеш(СтруктураПараметров);
	СтрокаХеша = ОбщегоНазначения.КонтрольнаяСуммаСтрокой(СтруктураПараметров);

	Если Не СпрГород.ХешCity = СтрокаХеша Тогда
...
Функция Обработать_getSettlements(ЭлементДом, Результат, Владелец, Параметры)
	СтруктураПараметров = Новый структура;
	Пока не ЭлементДом = неопределено цикл
		Если не СтруктураПараметров.Свойство(ЭлементДом.ИмяУзла)  тогда
			СтруктураПараметров.Вставить(Строка(ЭлементДом.ИмяУзла),ЭлементДом.ТекстовоеСодержимое);
		Иначе
			Текст = "Ошибка getSettlements";
			ВызватьИсключение Текст;
		КонецЕсли;	 
		ЭлементДом = ЭлементДом.СледующийСоседний; 
	КонецЦикла;

	СпрОбласть = Справочники.Области.НайтиПоНаименованию(СтруктураПараметров.AreaDescription); 
	СпрРайоны = Справочники.Районы.НайтиПоРеквизиту("Идентификатор", СтруктураПараметров.Region);
	Если НЕ ЗначениеЗаполнено(СпрРайоны) тогда
		РайоныОбъект = Справочники.Районы.СоздатьЭлемент();
		РайоныОбъект.Идентификатор = СтруктураПараметров.Region;
		РайоныОбъект.Наименование = СтруктураПараметров.RegionsDescription;
		РайоныОбъект.НаименованиеRU = СтруктураПараметров.RegionsDescriptionRu;
		РайоныОбъект.ОператорДоставки = Перечисления.ОператорыДоставки.НоваяПочта;
		РайоныОбъект.Записать();
		СпрРайоны = РайоныОбъект.Ссылка;
	КонецЕсли;
	Город = Справочники.ГородаДоставки.НайтиПоРеквизиту("ИдентификаторSettlement", СтруктураПараметров.Ref);  
	Если НЕ ЗначениеЗаполнено(Город) тогда
		СпрГород = Справочники.ГородаДоставки.СоздатьЭлемент();
	Иначе
		СпрГород = Город.ПолучитьОбъект();
	...
	СпрГород.ИдентификаторSettlement = СтруктураПараметров.Ref;		
	СпрГород.Наименование = СтруктураПараметров.Description;
	СпрГород.НаименованиеRU = СтруктураПараметров.DescriptionRu;
	СпрГород.Область = СпрОбласть;
	СпрГород.Район = СпрРайоны;
	СпрГород.ПН = ?(СтруктураПараметров.Delivery1 = "1",Истина,Ложь);
	СпрГород.ВТ = ?(СтруктураПараметров.Delivery2 = "1",Истина,Ложь);
	СпрГород.СР = ?(СтруктураПараметров.Delivery3 = "1",Истина,Ложь);
	СпрГород.ЧТ = ?(СтруктураПараметров.Delivery4 = "1",Истина,Ложь);
	СпрГород.ПТ = ?(СтруктураПараметров.Delivery5 = "1",Истина,Ложь);
	СпрГород.СБ = ?(СтруктураПараметров.Delivery6 = "1",Истина,Ложь);
	СпрГород.ВС = ?(СтруктураПараметров.Delivery7 = "1",Истина,Ложь);
	СпрГород.ОператорДоставки = Перечисления.ОператорыДоставки.НоваяПочта;
	СпрГород.Записать();
	
	Результат = СпрГород.Ссылка;

КонецФункции

Функция Обработать_searchSettlementStreets(ЭлементДом, Результат, Владелец, Параметры)
	...
	Пока не ЭлементДом = неопределено цикл
		Если не СтруктураПараметров.Свойство(ЭлементДом.ИмяУзла)  тогда
			СтруктураПараметров.Вставить(Строка(ЭлементДом.ИмяУзла),ЭлементДом.ТекстовоеСодержимое);
		Иначе
			Текст = "Ошибка searchSettlementStreets";
			ВызватьИсключение Текст;
		КонецЕсли;	 
		ЭлементДом = ЭлементДом.СледующийСоседний; 
	КонецЦикла;
	...
	Город = Справочники.ГородаДоставки.НайтиПоРеквизиту("ИдентификаторSettlement", СтруктураПараметров.SettlementRef);  
	Если ЗначениеЗаполнено(Город) тогда
		...
		Запрос.Текст =
			...
			|	Справочник.Улицы КАК Улицы
			|ГДЕ
			|	(Улицы.Наименование = &Наименование
			|	ИЛИ Улицы.Идентификатор = &Идентификатор)
			|	И Улицы.Город = &Город";
		
		Запрос.УстановитьПараметр("Город", Город);
		Запрос.УстановитьПараметр("Наименование", СтруктураПараметров.SettlementStreetDescription);
		Запрос.УстановитьПараметр("Идентификатор", СтруктураПараметров.SettlementStreetRef);
		...
		Если Выборка.Следующий() Тогда
			Улица = Выборка.Ссылка.ПолучитьОбъект();
		Иначе
			Улица = Справочники.Улицы.СоздатьЭлемент();
		КонецЕсли;
		
		Улица.Наименование = СтруктураПараметров.SettlementStreetDescription;
		Улица.НаименованиеRU = СтруктураПараметров.SettlementStreetDescriptionRu;
		Улица.Идентификатор = СтруктураПараметров.SettlementStreetRef;
		Улица.ОператорДоставки = Перечисления.ОператорыДоставки.НоваяПочта;
		Улица.Город = Город;
		Улица.ТипУлицы = Справочники.ТипыУлиц.НайтиПоНаименованию(СтруктураПараметров.StreetsTypeDescription);
		Улица.Записать();
		
		Результат = Улица.Ссылка;
...
Функция Обработать_getWarehouses(ЭлементДом,Результат, Владелец, Параметры)
	...
	//Запись Сетелмент Города

	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ГородаДоставки.Ссылка,
		|	ГородаДоставки.ИдентификаторSettlement = &ИдентификаторSettlement КАК Совпадает
		|ИЗ
		|	Справочник.ГородаДоставки КАК ГородаДоставки
		|ГДЕ
		|	ГородаДоставки.Идентификатор = &Идентификатор";
	
	Запрос.УстановитьПараметр("ИдентификаторSettlement", СтруктураПараметров.SettlementRef);
	Запрос.УстановитьПараметр("Идентификатор", СтруктураПараметров.CityRef);
	...
	Если Выборка.Следующий() Тогда
		Если Не Выборка.Совпадает Тогда
			ГородОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ГородОбъект.ИдентификаторSettlement = СтруктураПараметров.SettlementRef;
			ГородОбъект.Записать();
		КонецЕсли;
		ГородСсылка = Выборка.Ссылка;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	...
	Если Не СпрОтделение.Хеш = СтрокаХеша Тогда
		...
		СпрОтделение.Город = ГородСсылка;
...
Функция ЗапросДляПочты(ДокументЗапроса)
	...
	|		ИНАЧЕ ВЫБОР
	|			КОГДА НЕ ТаблицаЭН.КонтрагентПолучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА Контрагенты.Наименование
	|			ИНАЧЕ ТаблицаЭН.ФамилияПолучатель
	|		КОНЕЦ
	|	КОНЕЦ КАК ФамилияПолучатель,
	|	ВЫБОР
	|		КОГДА ТаблицаЭН.ВозвратОтКлиента
	|		ИЛИ НЕ ТаблицаЭН.КонтрагентПолучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА """"
	|		ИНАЧЕ ТаблицаЭН.ИмяПолучатель
	|	КОНЕЦ КАК ИмяПолучатель,
	|	ВЫБОР
	|		КОГДА ТаблицаЭН.ВозвратОтКлиента
	|		ИЛИ НЕ ТаблицаЭН.КонтрагентПолучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА """"
	...
	|	ВЫБОР
	|		КОГДА ТаблицаЭН.ВозвратОтКлиента
	|		ИЛИ НЕ ТаблицаЭН.МагазинПолучатель = ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка)
	|		ИЛИ НЕ ТаблицаЭН.КонтрагентПолучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА ""Organization""
	|		ИНАЧЕ ""PrivatePerson""
	...
	|		ИНАЧЕ ВЫБОР
	|			КОГДА НЕ ТаблицаЭН.КонтрагентПолучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА ТаблицаСобственности.Идентификатор
	|			ИНАЧЕ """"
	|		КОНЕЦ
	...
	|		КОГДА ТаблицаЭН.ВозвратОтКлиента
	|		ИЛИ НЕ ТаблицаЭН.МагазинПолучатель = ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка)
	|			ТОГДА УчетнаяПолитикаСрезПоследних.КонтактноеЛицоНовойПочты.КонтактноеЛицо
	|		ИНАЧЕ ВЫБОР
	|			КОГДА НЕ ТаблицаЭН.КонтрагентПолучатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА СОКРЛП(СОКРЛП(ТаблицаЭН.ФамилияПолучатель) + "" "" + СОКРЛП(ТаблицаЭН.ИмяПолучатель) + "" "" +
	|					СОКРЛП(ТаблицаЭН.ОтчествоПолучатель))
	|			ИНАЧЕ """"
	|		КОНЕЦ
	|	КОНЕЦ КАК КонтактноеЛицоНовойПочты,
	|	ТаблицаЭН.ВозвратОтКлиента КАК ВозвратОтКлиента,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаЭН.ГородПолучатель) = ТИП(Справочник.ГородаДоставки)
	|			ТОГДА ТаблицаЭН.ГородПолучатель.Наименование
	|		ИНАЧЕ ТаблицаЭН.ГородПолучатель
	|	КОНЕЦ = ТаблицаЭН.ГородОтправитель.Наименование
	|	И ЕСТЬNULL(УчетнаяПолитикаМагазиновСрезПоследних.ИспользоватьЛокалЕкспрес, ЛОЖЬ) КАК ЛокалЕкспрес,
	|	ТаблицаЭН.ЭтажОтправителя КАК ЭтажОтправителя,
	|	ТаблицаЭН.ЭтажПолучателя КАК ЭтажПолучателя
	|ИЗ
	|	Документ.ЭлектроннаяНакладная КАК ТаблицаЭН
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаМагазинов.СрезПоследних КАК УчетнаяПолитикаМагазиновСрезПоследних
	|			ПО ТаблицаЗД.МагазинОтправитель = УчетнаяПолитикаМагазиновСрезПоследних.Магазин
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФормаСобственности КАК ТаблицаСобственности
	|			ПО Контрагенты.ФормаСобственности = ТаблицаСобственности.Ссылка
	|		ПО ТаблицаЭН.КонтрагентПолучатель = Контрагенты.Ссылка
	|ГДЕ
...
Функция ТТНАдресАдрес(ДокСсылка,КлючAPI)  
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЛокалЕкспрес Тогда
			ПосылкаЛокал = "1";
			ДопУслуги = ДопУслуги + Символы.ПС + "ЛОКАЛ";
		Иначе
			ПосылкаЛокал = "0";
		КонецЕсли;
		ДопУслуги = ДопУслуги + Символы.ПС + Лев(СокрЛП(Выборка.КомментарийДляДоставки), 36) + "
		...
		Если Выборка.КонтрольОплаты тогда
			...
			|	<AfterpaymentOnGoodsCost>"
				...
				+"</AfterpaymentOnGoodsCost>";
		КонецЕсли;                   
		Если Не Выборка.ЭтажОтправителя = 0 тогда
			ДопУслуги = ДопУслуги + "
			|	<NumberOfFloorsDescent>"
				+ Выборка.ЭтажОтправителя
				+"</NumberOfFloorsDescent>";
		КонецЕсли;                   
		Если Не Выборка.ЭтажПолучателя = 0 тогда
			ДопУслуги = ДопУслуги + "
			|	<NumberOfFloorsLifting>"
				+ Выборка.ЭтажПолучателя
				+"</NumberOfFloorsLifting>";
		КонецЕсли;                   
		ФиоКлиента = "";
		...
		|<LocalExpress>"+ПосылкаЛокал+"</LocalExpress>
		|</methodProperties>
		|</root>";
...
Функция ПолучитьМестоОтправки(Магазин)
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.НастройкиСлужбДоставкиМагазинов КАК НастройкиСлужбДоставки
		|ГДЕ
...
Функция ПередатьФайл(ИмяФайлаЗаписи, ИмяФайлаЗаписиНаФТП) Экспорт
...
Функция Обработать_getStatusDocuments(ЭлементДом, Результат, ТаблицаРегистра, Параметры)
	...
	ЭНСоответствие.Вставить("DocumentCost", "СтоимостьДокумента");
	...
	Пока Выборка.Следующий() Цикл
		Если Не Выборка.СтатусЭН = СтатусТТН.СсылкаКодСтатуса Тогда
			...
			РегистрыСведений.СтатусыЭН.ЗаписьСтатуса(Выборка.Ссылка
							, СтатусТТН.СсылкаКодСтатуса
							, СтруктураНакладной.СтоимостьДокумента);
...
Процедура СоздатьЭН(ПараметрыДляЗаписи)
	...
	Иначе
		...
		Если Не ПустаяСтрока(ПараметрыДляЗаписи.ДокументОснование) Тогда
			...
			СтруктураОтветаЭН.ЗаказПокупателя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыДляЗаписи.ДокументОснование, "ЗаказПокупателя");
	...
	Если ПараметрыДляЗаписи.ОтказОтПолучения И ВозвратнаяНакладная Тогда
		...
		Запрос.Текст =
			...
			|		ПО ТаблицаЭН.ЗаказПокупателя = ТаблицаЗаказа.Ссылка
			|ГДЕ
			|	ТаблицаЭН.Ссылка = &ЭлектроннаяНакладная
			|	И ТаблицаЗаказа.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПокупателей.Отменён)";
...
Функция Обработать_insertDocumentsScanSheet(ЭлементДом, Результат, ТаблицаРегистра, Параметры)
	...
	Если Не ТабПараметров.Количество() = 0 Тогда
		...
		Иначе
			...
			ДанныйЭН = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.СсылкаЕН, "ДокументОснование, Локал");
			МагазинОтправитель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныйЭН.ДокументОснование, "МагазинОтправитель");
			ДокументРеестра.МагазинСклад = МагазинОтправитель;
			ДокументРеестра.ПосылкиЛокал = ДанныйЭН.Локал;
			ДокументРеестра.Записать();
...
Функция Обработать_removeDocuments(ЭлементДом, Результат, ТаблицаРегистра,Параметры)
	...
	Если Не ТабПараметров.Количество() = 0 Тогда
		...
		Если Не ДокументРеестрССылка = Неопределено И Не ДокументРеестрССылка.Пустая() Тогда
...
Функция ПолучитьХеш(СтруктураПараметров)

	ЗаписьJSON = Новый ЗаписьJSON;
    ЗаписьJSON.УстановитьСтроку();
    ЗаписатьJSON(ЗаписьJSON, СтруктураПараметров);
    СтрокаСтруктуры = ЗаписьJSON.Закрыть();

	ХешированиеДанных = Новый ХешированиеДанных(ХешФункция.SHA256);
	ХешированиеДанных.Добавить(СтрокаСтруктуры);

...
Функция Обработать_getStatusDocuments(ЭлементДом, Результат, ТаблицаРегистра,Параметры)
	...
	Запрос.Текст = 
		...
		|	ЭлектроннаяНакладная.Реестр КАК Реестр,
		...
		|		ПО ЭлектроннаяНакладная.ДокументОснование = ТаблицаЗапроса.Ссылка
		|ГДЕ
				...
				Если СтатусТТН.СтатусПосылки = Перечисления.СтатусыПосылокСлужбыДоставки.Отказ Тогда
					...
					Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
						...
						ОтветственныйПочта = Модуль.ПолучитьКонтактнуюИнформацияОбъекта(
								Выборка.Ответственный, Справочники.ВидыКонтактнойИнформации.EmailПользователя);
						Адрес = "im@antoshka.ua;" + ОтветственныйПочта;

...
Функция ЗапросДляПочты(ДокументЗапроса)
	...
	|	ЕСТЬNULL(ТаблицаЗД.СуммаДокумента, 0) КАК СуммаДокумента,
...
Функция ЗаполнитьКоличествоМест(ДокССылка)  
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Ширина, ЭлектроннаяНакладнаяУпаковки.Ширина) КАК Ширина,
		|	ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Высота, ЭлектроннаяНакладнаяУпаковки.Высота) КАК Высота,
		|	ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Глубина, ЭлектроннаяНакладнаяУпаковки.Глубина) КАК Длина,
		|	ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Вес, ЭлектроннаяНакладнаяУпаковки.Вес) КАК Вес
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭлектроннаяНакладная.Упаковки КАК ЭлектроннаяНакладнаяУпаковки
		|		ПО ЭлектроннаяНакладнаяУпаковки.Ссылка = ЭлектроннаяНакладная.Ссылка
		|ГДЕ
...
Функция ПолучитьВес(ДокССылка)  
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Вес, ЭлектроннаяНакладнаяУпаковки.Вес)) КАК Вес
		|ИЗ
		|	Документ.ЭлектроннаяНакладная КАК ЭлектроннаяНакладная
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗапросДоступностиТоваров.Упаковки КАК ЗапросДоступностиТоваровУпаковки
		|		ПО ЭлектроннаяНакладная.ДокументОснование = ЗапросДоступностиТоваровУпаковки.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектроннаяНакладная.Упаковки КАК ЭлектроннаяНакладнаяУпаковки
		|		ПО ЭлектроннаяНакладнаяУпаковки.Ссылка = ЭлектроннаяНакладная.Ссылка
		|ГДЕ
		|	ЭлектроннаяНакладная.Ссылка = &Ссылка";
...
Функция ПолучитьОбъем(ДокССылка)  
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Ширина, ЭлектроннаяНакладнаяУпаковки.Ширина) / 100 *
		|		ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Высота, ЭлектроннаяНакладнаяУпаковки.Высота) / 100 *
		|		ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.Глубина, ЭлектроннаяНакладнаяУпаковки.Глубина) / 100) КАК Объем
		|ИЗ
		|	Документ.ЭлектроннаяНакладная КАК ЭлектроннаяНакладная
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗапросДоступностиТоваров.Упаковки КАК ЗапросДоступностиТоваровУпаковки
		|		ПО ЭлектроннаяНакладная.ДокументОснование = ЗапросДоступностиТоваровУпаковки.Ссылка
...
Функция ПолучитьКоличествоМест(ДокССылка)  
	...
	Запрос.Текст = 
		...
		|	КОЛИЧЕСТВО(ЕСТЬNULL(ЗапросДоступностиТоваровУпаковки.НомерСтроки, ЗапросДоступностиТоваровУпаковки.НомерСтроки)) КАК
		|		Мест
		|ИЗ
		|	Документ.ЭлектроннаяНакладная КАК ЭлектроннаяНакладная
...
Процедура ПолучитьСтатусТТН(КлючAPI = Неопределено, НомерЭН = "", НоваяТТН = Ложь, Возвратная = Ложь) Экспорт
	...
	Запрос.Текст =
		...
		|			ПО (СтатусыЭНСрезПоследних.СтатусЭН = КодыСтатусовТТН.Ссылка)
	...
	Пока Выборка.Следующий() Цикл
		Если Не Выборка.СтатусЭН = СтатусТТН.СсылкаКодСтатуса Тогда
			...
			Попытка
				...
				НоваяЗапись.СтатусЭН   			= СтатусТТН.СсылкаКодСтатуса; 
				НаборЗаписей.Записать();
				...
				Если СтатусТТН.СтатусПосылки = Перечисления.СтатусыПосылокСлужбыДоставки.Отказ Тогда
							
					Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда

						Адрес = "im@antoshka.ua;";

						ПараметрыПисьма = Новый Структура;
						ПараметрыПисьма.Вставить("Кому", Адрес);
						ПараметрыПисьма.Вставить("Тема", "ЭН #"
										 + СокрЛП(Выборка.Номер)
										 + ", отказ от получения");
						ТелоПисьма = "Электронная накладная #"
							+ СокрЛП(Выборка.Номер)
							+ " по заказу #"
							+ СокрЛП(Выборка.НомерЗаказа)
							+ " на запрос "
							+ СокрЛП(Выборка.НомерЗапроса)
							+ ", была не забрана на почте. Клиент отказался или не пришёл.";
...
Процедура СделатьЗапиьВРССтатусТТН(ПараметрыДляЗаписи)
	...
	Иначе
		...
		ПолучитьСтатусТТН(ПараметрыДляЗаписи.КлючАПИ, ПараметрыДляЗаписи.IntDocNumber, Истина, ВозвратнаяНакладная);

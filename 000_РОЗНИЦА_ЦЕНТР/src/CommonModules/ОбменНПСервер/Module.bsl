...
Функция РегистрацияЕН(ДокССылка) Экспорт
	...
	СтруктураОтветаЭН = СтруктураОтветаПоЭНИнициализация(ДокССылка);
	...
	Если СтрЧислоВхождений(Ответтекст,"<success>true</success>") then 
		...
		Если Не ПустаяСтрока(СтруктураОтветаЭН.НомерТТН) Тогда
			...
			СтруктураОтветаЭН.ЭлектроннаяНакладная = СоздатьДокументЭН(СтруктураОтветаЭН);
...
Функция ЗапросДляПочты(ДокументЗапроса)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ЗапросДоступностиТоваров.ОплатаДоставки
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ОплатаДоставки,
		|	ЗапросДоступностиТоваров.ДокументОснование.СуммаДоставки
		|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.СтатусОплаты = Перечисления.СтатусОплаты.НеОплачен тогда
			...
			|	<AfterpaymentOnGoodsCost>"
				+ Формат(Выборка.СуммаДокумента + Выборка.СуммаДоставки * Выборка.ОплатаДоставки,"ЧГ=;")
				+"</AfterpaymentOnGoodsCost>";
		КонецЕсли;                   
		ФиоКлиента = "";
		...
		КонецЕсли;
		CargoType = "Parcel";
		СтоимостьЗаказа = Формат(Выборка.СуммаДокумента, "ЧГ=;");
		Мест = ПолучитьКоличествоМест(ДокСсылка);    

...
Процедура ОткрытьФормуЗаписиРегистраСведенийПоОтбору(
	...
	ОткрытьФорму(ПолноеИмяФормы, ПараметрыЗаписи, ФормаВладелец,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыполнитьОбменДаннымиОбработкаКоманды(УзелИнформационнойБазы, Владелец, ЗадатьВопрос = Истина, АдресДляВосстановленияПароляУчетнойЗаписи = "") Экспорт
	...
	ДополнительныеПараметры = Новый Структура(
		"Владелец, УзелИнформационнойБазы, АдресДляВосстановленияПароляУчетнойЗаписи, ЗакрытьФорму"
		, Владелец, УзелИнформационнойБазы, АдресДляВосстановленияПароляУчетнойЗаписи, НЕ ЗадатьВопрос = Истина);
	
	Если ЗадатьВопрос Тогда

		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьОбменДаннымиОбработкаКомандыЗавершение", ОбменДаннымиКлиент, ДополнительныеПараметры);
		ПоказатьВопрос(ОписаниеОповещения
			, "Синхронізувати дані з «" + СокрЛП(УзелИнформационнойБазы) + "»?"
	...
	Иначе
		...
		ВыполнитьОбменДаннымиОбработкаКомандыЗавершение(КодВозвратаДиалога.Да, ДополнительныеПараметры);

	КонецЕсли;

КонецПроцедуры

//	LNK 02.01.2018 14:18:46
Процедура ВыполнитьОбменДаннымиОбработкаКомандыЗавершение(КодВозврата, ДополнительныеПараметры)	Экспорт
	...
	Если КодВозврата = КодВозвратаДиалога.Да Тогда
		...
		ОткрытьФорму("Обработка.ВыполнениеОбменаДанными.Форма.Форма", ДополнительныеПараметры, Владелец, ДополнительныеПараметры.УзелИнформационнойБазы);
...
#Если ВебКлиент Тогда
	...
	ПоказатьПредупреждение(, НСтр("ru = 'В Веб-клиенте данная возможность не поддерживается'"));

...
//	Табличные части "ИмяТовары" и "ИмяОплата" связываются реквизитом "КлючСвязиБонусныхБаллов"
	...
	//2026-07-08 sa нам нужно не просто сумма документа, а сумма за вычетом услуг.(т.к. на услуги списание бонусов не действуют)
	//Кроме того есть благотворительные услуги типа Дети Героев, по которым списание бонусов не должно отрабатывать!
	//СуммаДокумента  = ТоварыДляОплаты.Итог("СуммаСНДС");
	
	//2026-07-21 sa решили, что списание бонусов на услуги таки действует. Добавили в карточку товара новый реквизит НеСписыватьБонусы. 
	//Теперь сумму документа доступную для списания определяем с учетом этого поля
	//ТЗТоварыБезУслуг = ТоварыДляОплаты.Скопировать(Новый Структура("ТипНоменклатуры",Перечисления.ТипыНоменклатуры.Товар));
	//СуммаДокумента = ТЗТоварыБезУслуг.Итог("СуммаСНДС");
	СуммаДокумента = 0;
	Для Каждого Строка_ Из ТоварыДляОплаты Цикл 
		Если Строка_.Номенклатура.НеСписыватьБонусы = Истина Тогда
			Продолжить;
		КонецЕсли;
		СуммаДокумента = СуммаДокумента + Строка_.СуммаСНДС;
	КонецЦикла;
...
//	Но ниже нужно будет уточнить эту сумму с учётом ограничения оплаты бонусами.
	...
	Если УточнитьСумму Тогда
        //2026-05-12 sa Нужно еще добавить проверку, если у клиента накоплено бонусов больше чем в чеке И Списываемая сумма бонусов больше или равно суммы в чеке, то ровняем.
		//Если ДанныеНакоплений.Программы.Итог("Сумма") >= СуммаДокумента Тогда
		Если ДанныеНакоплений.Программы.Итог("Сумма") >= СуммаДокумента И СуммаБонусныхБалловДляПогашения >= (СуммаДокумента - ТоварыДляОплаты.Количество()) * УчетнаяПолитика.ЦенаПродажиМинимальная
			Тогда

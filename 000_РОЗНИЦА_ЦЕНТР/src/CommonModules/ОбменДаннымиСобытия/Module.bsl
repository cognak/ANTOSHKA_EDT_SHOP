...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("Загрузка") ИЛИ ТехническаяПоддержкаВызовСервера.ИсключительныйРежим() Тогда
	...
	Если  Не Источник.ДополнительныеСвойства.Свойство("ПолучениеСообщенияОбмена")
		...
		И НЕ ТехническаяПоддержкаВызовСервера.ИсключительныйРежим() = Истина
		Тогда
...
//	LNK 19.09.2016 10:00:31
	Если НЕ Отказ И ОбменДаннымиПовтИсп.ЭтоТестовыйУзел() Тогда
		
		СписокУдалить = Новый Массив;

		Для каждого Получатель Из Объект.ОбменДанными.Получатели Цикл

			Если ОбменДаннымиПовтИсп.УзлыИсключить().Получить(Получатель) = Истина Тогда

				СписокУдалить.Добавить(Получатель);

			КонецЕсли;

		КонецЦикла;

		Для каждого Получатель Из СписокУдалить Цикл
...
Процедура ОпределитьМассивПолучателейПоНаборуЗаписей(МассивУзловРезультат,
	...
	//	определяем массив узлов для регистрации объекта "ПриОбработке"
		...
		//	определяем массив узлов для регистрации объекта "ПриОбработкеДополнительный"
		//	добавляем в ранее заполненный массив
			МассивУзловДополнительный = ОпределитьМассивУзловПоЗначениямСвойствДополнительный(ЗначенияСвойствОбъекта, ПРО_СтрокиНабора, ИмяПланаОбмена, СтрокаНабора);
			ОбщегоНазначения.ЗаполнитьМассивУникальнымиЗначениями(МассивУзлов, МассивУзловДополнительный);
...
Процедура ОпределитьМассивПолучателейПоНаборуЗаписей_ТИПОВОЙ(МассивУзловРезультат,
													НаборЗаписей,
													ПРО,
													ОбъектМетаданных,
													ИмяПланаОбмена,
...
//	LNK 31.03.2017 10:01:25
	Если ИспользоватьКэш = Истина И ТипЗнч(ЗначенияСвойств) = Тип("Структура") Тогда
...
Функция МассивУзловПоЗначениямСвойств(ЗначенияСвойств, Знач ТекстЗапроса, Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[ОбязательныеУсловия]",
				"И НЕ (ПланОбменаОсновнаяТаблица.ЭтотУзел ИЛИ ПланОбменаОсновнаяТаблица.ПометкаУдаления)
...
Функция ПолучитьЗначениеСвойстваОбъекта_ТИПОВОЙ(Объект, СтрокаСвойствОбъекта)
...
//	Значение получаем с учетом возможного разыменования свойства
	...
	Для Каждого ИмяСвойства ИЗ МассивПодстрок Цикл
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Значение)) Тогда
			...
			Значение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Значение, ИмяСвойства);

...
Процедура ВыполнитьОбменССайтом(Параметры, РезультатОбмена, ТаблицаИнформации) Экспорт
	...
	Иначе	ТаблицаКаталогов = Новый ТаблицаЗначений;
...
//
Функция ВыгрузитьДанныеНаСайт(Параметры, СтрокаТаблицыИнформации, ТипСоединения = Неопределено,ОписаниеОшибки="")
...
//Криворучко
Функция ВыгрузитьФайлНаСайт(Параметры, ИмяФайла, ИмяФайлаXML,ТипСоединения = Неопределено,ОписаниеОшибки="")
	...
	ИмяФайлаFTP="";
	Если ЗначениеЗаполнено(Параметры.КаталогВыгрузкиСайта) Тогда
		ИмяФайлаFTP="/"+СокрЛП(Параметры.КаталогВыгрузкиСайта)+"/"+ИмяФайлаXML;
	Иначе
		ИмяФайлаFTP = ИмяФайлаXML;		
	КонецЕсли;	
	
	Попытка
		Соединение.Записать(ИмяФайла,ИмяФайлаFTP);
	Исключение
		ШаблонСообщения = НСтр("ru = 'Ошибка передачи файла ");
				//ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИмяФайла);
				//ДобавитьОписаниеОшибки(ОписаниеОшибки, ШаблонСообщения);
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
			ОписаниеИсключительнойОшибки(НСтр("ru = 'Ошибка передачи файла'")
			+ " " + ИмяФайла + Символы.ПС + ОписаниеОшибки()));
				Успешно = Ложь;

	КонецПопытки;	
	//ОтветСервера = ОтправитьФайлНаСервер(ИмяФайла,
	//		Соединение,
	//		ОписаниеОшибки,
	//		АдресДляРаботы + НастройкиПодключения.ПараметрЗапросаHTTP_ПередачаФайла + ИмяФайла,
	//		ЗаголовкиЗапросов);
...
Процедура УдалитьПоляДругихНаборовДанных(ПоляНабораДанных, КомпоновщикНастройки)
	...
	Пока Счетчик < ПоляОтбора.Количество() Цикл
		...
		//+HVOYA.2016-09-26.Желиховская Юлия
...
Функция ВыполнитьОбменЗаказами(Параметры, ТаблицаИнформации)
	...
	Если Параметры.ВыгружатьНаСайт Тогда
		...
		Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() И Параметры.ОбменССайтомТолькоВГлавномУзле Тогда	//	LNK 26.12.2017 10:19:21

			УспешноЗагружено = ЗагрузитьЗаказыССайта(Параметры, ОписаниеОшибки);
			УспешноВыгружено = ВыгрузитьОтветНаСайт(Параметры, ОписаниеОшибки);
	...
	Возврат УспешноЗагружено И УспешноВыгружено;
	
КонецФункции

//Криворучко
Функция ВыгрузитьОтветНаСайт(Параметры, ОписаниеОшибки)
	
	ВыгрузкаУспешно = Ложь;
	
	Если ОбменДаннымиПовтИсп.ПроверитьСертификатИнформационнойБазы(Истина) = 1 Тогда	//	LNK 29.12.2017 10:34:14

		ИмяКаталога = КаталогВременныхФайлов();
		
		ИмяФайлаXML  = Строка(Формат(ТекущаяДата(),"ДФ=ddMMyyyyhhmmss"))+".xml";
		
		ИмяФайла = ИмяКаталога + ИмяФайлаXML;
		...
		КорневойЭлемент.УстановитьАтрибут("filedate",Строка(Формат(ТекущаяДата(),"ДЛФ=Д")));

		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Ведущая.Ссылка КАК ЧЕК,
		|	Ведущая.ЗаказПокупателя.Номер КАК НомерЗаказа,
		|	Ведущая.Дата КАК Дата,
		|	Ведущая.ЗаказПокупателя.Контрагент.IDN КАК Контрагент,
		|	Ведущая.ЗаказПокупателя.ТипОплаты КАК ТипОплатыЗаказ,
		|	Ведущая.Магазин.СкладПродажи.IDN КАК СкладIDN,
		|	Ведущая.Товары.(
		|		Ссылка КАК Ссылка,
		...
		|		ПричинаРучнойСкидки КАК ПричинаРучнойСкидки,
		|		АкционнаяЦена КАК АкционнаяЦена
		|	) КАК Товары,
		|	Ведущая.Оплата.(
		|		Ссылка КАК Ссылка,
		|		НомерСтроки КАК НомерСтроки,
		...
		|	) КАК Оплата,
		|	ЕСТЬNULL(УжеВыгружено.ИмяФайла, 0) КАК ИмяФайлаXML,
		|	Ведущая.СуммаДокумента КАК ЧекСумма
		|ИЗ
		|	Документ.ЧекККМ КАК Ведущая
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгруженыеДокументыНаСайт КАК УжеВыгружено
		|		ПО Ведущая.Ссылка = УжеВыгружено.ЧекККМДокумент
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитика.СрезПоследних КАК УчетнаяПолитика
		|		ПО (НЕ Ведущая.Магазин = УчетнаяПолитика.ИнтернетМагазин)
		|ГДЕ
		|	(Ведущая.СтатусЧекаККМ = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробитый)
		|			ИЛИ Ведущая.СтатусЧекаККМ = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Архивный))
		|	И Ведущая.ЗаказПокупателя <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)
		|	И Ведущая.Дата МЕЖДУ &ДатаНач И &ДатаКон
		|ИТОГИ
			...
			Если РезультатЗапроса.ИмяФайлаXML = 0 Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					Пока ВыборкаОплаты.Следующий() Цикл
						...
						Если ВыборкаОплаты.ВидОплаты = Справочники.ВидыОплатЧекаККМ.ОплатаПлатежнойКартой Тогда
							ТипОплаты = "БанковскаяКарта";
						ИначеЕсли ВыборкаОплаты.ВидОплаты = Справочники.ВидыОплатЧекаККМ.Наличные 
							или ВыборкаОплаты.ВидОплаты = Справочники.ВидыОплатЧекаККМ.ОплатаПодарочнымСертификатом тогда
							ТипОплаты = "Наличные";
						Иначе 	
							ТипОплаты = "БезналичныйРасчет";
						КонецЕсли;	
						//СтрокаОплаты.УстановитьАтрибут("ТипОплаты",XMLСтрока(ВыборкаОплаты.ВидОплаты.Наименование));
						//СтрокаОплаты.УстановитьАтрибут("ТипОплаты",XMLСтрока(Выборка.ТипОплатыЗаказ));
						СтрокаОплаты.УстановитьАтрибут("ТипОплаты",XMLСтрока(ТипОплаты));
						//СтрокаОплаты.УстановитьАтрибут("Сумма",XMLСтрока(ВыборкаОплаты.Сумма));
						СтрокаОплаты.УстановитьАтрибут("Сумма",XMLСтрока(Выборка.ЧекСумма));
						Прервать;
					...
					Пока ВыборкаТОВары.Следующий() Цикл
						...
						СтрокаТовар.УстановитьАтрибут("Склад", XMLСтрока(РезультатЗапроса.СкладIDN));
					//	СтрокаТовар.УстановитьАтрибут("Склад", XMLСтрока(ВыборкаТОВары.Склад.Наименование));
						...
						СтрокаТовар.УстановитьАтрибут("Наименование" , XMLСтрока(СокрЛП(ВыборкаТОВары.Номенклатура.Наименование)));
						СтрокаТовар.УстановитьАтрибут("Производитель", XMLСтрока(СокрЛП(ВыборкаТОВары.Номенклатура.Производитель.Наименование)));
		...
		Если КоличествоВыгруженных>0 Тогда
			Попытка
				...
				ЗаписьXML.Закрыть();
				ВыгрузкаУспешно= Истина;
			Исключение
				ДобавитьОписаниеОшибки(ОписаниеОшибки,
				ОписаниеИсключительнойОшибки(НСтр("ru = 'Не удалось записать XML-файл на диск:'")
				+ " " + ИмяФайла + Символы.ПС + ОписаниеОшибки()));
				ВыгрузкаУспешно= Ложь;
				Возврат ВыгрузкаУспешно;
			КонецПопытки;
		КонецЕсли;		
		
		Если Параметры.ВыгружатьНаСайт и КоличествоВыгруженных>0 Тогда
			
			//СтрокаТаблицыИнформации = ТаблицаИнформации.Добавить();
			ВыгрузкаУспешно = ВыгрузитьФайлНаСайт(Параметры, ИмяФайла,ИмяФайлаXML, "sale",ОписаниеОшибки);
...
Функция ЗагрузитьЗаказыССайта(Параметры, ОписаниеОшибки)
	...
	ИмяЛокальногоКаталога = РаботаСФайлами.ПроверитьСуществованиеКаталога(КаталогВременныхФайлов() + "WebBuyersOrder\");
	ИмяДляМагазинов       = "/magentotoshop/";

	ТаблицаФайлов = Новый ТаблицаЗначений;
	ТаблицаФайлов.Колонки.Добавить("НомерМагазина", Новый ОписаниеТипов("Строка"));
	ТаблицаФайлов.Колонки.Добавить("АрхивМагазина", Новый ОписаниеТипов("Строка"));
	ТаблицаФайлов.Колонки.Добавить("FTPФайл");//      , Новый ОписаниеТипов("FTPФайл"));
	ТаблицаФайлов.Колонки.Добавить("ФайлНаДиске"  , Новый ОписаниеТипов("Файл"));
	ТаблицаФайлов.Колонки.Добавить("ДатаВремя"    , Новый ОписаниеТипов("Дата"));
	ТаблицаФайлов.Колонки.Добавить("Принят"       , Новый ОписаниеТипов("Булево"));
	ТаблицаФайлов.Колонки.Добавить("АрхивЕсть"    , Новый ОписаниеТипов("Булево"));

	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел()  тогда

		Если Параметры.ОбменССайтомТолькоВГлавномУзле Тогда
			...
			|		ПО УчетнаяПолитика.ИнтернетМагазин = Магазины.Ссылка
			|ГДЕ
			|	НЕ(Магазины.ПометкаУдаления
			|				ИЛИ Магазины.Блокирован
			|				ИЛИ Магазины.СкладУправляющейСистемы
			...
			НомераВыборка = Запрос.Выполнить().Выбрать();

			Пока НомераВыборка.Следующий() Цикл

				Попытка

					НомерМагазина = Формат(НомераВыборка.НомерМагазина, "ЧН=0; ЧГ=");
					ДобавитьНайденныеФайлыВТаблицу(НомерМагазина, Соединение.НайтиФайлы(ИмяДляМагазинов + НомерМагазина, "*.xml"), ТаблицаФайлов);

				Исключение

				//	Не все номера магазинов должны присутствовать в виде каталогов.
				//	Так что здесь, скорее, отладочная строка.
	...
	Иначе
		...
		НомерМагазина = Формат(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбменДаннымиПовтИсп.ПолучитьДанныеУзла().Магазин, "НомерМагазина"), "ЧН=0; ЧГ=");
		ДобавитьНайденныеФайлыВТаблицу(НомерМагазина, Соединение.НайтиФайлы(ИмяДляМагазинов + НомерМагазина, "*.xml"), ТаблицаФайлов);

	КонецЕсли;

 	Если НЕ ТаблицаФайлов.Количество() = 0 Тогда

		Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл

		//	Загружаем только за последние трое суток, включая текущие.
		//	Остальные файлы уже не актуальны, как думается, - глубина резерва 3 суток.
			Если НачалоДня(СтрокаТаблицы.ДатаВремя) >= (НачалоДня(ТекущаяДатаСеанса()) - 86400 * 2) Тогда

			//	В процедуре проверки (создания) каталога все разделители "/" будут заменены на "\".
				СтрокаТаблицы.ФайлНаДиске = Новый Файл(РаботаСФайлами.ПроверитьСуществованиеКаталога(
						ИмяЛокальногоКаталога
							+ ?(ОбменДаннымиПовтИсп.ЭтоГлавныйУзел()
								, СтрокаТаблицы.FTPФайл.ПолноеИмя
								, СтрокаТаблицы.FTPФайл.Имя)
					, Истина));

				Если СтрокаТаблицы.ФайлНаДиске.Существует() Тогда

					УдалитьФайлы(СтрокаТаблицы.ФайлНаДиске.ПолноеИмя);

				КонецЕсли;

				Соединение.Получить(СтрокаТаблицы.FTPФайл.ПолноеИмя, СтрокаТаблицы.ФайлНаДиске.ПолноеИмя); 	

				Если СтрокаТаблицы.ФайлНаДиске.Существует() Тогда

					СтрокаТаблицы.Принят = Истина;
		...
		ТаблицаФайлов.Сортировать("Принят УБЫВ, НомерМагазина, ДатаВремя");

	Иначе	

		ТекстОшибки = НСтр("ru = 'Файлы на внешнем ресурсе не обнаружены!'");
		БылиОшибки  = Истина;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
		...
		Если СтрокаТаблицы.Принят Тогда

			ТекстовыйДокумент = Новый ТекстовыйДокумент;

			Попытка

				ТекстовыйДокумент.Прочитать(СтрокаТаблицы.ФайлНаДиске.ПолноеИмя, КодировкаТекста.UTF8);
				СтрокаXML = ТекстовыйДокумент.ПолучитьТекст();

				Если ПустаяСтрока(СтрокаXML) Тогда

					ДобавитьОписаниеОшибки(ОписаниеОшибки
					, НСтр("ru = 'Пропущено! Пустой файл «" + СтрокаТаблицы.FTPФайл.ПолноеИмя + "».'"));
					БылиОшибки = Истина;

					Продолжить;	//	LNK 26.12.2017 12:44:35
		...
		//	Попалась строка с "Принят=Ложь" - значит, дальше уже все такие будут.
		//	Выходим из цикла.

			Прервать;	//	LNK 26.12.2017 12:44:35

		КонецЕсли;

		НужноПереместить = Истина;

		Если НЕ ЗагрузитьЗаказы(СтрокаXML, Параметры, ОписаниеОшибки) Тогда 
			
			ДобавитьОписаниеОшибки(ОписаниеОшибки
			, НСтр("ru = 'Пропущено! Не удалось получить документ из файла «" + СтрокаТаблицы.FTPФайл.ПолноеИмя + "».'"));
			НужноПереместить = Ложь;
			БылиОшибки = Истина;
		...
		Если НужноПереместить Тогда
			...
			Если НЕ СтрокаТаблицы.АрхивЕсть Тогда

				Если Соединение.НайтиФайлы(СтрокаТаблицы.АрхивМагазина).Количество() = 0 Тогда

					Соединение.СоздатьКаталог(СтрокаТаблицы.АрхивМагазина);
					СтрокаТаблицы.АрхивЕсть = Истина;

				КонецЕсли;

			КонецЕсли;

			Соединение.Переместить(СтрокаТаблицы.FTPФайл.ПолноеИмя, СтрокаТаблицы.АрхивМагазина + СтрокаТаблицы.FTPФайл.Имя);
...
//	LNK 26.12.2017 13:15:15
Процедура ДобавитьНайденныеФайлыВТаблицу(НомерМагазина, НайденныеФайлы, ТаблицаФайлов)

	Для каждого FTPФайл Из НайденныеФайлы Цикл

		СтрокаТаблицы = ТаблицаФайлов.Добавить();
		СтрокаТаблицы.НомерМагазина = НомерМагазина;
		СтрокаТаблицы.FTPФайл       = FTPФайл;
		СтрокаТаблицы.ДатаВремя     = СтрокаТаблицы.FTPФайл.ПолучитьВремяИзменения();

		СтрокаТаблицы.АрхивМагазина = "/processed/" + СтрокаТаблицы.НомерМагазина + "/";
		СтрокаТаблицы.АрхивЕсть     = Ложь;
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, Параметры, ОписаниеОшибки) Экспорт
	...
	Для каждого ДокументXDTO Из МассивЗаказовXDTO Цикл 
		...
		НоваяСтрока.НомерЗаказа = ДокументXDTO.НомерЗаказа;

		Попытка
			
			//Криворучко //Добавим явное преобразование в дату
			НоваяСтрока.ДатаЗаказа = Дата(ДокументXDTO.Дата);	
		
		Исключение

			ОписаниеОшибки = ОписаниеОшибки();
			ЗаписьЖурналаРегистрации("ОбменССайтом.Загрузка", УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки);
	...
	ОбменССайтомПереопределяемый.ПолучитьРанееЗагруженныеДокументы(Параметры.ПрикладныеПараметры, НомераИДатыЗаказов, РанееЗагруженныеЗаказы);

	Параметры.Вставить("УчетнаяПолитика", ОбщегоНазначенияРТ.ПолучитьУчетнуюПолитику());
	...
	Исключение
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ Успешно Тогда
		...
		Если НЕ ПустаяСтрока(ОписаниеОшибки) Тогда
			...
			ЖурналСобытий.Регистрация("ИНТЕРНЕТЗАКАЗ.ИТОГ", УровеньЖурналаРегистрации.Ошибка
...
Процедура ИнициализироватьСтруктуруРеквизитов(РеквизитыЗаказа)
	...
	РеквизитыЗаказа.Вставить("ДатаЗаказаНаСайте",			 Неопределено);
	РеквизитыЗаказа.Вставить("ДатаОтгрузки",				 Неопределено);
	РеквизитыЗаказа.Вставить("ДатаРезерва",					 Неопределено);
	...
	РеквизитыЗаказа.Вставить("Магазин",	 					 Неопределено);	//	LNK 15.01.2018 08:43:54
	РеквизитыЗаказа.Вставить("Организация",					 Неопределено);	//	LNK 15.01.2018 08:43:54
	РеквизитыЗаказа.Вставить("МагазинНеУказан",				 Ложь);			//	LNK 06.01.2021 11:28:52
	...
	РеквизитыЗаказа.Вставить("ТипДоставки", 				 Неопределено);
	РеквизитыЗаказа.Вставить("ТипОплаты", 				     Неопределено);
	РеквизитыЗаказа.Вставить("СтатусОплаты", 			     Неопределено);	
	РеквизитыЗаказа.Вставить("СуммаОплаты", 			     Неопределено);
	РеквизитыЗаказа.Вставить("СуммаДоставкиОплачена",	     Неопределено);	//	LNK 11.06.2020 07:10:05
	РеквизитыЗаказа.Вставить("СуммаДоставки",			     Неопределено);	//	LNK 11.06.2020 07:53:45
	РеквизитыЗаказа.Вставить("СуммаДоставкиОператор",	     Неопределено);	//	LNK 11.06.2020 07:53:45
	//Криворучко
	РеквизитыЗаказа.Вставить("Адрес", 					     Неопределено);
	РеквизитыЗаказа.Вставить("ДисконтнаяКарта", 		     Неопределено);
//	LNK 11.12.2017 14:24:16
	РеквизитыЗаказа.Вставить("ИНН");
//	LNK 24.10.2018 10:23:07
	РеквизитыЗаказа.Вставить("РеквизитыПокупателя",          Новый Структура);
//	LNK 04.01.2021 13:10:33	
	РеквизитыЗаказа.Вставить("УчтеноNavision", Ложь);
...
Функция СоздатьОбновитьЗаказы(ДокументыXDTO, РанееЗагруженныеЗаказы, Параметры, ОписаниеОшибки)
	...
	Для Каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если ОбъектXDTOСодержитСвойство(ДокументXDTO, "ЗначенияРеквизитов")
			И ДокументXDTO.ЗначенияРеквизитов <> Неопределено	Тогда
			
			Если ОбъектXDTOСодержитСвойство(ДокументXDTO.ЗначенияРеквизитов, "ЗначениеРеквизита") Тогда
			...
			Иначе
				...
				Для каждого КлючЗначение Из СтруктураЗначенийРеквизитов() Цикл

					Если ОбъектXDTOСодержитСвойство(ДокументXDTO.ЗначенияРеквизитов, КлючЗначение.Ключ) Тогда

						Если КлючЗначение.Значение = Новый ОписаниеТипов("Булево") Тогда

							СвойстваЗаказа.Вставить(КлючЗначение.Ключ, ?(ДокументXDTO.ЗначенияРеквизитов[КлючЗначение.Ключ] = "1" ИЛИ ДокументXDTO.ЗначенияРеквизитов[КлючЗначение.Ключ] = 1, Истина, Ложь));

						Иначе

							СвойстваЗаказа.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение.ПривестиЗначение(ДокументXDTO.ЗначенияРеквизитов[КлючЗначение.Ключ]));

							Если ТипЗнч(СвойстваЗаказа[КлючЗначение.Ключ]) = Тип("Строка") И СвойстваЗаказа[КлючЗначение.Ключ] = "ОбъектXDTO" Тогда

								СвойстваЗаказа[КлючЗначение.Ключ] = "";
		...
		ИзвлечьСвойстваЗаказаИзДанныхДокументаXDTO(СтруктураРеквизитыЗаказа, ДокументXDTO, СвойстваЗаказа, Параметры, Успешно, ОписаниеОшибки);
		...
		// Поиск должен осуществляться по номеру и магазину!
		Для Каждого Стр Из РанееЗагруженныеЗаказы Цикл
			Стр.НомерЗаказаНаСайте = СокрЛП(Стр.НомерЗаказаНаСайте);	
		КонецЦикла;     	
		
		НайденныеСтроки = РанееЗагруженныеЗаказы.НайтиСтроки(
			Новый Структура(
				"НомерЗаказаНаСайте, Магазин"
				, ДокументXDTO.НомерЗаказа
				, СтруктураРеквизитыЗаказа.Магазин));

		Если НЕ НайденныеСтроки.Количество() = 0 Тогда
			
			ПолученФлагУчётаNavision = НЕ НайденныеСтроки[0].УчтеноNavision И СтруктураРеквизитыЗаказа.УчтеноNavision;

			Если НЕ ПолученФлагУчётаNavision Тогда

				ЗаказЗакрытОтменен = НайденныеСтроки[0].НеОбновлятьЗаказ;
				
				Если ЗаказЗакрытОтменен Тогда
					
					СтруктураСтатистики.Пропущено.Добавить(НайденныеСтроки[0].ЗаказПокупателя);
					ТекстОшибки = Строка(НайденныеСтроки[0].ЗаказПокупателя)+" " + НСтр("ru = 'пропущен по причине:'") + Символы.ПС 
			...
			ДокументОбъект = НайденныеСтроки[0].ЗаказПокупателя.ПолучитьОбъект();
	...
	//	заполняем документ заказ
		...
		ДокументОбъект.ДополнительныеСвойства.Вставить("РеквизитыПокупателя", СтруктураРеквизитыЗаказа.РеквизитыПокупателя);
		ЗаписатьДокумент(ДокументОбъект, ОписаниеОшибки);
...
//	LNK 15.01.2018 08:51:59
Функция ОпределитьРеквизитыАдресации(ДокументXDTO, СтруктураРеквизитыЗаказа, ОписаниеОшибки)

	Если ОбъектXDTOСодержитСвойство(ДокументXDTO, "Магазин") Тогда

		Если ТипЗнч(ДокументXDTO.Магазин) = Тип("ОбъектXDTO") Тогда

			ШаблонСообщения = НСтр("ru = 'Не заполнены данные адресации для номера %1: реквизит %2'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДокументXDTO.НомерЗаказа, "Магазин");
		...
		Иначе	
			...
			Запрос.УстановитьПараметр("IDN", ДокументXDTO.Магазин);

			Выборка = Запрос.Выполнить().Выбрать();
			
			Если Выборка.Следующий() Тогда

			    СтруктураРеквизитыЗаказа.Склад       = Выборка.Склад;	
			    СтруктураРеквизитыЗаказа.Магазин     = Выборка.Магазин;	
			    СтруктураРеквизитыЗаказа.Организация = Выборка.Организация;

			Иначе

				ШаблонСообщения = НСтр("ru = 'Нет соответствия адресации для номера %1: реквизит %2'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДокументXDTO.НомерЗаказа, "Магазин");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru = 'Нет определения адресации для номера %1: реквизит %2'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДокументXDTO.НомерЗаказа, "Магазин");

	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитыЗаказа.Магазин) Тогда	//	LNK 06.01.2021 10:25:18

		УчетнаяПолитика = ОбщегоНазначенияРТ.ПолучитьУчетнуюПолитику();

		СтруктураРеквизитыЗаказа.Склад       = УчетнаяПолитика.СкладПродажиИнтернетМагазина;
		СтруктураРеквизитыЗаказа.Магазин     = УчетнаяПолитика.ИнтернетМагазин;
		СтруктураРеквизитыЗаказа.Организация = УчетнаяПолитика.ОрганизацияИнтернетМагазина;
		СтруктураРеквизитыЗаказа.МагазинНеУказан = Истина;
	...
	Возврат ЗначениеЗаполнено(СтруктураРеквизитыЗаказа.Магазин);

КонецФункции

Процедура ИзвлечьСвойстваЗаказаИзДанныхДокументаXDTO(СтруктураРеквизитыЗаказа, ДокументXDTO, СвойстваЗаказа, Параметры, Успешно, ОписаниеОшибки)

	Если НЕ ОпределитьРеквизитыАдресации(ДокументXDTO, СтруктураРеквизитыЗаказа, ОписаниеОшибки) Тогда
	...
	Иначе
		...
		СтруктураРеквизитыЗаказа.Номер = СокрЛП(ДокументXDTO.НомерЗаказа); //сменила Номер на Ид
		СтруктураРеквизитыЗаказа.Ид	   = СокрЛП(ДокументXDTO.НомерЗаказа);
		СтруктураРеквизитыЗаказа.Дата  = ДатаВремяИзСтроки(ДокументXDTO.ДатаРезерва);
		СтруктураРеквизитыЗаказа.ДатаЗаказаНаСайте = ДатаВремяИзСтроки(ДокументXDTO.Дата, ВремяСтрока);
		СтруктураРеквизитыЗаказа.ДатаОтгрузки	   = ДатаОтгрузкиИзСвойствЗаказа(СвойстваЗаказа);
			
		//Контрагент = Неопределено;
		РеквизитыПокупателя = Новый Структура;
	//	В случае, если покупатель не указан, принимаем "розничного" - а что делать?
		РеквизитыПокупателя.Вставить("Идентифицирован", Ложь);
		РеквизитыПокупателя.Вставить("Контрагент"     , Справочники.Контрагенты.РозничныйПокупатель);
		РеквизитыПокупателя.Вставить("ДисконтнаяКарта", Справочники.ИнформационныеКарты.ПустаяСсылка());

		РеквизитыПокупателя.Вставить("IDN"            , "");
		РеквизитыПокупателя.Вставить("Фамилия"        , "");
		РеквизитыПокупателя.Вставить("Имя"            , "");
		РеквизитыПокупателя.Вставить("Отчество"       , "");

		РеквизитыПокупателя.Вставить("КодКарты"     , "");
		РеквизитыПокупателя.Вставить("НомерТелефона", "");
		РеквизитыПокупателя.Вставить("Электропочта" , "");

		РеквизитыПокупателя.Вставить("Область"       , "");
		РеквизитыПокупателя.Вставить("Город"         , ""); 
		РеквизитыПокупателя.Вставить("Адрес"		 , "");				
		РеквизитыПокупателя.Вставить("НомерОтделения", "");		

		Если ТипЗнч(ДокументXDTO.Контрагент) = Тип("ОбъектXDTO") Тогда

			ПолучитьКонтрагентаЗаказа(ДокументXDTO, РеквизитыПокупателя, Параметры);

		КонецЕсли;

		СтруктураРеквизитыЗаказа.Контрагент      = РеквизитыПокупателя.Контрагент;
		СтруктураРеквизитыЗаказа.ДисконтнаяКарта = РеквизитыПокупателя.ДисконтнаяКарта;
		СтруктураРеквизитыЗаказа.РеквизитыПокупателя = РеквизитыПокупателя;

		СтруктураРеквизитыЗаказа.СуммаДоставкиОплачена = СвойстваЗаказа["ОплатаДоставкиКартой"];	//	LNK 11.06.2020 07:22:09
		СтруктураРеквизитыЗаказа.СуммаДоставки         = СвойстваЗаказа["Сумма_оплаты_клиентом"];	//	LNK 15.06.2020 07:21:34
		СтруктураРеквизитыЗаказа.СуммаДоставкиОператор = СвойстваЗаказа["Фактическая_сумма"];		//	LNK 15.06.2020 07:21:16

		СтруктураРеквизитыЗаказа.УчтеноNavision = СвойстваЗаказа["УчтенNavision"] = 1;

	//	LNK 11.12.2017 14:25:51
		Если ОбъектXDTOСодержитСвойство(ДокументXDTO, "ИНН") И ТипЗнч(ДокументXDTO.ИНН) = Тип("Строка") Тогда

			СтруктураРеквизитыЗаказа.ИНН = СокрЛП(ДокументXDTO.ИНН);
		...
		Иначе
			...
			СтруктураРеквизитыЗаказа.ТочкаСамовывоза = ДокументXDTO.Магазин;
			...
			Если  ДокументXDTO.ЗначенияРеквизитов.ТипДоставки = "Самовывоз" Тогда
				СтруктураРеквизитыЗаказа.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз;
			ИначеЕсли ДокументXDTO.ЗначенияРеквизитов.ТипДоставки = "Новая Почта" Тогда
				СтруктураРеквизитыЗаказа.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.НоваяПочтаДоставкаВОтделение;
				СтруктураРеквизитыЗаказа.Адрес = ДокументXDTO.ЗначенияРеквизитов.Адрес_доставки;
			КонецЕсли;
			//Если  ДокументXDTO.ЗначенияРеквизитов.ТипДоставки = "Самовывоз" Тогда
			//	СтруктураРеквизитыЗаказа.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз;
			//ИначеЕсли ДокументXDTO.ЗначенияРеквизитов.ТипДоставки = "НП-курьер" Тогда
			//	СтруктураРеквизитыЗаказа.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.НоваяПочтаДоставкаВОтделение;
			//	СтруктураРеквизитыЗаказа.Адрес = ДокументXDTO.ЗначенияРеквизитов.Адрес_доставки;   
			//	СтруктураРеквизитыЗаказа.Город = ДокументXDTO.ЗначенияРеквизитов.Адрес_доставки;
			//ИначеЕсли ДокументXDTO.ЗначенияРеквизитов.ТипДоставки = "НП-отделение" Тогда
			//	СтруктураРеквизитыЗаказа.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.НоваяПочтаДоставкаВОтделение;
			//	СтруктураРеквизитыЗаказа.Адрес = ДокументXDTO.ЗначенияРеквизитов.Адрес_доставки;   
			//	СтруктураРеквизитыЗаказа.Адрес = ДокументXDTO.ЗначенияРеквизитов.Адрес_доставки;
			//	СтруктураРеквизитыЗаказа.Адрес = ДокументXDTO.ЗначенияРеквизитов.Адрес_доставки;								
			...
			Если  ДокументXDTO.ИнформацияОбОплате.ТипОплаты = "Безналичный расчет" Тогда
				СтруктураРеквизитыЗаказа.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.БезналичныйРасчет;
			ИначеЕсли ДокументXDTO.ИнформацияОбОплате.ТипОплаты = "Оплата картой ПриватБанк (LiqPay)" Тогда
				СтруктураРеквизитыЗаказа.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.ОплатаКартойПриват;
			ИначеЕсли ДокументXDTO.ИнформацияОбОплате.ТипОплаты = "Наличными" Тогда
				СтруктураРеквизитыЗаказа.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.Наличные;
			ИначеЕсли ДокументXDTO.ИнформацияОбОплате.ТипОплаты = "Банковской картой" Тогда
				СтруктураРеквизитыЗаказа.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.БанковскаяКарта;
			КонецЕсли;
			
			Если ДокументXDTO.ИнформацияОбОплате.СтатусОплаты = "Оплачен" Тогда
				СтруктураРеквизитыЗаказа.СтатусОплаты = Перечисления.СтатусОплаты.Оплачен;
			ИначеЕсли ДокументXDTO.ИнформацияОбОплате.СтатусОплаты = "Не оплачен" Тогда
				СтруктураРеквизитыЗаказа.СтатусОплаты = Перечисления.СтатусОплаты.НеОплачен;
			КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ОписаниеОшибки) Тогда
		...
		ЖурналСобытий.Регистрация("ИНТЕРНЕТЗАКАЗ.ДАННЫЕ", ?(Успешно = Ложь, УровеньЖурналаРегистрации.Ошибка, УровеньЖурналаРегистрации.Примечание)
			, Метаданные.Документы.ЗаказПокупателя
			,
			,
			, ОписаниеОшибки
			, СтруктураРеквизитыЗаказа.Номер + " от " + СтруктураРеквизитыЗаказа.Дата
			, Истина
			, Ложь
		);
...
Функция ПривестиТекст(Значение)

	СписокЛексем = Новый Массив;
	СписокЛексем.Добавить("null");
	СписокЛексем.Добавить("NULL");
	СписокЛексем.Добавить("ОбъектXDTO");

	ЗначениеПриведенное = Значение;

	Для каждого Лексема Из СписокЛексем Цикл

		ЗначениеПриведенное = СтрЗаменить(ЗначениеПриведенное, Лексема, "");

	КонецЦикла;

	Возврат СокрЛП(ЗначениеПриведенное);

КонецФункции

//	LNK 24.10.2018 09:27:59
Процедура ПолучитьКонтрагентаЗаказа(ДокументXDTO, РеквизитыПокупателя, Параметры)

	РеквизитыПокупателя.IDN = "i" + ПривестиТекст(ДокументXDTO.Контрагент.user_id);
	РеквизитыПокупателя.IDN = ?(РеквизитыПокупателя.IDN = "i", "", РеквизитыПокупателя.IDN);

	РеквизитыПокупателя.Фамилия  = ПривестиТекст(ДокументXDTO.Контрагент.Фамилия);
	РеквизитыПокупателя.Имя      = ПривестиТекст(ДокументXDTO.Контрагент.Имя);
	РеквизитыПокупателя.Отчество = ПривестиТекст(ДокументXDTO.Контрагент.Отчество);

	Если ОбъектXDTOСодержитСвойство(ДокументXDTO.Контрагент, "НомерТелефона") И ТипЗнч(ДокументXDTO.Контрагент.НомерТелефона) = Тип("Строка") Тогда

		РеквизитыПокупателя.НомерТелефона = СокрЛП(ДокументXDTO.Контрагент.НомерТелефона);

	КонецЕсли;

	Если ОбъектXDTOСодержитСвойство(ДокументXDTO.Контрагент, "ДисконтКарта") И ТипЗнч(ДокументXDTO.Контрагент.ДисконтКарта) = Тип("Строка") Тогда

		РеквизитыПокупателя.КодКарты = ПривестиТекст(ДокументXDTO.Контрагент.ДисконтКарта);

	КонецЕсли;

	Если ОбъектXDTOСодержитСвойство(ДокументXDTO.Контрагент, "АдресЭП") И ТипЗнч(ДокументXDTO.Контрагент.АдресЭП) = Тип("Строка") И НЕ Найти(ДокументXDTO.Контрагент.АдресЭП, "@") = 0 Тогда

		РеквизитыПокупателя.Электропочта = СокрЛП(ДокументXDTO.Контрагент.АдресЭП);

	КонецЕсли;

	Если ОбъектXDTOСодержитСвойство(ДокументXDTO.Контрагент, "Область") И ТипЗнч(ДокументXDTO.Контрагент.Область) = Тип("Строка") Тогда

		РеквизитыПокупателя.Область = ПривестиТекст(ДокументXDTO.Контрагент.Область);

	КонецЕсли;

	Если ОбъектXDTOСодержитСвойство(ДокументXDTO.Контрагент, "Город") И ТипЗнч(ДокументXDTO.Контрагент.Город) = Тип("Строка") Тогда

		РеквизитыПокупателя.Город = ПривестиТекст(ДокументXDTO.Контрагент.Город);

	КонецЕсли;

	Если НЕ НайтиКонтрагента(РеквизитыПокупателя) И ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда

		СоздатьКонтрагента(РеквизитыПокупателя, Параметры);

	КонецЕсли;

КонецПроцедуры

//	LNK 24.10.2018 11:08:27
Функция НайтиКонтрагента(РеквизитыПокупателя)	Экспорт

	Если НЕ ПустаяСтрока(РеквизитыПокупателя.КодКарты) Тогда
		...
		|	И НЕ ИнформационныеКарты.ВладелецКарты = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)"
		);
		Запрос.УстановитьПараметр("КодКарты", РеквизитыПокупателя.КодКарты);

		Выборка = Запрос.Выполнить().Выбрать();
		
		Если Выборка.Следующий() Тогда
			
			РеквизитыПокупателя.Контрагент      = Выборка.Контрагент;
			РеквизитыПокупателя.ДисконтнаяКарта = Выборка.ДисконтнаяКарта;

			РеквизитыПокупателя.Идентифицирован = Истина;
			
		КонецЕсли; 		

	КонецЕсли;

	Если НЕ РеквизитыПокупателя.Идентифицирован И НЕ ПустаяСтрока(РеквизитыПокупателя.НомерТелефона) Тогда

		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ТаблицаКонтакты.Ссылка КАК Контрагент,
		|	ЕСТЬNULL(ИнформационныеКарты.Ссылка, ЗНАЧЕНИЕ(Справочник.ИнформационныеКарты.ПустаяСсылка)) КАК ДисконтнаяКарта
		|ИЗ
		...
		|		ПО (ИнформационныеКарты.ВладелецКарты ССЫЛКА Справочник.Контрагенты)
		|			И ТаблицаКонтакты.Ссылка = ИнформационныеКарты.ВладелецКарты
		|			И (НЕ(ИнформационныеКарты.Блокирован
		|					ИЛИ ИнформационныеКарты.ПометкаУдаления))
		|ГДЕ
		|	(ТаблицаКонтакты.НомерТелефона В (&СписокНомеров)
		|			ИЛИ ТаблицаКонтакты.Представление В (&СписокНомеров)
		|			ИЛИ ТаблицаКонтакты.НомерТелефона В (&СписокНомеров))
		|	И ТаблицаКонтакты.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.Телефон)"
		);
		Запрос.УстановитьПараметр("СписокНомеров", КонтактнаяИнформацияСлужебный.ПолучитьВариантыНомераТелефона(РеквизитыПокупателя.НомерТелефона));
	...
	Если НЕ РеквизитыПокупателя.Идентифицирован И НЕ ПустаяСтрока(РеквизитыПокупателя.IDN) Тогда

	//	Хоть такой запрос и быстрее (наверное), но выполняем его вторым, так как в случае передачи
	//	номера карты дисконта, её нужно найти в любом случае.
	//	Так же поиск уточняем по префиксу кода "ИМ".. на всякий случай.
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ИнформационныеКарты КАК ИнформационныеКарты
		|		ПО Контрагенты.Ссылка = ИнформационныеКарты.ВладелецКарты
		|			И (НЕ(ИнформационныеКарты.Блокирован
		|					ИЛИ ИнформационныеКарты.ПометкаУдаления))
		|ГДЕ
		|	Контрагенты.IDN = &IDN
		|	И (ВЫРАЗИТЬ(Контрагенты.Код КАК СТРОКА(2))) = ""ИМ"""
		);
		Запрос.УстановитьПараметр("IDN", РеквизитыПокупателя.IDN);
	...
	Если НЕ РеквизитыПокупателя.Идентифицирован И НЕ ПустаяСтрока(РеквизитыПокупателя.Электропочта) Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ИнформационныеКарты КАК ИнформационныеКарты
		|		ПО КонтрагентыКонтактнаяИнформация.Ссылка = ИнформационныеКарты.ВладелецКарты
		|			И (НЕ(ИнформационныеКарты.Блокирован
		|					ИЛИ ИнформационныеКарты.ПометкаУдаления))
		|ГДЕ
		|	КонтрагентыКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты)
		|	И (КонтрагентыКонтактнаяИнформация.АдресЭП = &Электропочта
		|			ИЛИ КонтрагентыКонтактнаяИнформация.Представление = &Электропочта)"
		);
		Запрос.УстановитьПараметр("Электропочта", РеквизитыПокупателя.Электропочта);
	...
	Возврат РеквизитыПокупателя.Идентифицирован;

КонецФункции

//	LNK 24.10.2018 10:42:00
Процедура СоздатьКонтрагента(РеквизитыПокупателя, Параметры)	Экспорт
	...
	КонтрагентОбъект.IDN        = РеквизитыПокупателя.IDN;

	СтроковыеФункцииКлиентСервер.ПривестиНаименование(РеквизитыПокупателя.Фамилия);
	СтроковыеФункцииКлиентСервер.ПривестиНаименование(РеквизитыПокупателя.Имя);
	СтроковыеФункцииКлиентСервер.ПривестиНаименование(РеквизитыПокупателя.Отчество);

    КонтрагентОбъект.Наименование = СокрЛП(РеквизитыПокупателя.Фамилия + " " + РеквизитыПокупателя.Имя + " " + РеквизитыПокупателя.Отчество);
	...
	Если НРег(Прав(РеквизитыПокупателя.Имя, 1)) = "а" ИЛИ НРег(Прав(РеквизитыПокупателя.Имя, 1)) = "я" Тогда

			КонтрагентОбъект.Пол = Перечисления.ПолФизическогоЛица.Женский;

	Иначе	КонтрагентОбъект.Пол = Перечисления.ПолФизическогоЛица.Мужской;

	КонецЕсли;

	Если НЕ ПустаяСтрока(РеквизитыПокупателя.Электропочта) Тогда
		...
		СтрокаКонтакта.Представление = СокрЛП(РеквизитыПокупателя.Электропочта);
		СтрокаКонтакта.АдресЭП       = СокрЛП(РеквизитыПокупателя.Электропочта);

	КонецЕсли;

	Если НЕ ПустаяСтрока(РеквизитыПокупателя.Область) ИЛИ НЕ ПустаяСтрока(РеквизитыПокупателя.Город) Тогда

		ДанныеАдреса = Новый Структура(
			"Индекс, Область, Город, Район, Улица, Дом, Корпус, Квартира"
			, ""
			, РеквизитыПокупателя.Область
			, РеквизитыПокупателя.Город
			, "", "", "", "", ""
		);
		...
		СтрокаКонтакта.Область = РеквизитыПокупателя.Область;
		СтрокаКонтакта.Город   = РеквизитыПокупателя.Город;
		СтрокаКонтакта.ЗначенияПолей = КонтактнаяИнформацияСлужебный.СформироватьДанныеАдреса(ДанныеАдреса, СтрокаКонтакта.Представление);

	КонецЕсли;

	Если НЕ ПустаяСтрока(РеквизитыПокупателя.НомерТелефона) Тогда
		...
		СтрокаКонтакта.Представление = РеквизитыПокупателя.НомерТелефона;
	...
	Попытка
		...
		ИнформационныеКартыСлужебный.ПрисвоитьРезервнуюВиртуальнуюКарту(КонтрагентОбъект.ПолучитьСсылкуНового()
			, Параметры.УчетнаяПолитика.ИнтернетМагазин
			, РеквизитыПокупателя.НомерТелефона
			,
			, Ложь
		);
		...
		МенеджерЗаписи.Фамилия  = РеквизитыПокупателя.Фамилия;
		МенеджерЗаписи.Имя      = РеквизитыПокупателя.Имя;
		МенеджерЗаписи.Отчество = РеквизитыПокупателя.Отчество;
	...
	//	---------------------------------------------------------------------------------------
		РеквизитыПокупателя.Контрагент = КонтрагентОбъект.Ссылка;
...
Процедура ЗаполнитьДанныеКонтрагента(ДанныеКонтрагента, ДокументXDTO)
	...
	Если ТипЗнч(ДокументXDTO.Контрагент) = Тип("ОбъектXDTO") Тогда
		КонтрагентXDTO = ДокументXDTO.Контрагент;
	Иначе
		КонтрагентXDTO = ДокументXDTO.Контрагент;
	КонецЕсли;
...
Процедура ЗаписатьДокумент(ДокументОбъект, ОписаниеОшибки = "")
	...
	Если ДокументОбъект.ЭтоНовый() ИЛИ ДокументОбъект.Проведен Тогда
		...
		Исключение;
			...
			Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
				...
				ЗаписьЖурналаРегистрации("ОбменССайтом.ЗаказыПокупателей", УровеньЖурналаРегистрации.Ошибка
					, ДокументОбъект.Метаданные(),, ТекстОшибки);
...
Функция ДатаОтгрузкиИзСвойствЗаказа(СвойстваЗаказа)
	...
	ДатаЗначение = СвойстваЗаказа.Получить("ДатаОтгрузки");

	Если НЕ ДатаЗначение = Неопределено	Тогда
		
		Если ТипЗнч(ДатаЗначение) = Тип("Строка") Тогда

			ДатаСтрока = Лев(ДатаЗначение, 10);
			ВремяСтрока = Сред(ДатаЗначение, 12);

			ДатаОтгрузки = ДатаВремяИзСтроки(ДатаСтрока, ВремяСтрока);

		ИначеЕсли ТипЗнч(ДатаЗначение) = Тип("Дата") Тогда

			ДатаОтгрузки = ДатаЗначение;
...
//
Функция ИдентифицироватьНоменклатуру(РеквизитыЗаказа, ДокументXDTO, Параметры, ОписаниеОшибки)	//	LNK 15.01.2018 11:57:41

	Успешно = Истина;

	РеквизитыЗаказа.Товары = ИнициализироватьТаблицуЗапасов();
	...
	Если НЕ ОбъектXDTOСодержитСвойство(ДокументXDTO, "Товары")
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'Табличная часть «Товары» НЕ заполнена!'"));
		Возврат Успешно;
...
//	Предварительное формирование таблицы товаров:

	Для каждого ТоварXDTO Из МассивТоваровXDTO Цикл

		НомерСтроки = НомерСтроки + 1;

		Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Код") И ОбъектXDTOСодержитСвойство(ТоварXDTO, "Количество") Тогда

			СтрокаТаблицы = РеквизитыЗаказа.Товары.Добавить();
			СтрокаТаблицы.НомерСтроки = НомерСтроки;
			СтрокаТаблицы.IDN = СокрЛП(ТоварXDTO.Код);

			Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "warehouse") Тогда

				СтрокаТаблицы.IDNСклада = СокрЛП(ТоварXDTO.warehouse);

			КонецЕсли;
			
			СтрокаТаблицы.Количество = ЧислоИзСтроки(ТоварXDTO.Количество);
			СтрокаТаблицы.Резерв     = СтрокаТаблицы.Количество;

			Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Резерв") Тогда

				СтрокаТаблицы.Резерв = Мин(ЧислоИзСтроки(ТоварXDTO.Резерв), СтрокаТаблицы.Резерв);

			КонецЕсли;

			Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "ЦенаЗаЕдиницу") Тогда

				СтрокаТаблицы.Цена = ЧислоИзСтроки(ТоварXDTO.ЦенаЗаЕдиницу);

			КонецЕсли;

			Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Сумма") Тогда

				СтрокаТаблицы.Сумма = ЧислоИзСтроки(ТоварXDTO.Сумма);

			КонецЕсли;

		Иначе

			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'в строке " + Формат(НомерСтроки, "ЧДЦ=; ЧГ=") + " не указан IDN! Пропущено.'"));

		КонецЕсли;

	КонецЦикла;

//	Идентификация ссылочных объектов:

	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Товары.НомерСтроки КАК НомерСтроки,
	|	Товары.IDN КАК IDN,
	|	Товары.IDNСклада КАК IDNСклада
	|ПОМЕСТИТЬ Источник
	|ИЗ
	|	&Товары КАК Товары
	|
	...
	|		ПО (Источник.IDNСклада = ТаблицаСправочник.IDN
	|				И ТаблицаСправочник.НазначениеСклада = ЗНАЧЕНИЕ(Перечисление.НазначенияСкладов.ТорговыйЗал))
	|ГДЕ
	|	НЕ Источник.IDNСклада = """""
	);
	Запрос.УстановитьПараметр("Товары", РеквизитыЗаказа.Товары);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаТаблицы = РеквизитыЗаказа.Товары[Выборка.НомерСтроки - 1];
		СтрокаТаблицы.Номенклатура = Выборка.Номенклатура;
		СтрокаТаблицы.СтавкаНДС    = Выборка.СтавкаНДС;
		СтрокаТаблицы.НазначениеТовара = Выборка.НазначениеТовара;
...
//	А теперь контроль всего того, что должно быть заполнено!

	Для каждого СтрокаТаблицы Из РеквизитыЗаказа.Товары Цикл

		Если СтрокаТаблицы.Номенклатура.Пустая() Тогда

			Успешно = Ложь;
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'в строке " + Формат(СтрокаТаблицы.НомерСтроки, "ЧДЦ=; ЧГ=") + " по IDN «" + СтрокаТаблицы.IDN + "» товар не идентифицирован!'"));
		...
		Если СтрокаТаблицы.Количество = 0 Тогда
			...
			Успешно = Ложь;
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'в строке " + Формат(СтрокаТаблицы.НомерСтроки, "ЧДЦ=; ЧГ=") + " для IDN «" + СтрокаТаблицы.IDN + "» количество не указано!'"));
		...
		Если СтрокаТаблицы.Цена = 0 ИЛИ СтрокаТаблицы.Сумма = 0 Тогда
			...
			Успешно = Ложь;
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'в строке " + Формат(СтрокаТаблицы.НомерСтроки, "ЧДЦ=; ЧГ=") + " для IDN «" + СтрокаТаблицы.IDN + "» не указана цена (или сумма) продажи!'"));
...
КонецФункции // ИдентифицироватьНоменклатуру()
...
//	Булево - Истина, если не возникло ошибок.
//
Функция ИдентифицироватьНоменклатуру_УДАЛИТЬ(РеквизитыЗаказа, ДокументXDTO, Параметры, ОписаниеОшибки)
	...
	Для каждого ТоварXDTO Из МассивТоваровXDTO Цикл
		...
		Резерв = Количество;	//	LNK 12.12.2017 08:51:33 - был 0
		Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Резерв") Тогда
...
Функция ИнициализироватьТаблицуЗапасов()
	...
	ТаблицаЗапасы.Колонки.Добавить("IDN", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(20)));
	ТаблицаЗапасы.Колонки.Добавить("IDNСклада", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(20)));
	ТаблицаЗапасы.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаЗапасы.Колонки.Добавить("Склад", Новый ОписаниеТипов("СправочникСсылка.Склады"));
	ТаблицаЗапасы.Колонки.Добавить("Резерв", Новый ОписаниеТипов("Число"));
	ТаблицаЗапасы.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	ТаблицаЗапасы.Колонки.Добавить("Цена", Новый ОписаниеТипов("Число"));
	ТаблицаЗапасы.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число"));
	ТаблицаЗапасы.Колонки.Добавить("СтавкаНДС", Новый ОписаниеТипов("ПеречислениеСсылка.СтавкиНДС"));
	ТаблицаЗапасы.Колонки.Добавить("НазначениеТовара", Новый ОписаниеТипов("ПеречислениеСсылка.НазначенияТовара"));	//	LNK 06.01.2021 11:38:59

	//ТаблицаЗапасы.Колонки.Добавить("ЕдиницаИзмерения", Новый ОписаниеТипов("СправочникСсылка.УпаковкиНоменклатуры"));
	//ТаблицаЗапасы.Колонки.Добавить("Коэффициент", Новый ОписаниеТипов("Число"));
...
//	LNK 12.12.2017 10:02:18
Функция СтруктураЗначенийРеквизитов()

	ЗначенияРеквизитов = Новый Структура;
	ЗначенияРеквизитов.Вставить("Метод_доставки", Новый ОписаниеТипов("Строка"));
	ЗначенияРеквизитов.Вставить("Адрес_доставки", Новый ОписаниеТипов("Строка"));
	ЗначенияРеквизитов.Вставить("ТипДоставки"   , Новый ОписаниеТипов("Строка"));
	ЗначенияРеквизитов.Вставить("ДатаОтгрузки"  , Новый ОписаниеТипов("Дата"));
//	LNK 11.06.2020 07:52:09
	ЗначенияРеквизитов.Вставить("ОплатаДоставкиКартой", Новый ОписаниеТипов("Булево"));
//	LNK 15.06.2020 07:20:00
	ЗначенияРеквизитов.Вставить("Сумма_оплаты_клиентом", Новый ОписаниеТипов("Число"));
	ЗначенияРеквизитов.Вставить("Фактическая_сумма"    , Новый ОписаниеТипов("Число"));

	ЗначенияРеквизитов.Вставить("УчтенNavision", Новый ОписаниеТипов("Число"));

	Возврат ЗначенияРеквизитов;

КонецФункции // СтруктураЗначенийРеквизитов()
...
Функция ПолучитьНоменклатуру(ТоварXDTO, ЭтоУслуга,Параметры, СтавкаНДС, ОписаниеОшибки)
	...
	ИД = "";
	Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Код") Тогда
		Ид = ТоварXDTO.Код;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ДанныеНоменклатуры.Ид) Тогда
...
Функция ПолучитьНастройкиПодключения(НастройкиПодключения, ОписаниеОшибки)
	...
	Протокол = ?(НастройкиПодключения.ЗащищенноеСоединение, "ftps", "ftp");
...
Функция ПолучитьДанныеССервера(Соединение, ОписаниеОшибки, ПараметрыЗапроса = "", Заголовки = "")	
	...
	Попытка	
		...
		//Соединение.Получить(Источник, ИмяФайлаОтвета);
	Исключение
...
//				
//		Если Не РаспакованныеФайлы.Количество() = 0 Тогда
//				Для Каждого Стр Из РаспакованныеФайлы Цикл
...
Функция ПолучитьИзОбъектаXDTOСтрокуСФИО(КонтрагентXDTO)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Фамилия") И СвойствоXDTOЗаполнено(КонтрагентXDTO.Фамилия) И НЕ КонтрагентXDTO.Фамилия = "null" Тогда  
		ТекущаяСтрока = КонтрагентXDTO.Фамилия;
	КонецЕсли;
	
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Имя") И СвойствоXDTOЗаполнено(КонтрагентXDTO.Имя) И НЕ КонтрагентXDTO.Имя = "null" Тогда  
		ТекущаяСтрока = СокрЛП(ТекущаяСтрока + " " + КонтрагентXDTO.Имя);
	КонецЕсли;
	
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Отчество") И СвойствоXDTOЗаполнено(КонтрагентXDTO.Отчество) И НЕ КонтрагентXDTO.Отчество = "null" Тогда  
...
Функция ДатаВремяИзСтроки(ДатаСтрока, ВремяСтрока = "")
	...
	Попытка
		...
		Если ДатаВремя = Дата(1,1,1) Тогда
			...
			Иначе
				ДатаВремя = Дата(СтрЗаменить(ДатаСтрока, "-", ""));
			КонецЕсли;
...
Функция РазобратьАдресСайта(НастройкиПодключения, ОписаниеОшибки)
	...
	Если НЕ ПустаяСтрока(АдресСайта) Тогда
		...
		Если Порт = 0 Тогда
			Порт = ?(ЗащищенноеСоединение, 990, 21);
		КонецЕсли;

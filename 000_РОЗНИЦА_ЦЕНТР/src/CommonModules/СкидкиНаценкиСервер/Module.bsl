...
Функция ТекстЗапросаТаблицаУсловияСкидокПолучатели()
	...
	|"
	;
	Возврат Новый Структура("ТекстЗапроса, КоличествоТаблиц, НомерТаблицыРезультата, ИмяТаблицы", ТекстЗапроса, 1, 1, "УсловияСкидокПолучатели");

КонецФункции // ТекстЗапросаТаблицаУсловияСкидокПолучатели()
		...
		Если СтрокиУсловия.Количество() > 0 И НЕ ТоварыСегмента.Количество() = 0 Тогда
			...
			Для каждого СтрокаУсловия Из СтрокиУсловия Цикл
				...
				Если СтрокаУсловия.ТипСравнения = Перечисления.ТипыСравненияЗначенийСкидокНаценок.Кратно Тогда
					...
					КратнаяАкция  = Истина;
					ЗначениеСуммы = ПосчитаемСуммуДляСкидкиПоДокументу(ТоварыСегмента, СтрокаУсловия.ЗначениеУсловияОграничения);
...
Функция ПосчитаемСуммуДляСкидкиПоДокументу(ТоварыСегмента, ЗначениеУсловияОграничения)
	КоличествоТовара = 0;

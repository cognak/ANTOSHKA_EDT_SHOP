...
Функция ТекстЗапросаСкидкаЗаРазовуюПродажуСУсловиемПоСтроке(ПакетЗапросов, СсылкаНаУсловиеПредоставления)
	...
	|					КОГДА УсловияПредоставления.ТипСравнения = ЗНАЧЕНИЕ(Перечисление.ТипыСравненияЗначенийСкидокНаценок.ТолькоНаЭтоКоличество) ТОГДА 
	|						Товары.Количество >= УсловияПредоставления.ЗначениеУсловияОграничения 
	...
	//	LNK 18.10.2016 11:33:58
		Если СтрокиУсловия.Количество() > 0 И НЕ ТоварыСегмента.Количество() = 0 Тогда
			...
			Для каждого СтрокаУсловия Из СтрокиУсловия Цикл
				...
				// Сичкаренко
				ИначеЕсли СтрокаУсловия.ТипСравнения = Перечисления.ТипыСравненияЗначенийСкидокНаценок.ТолькоНаЭтоКоличество Тогда
				
					КратнаяАкция  = Истина;
					ЗначениеСкидкиНаценки = ПараметрыСкидки.ЗначениеСкидкиНаценки;
					
					МинимальнаяСкидка = 0;
					КлючСвязиМинимальнойСкидки = 0;
					
					Для Каждого ТоварСегмента Из ТоварыСегмента Цикл

						Сумма = ПолучитьСуммуДляСкидки(ТоварСегмента.Сумма, ТоварСегмента.КлючСвязи, Параметры);
						СуммаСкидки = Окр(((ЗначениеСкидкиНаценки / 100) * Сумма * СтрокаУсловия.ЗначениеУсловияОграничения) / ?(ТоварСегмента.Количество = 0, 1, ТоварСегмента.Количество), 2);
						
						Если МинимальнаяСкидка > СуммаСкидки Или МинимальнаяСкидка = 0 Тогда
							МинимальнаяСкидка = СуммаСкидки;
							КлючСвязиМинимальнойСкидки = ТоварСегмента.КлючСвязи;
						КонецЕсли;
					...
					Если Не КлючСвязиМинимальнойСкидки = 0 Тогда 
					
						Для Каждого СтрокаТаблицыДанных Из ТаблицаДанных Цикл
							
							Если СтрокаТаблицыДанных.КлючСвязи = КлючСвязиМинимальнойСкидки Тогда 
								
								Продолжить;
								
							КонецЕсли;
							
							СтрокаТаблицыДанных.Действует = Ложь;

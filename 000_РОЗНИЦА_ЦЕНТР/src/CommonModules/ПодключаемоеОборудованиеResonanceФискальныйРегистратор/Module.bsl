...
Функция ЗакрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаОплат, ВыходныеПараметры, ФискальныйЧек = Истина, СлужебнаяИнформация = Неопределено) Экспорт
	...
	Для ИндексОплаты = 0 По ТаблицаОплат.Количество() - 1 Цикл
		...
		ИначеЕсли ТаблицаОплат[ИндексОплаты][0].Значение = 1 Тогда
			...
			//2025-03-03 sa в ФР жестко прописано что ЕПЗ - это 3 по счету форма оплаты. Форму оплаты нужно передавать в функцию AddSlip чтобы
			//корректно отображались Платежные реквизиты.
			
			Параметры.ДопПараметрыЧека.Вставить("НомерТранзакции",ТаблицаОплат[ИндексОплаты][4].Значение);
			ФормаОплатыБезнал = 3;
			Успешно = ПодключаемоеОборудованиеResonanceФискальныйРегистраторИнтерфейс.ДобавитьСлип(ОбъектДрайвера,Параметры.ДопПараметрыЧека,ФормаОплатыБезнал);

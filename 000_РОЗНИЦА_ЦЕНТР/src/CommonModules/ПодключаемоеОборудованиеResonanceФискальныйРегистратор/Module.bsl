...
Функция ПечатьЧека(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если Параметры.Свойство("НовыеФункции") И Параметры.НовыеФункции = Истина Тогда
		Если ЧекВозврата Тогда
			...
			НапечататьНефискальнуюСтроку(ОбъектДрайвера,Параметры,ПараметрыПодключения,"ФР: "+ДопПараметрыЧека.ФискальныйРегистратор,0,ВыходныеПараметры);
		КонецЕсли;
	КонецЕсли;
	...
	//	Печатаем строки чека
		...
		//	Закрываем чек
			...
			Если Результат Тогда
				...
				ДанныеФН.Вставить("ФискальныйНомер",Строка(ВыходныеПараметры[1]-1));

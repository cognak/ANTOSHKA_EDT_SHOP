...
//	LNK 23.08.2016 14:01:19
Процедура ОбновитьИдентификаторыОбъектовМетаданных()	Экспорт

	Если ПравоДоступа("Добавление", Метаданные.Справочники.ИдентификаторыОбъектовМетаданных) Тогда

	//	По идее, сюда может попасть только роль "АдминистраторСистемы".
		ЕстьИзменения = Ложь;
		ЕстьУдаленные = Ложь;
	//	Сначала выполним только проверку!
		Справочники.ИдентификаторыОбъектовМетаданных.ОбновитьДанныеСправочника(ЕстьИзменения, ЕстьУдаленные, Истина);

		Если ЕстьИзменения ИЛИ ЕстьУдаленные Тогда
...
//	LNK 09.10.2017 10:06:15
Процедура ОтключитьОбновлениеПредопределенныхЭлементов()	Экспорт

	УстановитьПривилегированныйРежим(Истина);

	КлассыОбъектов = Новый Массив;
	КлассыОбъектов.Добавить(Справочники);
	КлассыОбъектов.Добавить(ПланыВидовХарактеристик);
	КлассыОбъектов.Добавить(ПланыВидовРасчета);
	КлассыОбъектов.Добавить(ПланыСчетов);

	Для каждого Класс Из КлассыОбъектов Цикл

		Для каждого Менеджер Из Класс Цикл

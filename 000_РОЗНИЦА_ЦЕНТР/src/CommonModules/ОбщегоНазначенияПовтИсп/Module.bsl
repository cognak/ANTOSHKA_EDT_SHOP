...
Функция РежимРаботыПрограммы() Экспорт
	...
	РежимРаботы.Вставить("ЭтоГлавныйУзел", ОбменДаннымиПовтИсп.ЭтоГлавныйУзел());
...
Функция ЕдиницаШтука()	Экспорт

	Возврат Справочники.БазовыеЕдиницыИзмерения.НайтиПоКоду("2009");

КонецФункции // ЕдиницаШтука()

Функция ЕдиницаУпаковка()	Экспорт

	Возврат Справочники.БазовыеЕдиницыИзмерения.НайтиПоКоду("2110");

КонецФункции // ЕдиницаУпаковка()

//	LNK 05.05.2020 06:57:15
Функция ПолучитьСтруктуруЗаписиРегистра(ПолноеИмя)	Экспорт

	Возврат ОбщегоНазначения.ПолучитьСтруктуруЗаписиРегистра(ПолноеИмя);
...
Функция ПолучитьТаблицуОбразРегистра(ПолноеИмя)	Экспорт	//	LNK 05.05.2022 07:01:12
	...
	Возврат ОбщегоНазначения.ПолучитьТаблицуОбразРегистра(ПолноеИмя);

КонецФункции

Функция ТипыАтрибутаРазделаУчета(ИмяКоллекции, ИмяМенеджера, ИмяРаздела, ИмяАтрибута)	Экспорт

	Возврат Метаданные[ИмяКоллекции][ИмяМенеджера][ИмяРаздела][ИмяАтрибута].Тип.Типы();
...
//	LNK 09.09.2016 11:44:40
Функция ЕстьОбщийРеквизит(Реквизит, ПолноеИмя)	Экспорт

	МетаданныеОбъекта = Метаданные.НайтиПоПолномуИмени(ПолноеИмя);

	Элемент = Метаданные.ОбщиеРеквизиты[Реквизит].Состав.Найти(МетаданныеОбъекта);

	Если НЕ Элемент = Неопределено Тогда

		Если Элемент.Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать Тогда

			ЕстьРеквизит = Истина;

		ИначеЕсли Элемент.Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.НеИспользовать Тогда

			ЕстьРеквизит = Ложь;

		Иначе

			ЕстьРеквизит = Метаданные.ОбщиеРеквизиты[Реквизит].АвтоИспользование = Метаданные.СвойстваОбъектов.АвтоИспользованиеОбщегоРеквизита.Использовать;

		КонецЕсли;

	Иначе	ЕстьРеквизит = Ложь;

	КонецЕсли;

	Возврат ЕстьРеквизит;

КонецФункции // ЕстьОбщийРеквизит()

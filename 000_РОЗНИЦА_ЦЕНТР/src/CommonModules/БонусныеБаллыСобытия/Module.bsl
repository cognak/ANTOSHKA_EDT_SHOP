			...
			Пока ВыборкаАбоненты.Следующий() Цикл
				...
				Попытка
					...
					Если ОтправитьОповещениеАннуляции(ВыборкаАбоненты, ТекстСообщения) Тогда
						...
						НаборЗаписей = РегистрыСведений.ОповещенияАннуляцииБонусныхБаллов.СоздатьНаборЗаписей();
						НаборЗаписей.Отбор.Период.Установить(ТекущаяДата());
						НаборЗаписей.Отбор.Контрагент.Установить(ВыборкаАбоненты.Контрагент);
						...
						ЗаписьНабора.НомерТелефона = ВыборкаАбоненты.НомерТелефона;
						ЗаписьНабора.ДатаАннуляции = Выборка.ДатаАннуляции;

						ЗаписьНабора.ДатаИзменения = ТекущаяДата();
						ЗаписьНабора.Сообщение	   = ТекстСообщения;
						ЗаписьНабора.Хранилище	   = Новый ХранилищеЗначения(ТекстСообщения, Новый СжатиеДанных(9));
...
Функция ВыборкаАбонентовОповещения(ДатаАннуляции)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОповещенияАннуляцииБонусныхБаллов КАК Фильтр
	|		ПО Аннуляция.Контрагент = Фильтр.Контрагент
	|			И (Фильтр.ПрограммаЛояльности = ЗНАЧЕНИЕ(Справочник.ПрограммыЛояльности.ПустаяСсылка))

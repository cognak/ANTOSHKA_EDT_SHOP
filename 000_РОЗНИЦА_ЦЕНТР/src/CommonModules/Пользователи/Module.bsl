...
Функция ЭтоПолноправныйПользователь(Пользователь = Неопределено,
	...
	Иначе
		...
		Если ПроверитьРольПолныеПрава
		   ...
		   И НЕ АвторизованныйПользователь() = Справочники.Пользователи.АдминистраторАвтоматов	Тогда
			Возврат Ложь;
		КонецЕсли;
		Если ПроверитьРольАдминистратораСистемы
...
//
Функция РолиДоступны(Знач ИменаРолей, Пользователь = Неопределено, ПроверятьПраваАдминистрированияСистемы = Ложь) Экспорт
	
	Если ЭтоПолноправныйПользователь(Пользователь, ПроверятьПраваАдминистрированияСистемы, Ложь) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Для каждого ИмяРоли Из МассивИменРолей Цикл
		...
		Если УказанТекущийПользовательИБ Тогда
			...
			Если Метаданные.Роли.Найти(СокрЛП(ИмяРоли)) = Неопределено Тогда

				ЖурналСобытий.Регистрация("Роли.Ошибка", УровеньЖурналаРегистрации.Ошибка,,,, ИменаРолей, ИмяРоли, Истина);

			ИначеЕсли РольДоступна(СокрЛП(ИмяРоли)) Тогда

...
Функция ПодключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Если Порт         = Неопределено
	 Или Наименование = Неопределено Тогда
		...
		ВыходныеПараметры.Добавить(НСтр("ru='Не настроєно параметри пристрою.
		|Для коректної роботи пристрою необхідно встановити параметри його роботи.
...
Функция ОтключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Иначе 
		Сообщить("Біосканер вже вимкнено!");
	КонецЕсли;
...
Функция ВыполнитьКоманду(Команда, ВходныеПараметры = Неопределено, ВыходныеПараметры = Неопределено,
	...
	ИначеЕсли Команда = "ПоследнийОтпечаток" ИЛИ Команда = "LastScan" Тогда
		...
		Исключение
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Не вдалося отримати відбиток: "+ОбъектДрайвера.LastError+" {"+ОписаниеОшибки()+"}");
			Результат = Ложь;
		КонецПопытки;         
	...
	ИначеЕсли Команда = "ЗахватОтпечатка" ИЛИ Команда = "StartCapture" Тогда
		...
		Иначе
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Не вдалося розпочати захоплення відбитка: "+ОбъектДрайвера.LastError);

			Результат = Ложь;
		КонецЕсли;	                                        

	// Закончить захват отпечатка
	ИначеЕсли Команда = "ОстановитьЗахват" ИЛИ Команда = "StopCapture" Тогда
		...
		Иначе
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Не вдалося зупинити захоплення відбитка: "+ОбъектДрайвера.LastError);
	...
	ИначеЕсли Команда = "РегистрацияПоДанным" ИЛИ Команда = "Register" Тогда
		Если ТипЗнч(ВходныеПараметры) <> Тип("Массив") ИЛИ ВходныеПараметры.Количество() =0  Тогда
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Немає даних для реєстрації співробітника!");
		...
		Если ТипЗнч(МассивОтпечатков) <> Тип("Массив") ИЛИ МассивОтпечатков.Количество() <>3 Тогда
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Немає даних для реєстрації працівника! Для реєстрації потрібно передати три відбитки!");
		...
		Иначе     
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Помилка реєстрації відбитка: "+ОбъектДрайвера.LastError);
	...
	ИначеЕсли Команда = "СравнитьОтпечатки" ИЛИ Команда = "Compare" Тогда 
		Если ТипЗнч(ВходныеПараметры) <> Тип("Массив") ИЛИ ВходныеПараметры.Количество() < 2 Тогда
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Немає даних для порівняння!");
	...
	ИначеЕсли Команда = "ИнициализацияБД" ИЛИ Команда = "InitDB" Тогда       
		Если ТипЗнч(ВходныеПараметры) <> Тип("Массив") ИЛИ ВходныеПараметры.Количество() =0  Тогда
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Немає даних для ініціалізації вбудованої БД!");
		...
		Иначе     
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Помилка ініціалізації вбудованої БД: "+ОбъектДрайвера.LastError);
	...
	ИначеЕсли Команда = "НайтиПоБД" ИЛИ Команда = "MatchDB" Тогда
		...
		Иначе
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить("Відбиток не знайдено за вбудованою БД!");
	...
	Иначе
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(НСтр("ru='Команда ""%Команда%"" не підтримується цим драйвером.'"));
...
Функция ТестУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
    ...
    ВыходныеПараметры.Добавить(?(Результат, "Біосканер s/n: "+ОбъектДрайвера.SerialNumber+" протестовано успішно!",
		НСтр("ru='Помилка при підключенні пристрою'")));
...
Функция ПолучитьВерсиюДрайвера(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры)
	...
	ВыходныеПараметры.Добавить(НСтр("ru='Драйвер встановлено'"));
	ВыходныеПараметры.Добавить(НСтр("ru='Не визначено'"));

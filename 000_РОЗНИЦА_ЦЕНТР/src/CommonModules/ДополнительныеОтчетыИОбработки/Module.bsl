...
//
Функция ПодключитьВнешнююОбработку(Ссылка, ОписаниеЗащитыОтОпасныхДействий = Неопределено) Экспорт
	...
	//	LNK 29.05.2019 15:01:31	- отключим
	...
	ИмяОбработки = Менеджер.Подключить(АдресВоВременномХранилище, , ПараметрыЗапуска.БезопасныйРежим, ОписаниеЗащитыОтОпасныхДействий);
...
Процедура ПриСозданииНаСервере(Форма) Экспорт
	...
	Параметры = ДополнительныеОтчетыИОбработкиПовтИсп.ПараметрыФормыНазначаемогоОбъекта(Форма.ИмяФормы);
...
Процедура ВыполнитьНазначаемуюКомандуНаСервере(Форма, ИмяЭлемента, РезультатВыполнения) Экспорт
	...
	ПараметрыКоманды.Вставить("ЭтотОбъект", Форма);
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если НЕ Объект.ЭтоНовый() И НЕ РегистрационныеДанные.Вид = Объект.Вид И НЕ Объект.Вид.Пустая() Тогда
...
Функция ПолучитьКолДозаказа(Остаток,Кратность,Номенклатура,ТипПереоценки,Магазин,ДатаКон,КолАм,ТипПоступления) Экспорт
	
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 7
		|	НоменклатурноеСостояние.КоличествоПродажи КАК КоличествоПродажи
		|ИЗ
		|	РегистрСведений.НоменклатурноеСостояние КАК НоменклатурноеСостояние
		|ГДЕ
		|	НоменклатурноеСостояние.Номенклатура = &Номенклатура
		|	И НоменклатурноеСостояние.Магазин = &Магазин
		|	И НоменклатурноеСостояние.ТипПереоценки = &ТипПереоценки
		|	И (НоменклатурноеСостояние.КоличествоНаСкладах > 0
		|			ИЛИ НоменклатурноеСостояние.КоличествоПродажи > 0)
		|	И НоменклатурноеСостояние.ПериодДень МЕЖДУ &ДатаНачала И &ДатаКонца
		|
		|УПОРЯДОЧИТЬ ПО
		|	НоменклатурноеСостояние.ПериодДень УБЫВ";
	
	Запрос.УстановитьПараметр("ДатаКонца", ДатаКон);
	Запрос.УстановитьПараметр("ДатаНачала", (ДатаКон-180*24*60*60));
	Запрос.УстановитьПараметр("Магазин", Магазин);
	Запрос.УстановитьПараметр("Номенклатура", Номенклатура);
	Запрос.УстановитьПараметр("ТипПереоценки", ТипПереоценки);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() тогда 
		СредняяПродажаЗадень = 0;
	Иначе
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		А = А + 1;
		Сумма = Сумма +  ВыборкаДетальныеЗаписи.КоличествоПродажи;
	КонецЦикла;
	СредняяПродажаЗадень = Сумма/А;
	КонецЕсли;

	Если ТипПоступления = 1 тогда 
		СредняяПродажаЗаНеделю =  СредняяПродажаЗадень*5;
	Иначе
		СредняяПродажаЗаНеделю =  СредняяПродажаЗадень*10;
	КонецЕсли;
	//Если Остаток  < СредняяПродажаЗаНеделю ИЛИ Остаток < КолАМ тогда 
	//	Если НЕ ЗначениеЗаполнено(Кратность) тогда 
	//		Кратность = 0;
	//	КонецЕсли;
	//	Если Кратность = 0  тогда
	//		КолЗаказа =  СредняяПродажаЗаНеделю - Остаток;
	//		если КолЗаказа < (КолАМ-Остаток) тогда
	//			Дозаказ = (КолАМ-Остаток);
	//		Иначе
	//			Дозаказ = КолЗаказа;
	//		КонецЕсли;         			
	//	Иначе
	//		КолЗаказа =  СредняяПродажаЗаНеделю - Остаток;	
	//		если КолЗаказа < (КолАМ-Остаток) тогда
	//			КолЗаказа = (КолАМ-Остаток);
	//		Иначе
	//			КолЗаказа = КолЗаказа;
	//		КонецЕсли;  
	//		Если  КолЗаказа/Кратность < 1 тогда 
	//			Кофициент = 1;
	//		Иначе
	//			ОкрКоэффициент =  КолЗаказа/Кратность;
	//			Кофициент = окр(ОкрКоэффициент); 				
	//		КонецЕсли;
	//		Дозаказ =  Кофициент*Кратность;
	//	КонецЕсли;
	//КонецЕсли;
	// + Бакан 17.11.2021 Убрал условие по АМ
	Если Остаток  < СредняяПродажаЗаНеделю тогда 
		Если НЕ ЗначениеЗаполнено(Кратность) тогда 
			Кратность = 0;
		КонецЕсли;
		Если Кратность = 0  тогда
			Дозаказ =  СредняяПродажаЗаНеделю - Остаток;
		Иначе
			КолЗаказа =  СредняяПродажаЗаНеделю - Остаток;	
			Если  КолЗаказа/Кратность < 1 тогда 
				Кофициент = 1;
...
Функция ПолучитьДоступностьРазмещенияРМ(ДатаС,ДатаПо,Магазин,РазмещениеРекламныхМатериалов) Экспорт
	Количество = 0;
	Занято  = 0;
	...
	Пока ТекДата<=ДатаПо Цикл;
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	СУММА(РекламныеМатериалы.Количество) КАК Количество
			|ИЗ
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Количество) тогда
				Занято = Макс(Занято,ВыборкаДетальныеЗаписи.Количество);
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьСписокМагазиновГдеБылиПокупки(клиент) Экспорт 
	
	СписокМагазиновСтрокой = "";
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЧекККМ.ВладелецДисконтнойКарты = &ВладелецДисконтнойКарты
		|	И ЧекККМ.Проведен = ИСТИНА
		|
		|СГРУППИРОВАТЬ ПО
		|	ЧекККМ.Магазин";
	
	Запрос.УстановитьПараметр("ВладелецДисконтнойКарты", клиент);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СписокМагазиновСтрокой = СписокМагазиновСтрокой + ",  "
	КонецЦикла;
	ДлинаСтроки =  СтрДлина(СписокМагазиновСтрокой);
	Если ДлинаСтроки > 0 тогда  
		СписокМагазиновСтрокой = Лев(СписокМагазиновСтрокой,ДлинаСтроки -2) + ".";	
	КонецЕсли;
	
	Возврат  СписокМагазиновСтрокой;

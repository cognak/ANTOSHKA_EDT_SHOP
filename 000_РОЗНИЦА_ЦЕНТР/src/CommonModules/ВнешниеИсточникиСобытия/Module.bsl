...
Процедура ОбработатьТоварныйОрдерПроведен(Запрос)
	...
	//	ИЛИ НЕ ВнешниеИсточникиСверка.НаличиеДокументаNavision(Выборка.IDN_Перемещения, "ПЕРЕМЕЩ") Тогда	//	LNK 05.06.2019 13:31:48
		...
		//	IDN основания не задан (не оптравлялся в NAV) или основания ещё (или уже) нет в NAV
			Продолжить;
...
Процедура ОчиститьОбъектДляОбработки(Период = '00010101', IDN = "", ДействиеКоманда = "", Объект = Неопределено)	Экспорт
	...
	Если НЕ (ПустаяСтрока(IDN) И ПустаяСтрока(ДействиеКоманда) И Объект = Неопределено) Тогда
...
Процедура СобытияВходящиеЗаданияNavision(Назначение, Отладка)
	...
	ИначеЕсли Назначение = "Характеристики" Тогда	//	LNK 17.07.2020 07:16:18
		...
		УсловиеОтбора = "= 'RR' AND ([Тип Задания] = 'ОБНОВИТЬ_ДОП_ХАРАКТЕРИСТИКИ' OR [Тип Задания] = 'ОБНОВИТЬ_ТОВАР_ВАРИАНТ')";
...
//	отключимся от внешнего источника...
	...
	Для каждого ИмяЗаданияNavision Из ИменаЗаданийNavision Цикл
			...
			Пока ОчередьNavisionВыборка.Следующий() Цикл
				...
				Попытка
					...
					Тогда
						...
						Тогда
							...
							Если ОчередьNavisionВыборка.ТипЗадания = "ДОК_ПЕРЕМЕЩ_УЧТЕНО" Тогда
								...
								|	ТаблицаРегистра.Магазин.СкладУправляющейСистемы КАК СкладУправляющейСистемы
								|ИЗ
								...
								|	И ТаблицаРегистра.Объект.ДокументОснование = &ДокументСсылка"
								);
								...
								//	При записи происходит регистрация в нужные узлы
									...
									Если Выборка.СкладУправляющейСистемы = Ложь Тогда
										...
										ВнешниеИсточникиСверка.НазначитьДокументДляСверки(ПараметрыЗадания.Ссылка, ПараметрыСверки, "Сверка после учёта из Navision");
					...
					//	Остановка = 1;
						...
						Запрос.Текст =
							...
							|	ТаблицаЗД.ОтправлятьВNavision КАК ОтправлятьВNavision,
						...
						Если ВыборкаЗД.Следующий() Тогда
							...
							ИначеЕсли ОчередьNavisionВыборка.ID2 = "ОТГРУЖЕН" Тогда
								...
								Если ДанныеСостояния = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Согласован Тогда
								...
								ИначеЕсли ВыборкаЗД.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Согласован Тогда
									
									ЗаданиеВыполнено = Истина;
									 
								ИначеЕсли ДанныеСостояния = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ВРаботе Тогда
									
									ЗапросДоступностиОбъект.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ВРаботе;
									ЗапросДоступностиОбъект.Записать();
									
								ИначеЕсли ВыборкаЗД.ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ГотовКЗакрытиюВNavision Тогда
									
									Для Каждого СтрокаТЧ Из ЗапросДоступностиОбъект.Товары Цикл
										
										СтрокаЗаписиВРегистр = РегистрыСведений.СостояниеСтрокЗаказаПокупателя.ИнициализацияСтрокиЗаписи();
										СтрокаЗаписиВРегистр.ЗаказПокупателя = ВыборкаЗД.СсылкаНаЗаказ;
										СтрокаЗаписиВРегистр.ЗапросДоступностиТоваров = ВыборкаЗД.Ссылка;
										СтрокаЗаписиВРегистр.КлючСвязи = СтрокаТЧ.КлючСвязи;
										СтрокаЗаписиВРегистр.СогласованоВNavision = Истина;
										
										РегистрыСведений.СостояниеСтрокЗаказаПокупателя.ЗаписьВРегистрСтроки(СтрокаЗаписиВРегистр);
									
									КонецЦикла;
									
									ЗапросДоступностиОбъект.ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ЗакрытВNavision;
									ЗапросДоступностиОбъект.Записать();
									ЗаданиеВыполнено = Истина;
				...
				Исключение
					...
					Иначе
						...
						ЗаписьЖурналаРегистрации("Navision.Обработка очереди", УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки, РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
						ПараметрыЗадания.Вставить(
							"ДанныеОшибки"
							, Новый Структура(
								"Заголовок, Сообщение, Выполнено"
		...
		Иначе
			...
			ОчередьNavision.Отбор.ТипЗадания.Установить(СтрЗаменить(ИмяЗаданияNavision, "_WEB", ""));
...
Процедура ВыгрузитьТаблицуБонусовMagento(ДатаВыгрузки, СверкаДокументов = Ложь, Отладка = Ложь)	Экспорт
...
Функция ПолучитьТаблицуЗаказовMagento(ДатаВыгрузки, СверкаДокументов = Ложь)
	...
	Пока УзлыВыборка.Следующий() Цикл
		...
		Для каждого ИмяКласса Из КлассификаторСостава.Классы Цикл
			...
			Пока ВыборкаИзменений.Следующий() Цикл
				...
				Если ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ЗапросДоступностиТоваров")
						И (ОбъектДанных.Ссылка.ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПустаяСсылка()
							Или ОбъектДанных.Ссылка.ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ПередатьВNavision
							Или ОбъектДанных.Ссылка.ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ГотовКЗакрытиюВNavision) Тогда
...
Процедура ВыполнитьВыгрузкуДанныхВNavision(ОбъектДанных, УзелСсылка, ПараметрыВыгрузки)
	...
	Если ТипЗнч(ОбъектДанных) = Тип("Структура") Тогда
		...
		Если ПараметрыЗадания.ИспользоватьЗадания И НомерЗадания = 0
		ИЛИ НЕ ПроверитьТранспортныеТаблицы(ОбъектДанных.IDN, ПараметрыЗадания, "NN", ПараметрыВыгрузки.СверкаДокументов, ПараметрыВыгрузки.Отладка) Тогда
			...
			ЖурналСобытий.Регистрация(ИмяСобытия("PUT_NAV", ПараметрыВыгрузки, Истина) + "ErrorConnections", УровеньЖурналаРегистрации.Предупреждение
				...
				,
				,
				, "Отложено. Не получено подтверждение о задании в Navision. Выгрузка не совершена."
		...
		ИначеЕсли НЕ ОбъектДанных.ПометкаУдаления ИЛИ ПараметрыВыгрузки.СверкаДокументов Тогда
			...
			Для каждого СтрокаТаблицыЗаданий Из ТаблицаЗаданий Цикл
				...
				ВыполнитьВыгрузку = ?(ПараметрыВыгрузки.СверкаДокументов = Истина 
					...
					, Истина
					, (ОчередьПолучитьНомер(ПараметрыЗадания.ТипЗадания, ОбъектДанных.IDN,, ПараметрыВыгрузки.Отладка) = 0 ИЛИ НЕ СтрокаТаблицыЗаданий.Проверять)
						И  НЕ ЕстьВЗаголовках(ПараметрыЗадания.ТипДокумента, ОбъектДанных.IDN, ОбъектДанных.ВерсияДанных, ПараметрыЗадания, ПараметрыВыгрузки.Отладка)
				);
				...
				//	---------------------------------------------------------------------------------------
					...
					Иначе
						...
						УдалятьРегистрацию = ПроверитьТранспортныеТаблицы(ОбъектДанных.IDN, ПараметрыЗадания, "NN", ПараметрыВыгрузки.СверкаДокументов, ПараметрыВыгрузки.Отладка);

						Если НЕ УдалятьРегистрацию Тогда

							УдалитьИзОчередиЗаданийИТранспортныхТаблиц(ОбъектДанных.IDN, ПараметрыЗадания, "NN", ПараметрыВыгрузки.Отладка);
...
Функция ПроверкаВозможностиВыгрузкиВNavision(ОбъектДанных, ПараметрыЗадания, СтрокаТаблицыЗаданий, УчетнаяПолитика, УдалятьРегистрацию)
	...
	ИначеЕсли ?(ПараметрыЗадания.СверкаДокументов, Ложь, НЕ ОбъектДанных.Проведен) Тогда
...
Процедура ОбновитьПереданоNavision(СсылкаНаОбъект, ПараметрыЗаписи, ТранзакцияУжеАктивна = Ложь)
...
//	упоминания объекта в этом регистре.
	...
	//	Назначим объект для сверки с данными в КСУ Navision.
		ВнешниеИсточникиСверка.НазначитьДокументДляСверки(СсылкаНаОбъект
			, Новый Структура("Магазины, РезультатСверки", Параметр, Перечисления.РезультатСверкиNavision.НеПроверялся));
...
Процедура ДобавитьКнопкуУчестьВNavision(Форма, КоманднаяПанель)	Экспорт
	...
	Форма.ИзменитьРеквизиты(ОбщегоНазначенияКлиентСервер.AAD(Новый РеквизитФормы(ИмяКнопки, Новый ОписаниеТипов("Булево"))));
...
//	---------------------------------------------------------------------------------------
	...
	+ ?(ПараметрыЗадания.ИспользоватьЗадания
		, Символы.ПС + ОчередьУстановкиЗаданияТекстЗапроса(ПараметрыЗадания.ВремяВыполнения, 
...
//	ВыборкаШапка.NUMBER
	Возврат ПередатьДанныеNavision(RequestText, ОбъектДанных.IDN, ПараметрыЗадания, Истина, ПараметрыВыгрузки.Отладка);
...
//	Общие реквизиты ...
	...
	Параметры.Вставить("КодОтдела"		, "АНТ");
...
//	LNK 25.10.2019 11:06:57 - специфика Документы.ЗаказПокупателя
	...
	Если ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ЗаказПоставщику") Тогда
		...
		Параметры.Вставить("Товары", ПараметрЗапросаТовары(ОбъектДанных.Товары));
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда	//	LNK 25.10.2019 10:12:03
		...
		Параметры.Вставить("Оператор"        , ?(ОбъектДанных.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.НоваяПочтаДоставкаВОтделение ИЛИ ОбъектДанных.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.НоваяПочтаДоставкаПоАдресу
			, "Новая Почта"
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ЗапросДоступностиТоваров") Тогда
		...
		Если Не ДанныеЗаказа.Самовывоз Тогда
			...
			КонецЕсли;
			Параметры.Вставить("АдресКлиента"    , АдресКлиента);  
		КонецЕсли;
		
		Параметры.Вставить("ДатаОтгрузки"    , Макс(ДокументЗаказа.ДатаПродажиЖелаемая, ДокументЗаказа.Дата));
		...
		Параметры.Вставить("СуммаЗаДоставкуКлиенту", ДокументЗаказа.СуммаДоставки);
		Параметры.Вставить("ЗатратыОператора"      , ДокументЗаказа.СуммаДоставкиОператор);
		...
		Параметры.Вставить("Товары", ПараметрЗапросаТоварыЗапросаДоступности(ОбъектДанных));
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ПересортицаТоваров") Тогда
		...
		Параметры.Вставить("Товары", ПараметрЗапросаТовары(ТаблицаПересортицаТоваров(ОбъектДанных, Параметры)));
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ПересчетТоваровСамостоятельный") Тогда
		...
		Параметры.Вставить("Товары", ПараметрЗапросаТовары(ТаблицаПересчетТоваровСамостоятельный(ОбъектДанных, Параметры)));
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("Структура") Тогда
		...
		Параметры.Вставить("Товары" , ПараметрЗапросаТовары(ТаблицаПродажПоМагазину(ОбъектДанных, ОбъектДанных.Команда)));
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.КассовыйДень") Тогда
		...
		Параметры.Вставить("Товары" , ПараметрЗапросаТовары(ТаблицаПродажПоМагазину(ОбъектДанных.Ссылка, ПараметрыЗадания.ТипДокумента)));
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ПеремещениеТоваров")
		ИЛИ   ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.КорректировкаПеремещения") Тогда

		МагазинОтправитель = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектДанных.МагазинОтправитель, "СкладПоступления, СкладУправляющейСистемы");
		МагазинПолучатель  = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектДанных.МагазинПолучатель , "СкладПоступления, СкладУправляющейСистемы");
		...
		Если ТипЗнч(ОбъектДанных.ДокументОснование) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
			...
			Параметры.Вставить("Склад"          , МагазинОтправитель.СкладПоступления);
		...
		Иначе
			...
			Если МагазинОтправитель.СкладУправляющейСистемы = Истина Тогда

				Параметры.Магазин = ОбъектДанных.МагазинПолучатель;

			ИначеЕсли МагазинПолучатель.СкладУправляющейСистемы = Истина Тогда
...
Функция ПараметрЗапросаТовары(ТабличнаяЧасть)
...
Функция ПараметрЗапросаТоварыЗапросаДоступности(ДокЗапросДоступности) 
	...
	Запрос.Текст = 
		...
		|	ЗаказПокупателяТовары.Обеспечение КАК Обеспечение,
		|	ЗаказПокупателяТовары.КлючСвязиЗапросаДоступности КАК КлючСвязиЗапросаДоступности,
		|	ЗаказПокупателяТовары.КлючСвязиСерийныхНомеров КАК КлючСвязиСерийныхНомеров
		|ИЗ
	...
	Возврат ПараметрЗапросаТовары(РезультатЗапроса);
...
Функция ТаблицаПродажПоМагазину(КассовыйДень, ТипДокумента)
	...
	ИначеЕсли ТипДокумента = "ОБЩ.ПРОД.КАССА" Тогда
		...
		ТаблицаРезультат = Результаты[4].Выгрузить();

		ВыборкаКонтроль  = Результаты[3].Выбрать();
...
//	6 - для товаров "ОплатаЧастями"
	...
	|		Фильтр.Ссылка КАК Ссылка,
	|		99999999999 КАК НомерСтроки,
	|		ТаблицаПродажи.Номенклатура КАК Номенклатура,
	|		ТаблицаПродажи.КоличествоОборот КАК Количество,
	|		ВЫРАЗИТЬ(ВЫБОР
	|				КОГДА ТаблицаПродажи.КоличествоОборот = 0
	|					ТОГДА ТаблицаПродажи.СтоимостьОборот
	|				ИНАЧЕ ТаблицаПродажи.СтоимостьОборот / ТаблицаПродажи.КоличествоОборот
	|			КОНЕЦ КАК ЧИСЛО(15, 2)) КАК Цена,
	|		ТаблицаПродажи.СтоимостьОборот КАК Сумма,
	|		ВЫБОР
	|			КОГДА ЕСТЬNULL(ТаблицаПродажи.Регистратор.УчитыватьНДС, ИСТИНА)
	|					И ЕСТЬNULL(ТаблицаПродажи.Регистратор.ЦенаВключаетНДС, ИСТИНА)
	|				ТОГДА ТаблицаПродажи.СтоимостьОборот - ТаблицаПродажи.НДСОборот
	|			ИНАЧЕ ТаблицаПродажи.СтоимостьОборот
	|		КОНЕЦ КАК СуммаБезНДС,
	|		ТаблицаПродажи.СтоимостьБезСкидокОборот КАК СуммаБезСкидки,
	|		ТаблицаПродажи.НДСОборот КАК СуммаНДС,
	|		ВЫБОР
	|			КОГДА ТаблицаПродажи.НДСОборот = 0
	|					ИЛИ НЕ ЕСТЬNULL(ТаблицаПродажи.Регистратор.УчитыватьНДС, ИСТИНА)
	|				ТОГДА 0
	|			ИНАЧЕ (ВЫРАЗИТЬ(1 / (ВЫБОР
	|						КОГДА ЕСТЬNULL(ТаблицаПродажи.Регистратор.ЦенаВключаетНДС, ИСТИНА)
	|							ТОГДА ТаблицаПродажи.СтоимостьОборот - ТаблицаПродажи.НДСОборот
	|						ИНАЧЕ ТаблицаПродажи.СтоимостьОборот
	|					КОНЕЦ / ТаблицаПродажи.НДСОборот) КАК ЧИСЛО(5, 2))) * 100
	|		КОНЕЦ КАК СтавкаНДС,
	|		Фильтр.КодОплаты КАК КодОплаты,
	|		Фильтр.КодОплаты КАК КлючГруппировки,
	...
	|			ПО Фильтр.Ссылка = ТаблицаПродажи.Регистратор
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваров КАК ТаблицаРеализация
	|			ПО Фильтр.Ссылка = ТаблицаРеализация.Ссылка
	|	ГДЕ
	|		НЕ Фильтр.РозничныеПродажи) КАК ТаблицаРеализация
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаРеализация.Дата,
	|	ТаблицаРеализация.Ссылка,
...
КонецФункции // ПолучитьСуммуИнкассацииКассовогоДня()
...
Функция ПередатьСправочникNavision(ОбъектДанных, ПараметрыЗадания, Отладка)
	...
	Возврат ПередатьДанныеNavision(RequestText, ОбъектДанных.IDN, ПараметрыЗадания, Истина, Отладка);
	...
	ИначеЕсли ТипЗначения = Тип("ДокументСсылка.ЗапросДоступностиТоваров") Тогда
		...
		НужноЗакрытьЗаказ = 
			(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДанных.Ссылка,
				"ОтправлятьВNavision",
				Перечисления.СтатусыЗапросаДоступностиВNavision.ПустаяСсылка(),
				Ложь) = Перечисления.СтатусыЗапросаДоступностиВNavision.ГотовКЗакрытиюВNavision);
...
Функция ОчередьУстановитьЗадание(Период, ТипЗадания, ID1, ID2 = "", ID3 = "", ID4 = "", ID5 = "", ID6 = "", ID7 = "", ID8 = "", ID9 = "")

	НомерЗадания = 0;
	ТекстЗапроса = "SET NOCOUNT ON" + Символы.ПС + ОчередьУстановкиЗаданияТекстЗапроса(Период, ТипЗадания, ID1, ID2, ID3, ID4, ID5, ID6, ID7, ID8, ID9);
...
Функция ОчередьУстановкиЗаданияТекстЗапроса(Период, ТипЗадания, ID1, ID2 = "", ID3 = "", ID4 = "", ID5 = "", ID6 = "", ID7 = "", ID8 = "", ID9 = "")
	...
	Если НЕ Отладка = Истина Тогда
		...
		Если НЕ ТаблицаNavision.Количество() = 0 Тогда
			...
			ТаблицыЗаполнены = ?(ПараметрыЗадания.ИспользоватьЗадания И НЕ СверкаДокументов = Истина
							...
							И  ТаблицаNavision[0].КВО_СТР = ПараметрыЗадания.ТабличнаяЧасть.Количество
					//		И  ТаблицаNavision[0].КВО_СТР > 0
			;
...
Процедура УдалитьИзОчередиЗаданийИТранспортныхТаблиц(IDN, ПараметрыЗадания, Назначение, Отладка = Ложь)
	...
	RequestText = ВнешниеИсточники.УточнитьТекстЗапроса(СтрЗаменить(
	"DELETE FROM [Euro2009].[dbo].[Европродукт 2009$Очередь Заданий]
 	|WHERE [ID1] = '#NUMBER#'
 	| 	AND [Код Филиала] = '#FILIAL#'
 	|	AND [Тип Задания] = '#TYPETASK#'
 	|
 	|DELETE FROM [Euro2009].[dbo].[Европродукт 2009$Exch RR HEAD]
 	|      WHERE [Документ Но_] = '#NUMBER#' AND [Тип Документа] = '#TYPEDOC#'
 	|
 	|DELETE FROM [Euro2009].[dbo].[Европродукт 2009$Exch RR LINE]
	|WHERE [Документ Но_] = '#NUMBER#' AND [Тип Документа] = '#TYPEDOC#'
	|", "#HEAD#", ПараметрыЗадания.Имена.HEAD)
	, Перечисления.ВнешниеИсточники.Navision
	);
...
Функция ПеремещениеПолучитьИзNavision(НомерДокумента, КодФилиала, НомерЗадания, ТипЗадания, ПараметрыЗадания, СсылкиОбработаны)	Экспорт	//	Экспорт для отладки
	...
	Если НЕ ТаблицаNavision.Количество() = 0 Тогда
		...
		//	LNK 20.03.2022 06:52:50
			...
			|			ТОГДА ТаблицаВедущая.МагазинОтправитель.Организация
			|		КОГДА ТаблицаВедущая.МагазинПолучатель.СкладУправляющейСистемы = ЛОЖЬ
			|			ТОГДА ТаблицаВедущая.МагазинПолучатель.Организация
		...
		//	Запрос.УстановитьПараметр("УчестьДокумент" , ТипЗадания = "ДОК_ПЕРЕМЕЩ_УЧТЕНО");
			...
			Если Выборка.Следующий() Тогда
				...
				Иначе
					...
					ДокументОбъект.ЗаказПокупателя = РезервированиеТоваровСервер.ПолучитьЗаказПокупателяПоНомеруМагазину(
...
Функция ПередатьДанныеNavision(RequestText, IDN, ПараметрыЗадания, ПроверитьВыполнение, Отладка)
	...
	Если НЕ Отладка = Истина Тогда
		...
		Если НЕ Connection = Неопределено Тогда
			...
			Исключение
				...
				ВызватьИсключение ТекстОшибки + Символы.ПС + RequestText;
	...
	Если ПроверитьВыполнение Тогда
		...
		Если ПараметрыЗадания.ИспользоватьЗадания Тогда
			...
			ЗначениеВозврата = ОчередьПолучитьНомер(ПараметрыЗадания.ТипЗадания, IDN,, Отладка);
		...
		Иначе
			...
			//	"100" - для отличия от "1".. по сути имеет значение отличие от "0".
				ЗначениеВозврата = ?(ЕстьВЗаголовках(ПараметрыЗадания.ТипДокумента, IDN,, ПараметрыЗадания, Отладка), 100, 0);
...
КонецФункции // ТекстЗапросаДокументыДляСверкиNavision()

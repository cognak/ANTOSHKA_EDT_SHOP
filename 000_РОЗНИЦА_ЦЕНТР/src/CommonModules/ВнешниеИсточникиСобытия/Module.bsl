	...
	Иначе
		...
		ИначеЕсли НЕ ОбъектДанных.ПометкаУдаления ИЛИ ПараметрыВыгрузки.СверкаДокументов Тогда
			...
			Для каждого СтрокаТаблицыЗаданий Из ТаблицаЗаданий Цикл
				...
				ВыполнитьВыгрузку = ?(ПараметрыВыгрузки.СверкаДокументов = Истина ИЛИ ПараметрыВыгрузки.ОбновитьТаблицы = Истина
					, Истина
...
Функция ПараметрЗапросаТоварыЗапросаДоступности(ДокЗапросДоступности) 
	...
	Запрос.Текст = 
		...
		|			И (НЕ(ЗапросДоступностиТоваровТовары.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваров.Удалён)
...
Функция ПолучитьПараметрыВыгрузки(УзелСсылка, СверкаДокументов, ОбновитьТаблицы, Отладка)

	ПараметрыВыгрузки = Новый Структура(
		"УчетнаяПолитика, ТаблицаСсылочныхТипов, СверкаДокументов, ОбновитьТаблицы, Отладка"
...
Функция ЭтоОбновление(ПараметрыЗадания)	//	LNK 27.07.2021 18:50:13
	...
	Возврат ПараметрыЗадания.СверкаДокументов = Истина ИЛИ ПараметрыЗадания.ОбновитьТаблицы = Истина;

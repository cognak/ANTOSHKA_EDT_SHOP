				...
				Попытка
					...
					//	Получение измененного запроса доступности со склада через Navision.
						...
						Запрос.Текст =
							...
							|
							|УПОРЯДОЧИТЬ ПО
							|	ЗапросДоступностиТоваров.Дата УБЫВ";
						...
						Если ВыборкаЗД.Следующий() Тогда
							...
							Если ОчередьNavisionВыборка.ID2 = "ОТМЕНЕН" Тогда
								...
								ИначеЕсли ВыборкаЗД.Ссылка.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Отменён Тогда
							...
							ИначеЕсли ОчередьNavisionВыборка.ID2 = "ОТГРУЖЕН" Тогда
								...
								ИначеЕсли ВыборкаЗД.Ссылка.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Согласован Тогда
	...
	Пока УзлыВыборка.Следующий() Цикл
		...
		Для каждого ИмяКласса Из КлассификаторСостава.Классы Цикл
			...
			Пока ВыборкаИзменений.Следующий() Цикл
				...
				ПараметрыВыгрузки.ОбновлениеИнтернетЗаказов = 
...
//	---------------------------------------------------------------------------------------
	...
	+ ?(ПараметрыЗадания.ИспользоватьЗадания
		, Символы.ПС + ОчередьУстановкиЗаданияТекстЗапроса(ПараметрыЗадания.ВремяВыполнения, ПараметрыЗадания.ТипЗадания, ОбъектДанных.IDN)
		, "") + Символы.ПС
	+
	"END TRY
	|BEGIN CATCH
...
//	гггг-мм-ддТчч:мм:сс.ммм (без пробелов) Note: Если значение миллисекунд (ммм) равно 0, оно не не отображается.
//	Например, значение «2012-11-07T18:26:20.000» будет отображено как «2012-11-07T18:26:20».

	Возврат НомерЗадания;

КонецФункции // ОчередьУстановитьЗадание()
...
КонецФункции // ОчередьУстановкиЗаданияТекстЗапроса()

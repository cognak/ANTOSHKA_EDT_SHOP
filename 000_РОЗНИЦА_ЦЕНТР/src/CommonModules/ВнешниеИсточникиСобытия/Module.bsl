...
//					ОтправкаСоСклада = ТипЗнч(ОбъектДанных.МагазинОтправитель) = Тип("СправочникСсылка.Склады");
//					ПолучательИМ = ОбъектДанных.МагазинПолучатель = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику().ИнтернетМагазин;
//					
//					Если ОтправкаСоСклада И ПолучательИМ Тогда
//						ПланыОбмена.УдалитьРегистрациюИзменений(ОбщегоНазначенияКлиентСервер.AAD(УзлыВыборка.Узел), ОбъектДанных);
//						Продолжить;
//					КонецЕсли;

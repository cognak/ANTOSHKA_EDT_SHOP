...
Процедура СобытияВходящиеЗаданияNavision(Назначение, Отладка)
	...
	ИначеЕсли Назначение = "ДатаЗапрета" Тогда	//	LNK 21.05.2021 08:32:23

		ИменаЗаданийNavision.Добавить("СКЛАД_ПЕРИОД");
		
		РаботаСДокументами   = ЛОЖЬ;
		УсловиеОтбора = "= 'RR' AND [Тип Задания] = 'СКЛАД_ПЕРИОД'";  //А++
	...
	ИначеЕсли Назначение = "ТорговыеМарки" Тогда	//	А++ 20250425 по задаче обновления торговых марок
		
		ИменаЗаданийNavision.Добавить("ОБНОВИТЬ_ТМ");
		
		РаботаСДокументами   = ЛОЖЬ;
		УсловиеОтбора = "= 'RR' AND [Тип Задания] = 'ОБНОВИТЬ_ТМ'";  //А++
...
//	отключимся от внешнего источника...
	...
	Для каждого ИмяЗаданияNavision Из ИменаЗаданийNavision Цикл
					...
					//	Остановка = 1;
						...
						Если ВыборкаЗД.Следующий() Тогда
							...
							Если ОчередьNavisionВыборка.ID2 = "ОТМЕНЕН" Тогда
								...
								Если ОчередьNavisionВыборка.ВремяСоздания > Дата(2026,06,16) Тогда

									СтруктураКомментария = РегистрыСведений.КомментарийПоСтрокам.СтруктураКомментария();

									СтруктураКомментария.КлючСвязи = 0;
									СтруктураКомментария.Комментарий = ОчередьNavisionВыборка.Примечание;
									СтруктураКомментария.Автор = ПараметрыСеанса.ТекущийПользователь;
									СтруктураКомментария.Документ = ВыборкаЗД.Ссылка;
									СтруктураКомментария.Дата = ТекущаяДатаСеанса();
									СтруктураКомментария.УзелСоздания = Справочники.СтруктураУзлов.НайтиПоКоду("104");

									РегистрыСведений.КомментарийПоСтрокам.ЗаписьКомментария(СтруктураКомментария);
									
								КонецЕсли;
								
								Если ДанныеСостояния = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Отменён
										Или ОчередьNavisionВыборка.ВремяСоздания > Дата(2026,06,16) Тогда
									...
									ЗапросДоступностиОбъект.ДополнительныеСвойства.Вставить("НеПроверятьРегистрациюВОбменNavision", Истина);
								...
								ОчередьОбновитьЗадание(ОчередьNavisionВыборка.ЗаданиеНо
									,
									, ?(ЗаданиеВыполнено, 1, 0)
									, ОчередьNavisionВыборка.Примечание);
							...
							ИначеЕсли ОчередьNavisionВыборка.ID2 = "ОТГРУЖЕН" Тогда
								...
								Запрос.Текст =
									"ВЫБРАТЬ
									|	ТаблицаКлиентов.КодКлиента КАК КодКлиента
									...
									|	ТаблицаКлиентов КАК ТаблицаКлиентов
									|ГДЕ
									|	НЕ Контрагенты.IDN = ТаблицаКлиентов.КодКлиента
									|	И ЗапросДоступностиТоваров.Ссылка = &Ссылка";
								
								Запрос.УстановитьПараметр("Ссылка", ВыборкаЗД.Ссылка);
								Запрос.УстановитьПараметр("ТаблицаКлиентов", ТаблицаNavision);
								...
								Пока Выборка.Следующий() Цикл
									...
									КонтрагентОбъект.IDN = СокрЛП(Выборка.КодКлиента);
								...
								ИначеЕсли Не (ВыборкаЗД.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Новый
										Или ВыборкаЗД.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ВРаботе)
									И ТипЗнч(ЗапросДоступностиОбъект.МагазинОтправитель) = Тип("СправочникСсылка.Склады") Тогда
		...
		//	LNK 05.06.2024 07:51:58
			...
			ИначеЕсли ИмяЗаданияNavision = "СКЛАД_ПЕРИОД" Тогда
				
				//А++ 20250420 автоматизация закрытия периода						
				УстановитьДатуЗапретаРедактирования(ОчередьNavision)    
				
			ИначеЕсли ИмяЗаданияNavision = "ОБНОВИТЬ_ТМ" Тогда
				
				//А++ 20250420 автоматизация закрытия периода						
				ОбновитьТоварныеКатегории(ОчередьNavision)    
...
//	Команда для отработки специфики задания в строке
	...
	//	LNK 09.10.2019 08:29:59	- разницу допускаем в 1 ГРН.
		...
		ИначеЕсли ПараметрыВыгрузки.УчетнаяПолитика.ИнтернетМагазин = ОбъектДанных.Магазин
				И НачалоДня(ОбъектДанных.Дата) >= НачалоДня(Дата(2026, 6, 8)) Тогда
...
Функция ПередатьДокументNavision(ОбъектДанных, ПараметрыЗадания, ПараметрыВыгрузки)
	...
	RequestText = УстановитьПараметрыВТекстеВнешнегоЗапроса(Шаблон_RR_HEAD, Результаты[1].Колонки, ВыборкаШапка);
...
//	---------------------------------------------------------------------------------------
	...
	//СЮП 27.11.2024 17.53.16
	ХешСовпадает = Ложь; 
	Если ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ЗапросДоступностиТоваров") Тогда
		ХешСовпадает = РегистрыСведений.КлючевыеАтрибутыЗапросовДоступности.ПроверкаИЗаписьХеша(ОбъектДанных, RequestText);
	КонецЕсли;
	
	Если Не ХешСовпадает Тогда
		//СЮП 17.06.2024 19.06.26 
		Если ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ВозвратИнтернетЗаказа") Тогда
			Если ОбъектДанных.ТипВозврата = Перечисления.ТипыВозвратаИнтернетЗаказа.АвтоВозврат Тогда
				НомерIDN = "WEB_ВОЗВРАТ";
			Иначе
				НомерIDN = ПараметрыЗадания.Номер;
			КонецЕсли;
		Иначе
			НомерIDN = ОбъектДанных.IDN;
		КонецЕсли;
		RequestText = RequestTextШапка + RequestText + Символы.ПС
	...
	//	ВыборкаШапка.NUMBER
		...
		Если ТипЗнч(ОбъектДанных) = Тип("Структура") Тогда
			...
			ЖурналСобытий.Регистрация(ИмяСобытия("PUT_NAV", ПараметрыВыгрузки, Истина) + ОбъектДанных.IDN
...
//	LNK 25.10.2019 11:06:57 - специфика Документы.ЗаказПокупателя
	...
	//СЮП 12.05.2025 12.27.02
	//Тип клиента интернет-заказа - 1 - Юридическое лицо, 0 - Физическое лицо 
	Параметры.Вставить("ТипКлиента"      , 0);
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ЗаказПокупателя") Тогда	//	LNK 25.10.2019 10:12:03
		...
		ДанныеЗаказа = ДанныеЗаказаПокупателя(ОбъектДанных.Ссылка);
		...
		Параметры.Вставить("КодКлиента"      , ДанныеЗаказа.КодКлиента);
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ЗапросДоступностиТоваров") Тогда
		...
		Если ОбъектДанных.ОператорДоставки = Перечисления.ОператорыДоставки.ВнутренняяЛогистика Тогда
			...
			Иначе
				СкладПродажи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДанных.МагазинПолучатель, "СкладПродажи");
				Параметры.Вставить("Склад"           , СкладПродажи);
			КонецЕсли;
		...
		Иначе
			...
			Иначе 
				СкладПродажи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДанных.МагазинОтправитель, "СкладПродажи");
				Параметры.Вставить("Склад"           , СкладПродажи);
			КонецЕсли;
		...
		Параметры.Вставить("КодОтдела"       , СокрЛП(ДанныеЗаказа.КодОтдела));
		Параметры.Вставить("Контрагент"      , ДанныеЗаказа.Контрагент);
		Параметры.Вставить("ВходящийДокумент", Ложь);
		...
		Параметры.Вставить("ПочтаКлиента"    , СокрЛП(ДанныеЗаказа.ПочтаКлиента));
		Параметры.Вставить("НомерТелефона"   , СокрЛП(ДанныеЗаказа.НомерТелефона)); 
		Параметры.Вставить("ТипКлиента"      , СокрЛП(ДанныеЗаказа.ТипКлиента)); 
		...
		Если Параметры.ТипДокумента = "WEB_ЗАКАЗ" Тогда
			Параметры.Вставить("ДатаОтгрузки"    , ОбъектДанных.Дата);
		Иначе
		...
		Параметры.Вставить("МетодОплаты"     , СокрЛП(ДанныеЗаказа.ТипОплаты));
		Параметры.Вставить("СтатусОплаты"    , СокрЛП(ДанныеЗаказа.СтатусОплаты));
		Параметры.Вставить("СуммаЗаДоставкуКлиенту", ОбъектДанных.СуммаОплатыДоставки);
		Параметры.Вставить("ЗатратыОператора"      , 0);

		Параметры.Вставить("НомерВнешнегоДокумента", СокрЛП(ДанныеЗаказа.НомерЗаказаНаСайте));

		Параметры.Вставить("УчитыватьНДС"   , ДанныеЗаказа.УчитыватьНДС);
		Параметры.Вставить("ЦенаВключаетНДС", ДанныеЗаказа.ЦенаВключаетНДС);
		Параметры.Вставить("СуммаДокумента" , ОбъектДанных.СуммаДокумента); 
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ВозвратИнтернетЗаказа") Тогда
		...
		Иначе
			Склады = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектДанных.МагазинВозврата, "СкладПродажи");
			Параметры.Вставить("СкладПолучатель", Склады);
			Параметры.Вставить("Склад"          , Склады);
		КонецЕсли;

		Если ОбъектДанных.ВозвратДоставки Тогда
			Параметры.Вставить("СуммаЗаДоставкуКлиенту", ОбъектДанных.СуммаДоставки);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ПеремещениеТоваров")
		...
		Если ТипЗнч(ОбъектДанных) = Тип("ДокументОбъект.ПеремещениеТоваров") Тогда
			...
			ИначеЕсли СкладWEB_OD = "WEB_OD" Тогда
			 ...
			 //А++ по запросу выбираем максимальную дату из ордеров или перемещения  Запрос.Параметры.Период
			 МаксДата = ПолучитьМаксимальнуюДатуОрдеров(ОбъектДанных.Ссылка);
			 Если Параметры.Свойство("Период") Тогда
				 Параметры.Период = МаксДата;
			 Иначе
				Параметры.Вставить("Период" + МаксДата);
...
Функция ПолучитьМаксимальнуюДатуОрдеров(Перемещение)  //А++ по запросу выбираем максимальную дату из ордеров или перемещения  Запрос.Параметры.Период
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПриходныйОрдерНаТовары.Дата КАК ДОК,
		|	""МДата"" КАК ДатаДокументов
		|ПОМЕСТИТЬ ВР_Даты
		|ИЗ
		|	Документ.ПриходныйОрдерНаТовары КАК ПриходныйОрдерНаТовары
		|ГДЕ
		|	ПриходныйОрдерНаТовары.ДокументОснование = &Перемещение
		...
		|ГДЕ
		|	РасходныйОрдерНаТовары.ДокументОснование = &Перемещение
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ВР_Даты.ДОК) КАК ДОК,
		|	ВР_Даты.ДатаДокументов КАК ДатаДокументов
		|ИЗ
		|	ВР_Даты КАК ВР_Даты
		|
		|СГРУППИРОВАТЬ ПО
		|	ВР_Даты.ДатаДокументов";
	...
	Если РезультатЗапроса.Пустой() Тогда
		Возврат Перемещение.Дата;
	Иначе
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		ВыборкаДетальныеЗаписи.Следующий();
		Возврат ВыборкаДетальныеЗаписи.ДОК;	
	КонецЕсли;
...
Функция ТекстЗапросаПередачиДокумента()	//	LNK 28.07.2021 05:42:39
	...
	|			ТОГДА ВЫБОР
	|				КОГДА
	|					Товары.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат)
	|					ТОГДА ""ИГРУШКИ""
	...
	|	Ведущая.CLIENTTYPE КАК CLIENTTYPE
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		&ОтклоненийНет КАК NODEVIAT,
	|		&ТипКлиента КАК CLIENTTYPE) КАК Ведущая
...
//	6 - для товаров "ОплатаЧастями"
	...
	|		И
	|			ТаблицаРозница.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.СкидочныйКупон)
	|			ТОГДА ""6005308""
	...
	|			ИЛИ ТаблицаТовары.УслугаБанка.ТипУслуги = ЗНАЧЕНИЕ(Перечисление.ТипыУслугБанка.ПакетШкольника)
	...
	|			ИЛИ ТаблицаВозвраты.УслугаБанка.ТипУслуги = ЗНАЧЕНИЕ(Перечисление.ТипыУслугБанка.ПакетШкольника)
	...
	|		И
	|			ТаблицаРеализация.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.СкидочныйКупон)
	|			ТОГДА ""6005308""
...
Функция ДанныеЗаказаПокупателя(СсылкаНаЗаказ)
	...
	|	ЗаказПокупателя.Номер КАК НомерЗаказаНаСайте,
	...
	|			ПО ЗаказПокупателя.Организация = Организации.Ссылка
...
Функция ПолучитьКлючевыеПараметрыЗадания(ТипЗначения, ОбъектДанных, КлючТипа = Неопределено, ВыгрузкаДанных = Истина, ПараметрыВыгрузки = Неопределено)	Экспорт
	...
	ИначеЕсли ТипЗначения = Тип("ДокументСсылка.ВозвратИнтернетЗаказа") Тогда
		...
		Если ОбъектДанных.ТипВозврата = Перечисления.ТипыВозвратаИнтернетЗаказа.АвтоВозврат Тогда
			ПараметрыЗадания.ТипЗадания   = "WEB_ВОЗВРАТ_СОЗДАТЬ";
			ПараметрыЗадания.ТипДокумента = "WEB_ВОЗВРАТ_СОЗДАТЬ";
		Иначе
...
//	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#TIME#"	, Формат(Период, "ДФ=yyyy-MM-ddTЧЧ:мм:сс.000"));	//	ISO8601 
	...
	Если СокрЛП(ТипЗадания) = "WEB_ВОЗВРАТ_СОЗДАТЬ" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#FILIAL#"	, "OD");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#FILIAL#"	, "NN");
	КонецЕсли;
...
Процедура УстановитьДатуЗапретаРедактирования(ОчередьNavision)	Экспорт  //А++ обработка события установки даты запрета редактирования "СКЛАД_ПЕРИОД"
	УстановитьПривилегированныйРежим(Истина);
	ТаблицаОчереди = Новый ТаблицаЗначений;
	ТаблицаОчереди.Колонки.Добавить("НомерЗадания", Новый ОписаниеТипов("Число"));
	ТаблицаОчереди.Колонки.Добавить("ID1"         , Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(20)));
	ТаблицаОчереди.Колонки.Добавить("ID2"   , Новый ОписаниеТипов("ДАТА"));
	ТаблицаОчереди.Колонки.Добавить("ВремяСоздания"   ,Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(20)));
	...
	Пока ОчередьNavisionВыборка.Следующий() Цикл
		...
		СтрокаТаблицы.ID1          = ОчередьNavisionВыборка.ID1;
		
		МассивДатаИВремя = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, ОчередьNavisionВыборка.ID2,, ".");
		Если МассивДатаИВремя.Количество() =3 Тогда
			СтрокаТаблицы.ID2    = Дата("20"+МассивДатаИВремя[2]+МассивДатаИВремя[1]+МассивДатаИВремя[0]);
		Иначе	
			СтрокаТаблицы.ID2 = Дата(1,1,1);
		КонецЕсли;
			
		СтрокаТаблицы.ВремяСоздания = ОчередьNavisionВыборка.ВремяСоздания; 	

	КонецЦикла;
	
	ТаблицаОчереди.Сортировать("ID2");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для Каждого СтрЗ Из ТаблицаОчереди Цикл
			Если СтрЗ.ID1 = "ЗАКРЫТЬ" Тогда
				Если ВыборкаДетальныеЗаписи.ДатаЗапрета < СтрЗ.ID2 Тогда
					...
					МенеджерЗаписи.ДатаЗапрета = СтрЗ.ID2;
					МенеджерЗаписи.ОписаниеДатыЗапрета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("АВТО запись из NAV. Дата измененения %1. Дата задания из NAV %2. Номер задания NAV %3. ",
														СтрЗ.ID2,"ДФ='dd.MM.yyyy HH:mm:ss'", СокрЛП(СтрЗ.ВремяСоздания),Формат(СтрЗ.НомерЗадания,"ЧДЦ=; ЧС=; ЧРГ=; ЧГ="));
					МенеджерЗаписи.Записать();					
				КонецЕсли;
			КОнецЕсли;
	...
	Для Каждого СтрЗ Из ТаблицаОчереди Цикл
		...
		ОчередьОбновитьЗадание(СтрЗ.НомерЗадания,, 1);
		
	КонецЦикла;
КонецПроцедуры


Процедура ОбновитьТоварныеКатегории(ОчередьNavision)	Экспорт  //А++ обработка события установки даты запрета редактирования "ОБНОВИТЬ_ТМ"

	
	ТаблицаОчереди = Новый ТаблицаЗначений;
	ТаблицаОчереди.Колонки.Добавить("НомерЗадания", Новый ОписаниеТипов("Число"));
	ТаблицаОчереди.Колонки.Добавить("ID1"         , Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(40)));
	ТаблицаОчереди.Колонки.Добавить("ID2"         , Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(40)));
	ТаблицаОчереди.Колонки.Добавить("ID3"         , Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(40)));
	ТаблицаОчереди.Колонки.Добавить("ID4"         , Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(40)));
	ТаблицаОчереди.Колонки.Добавить("ID5"         , Новый ОписаниеТипов("Число"));
	ТаблицаОчереди.Колонки.Добавить("ID6"         , Новый ОписаниеТипов("Число"));
	ТаблицаОчереди.Колонки.Добавить("IDN"         , Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(40)));
	...
	Пока ОчередьNavisionВыборка.Следующий() Цикл
		...
		СтрокаТаблицы.ID1          = ОчередьNavisionВыборка.ID1;
		СтрокаТаблицы.ID2          = ОчередьNavisionВыборка.ID2;
		СтрокаТаблицы.ID3          = ОчередьNavisionВыборка.ID3;
		СтрокаТаблицы.ID4          = ?(СокрЛП(ОчередьNavisionВыборка.ID4)="",ОчередьNavisionВыборка.ID3,ОчередьNavisionВыборка.ID4);
		СтрокаТаблицы.ID5          = ОчередьNavisionВыборка.ID5;
		СтрокаТаблицы.ID6          = ОчередьNavisionВыборка.ID6;  
		СтрокаТаблицы.IDN          = СокрЛП(ОчередьNavisionВыборка.ID3); 
		
		
	КонецЦикла; 
	
	
	//Тип Задания - "ОБНОВИТЬ ТМ"
	//Код Филиала - "RR"
	//ID1 - Код Проекта
	//ID2 - Тип справочника
	//ID3 - Код(IDN)
	//ID4 - Название
	//ID5 - Блокирован(0/1)
	//ID6 - Высокомаржинальная ТМ(0/1)
	
	
	
	Для Каждого СтрОчереди Из ТаблицаОчереди Цикл     
		ВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоРеквизиту("IDN",СтрОчереди.ID1);
		Если ВидНоменклатуры = Справочники.ВидыНоменклатуры.ПустаяСсылка() Тогда
			Продолжить;                  
		КонецЕсли;
		
		КодМарки = Формат(ВидНоменклатуры.НомерПроекта,"ЧДЦ=0; ЧРГ=; ЧГ=")+ СокрЛП(СтрОчереди.IDN);
		
		ЭлементСсылка = Справочники.Марки.НайтиПоРеквизиту("IDN", КодМарки);
		Если ЭлементСсылка = Справочники.Марки.ПустаяСсылка()	Тогда
			ЭлементСсылка = Справочники.Марки.НайтиПоРеквизиту("IDN", Лев(КодМарки,20));	
		КонецЕсли;
		Если ЭлементСсылка = Справочники.Марки.ПустаяСсылка()	Тогда
			
			ЭлементСсылка = Справочники.Марки.НайтиПоКоду(Лев(КодМарки,21));
		...
		Элемент.Код = КодМарки;
		Элемент.Наименование = СтрОчереди.ID4;
		Элемент.IDN = КодМарки;  
		Элемент.Блокирован = ?(СтрОчереди.ID5 = 0,ЛОЖЬ,Истина);
		Элемент.ВысокомаржинальнаяТМ = ?(СтрОчереди.ID6 = 0,ЛОЖЬ,Истина);
	...
	Для Каждого СтрОчереди Из ТаблицаОчереди Цикл     
		ОчередьОбновитьЗадание(СтрОчереди.НомерЗадания,,1);
	КонецЦикла;	
КонецПроцедуры

...
Процедура ОбработкаСобытийNavision(Назначение, Входящие, Исходящие) Экспорт
	...
	ИначеЕсли Назначение = "ПолучитьЦеныCompetera" Тогда

		ВнешниеИсточникиЦенообразование.ПолучитьЦеныCompeteraОтGraphQL(ТекущаяДата());
...
//	2-й этап. Формирование информативных остатков.
...
Функция ТаблицаОстатковЦентральныхСкладовNavision(НомерОперацииNavision)
	...
	|	T.КодСклада,
	|	T.Количество,
	|	T.[Операция Но_] AS ЗАДАНИЕНО
	|FROM [dbo].[GetStockBalanceGoods](#OPNUMBER#) AS T"
	, "#OPNUMBER#"
...
//	LNK 13.06.2023 10:47:57
	ДанныеNavision.НомерОперацииNavision = ?(ДанныеNavision.ОстаткиNavision.Количество() = 0
		, 0
		, ДанныеNavision.ОстаткиNavision[0].ЗАДАНИЕНО);
...
Процедура СформироватьИнформативныеОстаткиТоваровПоСкладам(Запрос, НомерОперацииNavision)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Количество = 0 И Выборка.КоличествоВРезерве = 0 Тогда
		...
		Иначе
			...
			Если НаборЗаписей.Количество() = 0 Тогда
					...
					ЗаписьНабора.Склад		  = НаборЗаписей.Отбор.Склад.Значение;
			...
			ЗаписьНабора.Количество = ?(НомерОперацииNavision = 0, Выборка.Количество, ЗаписьНабора.Количество + Выборка.Количество);

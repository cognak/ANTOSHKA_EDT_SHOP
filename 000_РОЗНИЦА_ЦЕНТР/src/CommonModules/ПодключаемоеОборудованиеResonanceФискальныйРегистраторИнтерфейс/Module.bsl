...
Функция ДобавитьСлип(ОбъектДрайвера,Параметры,ИндексОплаты) Экспорт
	...
	Если НЕ Параметры.Свойство("ПлатежныеРеквизиты") Тогда
		Возврат Ложь;	
	КонецЕсли;
	Если НЕ (Параметры.Свойство("НомерТранзакции") И ЗначениеЗаполнено(Параметры.НомерТранзакции)) Тогда
		Возврат Ложь;	
	КонецЕсли;
	
	ПлатежныеРеквизиты = Параметры.ПлатежныеРеквизиты.Получить(Параметры.НомерТранзакции);
	...
	Если ВсеРеквизитыЗаполнены Тогда
		...
		//2025-06-17 sa Теперь нужно выводить сумму банковской комиссии в чек.
		//Если ЗначениеЗаполнено(ПлатежныеРеквизиты.commission) Тогда
		...
		//КонецЕсли;
	Иначе
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"РЕКВІЗИТИ РОЗРАХУНКІВ")=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"НАЗВА БАНКА: "+СокрЛП(ПлатежныеРеквизиты.bank_id))=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"ID ЕКВАЙРА: "+СокрЛП(ПлатежныеРеквизиты.term_id))=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"ID ПРИСТРОЮ: "+СокрЛП(ПлатежныеРеквизиты.term_id))=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"ВИД ОПЕРАЦІЇ: "+СокрЛП(ПлатежныеРеквизиты.oper_type))=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"ЕПЗ: "+СокрЛП(ПлатежныеРеквизиты.cardmask))=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"ПЛАТІЖНА СИСТЕМА:"+СокрЛП(ПлатежныеРеквизиты.paysys))=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"КОД АВТ.: "+СокрЛП(ПлатежныеРеквизиты.auth_code))=1;
		Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"КОД ТРАНЗ.: "+СокрЛП(ПлатежныеРеквизиты.rrn))=1; 
		//Если ПлатежныеРеквизиты.Свойство("ПлатежныеРеквизиты") И ЗначениеЗаполнено(ПлатежныеРеквизиты.commission) Тогда
		//	Результат = ОбъектДрайвера.FreeTextLine(0,0,0,"КОМІСІЯ БАНКУ: "+СокрЛП(ПлатежныеРеквизиты.commission+" ГРН"))=1; 
		//КонецЕсли;
		Результат = Истина;
	КонецЕсли;

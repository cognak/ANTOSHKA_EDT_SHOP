...
Функция ЗавершитьНефискальныйЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	МассивСтрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПараметрыПодключения.СтрокаЛога,Символы.ПС);
	н = 0;
	Для Каждого Строка_ Из МассивСтрок Цикл 
		н = н + 1;                   
		ОбластьЯчеек = Таб.Область(н,1,н,1);
		ОбластьЯчеек.СодержитЗначение = Истина;
		ОбластьЯчеек.Значение = Строка(Строка_);
	КонецЦикла;
...
Функция ОткрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ЧекВозврата, ФискальныйЧек, ВыходныеПараметры) Экспорт
	...
	СтрокаЛога = СтрокаЛога + "===== " + ?(ЧекВозврата = Истина, "Відкриття чека повернення", "Відкриття чека продажу") + " =====" + Символы.ПС;

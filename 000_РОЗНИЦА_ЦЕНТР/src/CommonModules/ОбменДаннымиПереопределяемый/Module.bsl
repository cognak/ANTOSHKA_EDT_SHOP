...
Процедура ПолучитьПланыОбмена(ПланыОбменаПодсистемы) Экспорт
	...
	//ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменВнешнимиЗаданиями);
	//ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменВнешнимиЗаданиямиС1СДокументооборот);
...
//	LNK 28.08.2016 09:57:25
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменNavisionРозница);
//	LNK 01.09.2017 12:27:08
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ПоМагазинуКонвертация);
//	LNK 13.03.2018 15:45:13
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ПоУстройствамАнтошка);
//	LNK 28.08.2019 10:33:11
...
Функция ПрефиксИнформационнойБазыПоУмолчанию() Экспорт
	...
	Если ОбменДаннымиПовтИсп.ГлавныйУзел() = Неопределено Тогда
		...
		ПрефиксПоУмолчанию = НСтр("ru = 'РТ'");
		
	КонецЕсли;
	
	Возврат ПрефиксПоУмолчанию;

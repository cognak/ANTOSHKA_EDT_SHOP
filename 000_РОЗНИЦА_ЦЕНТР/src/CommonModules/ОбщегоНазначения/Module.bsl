...
// 
Функция ЗначенияРеквизитовОбъекта(Ссылка, Реквизиты, ВызыватьИсключение = Истина) Экспорт
	...
	//	LNK 03.03.2017 13:59:14
	//	Тут валиться может, вроде, только по обращению к метаданным "Ссылка.Метаданные()"... либо какая блокировка.
		ТекстОшибки = ОписаниеОшибки();

		Если ВызыватьИсключение = Истина Тогда

			ВызватьИсключение "При обращении к метаданным ссылки «" + Ссылка + "» возникла ошибка:" + Символы.ПС + ТекстОшибки;
...
// 
Функция ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита, ЗначениеПоУмолчанию = Неопределено, ВызыватьИсключение = Истина) Экспорт
	
	Результат = ЗначенияРеквизитовОбъекта(Ссылка, ИмяРеквизита, ВызыватьИсключение);
//	LNK 22.11.2016 13:13:58
	Возврат ?(ЗначениеЗаполнено(Результат[ИмяРеквизита]) ИЛИ ЗначениеПоУмолчанию = Неопределено
...
Функция ТаблицаЗначенийИзXML(Текст) Экспорт
	...
	Если ТипДанных=СериализаторXDTO.XMLТип(Тип("ТаблицаЗначений")) 
	 ИЛИ ТипДанных=СериализаторXDTO.XMLТип(Тип("ДеревоЗначений")) Тогда
		...
		Возврат Новый Структура("Данные, Описание", Данные);
...
//	LNK 04.01.2018 10:06:23
Функция ОбъектМетаданныхПоПолномуИмени(Знач ПолноеИмя, ВызыватьИсключение = Истина)	Экспорт

	Перем ОбъектМетаданных;

	Попытка

		ЧастиИмени = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, СокрЛП(ПолноеИмя),, ".", 2,, Новый ОписаниеТипов("Строка"));
		...
		ИначеЕсли ВызыватьИсключение = Истина Тогда
	...
	Исключение
		...
		Если ВызыватьИсключение = Истина Тогда
			...
			ТекстОшибки = ОписаниеОшибки();
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка типа объекта метаданных (%1): %2'"), КлассОМ, ТекстОшибки);
...
КонецФункции // ОбъектМетаданныхПоПолномуИмени()
...
//
Функция ЗначениеСсылочногоТипа(Значение, ТолькоОбъектные = Ложь) Экспорт
	...
	Если НЕ ТолькоОбъектные = Истина Тогда	//	LNK 17.11.2020 14:09:01
...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных) Экспорт
	...
	ИначеЕсли ТаблицаКлючей.Количество() > 1 Тогда

		СтрокаКлючей = "";	//	LNK 30.09.2021 08:46:04

		Для каждого СтрокаТаблицы Из ТаблицаКлючей Цикл

			СтрокаКлючей = СтрокаКлючей + ?(ПустаяСтрока(СтрокаКлючей), "", Символы.ПС)
			+ СокрЛП(СтрокаТаблицы.Ссылка.УникальныйИдентификатор());
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().
			           ...
			           |в справочнике ""Идентификаторы объектов метаданных"".
			           |
			           |Список ключей «УникальныйИдентификатор»:
			           |%2
			           |
	...
	//	Проверка соответствия ключа объекта метаданных полному имени объекта метаданных
		РезультатПроверки = Справочники.ИдентификаторыОбъектовМетаданных.КлючОбъектаМетаданныхСоответствуетПолномуИмени(ТаблицаКлючей[0]);
	...
	Иначе
		...
		Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда	//	LNK 30.09.2021 08:32:52

			ВызватьИсключение ТекстОшибки;

		Иначе

			ЖурналСобытий.Регистрация("ИДЕНТИФИКАТОРЫ_МЕТАДАННЫХ", УровеньЖурналаРегистрации.Ошибка
		...
		//	LNK 11.10.2021 06:24:09 - Погасим возможную ошибку..
			Если ТаблицаКлючей.Количество() = 0 Тогда

				ТаблицаКлючей.Добавить().Ссылка = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка();
	...
	Возврат ТаблицаКлючей[0].Ссылка;
...
//	LNK 20.11.2020 13:00:32
Функция ОбщийРеквизитИспользуется(МетаданныеОбъекта, ИмяОбщегоРеквизита)	Экспорт

	ЭлементСостава = Метаданные.ОбщиеРеквизиты[ИмяОбщегоРеквизита].Состав.Найти(МетаданныеОбъекта);

	РеквизитИспользуется = НЕ ЭлементСостава = Неопределено;
	...
	Если РеквизитИспользуется Тогда
		...
		Иначе
			...
			РеквизитИспользуется = Метаданные.ОбщиеРеквизиты[ИмяОбщегоРеквизита].АвтоИспользование = Метаданные.СвойстваОбъектов.АвтоИспользованиеОбщегоРеквизита.Использовать;

		КонецЕсли;

	КонецЕсли;

	Возврат РеквизитИспользуется;
...
Функция ЭтоРазделенныйОбъектМетаданных(Знач ОбъектМетаданных, Знач Разделитель) Экспорт
	...
	Возврат Ложь;	//	LNK 04.06.2018 11:06:27
...
#Область ПроцедурыФункцииДляРаботыСХранилищемПаролей
...
Процедура ЗаписатьДанныеВБезопасноеХранилище(Владелец, Данные, Ключ = "Пароль") Экспорт
	...
	Если НЕ ПривилегированныйРежим() Тогда	//	LNK 30.12.2020 05:34:14
...
Функция ЕстьРеквизитОбъекта(Объект, ИмяРеквизита) Экспорт
	...
	Возврат НЕ СтруктураРеквизита[ИмяРеквизита] = КлючУникальности;
	
КонецФункции

//	LNK 05.05.2020 06:57:15
Функция ПолучитьСтруктуруЗаписиРегистра(ПолноеИмя)	Экспорт
	...
	Для каждого ИмяКоллекции Из ИменаКоллекций Цикл
		...
		Для каждого Атрибут Из ОбъектМетаданных[ИмяКоллекции] Цикл
			...
			СтруктураРегистра.Вставить(Атрибут.Имя, Атрибут.Тип.ПривестиЗначение());
...
Функция ПолучитьТаблицуОбразРегистра(ПолноеИмя)	Экспорт	//	LNK 05.05.2022 07:01:12
	...
	Для каждого ИмяКоллекции Из ИменаКоллекций Цикл
		...
		Для каждого Атрибут Из ОбъектМетаданных[ИмяКоллекции] Цикл
			...
			ТаблицаЗначений.Колонки.Добавить(Атрибут.Имя, Атрибут.Тип);
...
//	LNK 08.06.2021 14:40:12
Функция ВыгрузитьКоллекцию(КоллекцияЗначений, СтрокиКоллекции = Неопределено, ИменаКолонок = Неопределено)	Экспорт

	Если ТипЗнч(КоллекцияЗначений) = Тип("ТаблицаЗначений") Тогда

		ТаблицаРезультат = КоллекцияЗначений.Скопировать(СтрокиКоллекции, ИменаКолонок);

	Иначе

		ТаблицаРезультат = КоллекцияЗначений.Выгрузить(СтрокиКоллекции, ИменаКолонок);
...
//	LNK 08.06.2021 15:41:26
Процедура ЗагрузитьВКоллекцию(КоллекцияИсточник, КоллекцияПриемник, ПерезаполнитьНомерСтроки = Ложь)	Экспорт

	Если ТипЗнч(КоллекцияПриемник) = Тип("ТаблицаЗначений") Тогда

		ОбщегоНазначенияРТСервер.ЗагрузитьВТаблицуЗначений(КоллекцияИсточник, КоллекцияПриемник, Истина);

		Если ПерезаполнитьНомерСтроки = Истина И НЕ КоллекцияПриемник.Колонки.Найти("НомерСтроки") = Неопределено Тогда

			НомерСтроки = 0;

			Для каждого СтрокаТаблицы Из КоллекцияПриемник Цикл
	...
	Иначе
		...
		КоллекцияПриемник.Загрузить(КоллекцияИсточник);

	КонецЕсли;

КонецПроцедуры

//СЮП 07.10.2024 19.36.42 
Функция ПолучитьДанныеКлючейСвязи(КоллекцияКлючей, ИмяКлюча) Экспорт
...
Процедура УдалениеВзрослыхДетей() Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КонтрагентыДети.Ссылка КАК Контрагент,
	...
	|	РАЗНОСТЬДАТ(КонтрагентыДети.ДатаРождения, &ТекущаяДата, ГОД) КАК ЛетРебенку,
	|	ДЕНЬГОДА(&ТекущаяДата) - ДЕНЬГОДА(КонтрагентыДети.ДатаРождения) КАК ДнейПослеДняРождения
	|ПОМЕСТИТЬ ВТ
	|ИЗ
	|	Справочник.Контрагенты.Дети КАК КонтрагентыДети
	|ГДЕ
	|	КонтрагентыДети.Ссылка.ПометкаУдаления = ЛОЖЬ
	|	И РАЗНОСТЬДАТ(КонтрагентыДети.ДатаРождения, &ТекущаяДата, ГОД) >= 16
	|	И КонтрагентыДети.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(Справочник.Контрагенты.Покупатели))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ.ЛетРебенку КАК ЛетРебенку,
	|	ВТ.ДнейПослеДняРождения КАК ДнейПослеДняРождения
	|ИЗ
	|	ВТ КАК ВТ
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ВТ.ЛетРебенку = 16
	|				ТОГДА ВТ.ДнейПослеДняРождения > 7
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|ИТОГИ ПО
	|	Контрагент";
	Запрос.УстановитьПараметр("ТекущаяДата", КонецДня(ТекущаяДата())); //Дата
	
	РезультатК = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Контрагент");
	
	Пока РезультатК.Следующий() Цикл 
		
		КонтрагентОбъект = РезультатК.Контрагент.ПолучитьОбъект();
		
		Результат = РезультатК.Выбрать();
		
		Пока Результат.Следующий() Цикл 
			
			УдаляемаяСтрока = КонтрагентОбъект.Дети.Найти(Результат.УникальныйИдентификатор, "УникальныйИдентификатор");	
			
			КонтрагентОбъект.Дети.Удалить(УдаляемаяСтрока);	
...
#Иначе
	...
	#КонецЕсли
		...
		РезультатПодключения = ПодключитьВнешнююКомпоненту(Местоположение
			, Идентификатор + "SymbolicName"
			,
			, ТипПодключения);
#КонецЕсли
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Подключение внешней компоненты на сервере'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если Не РезультатПодключения Тогда
		...
		ОшибкаСовместимостиКомпонентыИзМакета = "";
...
Функция ОбрезатьПослеЦифр(Стр) Экспорт  
	...
	Если Позиция = 0 Тогда
		Возврат Стр; // строка вся из цифр
	Иначе
		Возврат Лев(Стр, Позиция - 1);
	КонецЕсли;        

...
Процедура ЗаполнитьОрганизациюПродажиВТЧСервер(ТаблицаФормы, СтруктураПараметров) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	УслугиПоОрганизациям.НомерСтроки КАК НомерСтроки,
	|	УслугиПоОрганизациям.Номенклатура КАК Номенклатура,
	|	УслугиПоОрганизациям.Организация КАК Организация,
	|	УслугиПоОрганизациям.Приоритет КАК Приоритет
	...
	|		3
	|	ИЗ
	|		ТаблицаУслуга КАК ТаблицаУслуга) КАК УслугиПоОрганизациям
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО ТаблицаЗапроса.Склад.Магазин = ТаблицаОрганизаций.Владелец
	|ГДЕ
	...
	|			ТаблицаУслугиОрганизации.НомерСтроки) КАК ТаблицаСвёртка
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаУслугиОрганизации КАК ТаблицаУслугиОрганизации
	|			ПО ТаблицаСвёртка.НомерСтроки = ТаблицаУслугиОрганизации.НомерСтроки
	|				И ТаблицаСвёртка.Номенклатура = ТаблицаУслугиОрганизации.Номенклатура
	|				И ТаблицаСвёртка.Приоритет = ТаблицаУслугиОрганизации.Приоритет) КАК ТаблицаУслуги"
	);
	...
	Запрос.УстановитьПараметр("ОрганизацияПоУмолчанию",  Справочники.Магазины.РеквизитыМагазина(СтруктураПараметров.Магазин).Организация);
	Запрос.УстановитьПараметр("Период"                ,  ТекущаяДата());	//	LNK 15.11.2023 15:49:45

	ТаблицаРезультата = Запрос.Выполнить().Выгрузить();

	Для Каждого СтрокаТабличнойЧасти из ТаблицаФормы Цикл
		
		СтрокаРезультата = ТаблицаРезультата[СтрокаТабличнойЧасти.НомерСтроки - 1];
		
		СтрокаТабличнойЧасти.Организация = СтрокаРезультата.Организация;

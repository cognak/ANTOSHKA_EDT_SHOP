...
Функция СформироватьПредставлениеТелефона(КодСтраны, КодГорода, НомерТелефона, Добавочный, Комментарий) Экспорт
	...
	Представление = СтрЗаменить(СокрЛП(КодСтраны), " ", "");
	...
	Если НЕ ПустаяСтрока(КодГорода) Тогда
		...
		Представление = Представление + СтрЗаменить(СокрЛП(КодГорода), " ", "");

	КонецЕсли;

	Если НЕ ПустаяСтрока(НомерТелефона) Тогда

		Представление = Представление + СтрЗаменить(СокрЛП(НомерТелефона), " ", "");
...
// Возвращаемое значение - Строка - представление телефона.
//
Функция СформироватьПредставлениеТелефона_УДАЛИТЬ(КодСтраны, КодГорода, НомерТелефона, Добавочный, Комментарий) Экспорт

...
Процедура ПерезаполнитьПеремещениеПоОрдеру(ПеремещениеСсылка, ОрдерСсылка)	Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
//	LNK 23.04.2024 07:46:56
//	определяем исключения для выполнения этой тупой операции

	ПеремещениеОбъект = ПеремещениеСсылка.ПолучитьОбъект();

//	LNK 01.05.2024 06:30:16
	Если НЕ ПеремещениеОбъект.ВидПеремещения = Перечисления.ВидыПеремещенийТоваров.ПоступлениеПереброски Тогда
...
//	уникальность по "Наименование"... п..ц, но не я это делал. Длина строки равна длине этого реквизита.
	...
	Попытка
		...
		//2025-06-27 sa будем записывать элементы порциями по 1000 штук. Иначе они друг-другу мешают и вызывают конфликт блокировок.
		НачатьТранзакцию();
		Пока Выборка.Следующий() Цикл
			...
			Если СчетчикЭлементов % 1000 = 0 Тогда
				ЗафиксироватьТранзакцию();
				НачатьТранзакцию(); 
			КонецЕсли;

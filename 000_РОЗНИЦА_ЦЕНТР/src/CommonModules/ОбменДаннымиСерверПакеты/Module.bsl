...
//	LNK 26.11.2023 10:26:04
Функция ПолучитьПакетКлючевыеАтрибутыЗаказовПокупателей(УзелПолучатель, ИмяКласса)	Экспорт

	ТекстЗапроса   =
	"ВЫБРАТЬ ПЕРВЫЕ 100
	|	ТаблицаИзменений.Период КАК Период,
	|	ТаблицаИзменений.ЗаказПокупателя КАК ЗаказПокупателя
	|ИЗ
	|	РегистрСведений.КлючевыеАтрибутыЗаказовПокупателей.Изменения КАК ТаблицаИзменений
	|ГДЕ
	|	ТаблицаИзменений.Узел = &УзелПолучатель
	|	И ТаблицаИзменений.НомерСообщения ЕСТЬ НЕ NULL "
	;
	Возврат ВыделитьПакетОбъектов(РегистрыСведений.КлючевыеАтрибутыЗаказовПокупателей, ОбменДаннымиПовтИсп.ПолучитьРазмерПакетаВыгрузки(ИмяКласса), ТекстЗапроса, УзелПолучатель, "НаборЗаписей");

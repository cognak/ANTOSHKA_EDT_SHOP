	...
	//	назначенных строк, необходимо учитывать уже потраченные партии.
		...
		//	=======================================================================================
			Если СтрокаСписать.ВозвратБонусныхБаллов Тогда
				...
				Если СтрокаСписать.ВидОперации = Перечисления.ВидыОперацийБонусныхБаллов.Использование Тогда
				...
				Иначе	//	ВОЗВРАТ НАЧИСЛЕНИЙ

					//СуммаСписанияПартии = Макс(0, Мин(СуммаБонусныхБалловСписание, ДанныеДокументаПродажи.СуммаБонусныхБалловОстаток));
					СуммаСписанияПартии = Макс(0, Мин(СуммаБонусныхБалловСписание, СуммаБонусныхБалловСписание));
...
//					ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЧекККМ")
//							И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ВидОперации") = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда 
//
//						ПараметрыПогашения = Новый Структура(
//							"ДокументПродажи, Примечание, РеквизитыАннуляции, ПогаситьОстатокПереноса, ИспользоватьТекущую"
//							, СтрокаСписать.ДокументПродажи
...
//							, Ложь
//						);
//						ПогашениеАварийногоДвижения(СтрокаТекущейОперации,
//							-ДанныеСписания.Реставрация.СуммаБонусныхБаллов,
//							ТаблицаТекущая,
//							ПараметрыПогашения,
//							РЕГ);
...
//	#КонецЕсли
	...
	ПервыйПроход = ПараметрыСписания.ИспользоватьТекущую И НЕ ЗначениеЗаполнено(СтрокаТекущейОперации.ДокументНачисления);	//	ИСТИНА в списании продажи
...
Процедура РеставрацияАварийногоДвижения(Период, Регистратор, Контрагент, ТаблицаТекущая, Реставрация, РЕГ)
	...
	Для каждого СтрокаТекущейОперации Из ТаблицаТекущая Цикл
		...
		ВидОпирацииНачисление = (СтрокаТекущейОперации.ВидОперации = Перечисления.ВидыОперацийБонусныхБаллов.Начисление);
		ВидОпирацииВозврат = (СтрокаТекущейОперации.ВидОперации = Перечисления.ВидыОперацийБонусныхБаллов.Возврат);

		Если НЕ ((ВидОпирацииНачисление
					И СтрокаТекущейОперации.СуммаБонусныхБаллов > 0)
				Или (ВидОпирацииВозврат
					И СтрокаТекущейОперации.СуммаБонусныхБаллов < 0)
				И НЕ СтрокаТекущейОперации.СписаниеПартий) Тогда
		...
		Если Реставрация.СуммаБонусныхБаллов > 0 Тогда
			...
			СуммаБонусныхБалловТекущая = СтрокаТекущейОперации.СуммаБонусныхБаллов * ?(ВидОпирацииВозврат, -1, 1);
			...
			Для каждого СтрокаОстаток Из Реставрация.Таблица Цикл
				...
				//	запись реставрации всегда сторнирующая!
					...
					СтрокаРеставрации.СуммаБонусныхБаллов = СуммаБонусныхБалловПринять * ?(ВидОпирацииВозврат, 1, -1);
				...
				//	---------------------------------------------------------------------------------------
					...
					//	запись реставрации всегда сторнирующая!
						...
						СтрокаАннуляции.СуммаБонусныхБаллов	=  СуммаБонусныхБалловПринять * ?(ВидОпирацииВозврат, 1, -1);
				...
				//	---------------------------------------------------------------------------------------
					...
					СтрокаПогашения.СуммаБонусныхБаллов	=  СуммаБонусныхБалловПринять * ?(ВидОпирацииВозврат, -1, 1);
...
Функция ПолучитьДанныеДляСписания(Период, Регистратор, Контрагент)
	...
	|		КОГДА ТаблицаСписание.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийБонусныхБаллов.Возврат)
	|			ТОГДА -1
	|		КОГДА ТаблицаСписание.ДокументПродажи В (НЕОПРЕДЕЛЕНО, ЗНАЧЕНИЕ(Документ.РеализацияТоваров.ПустаяСсылка),
	|			ЗНАЧЕНИЕ(Документ.ЧекККМ.ПустаяСсылка), ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка))
	|			ТОГДА 1
	|		ИНАЧЕ -1
	...
	|	ИЛИ (Источник.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийБонусныхБаллов.Возврат)
	|	И Источник.ВозвратБонусныхБаллов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ Источник"
	...
	Если НЕ РезультатыЗапроса[2].Пустой() Тогда
		...
		Если ТипЗнч(ВыборкаПериод.ДатаНачисления) = Тип("Дата") И НЕ ВыборкаПериод.ДатаНачисления = '00010101' Тогда
			...
			Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЧекККМ")
					И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ВидОперации") = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда
				ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ЧекККМПродажа");
				Запрос.УстановитьПараметр("ГраницаСреза", Новый Граница(Новый МоментВремени(ВыборкаПериод.ДатаНачисления, ДокументОснование), ВидГраницы.Исключая));
			Иначе
...
Функция ПолучитьТаблицуПартийКонтрагента(Контрагент, Период, Регистратор, ПрограммаЛояльности, ПолучитьВсеПартии, ТаблицаИспользованныхПартий, ПустаяТаблица)
	...
	Если ПустаяТаблица = Ложь Тогда
		...
		|	И НЕ ТаблицаНакоплений.ДокументНачисления В (&СписокИспользовано)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ДокументНачисления
		|;
		...
		Запрос.УстановитьПараметр("СписокИспользовано"	, ТаблицаИспользованныхПартий.ВыгрузитьКолонку("ДокументНачисления"));

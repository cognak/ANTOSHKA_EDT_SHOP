...
//	LNK 21.04.2017 12:26:53
	ПараметрыСеанса.ТекущаяОрганизация = Справочники.Магазины.РеквизитыМагазина(ПараметрыСеанса.ТекущийМагазин).Организация;
...
//2025-08-11 sa в центральной базе Магазин и Организация определяются при Открытии РМК. Для того чтобы РМК работала корректно, нужно эти параметры переопределить. 
Процедура ПереустановитьПараметрыСеанса(Магазин,Организация) Экспорт  
	УстановитьПривилегированныйРежим(Истина);
	ПараметрыСеанса.ТекущийМагазин = Магазин;
	ПараметрыСеанса.ТекущаяОрганизация = Организация;
	УстановитьПривилегированныйРежим(Ложь);
КонецПроцедуры


//	LNK 04.03.2024 07:07:09 - используется в "ОбщегоНазначенияРТКлиент.УстановитьЗаголовокПриложенияРТ()"
...
Функция ПолучитьСтруктуруПоДокументу(ИмяДокумента, ТаблицыОтдельныйЭлемент = Истина) Экспорт
	...
	Для каждого ТабличнаяЧасть Из Метаданные.Документы[ИмяДокумента].ТабличныеЧасти Цикл
		...
		//СЮП 08.08.2024 10.53.11 
		Если ТабличнаяЧасть.Имя = "Товары" И Не ИмяДокумента = "ЗаказПокупателя" Тогда
			Таблица.Колонки.Добавить("АкционнаяЦенаБезСкидки", Новый ОписаниеТипов("Булево"));
		КонецЕсли;

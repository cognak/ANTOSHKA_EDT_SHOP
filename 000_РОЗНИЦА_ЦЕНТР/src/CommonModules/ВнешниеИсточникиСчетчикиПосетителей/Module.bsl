...
Процедура ПроверитьВсеЛиДанныеПолучены()
	Запрос = Новый Запрос;
	ДатаВремениПроверки = ТекущаяДата();
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	График.Магазин КАК Магазин
		|ИЗ
		|	Справочник.ИнтервалыРаботыМагазинов КАК График
		|		// Пробуем найти запись в документе за нужный час
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РегистрацияПодсчетаПосетителей.ДанныеПодсчета КАК ФактТЧ
		|		ПО График.Магазин = ФактТЧ.Ссылка.Магазин
		|			И (ФактТЧ.Ссылка.Дата МЕЖДУ &НачалоДня И &КонецДня)
		|			И (ФактТЧ.Ссылка.Проведен = ИСТИНА)
		|			И (ЧАС(ФактТЧ.ВремяРегистрации) = &ЧасПроверки)
		|ГДЕ
		|	// 1. Проверяем, что магазин должен работать в этот час
		|	&ЧасПроверки >= ЧАС(График.НачалоИнтервала)
		|	И &ЧасПроверки <= ЧАС(График.ОкончаниеИнтервала)
		|
		|	// 2. Условие ошибки: запись не найдена
		|	И ФактТЧ.Ссылка ЕСТЬ NULL
		|
		|	// 3. Стандартные фильтры
		|	И График.Магазин.ВведенВЭксплуатацию = ИСТИНА
		|	И График.Магазин.Блокирован = ЛОЖЬ
		|	И График.Магазин.НомерМагазина <> 104";

	ЧасДляПроверки = Час(ТекущаяДата() - 3*3600); 

	Запрос.УстановитьПараметр("ЧасПроверки", ЧасДляПроверки);
	...
	Пока Выборка.Следующий() Цикл
		...
		Запись = РегистрыСведений.ОшибкиЗагрузкиСчетчиколвПоситителей.СоздатьМенеджерЗаписи();
		Запись.Магазин = Выборка.Магазин;
		Запись.ПериодЗагрузки = ДатаВремениПроверки;

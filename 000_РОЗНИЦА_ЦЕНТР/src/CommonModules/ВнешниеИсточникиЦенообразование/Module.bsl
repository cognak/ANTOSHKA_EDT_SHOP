...
Процедура ПолучитьЦеныCompeteraОтGraphQL(Знач ПериодЦен)	Экспорт
	...
	Пока Истина Цикл
		...
		ЧтениеJSON.УстановитьСтроку(ВыполнитьЗапросДляGraphQL(ТекстЗапроса, Результат));
...
Функция ВыполнитьЗапросДляGraphQL(ТекстЗапроса, Результат)
...
Функция ПолучитьЦенуТовараCompeteraОтGraphQL(IDN) Экспорт
	...
	//	Выполняем подключение к серверу "https://graphql.awdev.pro/graphql"...
		...
		//	формирование заголовка
			...
			HTTPЗапрос = Новый HTTPЗапрос("/graphql", Заголовки);
			HTTPЗапрос.УстановитьТелоИзСтроки(ПолучитьТекстЗапросаЦенаТовараGraphQL(IDN));

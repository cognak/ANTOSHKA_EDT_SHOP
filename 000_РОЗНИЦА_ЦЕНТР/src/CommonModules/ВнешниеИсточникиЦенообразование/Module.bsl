...
Функция ПолучитьЦенуТовараCompeteraОтGraphQL(IDN) Экспорт

	//СЮП 23.02.2024 11.58.22 
	Цена = 0;
	ТекстЗапроса = ПолучитьТекстЗапросаЦенаТовараGraphQL(IDN);
	Результат = ВнешниеИсточникиGraphQL.ВыполнитьЗапросДляGraphQL(ТекстЗапроса);
	...
	Иначе
		...
		ЖурналСобытий.Регистрация("Ошибка запроса цены Competera",,,,,Результат.ОписаниеОшибки);	
...
Функция ПолучитьТекстЗапросаЦенаТовараGraphQL(IDN)

	МакетЗапросов = ПолучитьОбщийМакет("ЗапросыКСтороннимСистемам");

	ОбластьЗапроса = МакетЗапросов.ПолучитьОбласть("Цена_товара_Competera");
	ОбластьЗапроса.УдалитьСтроку(1);
	ОбластьЗапроса.УдалитьСтроку(ОбластьЗапроса.КоличествоСтрок());

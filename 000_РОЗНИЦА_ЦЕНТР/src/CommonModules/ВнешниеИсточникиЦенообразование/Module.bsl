...
Процедура ПолучитьЦеныCompeteraОтGraphQL(Знач ПериодЦен)	Экспорт
	...
	Пока Истина Цикл
		...
		ТекстЗапроса = ПолучитьТекстЗапросаGraphQL(ПериодЦен, КоличествоВОтвете);
		//СЮП 23.02.2024 11.55.26 
...
Функция ПолучитьЦенуТовараОтGraphQL(IDN, ТипЦены) Экспорт

	//СЮП 23.02.2024 11.58.22 
	Цена = 0;
	ТекстЗапроса = ПолучитьТекстЗапросаЦенаТовараGraphQL(IDN, ТипЦены);
	Результат = ВнешниеИсточникиGraphQL.ВыполнитьЗапросДляGraphQL(ТекстЗапроса);
	...
	Если  Результат.Ошибка = Ложь тогда
		...
		Если ДанныеЗапроса.data.offerProducts.data.Количество() <> 0 тогда  
			Если ДанныеЗапроса.data.offerProducts.data[0].offerPrices.Количество() <> 0 тогда
				Цена = Число(ДанныеЗапроса.data.offerProducts.data[0].offerPrices[0].value);
			КонецЕсли;
		КонецЕсли;
	Иначе
		// Сделать запись в журнал   
		ЖурналСобытий.Регистрация("Ошибка запроса цены GraphQL",,,,,Результат.ОписаниеОшибки);	
...
Функция ПолучитьТекстЗапросаЦенаТовараGraphQL(IDN, ТипЦены)

	МакетЗапросов = ПолучитьОбщийМакет("ЗапросыКСтороннимСистемам");

	ОбластьЗапроса = МакетЗапросов.ПолучитьОбласть("Цена_товара");
	ОбластьЗапроса.УдалитьСтроку(1);
	ОбластьЗапроса.УдалитьСтроку(ОбластьЗапроса.КоличествоСтрок());
	
	ТекстGraphQL = ОбластьЗапроса.ПолучитьТекст();
	ТекстGraphQL = СтрЗаменить(ТекстGraphQL, "#sku#" , IDN);
	ТекстGraphQL = СтрЗаменить(ТекстGraphQL, "#ТипЦены#" , ТипЦены);
	ДанныеЗапроса = Новый Структура("query", ТекстGraphQL);

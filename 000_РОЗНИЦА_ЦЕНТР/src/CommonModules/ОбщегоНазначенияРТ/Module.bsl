...
//	Кешировать результат нельзя, так как изменение политики должно сразу же вступать в силу.
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ТаблицаРегистра.ОсновнаяОрганизация КАК ОсновнаяОрганизация,
	|	ТаблицаРегистра.ВидЦенИнтернет КАК ВидЦенИнтернет,
	|	ТаблицаРегистра.КонтрольОстатковПриОтменеПроведения КАК КонтрольОстатковПриОтменеПроведения,
	|	ТаблицаРегистра.РучнаяСкидкаНаПодарочныйСертификат КАК РучнаяСкидкаНаПодарочныйСертификат,
	|	ТаблицаРегистра.МаксимальныйПроцентРучнойСкидки КАК МаксимальныйПроцентРучнойСкидки,
	|	ТаблицаРегистра.НомерТестовогоМагазина КАК НомерТестовогоМагазина,
	|	ТаблицаРегистра.ДатаПереходаНаЛетнееВремя КАК ДатаПереходаНаЛетнееВремя,
	|	ТаблицаРегистра.ДатаПереходаНаЗимнееВремя КАК ДатаПереходаНаЗимнееВремя,
	|	ТаблицаРегистра.КонтрольОписанияПричиныРучнойСкидки КАК КонтрольОписанияПричиныРучнойСкидки,
	|	ТаблицаРегистра.ИнтернетМагазин КАК ИнтернетМагазин,
	|	ТаблицаРегистра.ИнтернетМагазин.Организация КАК ОрганизацияИнтернетМагазина,
	|	ТаблицаРегистра.ИнтернетМагазин.СкладПродажи КАК СкладПродажиИнтернетМагазина,
	|	ТаблицаРегистра.СкладЦентральныйИнтернетМагазина КАК СкладЦентральныйИнтернетМагазина,
	|	ТаблицаРегистра.ПрограммаЛояльности КАК ПрограммаЛояльности,
	|	ТаблицаРегистра.ПродажиДляCRMпослеЗакрытияСмены КАК ПродажиДляCRMпослеЗакрытияСмены,
	|	ТаблицаРегистра.СуммаОплатыЧастямиДляЦенника КАК СуммаОплатыЧастямиДляЦенника,
	|	ТаблицаРегистра.НазваниеХарактеристикиДляПересчета КАК НазваниеХарактеристикиДляПересчета,
	|	ТаблицаРегистра.ЗначениеХарактеристикиДляПересчета КАК ЗначениеХарактеристикиДляПересчета,
	|	ВЫБОР
	|		КОГДА ТаблицаРегистра.КоличествоПопытокУдаления = 0
	|			ТОГДА 10
	|		ИНАЧЕ ТаблицаРегистра.КоличествоПопытокУдаления
	|	КОНЕЦ КАК КоличествоПопытокУдаления,
	|	ТаблицаРегистра.БлокироватьОперацииПриОтсутствииСоединенияЦБ КАК БлокироватьОперацииПриОтсутствииСоединенияЦБ,
	|	ТаблицаРегистра.РазрешитьПереносДокументаВДругойПериод КАК РазрешитьПереносДокументаВДругойПериод,
	|	ТаблицаРегистра.УчитыватьNavisionПоступлениеПереброски КАК УчитыватьNavisionПоступлениеПереброски,
	|	ТаблицаРегистра.МаксимальнаяСуммаНакопленияЛояльности КАК МаксимальнаяСуммаНакопленияЛояльности,
	|	ТаблицаРегистра.ПроверятьПеремещенияПоЗаказамВЦентре КАК ПроверятьПеремещенияПоЗаказамВЦентре,
	|	ТаблицаРегистра.РазрешитьТолькоТорговыйЗал КАК РазрешитьТолькоТорговыйЗал,
	|	ТаблицаРегистра.ИспользоватьОбменССайтомНаПериферийныхУзлах КАК ИспользоватьОбменССайтомНаПериферийныхУзлах,
	|	ТаблицаРегистра.ИспользоватьБонуснуюСистему КАК ИспользоватьБонуснуюСистему,
	|	ТаблицаРегистра.НеНачислятьБаллыПриОплатеБонусами КАК НеНачислятьБаллыПриОплатеБонусами,
	|	ТаблицаРегистра.ИспользоватьУкраинскоеНаименование КАК ИспользоватьУкраинскоеНаименование,
	|	ВЫБОР
	|		КОГДА ТаблицаРегистра.РазмерПакетаВиртуальныхКартРезерва = 0
	|			ТОГДА 1000
	|		ИНАЧЕ ТаблицаРегистра.РазмерПакетаВиртуальныхКартРезерва
	|	КОНЕЦ КАК РазмерПакетаВиртуальныхКартРезерва,
	|	ТаблицаРегистра.ВыполнятьСверкуИнформативныхОстатков КАК ВыполнятьСверкуИнформативныхОстатков,
	|	ТаблицаРегистра.ИспользоватьЧекиКакРегистраторыНачисленияБонусныхБаллов КАК ИспользоватьЧекиКакРегистраторыНачисленияБонусныхБаллов,
	|	ТаблицаРегистра.ДвиженияБонусныхБалловТолькоВЦентре КАК ДвиженияБонусныхБалловТолькоВЦентре,
	|	ВЫБОР
	|		КОГДА ТаблицаРегистра.ЦенаПродажиМинимальная = 0
	|			ТОГДА 0.1
	|		ИНАЧЕ ТаблицаРегистра.ЦенаПродажиМинимальная
	|	КОНЕЦ КАК ЦенаПродажиМинимальная,
	|	ТаблицаРегистра.ПечататьВЧекеДанныеБонусногоСчета КАК ПечататьВЧекеДанныеБонусногоСчета,
	|	ТаблицаРегистра.РазрешитьПрямуюИнтеграциюMagento КАК РазрешитьПрямуюИнтеграциюMagento,
	|	ТаблицаРегистра.РазрешитьОплатуБонусамиПоЗаказуПокупателяНаРМК КАК РазрешитьОплатуБонусамиПоЗаказуПокупателяНаРМК,
	|	ТаблицаРегистра.РазрешитьОплатуБонуснымиБалламиПодарочныхСертификатов КАК РазрешитьОплатуБонуснымиБалламиПодарочныхСертификатов,
	|	ТаблицаРегистра.TimeoutDDos КАК TimeoutDDos,
	|	ТаблицаРегистра.ВсеПродажиБезНДС КАК ВсеПродажиБезНДС,
	|	ВЫБОР
	|		КОГДА ТаблицаРегистра.ВремяУчетаКассовогоДня = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ДАТАВРЕМЯ(1, 1, 1, 17, 10, 50)
	|		ИНАЧЕ ТаблицаРегистра.ВремяУчетаКассовогоДня
	...
	|	ВЫБОР
	|		КОГДА ТаблицаРегистра.ГлубинаОповещенияОбАннуляцииБонусныхБаллов = 0
	|			ТОГДА 30
	|		ИНАЧЕ ТаблицаРегистра.ГлубинаОповещенияОбАннуляцииБонусныхБаллов
	|	КОНЕЦ КАК ГлубинаОповещенияОбАннуляцииБонусныхБаллов,
	|	ТаблицаРегистра.ВыполнятьЗагрузкуОбменаПередОбновлением КАК ВыполнятьЗагрузкуОбменаПередОбновлением,
	|	ВЫБОР
	|		КОГДА ТаблицаРегистра.ИнтервалОграниченияВыгрузкиСообщенияОбмена = 0
	|			ТОГДА 5
	|		ИНАЧЕ ТаблицаРегистра.ИнтервалОграниченияВыгрузкиСообщенияОбмена
	|	КОНЕЦ КАК ИнтервалОграниченияВыгрузкиСообщенияОбмена,
	|	ТаблицаРегистра.МинимальныйПериодРассчитанныхИтогов КАК МинимальныйПериодРассчитанныхИтогов,
	|	ВЫБОР
	|		КОГДА ТаблицаРегистра.СуммаНаличныхОграничение = 0
	|			ТОГДА 50000
	|		ИНАЧЕ ТаблицаРегистра.СуммаНаличныхОграничение
	|	КОНЕЦ КАК СуммаНаличныхОграничение,
	|	ТаблицаРегистра.ДатаИзменения КАК ДатаИзменения
	|ИЗ
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&Период, ) КАК ТаблицаРегистра"
	);
	...
	ОбщегоНазначенияРТ.ПеренестиСтрокуВыборкиВСтруктуру(РезультатЗапроса, ПолитикаВыборка, СоставПолитики,, Истина); 
...
Процедура ПеренестиСтрокуВыборкиВСтруктуру(РезультатЗапроса, СтрокаВыборки, СтруктураЭлемента, ВключаемыеПоля = Неопределено, Типизировать = Ложь) Экспорт
...
Функция СоздатьСтруктуруПоСтрокеВыборки(РезультатЗапроса, СтрокаВыборки, Типизировать = Ложь) Экспорт
	
	РезультирующаяСтруктура = Новый Структура;
	ПеренестиСтрокуВыборкиВСтруктуру(РезультатЗапроса, СтрокаВыборки, РезультирующаяСтруктура,, Типизировать);

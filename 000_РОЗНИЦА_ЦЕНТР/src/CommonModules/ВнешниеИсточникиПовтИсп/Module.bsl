...
Функция СоответствиеПолейВнешнихИсточников()	Экспорт
	...
	//2026-8-11 sa добавляем Учетную цену (себестоимость)
	СоставПолей.Вставить("УЧЕТНАЯЦЕНА"                    , Новый Структура("Да, Тип", Истина, Новый ОписаниеТипов("Число")));
...
//	Сичкаренко 11.09.2023
	...
	//А++ 20240919 по задаче КЭШБЭКА
	СоставПолей.Вставить(ВРег("Кешбек")              	  , Новый Структура("Да, Тип", Истина, Новый ОписаниеТипов("Булево")));
	//СоставПолей.Вставить(ВРег("Кешбек")              	  , Новый Структура("Да, Тип", Истина, Новый ОписаниеТипов("Число")));
...
//	LNK 12.04.2021 04:27:59
	...
	//А++ 20240920 по задаче Сергея
	СписокКодов.Вставить("6118479", Истина);
	СписокКодов.Вставить("6118480", Истина);
	...
	Запрос.Текст = 
		...
		|	И Номенклатура.ИсключитьИзОбменаNavision";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СписокКодов.Вставить(СокрЛП(ВыборкаДетальныеЗаписи.IDN), Истина);
	КонецЦикла;
	//А++ 20240920 по задаче Сергея
...
Функция ПолучитьТекстЗапроса(ИмяОбласти, Источник = Неопределено)	Экспорт
...
//	LNK 04.05.2024 16:48:33
Функция НужнаРегистрацияПоСверкеNAV(Значение)	Экспорт

	РезультатДопустимый = Новый Соответствие;
	РезультатДопустимый.Вставить(Перечисления.РезультатСверкиNavision.Соответствует, Истина);
	РезультатДопустимый.Вставить(Перечисления.РезультатСверкиNavision.НеСоответствует, Истина);

	Возврат РезультатДопустимый.Получить(Значение) = Истина;

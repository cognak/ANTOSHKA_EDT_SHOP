...
Процедура УстановкаПараметровСеанса(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Попытка
		...
		ИначеЕсли ПользовательНеТребуетАвторизации(ТекущийПользовательИБ.Имя)	Тогда
	...
	Если НЕ ПараметрыСеанса.ТекущийПользователь.Пустая() Тогда

	//	LNK 29.11.2017 15:54:36
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ЕСТЬNULL(ТаблицаСправочник.ФизЛицо, Ведущая.Пользователь) КАК ФизЛицо
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка) КАК Пользователь) КАК Ведущая
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК ТаблицаСправочник
		|		ПО (ТаблицаСправочник.Ссылка = &ТекущийПользователь)"
		);
		...
		ПараметрыСеанса.ТекущееФизЛицо = Выборка.ФизЛицо;

	Иначе

		ПараметрыСеанса.ТекущееФизЛицо = Справочники.ФизическиеЛица.ПустаяСсылка();

	КонецЕсли;

	УстановленныеПараметры.Добавить("ТекущееФизЛицо");
...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь) Экспорт
	...
	Если ПустаяСтрока(ТекущийПользовательИБ.Имя) ИЛИ ПользователиКлиентСервер.ИмяНеТребуетАвторизации(ТекущийПользовательИБ.Имя) Тогда
...
Функция ЗащищенныеРоли()	Экспорт	//	LNK 10.01.2020 13:24:52

//	Возможно назначение только для доп.права "АдминистраторПолномочный".

	СписокРолей = Новый Соответствие;

	СписокРолей.Вставить("ИнтерактивноеОткрытиеВнешнихОтчетовИОбработок", Истина);
	СписокРолей.Вставить("ВыводНаПринтерФайлБуферОбмена", Истина);
...
Функция ПроверитьВозможностьЗапускаСПустымСпискомПользователейИБ(ВернутьТекстОшибки = Ложь) Экспорт
	...
	Если Метаданные.ОсновныеРоли.Количество() = 0 Тогда
...
//	LNK 05.12.2017 12:25:58
Функция ПользовательНеТребуетАвторизации(ИмяПользователя = "")	Экспорт

	Возврат		ПользователиКлиентСервер.ИмяНеТребуетАвторизации(ИмяПользователя)
			ИЛИ ИмяПользователя = УправлениеДоступомСлужебныйПовтИсп.ИдентификаторWebПосредника()
			ИЛИ РольДоступна(Метаданные.Роли.ИспользованиеCOMВнешнее)
			ИЛИ РольДоступна(Метаданные.Роли.ИспользованиеWebСервисовВнешнее)
			ИЛИ РольДоступна(Метаданные.Роли.ИспользованиеWebСервисовРозница)
			ИЛИ РольДоступна(Метаданные.Роли.ОператорВиртуальнойПолки)
...
Процедура ОбновитьПараметрыРаботыПользователей_ТИПОВАЯ(ЕстьИзменения = Неопределено, ТолькоПроверка = Ложь) Экспорт
...
// ВключениеВыключениеПользователяИзЗУП
// Производит Включени или віключение пользователя ИБ и установка его ролей
//
// Параметры:
//  <Пользователь>  - <Справочники.Пользователи> - Пользователь каторого нужно включить или выключить в базе
//  <ДоступКБазе>  - <булево> - Доступ к базе истина - создать пользователя ИБ, ложь - удалить
//
Процедура ВключениеВыключениеПользователяИзЗУП(Пользователь, ДоступКБазе) Экспорт
	
	ПустойУН = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
	...
	// Включение или выключение пользователя
	Если ДоступКБазе И ПользовательОбъект.ИдентификаторПользователяИБ = ПустойУН Тогда 
		...
		ОписаниеПользователяИБ.ЗапрещеноИзменятьПароль = Ложь;
		ОписаниеПользователяИБ.Имя = ПользовательИнфБазыИмя;
		ОписаниеПользователяИБ.ОсновнойИнтерфейс = "";
		ОписаниеПользователяИБ.Пароль = "";
		ОписаниеПользователяИБ.ПарольУстановлен = Ложь;
		ОписаниеПользователяИБ.ПоказыватьВСпискеВыбора = Истина;
		ОписаниеПользователяИБ.ПолноеИмя = ПользовательОбъект.Наименование;
		ОписаниеПользователяИБ.ПользовательОС = "";
		ОписаниеПользователяИБ.РежимЗапуска = "Авто";
		ОписаниеПользователяИБ.Роли = Неопределено;
		ОписаниеПользователяИБ.СохраняемоеЗначениеПароля = Неопределено;
		ОписаниеПользователяИБ.СтарыйПароль = Неопределено;
		ОписаниеПользователяИБ.УникальныйИдентификатор = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
		ОписаниеПользователяИБ.Язык = "Русский";		
		ОписаниеПользователяИБ.Вставить("Действие", "Записать");
	...
	// Включение или выключение прав пользователя
	
	


КонецПроцедуры // ВключениеВыключениеПользователяИзЗУП()

...
// Параметры:
//   URL - Строка - URL ресурса, к которому будет отправлен запрос.
//   JSONКоннектор - Структура, Соответствие - данные, которые необходимо сериализовать в JSON.
//   ДополнительныеПараметры - См. НовыеПараметры
//   Сессия - См. СоздатьСессию
//
// Возвращаемое значение:
...
//     Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON
//
Функция PostJson(URL, JSONКоннектор, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт

	ТекущаяСессия = ТекущаяСессия(Сессия);
	
	Параметры = НовыеПараметры();
	Дополнить(Параметры, ДополнительныеПараметры);
	Дополнить(Параметры, ПараметрыИзАргументов(Неопределено, Неопределено, JSONКоннектор));
...
//     Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON
//
Функция PutJson(URL, JSONКоннектор, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
...
//     Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON
//
Функция DeleteJson(URL, JSONКоннектор, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
...
//
// Параметры:
//   JSONКоннектор - Поток, ДвоичныеДанные, Строка - данные в формате JSON.
...
//   Произвольный - значение, десериализованное из JSON.
//
Функция JsonВОбъект(JSONКоннектор, Кодировка = "utf-8", ПараметрыПреобразования = Неопределено) Экспорт

	ПараметрыПреобразованияJSON = Объединить(ПараметрыПреобразованияJSONПоУмолчанию(), ПараметрыПреобразования);

	ЧтениеJSON = Новый ЧтениеJSON;
	Если ТипЗнч(JSONКоннектор) = Тип("ДвоичныеДанные") Тогда
		ЧтениеJSON.ОткрытьПоток(JSONКоннектор.ОткрытьПотокДляЧтения(), Кодировка);
	ИначеЕсли ТипЗнч(JSONКоннектор) = Тип("Строка") Тогда
		ЧтениеJSON.УстановитьСтроку(JSONКоннектор);
	Иначе
		ЧтениеJSON.ОткрытьПоток(JSONКоннектор, Кодировка);
	КонецЕсли;
...
Функция ПодготовитьЗапрос(Сессия, Метод, URL, ДополнительныеПараметры) Экспорт
	...
	Заголовки = Объединить(Сессия.Заголовки, Скопировать(ЗаголовкиИзДополнительныхПараметров));
	ПараметрыПреобразованияJSON =
	...
	JSONКоннектор = ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "Json", Неопределено);
	ПараметрыЗаписиJSON = ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыЗаписиJSON", Неопределено);

	ПодготовитьТелоЗапроса(ПодготовленныйЗапрос, Данные, Файлы, JSONКоннектор, ПараметрыЗаписиJSON);
...
Функция ПараметрыИзАргументов(ПараметрыЗапроса, Данные, JSONКоннектор)

	Результат = Новый Структура;
	Результат.Вставить("ПараметрыЗапроса", ПараметрыЗапроса);
	Результат.Вставить("Данные", Данные);
	Результат.Вставить("Json", JSONКоннектор);
...
Процедура ПодготовитьТелоЗапроса(ПодготовленныйЗапрос, Данные, Файлы, JSONКоннектор, ПараметрыЗаписиJSON)
	...
	ИначеЕсли ЗначениеЗаполнено(Данные) Тогда
		...
		КонецЕсли;
	ИначеЕсли JSONКоннектор <> Неопределено Тогда
		ContentType = "application/json; charset=utf-8";
		СтрокаJson = ОбъектВJson(JSONКоннектор, ПодготовленныйЗапрос.ПараметрыПреобразованияJSON, ПараметрыЗаписиJSON);

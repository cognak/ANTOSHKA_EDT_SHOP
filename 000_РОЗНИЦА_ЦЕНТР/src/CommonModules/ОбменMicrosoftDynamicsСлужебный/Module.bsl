			...
			Если НЕ Отказ Тогда
				...
				Если НЕ ФильтрВыборки.Количество() = 0 Тогда
					...
					//	в текст используем эту вторую таблицу.
						...
						РаботаСJSONСервер.ЗаписатьСвойство("message1C"  , ?(ВыгрузкаОдногоОбъекта, -1, УзелОбъект.НомерОтправленного), ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("UUID_noInfo", СокрЛП(Новый УникальныйИдентификатор), ЗаписьJSON);
						...
						РаботаСJSONСервер.ЗаписатьСвойство("count_results", РазмерМассива, ЗаписьJSON);
			...
			Пока ТоварыВыборка.Следующий() Цикл
				...
				Пока УпаковкаВыборка.Следующий() Цикл
					...
					ЗаписьJSON.ЗаписатьНачалоОбъекта();
						...
						РаботаСJSONСервер.ЗаписатьСвойство("SKU", УпаковкаВыборка.IDN, ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("unitMeasurement", УпаковкаВыборка.Единица, ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("coefficient", УпаковкаВыборка.Коэффициент, ЗаписьJSON);
						...
						Пока ЦеныВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("IDD_store"  , ЦеныВыборка.GUID, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("IDC_store"  , СокрЛП(ЦеныВыборка.Магазин.УникальныйИдентификатор()), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("IDN_store"  , ЦеныВыборка.IDNМагазина, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("storeNumber", ЦеныВыборка.НомерМагазина, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("price"      , ЦеныВыборка.Цена, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("PromoPrice" , ЦеныВыборка.АкционнаяЦена, ЗаписьJSON);
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Пока ТоварыВыборка.Следующий() Цикл
				...
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
					...
					РаботаСJSONСервер.ЗаписатьСвойство("SKU", ТоварыВыборка.IDN, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("coefficient", ТоварыВыборка.Коэффициент, ЗаписьJSON);
					...
					Пока ДеталиВыборка.Следующий() Цикл
						...
						ЗаписьJSON.ЗаписатьНачалоОбъекта();
							...
							РаботаСJSONСервер.ЗаписатьСвойство("IDD_store"       , ДеталиВыборка.GUID, ЗаписьJSON);
							РаботаСJSONСервер.ЗаписатьСвойство("IDC_store"       , СокрЛП(ДеталиВыборка.Магазин.УникальныйИдентификатор()), ЗаписьJSON);
							РаботаСJSONСервер.ЗаписатьСвойство("IDN_store"       , ДеталиВыборка.IDNМагазина, ЗаписьJSON);
							РаботаСJSONСервер.ЗаписатьСвойство("storeNumber"     , ДеталиВыборка.НомерМагазина, ЗаписьJSON);
							РаботаСJSONСервер.ЗаписатьСвойство("availability"    , ДеталиВыборка.Количество, ЗаписьJSON);
							РаботаСJSONСервер.ЗаписатьСвойство("reserve"         , ДеталиВыборка.КоличествоРезерв, ЗаписьJSON);
							РаботаСJSONСервер.ЗаписатьСвойство("availableBalance", ДеталиВыборка.Количество - ДеталиВыборка.КоличествоРезерв, ЗаписьJSON);
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Пока МагазиныВыборка.Следующий() Цикл
				...
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
					...
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_store"     , СокрЛП(МагазиныВыборка.Магазин.УникальныйИдентификатор()), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("PrimaryDeleted", Ложь, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDN_store"     , МагазиныВыборка.IDN, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("storeName"     , СокрЛП(МагазиныВыборка.Наименование), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("storeNumber"   , МагазиныВыборка.НомерМагазина, ЗаписьJSON);
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Пока ЗначенияВыборка.Следующий() Цикл
				...
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
					...
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_Type", ЗначенияВыборка.ТипЗначения, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDC", СокрЛП(ЗначенияВыборка.Ссылка.УникальныйИдентификатор()), ЗаписьJSON);

					Если ТипЗнч(ЗначенияВыборка.Ссылка) = Тип("СправочникСсылка.ИменаЛюдей") Тогда

						РаботаСJSONСервер.ЗаписатьСвойство("Name"       , СокрЛП(ЗначенияВыборка.Наименование), ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("Name_UA"    , СокрЛП(ЗначенияВыборка.НаименованиеУкраинское), ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("Name_LAT"   , СокрЛП(ЗначенияВыборка.НаименованиеЛатиница), ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("Name_UA_LAT", СокрЛП(ЗначенияВыборка.НаименованиеУкраинскоеЛатиница), ЗаписьJSON);

						РаботаСJSONСервер.ЗаписатьСвойство("Gender" , ЗначенияВыборка.Пол, ЗаписьJSON);
						РаботаСJSONСервер.ЗаписатьСвойство("GenderDeterm", ЗначенияВыборка.ОпределяетПол, ЗаписьJSON);
		...
		Если НЕ Результаты[3].Пустой() Тогда
			...
			Для каждого СтрокаКлиента Из ТаблицаКлиентов Цикл
				...
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
					...
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_Client", СокрЛП(СтрокаКлиента.Контрагент.УникальныйИдентификатор()), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDN_client", СтрокаКлиента.IDN, ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("PrimaryDeleted", СтрокаКлиента.УдалениеОбъекта, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("blocked"       , СтрокаКлиента.ПометкаУдаления, ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("clientname", СокрЛП(СтрокаКлиента.Наименование), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("firstname" , СокрЛП(СтрокаКлиента.Имя), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("lastname"  , СокрЛП(СтрокаКлиента.Фамилия), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("middlename", СокрЛП(СтрокаКлиента.Отчество), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("phoneNumber", СокрЛП(СтрокаКлиента.НомерТелефона), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("phone" , СокрЛП(СтрокаКлиента.НомерТелефонаСлужебный), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("email" , СокрЛП(СтрокаКлиента.Электропочта), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("email2", СокрЛП(СтрокаКлиента.ЭлектропочтаСлужебный), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("mailingAddress", СокрЛП(СтрокаКлиента.АдресПочтовый), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("gender"   , СокрЛП(СтрокаКлиента.Пол), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("birthdate", Формат(СтрокаКлиента.ДатаРождения, "ДФ=yyyy/MM/dd"), ЗаписьJSON);
					...
					Если ДетиКлиентаВыборка.НайтиСледующий(Новый Структура("Контрагент", СтрокаКлиента.Контрагент)) Тогда
						...
						Пока ДетиВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("IDC_child", СокрЛП(ДетиВыборка.УникальныйИдентификатор), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("name"     , СокрЛП(ДетиВыборка.Имя), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("gender"   , СокрЛП(ДетиВыборка.Пол), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("birthdate", Формат(ДетиВыборка.ДатаРождения, "ДФ=yyyy/MM/dd"), ЗаписьJSON);
					...
					Если КартыКлиентаВыборка.НайтиСледующий(Новый Структура("Контрагент", СтрокаКлиента.Контрагент)) Тогда
						...
						Пока КартыВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("IDC_loyalCard" , СокрЛП(КартыВыборка.ДисконтнаяКарта.УникальныйИдентификатор()), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("IDC_store"     , СокрЛП(КартыВыборка.Магазин.УникальныйИдентификатор()), ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("blocked"       , КартыВыборка.ПометкаУдаления, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("number"        , СокрЛП(КартыВыборка.КодКарты), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("activationDate", Формат(КартыВыборка.ДатаОткрытия, "ДФ=yyyy/MM/dd"), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("endDate"       , Формат(КартыВыборка.ДатаЗакрытия, "ДФ=yyyy/MM/dd"), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("creationDate1C", Формат(КартыВыборка.ДатаСоздания, "ДФ=yyyy/MM/dd"), ЗаписьJSON);
		...
		Если НЕ Результаты[1].Пустой() Тогда
			...
			Пока ШапкаВыборка.Следующий() Цикл
				...
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
					...
					РаботаСJSONСервер.ЗаписатьСвойство("IDD_requestAvailability", IDC_Строкой(ШапкаВыборка.Ссылка), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("numberSeats", ШапкаВыборка.КоличествоМест, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("status"     , ШапкаВыборка.Статус, ЗаписьJSON);
					...
					Если ТоварыВыборка.НайтиСледующий(Новый Структура("Ссылка", ШапкаВыборка.Ссылка)) Тогда
						...
						Пока ТаблицаВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("IDD_itemAvailability", ТаблицаВыборка.УникальныйИдентификатор, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("IDN_SKU"     , ТаблицаВыборка.IDN, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("amountActual", ТаблицаВыборка.Количество, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("commentStok" , СокрЛП(Лев(ТаблицаВыборка.Примечание, 4000)), ЗаписьJSON);
					...
					Если МестаВыборка.НайтиСледующий(Новый Структура("Ссылка", ШапкаВыборка.Ссылка)) Тогда
						...
						Пока ТаблицаВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("width" , ТаблицаВыборка.Ширина, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("height", ТаблицаВыборка.Высота, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("length", ТаблицаВыборка.Глубина, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("weight", ТаблицаВыборка.Вес, ЗаписьJSON);
		...
		Если НЕ Результаты[2].Пустой() Тогда
			...
			Пока ШапкаВыборка.Следующий() Цикл
				...
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
					...
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_sale"   , IDC_Строкой(ШапкаВыборка.Ссылка), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("date"       , Формат(ШапкаВыборка.Дата, "ДФ='yyyy/MM/dd ЧЧ:мм:сс'"), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("number"     , СокрЛП(ШапкаВыборка.Номер), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("Posted"     , ШапкаВыборка.Проведен, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("typeDoc"    , ШапкаВыборка.ВидДокумента, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("operation"  , ШапкаВыборка.ВидОперации, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_Base"   , IDC_Строкой(ШапкаВыборка.ЧекККМПродажа), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_CashDay", IDC_Строкой(ШапкаВыборка.КассовыйДень), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("legalEntity", КодОрганизации, ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("IDD_client", СокрЛП(ШапкаВыборка.IDD_client), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDС_client", IDC_Строкой(ШапкаВыборка.Клиент), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("loyalCard"    , СокрЛП(ШапкаВыборка.КодКарты), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_loyalCard", IDC_Строкой(ШапкаВыборка.ДисконтнаяКарта), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("phoneNumberClient", СокрЛП(ШапкаВыборка.НомерТелефона), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("numberOrder", СокрЛП(ШапкаВыборка.НомерЗаказа), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDD_Order"  , IDC_Строкой(ШапкаВыборка.ЗаказПокупателя), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDC_Order"  , IDC_Строкой(ШапкаВыборка.ЗаказПокупателя), ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("IDD_store"  , СокрЛП(ШапкаВыборка.IDD_store), ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("totalSumNoDiscount"  , ШапкаВыборка.СуммаБезСкидки, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("totalSumWithDiscount", ШапкаВыборка.СуммаСоСкидкой, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("totalSumToPay"       , ШапкаВыборка.СуммаОплаты, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("paid"                , ШапкаВыборка.СуммаОплаты, ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("sumBonusesUsedBase"  , ШапкаВыборка.БонусБазаСписан, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("sumBonusesUsedPromo" , ШапкаВыборка.БонусАкцияСписан, ЗаписьJSON);

					РаботаСJSONСервер.ЗаписатьСвойство("CustPurchaseAmount"  , РозничныеПродажиСервер.ПолучитьСуммуНакоплений(ШапкаВыборка.Клиент), ЗаписьJSON);
					...
					Если ТоварыВыборка.НайтиСледующий(Новый Структура("Ссылка", ШапкаВыборка.Ссылка)) Тогда
						...
						Пока ТаблицаВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("rowNum", ТаблицаВыборка.НомерСтроки, ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("SKU"   , СокрЛП(ТаблицаВыборка.IDN), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("amount", ТаблицаВыборка.Количество, ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("amountunit", ТаблицаВыборка.КоличествоУпаковок, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("unitMeasurement", СокрЛП(ТаблицаВыборка.Упаковка), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("coefficient", ТаблицаВыборка.Коэффициент, ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("price", ТаблицаВыборка.ЦенаСНДС, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("discountHrn"  , ТаблицаВыборка.СуммаАвтоматическойСкидки, ЗаписьJSON);
							...
							//	Они НЕ УМЕЮТ РАБОТАТЬ С ОТРИЦАТЕЛЬНЫМИ ЧИСЛАМИ !!!
								...
								РаботаСJSONСервер.ЗаписатьСвойство("natcenkaHrn"  , СуммаАвтоматическойНаценки, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("manualDiscHrn", СуммаРучнойСкидки, ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("roundamount"  , ТаблицаВыборка.СуммаОкругления, ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("vatGroup"     , СокрЛП(ТаблицаВыборка.СтавкаНДС), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("PromoPrice"   , ТаблицаВыборка.АкционнаяЦена, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("isPresent"	 , ТаблицаВыборка.ПродажаПодарка, ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("bonusesUsedPromo"   , ТаблицаВыборка.БонусАкцияСписан, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("bonusesUsedBase"    , ТаблицаВыборка.БонусБазаСписан, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("bonusesAccruedBase" , ТаблицаВыборка.БонусБазаНачислен, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("bonusesAccruedPromo", ТаблицаВыборка.БонусАкцияНачислен, ЗаписьJSON);

								РаботаСJSONСервер.ЗаписатьСвойство("itemTotalSumWithVat", ТаблицаВыборка.СуммаСНДС, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("interactionWithLp"  , Ложь, ЗаписьJSON);
					...
					Если КупоныВыборка.НайтиСледующий(Новый Структура("Ссылка", ШапкаВыборка.Ссылка)) Тогда
						...
						Пока ТаблицаВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("number"   , СокрЛП(ТаблицаВыборка.НомерКупона), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("operation", ТаблицаВыборка.Операция, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("sum"      , ТаблицаВыборка.Сумма, ЗаписьJSON);
					...
					Если СкидкиВыборка.НайтиСледующий(Новый Структура("Ссылка", ШапкаВыборка.Ссылка)) Тогда
						...
						Пока ТаблицаВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("rowNum"        , ТаблицаВыборка.КлючСвязи, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("sequenceNumber", 0, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("IDD_campaign"  , СокрЛП(ТаблицаВыборка.УникальныйИдентификатор), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("discount"      , ТаблицаВыборка.СуммаСкидки, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("bonus"         , 0, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("saleItemSumBeforeUsedCampaign", 0, ЗаписьJSON);
					...
					Если ОплатаВыборка.НайтиСледующий(Новый Структура("Ссылка", ШапкаВыборка.Ссылка)) Тогда
						...
						Пока ТаблицаВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("IDC_payment", IDC_Строкой(ТаблицаВыборка.ВидОплаты), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("type", ОбменMicrosoftDynamicsПовтИсп.ПолучитьКодТипаОплаты(ТаблицаВыборка.ТипОплаты, ТаблицаВыборка.ТипУслугиБанка), ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("sum" , ТаблицаВыборка.Сумма, ЗаписьJSON);
					...
					Если СертификатыВыборка.НайтиСледующий(Новый Структура("Ссылка", ШапкаВыборка.Ссылка)) Тогда
						...
						Пока ТаблицаВыборка.Следующий() Цикл
							...
							ЗаписьJSON.ЗаписатьНачалоОбъекта();
								...
								РаботаСJSONСервер.ЗаписатьСвойство("number" , ТаблицаВыборка.НомерСертификата, ЗаписьJSON);
								РаботаСJSONСервер.ЗаписатьСвойство("nominal", ТаблицаВыборка.Номинал, ЗаписьJSON);
...
Процедура ДанныеПодтверждения(ВнешниеДанные, УзелОбъект, ТипРегистрации, АдресСервера)
	...
	РаботаСJSONСервер.ЗаписатьСвойство("IDBASE"       , ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("methodName"   , ОбменMicrosoftDynamicsПовтИсп.ИмяПроцедурыWebAPI(ТипРегистрации, УзелОбъект.Процедура), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("returnInfoFor", "Success", ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("UUID_noInfo"  , СокрЛП(Новый УникальныйИдентификатор), ЗаписьJSON);
...
//	---------------------------------------------------------------------------------------
	...
	РаботаСJSONСервер.ЗаписатьСвойство("IDBASE"       , ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("methodName"   , ОбменMicrosoftDynamicsПовтИсп.ИмяПроцедурыWebAPI(ТипРегистрации, УзелОбъект.Процедура), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("returnInfoFor", "Last", ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("UUID_noInfo"  , СокрЛП(Новый УникальныйИдентификатор), ЗаписьJSON);
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			ЗаписьJSON.ЗаписатьИмяСвойства("client");
				...
				ЗаписьJSON.ЗаписатьНачалоОбъекта();
					...
					РаботаСJSONСервер.ЗаписатьСвойство("phoneNumber", Идентификаторы.НомерТелефона, ЗаписьJSON);
					РаботаСJSONСервер.ЗаписатьСвойство("cardNumber" , Идентификаторы.КодКарты, ЗаписьJSON);
...
Функция ДанныеКассовогоДняCRM(КассовыйДень, ТекстОшибки)	Экспорт
	...
	Если НЕ ПустаяСтрока(АдресСервера) Тогда
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			РаботаСJSONСервер.ЗаписатьСвойство("IDBASE", ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("ИдентификаторУчетнойСистемы"), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("message1C", -1, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("IDC_CashDay", IDC_Строкой(КассовыйДень), ЗаписьJSON);
...
Функция ИдентифицироватьКлиентаПоДаннымCRM(ДокументПродажи, ДанныеКонтрагента, КодКарты, НомерТелефона, ДанныеВнешние, ПараметрыИдентификации = Неопределено, ДанныеФИО = Неопределено, УИКлиента = "")	Экспорт
	...
	Если НЕ ДанныеВнешниеПолучены Тогда
		...
		ИмитацияДанныхОтветаCRM(ДанныеВнешние, КодКарты, НомерТелефона, ДанныеФИО, УИКлиента);
...
Функция ИмитацияДанныхОтветаCRM(ДанныеВнешние, КодКарты, НомерТелефона, ДанныеФИО, УИКлиента)
	...
	ДанныеВнешние = Новый Структура(
		...
		, УИКлиента, "", "", 0, 0, Ложь, НомерТелефона, ДанныеФИО.Фамилия, ДанныеФИО.Имя, ДанныеФИО.Отчество, Новый Массив, Новый Массив
	);
...
Функция ПолучитьТекстВнешнегоЗапроса(ДанныеДокумента)
	...
	РаботаСJSONСервер.ЗаписатьСвойство("IDC_sale"  , ОбменMicrosoftDynamicsСлужебный.IDC_Строкой(ДанныеДокумента.ДокументПродажи), ЗаписьJSON);

	РаботаСJSONСервер.ЗаписатьСвойство("IDC_client", ОбменMicrosoftDynamicsСлужебный.IDC_Строкой(СсылкиВыборка.Контрагент), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("IDD_client", СсылкиВыборка.IDD_client, ЗаписьJSON);

	РаботаСJSONСервер.ЗаписатьСвойство("IDC_store" , ОбменMicrosoftDynamicsСлужебный.IDC_Строкой(СсылкиВыборка.Магазин), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("IDD_store" , СсылкиВыборка.IDD_store, ЗаписьJSON);

	РаботаСJSONСервер.ЗаписатьСвойство("bonusesUsedPromo"  , ДанныеДокумента.ИспользоватьБонусАкция, ЗаписьJSON);	//	LNK 08.11.2019 11:25:01
	РаботаСJSONСервер.ЗаписатьСвойство("sumBonusesUsedBase", ДанныеДокумента.СуммаБонусБаза, ЗаписьJSON);			//	LNK 08.11.2019 11:25:01

	РаботаСJSONСервер.ЗаписатьСвойство("CustPurchaseAmount", РозничныеПродажиСервер.ПолучитьСуммуНакоплений(СсылкиВыборка.Контрагент), ЗаписьJSON);
	...
	ЗаписьJSON.ЗаписатьИмяСвойства("saleItems");	//	ТОВАРЫ начало
		...
		Пока ТоварыВыборка.Следующий() Цикл
			...
			РаботаСJSONСервер.ЗаписатьСвойство("rowNum"        , ТоварыВыборка.НомерСтроки, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("SKU"           , ТоварыВыборка.IDN, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("amount"        , ТоварыВыборка.Количество, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("amountunit"    , ТоварыВыборка.КоличествоУпаковок, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("coefficient"   , ТоварыВыборка.Коэффициент, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("price"         , ТоварыВыборка.Цена, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("manualdiscount", ТоварыВыборка.СуммаРучнойСкидки, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("PromoPrice"    , ТоварыВыборка.АкционнаяЦена, ЗаписьJSON);
...
Функция ПолучитьДанныеКупона(НомерКупона, ТекстОшибки = "")	Экспорт
	...
	Если НЕ ПустаяСтрока(АдресСервера) Тогда
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			РаботаСJSONСервер.ЗаписатьСвойство("numberCupon", НомерКупона, ЗаписьJSON);

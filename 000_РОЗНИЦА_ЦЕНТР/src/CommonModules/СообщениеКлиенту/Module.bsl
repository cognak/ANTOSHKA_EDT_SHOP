...
Процедура ОтправитьViber(УзелОбмена, ОтправительViber)
	...
	|	СообщениеКлиенту.ТегиСообщения КАК ТегиСообщения
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПустаяСтрока(Выборка.ТекстСообщения) Тогда
		...
		МассивТегов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СокрЛП(Выборка.ТегиСообщения));
		
		Если МассивТегов.Количество() > 0 Тогда 
			СтруктураЗапроса.Вставить("tags"		, МассивТегов);  
		КонецЕсли;	
		...
		Если ОтветСервера.ТелоОтвета.Свойство("results")
			...
			СтруктураСообщенияКлиенту.ТегиСообщения				= Выборка.ТегиСообщения;

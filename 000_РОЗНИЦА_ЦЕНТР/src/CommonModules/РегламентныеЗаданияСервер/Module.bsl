...
Процедура ВыполнитьЗакрытиеОднойСменыНаСервере(КассаККМ, РабочееМесто)
	...
	ОбработкаОбъект = Обработки.ЗакрытиеКассовойСмены.Создать();   
	ОбработкаОбъект.КассаККМ = КассаККМ; 
	ОбработкаОбъект.ОбработатьЧекиКассовойСмены(ОтчетОРозничныхПродажах, Отказ, СписокДокументов, СуммаВыемки, СсылкаНаОтчет, Ложь);
	...
	ОписаниеОшибки = "";
	Результат = РозничныеПродажиСервер.ЗакрытьКассовуюСмену(КассаККМ, ОписаниеОшибки, СсылкаНаОтчет, РабочееМесто);

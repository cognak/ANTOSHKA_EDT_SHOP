...
Процедура ОбновитьРегистрСостоянияОплатЧеков() Экспорт 	//А++ 20250103 по задаче оптимизации работы журнала. 
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда 
		...
		|ГДЕ
		|	ТаблицаВедущая.Дата > &ДатаН
		|	И НЕ ТаблицаВедущая.Ссылка В
		|				(ВЫБРАТЬ
		|					СостояниеОплатЧеков.ЧекККМ КАК ЧекККМ
		|				ИЗ
		|					РегистрСведений.СостояниеОплатЧеков КАК СостояниеОплатЧеков)
		|{ГДЕ
		...
		Запрос.УстановитьПараметр("ДатаН",Дата(2022,01,01,00,00,00));
		РезультатЗапроса = Запрос.Выполнить();

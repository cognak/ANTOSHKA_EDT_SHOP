...
//	Второй элемент [1] может отсутствовать и содержит данные расчета бонусов.
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМ") ИЛИ   ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
...
Функция ПолучитьСкидкиПодаркиПереносуВТовары(ТаблицаПодарков)
	...
	|	ТаблицаТоваров.Упаковка,
	|	ТаблицаТоваров.КоличествоУпаковок,
	|	ТаблицаТоваров.Склад.Организация КАК Организация
	|ИЗ
	|	ТаблицаТоваров КАК ТаблицаТоваров
	|ГДЕ
	|	(НЕ ТаблицаТоваров.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|	И ТаблицаТоваров.СкидкаНаценка.УчитыватьПодарокКакПродажу
	|	И ТаблицаТоваров.Цена > 0
	|	И ТаблицаТоваров.Сумма > 0
	|	И ТаблицаТоваров.Количество > 0
	|	И (НЕ ТаблицаТоваров.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";

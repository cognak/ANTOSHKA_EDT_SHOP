...
//	Второй элемент [1] может отсутствовать и содержит данные расчета бонусов.
	...
	Для НомерРасчета = 1 По 2 Цикл
		...
		//Сичкаренко 28.07.2025
		ЕстьОплатаАкционнимиБонусами = Ложь;

		Если ВходныеПараметры.Свойство("НеНачислятьБонусыПриСписанииАкционныхБонусов") И НомерРасчета = 1 Тогда
			
			СтрокиСАкционнимиБонусами = Новый Соответствие;
			
			Для Каждого СтрокаОплатыБонусов Из Объект.ОплатаБонуснымиБаллами Цикл
				
				Если СтрокаОплатыБонусов.ТипБонусов = Перечисления.ТипыБонусов.Акционный Тогда
					
					СтрокиСАкционнимиБонусами.Вставить(СтрокаОплатыБонусов.КлючСвязиБонусныхБаллов, СтрокаОплатыБонусов.КлючСвязиБонусныхБаллов);
					ЕстьОплатаАкционнимиБонусами = Истина;
		...
		Если Не ЕстьОплатаАкционнимиБонусами Тогда
			
			Прервать;
			
		Иначе
			
			ВходныеПараметры.Вставить("СтрокиСАкционнимиБонусами", СтрокиСАкционнимиБонусами);
...
Процедура УточнитьВходныеПараметрыРасчетаСкидок(Объект, ВходныеПараметры)	//	LNK 07.06.2021 11:39:30
	...
	Если ВходныеПараметры.Свойство("ЗаказМадженто") Тогда 
		ВходныеПараметры.Вставить("СписыватьАкционныеБонусныеБаллы", Ложь);
	Иначе
	...
	//Сичкаренко 28.07.2025
	УчетнаяПолитика = ОбщегоНазначенияРТповтИсп.ПолучитьУчетнуюПолитику(ВходныеПараметры.ДатаРасчета);
	Если УчетнаяПолитика.НеНачислятьБонусыПриСписанииАкционныхБонусов Тогда
		ВходныеПараметры.Вставить("НеНачислятьБонусыПриСписанииАкционныхБонусов", Истина);
	КонецЕсли;
...
//	LNK 25.10.2022 05:43:38
  	...
  	Если УчетнаяПолитика.ИспользоватьСкидкуСотрудника  тогда      // Бакан 20250609 добавил условие
...
Функция ПодготовитьТоварыКРасчетуСкидок(КоллекцияТовары, КолонкиТовары, ВходныеПараметры)
	...
	//Сичкаренко 28.07.2025
	//Устанавливаем, что товар акционный, если на него списаны акционные бонусы
	Если ВходныеПараметры.Свойство("СтрокиСАкционнимиБонусами") Тогда

		Если ТаблицаТовары.Колонки.Найти("АкционныйТовар") = Неопределено Тогда

			ТаблицаТовары.Колонки.Вставить(0, "АкционныйТовар", Новый ОписаниеТипов("Булево"));
			
		КонецЕсли;
		
		СтрокиСАкционнимиБонусами = ВходныеПараметры.СтрокиСАкционнимиБонусами;
		
		Для Каждого СтрокаТовара Из ТаблицаТовары Цикл
			
			СтрокаТовара.АкционныйТовар = СтрокаТовара.АкционнаяЦена;
			
			Если Не СтрокиСАкционнимиБонусами.Получить(СтрокаТовара.КлючСвязиБонусныхБаллов) = Неопределено Тогда
				
				СтрокаТовара.АкционнаяЦена = Истина;

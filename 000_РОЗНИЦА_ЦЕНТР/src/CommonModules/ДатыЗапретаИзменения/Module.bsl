...
Функция ИзменениеЗапрещено(Знач Данные,
                           ...
                           Знач СообщитьОЗапрете     = Истина,
                                ДанныеЗапретаПоИнвентаризации = Неопределено) Экспорт
...
Функция ЗагрузкаЗапрещена(Данные, УзелПланаОбмена, СообщениеОбОшибке = "") Экспорт
	...
	ДатыЗапретаИзмененияСлужебный.ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(
		...
		УзелПланаОбмена,
		НайденныеЗапреты,
		УстановитьДанныеЗапретаПоИнвентаризации(Данные));
	
	Если Отказ И НайденныеЗапреты.Свойство("НайденЗапретЗагрузкиДанных") Тогда
...
Функция ОбъектПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	Если НЕ ТехническаяПоддержкаВызовСервера.ОтладочныйРежимРаботы() Тогда
		...
		Если ДатыЗапретаИзмененияСлужебный.ПропуститьПроверкуДатЗапрета(Источник, Ложь, Истина, Неопределено)
		Тогда
		...
		Если ИзменениеЗапрещено(ПолноеИмя, ИдентификаторДанных, Ложь, УстановитьДанныеЗапретаПоИнвентаризации(ТекущийОбъект)) Тогда
...
//	LNK 22.12.2016 11:15:35
Функция УстановитьДанныеЗапретаПоИнвентаризации(ДанныеОбъект = Неопределено)

	Перем ДокументОграничения;

	ДанныеЗапретаПоИнвентаризации = Новый Структура(
		"ЕстьИсключение, ДокументОграничения"
		, Ложь, Неопределено);

	Попытка

		ДополнительныеСвойства = ДанныеОбъект.ДополнительныеСвойства;
	...
	//	Для типов ДокументОбъект может быть указано "в корне" структуры...
		ДополнительныеСвойства.Свойство("ИсключитьЗапретПоИнвентаризации", ДокументОграничения);

		Если ДокументОграничения = Неопределено И ДополнительныеСвойства.Свойство("СвойстваРегистратора") Тогда

		//	Не указано. Для типов "НаборЗаписей" такое же свойство может быть во "вложенном" свойстве
			ДополнительныеСвойства.СвойстваРегистратора.Свойство("ИсключитьЗапретПоИнвентаризации", ДокументОграничения);

		КонецЕсли;

		Если НЕ ДокументОграничения = Неопределено Тогда

			Если (ТипЗнч(ДокументОграничения) = Тип("ДокументСсылка.ПриказНаПроведениеИнвентаризацииТоваров") ИЛИ ТипЗнч(ДокументОграничения) = Тип("ДокументСсылка.ПересчетТоваровСамостоятельный"))
				И ЗначениеЗаполнено(ДокументОграничения)	Тогда

				ДанныеЗапретаПоИнвентаризации.ЕстьИсключение      = Истина;
				ДанныеЗапретаПоИнвентаризации.ДокументОграничения = ДокументОграничения;
	...
	Возврат ДанныеЗапретаПоИнвентаризации;

КонецФункции // УстановитьДанныеЗапретаПоИнвентаризации()
...
//  ПолеОбъекта - Строка.
//
Процедура ДобавитьСтроку(Данные, Таблица, ПолеДаты, Раздел = "", ПолеОбъекта = "", ПолеСклада = "") Экспорт
	...
	НоваяСтрока.ПолеСклада  = ПолеСклада;
	
КонецПроцедуры

//	LNK 05.01.2018 14:36:01
Процедура ПодготовитьТаблицыДанныхДляПроверки(ДанныеДляПроверки, ДанныеДляПроверкиСклада)

//	Свертка лишних строк, чтобы сократить число проверок и сообщений.
	ДанныеДляПроверки.Свернуть("Дата, Раздел, Объект");
	ДанныеДляПроверкиСклада.Свернуть("Дата, Объект, Склад");

//	Назначение ключей для отдельных проверок в таблице "ДанныеДляПроверки".
...
//	Назначение ключей для отдельных проверок в таблице "ДанныеДляПроверки".
	ДанныеДляПроверкиСклада.Колонки.Добавить("КлючСтроки", Новый ОписаниеТипов("Число"));
	ДанныеДляПроверкиСклада.Колонки.Добавить("ИспользоватьПересчетыСамостоятельные", Новый ОписаниеТипов("Булево"));
	ТекущийКлючСтроки = 0;

	Для каждого Строка Из ДанныеДляПроверкиСклада Цикл

		Строка.КлючСтроки = ТекущийКлючСтроки;
		ТекущийКлючСтроки = ТекущийКлючСтроки + 1;

		Если ТипЗнч(Строка.Объект) = Тип("СправочникСсылка.Магазины") И НЕ Строка.Объект.Пустая() Тогда

			Строка.ИспользоватьПересчетыСамостоятельные = СкладскиеОперацииСерверПовтИсп.БлокироватьДвиженияПоПересчетуСамостоятельному(Строка.Объект);
...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки,
									...
									Знач ДанныеЗапретаПоИнвентаризации = Неопределено) Экспорт
	...
	//	LNK Таблица со значениями реквизитов объекта для проверки
	...
	Иначе
		...
		Иначе	ТаблицаДанных = ИдентификаторДанных.Метаданные().ПолноеИмя();

		КонецЕсли;

	КонецЕсли;

//	LNK 22.12.2016 11:37:21
	Если ДанныеЗапретаПоИнвентаризации = Неопределено Тогда

		ДанныеЗапретаПоИнвентаризации = УстановитьДанныеЗапретаПоИнвентаризации();
...
//	Приведение пустых ссылок объектов к единому значению Неопределено.
	...
	ДанныеДляПроверкиСклада = ДанныеДляПроверки.Скопировать();
	ПодготовитьТаблицыДанныхДляПроверки(ДанныеДляПроверки, ДанныеДляПроверкиСклада);
	...
	//	LNK 21.12.2016 14:01:34
		Если СвойстваВстраивания.ИспользоватьПриказыНаИнвентаризацию Тогда

			Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	ДанныеДляПроверки.Дата КАК Дата,
			|	ДанныеДляПроверки.Объект КАК Объект,
			|	ДанныеДляПроверки.Склад КАК Склад,
			|	ДанныеДляПроверки.ИспользоватьПересчетыСамостоятельные КАК ИспользоватьПересчетыСамостоятельные
			|ПОМЕСТИТЬ НачальныеДанные
			|ИЗ
			|	&ДанныеДляПроверки КАК ДанныеДляПроверки
			|ГДЕ
			|	ТИПЗНАЧЕНИЯ(ДанныеДляПроверки.Объект) = ТИП(Справочник.Магазины)
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Объект,
			|	Склад
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ ПЕРВЫЕ 1
			|	ОграниченияПоИнвентаризации.ГраницаЗапрета КАК Дата,
			|	ОграниченияПоИнвентаризации.ДокументОграничения КАК Ссылка,
			|	ОграниченияПоИнвентаризации.ДокументОграничения КАК Представление,
			|	НачальныеДанные.Объект КАК Магазин
			|ИЗ
			|	НачальныеДанные КАК НачальныеДанные
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОграниченияДвиженийПоИнвентаризации.СрезПоследних(
			|				,
			|				Магазин В
			|						(ВЫБРАТЬ
			|							НачальныеДанные.Объект
			|						ИЗ
			|							НачальныеДанные)
			|					И ВЫБОР
			...
			|						ИНАЧЕ Склад В
			|								(ВЫБРАТЬ
			|									НачальныеДанные.Склад
			|								ИЗ
			|									НачальныеДанные)
			|					КОНЕЦ
			|					И ВЫБОР
			|						КОГДА &ЕстьИсключение = ИСТИНА
			|							ТОГДА ДокументОграничения <> &ДокументОграничения
			|						ИНАЧЕ ИСТИНА
			|					КОНЕЦ) КАК ОграниченияПоИнвентаризации
			|		ПО НачальныеДанные.Объект = ОграниченияПоИнвентаризации.Магазин
			|			И (ВЫБОР
			|				КОГДА НачальныеДанные.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ НачальныеДанные.Склад = ОграниченияПоИнвентаризации.Склад
			|			КОНЕЦ)
			|ГДЕ
			|	НачальныеДанные.Дата <= ВЫБОР
			|			КОГДА ОграниченияПоИнвентаризации.ДокументОграничения ССЫЛКА Документ.ПересчетТоваровСамостоятельный
			|				ТОГДА ОграниченияПоИнвентаризации.ГраницаЗапрета
			|			ИНАЧЕ КОНЕЦПЕРИОДА(ОграниченияПоИнвентаризации.ГраницаЗапрета, ДЕНЬ)
			|		КОНЕЦ
			|	И ВЫБОР
			|			КОГДА ОграниченияПоИнвентаризации.ДокументОграничения ССЫЛКА Документ.ПересчетТоваровСамостоятельный
			|				ТОГДА НачальныеДанные.ИспользоватьПересчетыСамостоятельные = ИСТИНА
			|			ИНАЧЕ ИСТИНА
			|		КОНЕЦ
			|
			|УПОРЯДОЧИТЬ ПО
			...
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ НачальныеДанные"
			);
			Запрос.УстановитьПараметр("ДанныеДляПроверки"  , ДанныеДляПроверкиСклада);
			Запрос.УстановитьПараметр("ЕстьИсключение"     , ДанныеЗапретаПоИнвентаризации.ЕстьИсключение);
			Запрос.УстановитьПараметр("ДокументОграничения", ДанныеЗапретаПоИнвентаризации.ДокументОграничения);
			
			Выборка = Запрос.Выполнить().Выбрать();
			
			Если Выборка.Следующий() Тогда

				ТекстСообщения = "Изменения ограничены документом «" + Выборка.Представление + "» по магазину <"
					+ ?(ЗначениеЗаполнено(Выборка.Магазин), СокрЛП(Выборка.Магазин), "Все магазины") + ">";

				Если НЕ НайденныеЗапреты.Свойство("НайденЗапретИзмененияДанных") Тогда

					НайденныеЗапреты.Вставить("НайденЗапретИзмененияДанных", ТекстСообщения);

				КонецЕсли;

				Если НЕ УзелПланаОбмена = Неопределено И НЕ НайденныеЗапреты.Свойство("НайденЗапретЗагрузкиДанных") Тогда

					НайденныеЗапреты.Вставить("НайденЗапретЗагрузкиДанных", ТекстСообщения);

				КонецЕсли;

				СообщитьОЗапретеПользователю(ТекстСообщения, НЕ УзелПланаОбмена = Неопределено, СообщитьОЗапрете);
...
Процедура ПроверитьДатуЗапретаИзмененияПередЗаписью(Источник, Отказ) Экспорт
	...
	ИЛИ  ОбщегоНазначенияКлиентСервер.ЗначениеКлючаСтруктуры(Источник.ДополнительныеСвойства, "СлужебнаяЗапись", Ложь) = Истина	//	LNK 20.11.2021 21:20:33
	Тогда
	...
	ДатыЗапретаИзмененияСлужебный.ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(
		Источник, Отказ,,,,,,,, УстановитьДанныеЗапретаПоИнвентаризации(Источник));
...
Процедура ПроверитьДатуЗапретаИзмененияПередЗаписьюДокумента(
	...
	ДатыЗапретаИзмененияСлужебный.ПроверитьДоступностьИзмененияКассовыхОрдеров(Источник, Отказ);
...
Процедура ПроверитьДатуЗапретаИзмененияПередЗаписьюНабораЗаписей(Источник, Отказ, Замещение) Экспорт
	...
	ДатыЗапретаИзмененияСлужебный.ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(
		Источник, Отказ, Истина, Замещение,,,,,, УстановитьДанныеЗапретаПоИнвентаризации(Источник));
...
Процедура ПроверитьДатуЗапретаИзмененияПередЗаписьюНабораЗаписейРегистраБухгалтерии(
	...
	ДатыЗапретаИзмененияСлужебный.ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(
		Источник, Отказ, Истина,,,,,,, УстановитьДанныеЗапретаПоИнвентаризации(Источник));
...
Процедура ДобавитьДанныеДляПроверки(Знач ДанныеДляПроверки,
	...
	Для каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		Иначе
			...
			ДопИсточникДанных = Новый Структура("ПолеДаты, Раздел, ПолеОбъекта, ПолеСклада, Таблица");
		...
		//	LNK 05.01.2018 14:52:29
			ТЧПоляСклада = "";
			ПолеСклада   = "";

			Если ЗначениеЗаполнено(ДопИсточникДанных.ПолеСклада) Тогда
				ПозицияТочки = Найти(ДопИсточникДанных.ПолеСклада, ".");
				Если ПозицияТочки <> 0 Тогда
					ТЧПоляСклада = Лев(ДопИсточникДанных.ПолеСклада, ПозицияТочки - 1);
					Если ОбъектМетаданных.ТабличныеЧасти.Найти(ТЧПоляСклада) = Неопределено Тогда
						ТЧПоляСклада = "";
					Иначе
						ДопИсточникДанных.ПолеСклада = Сред(
							ДопИсточникДанных.ПолеСклада, ПозицияТочки + 1);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;	//	LNK 10.01.2018 13:41:56 - Ниже, в этой ветке не используется... нет модели для проверки
...
//	LNK 05.01.2018 15:07:21
	Если ЗначениеЗаполнено(ИсточникДанных.ПолеСклада) Тогда
		УстановитьЗначениеПоля(СтрокаОбъекта, НоваяСтрока.Склад, ИсточникДанных.ПолеСклада)
	КонецЕсли;
...
Функция НайденЗапретИзмененияИлиЗагрузкиДанных(Запрос,
	...
	|			И (ВЫБОР
	|				КОГДА &ВидНазначенияДатЗапретаДляВсех В (ЗНАЧЕНИЕ(Перечисление.ВидыНазначенияДатЗапрета.ДляВсехПользователей), ЗНАЧЕНИЕ(Перечисление.ВидыНазначенияДатЗапрета.ДляВсехИнформационныхБаз))
	|						ИЛИ ДатыЗапретаИзменения.Пользователь = &ВидНазначенияДатЗапретаДляВсех
	|					ТОГДА ИСТИНА
	...
	Если ЗапретыДанных.Строки.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(Текст) Тогда
			Если ПоискЗапретовЗагрузки Тогда
				Текст = Текст
					+ НСтр("ru = ' запрещено загружать в закрытом периоде.'")
					+ Символы.ПС
					+ Символы.ПС;
			Иначе
				Текст = Текст
					+ НСтр("ru = ' запрещено изменять или перемещать в закрытом периоде.'")
					+ Символы.ПС
					+ Символы.ПС;
			КонецЕсли;
		КонецЕсли;
		...
		Для каждого ОписаниеЗапретов Из ЗапретыДанных.Строки Цикл
			...
			Для каждого Запрет Из Запреты Цикл
				...
				Если Запрет.Пользователь = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей ИЛИ НЕ ЗначениеЗаполнено(Запрет.Пользователь) Тогда
				...
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						?(ЗначениеЗаполнено(Запрет.Пользователь), Запрет.Пользователь.Метаданные().Представление(), "все")) + Символы.ПС;
			КонецЦикла;
			Текст = Текст + Символы.ПС;
		КонецЦикла;
		
		СообщитьОЗапретеПользователю(Текст, ПоискЗапретовЗагрузки, СообщитьОЗапрете);
...
//	LNK 22.12.2016 12:08:23
Процедура СообщитьОЗапретеПользователю(ТекстСообщения, ЗапретЗагрузки, СообщитьОЗапрете)
	...
	//	тогда выполняется запись в журнал регистрации.
		...
		ЖурналСобытий.Регистрация(СобытиеЖурнала, УровеньЖурналаРегистрации.Ошибка
			,
			,
			,
			, ТекстСообщения

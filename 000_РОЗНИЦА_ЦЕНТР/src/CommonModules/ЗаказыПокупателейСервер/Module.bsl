...
Процедура АвтоматическоеИзменениеСтатусов() Экспорт
	...
	Запрос.Текст = Запрос.Текст + 
		...
		|	И ЗапросДоступностиТоваровТовары.Ссылка.ТипДоставки = ЗНАЧЕНИЕ(Перечисление.ТипДоставкиЗаказПокупателя.Самовывоз)
		|ИТОГИ
		|ПО
		|	ДокументЗаказа";
...
#Область ПереводЗапросаДоступностиСборка
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ИнтернетМагазин И Выборка.ЦентральныйСклад Тогда
...
#Область ОтменаТТННаОтмененныеЗапросы
...
#Область УстановкаРезервирования72Часа
...
#Область ВосстановлениеСтатусаВРегистре
...
#Область ЗакрытиеВозвратногоДокумента
		...
		Для Каждого СтрокаТовара Из ДанныеОтвета.data.productsImmediate Цикл
			...
			Для Каждого СтрокаСклада Из СтрокаТовара.balances Цикл
				...
				Если ВыборДляМассива Тогда
					...
					Если (ПервыйЭлементМассива Или Не МассивВыбора.Найти(КодСклада) = Неопределено)
							И Не НедостаточноКоличества Тогда
				...
				Иначе
					...
					Иначе
						...
						ДанныеВыбора.Добавить(ОбменMagentoСервер.ПолучитьСкладMagentoПоIDN(КодСклада).СкладВыбора,
						КодСклада + " (" + Строка(СтрокаСклада.qty) + ")");
...
//				КодСклада = СтрокаСклада.warehouse.code;
//				
//				Если СокрЛП(КодСклада) = "__SUPPLIER" И Не СтрокаСклада.qty = 0 Тогда
//					
//					ЕстьНаСкладеПоставщика = Истина;
//
//				КонецЕсли;
//				
//			КонецЦикла;
//	
//			Если Не ЕстьНаСкладеПоставщика Тогда
//				
//				ОбщийСклад = Ложь;
//				
//			КонецЕсли;

...
Процедура АвтоПечатьЧеков(ДополнительныйПараметр) Экспорт
	...
	Запрос.Текст =
		...
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СостояниеСтрокЗаказаПокупателя.СогласованоВNavision) КАК КоличествоСогласований,
		|	МАКСИМУМ(СостояниеСтрокЗаказаПокупателя.СогласованоВNavision)
		|	И МАКСИМУМ(СостояниеСтрокЗаказаПокупателя.ГотовКПродаже) КАК МожноПечатать,
		|	НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) >= ДАТАВРЕМЯ(2025, 3, 1, 0, 0, 0) КАК НоваяПечать
		|ПОМЕСТИТЬ ВТ
		|ИЗ
		...
		|	НАЧАЛОПЕРИОДА(&ТекущаяДата, ДЕНЬ) >= ДАТАВРЕМЯ(2025, 3, 1, 0, 0, 0)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|ГДЕ
		|	(ВременнаяТаблица.КоличествоСогласований = 1
		|	ИЛИ ВременнаяТаблица.НоваяПечать)
		|	И ВременнаяТаблица.ГотовКПродаже = 1

...
Процедура АвтоматическоеИзменениеСтатусов() Экспорт
	...
	Запрос.Текст = Запрос.Текст + 
		...
		|	И ЗапросДоступностиТоваровТовары.Ссылка.Проведен
		|	И
		|		ЗапросДоступностиТоваровТовары.Ссылка.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.Согласован)
		|ИТОГИ
		|ПО
		|	ДокументЗаказа";
...
#Область ПереводЗаказаПолучен
	...
	Пока ВыборкаДокументЗаказа.Следующий() Цикл
		...
		Если Выборка.Количество() = 1 Тогда
			...
			Если Выборка.Получен И Выборка.Отправлен Тогда

...
Процедура АвтоПечатьЧеков() Экспорт
	...
	Запрос.Текст =
		...
		|		ПО СостояниеСтрокЗаказаПокупателя.ЗаказПокупателя = ЧекККМТовары.Ссылка.ЗаказПокупателя
		|		И СостояниеСтрокЗаказаПокупателя.КлючСвязи = ЧекККМТовары.КлючСвязи
		|ГДЕ
		|	СостояниеСтрокЗаказаПокупателя.ГотовКПродаже
		|	И НЕ СостояниеСтрокЗаказаПокупателя.Продано
		|	И ТаблицаЗаказа.Дата > ДАТАВРЕМЯ(2025, 01, 01)
		|	И ТаблицаЗаказа.Проведен
		...
		|	И ЧекККМТовары.Ссылка ЕСТЬ NULL";

...
Функция ДоступностьДляРедактирования(ДокументСсылка) Экспорт
	...
	РеквизитыОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "МагазинОтправитель, СтатусЗапроса, МагазинПолучатель");
...
Процедура АвтоматическоеИзменениеСтатусов() Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрСведений.КлючевыеАтрибутыЗаказовПокупателей.СрезПоследних(,
		|		Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПокупателей.ГотовКПродаже)) КАК
		|		КлючевыеАтрибутыЗаказовПокупателейСрезПоследних
		|ГДЕ
		|	КлючевыеАтрибутыЗаказовПокупателейСрезПоследних.ЗаказПокупателя.Проведен
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		...
		|ИЗ
		|	РегистрСведений.КлючевыеАтрибутыЗаказовПокупателей.СрезПоследних(,
		|		Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПокупателей.ГотовКПеремещению)) КАК
		|		КлючевыеАтрибутыЗаказовПокупателейСрезПоследних
		|ГДЕ
		|	КлючевыеАтрибутыЗаказовПокупателейСрезПоследних.ЗаказПокупателя.Проведен
		|;
		...
		|ИЗ
		|	РегистрСведений.КлючевыеАтрибутыЗаказовПокупателей.СрезПоследних(,
		|		Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПокупателей.ГотовКДоставке)) КАК
		|		КлючевыеАтрибутыЗаказовПокупателейСрезПоследних
		|ГДЕ
		|	КлючевыеАтрибутыЗаказовПокупателейСрезПоследних.ЗаказПокупателя.Проведен
		|;
		...
		|				СтатусЭН.СтатусПосылки = ЗНАЧЕНИЕ(Перечисление.СтатусыПосылокСлужбыДоставки.ВДороге)) КАК СтатусыЭНСрезПоследних
		|			ПО ЭлектроннаяНакладная.Ссылка = СтатусыЭНСрезПоследних.ДокументРегистратор
		...
		|	ЗапросДоступностиТоваров.Ссылка КАК ДокументЗапроса
		|ИЗ
		|	РегистрСведений.СтатусыЭН.СрезПоследних(,
		|		СтатусЭН.СтатусПосылки = ЗНАЧЕНИЕ(Перечисление.СтатусыПосылокСлужбыДоставки.ВДороге)) КАК СтатусыЭНСрезПоследних
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектроннаяНакладная КАК ЭлектроннаяНакладная
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗапросДоступностиТоваров КАК ЗапросДоступностиТоваров
		|			ПО ЭлектроннаяНакладная.ДокументОснование = ЗапросДоступностиТоваров.Ссылка
		...
		|ГДЕ
		|	ЗапросДоступностиТоваров.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросовДоступностиТоваровШапка.КОтправке)
		...
		|		И
		|			ТаблицаДоступности.ОтправлятьВNavision = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросаДоступностиВNavision.ПроверкаВозможностиЗакрытияВNavision)
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|		НЕ ТаблицаОрдеры.ПометкаУдаления
		|		И
		|			ТаблицаДоступности.ОтправлятьВNavision = ЗНАЧЕНИЕ(Перечисление.СтатусыЗапросаДоступностиВNavision.ПроверкаВозможностиЗакрытияВNavision)) КАК
		|		ТаблицаОрдеры
...
#Область ПереводЗапросаДляЗакрытияВNavision
	...
	Пока ВыборкаДокументЗапроса.Следующий() Цикл
		...
		Если МожноЗакрывать Тогда
			...
			ДокументЗапроса = ВыборкаДокументЗапроса.ДокументЗапроса.ПолучитьОбъект();
			ДокументЗапроса.ОтправлятьВNavision = Перечисления.СтатусыЗапросаДоступностиВNavision.ГотовКЗакрытиюВNavision;
			ДокументЗапроса.Записать(РежимЗаписиДокумента.Проведение);

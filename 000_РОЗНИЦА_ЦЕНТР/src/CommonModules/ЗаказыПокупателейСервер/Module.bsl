	...
	//Проверка номерации для заказов от М2
		...
		Если ПустаяСтрока(Выборка.Комментарий)
			...
			Запрос.Текст =
				...
				|ПОМЕСТИТЬ ВТ_СовпадающиеПозиции
				|ИЗ
				|	(ВЫБРАТЬ
				|		ВТ_ТоварыЗаказов.Ссылка КАК Ссылка,
				|		ВТ_ТоварыЗаказов.Код КАК Код,
				|		ВТ_ТоварыЗаказов.Счетчик КАК Счетчик
				|	ИЗ
				...
				|		ВТ_ТоварыЗаказов.Код,
				|		ВТ_ТоварыЗаказа.Счетчик
				|	ИЗ
				|		ВТ_ТоварыЗаказов КАК ВТ_ТоварыЗаказов
				|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТоварыЗаказа КАК ВТ_ТоварыЗаказа
				|			ПО ВТ_ТоварыЗаказов.Код = ВТ_ТоварыЗаказа.Код) КАК ВложенныйЗапрос
				|ГДЕ
				|	НЕ ВложенныйЗапрос.Ссылка = &Ссылка
				|СГРУППИРОВАТЬ ПО
				...
				|ВЫБРАТЬ
				|	ВТ_ТоварыЗаказов.Ссылка КАК Ссылка,
				|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_ТоварыЗаказов.Код) КАК КоличествоПозиций
				|ПОМЕСТИТЬ ВТ_УникалныеВЗаказах
				|ИЗ
				|	ВТ_ТоварыЗаказов КАК ВТ_ТоварыЗаказов
				|СГРУППИРОВАТЬ ПО
				|	ВТ_ТоварыЗаказов.Ссылка
				...
				|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_ТоварыЗаказа.Код) КАК КоличествоПозиций
				|ПОМЕСТИТЬ ВТ_УникалныеВЗаказе
				|ИЗ
				...
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ ПЕРВЫЕ 1
				|	ВТ_СовпадающиеПозиции.Ссылка КАК Ссылка,
				|	ВТ_СовпадающиеПозиции.Код КАК Код,
				|	ВТ_СовпадающиеПозиции.Счетчик КАК Счетчик,
				|	ВТ_УникалныеВЗаказах.КоличествоПозиций КАК КоличествоПозиций,
				|	ЗаказПокупателя.Номер КАК Номер
				|ИЗ
				|	ВТ_СовпадающиеПозиции КАК ВТ_СовпадающиеПозиции
				|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_УникалныеВЗаказах КАК ВТ_УникалныеВЗаказах
				|		ПО ВТ_СовпадающиеПозиции.Ссылка = ВТ_УникалныеВЗаказах.Ссылка
				|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_УникалныеВЗаказе КАК ВТ_УникалныеВЗаказе
				|		ПО ИСТИНА
				|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя КАК ЗаказПокупателя
				|		ПО ВТ_СовпадающиеПозиции.Ссылка = ЗаказПокупателя.Ссылка
				|ГДЕ
				|	ВТ_СовпадающиеПозиции.Счетчик = ВТ_УникалныеВЗаказах.КоличествоПозиций
				|	И ВТ_УникалныеВЗаказах.КоличествоПозиций = ВТ_УникалныеВЗаказе.КоличествоПозиций
				|
				|УПОРЯДОЧИТЬ ПО
				|	Ссылка УБЫВ";

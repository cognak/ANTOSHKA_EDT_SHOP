...
Процедура АвтоматическоеИзменениеСтатусов() Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрСведений.СтатусыТТН КАК СтатусыТТН
		...
		|		ПО СтатусыТТН.Регистратор = ЭлектроннаяНакладная.Ссылка
...
#Область ПереводЗапросаДоступностиПолучено
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТЧ Из Выборка.ДокументЗапроса.Товары Цикл
			...
			СтрокаЗаписиВРегистр.КлючСвязи = СтрокаТЧ.КлючСвязиЗапросаДоступности;

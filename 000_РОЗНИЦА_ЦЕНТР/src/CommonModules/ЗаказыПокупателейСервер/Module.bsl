...
Функция ПолучитьСоответствиеПредставленийТовара(СписокТоваров) Экспорт
	...
	Если ДанныеОтвета.Свойство("data") Тогда
		...
		Для Каждого СтрокаТовара Из ДанныеОтвета.data.productsSummary Цикл
			
			Если Не СтрокаТовара.contentM2Product = Неопределено Тогда
				СоответствиеОтвета.Вставить(СтрокаТовара.sku, СтрокаТовара.contentM2Product.uk_name);
			КонецЕсли;
...
Функция ТекстЗапросаДанныеЗаказаПокупателя(ОбработкаВ1С) Экспорт
	...
	Если ОбработкаВ1С Тогда
		...
		|	ТаблицаПогашениеПодарочныхСертификатов.Ссылка,
		|	ТаблицаПогашениеПодарочныхСертификатов.ПодарочныйСертификат,
		|	ТаблицаПогашениеПодарочныхСертификатов.СерийныйНомер,
		|	ТаблицаПогашениеПодарочныхСертификатов.Номинал
		|ИЗ

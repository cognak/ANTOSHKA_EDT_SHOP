...
Процедура АвтоматическоеИзменениеСтатусов() Экспорт
	...
	Запрос.Текст =
		...
		|	И ЗаказПокупателя.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПокупателей.Продажа)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
...
Процедура СозданиеПеремещений() Экспорт
	...
	Запрос.Текст =
		...
		|	И НЕ ЗапросДоступностиТоваров.МагазинПолучатель = ЗапросДоступностиТоваров.МагазинОтправитель
		|	И ЗапросДоступностиТоваров.ОператорДоставки = ЗНАЧЕНИЕ(Перечисление.ОператорыДоставки.ВнутренняяЛогистика)
		|	И ЗаказПокупателя.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПокупателей.Перемещение))";

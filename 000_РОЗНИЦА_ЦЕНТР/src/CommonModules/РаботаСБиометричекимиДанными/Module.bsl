...
Функция УстановитьБиометриюСотруднику(ФизЛицо,Биометрия) Экспорт
	...
	Если Не ЗначениеЗаполнено(ФизЛицо) Тогда
		ЗаписьЖурналаРегистрации("Биометрия",УровеньЖурналаРегистрации.Ошибка,Метаданные.РегистрыСведений.ИсторияИзмененияБиометрическихДанных,,"Ошибка при записи биометрии (Добавление) Сотрудник: "
				+ФизЛицо+", Ошибка: Не задано физ.лицо",РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьСотрудниковСБиометрией()   Экспорт 
	//2025-09-04 sa Решили что любой сотрудник может сканироваться в любом магазине.

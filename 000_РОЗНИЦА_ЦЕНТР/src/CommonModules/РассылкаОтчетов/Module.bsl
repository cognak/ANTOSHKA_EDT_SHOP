...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	//	LNK 01.02.2018 11:34:04	- Ответственные для скрытых копий
		ПараметрыДоставки.Вставить("Ответственные", СформироватьСписокОтветственныхРассылки(ПараметрыЖурнала, Рассылка));
...
Процедура ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, Настройки) Экспорт
	...
	Иначе
		...
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	МАКСИМУМ(ТаблицаКонтакты.ОсновнойВидКИ_Почта) КАК ОсновнойВидКИ_Почта,
		|	МАКСИМУМ(ТаблицаКонтакты.ОсновнойВидКИ_Телефон) КАК ОсновнойВидКИ_Телефон
		|ИЗ
		|	(ВЫБРАТЬ ПЕРВЫЕ 1
		|		ТаблицаСправочник.Ссылка КАК ОсновнойВидКИ_Почта,
		|		NULL КАК ОсновнойВидКИ_Телефон
		|	ИЗ
		|		Справочник.ВидыКонтактнойИнформации КАК ТаблицаСправочник
		|	ГДЕ
		|		ТаблицаСправочник.Родитель = &Родитель
		|		И ТаблицаСправочник.Тип = &ТипПочта
		...
		|	ГДЕ
		|		ТаблицаСправочник.Родитель = &Родитель
		|		И ТаблицаСправочник.Тип = &ТипТелефон
		|		И НЕ ТаблицаСправочник.ПометкаУдаления) КАК ТаблицаКонтакты"
		);
		Запрос.УстановитьПараметр("Родитель"  , СтрокаТаблицы.ГруппаКИ);
		Запрос.Параметры.Вставить("ТипПочта"  , Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
		Запрос.Параметры.Вставить("ТипТелефон", Перечисления.ТипыКонтактнойИнформации.Телефон);

		Выборка = Запрос.Выполнить().Выбрать();

		Если Выборка.Следующий() Тогда

			СтрокаТаблицы.ОсновнойВидКИ      = Выборка.ОсновнойВидКИ_Почта;
			СтрокаТаблицы.ОсновнойВидТелефон = Выборка.ОсновнойВидКИ_Телефон;
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ВРег(Лев(Форма.ИмяФормы, 13)) = ВРег("ВнешнийОтчет.") Тогда
		Возврат;
	КонецЕсли;
...
//	LNK 01.02.2018 11:35:32
Функция СформироватьСписокОтветственныхРассылки(ПараметрыЖурнала, Рассылка)
	...
	|	&ТаблицаПолучателей КАК ТаблицаПолучателей
	|ГДЕ
	|	ТаблицаПолучателей.Исключен = ЛОЖЬ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	ТаблицаСправочник.Ссылка КАК Получатель,
	|	ТабличнаяЧасть.Представление КАК EMail
	|ИЗ
	|	Справочник.ФизическиеЛица КАК ТаблицаСправочник
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица.КонтактнаяИнформация КАК ТабличнаяЧасть
	|		ПО (ТабличнаяЧасть.Ссылка = ТаблицаСправочник.Ссылка)
	|			И (ТабличнаяЧасть.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.EmailФизическогоЛица))
	|ГДЕ
	|	ТаблицаСправочник.Ссылка В
	|			(ВЫБРАТЬ
	|				Получатели.Получатель
	|			ИЗ
	|				втТаблицаПолучателей КАК Получатели)
	|	И ТаблицаСправочник.ПометкаУдаления = ЛОЖЬ
	|	И НЕ ТаблицаСправочник.ЭтоГруппа"
	);
	Запрос.УстановитьПараметр("ТаблицаПолучателей", Рассылка.Ответственные);
	...
	Пока Выборка.Следующий() Цикл
		...
		СписокПолучателей.Вставить(Выборка.Получатель, Выборка.EMail);

	КонецЦикла;

	Возврат СписокПолучателей;

КонецФункции // СформироватьСписокОтветственныхРассылки()
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Иначе // ПараметрыОтчета.СКД 
		...
		Если ЗначениеЗаполнено(ПараметрыОтчета.Настройки) Тогда
			...
			ЭтоТаблицаЗначений = ТипЗнч(ПараметрыОтчета.Настройки) = Тип("ТаблицаЗначений");	//	LNK 17.04.2020 12:10:56
...
Процедура ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурнала = Неопределено, КомментарийСПараметрами = "", 
	...
	Если ЗаписатьВЖурнал Тогда
		...
		//	ТекстДляЖурнала
		//);
		ЖурналСобытий.Регистрация(ПараметрыЖурнала.ИмяСобытия	//	LNK 15.04.2020 07:00:43
			, УровеньЖурнала
			, ПараметрыЖурнала.Метаданные
			, ПараметрыЖурнала.Данные
			,
			, ТекстДляЖурнала
			, СокрЛП(ПараметрыЖурнала.Данные)
			, Истина
		);
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	ПараметрыДоставки.Вставить("ДатаВчерашняя" , ТекущаяДатаСеанса() - 24 * 3600);
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		СтруктураЗаполненияШаблонов = Новый Структура("НаименованиеРассылки, Автор, ЗаголовокСистемы, ДатаВыполнения, ДатаВчерашняя");
		...
		СтруктураЗаполненияШаблонов.ДатаВчерашняя        = ПараметрыДоставки.ДатаВчерашняя;
	...
	Если Получатель = Неопределено Тогда
		...
		Если ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеТемы ИЛИ ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеСообщения Тогда
			...
			Для Каждого КлючИЗначение Из ПараметрыДоставки.Получатели Цикл
				...
				УстановитьОтветственныхВСтрокеСлепыеКопии(ПараметрыДоставки, ПараметрыПисьма);	//	LNK 01.02.2018 12:41:49
...
//	LNK 01.02.2018 12:40:57
Процедура УстановитьОтветственныхВСтрокеСлепыеКопии(ПараметрыДоставки, ПараметрыПисьма)

	Если ПараметрыДоставки.Свойство("Ответственные") И НЕ ПараметрыДоставки.Ответственные.Количество() = 0 Тогда

		Если НЕ ПараметрыПисьма.Свойство("СлепыеКопии") Тогда

			ПараметрыПисьма.Вставить("СлепыеКопии", "");

		КонецЕсли;

		Для Каждого КлючИЗначение Из ПараметрыДоставки.Ответственные Цикл

			ПараметрыПисьма.СлепыеКопии = ПараметрыПисьма.СлепыеКопии 
				+ ?(ПустаяСтрока(ПараметрыПисьма.СлепыеКопии), "", ", ")

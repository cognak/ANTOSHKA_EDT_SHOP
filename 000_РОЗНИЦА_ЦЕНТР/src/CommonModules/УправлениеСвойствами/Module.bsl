...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено) Экспорт
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		//2025-09-09 sa для ЗначенийСвойствОбъектов программные отборы не нужны т.к. они конфликтуют с стандартным
		Если ОписаниеСвойства.ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
			
		ИначеЕсли ОписаниеСвойства.ИмяРеквизитаСвойство <> "" Тогда
		...
		ИначеЕсли ОписаниеСвойства.ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.СвойстваНоменклатуры") Тогда    
			//2025-09-09 sa какой-то умник сделал аналог справочника ЗначенияСвойствОбъектов, а о связях с планом видов характеристик не позаботился
			Если ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.МатериалТовара Тогда
				Свойство_ = Перечисления.ТипыСвойствНоменклатуры.МатериалТовара;
			ИначеЕсли ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.Коллекция Тогда
				Свойство_ = Перечисления.ТипыСвойствНоменклатуры.Коллекция;
			ИначеЕсли ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.МодельТовара Тогда
				Свойство_ = Перечисления.ТипыСвойствНоменклатуры.МодельТовара;
			ИначеЕсли ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.Сезонность Тогда
				Свойство_ = Перечисления.ТипыСвойствНоменклатуры.Сезонность;
			ИначеЕсли ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.СтатусТовара Тогда
				Свойство_ = Перечисления.ТипыСвойствНоменклатуры.СтатусТовара;
			ИначеЕсли ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.Фурнитура Тогда
				Свойство_ = Перечисления.ТипыСвойствНоменклатуры.Фурнитура;  
			Иначе
				Продолжить; //без отбора
			КонецЕсли;
			ЭлементыДляОтбора = Новый Массив;
			ЭлементыДляОтбора.Добавить(Свойство_);
			ФиксЭлементы = Новый ФиксированныйМассив(ЭлементыДляОтбора);
			Связь = Новый ПараметрВыбора("Отбор.ТипСвойства", ФиксЭлементы); 
			Связи = Новый Массив;
			Связи.Добавить(Связь);
			Элемент.ПараметрыВыбора = Новый ФиксированныйМассив(Связи);
		КонецЕсли;
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, ИмяЭлементаДляРазмещения, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			Если Пользователи.РолиДоступны("АдминистраторСистемы",, Истина) Тогда
		//	Если Пользователи.РолиДоступны("ДобавлениеИзменениеБазовойНСИ") Тогда	//	LNK 22.05.2017 11:20:07
				// Добавление команды.

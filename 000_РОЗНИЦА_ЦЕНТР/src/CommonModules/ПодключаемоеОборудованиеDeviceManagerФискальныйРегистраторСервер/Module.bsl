...
Функция ПолучитьТабличныйДокументЧек(ОтветККМ, ПараметрыЗакрытия, ТекстЗаголовка = "", URL = "", ТабличныйДокумент = Неопределено, Копия = Ложь, НулевойЧек = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ОтветККМ) Тогда
		...
		Если ТипЗнч(сИнфо) = Тип("Соответствие") Тогда
			...
			URL = Лев(URL, СтрНайти(URL, "?MAC:")-1);
			...
			//2025-03-01 sa отказались от использования ID Организации
		...
		//	LNK 27.10.2023 11:27:56
			...
			Для каждого СтрокаТоваров ИЗ goods Цикл
				...
				ОбластьМакета.Параметры.cnt = Формат(ПолучитьЗначениеСоответствия(СтрокаТоваров, "cnt", ""),"ЧДЦ=3; ЧГ="); 
				ОбластьМакета.Параметры.price = Формат(ПолучитьЗначениеСоответствия(СтрокаТоваров, "price", ""),"ЧДЦ=2; ЧГ=");  			
				ТабличныйДокумент.Вывести(ОбластьМакета);
				...
				ОбластьМакета.Параметры.cost =  Формат(ПолучитьЗначениеСоответствия(СтрокаТоваров, "cost", ""),"ЧДЦ=2; ЧГ=");  
				...
				Если ЗначениеЗаполнено(disc) Тогда
					...
					ОбластьМакета.Параметры.disc = Формат(disc,"ЧДЦ=2; ЧГ="); 
					ОбластьМакета.Параметры.taxlit = ПолучитьЗначениеСоответствия(СтрокаТоваров, "taxlit", ""); 
				...
				//2025-04-03 sa решили оптимизировать длину чека. 
			...
			Для каждого СтрокаОплат ИЗ pays Цикл
				...
				Если ТипОплаты = 0 Тогда   //наличка
					...
					ОбластьМакета.Параметры.sum = Формат(ПолучитьЗначениеСоответствия(СтрокаОплат, "sum", "") ,"ЧДЦ=2; ЧГ=") + " грн.";
				...
				ИначеЕсли ТипОплаты = 6 Тогда //Сертификат
					ОбластьМакета = Макет.ПолучитьОбласть("Оплата");
					ОбластьМакета.Параметры.sum = Формат(ПолучитьЗначениеСоответствия(СтрокаОплат, "sum", "") ,"ЧДЦ=2; ЧГ=") + " грн.";
					ОбластьМакета.Параметры.type = "БЕЗГОТІВКОВА"; 
					ТабличныйДокумент.Вывести(ОбластьМакета);
					ОбластьМакета = Макет.ПолучитьОбласть("ФормаОплаты");
					ОбластьМакета.Параметры.typen = ИмяТипаОплаты(СтрокаОплат)+" "+ДопИнфо; 
					ТабличныйДокумент.Вывести(ОбластьМакета);
				Иначе
					ОбластьМакета = Макет.ПолучитьОбласть("Оплата");
					ОбластьМакета.Параметры.sum = Формат(ПолучитьЗначениеСоответствия(СтрокаОплат, "sum", "") ,"ЧДЦ=2; ЧГ=") + " грн.";
					...
					//КонецЕсли;
					//приняли решение что форма оплаты Инше не используется
					ОбластьМакета.Параметры.type = "БЕЗГОТІВКОВА"; 
					ТабличныйДокумент.Вывести(ОбластьМакета);
					ОбластьМакета = Макет.ПолучитьОбласть("ФормаОплаты");
			...
			Если НЕ round = 0 Тогда
				...
				ОбластьМакета.Параметры.round = Формат(round, "ЧДЦ=2; ЧГ=");  
				ТабличныйДокумент.Вывести(ОбластьМакета);	
			...
			ОбластьМакета.Параметры.sum_topay = Формат(sum_0-sum_disc, "ЧДЦ=2; ЧН=0,00; ЧГ=") + " грн.";  
			ТабличныйДокумент.Вывести(ОбластьМакета);	
			...
			Для каждого СтрокаНалогов ИЗ taxes Цикл
				...
				tax_sum = Формат(ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_sum", ""),"ЧДЦ=2; ЧГ=");  

				Если ЗначениеЗаполнено(tax_sum) Тогда

					ОбластьМакета = Макет.ПолучитьОбласть("Налог");
					ОбластьМакета.Параметры.tax_percent = Формат(ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_percent", ""),"ЧДЦ=2; ЧГ=");  
					ОбластьМакета.Параметры.tax_name = ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_name", ""); 
					ОбластьМакета.Параметры.tax_sum = Формат(ПолучитьЗначениеСоответствия(СтрокаНалогов, "tax_sum", ""),"ЧДЦ=2; ЧГ=");  
				...
				Если ЗначениеЗаполнено(ex_sum) Тогда
					...
					ОбластьМакета.Параметры.ex_sum = Формат(ex_sum, "ЧДЦ=2; ЧН=0,00; ЧГ=");  
			...
			//	ТабличныйДокумент.Вывести(Макет.ПолучитьОбласть("Пробел"));		
				...
				ОбластьМакета.Параметры.sum_topay = Формат(ПолучитьЗначениеСоответствия(printinfo, "sum_topay", ""), "ЧДЦ=2; ЧГ=") + " грн."; 
				ТабличныйДокумент.Вывести(ОбластьМакета);	

				//2025-02-28 Sa Сдача теперь обязательное поле для налички
				Если Не ЭтоВозврат Тогда
					ВывестиПараметрЧека(ТабличныйДокумент,Макет,"РЕШТА",""+Формат(СдачаПоЧеку, "ЧДЦ=2; ЧН=0.00; ЧГ=")+" грн.");
				КонецЕсли;
			...
			Если НЕ НулевойЧек И ЗначениеЗаполнено(URL) Тогда
				...
				Рисунок.Высота	= 19;//59; 
				Рисунок.Ширина	= 19;//59;         
...
Функция ЗаписатьФискальныйНомер(ОтветККМ,ЧекККМОбъект) Экспорт
	...
	СтруктураОтвета = Документы.ЧекККМ.ЗаписатьФискальныйНомер(ЧекККМОбъект.Ссылка,ФН,ФР,ОтветККМ);
...
//	"ПараметрыКассыККМ" - ОбщиеПараметры[21]
	...
	КоллекцияПлатежныхРеквизитов = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистраторСервер.ПолучитьПлатежныеРеквизиты(ОбъектДрайвера.Параметры.УИД);
	...
	Для ИндексОплаты = 0 По ТаблицаОплат.Количество() - 1 Цикл
		...
		Если НЕ Стр.Сумма = 0 Тогда			
			...
			Если Стр.ТипОплаты = 1 ИЛИ Стр.ТипОплаты = 2  ИЛИ Стр.ТипОплаты = 3 Тогда
				ПлатежныеРеквизиты = КоллекцияПлатежныхРеквизитов.Получить(Стр.НомерТранзакции);
				Если ЗначениеЗаполнено(ПлатежныеРеквизиты) Тогда
					...
					//2025-06-17 sa теперь нужно передавать сумму комиссии банка.
					//СуммаКомиссииБанка = ПлатежныеРеквизиты.commission; 
					//Если СуммаКомиссииБанка >0 Тогда 
					...
					//	pay.insert("commission",СуммаКомиссииБанка);
					//	pay.insert("commission_included",1);
					//	pay.insert("show_additional_info",Истина);
					//
					//КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
//	LNK 25.10.2023 05:41:37
	...
	Если НЕ ТаблицаОплат = Неопределено Тогда
		...
		Для ИндексОплаты = 0 По ТаблицаОплат.Количество() - 1 Цикл
			...
			СтрокаОплаты.Вставить("НомерТранзакции"	, ТаблицаОплат[ИндексОплаты][4].Значение);

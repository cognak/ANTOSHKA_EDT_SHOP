...
Функция ПолучитьТабличныйДокументЧек(ОтветККМ, ПараметрыЗакрытия, ТекстЗаголовка = "", URL = "") Экспорт
	...
	ТабДокумент.Вывод = ИспользованиеВывода.Разрешить;	//	LNK 08.01.2023 09:15:22
...
//	LNK 09.01.2023 07:25:09
	...
	Если НЕ money.Количество() = 0 Тогда
		...
		Область.Параметры.Комментарий = ОбщегоНазначенияКлиентСервер.PAD("------Внесення\винесення:", 54, Ложь, "-");
		ТабличныйДокумент.Вывести(Область);
	...
	Если НЕ pays.Количество() = 0 Тогда
		...
		Область = Макет.ПолучитьОбласть("О_Текст|Тело");
		Область.Параметры.Комментарий = ОбщегоНазначенияКлиентСервер.PAD("------Оплати:", 59, Ложь, "-");
		ТабличныйДокумент.Вывести(Область);
	...
	Если НЕ taxes.Количество() = 0 Тогда
		...
		Область = Макет.ПолучитьОбласть("О_Текст|Тело");
		Область.Параметры.Комментарий = ОбщегоНазначенияКлиентСервер.PAD("------Податки:", 59, Ложь, "-");
		ТабличныйДокумент.Вывести(Область);
	...
	Область = Макет.ПолучитьОбласть("О_Текст|Тело");
	Область.Параметры.Комментарий = ОбщегоНазначенияКлиентСервер.PAD("------Сейф:", 60, Ложь, "-");
	ТабличныйДокумент.Вывести(Область);

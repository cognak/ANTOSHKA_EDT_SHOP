...
Функция ПолучитьОбработчикДрайвера(ОбработчикДрайвера) Экспорт
	...
	Если ОбработчикДрайвера <> Неопределено Тогда
		...
		//2026-06-29 sa Фискальный регистратор Checkbox через API
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикЧекбоксФискальныйРегистратор") Тогда
			Возврат ПодключаемоеОборудованиеФискальныйРегистраторЧекбокс;
		КонецЕсли;
		...
		//2025-11-26 sa Монитор покупателя работающий по сети по протоколу JSON
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАнтошкаДисплейПокупателя") Тогда
			Возврат ПодключаемоеОборудованиеДисплейПокупателяАнтошка;
		КонецЕсли;                 
		...
		//2023-10-01 sa Биометрический сканер
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикZKTecoБиометрическийСканер") Тогда
			Возврат ПодключаемоеОборудованиеБиометрическийСканерZKT;
		//2024-10-17 sa Терминал приват-банка работающий по сети по протоколу JSON
		ИначеЕсли ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикIngenicoПриватбанкJSON") Тогда
			Возврат ПодключаемоеОборудованиеБанковскийТерминалПриватбанк;
		КонецЕсли;

...
Процедура ОбновлениеПромежуточныхВерсий() Экспорт
	...
	ОбновлениеИнформационнойБазыРТ.ЗаполнитьОрганизациюВРегистреДвиженияСерийныхНомеров();
...
// Процедура заполняет реквизит узлов плана обмена РИБ "По рабочему месту"
Процедура ЗаполнитьОпциюПередачиДанныхРИБПоРабочемуМесту() Экспорт
	
	Если НЕ ПланыОбмена.ПоРабочемуМесту.ЭтоРабочееМесто() Тогда
		...
		|ГДЕ
		|	ПоРабочемуМесту.Ссылка <> &ГлавныйУзел
		|	И (НЕ ПоРабочемуМесту.ПометкаУдаления)");
		Запрос.УстановитьПараметр("ГлавныйУзел", ОбменДаннымиПовтИсп.ГлавныйУзел());
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			УзелОбъект = Выборка.Ссылка.ПолучитьОбъект();
			УзелОбъект.ПередаватьДанныеПослеКаждогоПробитогоЧека = Истина;
			УзелОбъект.Записать();
		КонецЦикла;
...
// Процедура заполниет ресурс Организация в регистре "Движения серийных номеров"
Процедура ЗаполнитьОрганизациюВРегистреДвиженияСерийныхНомеров() Экспорт
	...
	|ГДЕ
	|	ДвиженияСерийныхНомеров.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период,
	|	Регистратор";
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		|	ДвиженияСерийныхНомеров.НомерСтроки,
		|	ДвиженияСерийныхНомеров.Активность,
		...
		|	ДвиженияСерийныхНомеров.Получатель,
		|	ДвиженияСерийныхНомеров.Организация КАК ОрганизацияРегистра,
		|	ВЫБОР
		|		КОГДА ДвиженияСерийныхНомеров.Регистратор ССЫЛКА Документ.ПриходныйОрдерНаТовары
		|				ИЛИ ДвиженияСерийныхНомеров.Регистратор ССЫЛКА Документ.РасходныйОрдерНаТовары
		|			ТОГДА ДвиженияСерийныхНомеров.Регистратор.ДокументОснование.Организация
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ДвиженияСерийныхНомеров.Регистратор ССЫЛКА Документ.КорректировкаРегистров
		|					ТОГДА ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|				ИНАЧЕ ДвиженияСерийныхНомеров.Регистратор.Организация
		|			КОНЕЦ
		...
		|	РегистрСведений.ДвиженияСерийныхНомеров КАК ДвиженияСерийныхНомеров
		|ГДЕ
		|	ДвиженияСерийныхНомеров.Регистратор = &Регистратор
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	ТаблицаРегистра.АналитикаХозяйственнойОперации,
		|	ТаблицаРегистра.Количество,
		|	ТаблицаРегистра.Отправитель,
		|	ТаблицаРегистра.Получатель,
		|	ТаблицаРегистра.ОрганизацияРегистра,
		|	ВЫБОР
		|		КОГДА ТаблицаРегистра.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|			ТОГДА ВЫБОР
		|					КОГДА ТаблицаРегистра.Отправитель = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|						ТОГДА ТаблицаРегистра.Получатель.Организация
		|					ИНАЧЕ ТаблицаРегистра.Отправитель.Организация
		|				КОНЕЦ
		|		ИНАЧЕ ТаблицаРегистра.Организация
		|	КОНЕЦ КАК Организация
		|ИЗ
		|	ТаблицаРегистра КАК ТаблицаРегистра";
		
		ЗапросПоДвижениям.УстановитьПараметр("Регистратор", ДокументСсылка);

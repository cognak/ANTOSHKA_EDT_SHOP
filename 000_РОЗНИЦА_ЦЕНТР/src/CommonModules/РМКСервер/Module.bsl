...
Процедура ЛогВТекстовыйФайл(Текст,ИмяФайла = Неопределено) Экспорт     
	Если ИмяФайла = Неопределено Тогда
		ИмяФайла = КаталогВременныхФайлов()+"\Ans_"+Формат(ТекущаяДата(),"ДФ=yyyy-MM-dd_ЧЧ-мм-сс")+".json";  
	КонецЕсли;
	ТекстовыйДок_ = Новый ТекстовыйДокумент;
	Файл = Новый Файл(ИмяФайла);
	Если Файл.Существует() Тогда
		ТекстовыйДок_.Прочитать(ИмяФайла,КодировкаТекста.UTF8);
		Текст = ТекстовыйДок_.ПолучитьТекст() + Символы.ПС + Текст;
	КонецЕсли;
	ТекстовыйДок_.УстановитьТекст(Текст);
	УстановитьПривилегированныйРежим(Истина);     
	ТекстовыйДок_.Записать(ИмяФайла,КодировкаТекста.UTF8); 

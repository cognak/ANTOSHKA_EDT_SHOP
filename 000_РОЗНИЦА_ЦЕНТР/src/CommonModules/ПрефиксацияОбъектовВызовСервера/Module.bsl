...
//	LNK 14.04.2017 12:28:40
Функция ДатаИлиМагазинОбъектаИзменены(Ссылка, Знач ДатаПослеИзменения, Знач МагазинПослеИзменения, ПолноеИмяТаблицы)	Экспорт

	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ШапкаОбъекта.Дата КАК Дата,
	|	ЕСТЬNULL(ШапкаОбъекта.Магазин.НомерМагазина, """") КАК ПрефиксМагазинаДоИзменения
	|ИЗ
	|	Документ.ОприходованиеТоваров КАК ШапкаОбъекта
	|ГДЕ
	|	ШапкаОбъекта.Ссылка = &Ссылка"
	;

	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.ОприходованиеТоваров", ПолноеИмяТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ЕСТЬNULL(ШапкаОбъекта.Магазин.", "ЕСТЬNULL(ШапкаОбъекта." + ПрефиксацияОбъектовСобытия.ИмяРеквизитаМагазин(ПолноеИмяТаблицы) + ".");
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	ПрефиксМагазинаДоИзменения = Формат(Выборка.ПрефиксМагазинаДоИзменения, "ЧГ=");

	ПрефиксМагазинаПослеИзменения = Неопределено;
	ПерепрефиксацияОбъектов.ПриОпределенииПрефиксаМагазина(МагазинПослеИзменения, ПрефиксМагазинаПослеИзменения);
	
	Возврат НЕ ПрефиксМагазинаДоИзменения = ПрефиксМагазинаПослеИзменения
		ИЛИ НЕ ДатыОбъектаОдногоПериода(Выборка.Дата, ДатаПослеИзменения, Ссылка);

КонецФункции // ДатаИлиМагазинОбъектаИзменены()

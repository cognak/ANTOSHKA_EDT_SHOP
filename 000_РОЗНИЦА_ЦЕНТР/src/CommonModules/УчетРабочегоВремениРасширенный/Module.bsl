...
Процедура ЗарегистрироватьДанныеГрафика(Движения, ДанныеОВремени) Экспорт
	Для Каждого СтрокаДанных Из ДанныеОВремени Цикл
		Запись = Движения.ОтсуствиеНаРаботе.Добавить();
		
		Запись.ПериодРегистрации = СтрокаДанных.Дата;
		Запись.Сотрудник = СтрокаДанных.Сотрудник;
		Запись.Организация = СтрокаДанных.Организация;
		Запись.Магазин = СтрокаДанных.Магазин;
		Запись.Состояние = СтрокаДанных.Состояние;
	КонецЦикла;	
	
	Движения.ОтсуствиеНаРаботе.Записывать = Истина;
КонецПроцедуры

...
Функция ОтменитьЗаявкуНаОформлениеКредита(СчетПокупателюСсылка, КредитнаяПрограмма, ТекстСообщения = "")	Экспорт
	...
	Если ПроверитьНазначениеСоединения(ДанныеСоединения, КредитнаяПрограмма, ДанныеОтвета.ТекстСообщения, ДанныеОтвета.Отказ) Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			...
			РаботаСJSONСервер.ЗаписатьСвойство("orderId", Выборка.Идентификатор, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("messageId", Выборка.ИдентификаторБанка, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("cancelId", "test", ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("rasonCancel", "test", ЗаписьJSON);
...
Функция ПолучитьJSONЗаявкиНаОформлениеПокупкиЧастями(СчетПокупателюСсылка, КредитнаяПрограмма, ДанныеСоединения)
	...
	РаботаСJSONСервер.ЗаписатьСвойство("mPhone"			, ПодготовитьНомерТелефона(Шапка.НомерТелефона), ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("orderId"			, Шапка.Идентификатор, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("orderSum"		, Шапка.СуммаКредита * 100, ЗаписьJSON, );
	РаботаСJSONСервер.ЗаписатьСвойство("shopId"			, Шапка.Код, ЗаписьJSON, );
	РаботаСJSONСервер.ЗаписатьСвойство("callBackURL"		, "https://antoshka.ua", ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("eMailPartner"	, "test@mytestemail.com", ЗаписьJSON);

		Если Программы.Следующий() Тогда 
			ЗначенияГлубины = Программы.Выбрать();
			Если ЗначенияГлубины.Следующий() Тогда 
				РаботаСJSONСервер.ЗаписатьСвойство("panEnd"			, Формат(ЗначенияГлубины.КартаКлиентаДляОплатыЧастями,"ЧЦ=4; ЧН=Ноль; ЧВН=; ЧГ="), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("orderTerm"		, ЗначенияГлубины.ПериодПредоставления, ЗаписьJSON);
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьJSONЗаявкиНаОформлениеВозврата(СчетПокупателюСсылка, КредитнаяПрограмма, ДанныеСоединения)
	...
	РаботаСJSONСервер.ЗаписатьСвойство("orderId"			, Шапка.Идентификатор, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("reversalId"		, Шапка.ИдентификаторВозврата, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("reversalSum"		, Шапка.СуммаКредита * 100, ЗаписьJSON, );
	РаботаСJSONСервер.ЗаписатьСвойство("shopId"		, Шапка.Код, ЗаписьJSON, );
	РаботаСJSONСервер.ЗаписатьСвойство("callBackURL"		, "https://antoshka.ua", ЗаписьJSON);
...
Функция ПолучитьJSONИДБанкаЗаявки(СчетПокупателюСсылка, КредитнаяПрограмма, ДанныеСоединения)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		РаботаСJSONСервер.ЗаписатьСвойство("messageId", Выборка.messageId, ЗаписьJSON);

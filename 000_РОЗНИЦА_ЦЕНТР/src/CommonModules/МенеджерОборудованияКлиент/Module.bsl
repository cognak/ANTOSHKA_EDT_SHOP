...
//	LNK 08.03.2024 09:37:39 - Экспорт в целях отладки процессов
...
Процедура УстановитьДрайверИзДистрибутива(ИмяМакета, ИмяФайла) Экспорт
	...
	Для Каждого ЭлементМакета Из ФайлМакета.Элементы Цикл
		Если Нрег(ЭлементМакета.Имя) = Нрег(ИмяФайла) + ".zip" Тогда
	...
	Если ОбъектДрайвера = Неопределено Тогда
		...
		// Фискальные регистраторы
			...
			ИначеЕсли ДрайверУстройства = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.Обработчик1СФискальныйРегистраторЭмулятор") Тогда
				...
				ОбъектДрайвера = Новый COMОбъект("AddIn.EmulatorFP1C");
		...
		// Терминалы сбора данных
			...
			//2024-10-17 sa Добавляем компоненту для работы с TCP протоколом для терминалов Приватбанка	
			ИначеЕсли ДрайверУстройства = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикIngenicoПриватбанкJSON") Тогда
		...
		// Весы с печатью этикеток
			...
			ИначеЕсли ДрайверУстройства = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикZKTecoБиометрическийСканер") Тогда
				...
				#Иначе
					ProgID = "AddIn.V8Bioscanner";
					Если ПодключитьВнешнююКомпоненту(ProgID) Тогда
						ОбъектДрайвера = Новый (ProgID);
					КонецЕсли;
					//ОбъектДрайвера = Новый COMОбъект(ProgID);
				#КонецЕсли
			//2025-11-26 sa Добавляем новый вид подключаемого оборудования Многострочный дисплей покупателя Red Head	
			ИначеЕсли ДрайверУстройства = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАнтошкаДисплейПокупателя") Тогда
				#Если ВебКлиент Тогда
					
				#Иначе
					ОбъектДрайвера = ПодключаемоеОборудованиеДисплейПокупателяАнтошка.ИнициализироватьУстройство();
				#КонецЕсли
			//2026-06-29 sa Добавляем новый вид подключаемого оборудования Фискальный регистратор CheckBox
			ИначеЕсли ДрайверУстройства = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикЧекбоксФискальныйРегистратор") Тогда
				ОбъектДрайвера = ПодключаемоеОборудованиеФискальныйРегистраторЧекбокс.ИнициализироватьУстройство(ПараметрыУстройства);
...
Функция СоздатьОбъектДрайвераДляCOM(ОбъектДрайвера)
	...
	//Если Строка(ОбъектДрайвера) = "AddIn.V8Bioscanner" Тогда
	//	НовыйОбъект = Новый COMОбъект("AddIn.V8Bioscanner");
	//КонецЕсли;
...
Процедура УстановитьДрайвер(Идентификатор) Экспорт
	...
	// Эквайринговые терминалы        
		...
		//2024-10-17 sa добавляем эквайринговый терминал Приватбанка работающий в режиме JSON по вай-фай.	
		ИначеЕсли ДрайверУстройства = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикIngenicoПриватбанкJSON") Тогда
			//УстановитьВнешнююКомпоненту("ОбщийМакет.ДрайверПриватбанкTCPClient");
			УстановитьДрайверИзДистрибутива("ДрайверПриватбанкTCPClient", "setup.exe");
	// Конец Эквайринговые терминалы
	...
	// 2026-06-01 sa Монитор покупателя
		ИначеЕсли ДрайверУстройства = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикАнтошкаДисплейПокупателя") Тогда
				УстановитьДрайверИзДистрибутива("МониторПокупателяRedHead", "setup.exe");

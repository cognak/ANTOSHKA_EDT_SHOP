...
Процедура ОбновитьРолиПользователей(Знач Пользователи1 = Неопределено,
	...
	ЗащищенныеРоли                = ПользователиСлужебный.ЗащищенныеРоли();	//	LNK 10.01.2020 13:27:23
	...
	Для каждого ОписаниеПользователя Из ИдентификаторыПользователейИБ Цикл
		...
		АдминистраторПолномочный    = УправлениеПользователями.ПолучитьБулевоЗначениеПраваПользователя(
			ПланыВидовХарактеристик.ПраваПользователей.АдминистраторПолномочный, Ложь, ТекущийПользователь);
			...
			Иначе // Администратор.
				...
				Для каждого Роль Из ПользовательИБ.Роли Цикл
					...
					Если РолиАдминистратора[ИмяРоли] = Неопределено Тогда
						...
						Если НЕ (АдминистраторПолномочный = Истина И ЗащищенныеРоли[ИмяРоли] = Истина) Тогда	//	LNK 10.01.2020 13:31:17
	...
	Если ЕстьТекущиеИзменения Тогда
		...
		//	LNK 26.10.2016 08:22:20
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации("ПраваПользователя.Применение"
					, УровеньЖурналаРегистрации.Ошибка
					, НаборЗаписей.Метаданные(),, ТекстОшибки, РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
...
Функция ЕстьИзмененияПараметровОграниченияДоступа() Экспорт
	...
	Для каждого ПроверяемыйПараметр Из ПроверяемыеПараметры Цикл
		...
		Если ПоследниеИзменения.Количество() > 0 Тогда
			Возврат Истина;
		КонецЕсли;
...
#Область ПроверкаНаличияСлужебныхУчетныхЗаписей

//	LNK 05.01.2017 15:42:33
Процедура ПроверитьАдминистратораАвтоматов()	Экспорт
...
//	Им должен являться "Справочники.Пользователи.АдминистраторАвтоматов"

	СписокРолей = Новый Соответствие;
	СписокРолей.Вставить(Метаданные.Роли.АдминистраторСистемы, Истина);
	СписокРолей.Вставить(Метаданные.Роли.ПолныеПрава, Истина);

	ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоИмени(УправлениеДоступомСлужебныйПовтИсп.ИмяАдминистратораАвтоматов());

	Если ПользовательИБ = Неопределено Тогда

		ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
		ПользовательИБ.Имя       = УправлениеДоступомСлужебныйПовтИсп.ИмяАдминистратораАвтоматов();
		ПользовательИБ.Пароль    = УправлениеДоступомСлужебныйПовтИсп.ПарольАдминистратораАвтоматов();
		ПользовательИБ.ПолноеИмя = УправлениеДоступомСлужебныйПовтИсп.НаименованиеАдминистратораАвтоматов();

		ПроверитьСвойстваПользователяИБ(ПользовательИБ, Ложь, Истина, Истина, Ложь, Ложь);
		ПроверитьСоставРолейПользователя(ПользовательИБ, СписокРолей, Ложь, Истина);
	...
	Иначе
		...
		ПроверитьСвойстваПользователяИБ(ПользовательИБ, Истина, Истина, Истина, Ложь, Ложь);
		ПроверитьСоставРолейПользователя(ПользовательИБ, СписокРолей, Истина, Истина);

	КонецЕсли;

	Если НЕ УправлениеДоступомСлужебныйПовтИсп.ИдентификаторАдминистратораАвтоматов() = ПользовательИБ.УникальныйИдентификатор Тогда

		АдминистраторОбъект = Справочники.Пользователи.АдминистраторАвтоматов.ПолучитьОбъект();

		АдминистраторОбъект.ИдентификаторПользователяИБ = ПользовательИБ.УникальныйИдентификатор;
		АдминистраторОбъект.ОбменДанными.Загрузка = Истина;
		АдминистраторОбъект.Записать();
...
//	LNK 04.12.2017 08:29:20
Процедура ПроверитьОператораВиртуальнойПолки()	Экспорт
	...
	СписокРолей = Новый Соответствие;
	СписокРолей.Вставить(Метаданные.Роли.БазовыеПрава, Истина);
	СписокРолей.Вставить(Метаданные.Роли.ОператорВиртуальнойПолки, Истина);

	ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоИмени(ПользователиКлиентСервер.ИмяОператораВиртуальнойПолки());

	Если ПользовательИБ = Неопределено Тогда

		ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
		ПользовательИБ.Имя       = ПользователиКлиентСервер.ИмяОператораВиртуальнойПолки();
		ПользовательИБ.Пароль    = "";
		ПользовательИБ.ПолноеИмя = ПользователиКлиентСервер.ИмяОператораВиртуальнойПолки();

		ПроверитьСвойстваПользователяИБ(ПользовательИБ, Ложь, Истина, Истина, Истина, Истина);
		ПроверитьСоставРолейПользователя(ПользовательИБ, СписокРолей, Ложь, Истина);
	...
	Иначе
		...
		ПроверитьСвойстваПользователяИБ(ПользовательИБ, Истина, Истина, Истина, Истина, Истина);
		ПроверитьСоставРолейПользователя(ПользовательИБ, СписокРолей, Истина, Истина);
...
//	LNK 22.02.2018 14:46:20
Процедура ПроверитьУчетнуюЗаписьWebПосредника()	Экспорт
	...
	СписокРолей = Новый Соответствие;
	СписокРолей.Вставить(Метаданные.Роли.ИспользованиеWebСервисовРозница, Истина);

	ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоИмени(УправлениеДоступомСлужебныйПовтИсп.ИдентификаторWebПосредника());

	Если ПользовательИБ = Неопределено Тогда

		ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
		ПользовательИБ.Имя       = УправлениеДоступомСлужебныйПовтИсп.ИдентификаторWebПосредника();
		ПользовательИБ.Пароль    = УправлениеДоступомСлужебныйПовтИсп.ПарольWebПосредника();
		ПользовательИБ.ПолноеИмя = УправлениеДоступомСлужебныйПовтИсп.ИдентификаторWebПосредника();
...
//	LNK 01.07.2020 12:28:48
Процедура ПроверитьУчетнуюЗаписьВнешнегоСоединения()	Экспорт
	...
	СписокРолей = Новый Соответствие;
	СписокРолей.Вставить(Метаданные.Роли.БазовыеПрава, Истина);
	СписокРолей.Вставить(Метаданные.Роли.БазовыеПраваРТ, Истина);
	СписокРолей.Вставить(Метаданные.Роли.ИспользованиеCOMВнешнее, Истина);

	ПользовательИБ = ПользователиИнформационнойБазы.НайтиПоИмени(ПользователиКлиентСервер.ИмяПользователяВнешнегоСоединения());

	Если ПользовательИБ = Неопределено Тогда

		ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
		ПользовательИБ.Имя       = ПользователиКлиентСервер.ИмяПользователяВнешнегоСоединения();
		ПользовательИБ.Пароль    = УправлениеДоступомСлужебныйПовтИсп.ПарольВнешнегоСоединения();
		ПользовательИБ.ПолноеИмя = ПользователиКлиентСервер.ИмяПолноеПользователяВнешнегоСоединения();
...
//	LNK 02.07.2020 10:12:44
Процедура ПроверитьСвойстваПользователяИБ(ПользовательИБ, Записывать = Ложь
				, АутентификацияСтандартная = Истина
				, ЗапрещеноИзменятьПароль = Истина
				, ПоказыватьВСпискеВыбора = Ложь
				, ПредупреждатьОбОпасныхДействиях = Истина)

	ЕстьИзменение = Ложь;

	Если НЕ ПользовательИБ.АутентификацияСтандартная = АутентификацияСтандартная Тогда
	...
	Если НЕ ПользовательИБ.ЗапрещеноИзменятьПароль = ЗапрещеноИзменятьПароль Тогда
	...
	Если НЕ ПользовательИБ.ПоказыватьВСпискеВыбора = ПоказыватьВСпискеВыбора Тогда
	...
	Если НЕ ПользовательИБ.ЗащитаОтОпасныхДействий.ПредупреждатьОбОпасныхДействиях = ПредупреждатьОбОпасныхДействиях Тогда

		ПользовательИБ.ЗащитаОтОпасныхДействий.ПредупреждатьОбОпасныхДействиях = ПредупреждатьОбОпасныхДействиях;
		ЕстьИзменение = Истина;

	КонецЕсли;

	Если ЕстьИзменение И Записывать Тогда
...
//	LNK 02.07.2020 08:34:50
Процедура ПроверитьСоставРолейПользователя(ПользовательИБ, СписокРолей, Записывать, УдалитьДругиеРоли = Истина)

	ЕстьИзменение = Ложь;

	Для каждого КлючЗначение Из СписокРолей Цикл

		Если НЕ ПользовательИБ.Роли.Содержит(КлючЗначение.Ключ) Тогда

			ПользовательИБ.Роли.Добавить(КлючЗначение.Ключ);
			ЕстьИзменение = Истина;
	...
	Если УдалитьДругиеРоли = Истина Тогда

		СписокЛишнихРолей = Новый Массив;

		Для каждого Роль Из ПользовательИБ.Роли Цикл

			Если СписокРолей.Получить(Роль) = Неопределено Тогда

				СписокЛишнихРолей.Добавить(Роль);

			КонецЕсли;

		КонецЦикла;

		Для каждого Роль Из СписокЛишнихРолей Цикл

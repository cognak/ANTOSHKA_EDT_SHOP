	...
	Если НЕ ИсточникРегистр
	   ...
	   И НЕ Удаление Тогда
		...
		Если ИзменениеИлиЗагрузкаЗапрещена(
				...
				УзелПланаОбмена,
				НайденныеЗапреты,
				ДанныеЗапретаПоИнвентаризации) Тогда
...
Функция ПропуститьПроверкуДатЗапрета(Источник,
	...
	ИЛИ  ПараметрыСеанса.ВыполняютсяСлужебныеДействия = Истина	Тогда
...
Функция ИзменениеИлиЗагрузкаЗапрещена(
	...
	Если НЕ ТехническаяПоддержкаВызовСервера.ОтладочныйРежимРаботы() Тогда	//	LNK 22.01.2021 11:16:05
		...
		Если НЕ ТипЗнч(Данные) = Тип("Строка") И ДанныеДляПроверки.ЗначенияПолейИзОбъекта = Неопределено Тогда

		//	LNK 24.07.2017 15:15:51 - я не знаю, когда передается строка, вот поэтому и условие
			ЗапретНайден = Ложь;
		...
		Иначе
			...
			ЗапретНайден = ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(
				ДанныеДляПроверки, СообщитьОЗапрете, , СтандартнаяОбработка, УзелПланаОбмена, НайденныеЗапреты, ДанныеЗапретаПоИнвентаризации);
...
//	LNK 18.02.2019 11:39:06
Процедура ПроверитьДоступностьИзмененияКассовыхОрдеров(Источник, Отказ)	Экспорт

	Если Отказ = Истина
	ИЛИ  ТехническаяПоддержкаВызовСервера.ИсключительныйРежим()
	ИЛИ
		НЕ (ТипЗнч(Источник) = Тип("ДокументОбъект.ПриходныйКассовыйОрдер") ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.РасходныйКассовыйОрдер")) Тогда
	...
	ВоздействиеДопустимо = НачалоДня(Источник.Дата) >= НачалоДня(ДобавитьМесяц(ТекущаяДата(), -1));

	Если НЕ ВоздействиеДопустимо Тогда

		Если Источник.ДополнительныеСвойства.Ссылка.Проведен = Истина Тогда

			Текст = НСтр("ru = 'Отказано! Воздействие на проведенный «" + Источник + "» недопустимо.'");
...
Функция ЗапретИзмененияДанныхНеИспользуется()
	...
	|	ИСТИНА
	|ИЗ
	|	РегистрСведений.ОграниченияДвиженийПоИнвентаризации.СрезПоследних КАК ОграниченияПоИнвентаризации"
	);
...
//	Если источники не определены, в процедуре ниже будет создано сообщение в журнале событий.
	ИсточникиДанных = ПолучитьИсточникиДанных(Отбор);
	
	Если ИсточникиДанных = Неопределено Тогда

		ЗначенияПолей = Неопределено;	//	LNK 24.07.2017 15:09:17
	...
	Иначе
		...
		//	LNK 28.12.2016 12:40:17
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации("КонтрольДатыЗапрета", УровеньЖурналаРегистрации.Ошибка
				,,, "Ошибка при обращении к таблице [" + Отбор.Таблица + "] поля [" + ПоляРегистра + "]", РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
...
Функция ПолучитьИсточникиДанных(Отбор)
	...
	//	LNK 24.07.2017 15:01:22
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для проверки запрета изменения не найдены
			           |источники данных для таблицы ""%1"".'"),
			Отбор.Таблица);
		ЖурналСобытий.Регистрация("ЗапретИзменения.Источники", УровеньЖурналаРегистрации.Ошибка
			,
			,
			,
			, ТекстСообщения

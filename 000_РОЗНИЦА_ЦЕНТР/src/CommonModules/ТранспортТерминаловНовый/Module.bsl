...
Процедура ВыгрузкаЦенВТСД() Экспорт
	...
	УзелОбмена = ПланыОбмена.ТранспортТерминалов.ПолучитьУзел_TSD_DEPOT().ПолучитьОбъект();

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВыгрузкаЦенВТСД.ДокПрименениеЦен КАК ДокументСсылка
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		НаборЗаписей.Отбор.ДокПрименениеЦен.Установить(Выборка.ДокументСсылка);

Процедура ВыгрузитьДокументПереоценка(Ссылка) ЭКСПОРТ
	...
	Запрос.Текст = 
		...
		|	"""" КАК DOC_OWNER,
		|	ПереоценкаТоваровНаСкладахТовары.Номенклатура.Марка.IDN КАК ENT_IDN
		|ИЗ
		...
		|	ПереоценкаТоваровНаСкладахТовары.Номенклатура.Марка.IDN
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(Марки.ВысокомаржинальнаяТМ, ЛОЖЬ) = ИСТИНА
		|			ТОГДА ""8""
		|		ИНАЧЕ ""0""
		|	КОНЕЦ КАК MARGA,
		|	ВЫБОР
		...
		|			ТОГДА ""НДС20""
		|		КОГДА Товары.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
		|			ТОГДА ""БезНДС""
		|		КОГДА Товары.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
		|			ТОГДА ""НДС0""
		|		КОГДА Товары.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
		|			ТОГДА ""НДС7""
		|		КОГДА Товары.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НеНДС)
		|			ТОГДА ""НеНДС""
		|	КОНЕЦ КАК TNDS,
		|	ЕСТЬNULL(Марки.IDN, NULL) КАК IDN_ENT,
		|	Товары.DSCN КАК DSCN,
		|	Товары.PRICE_SUPPLIER КАК PRICE_SUPPLIER
		|ИЗ
		|	Товары КАК Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
		|		ПО (Штрихкоды.Владелец ССЫЛКА Справочник.Номенклатура)
		...
		|		ПО Товары.Номенклатура.Марка = Марки.Ссылка
		|
		|УПОРЯДОЧИТЬ ПО
		|	Товары.IDN
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПереоценкаТоваровНаСкладахТовары.Номенклатура.Марка.IDN КАК IDN,
		|	ПереоценкаТоваровНаСкладахТовары.Номенклатура.Марка.Наименование КАК NM,
		|	ВЫБОР
		|		КОГДА ПереоценкаТоваровНаСкладахТовары.Номенклатура.Марка.ВысокомаржинальнаяТМ
		|			ТОГДА ""8""
		|		ИНАЧЕ ""0""
		|	КОНЕЦ КАК MARGA
		|ИЗ
		...
		ВыгрузитьМаркиПоДокументу((Результаты[4].Выбрать()));
...
Процедура ВыгрузитьДокументПеремещение(Ссылка,РазрешитьДобавлятьСтрокиВТСД) ЭКСПОРТ
	...
	|	КОНЕЦ КАК TNDS,
	|	ЕСТЬNULL(Марки.IDN, NULL) КАК IDN_ENT,
	|	ЕСТЬNULL(ОпорныеЦеныНоменклатурыСрезПоследних.Цена, 0) КАК PRICE_RETAIL,
	|	ВЫБОР
	|		КОГДА ЦеныНоменклатурыСрезПоследних.ТипПереоценки = ЗНАЧЕНИЕ(Перечисление.ТипыПереоценок.ПромоНачало)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК DSCN
	|ИЗ
	...
	|		ПО Товары.Номенклатура.Марка = Марки.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОпорныеЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, ) КАК ОпорныеЦеныНоменклатурыСрезПоследних
	|		ПО Товары.Номенклатура = ОпорныеЦеныНоменклатурыСрезПоследних.Номенклатура
	|			И Товары.Упаковка = ОпорныеЦеныНоменклатурыСрезПоследних.Упаковка
	|			И Товары.Магазин = ОпорныеЦеныНоменклатурыСрезПоследних.Магазин
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, ) КАК ЦеныНоменклатурыСрезПоследних
	|		ПО Товары.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|			И Товары.Магазин.ПравилоЦенообразования.ВидЦен = ЦеныНоменклатурыСрезПоследних.ВидЦены
	|			И Товары.Упаковка = ЦеныНоменклатурыСрезПоследних.Упаковка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаТовары.Номенклатура.Марка.IDN КАК IDN,
	|	ТаблицаТовары.Номенклатура.Марка.Наименование КАК NM,
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.Номенклатура.Марка.ВысокомаржинальнаяТМ
	|			ТОГДА ""8""
	|		ИНАЧЕ ""0""
	|	КОНЕЦ КАК MARGA
	|ИЗ
	...
	Запрос.УстановитьПараметр("Магазин", ОбменДаннымиПовтИсп.ПолучитьДанныеУзла().Магазин);   	
	Результаты = Запрос.ВыполнитьПакет();
	ВыгрузитьМаркиПоДокументу(Результаты[6].Выбрать());
	ВыгрузитьТовары(Результаты[3].Выбрать());
...
Процедура ВыгрузитьПакетДокументов(НомерПакета) ЭКСПОРТ
	...
	|	КОНЕЦ КАК TNDS,
	|	ЕСТЬNULL(Марки.IDN, NULL) КАК IDN_ENT,
	|	ЕСТЬNULL(ОпорныеЦеныНоменклатурыСрезПоследних.Цена, 0) КАК PRICE_SUPPLIER,
	|	ВЫБОР
	|		КОГДА ЦеныНоменклатурыСрезПоследних.ТипПереоценки = ЗНАЧЕНИЕ(Перечисление.ТипыПереоценок.ПромоНачало)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	...
	|		ПО Товары.Номенклатура.Марка = Марки.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОпорныеЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, ) КАК ОпорныеЦеныНоменклатурыСрезПоследних
	|		ПО Товары.Упаковка = ОпорныеЦеныНоменклатурыСрезПоследних.Упаковка
	|			И Товары.Магазин = ОпорныеЦеныНоменклатурыСрезПоследних.Магазин
	|			И Товары.Номенклатура = ОпорныеЦеныНоменклатурыСрезПоследних.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, ) КАК ЦеныНоменклатурыСрезПоследних
	|		ПО Товары.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|			И Товары.Магазин.ПравилоЦенообразования.ВидЦен = ЦеныНоменклатурыСрезПоследних.ВидЦены
	|			И Товары.Упаковка = ЦеныНоменклатурыСрезПоследних.Упаковка
	...
	Запрос.УстановитьПараметр("Номерпакета", НомерПакета);   	
	Результаты = Запрос.ВыполнитьПакет();
	ВыгрузитьМаркиПоДокументу((Результаты[7].Выбрать()));
...
Процедура ВыгрузитьТовары(ТоварыВыборка)
	...
	Пока ТоварыВыборка.Следующий() Цикл
		...
		КонецЕсли;
		Если ТоварыВыборка.VERSION <> ТоварВнешний.VERSION ИЛИ ТоварыВыборка.PRICE <> ТоварВнешний.PRICE ИЛИ НЕ ТоварыВыборка.MARGA = ТоварВнешний.MARGA тогда
...
Процедура ВыгрузитьМарки(ТолькоИзменения)
	УстановитьПривилегированныйРежим(Истина); 
	Счетчик = ПолучитьПоследнийНомерТМ();
	Если НЕ ТолькоИзменения тогда
		...
		Запрос.Текст = 
			...
			|	Марки.IDN КАК IDN,
			|	ВЫБОР
			|		КОГДА Марки.ВысокомаржинальнаяТМ
			|			ТОГДА ""8""
			|		ИНАЧЕ ""0""
			|	КОНЕЦ КАК MARGA,
			|	Марки.Наименование КАК NM
			|ИЗ
			|	Справочник.Марки КАК Марки
			|ГДЕ
			|	Марки.ПометкаУдаления = ЛОЖЬ
			|	И Марки.ЭтоГруппа = ЛОЖЬ";
	...
	Иначе
		...
		Запрос.Текст = 
		  "ВЫБРАТЬ
		  |	ТаблицаИзменения.Ссылка.IDN КАК IDN,
		  |	ВЫБОР
		  |		КОГДА ТаблицаИзменения.Ссылка.ВысокомаржинальнаяТМ
		  |			ТОГДА ""8""
		  |		ИНАЧЕ ""0""
		  |	КОНЕЦ КАК MARGA,
		  |	ТаблицаИзменения.Ссылка.Наименование КАК NM
		  |ИЗ
		  |	Справочник.Марки.Изменения КАК ТаблицаИзменения
		  |ГДЕ
	...
	//	делаем фиктивную выборку для присвоения регистрациям номера сообщения.
		ПланыОбмена.ВыбратьИзменения(УзелОбъект.Ссылка, УзелОбъект.НомерОтправленного,Метаданные.Справочники.Марки);

		УзелОбъект.Записать();

		Запрос.УстановитьПараметр("ТекущийУзел"       , УзелОбъект.Ссылка);
		Запрос.УстановитьПараметр("НомерОтправленного", УзелОбъект.НомерОтправленного);
		КонецЕсли;
		РезультатЗапроса = Запрос.Выполнить();  
		ВЫборка = РезультатЗапроса.Выбрать();
		...
		МаркаВн = ВнешниеИсточникиДанных.ТерминалыСбораДанных.Таблицы.dbo_ENT.НайтиПоПолю("IDN", СокрЛП(выборка.IDN));

		Если МаркаВн.Пустая() Тогда

				Марка = ВнешниеИсточникиДанных.ТерминалыСбораДанных.Таблицы.dbo_ENT.СоздатьОбъект();
				Марка.CODE = Счетчик;  
				
		Иначе	Марка = МаркаВн.ПолучитьОбъект();

		КонецЕсли;

		Марка.IDN  = СокрЛП(выборка.IDN);
		Марка.MARGA = СокрЛП(выборка.MARGA); 
		Марка.NM = СокрЛП(выборка.NM);      
        Попытка
			Марка.Записать();
	        Счетчик = Счетчик + 1;   
		ИСключение
...
Процедура ВыгрузитьМаркиПоДокументу(Выборка)  
	УстановитьПривилегированныйРежим(Истина);   
	Счетчик = ПолучитьПоследнийНомерТМ();
	Пока Выборка.Следующий()  цикл				
		ЕСЛИ ЗначениеЗаполнено(Выборка.IDN) тогда	
			МаркаВн = ВнешниеИсточникиДанных.ТерминалыСбораДанных.Таблицы.dbo_ENT.НайтиПоПолю("IDN", СокрЛП(Выборка.IDN));
...
Функция ПолучитьПоследнийНомерТМ()
	Код = 1;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	dbo_ENT.CODE КАК CODE
		|ИЗ
		|	ВнешнийИсточникДанных.ТерминалыСбораДанных.Таблица.dbo_ENT КАК dbo_ENT
		|
		|УПОРЯДОЧИТЬ ПО
		|	CODE УБЫВ";
...
Процедура ОбновитьНоменклатуру_УДАЛИТЬ() Экспорт
	...
	Запрос.Текст = 
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Марки КАК Марки
		|			ПО СпрНоменклатура.Марка = Марки.Ссылка
...
Процедура ОбновитьСправочникиВТСД(ТолькоИзменения)	Экспорт
	...
	Правило = Магазин.ПравилоЦенообразования;
	ВидЦены = Правило.ВидЦен; 
...
Процедура ЗаписатьТоварВоВнешнийИсточник(ТоварыВыборка)
	...
	Если НЕ ТоварыВыборка.VERSION = ТоварВнешний.VERSION ИЛИ НЕ ТоварыВыборка.PRICE = ТоварВнешний.PRICE ИЛИ НЕ ТоварыВыборка.IDN_ENT = ТоварВнешний.IDN_ENT
		ИЛИ НЕ ТоварыВыборка.PRICE_SUPPLIER = ТоварВнешний.PRICE_SUPPLIER ИЛИ НЕ ТоварыВыборка.DSCN = ТоварВнешний.DSCN тогда
...
Функция ПолучитьТекстЗапросаДанных()
	...
	ТекстЗапросаДанных =
		...
		|	КОНЕЦ КАК TNDS,
		|	ВЫБОР
		|		КОГДА ТаблицаЦены.ТипПереоценки = ЗНАЧЕНИЕ(Перечисление.ТипыПереоценок.ПромоНачало)
		|			ТОГДА ЕСТЬNULL(ОпорныеЦеныНоменклатурыСрезПоследних.Цена, 0)
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК PRICE_SUPPLIER,
		|	ВЫБОР
		|		КОГДА ТаблицаЦены.ТипПереоценки = ЗНАЧЕНИЕ(Перечисление.ТипыПереоценок.ПромоНачало)
		...
		|			И (ТаблицаЦены.ВидЦены = &ВидЦены)
		|			И Ведущая.Единица = ТаблицаЦены.Упаковка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Марки КАК Марки
		|		ПО Ведущая.Номенклатура.Марка = Марки.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОпорныеЦеныНоменклатуры.СрезПоследних(
		|				&ТекущаяДата,
		|				Магазин = &Магазин
		|					И Номенклатура В
		|						(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|							Товары.Номенклатура
		|						ИЗ
		|							Товары)) КАК ОпорныеЦеныНоменклатурыСрезПоследних
		|		ПО Ведущая.Номенклатура = ОпорныеЦеныНоменклатурыСрезПоследних.Номенклатура
		|ГДЕ
...
Функция ПолучитьТаблицаСТСДПакеты(Номер,ТипДок,Дата)
	...
	Запрос.Текст = 
		...
		|	СУММА(dbo_DOC_LINE.COUNT_PLAN) КАК COUNT_PLAN,
	...
	Запрос.Текст =		"ВЫБРАТЬ
	              		...
	              		|	ТабТСД.COUNT_PLAN КАК COUNT_PLAN,
	              		|	ТабТСД.IDN КАК IDN
	              		...
	              		|ГДЕ
	              		|	ТаблицаТСД.COUNT_FACT > 0 ИЛИ ТаблицаТСД.COUNT_PLAN > 0
	              		|
	              		|СГРУППИРОВАТЬ ПО
...
Процедура ВыгрузитьВсеПереоценки()
	...
	Запрос.Текст = 
		...
		|	ПереоценкаТоваровНаСкладахТовары.Номенклатура КАК Номенклатура,
		|	ПереоценкаТоваровНаСкладахТовары.Упаковка КАК Упаковка,
		|	ОпорныеЦеныНоменклатурыСрезПоследних.Цена КАК PRICE_SUPPLIER
		|ПОМЕСТИТЬ Товары
		|ИЗ
		|	Документ.ПереоценкаТоваровНаСкладах.Товары КАК ПереоценкаТоваровНаСкладахТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДействующиеЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, ОбъектЦенообразования = &Магазин) КАК ДействующиеЦеныНоменклатурыСрезПоследних
		...
		|			И ПереоценкаТоваровНаСкладахТовары.Упаковка = ДействующиеЦеныНоменклатурыСрезПоследних.Упаковка
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОпорныеЦеныНоменклатуры.СрезПоследних(&ТекущаяДатаКонец, ) КАК ОпорныеЦеныНоменклатурыСрезПоследних
		|		ПО ПереоценкаТоваровНаСкладахТовары.Номенклатура = ОпорныеЦеныНоменклатурыСрезПоследних.Номенклатура
		|			И ПереоценкаТоваровНаСкладахТовары.Упаковка = ОпорныеЦеныНоменклатурыСрезПоследних.Упаковка
		|			И ПереоценкаТоваровНаСкладахТовары.Склад.Магазин = ОпорныеЦеныНоменклатурыСрезПоследних.Магазин
		|ГДЕ
...
Процедура ВыгрузкаЦенВТСД() Экспорт
	...
	Запрос.Текст = 
		...
		|		ПО ВыгрузкаЦенВТСД.ДокПрименениеЦен = УстановкаЦенНоменклатурыТовары.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
   ...
   Выборка = РегистрыСведений.ВыгрузкаЦенВТСД.Выбрать();
	Пока Выборка.Следующий() Цикл
		...
		//	
		//	ПланыОбмена.ЗарегистрироватьИзменения(УзелОбмена.ссылка, СтрокаТЧ.Номенклатура);
		//	
		//КонецЦикла;
		
		НаборЗаписей = РегистрыСведений.ВыгрузкаЦенВТСД.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ДокПрименениеЦен.Установить(Выборка.ДокПрименениеЦен);
...
Процедура ОбновлениеЦенВТСД() Экспорт
	УстановитьПривилегированныйРежим(Истина);
    ВыгрузитьМарки(Истина);
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ДействующиеЦеныНоменклатуры.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)) КАК Единица
		|ПОМЕСТИТЬ Товары
		|ИЗ
		|	РегистрСведений.ДействующиеЦеныНоменклатуры КАК ДействующиеЦеныНоменклатуры
		|ГДЕ
		|	КОНЕЦПЕРИОДА(ДействующиеЦеныНоменклатуры.Период, ДЕНЬ) = &ТекущаяДата

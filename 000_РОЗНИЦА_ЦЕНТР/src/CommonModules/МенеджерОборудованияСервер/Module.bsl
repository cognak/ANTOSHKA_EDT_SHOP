...
Функция НайтиРабочиеМестаПоИД(ИдентификаторКлиента) Экспорт
	...
	|				И НЕ РабочиеМеста.РабочееМестоНаПериферии = ЗНАЧЕНИЕ(Справочник.РабочиеМеста.ПустаяСсылка)
	|				И НЕ РабочиеМеста.РабочееМестоНаПериферии.Магазин = ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка)
	|			ТОГДА РабочиеМеста.РабочееМестоНаПериферии
	...
	|				ТОГДА РабочиеМеста.Магазин = ЗНАЧЕНИЕ(Справочник.Магазины.ПустаяСсылка)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ");
	...
	СписокРабочихМест = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
...
Функция ПолучитьКартинкуШтрихкода(ПараметрыШтрихкода) Экспорт
	...
	ВнешняяКомпонента = МенеджерОборудованияСервер.ПодключитьВнешнююКомпонентуПечатиШтрихкода();
	...
	ВремЗначение = ВнешняяКомпонента.Высота;
	
	Если ПараметрыШтрихкода.Свойство("УголПоворота") Тогда	
		ВнешняяКомпонента.УголПоворота = ПараметрыШтрихкода.УголПоворота;
		Если ПараметрыШтрихкода.УголПоворота = 90 Или ПараметрыШтрихкода.УголПоворота = 270 Тогда
	...
	ВнешняяКомпонента.Высота = ВремЗначение; 
...
Функция ПодключитьВнешнююКомпонентуПечатиШтрихкода() Экспорт
	...
	ПодключениеВыполнено = ПодключитьВнешнююКомпоненту("ОбщийМакет.КомпонентаПечатиШтрихкодов", "КартинкаШтрихкода", ТипВнешнейКомпоненты.COM);
	...
	Если ПодключениеВыполнено Тогда
		...
		Исключение
			Возврат Неопределено;
			ЖурналСобытий.Регистрация("Ошибка печати штрих кода. Ошибка внешней компоненты.",,,,,,, Истина, Истина);
		КонецПопытки;
	Иначе		
		Возврат Неопределено;
	КонецЕсли;
...
//	LNK 04.01.2018 14:10:41
Процедура ЗаполнитьТипыОборудования(ТипыОборудования)	Экспорт

	ТипыОборудования = Новый Соответствие;

	Для каждого Элемент Из Перечисления.ТипыПодключаемогоОборудования Цикл

		ТипыОборудования.Вставить(Элемент, ПолучитьИмяТипаОборудования(Элемент));

	КонецЦикла;

КонецПроцедуры

//	LNK 22.07.2019 10:41:15
Процедура ТаблицаНалоговыхГруппПоУмолчанию(ТаблицаНалоговыхГрупп)	Экспорт

	ТаблицаНалоговыхГрупп.Очистить();

	СписокСтавокНДС = Новый Массив;
	...
	Для каждого СтавкаНДС Из СписокСтавокНДС Цикл

		НомерСтроки = НомерСтроки + 1;

		СтрокаТаблицы = ТаблицаНалоговыхГрупп.Добавить();

		СтрокаТаблицы.НалоговаяГруппаРРО = НомерСтроки;
...
Функция ПодключитьОборудование(ИдентификаторКлиента, ТипыПО = Неопределено,
	...
	СписокОборудования = ПолучитьСписокОборудования(ТипыПО, ИдентификаторУстройства);
	...
	Если СписокОборудования.Количество() > 0 Тогда
		...
		//	Проверим, не подключено ли устройство ранее
			ДанныеПодключаемогоОболрудования = ПараметрыСеанса.ДанныеПодключаемогоОболрудования.Получить();
			ПодключенноеУстройство = ПолучитьПодключенноеУстройство(ДанныеПодключаемогоОболрудования.ПараметрыПодключенияПО,
			                                                        Устройство.Ссылка);

			Если ПодключенноеУстройство = Неопределено Тогда // Если устройство не было подключено ранее

				Если Устройство.ОбработчикДрайвера = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор Тогда
					
					ПараметрыУстройства = ПолучитьПараметрыУстройства(Устройство.Ссылка);
				...
				НовоеПодключение.Вставить("Параметры"				, ПолучитьПараметрыУстройства(Устройство.Ссылка));
				НовоеПодключение.Вставить("КоличествоПодключенных"	, 1);
				НовоеПодключение.Вставить("ПараметрыПодключения"	, Новый Структура());

				Если НЕ Устройство.РежимВиртуальный = Истина 
						И НовоеПодключение.ОбработчикДрайвера = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор Тогда

					ВыходныеПараметры = Неопределено;
					Результат = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистраторСервер.ПодключитьУстройство(
						    ОбъектДрайвера,
				...
				Если Результат Тогда
					...
					ДанныеПодключаемогоОболрудования.ПараметрыПодключенияПО.Добавить(НовоеПодключение);
					ПараметрыСеанса.ДанныеПодключаемогоОболрудования = Новый ХранилищеЗначения(ДанныеПодключаемогоОболрудования);
...
Функция ОтключитьОборудование(ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено, ОписаниеОшибки = "")
	...
	Если ДанныеПодключаемогоОболрудования.ПараметрыПодключенияПО <> Неопределено Тогда
		КоличествоУстройств = ДанныеПодключаемогоОболрудования.ПараметрыПодключенияПО.Количество();
		Для Индекс = 1 По КоличествоУстройств Цикл
			ПодключенноеУстройство = ДанныеПодключаемогоОболрудования.ПараметрыПодключенияПО[КоличествоУстройств - Индекс];
			...
			Если КлиентПодключения <> Неопределено
			   ...
			   И (ИдентификаторУстройства = Неопределено
				...
				Если ПодключенноеУстройство.КоличествоПодключенных = 1 Тогда
					...
					ВыходныеПараметры = Неопределено;
					Если ПодключенноеУстройство.ОбработчикДрайвера = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор Тогда

						//	Вызов метода выполнения команды у обработчика
						Результат = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистраторСервер.ОтключитьУстройство(
							    ОбъектДрайвера,
					...
					НомерСтрокиМассива = ДанныеПодключаемогоОболрудования.ПараметрыПодключенияПО.Найти(ПодключенноеУстройство);
					Если НомерСтрокиМассива <> Неопределено Тогда
						ДанныеПодключаемогоОболрудования.ПараметрыПодключенияПО.Удалить(НомерСтрокиМассива);
					КонецЕсли;
				Иначе
					ПодключенноеУстройство.КоличествоПодключенных = ПодключенноеУстройство.КоличествоПодключенных - 1;
...
Функция ВыполнитьКоманду(Идентификатор, Команда, ВходныеПараметры, ВыходныеПараметры, Таймаут = -1) Экспорт
	...
	//	Получение объекта драйвера
		ОбъектДрайвера = ПолучитьОбъектДрайвера(ПодключенноеУстройство.ОбработчикДрайвера
			,
			, ?(ПодключенноеУстройство.ОбработчикДрайвера = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор
				, ПодключенноеУстройство.Параметры
				, Неопределено
			)
		);
		...
		Иначе
			...
			Если ОбработчикДрайвера = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор Тогда

				//	Вызов метода выполнения команды у обработчика
				Результат = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистраторСервер.ВыполнитьКоманду(Команда,
				                                                                             ВходныеПараметры,
				                                                                             ВыходныеПараметры,
				                                                                             ОбъектДрайвера,
				                                                                             Параметры,
...
// Получение объекта драйвера.
//
Функция ПолучитьОбъектДрайвера(ДрайверУстройства, ОписаниеОшибки = "", ПараметрыУстройства = Неопределено)

	ОбъектДрайвера = Неопределено;
	ДанныеПодключаемогоОболрудования = ПараметрыСеанса.ДанныеПодключаемогоОболрудования.Получить();
	
	Для Каждого ДрайверПО Из ДанныеПодключаемогоОболрудования.ДрайвераПодключаемогоОборудования Цикл

		Если ДрайверПО.Ключ = ДрайверУстройства Тогда

			ОбъектДрайвера = ДрайверПО.Значение;

			Если ДрайверУстройства = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор
					И ОбъектДрайвера.Параметры = Неопределено Тогда
	...
	Если ОбъектДрайвера = Неопределено Тогда
		...
		// Сканеры штрихкода/Считыватели магнитных карт
			...
			Если ДрайверУстройства = Перечисления.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикDeviceManagerФискальныйРегистратор Тогда

				ОбъектДрайвера = ПодключаемоеОборудованиеDeviceManagerФискальныйРегистраторСервер.СоздатьОбъектДрайвера(ПараметрыУстройства);
...
//			ЗаписьЖурналаРегистрации("ПолучитьОбъектДрайвера.Ошибка", УровеньЖурналаРегистрации.Ошибка,, ДрайверУстройства, ОписаниеОшибки);
	...
	Если НЕ ОбъектДрайвера = Неопределено Тогда
		...
		ДанныеПодключаемогоОболрудования.ДрайвераПодключаемогоОборудования.Вставить(ДрайверУстройства, ОбъектДрайвера);
		ОбъектДрайвера = ДанныеПодключаемогоОболрудования.ДрайвераПодключаемогоОборудования[ДрайверУстройства];
		ПараметрыСеанса.ДанныеПодключаемогоОболрудования = Новый ХранилищеЗначения(ДанныеПодключаемогоОболрудования);

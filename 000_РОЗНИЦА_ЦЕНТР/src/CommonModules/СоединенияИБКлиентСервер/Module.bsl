...
//	LNK 02.02.2018 13:40:11
Функция ПолучитьПараметрСтрокиСоединения(Знач Ключ, СтрокаСоединения = "")	Экспорт

	Параметр = "";
	КлючПараметра = ВРег(Ключ);
	КлючПараметра = ?(КлючПараметра = "SRVR" ИЛИ КлючПараметра = "SRVR=" ИЛИ КлючПараметра = "SERVER" ИЛИ КлючПараметра = "СЕРВЕР" ИЛИ
					  КлючПараметра = "PORT" ИЛИ КлючПараметра = "ПОРТ"
					, "Srvr=",
					?(КлючПараметра = "REF" ИЛИ КлючПараметра = "REF=" ИЛИ КлючПараметра = "ИБ" ИЛИ КлючПараметра = "БАЗА"
					, "Ref="
					, ""));
	ВернутьПорт   = (ВРег(Ключ) = "PORT" ИЛИ ВРег(Ключ) = "ПОРТ");

	Если НЕ ПустаяСтрока(КлючПараметра) Тогда

		Если ПустаяСтрока(СтрокаСоединения) Тогда

			СтрокаСоединения = ПолучитьСтрокуСоединенияИнформационнойБазы();

		КонецЕсли;

		ДанныеСтроки = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, СтрокаСоединения,, ";");

		Для каждого ЭлементСтроки Из ДанныеСтроки Цикл

			Если Лев(ЭлементСтроки, СтрДлина(КлючПараметра)) = КлючПараметра Тогда

				Если ВернутьПорт И КлючПараметра = "Srvr=" Тогда

					ДанныеПараметра = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, ЭлементСтроки,, ":");

					Если ДанныеПараметра.Количество() = 2 Тогда

						Параметр = СокрЛП(СтрЗаменить(ДанныеПараметра[1], """", ""));
				...
				Иначе
					...
					Если КлючПараметра = "Srvr=" Тогда

						ДанныеПараметра = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, ЭлементСтроки,, ":");

						Если ДанныеПараметра.Количество() = 2 Тогда	//	здесь указан порт сервера!

							ЭлементСтроки = ДанныеПараметра[0];	//	очистим имя сервера от порта.

						КонецЕсли;

					КонецЕсли;

					Параметр = СокрЛП(СтрЗаменить(Сред(ЭлементСтроки, СтрДлина(КлючПараметра) + 1), """", ""));
...
КонецФункции // ПолучитьПараметрСтрокиСоединения()

//	LNK 02.02.2018 10:08:10
Функция ПолучитьИдентификаторИмениСтрокиСоединения(СтрокаСоединения)	Экспорт

	Идентификатор = "";

	ЛишниеСимволы = Новый Массив;
	ЛишниеСимволы.Добавить("SRVR=");
	ЛишниеСимволы.Добавить("REF=");
	...
	ДанныеСтроки = ОбщегоНазначенияКлиентСервер.lx_FillValueList(, ВРег(СтрокаСоединения),, ";");

	Для каждого ЭлементСтроки Из ДанныеСтроки Цикл

		Для каждого ЛишнийСимвол Из ЛишниеСимволы Цикл

			ЭлементСтроки = СтрЗаменить(ЭлементСтроки, ЛишнийСимвол, "");

		КонецЦикла;

		Идентификатор = Идентификатор + ?(ПустаяСтрока(Идентификатор), "", "_") + ЭлементСтроки;

	КонецЦикла;

	Возврат Идентификатор;

КонецФункции // ПолучитьИдентификаторИмениСтрокиСоединения()

	...
	//	 1	Документ ПОЗЖЕ
		...
		Если НЕ ПозицияОтносительноГраницы = 0 Тогда	//	документ не является ГП!
			...
			//	Граница переносится назад, если документ проводится задним числом
				УстановитьГраницу = ДанныеИзменены;	//	зависимость от изменения атрибутов записи
...
Процедура ВыполнитьКорректировкуПоследовательности(Контрагент = Неопределено, УчетнаяПолитика = Неопределено)	Экспорт
...
//	1.	Устраняем "недостатки" регистрации в последовательности
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		БонусныеБаллыПоследовательность.РегистрацияВПоследовательности(

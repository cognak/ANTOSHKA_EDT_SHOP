...
Процедура ПередЗаписьюДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если НЕ Источник.ЭтоНовый() Тогда
		...
		Если ТипИсточника = Тип("ДокументОбъект.РеализацияТоваров") тогда 
			Если НЕ Источник.ДатаОплаты = Источник.ССылка.ДатаОплаты  тогда
				СделатьЗаписьВРСОтправкаРеализаций(Источник.Ссылка);	
			КонецЕсли;
		КонецЕсли;
...
Процедура СделатьЗаписьВРСОтправкаРеализаций(ДокССылка)
	НаборЗаписей = РегистрыСведений.РеализацииДляОтправки.СоздатьНаборЗаписей(); 

	НаборЗаписей.Отбор.ДокументРеализации.Установить(ДокССылка);

	НоваяЗапись = НаборЗаписей.Добавить();
	НоваяЗапись.ДокументРеализации = ДокССылка;

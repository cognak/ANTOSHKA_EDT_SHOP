...
Функция ВозвратПоКредитнойПрограмме(СчетПокупателюСсылка, КредитнаяПрограмма, ДокументВозврата, СуммаКредитаВозврат)	Экспорт
	...
	Если ПроверитьНазначениеСоединения(ДанныеСоединения, КредитнаяПрограмма, ДанныеОтвета.ТекстСообщения, ДанныеОтвета.Отказ) Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			...
			РаботаСJSONСервер.ЗаписатьСвойство("order_id", Выборка.ИдентификаторБанка, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("return_money_to_card", Истина, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("store_return_id", СтрЗаменить(СокрЛП(ДокументВозврата.УникальныйИдентификатор()), "-", ""), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("sum", СуммаКредитаВозврат, ЗаписьJSON);

			ЗаписьJSON.ЗаписатьИмяСвойства("additional_params");

				ЗаписьJSON.ЗаписатьНачалоОбъекта();

				РаботаСJSONСервер.ЗаписатьСвойство("nds", Выборка.СуммаНДС, ЗаписьJSON, 2);

...
//
Функция ДесериализацияАдреса(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено, АдресУкраины = Ложь) Экспорт
...
//	Пустой объект с представлением
	...
	Если АдресУкраины = Истина Тогда	//	LNK 05.01.2022 07:49:35

		Результат.Состав.Страна = "УКРАЇНА";
		Результат.Состав.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраины"));
...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) Экспорт
	...
	НужныРФПараметрыВНаимновании = (ТипЗнч(АдресПострочно)=Тип("ОбъектXDTO") И АдресПострочно.Тип()=ФабрикаXDTO.Тип(КонтактнаяИнформацияКлиентСерверПовтИсп.ПространствоИмен(), "АдресРФ"));
	...
	ИначеЕсли НужныРФПараметрыВНаимновании И АдресПострочно.СвРайМО.СвМО<>Неопределено Тогда
	...
	Возврат КонтактнаяИнформацияКлиентСервер.ПолноеНаименование(
		...
		Район, "",
		?(НужныРФПараметрыВНаимновании, АдресПострочно.Округ, АдресПострочно.Область), "",
		?(НужныРФПараметрыВНаимновании, АдресПострочно.СубъектРФ, ""), ""
	);
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей=Истина) Экспорт
	...
	XDTOТип = АдресРФ.Тип();
	Если XDTOТип=ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраины") Тогда
		...
		ДобавитьЗначение(Список, "Широта", АдресРФ.Широта);
		ДобавитьЗначение(Список, "Долгота", АдресРФ.Долгота);
...
Функция СписокРеквизитовНаселенныйПунктКОАТУУ(Код=Неопределено)
	...
	Результат = Новый Структура("Область, Район, Город, НаселенныйПункт",
		// Второй уровень - одинаково для всех классификаторов
		ЭлементАдреснойСтруктуры("Область",   "Область"),
		ЭлементАдреснойСтруктуры("Район",     "СвРайМО/Район"),
		ЭлементАдреснойСтруктуры("Город",     "Город"),
		ЭлементАдреснойСтруктуры("Нас.пункт", "НаселПункт", ,Истина)
	);
...
Функция ДесериализацияАдресаОбщая(ЗначенияПолей, Представление, ОжидаемыйТип=Неопределено)
	...
	Адрес.Состав = ?(АдресУкраинский, АдресУкраины, Представление);
...
//	LNK 03.10.2018 12:45:36
Функция ПолучитьВариантыНомераТелефона(НомерТелефона)	Экспорт

	СписокНомеров = Новый Массив;

//	LNK 29.07.2019 09:40:43	- Заменяем "3800" на "380"
	СписокЛексем = Новый Массив;
	СписокЛексем.Добавить("+3800");
	СписокЛексем.Добавить("3800");
//	СписокЛексем.Добавить("800");	могут быть варианты

	Для каждого Лексема Из СписокЛексем Цикл

		Если Лев(НомерТелефона, СтрДлина(Лексема)) = Лексема Тогда

			НомерТелефона = Лев(Лексема, СтрДлина(Лексема) - 1) + Сред(НомерТелефона, СтрДлина(Лексема) + 1);
			Прервать;
	...
	СписокНомеров.Добавить(НомерТелефона);

	Если Лев(НомерТелефона, 1) = "+" Тогда

		СписокНомеров.Добавить(Сред(НомерТелефона, 2));

	КонецЕсли;

	Если Лев(НомерТелефона, 3) = "+38" Тогда

		СписокНомеров.Добавить(Сред(НомерТелефона, 4));

	КонецЕсли;

	Если НЕ Лев(НомерТелефона, 3) = "+38" Тогда

		Если Лев(НомерТелефона, 1) = "0" Тогда

			СписокНомеров.Добавить("+38" + НомерТелефона);

		КонецЕсли;

		Если Лев(НомерТелефона, 2) = "80" Тогда

			СписокНомеров.Добавить("+3" + НомерТелефона);
...
КонецФункции // ПолучитьВариантыНомераТелефона()

//	LNK 24.10.2018 11:32:13
Функция ПолучитьСтрокуКонтактнойИнформации(Вид, КонтактнаяИнформация, Добавить = Истина)	Экспорт

	СтрокаКонтакты = КонтактнаяИнформация.Найти(Вид, "Вид");

	Если СтрокаКонтакты = Неопределено И Добавить = Истина Тогда
...
//	LNK 24.10.2018 11:40:39
Функция СформироватьДанныеАдреса(ДанныеАдреса, ПредставлениеАдреса)	Экспорт
	...
	Результат.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"));
	СоставАдреса = Результат.Состав;
	
	СоставАдреса.Страна = "УКРАИНА";
	СоставАдреса.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "АдресУкраины"));
	АдресПострочно      = СоставАдреса.Состав;
	
	Если НЕ ПустаяСтрока(ДанныеАдреса.Область) Тогда

		КонтактнаяИнформацияСлужебный.УстановитьСвойствоПоXPath(АдресПострочно, "Область", ДанныеАдреса.Область);

	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ДанныеАдреса.Город) Тогда

		КонтактнаяИнформацияСлужебный.УстановитьСвойствоПоXPath(АдресПострочно, "Город", ДанныеАдреса.Город);

	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ДанныеАдреса.Район) Тогда

		КонтактнаяИнформацияСлужебный.УстановитьСвойствоПоXPath(АдресПострочно, "СвРайМО/Район", ДанныеАдреса.Район);

	КонецЕсли;

	АдресПострочно.Улица = ДанныеАдреса.Улица;

	ТаблицаЗданий = Новый ТаблицаЗначений;
	ТаблицаЗданий.Колонки.Добавить("Тип", Новый ОписаниеТипов("Строка"));
	ТаблицаЗданий.Колонки.Добавить("Значение", Новый ОписаниеТипов("Строка"));
	ТаблицаЗданий.Колонки.Добавить("ПутьXPath", Новый ОписаниеТипов("Строка"));

	Если НЕ ПустаяСтрока(ДанныеАдреса.Дом) Тогда

		СтрокаЗдания = ТаблицаЗданий.Вставить(0);
		СтрокаЗдания.Тип      = "Дом";
		СтрокаЗдания.Значение = ДанныеАдреса.Дом;

	КонецЕсли;

	Если НЕ ПустаяСтрока(ДанныеАдреса.Корпус) Тогда

		СтрокаЗдания = ТаблицаЗданий.Вставить(0);
		СтрокаЗдания.Тип      = "Корпус";
		СтрокаЗдания.Значение = ДанныеАдреса.Корпус;

	КонецЕсли;

	Если НЕ ПустаяСтрока(ДанныеАдреса.Квартира) Тогда

		СтрокаЗдания = ТаблицаЗданий.Вставить(0);
		СтрокаЗдания.Тип      = "Квартира";
		СтрокаЗдания.Значение = ДанныеАдреса.Квартира;

	КонецЕсли;

	КонтактнаяИнформацияСлужебный.ЗданияИПомещенияАдреса(АдресПострочно, 
		Новый Структура("Здания, Помещения", ТаблицаЗданий, ТаблицаЗданий.СкопироватьКолонки())
	);
//	Индекс
	КонтактнаяИнформацияСлужебный.ПочтовыйИндексАдреса(АдресПострочно, ДанныеАдреса.Индекс);
//	Представление
	Результат.Представление = КонтактнаяИнформацияСлужебный.ПредставлениеАдреса(СоставАдреса, Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента);
...
КонецФункции // СформироватьДанныеАдреса()

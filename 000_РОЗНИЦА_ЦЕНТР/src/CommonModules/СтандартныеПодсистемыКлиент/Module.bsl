...
Процедура ПоказатьРезультатВыполнения(Форма, Результат) Экспорт
	...
	Если Результат.Свойство("ВыводПредупреждения") И Результат.ВыводПредупреждения.Использование Тогда
		Если ЗначениеЗаполнено(Результат.ВыводПредупреждения.ТекстОшибок) Тогда
			...
			Если ТипЗнч(Форма) = Тип("УправляемаяФорма") И ЗначениеЗаполнено(Результат.ВыводПредупреждения.ПутьКРеквизитуФормы) Тогда
		...
		Иначе
			ПоказатьПредупреждение(, Результат.ВыводПредупреждения.Текст);
		КонецЕсли;
	КонецЕсли;
...
//	LNK 27.09.2022 06:55:08
	РозничныеПродажиКлиент.ПротоколОС("ПередНачаломРаботыСистемы");
	...
	Если НЕ ПустаяСтрока(ПараметрыКлиента.ИнформационнаяБазаЗаблокированаДляОбновления) Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(, ПараметрыКлиента.ИнформационнаяБазаЗаблокированаДляОбновления);
		Возврат;
	КонецЕсли;
	...
	//	LNK 23.08.2016 13:55:54
		ИначеЕсли ПараметрыКлиента.ЭтоГлавныйУзел Тогда

			СтандартныеПодсистемыВызовСервера.ОбновитьИдентификаторыОбъектовМетаданных();

		Иначе

		//	LNK 09.10.2017 10:05:33 - на периферии отключаем обновление предопределённых элементов!
			СтандартныеПодсистемыВызовСервера.ОтключитьОбновлениеПредопределенныхЭлементов();
	...
	// СтандартныеПодсистемы.БазоваяФункциональность
	ОбщегоНазначенияРТКлиент.УстановитьЗаголовокПриложенияРТ(Истина);	 // Для вспомогательных окон.
...
Процедура ДействияПриНачалеРаботыСистемы(ОбработкаПараметровЗапуска = Истина) Экспорт
	...
	// СтандартныеПодсистемы.БазоваяФункциональность
	ОбщегоНазначенияРТКлиент.УстановитьЗаголовокПриложенияРТ(Истина); // Для главного окна.
...
Процедура ДействияПередЗавершениемРаботыСистемы(Отказ, ТекстПредупреждения) Экспорт
	...
	Если ЗапрашиватьПодтверждениеПриЗавершенииПрограммы Тогда

		Отказ = Истина;
		ТекстПредупреждения = "Вы желаете завершить работу?";

	КонецЕсли;
	
	Если Ложь Тогда	//	LNK 08.09.2017 10:08:20
...
Процедура ЗагрузитьОбновитьПараметрыРаботыПрограммы(Отказ, ПараметрыКлиента)	Экспорт
	...
	//Попытка
	//	Пока Истина Цикл
			Состояние(НСтр("ru = 'Пожалуйста, подождите, выполняется подготовка к обновлению информационной базы...'"));
	//		
	...
	//			ПоказатьПредупреждение(, ОшибкаУстановкиМонопольногоРежима);
	//			Отказ = Истина;
	//			Возврат;
	//		КонецЕсли;
	//		

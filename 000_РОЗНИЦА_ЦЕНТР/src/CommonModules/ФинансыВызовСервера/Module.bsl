...
Функция СписокДоступныхРасчетныхСчетов(Организация, КассаИлиМагазин, ВернутьСписокЗначений = Ложь)	Экспорт

	Возврат ФинансыСервер.СписокДоступныхРасчетныхСчетов(Организация, КассаИлиМагазин, ВернутьСписокЗначений);
...
Функция РезультатЗапросаДоступныхУслугБанка(СписокТоваров, КассаККМ, ТипУслуги, ИнтернетБанкинг)
	...
	|				ТОГДА НЕ ЕСТЬNULL(ТаблицаУслуги.Банк.ТипИнтернетБанкинга, ЗНАЧЕНИЕ(Перечисление.ТипыИнтернетБанкинга.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ТипыИнтернетБанкинга.ПустаяСсылка)
	|			ИНАЧЕ ЕСТЬNULL(ТаблицаУслуги.Банк.ТипИнтернетБанкинга, ЗНАЧЕНИЕ(Перечисление.ТипыИнтернетБанкинга.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ТипыИнтернетБанкинга.ПустаяСсылка)
	|		КОНЕЦ
	|	И ВЫБОР
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ УслугиБанка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ Исключить"
	);
	Запрос.УстановитьПараметр("СписокТоваров"	, СписокТоваров);

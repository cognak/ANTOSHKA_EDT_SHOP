...
Процедура ПараметрыРаботыКлиента(Параметры) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Ведущая.Ссылка КАК ТекущаяОрганизация,
	|	ЕСТЬNULL(ТаблицаСправочник.КодПоЕДРПОУ, Ведущая.КодПоЕДРПОУ) КАК КодПоЕДРПОУ,
	|	ЕСТЬNULL(ТаблицаСправочник.Наименование, ""undef"") КАК Наименование
	|ИЗ
	|	(ВЫБРАТЬ
	|		&ТекущаяОрганизация КАК Ссылка,
	|		"""" КАК КодПоЕДРПОУ) КАК Ведущая
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК ТаблицаСправочник
	|		ПО Ведущая.Ссылка = ТаблицаСправочник.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ТекущаяОрганизация"	, ОбщегоНазначенияРТСервер.ТекущаяОрганизация());	//	LNK 04.03.2024 07:53:18

	РезультатыЗапроса = Запрос.ВыполнитьПакет();

	ВыборкаМагазин		= РезультатыЗапроса[0].Выбрать();
	ВыборкаМагазин.Следующий();
	ВыборкаОрганизация	= РезультатыЗапроса[1].Выбрать();
	ВыборкаОрганизация.Следующий();	//	LNK 04.03.2024 07:53:34
	ВыборкаУзел			= РезультатыЗапроса[2].Выбрать();
	...
	Параметры.Вставить("ТекущаяОрганизацияРеквизиты", ОбщегоНазначенияРТ.СоздатьСтруктуруПоСтрокеВыборки(РезультатыЗапроса[1], ВыборкаОрганизация, Истина));
	Параметры.Вставить("ТекущийУзелРеквизиты"		, ОбщегоНазначенияРТ.СоздатьСтруктуруПоСтрокеВыборки(РезультатыЗапроса[2], ВыборкаУзел, Истина));

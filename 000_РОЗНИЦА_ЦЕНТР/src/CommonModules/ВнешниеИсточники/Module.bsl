...
Функция ПолучитьДанныеОтNavision(ИмяМакета, СтрокаПоиска = "IN ('9999999999')", СтрокаЗамены = "", ПереданТекстЗапроса = Ложь, ДлительностьПаузы = 1000, ПараметрПервые = "")	Экспорт
	...
	Если НЕ ПереданТекстЗапроса Тогда
		...
		ЗначениеОтметки = РегистрыСведений.Отметки_timestamp.ПолучитьОтметку(ИмяМакета);

		ТекстЗапроса = СтрЗаменить(ТекстЗапроса
			, "[timestamp] >= CAST(CAST('000000000000' AS BIGINT)"
			, "[timestamp] >= CAST(CAST('" + ЗначениеОтметки + "' AS BIGINT)"
		);
...
Процедура ПолучитьПоставщиковNavision(Отладка = Ложь)	Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		КодИзменения = "";	//	LNK 12.12.2023 05:40:33
		...
		//	.. возвращаемое значение здесь не нужно
			...
			Если ПустаяСтрока(КодИзменения) ИЛИ Выборка.КодИзменения > КодИзменения Тогда

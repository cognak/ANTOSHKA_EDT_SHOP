...
Функция ПолучитьДанныеОтNavision(ИмяМакета, СтрокаПоиска = "IN ('9999999999')", СтрокаЗамены = "", ПереданТекстЗапроса = Ложь, ДлительностьПаузы = 1000, ПараметрПервые = "", КодИзменения = "")	Экспорт
	...
	Если НЕ ПереданТекстЗапроса Тогда

		КодИзменения = РегистрыСведений.Отметки_timestamp.ПолучитьОтметку(ИмяМакета);

		ТекстЗапроса = СтрЗаменить(ТекстЗапроса
			, "[timestamp] > CAST(CAST('000000000000' AS BIGINT)"
			, "[timestamp] > CAST(CAST('" + КодИзменения + "' AS BIGINT)"
		);
...
Процедура УстановитьДанныеДокумента_УДАЛИТЬ(ВыборкаДокументы, Колонки, ОбновитьАналитику, СсылкиОбработаны)	//	LNK 10.01.2024 09:07:40
...
//	"Прямой" вызов функции для определения элемента в параметре "Номенклатура" передает пустую ссылку!
	...
	//2026-06-29 sa Если код изменения не изменился, то нехер маслать лишний раз базу.
	//2026-07-21 sa Код изменения не актуален. Т.к. в Навижн есть поля которые 1С Розница не интересуют.
	СтруктураТовара_ = ПолучитьСтруктуруПоСтрокеТЗ(ДанныеNavision);
	ХешТовара = ПолучитьХешSHA256Структуры(СтруктураТовара_);
	Если СокрЛП(Объект.Хеш) = ХешТовара Тогда
		...
		Если НЕ Блокиратор = Неопределено И НЕ Объект.ЭтоГруппа Тогда
	...
	Если ТребуетсяОбновление Тогда
		...
		//2026-07-23 sa если это Объект и добрались сюда, значит обновляем Хеш
		Объект.Хеш = ХешТовара;
	...
	Если ТребуетсяОбновление Тогда
		...
		Если НЕ Объект.ЭтоГруппа Тогда
			//2026-08-10 sa добавляем учетную цену из Навижн. Почему не регистр сведений? - Потому что так поставлена задача от Маркетинга.
			Объект.УчетнаяЦена = ДанныеNavision.УчетнаяЦена;
		...
		//	LNK 10.05.2021 07:00:57
			...
			//Объект.Кешбек =   ?(ДанныеNavision.Кешбек = 0 , ЛОЖЬ,ИСтина); // А++ 20240919 по задаче кэшбэка
			Объект.Кешбек =   ДанныеNavision.Кешбек; // А++ 20240919 по задаче кэшбэка
		...
		Если НЕ ПустаяСтрока(Объект.IDN) Тогда
			...
			Если Объект.ЭтоНовый() Тогда	//	LNK 29.07.2024 06:30:47
				...
				Запрос.УстановитьПараметр("IDN", Объект.IDN);
				
				РезультатЗапроса = Запрос.Выполнить();
				ВыполнитьЗапись  = РезультатЗапроса.Пустой();
			...
			Если ВыполнитьЗапись Тогда	//	LNK 29.07.2024 06:30:47
				// Бакан ТС 2025.08.08 Добавил блокировку  
			...
			Иначе
				...
				ЗаписьЖурналаРегистрации("НоменклатураNavision", УровеньЖурналаРегистрации.Ошибка
					, Метаданные.Справочники.Номенклатура, Объект.Ссылка, "Пресечена запись дубля IDN", РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации("НоменклатураNavision", УровеньЖурналаРегистрации.Предупреждение
				, Метаданные.Справочники.Номенклатура, Объект.Ссылка, "Пресечена запись с пустым IDN", РежимТранзакцииЗаписиЖурналаРегистрации.Транзакционная);
...
//	LNK 29.07.2024 08:11:58 - перенёс в это место
//	---------------------------------------------------------------------------------------
	Если НЕ Блокиратор = Неопределено И НЕ Объект.ЭтоГруппа Тогда
...
Процедура ПолучитьПоставщиковNavision(Отладка = Ложь)	Экспорт
	...
	КодИзмененияПервый = "0";
	Запрос.УстановитьПараметр("ТаблицаNavision", ПолучитьДанныеОтNavision("ПоставщикиNavision",,,,,, КодИзмененияПервый));

	РезультатЗапроса = Запрос.Выполнить();

	Если НЕ РезультатЗапроса.Пустой() Тогда

		КодИзменения = "0";	//	LNK 12.12.2023 05:40:33
		...
		//	.. возвращаемое значение здесь не нужно
			...
			Если Число(Выборка.КодИзменения) > Число(КодИзменения) Тогда

			//	LNK 12.12.2023 05:41:59
		...
		РегистрыСведений.Отметки_timestamp.УстановитьОтметку("ПоставщикиNavision", КодИзменения);

		ЖурналСобытий.Регистрация("TIMESTAMP.ПоставщикиNavision"
			, УровеньЖурналаРегистрации.Примечание
			, Метаданные.Справочники.Контрагенты
			,
			,
			, "timestamp(1): " + КодИзмененияПервый + Символы.ПС +
			  "timestamp(2): " + КодИзменения + Символы.ПС +
			  "выборка: " + Выборка.Количество()
			,
			, Истина
			, Ложь
		);
...
Процедура ПолучитьПокупателейБезналNavision(Отладка = Ложь)	Экспорт
	...
	|			ИЛИ ВЫБОР
	|				КОГДА ДанныеNavision.Отправитель = ""АНТ""
	|					ТОГДА НЕ ДанныеNavision.КодИзменения = ТаблицаКонтрагенты.КодИзменения
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	КодИзмененияПервый = "0";
	Запрос.УстановитьПараметр("ТаблицаNavision", ПолучитьДанныеОтNavision("ПокупателиNavision",,,,,, КодИзмененияПервый));
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		РегистрыСведений.Отметки_timestamp.УстановитьОтметку("ПокупателиNavision", КодИзменения);

		ЖурналСобытий.Регистрация("TIMESTAMP.ПокупателиNavision"
			, УровеньЖурналаРегистрации.Примечание
			, Метаданные.Справочники.Контрагенты
			,
			,
...
Процедура ОбновитьКлассификаторыПоДаннымNavision()	Экспорт
	...
	ПроверитьИнфраструктуруМагазинов();
...
Процедура ОбновитьКлассификаторыПоДаннымNavision_Склады()	Экспорт	//	Экспорт в целях отладки!
	...
	|		ПО ТаблицаNavision.Отдел = ТаблицаОрганизации.IDN
	|ГДЕ
	...
	РезультатЗапроса = Запрос.Выполнить();	//	в целях отладки - получить таблицу значений.

	Если НЕ РезультатЗапроса.Пустой() Тогда

		СкладыВыборка = РезультатЗапроса.Выбрать();
...
Функция УстановитьМагазинПоДаннымNavision(НомерМагазина, ДанныеNavision,  СкладСсылка)
	...
	Если ЗначениеЗаполнено(НомерМагазина) Тогда
		...
		Если СкладУправляющейСистемы Тогда
			...
			|	ТаблицаСправочник.СкладСвязи = &СкладСвязи КАК ЭтоСкладСвязи
			|ИЗ
			...
			Запрос.УстановитьПараметр("СкладСвязи", СкладСсылка);
			...
			Если Выборка.Следующий() Тогда
				...
				НужноОбновлять = Выборка.ЭтоСкладСвязи;
		...
		//	Наименование в NAV совсем не такое, как было в магазинах на 7.7, поэтому такие приседания
			...
			Иначе
				...
				МагазинОбъект.Код = "00-" + Формат(НомерМагазина, "ЧЦ=6; ЧДЦ=; ЧН=000000; ЧВН=; ЧГ=");
		...
		Если НужноОбновлять Тогда
			...
			УстановитьКодБюджетаМагазина(МагазинОбъект, ДанныеNavision);

			Если МагазинОбъект.СкладПоступления.Пустая() Тогда

				МагазинОбъект.СкладПоступления = СкладСсылка;
...
Функция УстановитьСкладПоДаннымNavision(ДанныеNavision, ОбновитьАналитику)

	Если ЗначениеЗаполнено(ДанныеNavision.Организация) Тогда	//	LNK 19.11.2023 12:05:03
	...
	//	---------------------------------------------------------------------------------------
		МагазинСклада = УстановитьМагазинПоДаннымNavision(
			  ?(СкладОбъект.ТипСклада = Перечисления.ТипыСкладов.ТорговыйЗал
					, Число(ДанныеNavision.Префикс)
					, ДанныеNavision.Филиал
				)
			, ДанныеNavision
	...
	//	Магазин установили. Теперь нужно для установленного магазина определить Организация!
	//	У самого склада реквизита "Организация" теперь нет. Используется регистр сведений.
	//	---------------------------------------------------------------------------------------
		Справочники.Магазины.УстановитьОрганизацию(СкладОбъект.Магазин, ДанныеNavision.Организация);
...
//	LNK 19.09.2023 07:58:11
Процедура УстановитьКодБюджетаМагазина(МагазинОбъект, ДанныеNavision)


	Если ПустаяСтрока(ДанныеNavision.КодБюджета) Тогда

		Если НЕ МагазинОбъект.КодБюджета.Пустая() Тогда

			МагазинОбъект.КодБюджета = Справочники.КодыБюджетов.ПустаяСсылка();
	...
	Иначе
		...
		|	ТаблицаСправочник.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.КодыБюджетов КАК ТаблицаСправочник
		|ГДЕ
		|	ТаблицаСправочник.Код = &КодБюджета"
		);
		Запрос.УстановитьПараметр("КодБюджета", СокрЛП(ДанныеNavision.КодБюджета));

		РезультатЗапроса = Запрос.Выполнить();

		Если РезультатЗапроса.Пустой() Тогда

			КодБюджетаОбъект = Справочники.КодыБюджетов.СоздатьЭлемент();
			КодБюджетаОбъект.Код = СокрЛП(ДанныеNavision.КодБюджета);
			КодБюджетаОбъект.Наименование = СокрЛП(МагазинОбъект.Наименование);

			КодБюджетаОбъект.Записать();
		...
		Иначе
			...
			МагазинОбъект.КодБюджета = Выборка.Ссылка;
...
//	LNK 20.11.2023 06:52:32
Процедура ПроверитьИнфраструктуруМагазинов()	Экспорт	//	"Экспорт" в целях отладки
	...
	|	ТаблицаВедущая.КассаМагазина КАК КассаМагазина,
	|	ТаблицаОрганизаций.Организация КАК Организация,
	|	ТаблицаВедущая.КассаМагазина.Владелец КАК ОрганизацияКассы,
	|	ТаблицаВедущая.НомерМагазина КАК НомерМагазина
	|ИЗ
	...
	|ГДЕ
	|	НЕ(ТаблицаВедущая.Блокирован
	|				ИЛИ ТаблицаВедущая.ПометкаУдаления
	|				ИЛИ ТаблицаВедущая.СкладУправляющейСистемы)
	|	И ТаблицаВедущая.ВведенВЭксплуатацию
	|	И (ТаблицаВедущая.КассаМагазина = ЗНАЧЕНИЕ(Справочник.Кассы.ПустаяСсылка)
	|			ИЛИ НЕ ТаблицаОрганизаций.Организация = ТаблицаВедущая.КассаМагазина.Владелец)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерМагазина"
	);
...
//	В текущей реализации отрабатываем только основную кассу магазина.
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		//	Заменить кассу нужно в любом случае - что она пустая, что принадлежит другой организации!
		//	Для начала пытаемся найти нужную кассу среди существующих.
...
// 2026-07-21 sa Преобразуем строку ТЗ в Структуру
Функция ПолучитьСтруктуруПоСтрокеТЗ(СтрокаТаблицы)
	СтруктураТовара = Новый Структура;
	Для Каждого Колонка_ Из СтрокаТаблицы.Владелец().Колонки Цикл
        СтруктураТовара.Вставить(Колонка_.Имя, СтрокаТаблицы[Колонка_.Имя]);
    КонецЦикла;
    
    Возврат СтруктураТовара;		
КонецФункции  

// 2026-07-21 sa Получаем Хеш структуры по SHA256
Функция ПолучитьХешSHA256Структуры(ИсходнаяСтруктура) Экспорт
	//Код изменения больше не актуален. Он может изменится по полям которые 1С Розница не интересуют. 
	Если ИсходнаяСтруктура.Свойство("КодИзменения") Тогда
		ИсходнаяСтруктура.Удалить("КодИзменения");
	КонецЕсли;
    ...
    ХешОбъект = Новый ХешированиеДанных(ХешФункция.SHA256);
    ХешОбъект.Добавить(СтрокаДанных);
    
    ХешСтрока = ПолучитьHexСтрокуИзДвоичныхДанных(ХешОбъект.ХешСумма);
    
    Возврат НРег(ХешСтрока);

	...
	//	а именно "4" для уточнения (в отладке), что клиент НЕ единственный.
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			//	Сохраняем значения "взведенных" по ссылке флагов!
				Если ДанныеОбъекта[Имя] = Ложь И НЕ ДанныеОбъекта[Имя] = Выборка[Имя] Тогда
...
Процедура ПроверитьНоменклатуруПоОстаткамNavision()	Экспорт
	...
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ТаблицаТовары.КодИзменения ЕСТЬ NULL 
	|				ТОГДА ИСТИНА
...
Процедура ПроверитьНоменклатуруПоОстаткамНаСкладах()	Экспорт
	...
	Пока Истина Цикл
		...
		Запрос.УстановитьПараметр("IDN", "6037709");
		СписокКодов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("IDN");
		Запрос.УстановитьПараметр("СписокОтработанныхКодов"
		, ОбщегоНазначенияКлиентСервер.ADD(Запрос.Параметры.СписокОтработанныхКодов, СписокКодов));
		...
		//	Обновим существующие товарные позиции или создадим новые, если так случилось.
			...
			Сообщить(СписокКодов[СписокКодов.Количество() - 1].IDN);

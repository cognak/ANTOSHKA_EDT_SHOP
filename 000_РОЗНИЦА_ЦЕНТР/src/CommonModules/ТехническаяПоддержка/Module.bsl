...
Процедура ОчисткаПротоколСообщений()

	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 25000
		|	ПротоколСообщений.Период КАК Период,
		|	ПротоколСообщений.Объект КАК Объект,
		|	ПротоколСообщений.Контрагент КАК Контрагент,
		|	ПротоколСообщений.ВидСообщения КАК ВидСообщения
		|ИЗ
		|	РегистрСведений.ПротоколСообщений КАК ПротоколСообщений
		|ГДЕ
		|	ПротоколСообщений.Период < ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, -3)";
	...
	Наборзаписей = РегистрыСведений.ПротоколСообщений.СоздатьНаборЗаписей();
	НаборЗаписей.ОбменДанными.Загрузка = Истина;
	НаборЗаписей.ОбменДанными.Получатели.АвтоЗаполнение = Ложь;
	НаборЗаписей.ДополнительныеСвойства.Вставить("ОтключитьМеханизмРегистрацииОбъектов", Истина);
	НаборЗаписей.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения"  , Истина);
	...
	Пока Выборка.Следующий() Цикл
		...
		Наборзаписей.Отбор.ВидСообщения.Установить(Выборка.ВидСообщения);
		Наборзаписей.Записать();

...
//	LNK 13.01.2017 09:56:03
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.КонтрольДинамическогоОбновленияКонфигурации") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("КонтрольДинамическогоОбновленияКонфигурацииСлужебный");
		Модуль.ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики);
	КонецЕсли;
...
//	LNK 14.08.2017 09:44:04
	ФинансыСлужебный.ОбновитьДанныеПодсистемыФинансы();
...
Процедура ОбновитьПараметрыРаботыПрограммы_ТИПОВАЯ(НайтиИзменения = Ложь,
                                           ЕстьИзменения = Неопределено,
                                           ОшибкаУстановкиМонопольногоРежима = Неопределено) Экспорт
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена) Экспорт
	...
	|ИЗ
	|	ПланОбмена.ПоМагазину КАК ПланОбмена
	|ГДЕ
	|	НЕ ПланОбмена.ЭтотУзел
	|	И НЕ ПланОбмена.ПометкаУдаления"
	, ".ПоМагазину", "." + ИмяПланаОбмена)
	);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Объект.ОбменДанными.Получатели.Добавить(Выборка.Получатель);
...
Функция ТипОбъектаИлиНабораЗаписейОбъектаМетаданных(ОбъектМетаданных) Экспорт
	...
	//	для перечисления что ссылка, что объект...
		Менеджер = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ОбъектМетаданных.ПолноеИмя());
		Тип = ТипЗнч(Менеджер.ПустаяСсылка());
...
Процедура ЗагрузитьОбновитьПараметрыРаботыПрограммы(ОшибкаУстановкиМонопольногоРежима = Неопределено) Экспорт
	...
	Если НЕ НеобходимоОбновлениеПараметровРаботыПрограммы(Ложь) Тогда
		Возврат;
	КонецЕсли;
...
// Только для внутреннего использования.
Процедура ЗагрузитьОбновитьПараметрыРаботыПрограммы_ТИПОВАЯ(ОшибкаУстановкиМонопольногоРежима = Неопределено) Экспорт
...
Функция ИменаСерверныхМодулей() Экспорт
	...
	Для каждого ОбщийМодуль Из Метаданные.ОбщиеМодули Цикл
		...
		#Если ТолстыйКлиентУправляемоеПриложение Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			Если ОбщийМодуль.Сервер Или ИнформационнаяБазаФайловая Тогда
				СерверныеМодули.Вставить(Вычислить(ОбщийМодуль.Имя), ОбщийМодуль.Имя);
			КонецЕсли;
		#Иначе
			Если ОбщийМодуль.Сервер Тогда
				СерверныеМодули.Вставить(Вычислить(ОбщийМодуль.Имя), ОбщийМодуль.Имя);
			КонецЕсли;
		#КонецЕсли
...
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	Если ИменаПараметровСеанса = Неопределено Тогда
		...
		ДанныеПодключаемогоОболрудования = Новый Структура;
		ДанныеПодключаемогоОболрудования.Вставить("ДрайвераПодключаемогоОборудования"	, Новый Соответствие());
		ДанныеПодключаемогоОболрудования.Вставить("ПараметрыПодключенияПО"				, Новый Массив());
		ДанныеПодключаемогоОболрудования.Вставить("ПоследнийСлипЧек"					, "");
		ДанныеПодключаемогоОболрудования.Вставить("ТаблицаУстройствДМ"					, Новый Структура());
		ДанныеПодключаемогоОболрудования.Вставить("ПараметрыДрайверМенеджера"			, Новый Структура());
	...
	ИначеЕсли НЕ ИменаПараметровСеанса.Найти("ПараметрыКлиентаНаСервере") = Неопределено Тогда
		
		ПараметрыСеанса.ПараметрыКлиентаНаСервере = Новый ФиксированноеСоответствие(Новый Соответствие);
		УстановленныеПараметры.Добавить("ПараметрыКлиентаНаСервере");
		
	ИначеЕсли НЕ ИменаПараметровСеанса.Найти("ИсключительныйРежим") = Неопределено Тогда
		
	//	LNK 27.01.2017 14:35:46
		ПараметрыСеанса.ИсключительныйРежим = Ложь;
		
	ИначеЕсли НЕ ИменаПараметровСеанса.Найти("КонтекстЦентральногоУзла") = Неопределено Тогда

	//	LNK 14.09.2016 10:24:36
		ПараметрыСеанса.КонтекстЦентральногоУзла = ОбменДаннымиПовтИсп.ЭтоГлавныйУзел();
		
	ИначеЕсли НЕ ИменаПараметровСеанса.Найти("ИмяВнешнегоРМК") = Неопределено Тогда

		ПараметрыСеанса.ИмяВнешнегоРМК = "";	//	LNK 29.05.2019 13:53:30
...
Процедура ПослеПолученияДанных(Источник, Отказ) Экспорт
	...
	Если Источник.Метаданные().РаспределеннаяИнформационнаяБаза Тогда
		...
		Если Источник.НомерПринятого <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник.Ссылка, "НомерПринятого") Тогда
			...
			Если ОбменДаннымиПовтИсп.ГлавныйУзел() = Источник.Ссылка Тогда
...
//	LNK 03.09.2017 10:36:14 - обязательно ДО вызовов функция определения контекста обменов "ПоМагазину"!
	Константы.КонтекстОбменовПоМагазину.СоздатьМенеджерЗначения().ПроверитьЗаполнениеЗначения();

//	LNK 25.04.2017 12:33:59
	ПараметрыСеанса.ЭтоГлавныйУзел = ОбменДаннымиПовтИсп.ЭтоГлавныйУзел();
//	LNK 21.04.2017 09:21:21
	УстановитьПараметрСеансаОрганизацииУзла();
//	LNK 06.05.2017 07:27:23
	ПараметрыСеанса.ИмитацияПериферийногоУзла = Ложь;
	
	Если НЕ ПользователиСлужебный.ПользовательНеТребуетАвторизации() Тогда	//	LNK 04.06.2018 10:52:06

		Если ПустаяСтрока(Метаданные.Версия) Тогда
			ВызватьИсключение "Не визначена властивість конфігурації «Версія»! Подальші дії неможливі.";
		Иначе
			Попытка
			...
			Исключение
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1"".
					           |Правильный формат, например: ""2.1.2.94"".'"),
					Метаданные.Версия);
			КонецПопытки;
...
//	LNK 21.04.2017 09:22:10
Процедура УстановитьПараметрСеансаОрганизацииУзла()
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ &ТекущийУзел = ТаблицаУзлы.Ссылка
	|		КОНЕЦ
	|
	|ОБЪЕДИНИТЬ
	|
	...
	|ИЗ
	|	Справочник.СтруктураУзлов.Магазины КАК ТаблицаВедущая
	...
	|ГДЕ
	|	НЕ &ЭтоГлавныйУзел
	|	И ТаблицаВедущая.Ссылка = &ЭлементСтруктуры
	|	И НЕ ЕСТЬNULL(ТаблицаОрганизаций.Организация, ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	ПланОбмена.ПоМагазину.Организации КАК ТабличнаяЧасть
	|ГДЕ
	|	НЕ &ЭтоГлавныйУзел
	|	И НЕ ТабличнаяЧасть.Ссылка.ПометкаУдаления
	|	И НЕ ЕСТЬNULL(ТабличнаяЧасть.Организация, ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|	И ВЫБОР
	|			КОГДА &ЭтоГлавныйУзел
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ &ТекущийУзел = ТабличнаяЧасть.Ссылка
	|		КОНЕЦ
	|
	|ОБЪЕДИНИТЬ
	|
...
//	Идея в чем... в центральном узле должны быть доступны все организации (кроме помеченных).
//	А на периферии только некоторые - та, которая указана для "ЭтотУзел", а так же организации, указанные в магазинах узла.
//	Последние не присутствуют в периферии напрямую, а передаются из центра в табличной части элемента структуры узлов.
	ПараметрыСеанса.ОрганизацииУзла = Новый ФиксированныйМассив(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Организация"));

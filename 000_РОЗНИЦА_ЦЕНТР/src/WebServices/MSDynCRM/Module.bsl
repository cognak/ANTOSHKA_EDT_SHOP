...
Функция MiscellaniusInfo(Command = "")
	...
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда// И КлючиПолитики.РазрешитьОбменВходящий = Истина Тогда
		...
		Иначе
			...
			РаботаСJSONСервер.ЗаписатьСвойство("error", "unknown command", ЗаписьJSON);

		КонецЕсли;

	Иначе

		РаботаСJSONСервер.ЗаписатьСвойство("error", ВходящийЗапрещён("MSDynCRM.MiscellaniusInfo"), ЗаписьJSON);
...
Процедура ПолучитьСостояниеОбменаДанными(ЗаписьJSON)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			РаботаСJSONСервер.ЗаписатьСвойство("code", СокрЛП(Выборка.Код), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("type", ОбщегоНазначенияКлиентСервер.РеквизитПеречисления(Выборка.ТипРегистрации), ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("messageNumber", Выборка.НомерОтправленного, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("count", Выборка.Количество, ЗаписьJSON);
...
Функция ОтветНаЗапрос(ТекстОшибки)	//	LNK 03.04.2019 12:11:13
	...
	ЗаписьJSON.ЗаписатьНачалоОбъекта();
	РаботаСJSONСервер.ЗаписатьСвойство("error", СокрЛП(ТекстОшибки), ЗаписьJSON);
	ЗаписьJSON.ЗаписатьКонецОбъекта();

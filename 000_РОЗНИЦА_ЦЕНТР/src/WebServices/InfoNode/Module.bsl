...
Функция ЗаписатьЗаказПокупателя(ДокументОбъект, УчетнаяПолитика, СоставОшибки)
	...
	Попытка
		...
		ПерваяЦифра = Число(Лев(ДокументОбъект.Номер, 1));
	...
	Запрос.Текст =
		...
		|	Контрагенты.Блокирован КАК Блокирован,
		|	Контрагенты.Ссылка
		|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	КонтрагентЗаблокирован = Выборка.Блокирован;
	...
	Запрос.Текст =
		...
		|	ВТ_УникалныеВЗаказе КАК ВТ_УникалныеВЗаказе
		|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		ДокументОбъект.Комментарий = ?(Не ПустаяСтрока(ДокументОбъект.Комментарий),ДокументОбъект.Комментарий  + Символы.ПС, "") + "Дубль заказа №" + СокрЛП(Выборка.Ссылка.Номер);
		ДокументОбъект.Записать();
	КонецЕсли;

	Если НЕ СоставОшибки.Отказ
			И ДокументОбъект.Статус = Перечисления.СтатусыЗаказовПокупателей.Новый
			И Не ДокументОбъект.УчетнаяСистема = Перечисления.УчетныеСистемыКомпании.Magento
			И ДокументОбъект.НеПерезванивать
			И ПустаяСтрока(ДокументОбъект.Комментарий)
			И ДокументОбъект.Проведен
			И Не КонтрагентЗаблокирован
			И Не СотрудникАнтошки
			И СтараяНумерация Тогда
//			И (ДокументОбъект.СуммаДокумента > УчетнаяПолитика.МинимальнаяСуммаАвтовыполнения
//				Или ОдинаковИсточникПриемник) Тогда

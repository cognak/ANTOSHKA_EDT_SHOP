			...
			ЗаписьJSON.ЗаписатьИмяСвойства("programs");
				...
				Пока УслугиВыборка.Следующий() Цикл
					...
					ЗаписьJSON.ЗаписатьНачалоОбъекта();
						...
						РаботаСJSONСервер.ЗаписатьСвойство("ref", ОбменMagentoСлужебный.IDC_Строкой(УслугиВыборка.УслугаБанка), ЗаписьJSON);
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
				...
				РаботаСJSONСервер.ЗаписатьСвойство("ref"    , ОбменMagentoСлужебный.IDC_Строкой(Выборка.Услуга), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("blocked", Выборка.Блокирован, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("name"   , Выборка.Наименование, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("type"   , ОбщегоНазначенияКлиентСервер.РеквизитПеречисления(Выборка.ТипУслуги), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("bank"   , СокрЛП(Выборка.МФО), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("depth"  , Выборка.ПериодПредоставления, ЗаписьJSON);
	...
	//	---------------------------------------------------------------------------------------
		...
		Пока ОбщаяВыборка.Следующий() Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
				...
				РаботаСJSONСервер.ЗаписатьСвойство("wh_code"	, ОбщаяВыборка.КодСклада, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("sku"		, ОбщаяВыборка.КодТовара, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("qty"		, Формат(Макс(0, ОбщаяВыборка.КоличествоСклад)  , "ЧН=0; ЧГ="), ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("qty_r1"	, Формат(Макс(0, ОбщаяВыборка.КоличествоЗаказы) , "ЧН=0; ЧГ="), ЗаписьJSON);
				...
				Пока ДеталиВыборка.НайтиСледующий(СтруктураОтбора) Цикл
					...
					ЗаписьJSON.ЗаписатьНачалоОбъекта();
						...
						РаботаСJSONСервер.ЗаписатьСвойство("qty_r1", Формат(ДеталиВыборка.КоличествоЗаказы, "ЧН=0; ЧГ="), ЗаписьJSON);
		...
		РаботаСJSONСервер.ЗаписатьСвойство("count_results", Формат(ОбщаяВыборка.Количество(), "ЧН=0; ЧГ="), ЗаписьJSON);
	...
	//	---------------------------------------------------------------------------------------
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
				...
				РаботаСJSONСервер.ЗаписатьСвойство("sku", Выборка.КодТовара, ЗаписьJSON);
				РаботаСJSONСервер.ЗаписатьСвойство("qty", Формат(Выборка.КоличествоДоступно, "ЧН=0; ЧГ="), ЗаписьJSON);
		...
		РаботаСJSONСервер.ЗаписатьСвойство("count_results", Формат(Выборка.Количество(), "ЧН=0; ЧГ="), ЗаписьJSON);
...
Функция FetchRest_СоздатьJSON(SKU, ТекстОшибки)
	...
	РаботаСJSONСервер.ЗаписатьСвойство("query", SKU, ЗаписьJSON);
	РаботаСJSONСервер.ЗаписатьСвойство("ErrorMessage", ТекстОшибки, ЗаписьJSON);
		...
		Попытка
			...
			//	ДокументОбъект = ПолучитьДокументMagento(ДанныеЗапроса.OrderData, "ЧекККМ", Истина, УчетнаяПолитика, СоставОшибки);
				...
				//	и вот переходим к расчёту скидок!
					...
					СтруктураПараметры.Вставить("ЗаказМадженто", Истина); 
                    //2025-07-10 sa включаем программу лояльности для сотрудников в расчет
...
//	так и обычным объектом.. например, при выгрузке. Учитываем такой факт.
	...
	Если НЕ СокращенныйВариант = Истина Тогда
		...
		РаботаСJSONСервер.ЗаписатьСвойство("Сompany", СокрЛП(РеквизитыШапки.КодПоЕДРПОУ), ЗаписьJSON);
...
Функция NameClassifier()
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
			...
			РаботаСJSONСервер.ЗаписатьСвойство("name_ru", Выборка.Наименование, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("name_ua", Выборка.НаименованиеУкраинское, ЗаписьJSON);
			РаботаСJSONСервер.ЗаписатьСвойство("gender" , Выборка.Пол, ЗаписьJSON);
...
//		update, close, cancel
	...
	СоставОшибки = ЗаказыПокупателейСервер.СоставОшибкиСтруктура();

	ДанныеИнтернетЗапроса = Новый Структура;
	ДанныеИнтернетЗапроса.Вставить("КомандаЗаказа", Command);
	ДанныеИнтернетЗапроса.Вставить("ДанныеЗапроса", ДанныеЗапроса);

	ЗаписьJSON = Новый ЗаписьJSON;			
	ЗаписьJSON.УстановитьСтроку();
	ЗаписатьJSON(ЗаписьJSON, ДанныеИнтернетЗапроса);			
	...
	Если ОшибкаЗаписиРегистра Тогда
		
		СоставОшибки.ОписаниеОшибки = "Ошибка записи заказа в регистр";
		СоставОшибки.Отказ = Истина;
		СоставОшибки.СтатусОшибки = "SYS_ERROR";

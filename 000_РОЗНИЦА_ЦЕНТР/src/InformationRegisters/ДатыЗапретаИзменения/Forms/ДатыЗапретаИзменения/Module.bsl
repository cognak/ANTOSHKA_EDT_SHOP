...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда	//	LNK 17.02.2021 05:49:35
...
Процедура ПользователиОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	Для каждого Значение Из ВыбранноеЗначение Цикл
		...
		Если ПользователиДатЗапрета.НайтиСтроки(Отбор).Количество() = 0 Тогда
			...
			ОписаниеПользователя.Представление = ТекстПредставленияПользователя(ЭтотОбъект, Отбор.Пользователь);
...
Процедура ДатыЗапретаПередНачаломИзменения(Элемент, Отказ)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Представление) Тогда
		ПрочитанныеСвойства = ЗаблокироватьЗаписьПользователя(ЭтотОбъект, ТекущийРаздел(), ТекущиеДанные.Объект, ТекущийПользователь);
...
Процедура ОбновитьДанныеПользователя()
	...
	Если ТекущийПользователь <> Неопределено И СпособУказанияЗначениеВСписке <> Неопределено Тогда
		...
		Если ТекущийСпособУказания <> СпособУказанияЗначениеВСписке.Значение Тогда
			ПоказатьПредупреждение(, ТекстСообщенияСпособУказанияНеИспользован(
...
Функция ЗаменитьОбъектВЗаписиПользователяНаСервере(Знач Раздел, Знач СтарыйОбъект, Знач НовыйОбъект, Знач Пользователь, ТекущиеЗначенияСвойств)
	...
	Если ЗначениеЗаполнено(СтарыйОбъект) Тогда
		...
		ПрочитанныеСвойства = ЗаблокироватьЗаписьПользователя(ЭтотОбъект, Раздел, СтарыйОбъект, Пользователь);
...
Функция ТекстВопросаОНесохраненныхДанных(БезПроверкиПользователей = Ложь)
	...
	Если НЕ ВсеПользователиВыбраны Тогда
		ТекстВопроса = ТекстВопроса + ТекстСообщенияНастройкиСНевыбраннымиПользователямиНеСохранены(ЭтотОбъект);
		//
	...
	Если УдалитьЛишнее Тогда
		...
		Попытка
			...
			Для каждого ЗначенияКлючаЗаписи Из ЗначенияКлючейЗаписей Цикл
				ЗаблокироватьЗаписьПользователя(ЭтотОбъект, ЗначенияКлючаЗаписи.Раздел, ЗначенияКлючаЗаписи.Объект, ЗначенияКлючаЗаписи.Пользователь);
			КонецЦикла;
...
Процедура РедактироватьДатуЗапретаВФорме()
	...
	Если ВыделенныеСтроки.Количество() > 0 Тогда
		Если ОбновитьВыделение Тогда
			...
			ПоказатьПредупреждение(, ТекстСообщенияСнятоВыделениеСНезаполненныхСтрок());
		КонецЕсли;
		// Блокировка записей выделенных строк
		Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			ТекущиеДанные = ДатыЗапрета.НайтиПоИдентификатору(ВыделеннаяСтрока);
			ПрочитанныеСвойства = ЗаблокироватьЗаписьПользователя(ЭтотОбъект, ТекущийРаздел(ТекущиеДанные), ТекущиеДанные.Объект, ТекущийПользователь);
			ОбновитьЗначенияПрочитанныхСвойств(ТекущиеДанные, ПрочитанныеСвойства, Элементы.Пользователи.ТекущиеДанные);
		КонецЦикла;
		...
		Иначе
			ПараметрыФормы.Вставить("ПользовательПредставление", ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
		КонецЕсли;
...
Функция ТекстСообщенияЗначениеДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение %1 не изменяется.'"), ТекстПредставленияДляВсехПользователей(ЭтотОбъект));
...
Функция ТекстСообщенияКомментарийДляВсехПользователейНеИзменяется()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Комментарий %1 не изменяется.'"), ТекстПредставленияДляВсехПользователей(ЭтотОбъект));

...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		...
		Если Не ПустаяСтрока(Запись.Серия) Тогда
			ТекстОшибки = "";
			ФизическиеЛицаКлиентСервер.СерияДокументаУказанаПравильно(Запись.ВидДокумента, Запись.Серия, ТекстОшибки);
		...
		Если Не ПустаяСтрока(Запись.Номер) Тогда
			ТекстОшибки = "";
			ФизическиеЛицаКлиентСервер.НомерДокументаУказанПравильно(Запись.ВидДокумента, Запись.Номер, ТекстОшибки);
			Если Не ПустаяСтрока(ТекстОшибки) Тогда
...
//	LNK 29.08.2018 13:25:43
Процедура ЗаполнитьПредставление()	Экспорт

...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если НЕ ОбменДанными.Загрузка Тогда	//	LNK 28.12.2023 06:28:57

		Для каждого ЗаписьНабора Из ЭтотОбъект Цикл

			ЗаписьНабора.ДатаОбновления	= ТекущаяДата();	//	LNK 28.12.2023 06:24:27
			ЗаписьНабора.ИмяОбъекта		= ЗаписьНабора.Объект.Метаданные().ПолноеИмя();

			Если НЕ ЗначениеЗаполнено(ЗаписьНабора.Автор) Тогда

				ЗаписьНабора.Автор = ПараметрыСеанса.ТекущийПользователь;	//	LNK 28.12.2023 06:27:07

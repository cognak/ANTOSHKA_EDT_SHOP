...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе	ПроверитьТолькоПросмотрУчтеноNavision();
...
&НаСервере	//	LNK 27.01.2023 06:17:08
Процедура ПроверитьТолькоПросмотрУчтеноNavision()

	Если НЕ (
		ТипЗнч(Запись.Объект) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары")
	ИЛИ	ТипЗнч(Запись.Объект) = Тип("ДокументСсылка.РасходныйОрдерНаТовары")
	)	Тогда
	...
	УчтеноОснование = РегистрыСведений.ОбъектУчтенВNavision.ПолучитьУчтеноNavisionДляОснования(Запись.Объект);

	Если УчтеноОснование И НЕ Запись.УчтеноNavision Тогда

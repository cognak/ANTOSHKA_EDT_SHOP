...
//  Массив - Получить оповещения пользователя
...
// 
// Параметры:
//  СписокОповещений - Массив - Список оповещений
...
//  ДействиеКоманда - Строка - Действие команда
//  Пользователь - СправочникСсылка.Пользователи,Неопределено - Пользователь
Процедура Установить(Объект, ДействиеКоманда, Пользователь = Неопределено)	Экспорт

	УстановитьПривилегированныйРежим(Истина);

	Если Пользователь = Неопределено
			И (ПараметрыСеанса.ТекущийПользователь = Справочники.Пользователи.ПустаяСсылка()
	...
	Иначе
		...
		ПользовательОповещения = ?(Пользователь = Неопределено, ПараметрыСеанса.ТекущийПользователь, Пользователь);
	...
	Попытка
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОповещенияПользователя");
		ЭлементБлокировки.УстановитьЗначение("Пользователь", ПользовательОповещения);
		...
		НаборЗаписей.Отбор.Пользователь.Установить(ПользовательОповещения);

...
Процедура ЗаписьСостояниеСтрокЗаказаПокупателя(СсылкаДокументаПроверки) Экспорт 
	...
	Если ТипЗнч(СсылкаДокументаПроверки) = Тип("ДокументСсылка.ЗапросДоступностиТоваров") Тогда
		...
		ДокументЗаказа = АтрибутыДокументаПроверки.ДокументОснование;
		
		АтрибутыДокументЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументЗаказа, "ТипОплаты");

		ДанныеДляЗаписи.СогласованоВNavision = 
			?((АтрибутыДокументЗаказа.ТипОплаты = Перечисления.ТипОплатыЗаказПокупателя.Наличные
					И АтрибутыДокументаПроверки.ТипДоставки = Перечисления.ТипДоставкиЗаказПокупателя.Самовывоз)
				,Истина
				,Неопределено);

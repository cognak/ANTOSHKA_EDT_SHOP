...
//  Булево - Есть продажи
Функция ЕстьПродажи(ДокументПроверки) Экспорт
...
//						И (ДанныеДляЗаписи.ДокументДляЗаписи.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.Сборка
//							Или ДанныеДляЗаписи.ДокументДляЗаписи.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.КОтправке) Тогда
//							
//					СтрокаЗаписи.ГотовКПродаже = Истина;
//					
...
//					СтрокаЗаписи.ГотовКПродаже = Истина;
//					
//				ИначеЕсли ДанныеДляЗаписи.ПополнениеМагазина
//						И ДанныеДляЗаписи.ДокументДляЗаписи.СтатусЗапроса = Перечисления.СтатусыЗапросовДоступностиТоваровШапка.ПродажаНаКассе Тогда 
//					
//					СтрокаЗаписи.ГотовКПродаже = Истина;
//					
//				КонецЕсли; 

				СатусЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаЗаписи.ЗаказПокупателя, "Статус");
				СтрокаЗаписи.ГотовКПродаже = Не (СатусЗаказа = Перечисления.СтатусыЗаказовПокупателей.Новый
								Или СатусЗаказа = Перечисления.СтатусыЗаказовПокупателей.ВРаботе
								Или СатусЗаказа = Перечисления.СтатусыЗаказовПокупателей.ЗапросДоступности);

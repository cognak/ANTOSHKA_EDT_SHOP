...
Процедура ПроверитьПодключениеWS(Команда)
	...
	Если Отказ Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Ошибка установки подключения.'"));
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Подключение успешно установлено.'"));
	КонецЕсли;
...
//	LNK 27.10.2016 08:51:28
&НаКлиенте
Процедура ЗаполнитьFTPЗначения(Команда)

//	некий ускоритель - ну очень долго для каждой ИБ заполнять!
//	Потом, когда конфа будет на поставке, поместить в скрытый модуль... пока так.
	Запись.FTPСоединениеПуть = "mail.antoshka.com.ua";
	Запись.FTPСоединениеПорт = 21;
	Запись.FTPСоединениеПользователь = "shop";
	Запись.FTPСоединениеПароль = "hB87P0cHXf2OON";

	Запись.FTPСжиматьФайлИсходящегоСообщения = Истина;
	Запись.ПарольАрхиваСообщенияОбмена = "Vbhjndjhtw2010";

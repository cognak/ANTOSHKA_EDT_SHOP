...
Процедура ПроверитьРегламентФормированиеСегментов()	Экспорт
	...
	Если НЕ ОбменДаннымиПовтИсп.ЭтоГлавныйУзел() Тогда
		...
		Если НЕ БылоВоздействие = НачалоДня(ТекущаяДатаСеанса())  Тогда
...
//	ВКЛ.
	...
	РегламентноеЗадание.Наименование  = "ФОРМИРОВАНИЕ СЕГМЕНТОВ (" + Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy") + ")";
	...
	РегламентноеЗадание.Расписание.ДатаНачала  = НачалоДня(ТекущаяДатаСеанса());
